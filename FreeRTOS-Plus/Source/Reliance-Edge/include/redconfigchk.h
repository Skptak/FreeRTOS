...
#define REDCONFIGCHK_H
    ...
    #if( RED_VERSION_VAL & 0xFFFFFF00U ) < \
        ( RED_CONFIG_MINCOMPAT_VER & 0xFFFFFF00U )
        /*  This indicates that your configuration files were generated by a
            ...
            for a more recent version of Reliance Edge and is no longer
           compatible with this version.  You can update to the most recent
           version of Reliance Edge or contact RelianceEdgeSupport@datalight.com
           to obtain the correct legacy version of the Configuration Utility.
        */
        #error \
    ...
    #error "Configuration error: REDCONF_API_FSE must be defined."
        ...
        #error \
            "Configuration error: REDCONF_API_POSIX_FTRUNCATE must be defined."
    #endif
        ...
        #error \
            "Configuration error: REDCONF_API_FSE_TRANSMASKSET must be defined."
    #endif
    #ifndef REDCONF_API_FSE_TRANSMASKGET
        #error \
            "Configuration error: REDCONF_API_FSE_TRANSMASKGET must be defined."
    #endif
#endif
    ...
    */
    #error \
        "Configuration error: your redconf.h is not compatible. Update your redconf files with a compatible version of the configuration utility."
#endif
    ...
    #error \
        "Configuration error: either REDCONF_API_FSE or REDCONF_API_POSIX must be set to 1."
#endif

#if REDCONF_API_POSIX == 1
    #if REDCONF_API_FSE != 0
        #error \
            "Configuration error: REDCONF_API_FSE must be 0 if REDCONF_API_POSIX is 1"
    #endif

    #if( REDCONF_API_POSIX_FORMAT != 0 ) && ( REDCONF_API_POSIX_FORMAT != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_FORMAT must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_UNLINK != 0 ) && ( REDCONF_API_POSIX_UNLINK != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_UNLINK must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_MKDIR != 0 ) && ( REDCONF_API_POSIX_MKDIR != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_MKDIR must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_RMDIR != 0 ) && ( REDCONF_API_POSIX_RMDIR != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_RMDIR must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_RENAME != 0 ) && ( REDCONF_API_POSIX_RENAME != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_RENAME must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_LINK != 0 ) && ( REDCONF_API_POSIX_LINK != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_LINK must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_FTRUNCATE != 0 ) && \
        ( REDCONF_API_POSIX_FTRUNCATE != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_FTRUNCATE must be either 0 or 1."
    #endif

    #if( REDCONF_API_POSIX_READDIR != 0 ) && ( REDCONF_API_POSIX_READDIR != 1 )
        #error \
            "Configuration error: REDCONF_API_POSIX_READDIR must be either 0 or 1."
    #endif

    #if( REDCONF_NAME_MAX < 1U ) || \
        ( REDCONF_NAME_MAX > ( REDCONF_BLOCK_SIZE - 4U ) )
        ...
        #error \
            "Configuration error: REDCONF_RENAME_ATOMIC must be either 0 or 1."
    #endif
        ...
        #error \
            "Configuration error: REDCONF_API_FSE_FORMAT must be either 0 or 1."
    #endif

    #if( REDCONF_API_FSE_TRUNCATE != 0 ) && ( REDCONF_API_FSE_TRUNCATE != 1 )
        #error \
            "Configuration error: REDCONF_API_FSE_TRUNCATE must be either 0 or 1."
    #endif

    #if( REDCONF_API_FSE_TRANSMASKSET != 0 ) && \
        ( REDCONF_API_FSE_TRANSMASKSET != 1 )
        #error \
            "Configuration error: REDCONF_API_FSE_TRANSMASKSET must be either 0 or 1."
    #endif

    #if( REDCONF_API_FSE_TRANSMASKGET != 0 ) && \
        ( REDCONF_API_FSE_TRANSMASKGET != 1 )
        #error \
            "Configuration error: REDCONF_API_FSE_TRANSMASKGET must be either 0 or 1."
    #endif
#endif
...
#if( REDCONF_ALIGNMENT_SIZE != 1U ) && ( REDCONF_ALIGNMENT_SIZE != 2U ) && \
    ( REDCONF_ALIGNMENT_SIZE != 4U ) && ( REDCONF_ALIGNMENT_SIZE != 8U )
 ...
 */
    ...
    #error \
        "Configuration error: REDCONF_INODE_TIMESTAMPS must be either 0 or 1."
#endif
    ...
    #error \
        "Configuration error: REDCONF_ATIME must be 0 when REDCONF_INODE_TIMESTAMPS is 0."
#endif
 ...
 */
    ...
    #error \
        "Configuration error: At least one of REDCONF_IMAP_INLINE and REDCONF_IMAP_EXTERNAL must be set"
#endif
 ...
 */
    ...
    #error \
        "REDCONF_DISCARDS not supported in Reliance Edge under GPL. Contact sales@datalight.com to upgrade."
#endif
