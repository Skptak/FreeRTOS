...
#else
    #error \
        "REDCONF_BLOCK_SIZE must be a power of two value between 256 and 65536"
#endif

#define REDMIN( a, b ) ( ( ( a ) < ( b ) ) ? ( a ) : ( b ) )

#define INODE_INVALID \
    ( 0U ) /* General-purpose invalid inode number (must be zero). */
#define INODE_FIRST_VALID ( 2U ) /* First valid inode number. */
#define INODE_ROOTDIR \
    ( INODE_FIRST_VALID ) /* Inode number of the root directory. */
