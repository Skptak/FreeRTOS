...
static int red_stat( const char * pszPath, REDSTAT * pStat );
    ...
    */
    ...
    #define rename( old, new ) \
        red_rename( MakeFullPath( old ), MakeFullPath( new ) )
    #define link( path, hardlink ) \
        red_link( MakeFullPath( path ), MakeFullPath( hardlink ) )
                                 ...
                                 const char ** ppszDevice )
{
    ...
    while(
        ( c = RedGetoptLong( argc, argv, "cl:n:rs:vD:H", aLongopts, NULL ) ) !=
        -1 )
    {
        switch( c )
        {
...
static void usage( const char * progname )
{
    ...
    RedPrintf( "Where:\n" );
    RedPrintf( "  VolumeID\n" );
    RedPrintf( "      A volume number (e.g., 2) or a volume path prefix (e.g., "
               "VOL1: or /data)\n" );
    ...
    RedPrintf( "      Specifies the number of times the entire test should "
               "loop.  Use 0 for\n" );
    ...
    RedPrintf( "  --namepad, -r\n" );
    RedPrintf( "      Specifies to use random name padding (resulting in "
               "longer names).\n" );
    RedPrintf( "  --seed=value, -s value\n" );
    RedPrintf( "      Specifies the seed for the random number generator "
               "(default timestamp).\n" );
    RedPrintf( "  --verbose, -v\n" );
    RedPrintf(
        "      Specifies verbose mode (without this, test is very quiet).\n" );
    RedPrintf( "  --dev=devname, -D devname\n" );
    RedPrintf( "      Specifies the device name.  This is typically only "
               "meaningful when\n" );
    RedPrintf( "      running the test on a host machine.  This can be \"ram\" "
               "to test on a RAM\n" );
    RedPrintf( "      disk, the path and name of a file disk (e.g., red.bin); "
               "or an OS-specific\n" );
    RedPrintf( "      reference to a device (on Windows, a drive letter like "
               "G: or a device name\n" );
    RedPrintf( "      like \\\\.\\PhysicalDrive7).\n" );
    RedPrintf( "  --help, -H\n" );
    RedPrintf( "      Prints this usage text and exits.\n\n" );
    RedPrintf( "Warning: This test will format the volume -- destroying all "
               "existing data.\n\n" );
}
...
static void creat_f( int opno, long r )
{
    ...
    if( !get_fname( FT_DIRm, r, NULL, NULL, &fep, &v1 ) )
        ...
        parid = fep->id;
    init_pathname( &f );
    type = rtpct ? ( ( int ) ( random() % 100 ) > rtpct ? FT_REG : FT_RTF )
                 : FT_REG;
