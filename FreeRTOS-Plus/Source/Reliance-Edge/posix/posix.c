...
-------------------------------------------------------------------*/
    ...
    */
     ...
     */
    #define RED_O_MASK                                              \
        ( RED_O_RDONLY | RED_O_WRONLY | RED_O_RDWR | RED_O_APPEND | \
          RED_O_CREAT | RED_O_EXCL | RED_O_TRUNC )
    ...
    if( ret == 0 )
    {
        ...
        */
        ...
        if( ret == 0 )
        {
             ...
             */
            for( uHandleIdx = 0U; uHandleIdx < REDCONF_HANDLE_COUNT;
                 uHandleIdx++ )
            {
                const REDHANDLE * pHandle = &gaHandle[ uHandleIdx ];

                if( ( pHandle->ulInode != INODE_INVALID ) &&
...
int32_t red_open( const char * pszPath, uint32_t ulOpenMode )
{
    ...
    if( ( ulOpenMode != ( ulOpenMode & RED_O_MASK ) ) ||
        ( ( ulOpenMode & ( RED_O_RDONLY | RED_O_WRONLY | RED_O_RDWR ) ) ==
          0U ) ||
        ...
        ( ( ( ulOpenMode & ( RED_O_TRUNC | RED_O_CREAT | RED_O_EXCL ) ) !=
            0U ) &&
          ( ( ulOpenMode & RED_O_RDONLY ) != 0U ) ) ||
        ( ( ( ulOpenMode & RED_O_EXCL ) != 0U ) &&
          ( ( ulOpenMode & RED_O_CREAT ) == 0U ) ) )
    {
        ret = -RED_EINVAL;
    }
    ...
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            ...
            if( ret == 0 )
            {
                ...
                ret = RedPathToName( pszOldLocalPath,
                                     &ulOldPInode,
                                     &pszOldName );
                ...
                if( ret == 0 )
                {
                    ...
                    ret = RedPathToName( pszNewLocalPath,
                                         &ulNewPInode,
                                         &pszNewName );
        ...
        if( ret == 0 )
        {
            ...
            if( ret == 0 )
            {
                ...
                if( ret == 0 )
                {
                    ...
                    ret = RedPathToName( pszLinkLocalPath,
                                         &ulLinkPInode,
                                         &pszLinkName );
...
int32_t red_read( int32_t iFildes, void * pBuffer, uint32_t ulLength )
{
    ...
    if( ret == 0 )
    {
        ...
        {
            ulLenRead = ulLength;
            ret = RedCoreFileRead( pHandle->ulInode,
                                   pHandle->ullOffset,
                                   &ulLenRead,
                                   pBuffer );
        }
...
int32_t red_write( int32_t iFildes, const void * pBuffer, uint32_t ulLength )
{
    ...
    if( ret == 0 )
    {
        ...
        {
            ulLenWrote = ulLength;
            ret = RedCoreFileWrite( pHandle->ulInode,
                                    pHandle->ullOffset,
                                    &ulLenWrote,
                                    pBuffer );
        }
...
int64_t red_lseek( int32_t iFildes, int64_t llOffset, REDWHENCE whence )
{
    ...
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            ...
            */
            if( ( llOffset > 0 ) &&
                ( ( ( uint64_t ) llFrom + ( uint64_t ) llOffset ) >
                  ( uint64_t ) INT64_MAX ) )
            {
                ret = -RED_EINVAL;
            }
            else
            {
                ...
                if( ( llNewOffset < 0 ) || ( ( uint64_t ) llNewOffset >
    ...
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            ...
            */
            ...
            ret = RedCoreDirRead( pDirStream->ulInode,
                                  &ulDirPosition,
                                  pDirStream->dirent.d_name,
            ...
            if( ret == 0 )
            {
                 ...
                 */
                ret = RedCoreStat( pDirStream->dirent.d_ino,
                                   &pDirStream->dirent.d_stat );
                if( ret == 0 )
                {
...
static REDSTATUS UnlinkSub( const char * pszPath, FTYPE type )
{
    ...
    #endif /* (REDCONF_API_POSIX_UNLINK == 1) || (REDCONF_API_POSIX_RMDIR == \
              1) */
                             ...
                             int32_t * piFildes )
{
    ...
    #if REDCONF_READ_ONLY == 0
        else if( gaRedVolume[ bVolNum ].fReadOnly &&
                 ( ulOpenMode != RED_O_RDONLY ) )
        {
            ret = -RED_EROFS;
        }
    #endif
            ...
            else
            {
    ...
    #endif
                {
                    ...
                    if( ( ulOpenMode & RED_O_CREAT ) != 0U )
                    {
                        ...
                        ret = RedPathToName( pszLocalPath,
                                             &ulPInode,
                                             &pszName );
                        if( ret == 0 )
                        {
                            ret = RedCoreCreate( ulPInode,
                                                 pszName,
                                                 false,
                                                 &ulInode );
                            if( ret == 0 )
                ...
                */
                if( !fCreated )
                {
                    ...
                    {
                        ret = RedCoreFileTruncate( ulInode,
                                                   UINT64_SUFFIX( 0 ) );
                    }
    #endif
                }
