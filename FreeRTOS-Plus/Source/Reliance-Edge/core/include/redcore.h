...
typedef struct
{
    ...
    */
...
#if DINDIR_POINTERS > 0U
    uint32_t ulDindirBlock; /**< Physical block number of the double indirect
                               node. */
#endif
#if REDCONF_DIRECT_POINTERS < INODE_ENTRIES
    ...
    uint16_t uInodeEntry; /**< Which inode entry to traverse to reach
                             ulLogicalBlock. */
#if DINDIR_POINTERS > 0U
    uint16_t uDindirEntry; /**< Which double indirect entry to traverse to reach
                              ulLogicalBlock. */
#endif
#if REDCONF_DIRECT_POINTERS < INODE_ENTRIES
    uint16_t uIndirEntry; /**< Which indirect entry to traverse to reach
                             ulLogicalBlock. */
#endif
} CINODE;

#define CINODE_IS_MOUNTED( pInode )                                      \
    ( ( ( pInode ) != NULL ) && INODE_IS_VALID( ( pInode )->ulInode ) && \
      ( ( pInode )->pInodeBuf != NULL ) )
#define CINODE_IS_DIRTY( pInode ) \
    ( CINODE_IS_MOUNTED( pInode ) && ( pInode )->fDirty )

#define IPUT_UPDATE_ATIME ( 0x01U )
#define IPUT_UPDATE_MTIME ( 0x02U )
#define IPUT_UPDATE_CTIME ( 0x04U )
#define IPUT_UPDATE_MASK \
    ( IPUT_UPDATE_ATIME | IPUT_UPDATE_MTIME | IPUT_UPDATE_CTIME )
