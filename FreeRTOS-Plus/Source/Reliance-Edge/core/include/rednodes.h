...
typedef struct
{
    uint32_t ulSignature; /**< Value which uniquely identifies the metadata node
                             type. */
    uint32_t ulCRC; /**< CRC-32 checksum of the node contents, starting after
                       the CRC. */
    uint64_t ullSequence; /**< Current sequence number at the time the node was
                             written to disk. */
} NODEHEADER;
...
/** @brief Node which identifies the volume and stores static volume
 * information.
 */
typedef struct
{
    ...
    char acBuildNum[ 8U ]; /**< Build number of the product (not null
                              terminated). */
    ...
    uint32_t ulBlockCount; /**< Compile-time configured number of logical
                              blocks. */
    uint16_t uMaxNameLen; /**< Compile-time configured maximum file name length.
                           */
    uint16_t uDirectPointers;   /**< Compile-time configured number of direct
                                   pointers per inode. */
    uint16_t uIndirectPointers; /**< Compile-time configured number of indirect
                                   pointers per inode. */
    uint8_t bBlockSizeP2; /**< Compile-time configured block size, expressed as
                             a power of two. */
    ...
    #define METAROOT_HEADER_SIZE                                          \
        ( NODEHEADER_SIZE + 16U ) /* Size in bytes of the metaroot header \
                                     fields. */
#else
    #define METAROOT_HEADER_SIZE                                          \
        ( NODEHEADER_SIZE + 12U ) /* Size in bytes of the metaroot header \
                                     fields. */
#endif
#define METAROOT_ENTRY_BYTES                                                   \
    ( REDCONF_BLOCK_SIZE - METAROOT_HEADER_SIZE ) /* Number of bytes remaining \
                                                     in the metaroot block for \
                                                     entries. */
...
typedef struct
{
    ...
    */
    ...
    #define IMAPNODE_HEADER_SIZE                                             \
        ( NODEHEADER_SIZE ) /* Size in bytes of the imap node header fields. \
                             */
    #define IMAPNODE_ENTRY_BYTES                                               \
        ( REDCONF_BLOCK_SIZE - IMAPNODE_HEADER_SIZE ) /* Number of bytes       \
...
#define INODE_HEADER_SIZE                                                  \
    ( NODEHEADER_SIZE + 8U + ( ( REDCONF_INODE_BLOCKS == 1 ) ? 4U : 0U ) + \
      ( ( REDCONF_INODE_TIMESTAMPS == 1 ) ? 12U : 0U ) + 4U +              \
      ( ( REDCONF_API_POSIX == 1 ) ? 4U : 0U ) )
#define INODE_ENTRIES ( ( REDCONF_BLOCK_SIZE - INODE_HEADER_SIZE ) / 4U )

#if( REDCONF_DIRECT_POINTERS < 0 ) || \
    ( REDCONF_DIRECT_POINTERS >       \
      ( INODE_ENTRIES - REDCONF_INDIRECT_POINTERS ) )
    #error "Configuration error: invalid value of REDCONF_DIRECT_POINTERS"
#endif
#if( REDCONF_INDIRECT_POINTERS < 0 ) || \
    ( REDCONF_INDIRECT_POINTERS >       \
      ( INODE_ENTRIES - REDCONF_DIRECT_POINTERS ) )
...
typedef struct
{
    ...
    uint32_t ulBlocks; /**< Total number file data blocks allocated to the
                          inode. */
#endif
#if REDCONF_INODE_TIMESTAMPS == 1
    uint32_t ulATime; /**< Time of last access (seconds since January 1, 1970).
                       */
    uint32_t ulMTime; /**< Time of last modification (seconds since January 1,
                         1970). */
    uint32_t ulCTime; /**< Time of last status change (seconds since January 1,
                         1970). */
#endif
...
#endif
#if REDCONF_API_POSIX == 1
    uint32_t ulPInode; /**< Parent inode number.  Only guaranteed to be accurate
                          for directories. */
#endif
