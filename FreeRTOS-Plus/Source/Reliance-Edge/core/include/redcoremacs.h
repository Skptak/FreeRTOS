...
#define BLOCK_NUM_FIRST_METAROOT \
    ( 1UL ) /* Block number of the first metaroot. */

#define BLOCK_SPARSE ( 0U )

#define DINDIR_POINTERS \
    ( ( INODE_ENTRIES - REDCONF_DIRECT_POINTERS ) - REDCONF_INDIRECT_POINTERS )
...
#define INODE_DATA_BLOCKS \
    ( REDCONF_DIRECT_POINTERS + INODE_INDIR_BLOCKS + INODE_DINDIR_BLOCKS )
#define INODE_SIZE_MAX \
    ( UINT64_SUFFIX( 1 ) * REDCONF_BLOCK_SIZE * INODE_DATA_BLOCKS )
...
#define INODE_IS_VALID( INODENUM )             \
    ( ( ( INODENUM ) >= INODE_FIRST_VALID ) && \
      ( ( INODENUM ) < ( INODE_FIRST_VALID + gpRedVolConf->ulInodeCount ) ) )
