...
REDSTATUS RedImapEBlockGet( uint8_t bMR, uint32_t ulBlock, bool * pfAllocated )
{
    ...
    else
    {
        ...
        */
        ...
        ret = RedBufferGet( RedImapNodeBlock( bMRToRead, ulImapNode ),
                            BFLAG_META_IMAP,
                            CAST_VOID_PTR_PTR( &pImap ) );

        if( ret == 0 )
        {
            *pfAllocated = RedBitGet( pImap->abEntries,
                                      ulOffset % IMAPNODE_ENTRIES );
...
static REDSTATUS ImapNodeBranch( uint32_t ulImapNode, IMAPNODE ** ppImap )
{
    ...
    else if( ImapNodeIsBranched( ulImapNode ) )
    {
         ...
         */
        ret = RedBufferGet( RedImapNodeBlock( gpRedCoreVol->bCurMR,
                                              ulImapNode ),
                            BFLAG_META_IMAP | BFLAG_DIRTY,
                            CAST_VOID_PTR_PTR( ppImap ) );
    }
    else
    {
...
static bool ImapNodeIsBranched( uint32_t ulImapNode )
{
    bool fNodeBitSetInMetaroot0 = RedBitGet( gpRedCoreVol->aMR[ 0U ].abEntries,
                                             ulImapNode );
    bool fNodeBitSetInMetaroot1 = RedBitGet( gpRedCoreVol->aMR[ 1U ].abEntries,
                                             ulImapNode );
