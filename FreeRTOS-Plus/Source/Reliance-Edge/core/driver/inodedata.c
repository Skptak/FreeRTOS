                            ...
                            void * pBuffer )
{
    ...
    if( !CINODE_IS_MOUNTED( pInode ) || ( pulLen == NULL ) ||
        ( pBuffer == NULL ) )
    {
        ret = -RED_EINVAL;
    }
    ...
    else
    {
        ...
        */
        ...
        {
            uint32_t ulBytesInFirstBlock = REDCONF_BLOCK_SIZE -
                                           ( uint32_t ) ( ullStart &
                                                          ( REDCONF_BLOCK_SIZE -
                                                            1U ) );
        ...
        {
            uint32_t ulBlockOffset = ( uint32_t ) ( ( ullStart +
                                                      ulReadIndex ) >>
                                                    BLOCK_SIZE_P2 );
            uint32_t ulBlockCount = ulRemaining >> BLOCK_SIZE_P2;

            REDASSERT( ( ( ullStart + ulReadIndex ) &
                         ( REDCONF_BLOCK_SIZE - 1U ) ) == 0U );

            ret = ReadAligned( pInode,
                               ulBlockOffset,
                               ulBlockCount,
                               &pbBuffer[ ulReadIndex ] );
        ...
        if( ( ret == 0 ) && ( ulRemaining > 0U ) )
        {
            ...
            ret = ReadUnaligned( pInode,
                                 ullStart + ulReadIndex,
                                 ulRemaining,
                                 &pbBuffer[ ulReadIndex ] );
        }
                             ...
                             const void * pBuffer )
{
    ...
    if( !CINODE_IS_DIRTY( pInode ) || ( pulLen == NULL ) ||
        ( pBuffer == NULL ) )
    {
        ret = -RED_EINVAL;
    }
    else if( ( ullStart > INODE_SIZE_MAX ) ||
             ( ( ullStart == INODE_SIZE_MAX ) && ( *pulLen > 0U ) ) )
    {
        ret = -RED_EFBIG;
    }
    else if( *pulLen == 0U )
    ...
    else
    {
        ...
        */
         ...
         */
        if( ( ret == 0 ) &&
            ( ( ( ullStart & ( REDCONF_BLOCK_SIZE - 1U ) ) != 0U ) ||
        ...
        if( ( ret == 0 ) && ( ulRemaining >= REDCONF_BLOCK_SIZE ) )
        {
            ...
            REDASSERT( ( ( ullStart + ulWriteIndex ) &
                         ( REDCONF_BLOCK_SIZE - 1U ) ) == 0U );

            ret = WriteAligned( pInode,
                                ulBlockOffset,
                                &ulBlocksWritten,
        ...
        if( ( ret == 0 ) && ( ulRemaining > 0U ) )
        {
            ...
            ret = WriteUnaligned( pInode,
                                  ullStart + ulWriteIndex,
                                  ulRemaining,
                                  &pbBuffer[ ulWriteIndex ] );
...
static REDSTATUS Shrink( CINODE * pInode, uint64_t ullSize )
{
    ...
    */
    ...
    {
        uint32_t ulTruncBlock = ( uint32_t ) ( ( ullSize + REDCONF_BLOCK_SIZE -
                                                 1U ) >>
                                               BLOCK_SIZE_P2 );
        ...
        {
            ret = TruncDataBlock( pInode,
                                  &pInode->pInodeBuf->aulEntries[ ulTruncBlock ],
                                  true );
        ...
        while( ( ret == 0 ) && ( ulTruncBlock < ( REDCONF_DIRECT_POINTERS +
                                                  INODE_INDIR_BLOCKS ) ) )
        {
            ...
            if( ( ret == 0 ) || ( ret == -RED_ENODATA ) )
            {
                ...
                if( ret == 0 )
                {
                    ...
                    {
                        pInode->pInodeBuf
                            ->aulEntries[ pInode->uInodeEntry ] = BLOCK_SPARSE;
                    }
            ...
            if( ( ret == 0 ) || ( ret == -RED_ENODATA ) )
            {
                ...
                */
                ...
                if( ret == 0 )
                {
                    ...
                    {
                        pInode->pInodeBuf
                            ->aulEntries[ uOrigInodeEntry ] = BLOCK_SPARSE;
                    }

                    /*  The next seek will go to the beginning of the next
                        double indirect.
                    */
                    ulTruncBlock += ( DINDIR_DATA_BLOCKS -
                                      ( uOrigDindirEntry * INDIR_ENTRIES ) ) -
                                    uOrigIndirEntry;
                }
            }
        }
...
static REDSTATUS TruncDindir( CINODE * pInode, bool * pfFreed )
{
    ...
    else
    {
        ...
        */
        for( uEntry = 0U; !fBranch && ( uEntry < pInode->uDindirEntry );
             uEntry++ )
        {
        ...
        */
        if( !fBranch && ( pInode->pDindir->aulEntries[ pInode->uDindirEntry ] !=
                          BLOCK_SPARSE ) )
        {
            for( uEntry = 0U; !fBranch && ( uEntry < pInode->uIndirEntry );
                 uEntry++ )
            {
        ...
        if( ret == 0 )
        {
            ...
            uint16_t uStart = pInode->uDindirEntry; /* pInode->uDindirEntry will
                                                       change. */
...
static REDSTATUS TruncIndir( CINODE * pInode, bool * pfFreed )
{
    ...
    else
    {
        ...
        */
        ...
        if( ret == 0 )
        {
            for( uEntry = pInode->uIndirEntry; uEntry < INDIR_ENTRIES;
                 uEntry++ )
            {
                ret = TruncDataBlock( pInode,
                                      &pInode->pIndir->aulEntries[ uEntry ],
                                      fBranch );
...
static REDSTATUS ExpandPrepare( CINODE * pInode )
{
    ...
    {
        uint32_t
            ulOldSizeByteInBlock = ( uint32_t ) ( pInode->pInodeBuf->ullSize &
                                                  ( REDCONF_BLOCK_SIZE - 1U ) );

        if( ulOldSizeByteInBlock != 0U )
        {
            ret = RedInodeDataSeek( pInode,
                                    ( uint32_t ) ( pInode->pInodeBuf->ullSize >>
                                                   BLOCK_SIZE_P2 ) );
            ...
            else if( ret == 0 )
            {
                ...
                if( ret == 0 )
                {
                    RedMemSet( &pInode->pbData[ ulOldSizeByteInBlock ],
                               0U,
...
REDSTATUS RedInodeDataSeekAndRead( CINODE * pInode, uint32_t ulBlock )
{
    ...
    if( ( ret == 0 ) && ( pInode->pbData == NULL ) )
    {
        ...
        ret = RedBufferGet( pInode->ulDataBlock,
                            0U,
    ...
    else
    {
        ...
        if( pInode->uDindirEntry != COORD_ENTRY_INVALID )
        {
            ...
            else
            {
                ...
                {
                    ret = RedBufferGet( pInode->ulDindirBlock,
                                        BFLAG_META_DINDIR,
                ...
                if( ret == 0 )
                {
                    pInode->ulIndirBlock = pInode->pDindir->aulEntries
                                               [ pInode->uDindirEntry ];
                }
            }
        }
        ...
        if( ( ret == 0 ) && ( pInode->uIndirEntry != COORD_ENTRY_INVALID ) )
        {
            ...
            else
            {
                ...
                {
                    ret = RedBufferGet( pInode->ulIndirBlock,
                                        BFLAG_META_INDIR,
                ...
                {
                    pInode
                        ->ulDataBlock = pInode->pIndir
                                            ->aulEntries[ pInode->uIndirEntry ];
                }
            }
        }
#endif
...
static void SeekCoord( CINODE * pInode, uint32_t ulBlock )
{
    ...
    else if( ( pInode->ulLogicalBlock != ulBlock ) || !pInode->fCoordInited )
    {
    ...
    #endif
        {
            ...
            pInode->ulDataBlock = pInode->pInodeBuf
                                      ->aulEntries[ pInode->uInodeEntry ];
    ...
    #endif
        {
            ...
            uint16_t uInodeEntry = ( uint16_t ) ( ( ulIndirRangeOffset /
                                                    INDIR_ENTRIES ) +
                                                  REDCONF_DIRECT_POINTERS );
            uint16_t uIndirEntry = ( uint16_t ) ( ulIndirRangeOffset %
                                                  INDIR_ENTRIES );
            ...
            */
            if( ( pInode->uInodeEntry != uInodeEntry ) ||
                !pInode->fCoordInited )
            {
                RedInodePutIndir( pInode );
                ...
                pInode->ulIndirBlock = pInode->pInodeBuf
                                           ->aulEntries[ pInode->uInodeEntry ];
            }
...
#if DINDIR_POINTERS > 0U
        {
            uint32_t ulDindirRangeOffset = ( ulBlock -
                                             REDCONF_DIRECT_POINTERS ) -
                                           INODE_INDIR_BLOCKS;
            uint16_t uInodeEntry = ( uint16_t ) ( ( ulDindirRangeOffset /
                                                    DINDIR_DATA_BLOCKS ) +
                                                  REDCONF_DIRECT_POINTERS +
                                                  REDCONF_INDIRECT_POINTERS );
            uint32_t ulDindirNodeOffset = ulDindirRangeOffset %
                                          DINDIR_DATA_BLOCKS;
            uint16_t uDindirEntry = ( uint16_t ) ( ulDindirNodeOffset /
                                                   INDIR_ENTRIES );
            uint16_t uIndirEntry = ( uint16_t ) ( ulDindirNodeOffset %
                                                  INDIR_ENTRIES );
            ...
            */
                ...
                !pInode->fCoordInited )
            {
                ...
                pInode->ulDindirBlock = pInode->pInodeBuf
                                            ->aulEntries[ pInode->uInodeEntry ];
            }
                                ...
                                uint8_t * pbBuffer )
{
     ...
     */
    if( ( ( ullStart >> BLOCK_SIZE_P2 ) !=
          ( ( ( ullStart + ulLen ) - 1U ) >> BLOCK_SIZE_P2 ) ) ||
        ( pbBuffer == NULL ) )
    {
        REDERROR();
        ret = -RED_EINVAL;
    ...
    {
        ret = RedInodeDataSeekAndRead( pInode,
                                       ( uint32_t ) ( ullStart >>
                                                      BLOCK_SIZE_P2 ) );
        ...
        {
            RedMemCpy( pbBuffer,
                       &pInode->pbData[ ullStart & ( REDCONF_BLOCK_SIZE - 1U ) ],
                       ulLen );
        }
                              ...
                              uint8_t * pbBuffer )
{
    ...
    else
    {
        ...
        while( ( ret == 0 ) && ( ulBlockIndex < ulBlockCount ) )
        {
            ...
            ret = GetExtent( pInode,
                             ulBlockStart + ulBlockIndex,
                             &ulExtentStart,
                             &ulExtentLen );
            ...
            else if( ret == -RED_ENODATA )
            {
                /*  Sparse block, return zeroed data.
                 */
                RedMemSet( &pbBuffer[ ulBlockIndex << BLOCK_SIZE_P2 ],
                           0U,
                           REDCONF_BLOCK_SIZE );
                ulBlockIndex++;
                ret = 0;
                                 ...
                                 const uint8_t * pbBuffer )
{
    ...
    {
        ret = RedInodeDataSeek( pInode,
                                ( uint32_t ) ( ullStart >> BLOCK_SIZE_P2 ) );
                               ...
                               const uint8_t * pbBuffer )
{
    ...
    else
    {
         ...
         */
        for( ulBlockIndex = 0U; ( ulBlockIndex < ulBlockCount ) && !fFull;
             ulBlockIndex++ )
        {
        ...
        if( ret == 0 )
        {
            ...
            while( ( ret == 0 ) && ( ulRunLen < ulExtentLen ) )
            {
                ...
                */
                if( ( ret == -RED_ENODATA ) ||
                    ( ( ret == 0 ) &&
                      ( pInode->ulDataBlock != ( ulFirstBlock + ulRunLen ) ) ) )
                {
                    ret = 0;
                    break;
                }
...
#if REDCONF_READ_ONLY == 0
/** @brief Allocate or branch the file metadata path and data block if
   necessary.
...
static REDSTATUS BranchBlock( CINODE * pInode, BRANCHDEPTH depth, bool fBuffer )
{
    ...
    if( ret == 0 )
    {
        ...
        if( pInode->uDindirEntry != COORD_ENTRY_INVALID )
        {
            ret = BranchOneBlock( &pInode->ulDindirBlock,
                                  CAST_VOID_PTR_PTR( &pInode->pDindir ),
                                  BFLAG_META_DINDIR );
            ...
            if( ret == 0 )
            {
                ...
                pInode->pInodeBuf
                    ->aulEntries[ pInode->uInodeEntry ] = pInode->ulDindirBlock;
            }
        }
    ...
    #if REDCONF_DIRECT_POINTERS < INODE_ENTRIES
        {
            if( ( pInode->uIndirEntry != COORD_ENTRY_INVALID ) &&
                ( depth >= BRANCHDEPTH_INDIR ) )
            {
                ret = BranchOneBlock( &pInode->ulIndirBlock,
                                      CAST_VOID_PTR_PTR( &pInode->pIndir ),
                                      BFLAG_META_INDIR );
                ...
                if( ret == 0 )
                {
                    ...
                    if( pInode->uDindirEntry != COORD_ENTRY_INVALID )
                    {
                        pInode->pDindir->aulEntries
                            [ pInode->uDindirEntry ] = pInode->ulIndirBlock;
                    }
                    else
        #endif
                    {
                        pInode->pInodeBuf->aulEntries
                            [ pInode->uInodeEntry ] = pInode->ulIndirBlock;
                    }
                }
            }
        }
    ...
    #endif
        {
            if( depth == BRANCHDEPTH_FILE_DATA )
            {
                ...
                if( ret == 0 )
                {
                    ...
                    if( pInode->uIndirEntry != COORD_ENTRY_INVALID )
                    {
                        pInode->pIndir->aulEntries
                            [ pInode->uIndirEntry ] = pInode->ulDataBlock;
                    }
                    else
    #endif
                    {
                        pInode->pInodeBuf->aulEntries
                            [ pInode->uInodeEntry ] = pInode->ulDataBlock;
                    }
                        ...
                        */
                        if( ppBuffer != NULL )
                        {
                            ...
                            else
                            {
                                ret = RedBufferGet(
                                    *pulBlock,
                                    ( uint16_t ) ( ( uint32_t ) uBFlag |
                                                   BFLAG_NEW | BFLAG_DIRTY ),
                                    ppBuffer );
                            }
                        }
                    }
                                  ...
                                  uint32_t * pulCost )
{
    ...
    if( !CINODE_IS_MOUNTED( pInode ) || !pInode->fCoordInited ||
        ( depth > BRANCHDEPTH_MAX ) || ( pulCost == NULL ) )
    {
        REDERROR();
        ret = -RED_EINVAL;
    }
