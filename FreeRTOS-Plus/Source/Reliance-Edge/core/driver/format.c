    ...
    if( ret == 0 )
    {
        ...
        */
        ...
        if( ( ret == 0 ) && !gpRedCoreVol->fImapInline )
        {
            ...
            uint32_t ulImapBlockLimit = gpRedCoreVol->ulImapStartBN +
                                        ( gpRedCoreVol->ulImapNodeCount * 2U );
            uint16_t uImapFlags = ( uint16_t ) ( ( uint32_t ) BFLAG_META_IMAP |
                                                 BFLAG_NEW | BFLAG_DIRTY );
            ...
            */
            for( ulImapBlock = gpRedCoreVol->ulImapStartBN;
                 ulImapBlock < ulImapBlockLimit;
                 ulImapBlock++ )
            {
                IMAPNODE * pImap;

                ret = RedBufferGet( ulImapBlock,
                                    uImapFlags,
        ...
        */
        if( ret == 0 )
        {
            ...
            for( ulInodeIdx = 0U; ulInodeIdx < gpRedVolConf->ulInodeCount;
                 ulInodeIdx++ )
            {
                CINODE ino;
        ...
        {
            ret = RedBufferGet( BLOCK_NUM_MASTER,
                                ( uint16_t ) ( ( uint32_t ) BFLAG_META_MASTER |
                                               BFLAG_NEW | BFLAG_DIRTY ),
                                CAST_VOID_PTR_PTR( &pMB ) );
        }
