...
REDSTATUS RedVolMountMaster( void )
{
    ...
    /*  Read the master block, to ensure that the disk was formatted with
        Reliance Edge.
    */
    ret = RedBufferGet( BLOCK_NUM_MASTER,
                        BFLAG_META_MASTER,
                        CAST_VOID_PTR_PTR( &pMB ) );
    ...
    if( ret == 0 )
    {
        ...
        */
        if( ( pMB->ulVersion != RED_DISK_LAYOUT_VERSION ) ||
            ...
            ( ( ( pMB->bFlags & MBFLAG_API_POSIX ) != 0U ) !=
              ( REDCONF_API_POSIX == 1 ) ) ||
            ( ( ( pMB->bFlags & MBFLAG_INODE_TIMESTAMPS ) != 0U ) !=
              ( REDCONF_INODE_TIMESTAMPS == 1 ) ) ||
            ( ( ( pMB->bFlags & MBFLAG_INODE_BLOCKS ) != 0U ) !=
              ( REDCONF_INODE_BLOCKS == 1 ) ) )
        {
            ret = -RED_EIO;
        }
...
REDSTATUS RedVolMountMetaroot( void )
{
    ...
    */
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            if( MetarootIsValid( &gpRedCoreVol->aMR[ 1U ],
                                 &fSectorCRCIsValid ) )
            {
#ifdef REDCONF_ENDIAN_SWAP
                ...
                if( ( bMR != 0U ) ||
                    ( gpRedCoreVol->aMR[ 1U ].hdr.ullSequence >
                      gpRedCoreVol->aMR[ 0U ].hdr.ullSequence ) )
                {
                    bMR = 1U;
                }
            }
...
static bool MetarootIsValid( METAROOT * pMR, bool * pfSectorCRCIsValid )
{
    ...
    else
    {
        ...
        */
        ...
        ulCRC = RedCrc32Update( 0U,
                                &pbMR[ 8U ],
                                gpRedVolConf->ulSectorSize - 8U );
        ...
        *pfSectorCRCIsValid = fRet;
        ...
        if( fRet )
        {
            ...
            {
                ulCRC = RedCrc32Update( ulCRC,
                                        &pbMR[ gpRedVolConf->ulSectorSize ],
                                        REDCONF_BLOCK_SIZE -
                                            gpRedVolConf->ulSectorSize );
            }
...
REDSTATUS RedVolTransact( void )
{
    ...
    if( gpRedCoreVol->fBranched )
    {
        ...
        if( ret == 0 )
        {
            ...
            ulSectorCRC = RedCrc32Update( 0U,
                                          &pbMR[ 8U ],
                                          gpRedVolConf->ulSectorSize - 8U );

            if( gpRedVolConf->ulSectorSize < REDCONF_BLOCK_SIZE )
            {
                gpRedMR->hdr.ulCRC = RedCrc32Update(
                    ulSectorCRC,
                    &pbMR[ gpRedVolConf->ulSectorSize ],
                    REDCONF_BLOCK_SIZE - gpRedVolConf->ulSectorSize );
            }
            else
            {
            ...
            /*  Flush the block device before writing the metaroot, so that all
                previously written blocks are guaranteed to be on the media
               before the metaroot is written.  Otherwise, if the block device
               reorders the writes, the metaroot could reach the media before
               metadata it points at, creating a window for disk corruption if
               power is lost.
            */
        ...
        if( ret == 0 )
        {
            ret = RedIoWrite( gbRedVolNum,
                              BLOCK_NUM_FIRST_METAROOT + gpRedCoreVol->bCurMR,
                              1U,
                              gpRedMR );
        ...
        /*  Flush the block device to force the metaroot write to the media.
           This guarantees the transaction point is really complete before we
           return.
        */
        if( ret == 0 )
        {
        ...
        /*  Toggle to the other metaroot buffer.  The working state and
           committed state metaroot buffers exchange places.
        */
        if( ret == 0 )
        {
...
void RedVolCriticalError( const char * pszFileName, uint32_t ulLineNum )
{
    ...
    {
        RedOsOutputString( "Critical file system error in Reliance Edge, "
