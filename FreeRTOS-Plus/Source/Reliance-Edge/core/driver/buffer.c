...
#define INODE_BUFFERS ( INODE_META_BUFFERS + 1U ) /* Add data buffer */
    ...
    #define MINIMUM_BUFFER_COUNT ( INODE_BUFFERS + IMAP_BUFFERS )
            ...
            /*  Two parent directories all the way down.  Source and destination
               inode buffer.  One inode buffer for cyclic rename detection.
               Imap.  The parent inode buffers are released before deleting the
               destination inode, so that does not increase the minimum.
            */
            #define MINIMUM_BUFFER_COUNT \
                ( INODE_BUFFERS + INODE_BUFFERS + 3U + IMAP_BUFFERS )
        #else
            /*  Two parent directories all the way down.  Source inode buffer.
               One inode buffer for cyclic rename detection.  Imap.
            */
            #define MINIMUM_BUFFER_COUNT \
                ( INODE_BUFFERS + INODE_BUFFERS + 2U + IMAP_BUFFERS )
        #endif
    #else
        /*  Link/create: Needs a parent inode all the way down, an extra inode
            buffer, and an imap buffer.

            Unlink is the same, since the parent inode buffers are released
           before the inode is deleted.
        */
...
#define BFLAG_META_MASK                                              \
    ( uint16_t )( ( uint32_t ) BFLAG_META_MASTER | BFLAG_META_IMAP | \
                  BFLAG_META_INODE | BFLAG_META_INDIR | BFLAG_META_DINDIR )
#define BFLAG_MASK \
    ( uint16_t )( ( uint32_t ) BFLAG_DIRTY | BFLAG_NEW | BFLAG_META_MASK )
...
typedef struct
{
    uint32_t ulBlock;  /**< Block number the buffer is associated with;
                          BBLK_INVALID if unused. */
...
void RedBufferInit( void )
{
    ...
    for( bIdx = 0U; bIdx < REDCONF_BUFFER_COUNT; bIdx++ )
    {
        ...
        */
        gBufCtx.abMRU[ bIdx ] = ( uint8_t ) ( ( REDCONF_BUFFER_COUNT - bIdx ) -
                                              1U );
...
REDSTATUS RedBufferGet( uint32_t ulBlock, uint16_t uFlags, void ** ppBuffer )
{
    ...
    if( ( ulBlock >= gpRedVolume->ulBlockCount ) ||
        ( ( uFlags & BFLAG_MASK ) != uFlags ) || ( ppBuffer == NULL ) )
    {
        REDERROR();
        ret = -RED_EINVAL;
    }
    else
    {
        if( BufferFind( ulBlock, &bIdx ) )
        {
            ...
            */
            if( ( ( uFlags & BFLAG_NEW ) != 0U ) ||
                ( ( uFlags & BFLAG_META_MASK ) !=
                  ( gBufCtx.aHead[ bIdx ].uFlags & BFLAG_META_MASK ) ) )
            {
                CRITICAL_ERROR();
                ret = -RED_EFUBAR;
            }
        ...
        else
        {
            ...
            /*  Search for the least recently used buffer which is not
                referenced.
            */
            for( bIdx = ( uint8_t ) ( REDCONF_BUFFER_COUNT - 1U ); bIdx > 0U;
                 bIdx-- )
            {
            ...
            if( ret == 0 )
            {
                if( ( uFlags & BFLAG_NEW ) == 0U )
                {
                    ...
                    */
                    ...
                    ret = RedIoRead( gbRedVolNum,
                                     ulBlock,
                                     1U,
                                     gBufCtx.b.aabBuffer[ bIdx ] );

                    if( ( ret == 0 ) && ( ( uFlags & BFLAG_META ) != 0U ) )
                    {
                        if( !BufferIsValid( gBufCtx.b.aabBuffer[ bIdx ],
                                            uFlags ) )
                        {
                            ...
                            */
                            CRITICAL_ASSERT( ( uFlags & BFLAG_META_MASTER ) ==
                                             BFLAG_META_MASTER );
                            ret = -RED_EIO;
                        }
                    }
                ...
                {
                    RedMemSet( gBufCtx.b.aabBuffer[ bIdx ],
                               0U,
                               REDCONF_BLOCK_SIZE );
                }
            }
...
static bool BufferIsValid( const uint8_t * pbBuffer, uint16_t uFlags )
{
    ...
    else
    {
        ...
        */
        RedMemCpy( &buf.ulSignature,
                   &pbBuffer[ NODEHEADER_OFFSET_SIG ],
                   sizeof( buf.ulSignature ) );
        RedMemCpy( &buf.ulCRC,
                   &pbBuffer[ NODEHEADER_OFFSET_CRC ],
                   sizeof( buf.ulCRC ) );
        RedMemCpy( &buf.ullSequence,
                   &pbBuffer[ NODEHEADER_OFFSET_SEQ ],
                   sizeof( buf.ullSequence ) );
        ...
        */
        ...
        if( fValid )
        {
            ...
            */
            ...
            else if( gpRedVolume->fMounted &&
                     ( buf.ullSequence >= gpRedVolume->ullSequence ) )
            {
                fValid = false;
            }
            else
...
static REDSTATUS BufferFinalize( uint8_t * pbBuffer, uint16_t uFlags )
{
    ...
    else
    {
        ...
        else
        {
            ...
            if( ret == 0 )
            {
                ...
                RedMemCpy( &pbBuffer[ NODEHEADER_OFFSET_SIG ],
                           &ulSignature,
                           sizeof( ulSignature ) );
                RedMemCpy( &pbBuffer[ NODEHEADER_OFFSET_SEQ ],
                           &ullSeqNum,
                           sizeof( ullSeqNum ) );
    ...
    #endif
                RedMemCpy( &pbBuffer[ NODEHEADER_OFFSET_CRC ],
                           &ulCrc,
                           sizeof( ulCrc ) );
            }
        }
...
/** @brief Swap the byte order of a metadata buffer
    ...
    @param uFlags   The associated buffer flags.  Used to determin the type of
                    metadata node.
*/
static void BufferEndianSwap( void * pBuffer, uint16_t uFlags )
{
...
static void BufferEndianSwapInode( INODE * pInode )
{
    ...
    else
    {
        ...
        {
            pInode->aulEntries[ ulIdx ] = RedRev32(
                pInode->aulEntries[ ulIdx ] );
        }
    }
}
...
static void BufferEndianSwapIndir( INDIR * pIndir )
{
    ...
    else
    {
        ...
        {
            pIndir->aulEntries[ ulIdx ] = RedRev32(
                pIndir->aulEntries[ ulIdx ] );
        }
    }
}
        ...
        */
        ...
        if( bMruIdx < ( REDCONF_BUFFER_COUNT - 1U ) )
        {
            /*  Move the buffer index to the back of the MRU array, making it
                the LRU buffer.
            */
            RedMemMove( &gBufCtx.abMRU[ bMruIdx ],
                        &gBufCtx.abMRU[ bMruIdx + 1U ],
