        ...
        #define VERIFY_EVENT_SIZE( i )                                       \
            ...
            {                                                                \
                xTraceDiagnosticsSetIfHigher(                                \
                    TRC_DIAGNOSTICS_BLOB_MAX_BYTES_TRUNCATED,                \
                                        ...
                                        TraceEventHandle_t * pxEventHandle )
{
    ...
    /* We add 1 since xTraceISRGetCurrentNesting(...) returns -1 if no ISR is
     * active */
    pxEventData = &pxTraceEventDataTable
                       ->coreEventData[ TRC_CFG_GET_CURRENT_CORE() ]
                       .eventData[ ISR_nesting + 1 ];

    /* This should never fail */
    TRC_ASSERT_CUSTOM_ON_FAIL( pxEventData->pvBlob == 0,
                               TRACE_EXIT_CRITICAL_SECTION();
                               return TRC_FAIL; );

    VERIFY_EVENT_SIZE( uiSize );

    pxEventData->size = ( ( uiSize + ( sizeof( uint32_t ) - 1 ) ) /
                          sizeof( uint32_t ) ) *
    ...
    if( xTraceStreamPortAllocate( pxEventData->size, &pxEventData->pvBlob ) ==
        TRC_FAIL )
    {
        TRACE_EXIT_CRITICAL_SECTION();
        return TRC_FAIL;
    ...
    TraceEventHandle_t * pxEventHandle )
{
    ...
    /* This can fail and we should handle it */
    while( xTraceStreamPortAllocate( pxEventData->size,
                                     &pxEventData->pvBlob ) != TRC_SUCCESS )
    {
        uiAttempts++;
    }
...
traceResult xTraceEventEndOfflineBlocking( TraceEventHandle_t xEventHandle )
{
    ...
    {
        iBytesCommitted = 0;
        xTraceStreamPortCommit( pxEventData->pvBlob,
                                pxEventData->size,
                                &iBytesCommitted );

        pxEventData->size -= iBytesCommitted;
        pxEventData->pvBlob = ( ( uint8_t * ) pxEventData->pvBlob ) +
                              iBytesCommitted;
    }
                                ...
                                uint32_t uiSize )
{
    ...
    /* This should never fail */
    TRC_ASSERT( ( ( TraceEventData_t * ) xEventHandle )->offset + uiSize <=
                ( ( TraceEventData_t * ) xEventHandle )->size );
...
traceResult xTraceEventGetSize( void * pvAddress, uint32_t * puiSize )
{
    ...
    /* This should never fail */
    TRC_ASSERT( ( sizeof( TraceBaseEvent_t ) +
                  ( TRC_EVENT_GET_PARAM_COUNT(
                      ( ( TraceBaseEvent_t * ) pvAddress )->EventID ) ) *
                      sizeof( uint32_t ) ) <= TRC_MAX_BLOB_SIZE );
                                   ...
                                   void * pvAddress )
{
    ...
    /* This should never fail */
    TRC_ASSERT( ( ( TraceEventData_t * ) xEventHandle )->offset +
                    sizeof( void * ) <=
                ( ( TraceEventData_t * ) xEventHandle )->size );
                                            ...
                                            TraceUnsignedBaseType_t uxValue )
{
    ...
    /* This should never fail */
    TRC_ASSERT( ( ( ( TraceEventData_t * ) xEventHandle )->offset &
                  ( sizeof( TraceUnsignedBaseType_t ) - 1 ) ) == 0 );
