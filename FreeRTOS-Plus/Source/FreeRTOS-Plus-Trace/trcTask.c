...
#include <trcRecorder.h>
         ...
         * "(startup)". This value was used since NULL/0 was already reserved
         * for the idle task. */
                                  ...
                                  TraceUnsignedBaseType_t uxPriority )
{
    ...
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntryGetAddress( ( TraceEntryHandle_t ) xTaskHandle, &pvTask ) ==
        TRC_SUCCESS );

    xTraceStackMonitorRemove( pvTask );

    return xTraceObjectUnregister( ( TraceObjectHandle_t ) xTaskHandle,
                                   PSF_EVENT_TASK_DELETE,
                                   uxPriority );
}
                                   ...
                                   TraceUnsignedBaseType_t uxPriority )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceObjectSetState( ( TraceObjectHandle_t ) xTaskHandle,
                              uxPriority ) == TRC_SUCCESS );
    ...
    if( xTraceEventBegin( PSF_EVENT_TASK_PRIORITY,
...
traceResult xTraceTaskSetPriorityWithoutHandle(
    void * pvTask,
    TraceUnsignedBaseType_t uxPriority )
{
    TraceEventHandle_t xEventHandle = 0;
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceObjectSetState( ( TraceObjectHandle_t ) xEntryHandle,
                              uxPriority ) == TRC_SUCCESS );
...
traceResult xTraceTaskReady( void * pvTask )
{
    ...
    if( xTraceEventBegin( PSF_EVENT_TASK_READY,
                          sizeof( void * ),
                          &xEventHandle ) == TRC_SUCCESS )
    {
        xTraceEventAddPointer( xEventHandle, pvTask );
