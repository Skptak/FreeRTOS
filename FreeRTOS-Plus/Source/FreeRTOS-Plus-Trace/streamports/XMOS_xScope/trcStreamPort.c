...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer )
{
    ...
    return xTraceInternalEventBufferInitialize(
        pxStreamPortXS->uiBufferInternal,
        sizeof( pxStreamPortXS->uiBufferInternal ) );
        #else
    return TRC_SUCCESS;
        #endif
}
                                       ...
                                       int32_t * piBytesWritten )
{
    ...
    if( piBytesWritten != 0 )
    {
        /* Since xScope always write all bytes (not all might be received) we
         * flag this as a full write */
