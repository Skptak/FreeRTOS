...
extern "C" {
        ...
        #define TRC_USE_INTERNAL_BUFFER \
        ...
        #define TRC_STREAM_PORT_RTT_DOWN_BUFFER_SIZE             \
            ( ( ( ( TRC_CFG_STREAM_PORT_RTT_DOWN_BUFFER_SIZE ) + \
                  sizeof( TraceUnsignedBaseType_t ) - 1 ) /      \
                sizeof( TraceUnsignedBaseType_t ) ) *            \
              sizeof( TraceUnsignedBaseType_t ) )
...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer );
         ...
         * implementation/configuration of the stream port this data might be
         * directly written to the stream port interface, buffered, or something
         * else.
         *
         ...
         * @retval TRC_SUCCESS Success
         */
        #if( defined( TRC_CFG_STREAM_PORT_RTT_NO_LOCK_WRITE ) && \
             TRC_CFG_STREAM_PORT_RTT_NO_LOCK_WRITE == 1 )
            #define xTraceStreamPortWriteData( pvData,                        \
                                               uiSize,                        \
                                               piBytesWritten )               \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                           \
                    *( piBytesWritten ) = ( int32_t ) SEGGER_RTT_WriteNoLock( \
                        ( TRC_CFG_STREAM_PORT_RTT_UP_BUFFER_INDEX ),          \
                        ( const char * ) pvData,                              \
                        uiSize ),                                             \
                    TRC_SUCCESS )
        #else
            #define xTraceStreamPortWriteData( pvData,                  \
                                               ...
                                               piBytesWritten )         \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                     \
                    *( piBytesWritten ) = ( int32_t ) SEGGER_RTT_Write( \
                        ( TRC_CFG_STREAM_PORT_RTT_UP_BUFFER_INDEX ),    \
                        ( const char * ) pvData,                        \
                        uiSize ),                                       \
                    TRC_SUCCESS )
        ...
        #define xTraceStreamPortReadData( pvData, uiSize, piBytesRead )  \
            ( ( SEGGER_RTT_HASDATA(                                      \
                  TRC_CFG_STREAM_PORT_RTT_DOWN_BUFFER_INDEX ) )          \
                  ? ( *( piBytesRead ) = ( int32_t ) SEGGER_RTT_Read(    \
                          ( TRC_CFG_STREAM_PORT_RTT_DOWN_BUFFER_INDEX ), \
                          ( char * ) ( pvData ),                         \
                          uiSize ),                                      \
                      TRC_SUCCESS )                                      \
