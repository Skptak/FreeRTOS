...
extern "C" {
        ...
        #define TRC_STREAM_PORT_BUFFER_SIZE                 \
            ( ( ( ( TRC_CFG_STREAM_PORT_BUFFER_SIZE ) +     \
...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer );
        ...
        #define xTraceStreamPortAllocate( uiSize, ppvData ) \
            ...
            /* Push to internal buffer. It will call on
             * xTraceStreamPortWriteData() periodically. */
        ...
        #define xTraceStreamPortWriteData( pvData, uiSize, piBytesWritten ) \
            ( prvTraceWriteToSocket( pvData, uiSize, piBytesWritten ) == 0  \
                  ? TRC_SUCCESS                                             \
                  : TRC_FAIL )

        #define xTraceStreamPortReadData( pvData, uiSize, piBytesRead )  \
            ( prvTraceReadFromSocket( pvData, uiSize, piBytesRead ) == 0 \
                  ? TRC_SUCCESS                                          \
                  : TRC_FAIL )
