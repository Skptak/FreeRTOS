...
#define xTraceStreamPortAllocate( uiSize, ppvData ) \
    ...
    /* Push to internal buffer. It will call on xTraceStreamPortWriteData()
     * periodically. */
...
#define xTraceStreamPortWriteData( pvData, uiSize, piBytesWritten )         \
    ( prvTraceTcpWrite( pvData, uiSize, piBytesWritten ) == 0 ? TRC_SUCCESS \
                                                              : TRC_FAIL )

#define xTraceStreamPortReadData( pvData, uiSize, piBytesRead )         \
    ( prvTraceTcpRead( pvData, uiSize, piBytesRead ) == 0 ? TRC_SUCCESS \
                                                          : TRC_FAIL )
