...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer )
{
    ...
    pxRingBuffer->xEventBuffer.uiSize = sizeof(
    ...
    if( xTraceMultiCoreEventBufferInitialize(
            &pxStreamPortData->xMultiCoreEventBuffer,
    ...
    if( xTraceEntryTableInitialize( &pxRingBuffer->xEntryTableBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }
                                    ...
                                    int32_t * piBytesCommitted )
{
    ...
    xTraceMultiCoreEventBufferPush( &pxStreamPortData->xMultiCoreEventBuffer,
                                    pvData,
                                    uiSize,
                                    piBytesCommitted );
    ...
    /* If no bytes was written it means that the buffer is full and we should
     * stop tracing.
     */
...
{
    return xTraceMultiCoreEventBufferClear(
        &pxStreamPortData->xMultiCoreEventBuffer );
}
