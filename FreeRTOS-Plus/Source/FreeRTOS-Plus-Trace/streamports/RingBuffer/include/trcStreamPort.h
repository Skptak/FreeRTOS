...
extern "C" {
         ...
         *
         * @brief This Stream Port requires additional information to be sent
        ...
        #define TRC_STREAM_PORT_BUFFER_SIZE                                  \
            ( ( ( ( TRC_CFG_STREAM_PORT_BUFFER_SIZE ) + sizeof( uint32_t ) - \
                  1 ) /                                                      \
                sizeof( uint32_t ) ) *                                       \
              sizeof( uint32_t ) )
...
extern TraceStreamPortData_t * pxStreamPortData;
         ...
         */
        #define TRC_STREAM_PORT_DATA_BUFFER_SIZE \
            ( sizeof( TraceStreamPortData_t ) )
...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer );
         ...
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStreamPortAllocate( uiSize, ppvData ) \
            ( ( void ) uiSize, xTraceStaticBufferGet( ppvData ) )

/**
 * @brief Commits data to the stream port, depending on the
 * implementation/configuration of the stream port this data might be directly
 * written to the stream port interface, buffered, or something else.
 *
...
traceResult xTraceStreamPortCommit( void * pvData,
        ...
        #define xTraceStreamPortWriteData( pvData, uiSize, piBytesWritten ) \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_4( ( void ) pvData,            \
                                                ( void ) uiSize,            \
                                                ( void ) piBytesWritten,    \
                                                TRC_SUCCESS )
         ...
         *
         * @param[in] uiStartOption Start option used when enabling trace
         * recorder
         *
         * @retval TRC_FAIL Read failed
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStreamPortOnEnable( uiStartOption )                   \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2( ( void ) ( uiStartOption ), \
                                                TRC_SUCCESS )
         ...
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStreamPortOnDisable() \
...
traceResult xTraceStreamPortOnTraceBegin();
         ...
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStreamPortOnTraceEnd() \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_1( TRC_SUCCESS )
