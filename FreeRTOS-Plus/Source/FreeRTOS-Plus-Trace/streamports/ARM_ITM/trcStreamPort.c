...
volatile char tz_host_command_data[ 32 ];
    ...
    /* These variables are used for reading commands from the host, using
     * read_from_host(). This is not required if using vTraceEnable(TRC_START).
...
/* This reads "command" data from a RAM buffer, written by a host macro in the
 * debugger */
