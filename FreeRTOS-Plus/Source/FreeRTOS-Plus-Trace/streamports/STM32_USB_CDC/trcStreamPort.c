                                ...
                                int32_t * piBytesReceived )
{
    ...
    {
        if( ( TraceUnsignedBaseType_t ) uiSize >=
            pxUSBBuffers->idx ) // More than what is stored, number of bytes
                                // will be .idx
        {
        ...
        else // If some data in the buffer is not read
        {
            ...
            for( i = 0; i < uiDiff; i++ )
            {
                pxUSBBuffers->bufferUSB[ i ] = pxUSBBuffers
                                                   ->bufferUSB[ i + uiSize ];
            }
                                 ...
                                 int32_t * piBytesSent )
{
    ...
    *piBytesSent = 0;
    ...
    else
    {
        ...
        /* We keep trying to send more pvData. If busy, we delay for a while.
         * This function will be called again afterwards. */
...
traceResult xTraceStreamPortInitialize( TraceStreamPortBuffer_t * pxBuffer )
{
    ...
    return xTraceInternalEventBufferInitialize(
        pxUSBBuffers->bufferInternal,
        sizeof( pxUSBBuffers->bufferInternal ) );
}
