...
#include <trcRecorder.h>
        ...
        #if( ( ( TRC_CFG_ENABLE_STACK_MONITOR ) == 1 ) && \
...
traceResult xTraceStackMonitorAdd( void * pvTask )
{
    ...
    if( xTraceKernelPortGetUnusedStack( pvTask, &uxLowMark ) == TRC_SUCCESS )
    {
        ...
        pxStackMonitor->xEntries[ pxStackMonitor->uiEntryCount ]
            .uxPreviousLowWaterMark = uxLowMark;
...
traceResult xTraceStackMonitorRemove( void * pvTask )
{
    ...
    for( i = 0; i < pxStackMonitor->uiEntryCount; i++ )
    {
        ...
        {
            if( pxStackMonitor->uiEntryCount > 1 &&
                i != ( pxStackMonitor->uiEntryCount - 1 ) )
            {
                /* There are more entries and this is NOT the last entry. Move
                 * last entry to this slot. */
                pxStackMonitor->xEntries[ i ]
                    .pvTask = pxStackMonitor
                                  ->xEntries[ pxStackMonitor->uiEntryCount - 1 ]
                                  .pvTask;
                pxStackMonitor->xEntries[ i ].uxPreviousLowWaterMark =
                    pxStackMonitor->xEntries[ pxStackMonitor->uiEntryCount - 1 ]
                        .uxPreviousLowWaterMark;

                /* Clear old entry that was moved */
                pxStackMonitor->xEntries[ pxStackMonitor->uiEntryCount - 1 ]
                    .pvTask = 0;
                pxStackMonitor->xEntries[ pxStackMonitor->uiEntryCount - 1 ]
                    .uxPreviousLowWaterMark = 0;
            }
            else
            {
...
traceResult xTraceStackMonitorGetAtIndex(
    uint32_t uiIndex,
    void ** ppvTask,
    TraceUnsignedBaseType_t * puxLowWaterMark )
{
    ...
    *puxLowWaterMark = pxStackMonitor->xEntries[ uiIndex ]
                           .uxPreviousLowWaterMark;
...
traceResult xTraceStackMonitorReport( void )
{
    ...
    uiToReport = TRC_CFG_STACK_MONITOR_MAX_REPORTS <=
                         pxStackMonitor->uiEntryCount
    ...
    for( i = 0; i < uiToReport; i++ )
    {
        ...
        uiCurrentIndex = uiCurrentIndex < pxStackMonitor->uiEntryCount
                             ? uiCurrentIndex
                             : 0;

        pxStackMonitorEntry = &pxStackMonitor->xEntries[ uiCurrentIndex ];

        xTraceKernelPortGetUnusedStack( pxStackMonitorEntry->pvTask,
                                        &uxLowWaterMark );
        ...
        if( xTraceEventBegin( PSF_EVENT_UNUSED_STACK,
                              ...
                              &xEventHandle ) == TRC_SUCCESS )
        {
            ...
            xTraceEventAdd32( xEventHandle,
                              ( uint32_t )
                                  pxStackMonitorEntry->uxPreviousLowWaterMark );
    ...
    return TRC_SUCCESS;
        ...
        #endif /* (((TRC_CFG_ENABLE_STACK_MONITOR) == 1) && \
                  ((TRC_CFG_SCHEDULING_ONLY) == 0)) */
