 ...
 */
static traceResult prvTraceEventBufferPop(
    TraceEventBuffer_t * pxTraceEventBuffer )
{
    ...
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEventGetSize(
            ( ( void * ) &(
                pxTraceEventBuffer->puiBuffer[ pxTraceEventBuffer->uiTail ] ) ),
            &uiFreeSize ) == TRC_SUCCESS );

    pxTraceEventBuffer->uiFree += uiFreeSize;

    /* Update tail to point to the new last event */
    pxTraceEventBuffer->uiTail = ( pxTraceEventBuffer->uiTail + uiFreeSize ) %
                                 pxTraceEventBuffer->uiSize;
                                   ...
                                   int32_t * piBytesWritten )
{
    ...
    *piBytesWritten = 0;
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE( xTraceTimestampGetWraparounds(
                                    &pxTraceEventBuffer->uiTimerWraparounds ) ==
                                TRC_SUCCESS );

    /* In ring buffer mode we cannot provide lock free access since the producer
     * modified the head and tail variables in the same call. This option is
     * only safe when used with an internal buffer (streaming snapshot) which no
     * consumer accesses.
     */
    switch( pxTraceEventBuffer->uiOptions )
    {
        case TRC_EVENT_BUFFER_OPTION_OVERWRITE:
        {
            ...
            /* If there isn't enough space in the buffer pop events until there
             * is */
            ...
            {
                TRC_MEMCPY( &pxTraceEventBuffer->puiBuffer[ uiHead ],
                            pxData,
                            uiDataSize );
            }
            else
        ...
        {
            /* Since a consumer could potentially update tail (free) during the
             * procedure we have to save it here to avoid problems with the push
             * algorithm.
             */
            uint32_t uiHead = pxTraceEventBuffer->uiHead;
            ...
            if( uiHead >= uiTail )
            {
                uint32_t uiFreeSpace = ( uiBufferSize - uiHead -
                                         sizeof( uint32_t ) ) +
                                       uiTail;
                ...
                if( ( uiBufferSize - uiHead ) > uiDataSize )
                {
                    TRC_MEMCPY( &pxTraceEventBuffer
                                     ->puiBuffer[ pxTraceEventBuffer->uiHead ],
                                pxData,
                                uiDataSize );
                }
                else
                                       ...
                                       int32_t * piBytesWritten )
{
    ...
    if( uiHead > uiTail )
    {
        xTraceStreamPortWriteData( &pxTraceEventBuffer->puiBuffer[ uiTail ],
                                   ( uiHead - uiTail ),
                                   &iBytesWritten );
    ...
    {
        xTraceStreamPortWriteData( &pxTraceEventBuffer->puiBuffer[ uiTail ],
                                   ( pxTraceEventBuffer->uiSize - uiTail ),
                                   &iBytesWritten );

        iSumBytesWritten += iBytesWritten;

        xTraceStreamPortWriteData( pxTraceEventBuffer->puiBuffer,
                                   uiHead,
                                   &iBytesWritten );
