...
static TraceHeader_t * pxHeader;
         ...
         * This will avoid issues where the recorder must be started before
         *main(), which can lead to RecorderInitialized be cleared by late
         *initialization after xTraceEnable(TRC_INIT) was called and assigned
         *RecorderInitialized its' value.
         ******************************************************************************/
...
traceResult xTraceInitialize( void )
{
    ...
    pxTraceRecorderData->uiTraceSystemState = TRC_STATE_IN_STARTUP;
        ...
        #if( TRC_EXTERNAL_BUFFERS == 0 )
    if( xTraceHeaderInitialize( &pxTraceRecorderData->xHeaderBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceEntryTableInitialize( &pxTraceRecorderData->xEntryTableBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceTimestampInitialize( &pxTraceRecorderData->xTimestampBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }
    ...
    if( xTraceStackMonitorInitialize(
            &pxTraceRecorderData->xStackMonitorBuffer ) == TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceStreamPortInitialize( &pxTraceRecorderData->xStreamPortBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceAssertInitialize( &pxTraceRecorderData->xAssertBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceDiagnosticsInitialize(
            &pxTraceRecorderData->xDiagnosticsBuffer ) == TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceStaticBufferInitialize(
            &pxTraceRecorderData->xStaticBufferBuffer ) == TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceEventInitialize( &pxTraceRecorderData->xEventDataBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTracePrintInitialize( &pxTraceRecorderData->xPrintBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceErrorInitialize( &pxTraceRecorderData->xErrorBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceISRInitialize( &pxTraceRecorderData->xISRInfoBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceTaskInitialize( &pxTraceRecorderData->xTaskInfoBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }

    if( xTraceKernelPortInitialize( &pxTraceRecorderData->xKernelPortBuffer ) ==
        TRC_FAIL )
    {
        return TRC_FAIL;
    }
...
traceResult xTraceEnable( uint32_t uiStartOption )
{
    ...
    {
        /* We keep trying to read commands from host until the recorder has been
         * started */
        do
        {
            iBytes = 0;

            if( xTraceStreamPortReadData( &xCommand,
                                          sizeof( TraceCommand_t ),
                                          ( int32_t * ) &iBytes ) == TRC_FAIL )
            {
            ...
            if( iBytes == sizeof( TraceCommand_t ) )
            {
                ...
                {
                    if( xCommand.cmdCode == CMD_SET_ACTIVE &&
                        xCommand.param1 == 1 )
                    {
    ...
    {
        /* We start streaming directly - this assumes that the host interface is
         * ready! */
        TRC_PORT_SPECIFIC_INIT();
    ...
    {
        /* We prepare the system to receive commands from host, but let system
         * resume execution until that happens */
        TRC_PORT_SPECIFIC_INIT();
    }
...
static void prvSetRecorderEnabled( void )
{
    ...
    /* If not overridden using 	xTraceTimestampSetFrequency(...), use default
     * value */
...
static void prvTraceStoreHeader( void )
{
    ...
    if( xTraceEventBeginRawOfflineBlocking( sizeof( TraceHeader_t ),
                                            &xEventHandle ) == TRC_SUCCESS )
    {
...
static void prvTraceStoreTimestampInfo( void )
{
    ...
    if( xTraceEventBeginRawOfflineBlocking( sizeof( TraceTimestampBuffer_t ),
                                            &xEventHandle ) == TRC_SUCCESS )
    {
        xTraceEventAddData( xEventHandle,
                            &pxTraceRecorderData->xTimestampBuffer,
                            sizeof( TraceTimestampBuffer_t ) );
        xTraceEventEndOfflineBlocking( xEventHandle );
    }
}
...
static void prvTraceStoreEntryTable( void )
{
    ...
    for( i = 0; i < ( TRC_ENTRY_TABLE_SLOTS ); i++ )
    {
        ...
        {
            /* Send entry */
            if( xTraceEventBeginRawOfflineBlocking( sizeof( TraceEntry_t ),
                                                    &xEventHandle ) ==
                TRC_SUCCESS )
            {
                xTraceEventAddData( xEventHandle,
...
static void prvTraceStoreStartEvent()
{
    ...
    if( xTraceEventBeginOffline( PSF_EVENT_TRACE_START,
                                 sizeof( uint32_t ),
                                 &xEventHandle ) == TRC_SUCCESS )
    {
...
static int prvIsValidCommand( TraceCommand_t * cmd )
{
    uint16_t checksum = ( uint16_t ) ( 0xFFFF - ( cmd->cmdCode + cmd->param1 +
