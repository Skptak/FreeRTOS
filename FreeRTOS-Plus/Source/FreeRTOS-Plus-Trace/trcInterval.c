                                  ...
                                  TraceIntervalHandle_t * pxIntervalHandle )
{
    ...
    if( xTraceObjectRegister( PSF_EVENT_INTERVAL_CREATE,
                              0,
                              szName,
                              0,
    ...
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetOptions( ( TraceEntryHandle_t ) xObjectHandle,
                               TRC_ENTRY_OPTION_INTERVAL ) == TRC_SUCCESS );
...
traceResult xTraceIntervalStart( TraceIntervalHandle_t xIntervalHandle )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetState( ( TraceEntryHandle_t ) xIntervalHandle,
                             TRC_INTERVAL_STATE_INDEX,
                             1 ) == TRC_SUCCESS );

    /* We need to check this */
    if( xTraceEventBegin( PSF_EVENT_INTERVAL_STATECHANGE,
...
{
    return xTraceEntryGetState( ( TraceEntryHandle_t ) xIntervalHandle,
                                TRC_INTERVAL_STATE_INDEX,
                                puxState );
}
