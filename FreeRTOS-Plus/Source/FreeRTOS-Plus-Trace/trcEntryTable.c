...
#include <trcRecorder.h>
        ...
        #define VALIDATE_ENTRY_HANDLE( xEntryHandle )             \
            ( ( ( ( TraceUnsignedBaseType_t ) ( xEntryHandle ) >= \
                  ( TraceUnsignedBaseType_t ) pxEntryTable ) &&   \
                ( ( TraceUnsignedBaseType_t ) ( xEntryHandle ) <  \
                  ( ( TraceUnsignedBaseType_t ) pxEntryTable +    \
                    sizeof( TraceEntryTable_t ) ) ) ) )

        #define GIVE_ENTRY_INDEX( xIndex )                                   \
            xIndexTable                                                      \
                .axFreeIndexes[ xIndexTable.uiFreeIndexCount ] = ( xIndex ); \
            xIndexTable.uiFreeIndexCount++

        #define GET_FREE_INDEX_COUNT() xIndexTable.uiFreeIndexCount

        #define CALCULATE_ENTRY_INDEX( xEntryHandle )                                        \
            ( TraceEntryIndex_t )( (                                                         \
                ( TraceUnsignedBaseType_t ) ( ( TraceUnsignedBaseType_t ) ( xEntryHandle ) - \
                             ...
                             TraceEntryHandle_t * pxEntryHandle )
{
    ...
    for( i = 0; i < TRC_ENTRY_TABLE_SLOTS; i++ )
    {
        pxEntry = ( TraceEntry_t * ) ( ( ( uint32_t ) pxEntryTable->axEntries ) +
                                       ( i * sizeof( TraceEntry_t ) ) );
                                  ...
                                  const char * szSymbol )
{
    ...
    for( i = 0; i < ( TRC_ENTRY_TABLE_SYMBOL_LENGTH ); i++ )
    {
        ( ( TraceEntry_t * ) xEntryHandle )
            ->szSymbol[ i ] = szSymbol[ i ]; /* We do this first to ensure we
                                                also get the 0 termination, if
                                                there is one */
    ...
    xTraceDiagnosticsSetIfHigher( TRC_DIAGNOSTICS_ENTRY_SYMBOL_LONGEST_LENGTH,
                                  i );
                                   ...
                                   TraceEntryHandle_t * pxEntryHandle )
{
    ...
    *pxEntryHandle = ( TraceEntryHandle_t ) ( ( uint32_t ) ( pxEntryTable
                                                                 ->axEntries ) +
                                              ( index *
                                                sizeof( TraceEntry_t ) ) );
