...
#include <trcRecorder.h>
         ...
         */
        #define GET_ERROR_WARNING_FLAG( errorCode ) \
            ( pxErrorInfo->uiErrorAndWarningFlags & \
              ( 1 << ( ( errorCode ) -1 ) ) )
        #define SET_ERROR_WARNING_FLAG( errorCode ) \
            ( pxErrorInfo                           \
                  ->uiErrorAndWarningFlags |= ( 1 << ( ( errorCode ) -1 ) ) )
...
traceResult xTraceWarning( uint32_t uiErrorCode )
{
    ...
    if( GET_ERROR_WARNING_FLAG( uiErrorCode ) == 0 )
    {
         ...
         * verify if uiErrorAndWarningFlags[uiErrorCode] has already been set */
...
traceResult xTraceError( uint32_t uiErrorCode )
{
    ...
    if( pxErrorInfo->uiErrorCode == TRC_ERROR_NONE )
    {
         ...
         * uiErrorCode has already been set */
        SET_ERROR_WARNING_FLAG( uiErrorCode );
        ...
        xTracePrint( pxErrorInfo->xWarningChannel,
                     "Recorder stopped in xTraceError(...)!" );
        xTraceDisable();
    }
...
traceResult prvTraceErrorPrint( uint32_t uiErrorCode )
{
    ...
    if( pxErrorInfo->xWarningChannel == 0 )
    {
         ...
         */
        if( xTraceStringRegister( "#WFR", &pxErrorInfo->xWarningChannel ) ==
            TRC_FAIL )
        {
            return TRC_FAIL;
        }
    ...
    switch( uiErrorCode )
    {
        ...
        case TRC_ERROR_ASSERT:
            /* A TRC_ASSERT has triggered */
            {
                ...
                xTracePrintF( pxErrorInfo->xWarningChannel,
                              szDesc,
                              xFileName,
                              ( uint32_t ) uxLineNumber );
                                         ...
                                         const char ** pszDesc )
{
    ...
    switch( uiErrorCode )
    {
        ...
        case TRC_WARNING_ENTRY_TABLE_SLOTS:
            ...
            this variable and increase TRC_CFG_ENTRY_TABLE_SLOTS by at least
            that value. */
        ...
        case TRC_WARNING_STREAM_PORT_INITIAL_BLOCKING:
            /* Blocking occurred during xTraceEnable. This happens if the trace
        ...
        case TRC_WARNING_STACKMON_NO_SLOTS:
            /* Some tasks did not fit in the stack monitor. Increase the slot
             * count. */
        ...
        case TRC_ERROR_ISR_NESTING_OVERFLOW:
            ...
            call vTraceStoreISRExit in the end of all ISR handlers. Or increase
        ...
        case TRC_ERROR_TZCTRLTASK_NOT_CREATED:
            /* xTraceEnable failed creating the trace control task (TzCtrl) -
