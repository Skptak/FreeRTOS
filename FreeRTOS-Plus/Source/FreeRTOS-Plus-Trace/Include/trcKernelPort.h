...
/* Added mainly for our internal testing. This makes it easier to create test
   applications that runs on multiple FreeRTOS versions. */
    ...
    #define INCLUDE_xTaskGetCurrentTaskHandle 1
        ...
        #define TRC_KERNEL_PORT_BUFFER_SIZE \
            ( sizeof( TraceHeapHandle_t ) + sizeof( void * ) )
...
unsigned char xTraceKernelPortIsSchedulerSuspended( void );
     ...
     */
    #define TRC_KERNEL_PORT_SET_INTERRUPT_MASK() \
        ( ( TraceBaseType_t ) portSET_INTERRUPT_MASK_FROM_ISR() )
         ...
         */
        #define TRC_KERNEL_PORT_CLEAR_INTERRUPT_MASK( xMask ) \
            portCLEAR_INTERRUPT_MASK_FROM_ISR( ( UBaseType_t ) ( xMask ) )
...
void vTraceSetEventGroupName( void * pvEventGroup, const char * szName );
             ...
             */
            #define vTraceSetEventGroupName( __pvEventGroup, __szName ) \
                ( ( void ) ( __pvEventGroup ), ( void ) ( __szName ) )
...
void vTraceSetMessageBufferName( void * pvMessageBuffer, const char * szName );
             ...
             */
            #define vTraceSetStreamBufferName( __pvStreamBuffer, __szName ) \
                ( ( void ) ( __pvStreamBuffer ), ( void ) ( __szName ) )

            /**
             * @brief Disabled by TRC_CFG_INCLUDE_STREAM_BUFFER_EVENTS
             */
            #define vTraceSetMessageBufferName( __pvMessageBuffer, __szName ) \
                ( ( void ) ( __pvMessageBuffer ), ( void ) ( __szName ) )
 ...
 * @retval TRC_SUCCESS Success
 */
traceResult xTraceKernelPortGetUnusedStack(
    void * pvTask,
    TraceUnsignedBaseType_t * puxUnusedStack );
         ...
         */
        #define vTraceSetQueueName( __pvQueue, __szName ) \
            ( ( void ) ( __pvQueue ), ( void ) ( __szName ) )

        /**
         * @brief Disabled by TRC_CFG_SCHEDULING_ONLY
         */
        #define vTraceSetSemaphoreName( __pvSemaphore, __szName ) \
            ( ( void ) ( __pvSemaphore ), ( void ) ( __szName ) )

        /**
         * @brief Disabled by TRC_CFG_SCHEDULING_ONLY
         */
        #define vTraceSetMutexName( __pvMutex, __szName ) \
            ( ( void ) ( __pvMutex ), ( void ) ( __szName ) )
         ...
         * @brief Disabled by TRC_CFG_SCHEDULING_ONLY
         */
        #define xTraceKernelPortGetUnusedStack( pvTask, puxUnusedStack ) \
            ( ( void ) ( pvTask ), ( void ) ( puxUnusedStack ) )
    ...
    #define TRACE_MALLOC( size ) pvPortMalloc( size )
        ...
        /**
         * @brief Fix for FreeRTOS v9.0.1 to correctly identify xQueuePeek
         * events.
         *
         * In FreeRTOS v9.0.1, the below trace hooks are incorrectly used from
         * three different functions. This as the earlier function
         * xQueueGenericReceive has been replaced by xQueuePeek,
         * xQueueSemaphoreTake and xQueueReceive.
         *
         * xQueueGenericReceive had a parameter "xJustPeeking", used by the
         * trace hooks to tell between xQueuePeek events and others. This is no
         * longer present, so we need another way to correctly identify peek
         * events. Since all three functions call the same trace macros, the
         * context of these macro is unknown.
         *
         * We therefore check the __LINE__ macro inside of the trace macros.
         * This gives the line number of queue.c, where the macros are used.
         * This can be used to tell if the context is xQueuePeek or another
         * function.
         * __LINE__ is a standard compiler feature since ancient times, so it
         * should work on all common compilers.
         *
         * This might seem as a quite brittle and unusual solution, but works in
         * this particular case and is only for FreeRTOS v9.0.1. Future versions
         * of FreeRTOS should not need this fix, as we have submitted a
         * correction of queue.c with individual trace macros for each function.
         */
         ...
         */
        #define isQueueReceiveHookActuallyPeek                         \
            ( __LINE__ < 0 ) /* instead of pdFALSE to fix a warning of \
                                "constant condition" */
    ...
    /*
     * The following xQueueGiveFromISR macro hacks make sure xQueueGiveFromISR
     * also has a xCopyPosition parameter
     */

    /* Expands name if this header is included... uxQueueType must be a macro
     * that only exists in queue.c or whatever, and it must expand to nothing or
     * to something that's valid in identifiers */
    #define xQueueGiveFromISR( a, b ) \
        TZ__CAT( xQueueGiveFromISR__, uxQueueType )( a, b )

    /* If in queue.c, the "uxQueueType" macro expands to "pcHead".
     * queueSEND_TO_BACK is the value we need to send in */
    #define xQueueGiveFromISR__pcHead( __a, __b )                          \
        MyWrapper_xQueueGiveFromISR( __a,                                  \
                                     __b,                                  \
                                     const BaseType_t xCopyPosition );     \
        BaseType_t xQueueGiveFromISR( __a, __b )                           \
        {                                                                  \
            return MyWrapper_xQueueGiveFromISR( xQueue,                    \
                                                pxHigherPriorityTaskWoken, \
                                                queueSEND_TO_BACK );       \
        }                                                                  \
        BaseType_t MyWrapper_xQueueGiveFromISR( __a,                       \
                                                __b,                       \
                                                const BaseType_t xCopyPosition )

    /* If not in queue.c, "uxQueueType" isn't expanded */
    #define xQueueGiveFromISR__uxQueueType( __a, __b ) \
        xQueueGiveFromISR( __a, __b )
...
void prvTraceSetStreamBufferNumberHigh16( void * pvStreamBuffer,
         ...
         */
        #define TRACE_GET_QUEUE_FILTER( pxQueue ) \
            prvTraceGetQueueNumberHigh16( ( void * ) pxQueue )
         ...
         * @param[in] group Group
         */
        #define TRACE_SET_QUEUE_FILTER( pxQueue, group ) \
            prvTraceSetQueueNumberHigh16( ( void * ) pxQueue, group )
             ...
             * @returns uint16_t Queue filter
             */
            #define TRACE_GET_EVENTGROUP_FILTER( pxEventGroup ) \
                prvTraceGetEventGroupNumberHigh16( ( void * ) pxEventGroup )
             ...
             * @param[in] group Group
             */
            #define TRACE_SET_EVENTGROUP_FILTER( pxEventGroup, group )      \
                prvTraceSetEventGroupNumberHigh16( ( void * ) pxEventGroup, \
                                                   group )
             ...
             */
            #define TRACE_GET_TIMER_FILTER( pxTimer ) \
                prvTraceGetTimerNumberHigh16( ( void * ) pxTimer )
             ...
             * @param[in] group Group
             */
            #define TRACE_SET_TIMER_FILTER( pxTimer, group ) \
                prvTraceSetTimerNumberHigh16( ( void * ) pxTimer, group )
         ...
         * @returns uint16_t Timer filter
         */
        #define TRACE_GET_STREAMBUFFER_FILTER( pxStreamBuffer ) \
            prvTraceGetStreamBufferNumberHigh16( ( void * ) pxStreamBuffer )
         ...
         * @param[in] group Group
         */
        #define TRACE_SET_STREAMBUFFER_FILTER( pxStreamBuffer, group )      \
            prvTraceSetStreamBufferNumberHigh16( ( void * ) pxStreamBuffer, \
                                                 group )
        ...
        #define TRACE_KERNEL_OBJECT_COUNT                                 \
            ( ( TRC_CFG_NQUEUE ) + ( TRC_CFG_NSEMAPHORE ) +               \
              ( TRC_CFG_NMUTEX ) + ( TRC_CFG_NTASK ) + ( TRC_CFG_NISR ) + \
              ( TRC_CFG_NTIMER ) + ( TRC_CFG_NEVENTGROUP ) +              \
              ( TRC_CFG_NSTREAMBUFFER ) + ( TRC_CFG_NMESSAGEBUFFER ) )
         ...
         */
        #define PropertyTableSizeSemaphore \
            ( ( TRC_CFG_NAME_LEN_SEMAPHORE ) + 1 )
        ...
        /* TRC_CFG_NEVENTGROUP properties:			Byte 0-3: state (unused for
         * now)*/
        ...
        #define StartIndexMutex     \
            ( StartIndexSemaphore + \
              ( TRC_CFG_NSEMAPHORE ) *PropertyTableSizeSemaphore )
        #define StartIndexTask \
            ( StartIndexMutex + ( TRC_CFG_NMUTEX ) *PropertyTableSizeMutex )
        #define StartIndexISR \
            ( StartIndexTask + ( TRC_CFG_NTASK ) *PropertyTableSizeTask )
        #define StartIndexTimer \
            ( StartIndexISR + ( TRC_CFG_NISR ) *PropertyTableSizeISR )
        ...
        #define StartIndexStreamBuffer \
            ( StartIndexEventGroup +   \
              ( TRC_CFG_NEVENTGROUP ) *PropertyTableSizeEventGroup )
...
extern traceObjectClass TraceQueueClassTable[ 5 ];
         ...
         * EVENTGROUP_TS
         *
         * Events for storing task-switches and interrupts. The RESUME events
         *are generated if the task/interrupt is already marked active.
         ******************************************************************************/
         ...
         *
         * About Close Events
         * When an object is evicted from the object property table (object
         *close), two internal events are stored (EVENTGROUP_OBJCLOSE_NAME and
         * EVENTGROUP_OBJCLOSE_PROP), containing the handle-name mapping and
         *object properties valid up to this point.
         ******************************************************************************/
        #define EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS \
            ( EVENTGROUP_TS + 4UL ) /*0x08*/

        /*******************************************************************************
         * EVENTGROUP_OBJCLOSE_PROP
         *
         * The internal event carrying properties of deleted objects
         * The handle and object class of the closed object is not stored in
         *this event, but is assumed to be the same as in the preceding CLOSE
         *event. Thus, these two events must be generated from within a critical
         *section. When queues are closed, arg1 is the "state" property (i.e.,
         *number of buffered messages/signals). When actors are closed, arg1 is
         *priority, arg2 is handle of the "instance finish" event, and arg3 is
         *event code of the "instance finish" event. In this case, the lower
         *three bits is the object class of the instance finish handle. The
         *lower three bits are not used (always zero) when queues are closed
         *since the queue type is given in the previous OBJCLOSE_NAME event.
         ******************************************************************************/
         ...
         * The lower three bits in the event code gives the object class, i.e.,
         *type of create operation (task, queue, semaphore, etc).
         ******************************************************************************/
         ...
         * semaphores and mutexes The lower three bits in the event code gives
         *the object class, i.e., what type of object that is operated on
         *(queue, semaphore or mutex).
         ******************************************************************************/
         ...
         * EVENTGROUP_RECEIVE
         *
         * The events in this group are used to log Receive/Take events on
         *queues, semaphores and mutexes. The lower three bits in the event code
         *gives the object class, i.e., what type of object that is operated on
         *(queue, semaphore or mutex).
         ******************************************************************************/
        ...
        /* "Failed" event type versions of above (timeout, failed allocation,
         * etc) */
        ...
        #define EVENTGROUP_CREATE_OBJ_TRCFAILED \
            ( EVENTGROUP_KSE_TRCFAILED ) /*0x40*/
        ...
        #define EVENTGROUP_RECEIVE_TRCFAILED \
            ( EVENTGROUP_SEND_TRCFAILED + 8UL ) /*0x50*/
        ...
        #define EVENTGROUP_SEND_TRCBLOCK \
            ( EVENTGROUP_RECEIVE_TRCBLOCK + 8UL ) /*0x70*/

        /* Events on queue peek (receive) */
        #define EVENTGROUP_PEEK_TRCSUCCESS \
            ( EVENTGROUP_SEND_TRCBLOCK + 8UL ) /*0x78*/
        ...
        #define EVENTGROUP_OTHERS \
            ( EVENTGROUP_DELETE_OBJ_TRCSUCCESS + 8UL )                  /*0x88*/
        ...
        #define PEND_FUNC_CALL_FROM_ISR \
            ( EVENTGROUP_MISC_PLACEHOLDER + 1UL ) /*0x91*/
        #define PEND_FUNC_CALL_TRCFAILED \
            ( EVENTGROUP_MISC_PLACEHOLDER + 2UL ) /*0x92*/
        ...
        /* User events */
        #define EVENTGROUP_USEREVENT \
            ( EVENTGROUP_MISC_PLACEHOLDER + 8UL ) /*0x98*/
         ...
         * The timestamps used in the recorder are "differential timestamps"
         *(DTS), i.e. the time since the last stored event. The DTS fields are
         *either 1 or 2 bytes in the other events, depending on the bytes
         *available in the event struct. If the time since the last event (the
         *DTS) is larger than allowed for by the DTS field of the current event,
         *an XTS event is inserted immediately before the original event. The
         *XTS event contains up to 3 additional bytes of the DTS value - the
         *higher bytes of the true DTS value. The lower 1-2 bytes are stored in
         *the normal DTS field. There are two types of XTS events, XTS8 and
         *XTS16. An XTS8 event is stored when there is only room for 1 byte (8
         *bit) DTS data in the original event, which means a limit of 0xFF
         *(255UL). The XTS16 is used when the original event has a 16 bit DTS
         *field and thereby can handle values up to 0xFFFF (65535UL).
         *
         * Using a very high frequency time base can result in many XTS events.
         * Preferably, the time between two OS ticks should fit in 16 bits,
         *i.e., at most 65535. If your time base has a higher frequency, you can
         *define the TRACE
         ******************************************************************************/
        ...
        #define TIMER_CHANGE_PERIOD_TRCFAILED \
            ( EVENTGROUP_TIMER + 13UL )                            /*0xBD*/
        #define TIMER_DELETE_TRCFAILED ( EVENTGROUP_TIMER + 14UL ) /*0xBE*/
        #define TIMER_START_FROM_ISR_TRCFAILED \
            ( EVENTGROUP_TIMER + 15UL ) /*0xBF*/
        #define TIMER_RESET_FROM_ISR_TRCFAILED \
            ( EVENTGROUP_TIMER + 16UL ) /*0xC0*/
        #define TIMER_STOP_FROM_ISR_TRCFAILED \
            ( EVENTGROUP_TIMER + 17UL ) /*0xC1*/
        ...
        #define EVENT_GROUP_WAIT_BITS_END_TRCFAILED \
            ( EVENTGROUP_EG + 11UL )                                   /*0xCD*/
        #define EVENT_GROUP_SET_BITS_FROM_ISR ( EVENTGROUP_EG + 12UL ) /*0xCE*/
        #define EVENT_GROUP_SET_BITS_FROM_ISR_TRCFAILED \
            ( EVENTGROUP_EG + 13UL ) /*0xCF*/

        #define TASK_INSTANCE_FINISHED_NEXT_KSE \
            ( EVENTGROUP_EG + 14UL )                                   /*0xD0*/
        ...
        #define TRACE_TASK_NOTIFY_TAKE \
            ( TRACE_TASK_NOTIFY_GROUP + 1UL ) /*0xD3*/
        #define TRACE_TASK_NOTIFY_TAKE_TRCBLOCK \
            ( TRACE_TASK_NOTIFY_GROUP + 2UL ) /*0xD4*/
        #define TRACE_TASK_NOTIFY_TAKE_TRCFAILED \
            ( TRACE_TASK_NOTIFY_GROUP + 3UL ) /*0xD5*/
        #define TRACE_TASK_NOTIFY_WAIT \
            ( TRACE_TASK_NOTIFY_GROUP + 4UL ) /*0xD6*/
        #define TRACE_TASK_NOTIFY_WAIT_TRCBLOCK \
            ( TRACE_TASK_NOTIFY_GROUP + 5UL ) /*0xD7*/
        #define TRACE_TASK_NOTIFY_WAIT_TRCFAILED \
            ( TRACE_TASK_NOTIFY_GROUP + 6UL ) /*0xD8*/
        #define TRACE_TASK_NOTIFY_FROM_ISR \
            ( TRACE_TASK_NOTIFY_GROUP + 7UL ) /*0xD9*/
        #define TRACE_TASK_NOTIFY_GIVE_FROM_ISR \
            ( TRACE_TASK_NOTIFY_GROUP + 8UL ) /*0xDA*/

        #define TIMER_EXPIRED ( TRACE_TASK_NOTIFY_GROUP + 9UL ) /*0xDB*/

        /* Events on queue peek (receive) */
        #define EVENTGROUP_PEEK_TRCBLOCK \
        ...
        /* Events on queue peek (receive) */
        #define EVENTGROUP_PEEK_TRCFAILED \
            ( EVENTGROUP_PEEK_TRCBLOCK + 3UL ) /*0xDF*/
        /* peek failed on queue:		0xDF	*/
        /* peek failed on semaphore:	0xE0	*/
        /* peek failed on mutex:		0xE1	*/

        #define EVENTGROUP_STREAMBUFFER_DIV \
            ( EVENTGROUP_PEEK_TRCFAILED + 3UL ) /*0xE2*/
        #define TRACE_STREAMBUFFER_RESET \
            ( EVENTGROUP_STREAMBUFFER_DIV + 0 ) /*0xE2*/
        #define TRACE_MESSAGEBUFFER_RESET \
            ( EVENTGROUP_STREAMBUFFER_DIV + 1UL ) /*0xE3*/
        ...
        #define EVENTGROUP_MALLOC_FAILED \
            ( EVENTGROUP_STREAMBUFFER_DIV + 6UL ) /*0xE8*/
        #define MEM_MALLOC_SIZE_TRCFAILED \
            ( EVENTGROUP_MALLOC_FAILED + 0UL ) /*0xE8*/
        #define MEM_MALLOC_ADDR_TRCFAILED \
            ( EVENTGROUP_MALLOC_FAILED + 1UL ) /*0xE9*/
        ...
        /* The following are using previously "lost" event codes. These macros
         * aren't even directly referenced, instead we do (equivalent
         * STREAMBUFFER code) + 1. */
         ...
         ****************************/
        #define TRACE_GET_TRACE_CLASS_FROM_TASK_CLASS( kernelClass ) \
            ( TRACE_CLASS_TASK )
        #define TRACE_GET_TRACE_CLASS_FROM_TASK_OBJECT( pxObject ) \
            ( TRACE_CLASS_TASK )
        ...
        #define TRACE_GET_TRACE_CLASS_FROM_QUEUE_OBJECT( pxObject ) \
            TRACE_GET_TRACE_CLASS_FROM_QUEUE_CLASS(                 \
                prvTraceGetQueueType( pxObject ) )

        #define TRACE_GET_TRACE_CLASS_FROM_TIMER_CLASS( kernelClass ) \
            ( TRACE_CLASS_TIMER )
        #define TRACE_GET_TRACE_CLASS_FROM_TIMER_OBJECT( pxObject ) \
            ( TRACE_CLASS_TIMER )
        ...
        /* TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_CLASS can only be accessed
         * with a parameter indicating if it is a MessageBuffer */
        #define TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_CLASS(  \
            xIsMessageBuffer )                                  \
        ...
        #define TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_OBJECT( pxObject ) \
            ( prvGetStreamBufferType( pxObject ) == 1                      \
        ...
        #define TRACE_SET_TASK_NUMBER( pxTCB ) \
            prvTraceSetTaskNumberLow16(        \
                ...
                prvTraceGetObjectHandle(       \
                    TRACE_GET_OBJECT_TRACE_CLASS( TASK, pxTCB ) ) );
        ...
        #define TRACE_SET_QUEUE_NUMBER( queue )       \
            ...
            #define TRACE_SET_TIMER_NUMBER( tmr )                               \
                ( ( Timer_t * ) tmr )->uxTimerNumber = prvTraceGetObjectHandle( \
                    TRACE_GET_OBJECT_TRACE_CLASS( TIMER, tmr ) );
        ...
        #else /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
            #define TRACE_GET_EVENTGROUP_NUMBER( eg ) \
                ( ( traceHandle ) uxEventGroupGetNumber( eg ) )
            #define TRACE_SET_EVENTGROUP_NUMBER( eg )               \
                ( ( EventGroup_t * ) eg )                           \
                    ->uxEventGroupNumber = prvTraceGetObjectHandle( \
                    TRACE_GET_OBJECT_TRACE_CLASS( EVENTGROUP, eg ) );
        #endif /* (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0) */
         ...
         ******************************/
        #define TRACE_GET_TASK_CLASS_EVENT_CODE( SERVICE,      \
                                                 RESULT,       \
                                                 kernelClass ) \
            ( uint8_t )( EVENTGROUP_##SERVICE##_##RESULT +     \
                         TRACE_GET_CLASS_TRACE_CLASS( TASK, kernelClass ) )
        #define TRACE_GET_QUEUE_CLASS_EVENT_CODE( SERVICE,      \
                                                  RESULT,       \
                                                  kernelClass ) \
            ( uint8_t )( EVENTGROUP_##SERVICE##_##RESULT +      \
                         TRACE_GET_CLASS_TRACE_CLASS( QUEUE, kernelClass ) )
        #define TRACE_GET_TIMER_CLASS_EVENT_CODE( SERVICE,      \
                                                  RESULT,       \
                                                  kernelClass ) \
            --THIS IS NOT USED--
        #define TRACE_GET_EVENTGROUP_CLASS_EVENT_CODE( SERVICE,      \
                                                       RESULT,       \
                                                       kernelClass ) \
            --THIS IS NOT USED--
        #define TRACE_GET_STREAMBUFFER_CLASS_EVENT_CODE( SERVICE,          \
                                                         RESULT,           \
                                                         isMessageBuffer ) \
            ( uint8_t )( TRACE_STREAMBUFFER_##SERVICE##_##RESULT +         \
                         ( uint8_t ) isMessageBuffer )
        ...
        #define TRACE_GET_TIMER_OBJECT_EVENT_CODE( SERVICE, RESULT, UNUSED ) \
            --THIS IS NOT USED--
        #define TRACE_GET_EVENTGROUP_OBJECT_EVENT_CODE( SERVICE, \
                                                        RESULT,  \
                                                        UNUSED ) \
            --THIS IS NOT USED--
        #define TRACE_GET_STREAMBUFFER_OBJECT_EVENT_CODE( SERVICE,   \
                                                          RESULT,    \
                                                          pxObject ) \
            ( uint8_t )( TRACE_STREAMBUFFER_##SERVICE##_##RESULT +   \
        ...
        /* Generic versions */
        #define TRACE_GET_CLASS_EVENT_CODE( SERVICE,      \
                                            RESULT,       \
                                            CLASS,        \
                                            kernelClass ) \
         ...
         ******************************/
        #define TRACE_GET_TASK_PRIORITY( pxTCB ) \
            ( ( uint8_t ) pxTCB->uxPriority )
        #define TRACE_GET_TASK_NAME( pxTCB ) ( ( char * ) pxTCB->pcTaskName )

        /*** The trace macros for snapshot mode
         * **************************************/

        /* A macro that will update the tick count when returning from tickless
         * idle */
        ...
        /* Called on each OS tick. Will call uiPortGetTimestamp to make sure it
         * is called at least once every OS tick. */
...
extern volatile uint32_t uiTraceSystemState;
        ...
        #define traceTASK_CREATE( pxNewTCB )                 \
            ...
            {                                                \
                trcKERNEL_HOOKS_TASK_CREATE(                 \
                    TRACE_GET_OBJECT_EVENT_CODE( CREATE_OBJ, \
                                                 TRCSUCCESS, \
                                                 TASK,       \
                                                 pxNewTCB ), \
                    TASK,                                    \
        ...
        #define traceTASK_CREATE_FAILED()               \
            trcKERNEL_HOOKS_OBJECT_CREATE_FAILED(       \
                TRACE_GET_CLASS_EVENT_CODE( CREATE_OBJ, \
                                            TRCFAILED,  \
                                            TASK,       \
                                            NOT_USED ), \
                TRACE_GET_CLASS_TRACE_CLASS( TASK, NOT_USED ) )
        ...
        #define traceTASK_DELETE( pxTaskToDelete )                 \
            {                                                      \
                ...
                trcKERNEL_HOOKS_TASK_DELETE(                       \
                    TRACE_GET_OBJECT_EVENT_CODE( DELETE_OBJ,       \
                                                 TRCSUCCESS,       \
                                                 TASK,             \
                                                 pxTaskToDelete ), \
                    TRACE_GET_OBJECT_EVENT_CODE( OBJCLOSE_NAME,    \
                                                 TRCSUCCESS,       \
                                                 TASK,             \
                                                 pxTaskToDelete ), \
                    TRACE_GET_OBJECT_EVENT_CODE( OBJCLOSE_PROP,    \
                                                 TRCSUCCESS,       \
                                                 TASK,             \
                                                 pxTaskToDelete ), \
                    pxTaskToDelete );                              \
            ...
            /* Called on vTaskDelay - note the use of FreeRTOS variable
             * xTicksToDelay */
            #undef traceTASK_DELAY
            #define traceTASK_DELAY()                        \
                trcKERNEL_HOOKS_TASK_DELAY( TASK_DELAY,      \
                                            pxCurrentTCB,    \
                                            xTicksToDelay ); \
                trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();

            /* Called on vTaskDelayUntil - note the use of FreeRTOS variable
             * xTimeToWake */
            #undef traceTASK_DELAY_UNTIL
            #if( TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 )

                #define traceTASK_DELAY_UNTIL( xTimeToWake )      \
                    trcKERNEL_HOOKS_TASK_DELAY( TASK_DELAY_UNTIL, \
                                                pxCurrentTCB,     \
                                                xTimeToWake );    \
                    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
            #else
            ...
            /* Called in xQueueCreate, and thereby for all other object based on
             * queues, such as semaphores. */
            ...
            #define traceQUEUE_CREATE_FAILED( queueType )    \
                trcKERNEL_HOOKS_OBJECT_CREATE_FAILED(        \
                    TRACE_GET_CLASS_EVENT_CODE( CREATE_OBJ,  \
                                                ...
                                                QUEUE,       \
                                                queueType ), \
                    TRACE_GET_CLASS_TRACE_CLASS( QUEUE, queueType ) )
            ...
            /* This macro is not necessary as of FreeRTOS v9.0.0 */
                ...
                /* Called in xQueueCreateMutex when the operation fails (when
            ...
            #define traceGIVE_MUTEX_RECURSIVE_FAILED( pxMutex ) \
                trcKERNEL_HOOKS_KERNEL_SERVICE(                 \
                    TRACE_GET_OBJECT_EVENT_CODE( SEND,          \
                                                 TRCFAILED,     \
                                                 QUEUE,         \
                                                 pxMutex ),     \
                    QUEUE,                                      \
            ...
            #define traceQUEUE_SEND( pxQueue )                        \
                ...
                trcKERNEL_HOOKS_SET_OBJECT_STATE(                     \
                    QUEUE,                                            \
                    pxQueue,                                          \
                    TRACE_GET_OBJECT_TRACE_CLASS( QUEUE, pxQueue ) == \
                            TRACE_CLASS_MUTEX                         \
                        ? ( uint8_t ) 0                               \
            ...
            /* Called when the task is blocked due to a send operation on a full
             * queue */
            ...
            #define traceQUEUE_RECEIVE( pxQueue )                     \
                ...
                {                                                     \
                    trcKERNEL_HOOKS_KERNEL_SERVICE(                   \
                        TRACE_GET_OBJECT_EVENT_CODE( PEEK,            \
                                                     TRCSUCCESS,      \
                                                     QUEUE,           \
                                                     pxQueue ),       \
                        QUEUE,                                        \
                ...
                {                                                     \
                    trcKERNEL_HOOKS_KERNEL_SERVICE(                   \
                        TRACE_GET_OBJECT_EVENT_CODE( RECEIVE,         \
                                                     TRCSUCCESS,      \
                                                     QUEUE,           \
                                                     pxQueue ),       \
                        QUEUE,                                        \
            ...
            /* Called when the task is blocked due to a receive operation on an
             * empty queue */
            ...
            #define traceBLOCKING_ON_QUEUE_RECEIVE( pxQueue )         \
                ...
                if( TRACE_GET_OBJECT_TRACE_CLASS( QUEUE, pxQueue ) != \
                    TRACE_CLASS_MUTEX )                               \
                {                                                     \
                    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();     \
                }
            ...
            #define traceQUEUE_SEND_FROM_ISR( pxQueue )                  \
                trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                 \
                    xCopyPosition == queueSEND_TO_BACK                   \
                        ? ( TRACE_GET_OBJECT_EVENT_CODE( SEND_FROM_ISR,  \
                                                         TRCSUCCESS,     \
                                                         QUEUE,          \
                                                         pxQueue ) )     \
            ...
            /* Called when a message send from interrupt context fails (since
             * the queue was full) */
            ...
            /* Called when a message is received in interrupt context, e.g.,
             * using xQueueReceiveFromISR */
            ...
            /* Called when a message receive from interrupt context fails (since
             * the queue was empty) */
            ...
            #define traceQUEUE_REGISTRY_ADD( object, name )                    \
                prvTraceSetObjectName( TRACE_GET_OBJECT_TRACE_CLASS( QUEUE,    \
                                                                     object ), \
                                       TRACE_GET_OBJECT_NUMBER( QUEUE,         \
                                                                object ),      \
                                       name );
            ...
            #define traceTASK_PRIORITY_SET( pxTask, uxNewPriority )      \
                trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE( TASK_PRIORITY_SET, \
                                                      pxTask,            \
                                                      uxNewPriority );

            /* Called in vTaskPriorityInherit, which is called by Mutex
             * operations */
            #undef traceTASK_PRIORITY_INHERIT
            #define traceTASK_PRIORITY_INHERIT( pxTask, uxNewPriority )      \
                trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE( TASK_PRIORITY_INHERIT, \
                                                      pxTask,                \
                                                      uxNewPriority );

            /* Called in vTaskPriorityDisinherit, which is called by Mutex
             * operations */
            #undef traceTASK_PRIORITY_DISINHERIT
            #define traceTASK_PRIORITY_DISINHERIT( pxTask, uxNewPriority )      \
                trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE( TASK_PRIORITY_DISINHERIT, \
                                                      pxTask,                   \
                                                      uxNewPriority );
            ...
            #define traceTASK_RESUME_FROM_ISR( pxTaskToResume )             \
                trcKERNEL_HOOKS_TASK_RESUME_FROM_ISR( TASK_RESUME_FROM_ISR, \
                                                      pxTaskToResume );
...
extern void vTraceStoreMemMangEvent( uint32_t ecode,
                    ...
                    /* MALLOC and FREE are always stored, no matter if they
                     * happen inside filtered task */
                    #undef traceMALLOC
                    #define traceMALLOC( pvAddress, uiSize )                    \
                        if( pvAddress != 0 )                                    \
                        {                                                       \
                            vTraceStoreMemMangEvent( MEM_MALLOC_SIZE,           \
                                                     ( uint32_t ) pvAddress,    \
                                                     ( int32_t ) uiSize );      \
                        }                                                       \
                        else                                                    \
                        {                                                       \
                            vTraceStoreMemMangEvent( MEM_MALLOC_SIZE_TRCFAILED, \
                                                     ( uint32_t ) pvAddress,    \
                                                     ( int32_t ) uiSize );      \
                        }
                    ...
                    #define traceFREE( pvAddress, uiSize )               \
                        vTraceStoreMemMangEvent( MEM_FREE_SIZE,          \
                                                 ( uint32_t ) pvAddress, \
                                                 -( ( int32_t ) uiSize ) );
                    ...
                    #define traceTIMER_CREATE( tmr )                 \
                        trcKERNEL_HOOKS_OBJECT_CREATE( TIMER_CREATE, \
                                                       TIMER,        \
                                                       tmr );
                    ...
                    #define traceTIMER_CREATE_FAILED()        \
                        trcKERNEL_HOOKS_OBJECT_CREATE_FAILED( \
                            TIMER_CREATE_TRCFAILED,           \
                            TRACE_GET_CLASS_TRACE_CLASS( TIMER, NOT_USED ) )

                    /* Note that xCommandID can never be
                     * tmrCOMMAND_EXECUTE_CALLBACK (-1) since the trace macro is
                     * not called in that case */
                    #undef traceTIMER_COMMAND_SEND
                    #define traceTIMER_COMMAND_SEND( tmr,                      \
                                                     xCommandID,               \
                                                     xOptionalValue,           \
                                                     xReturn )                 \
                            ...
                            {                                                  \
                                trcKERNEL_HOOKS_OBJECT_DELETE(                 \
                                    ...
                                    EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS +      \
                                        TRACE_GET_OBJECT_TRACE_CLASS( TIMER,   \
                                                                      tmr ),   \
                    ...
                    #define traceTIMER_EXPIRED( tmr )                  \
                        trcKERNEL_HOOKS_KERNEL_SERVICE( TIMER_EXPIRED, \
                                                        TIMER,         \
                                                        tmr );
                    ...
                    #define tracePEND_FUNC_CALL( func, arg1, arg2, ret ) \
                        ...
                        {                                                \
                            trcKERNEL_HOOKS_KERNEL_SERVICE(              \
                                PEND_FUNC_CALL_TRCFAILED,                \
                                TASK,                                    \
                                xTimerGetTimerDaemonTaskHandle() );      \
                        }

                    #undef tracePEND_FUNC_CALL_FROM_ISR
                    #define tracePEND_FUNC_CALL_FROM_ISR( func,           \
                                                          arg1,           \
                                                          arg2,           \
                                                          ret )           \
                ...
                #define traceEVENT_GROUP_CREATE( eg )                  \
                    trcKERNEL_HOOKS_OBJECT_CREATE( EVENT_GROUP_CREATE, \
                                                   EVENTGROUP,         \
                                                   eg )
                ...
                #define traceEVENT_GROUP_CREATE_FAILED()  \
                    trcKERNEL_HOOKS_OBJECT_CREATE_FAILED( \
                        ...
                        TRACE_GET_CLASS_TRACE_CLASS( EVENTGROUP, NOT_USED ) )
                ...
                #define traceEVENT_GROUP_DELETE( eg )                     \
                    {                                                     \
                        ...
                        trcKERNEL_HOOKS_OBJECT_DELETE(                    \
                            ...
                            EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS +         \
                                TRACE_GET_OBJECT_TRACE_CLASS( EVENTGROUP, \
                                                              eg ),       \
                            EVENTGROUP_OBJCLOSE_NAME_TRCSUCCESS +         \
                                TRACE_GET_OBJECT_TRACE_CLASS( EVENTGROUP, \
                                                              eg ),       \
                ...
                #define traceEVENT_GROUP_SYNC_BLOCK( eg,             \
                                                     bitsToSet,      \
                                                     bitsToWaitFor ) \
                    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(       \
                        EVENT_GROUP_SYNC_TRCBLOCK,                   \
                        EVENTGROUP,                                  \
                        eg,                                          \
                        bitsToWaitFor );

                #undef traceEVENT_GROUP_SYNC_END
                #define traceEVENT_GROUP_SYNC_END( eg,             \
                                                   bitsToSet,      \
                                                   bitsToWaitFor,  \
                                                   wasTimeout )    \
                    if( wasTimeout )                               \
                    {                                              \
                        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM( \
                            EVENT_GROUP_SYNC_END_TRCFAILED,        \
                            EVENTGROUP,                            \
                            eg,                                    \
                            bitsToWaitFor );                       \
                    }                                              \
                ...
                #define traceEVENT_GROUP_WAIT_BITS_BLOCK( eg, bitsToWaitFor ) \
                    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                \
                        EVENT_GROUP_WAIT_BITS_TRCBLOCK,                       \
                        EVENTGROUP,                                           \
                        eg,                                                   \
                        bitsToWaitFor );                                      \
                    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
                ...
                #define traceEVENT_GROUP_WAIT_BITS_END( eg,            \
                                                        bitsToWaitFor, \
                                                        wasTimeout )   \
                    if( wasTimeout )                                   \
                    {                                                  \
                        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(     \
                            EVENT_GROUP_WAIT_BITS_END_TRCFAILED,       \
                            EVENTGROUP,                                \
                            eg,                                        \
                            bitsToWaitFor );                           \
                    }                                                  \
                ...
                #define traceEVENT_GROUP_CLEAR_BITS_FROM_ISR( eg,           \
                                                              bitsToClear ) \
            ...
            #undef traceTASK_NOTIFY_TAKE
                ...
                #define traceTASK_NOTIFY_TAKE()                    \
                    ...
                    {                                              \
                        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM( \
                            TRACE_TASK_NOTIFY_TAKE_TRCFAILED,      \
                            TASK,                                  \
                            pxCurrentTCB,                          \
                            xTicksToWait );                        \
                    }
                ...
                #define traceTASK_NOTIFY_TAKE( index )             \
                    if( pxCurrentTCB->ucNotifyState[ index ] ==    \
                        taskNOTIFICATION_RECEIVED )                \
                    {                                              \
                        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM( \
                            TRACE_TASK_NOTIFY_TAKE,                \
            ...
            #undef traceTASK_NOTIFY_TAKE_BLOCK
                ...
                #define traceTASK_NOTIFY_TAKE_BLOCK()          \
                    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM( \
                        TRACE_TASK_NOTIFY_TAKE_TRCBLOCK,       \
                        TASK,                                  \
                        pxCurrentTCB,                          \
                        xTicksToWait );                        \
                    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
            ...
            #undef traceTASK_NOTIFY_WAIT
                ...
                #define traceTASK_NOTIFY_WAIT()                         \
                    if( TRACE_GET_OBJECT_FILTER( TASK, pxCurrentTCB ) & \
                        CurrentFilterMask )                             \
                    {                                                   \
                        if( pxCurrentTCB->eNotifyState == eNotified )   \
                            prvTraceStoreKernelCallWithParam(           \
                                ...
                                TRACE_CLASS_TASK,                       \
                                TRACE_GET_TASK_NUMBER( pxCurrentTCB ),  \
                                xTicksToWait );                         \
                        else                                            \
                            prvTraceStoreKernelCallWithParam(           \
                                TRACE_TASK_NOTIFY_WAIT_TRCFAILED,       \
                                TRACE_CLASS_TASK,                       \
                                TRACE_GET_TASK_NUMBER( pxCurrentTCB ),  \
                                xTicksToWait );                         \
                    }
            ...
            #undef traceTASK_NOTIFY
                ...
                #define traceTASK_NOTIFY()                                    \
                    if( TRACE_GET_OBJECT_FILTER( TASK,                        \
                                                 TRACE_GET_CURRENT_TASK() ) & \
                        CurrentFilterMask )                                   \
                        if( TRACE_GET_OBJECT_FILTER( TASK, xTaskToNotify ) &  \
                            CurrentFilterMask )                               \
                            prvTraceStoreKernelCall( TRACE_TASK_NOTIFY,       \
                                                     TRACE_CLASS_TASK,        \
                                                     TRACE_GET_TASK_NUMBER(   \
                                                         xTaskToNotify ) );
            ...
            #undef traceTASK_NOTIFY_FROM_ISR
                ...
                #define traceTASK_NOTIFY_FROM_ISR()                          \
                    if( TRACE_GET_OBJECT_FILTER( TASK, xTaskToNotify ) &     \
                        CurrentFilterMask )                                  \
                        prvTraceStoreKernelCall( TRACE_TASK_NOTIFY_FROM_ISR, \
                                                 TRACE_CLASS_TASK,           \
                                                 TRACE_GET_TASK_NUMBER(      \
                                                     xTaskToNotify ) );
            ...
            #undef traceTASK_NOTIFY_GIVE_FROM_ISR
                ...
                #define traceTASK_NOTIFY_GIVE_FROM_ISR()                 \
                    if( TRACE_GET_OBJECT_FILTER( TASK, xTaskToNotify ) & \
                        CurrentFilterMask )                              \
                        prvTraceStoreKernelCall(                         \
                            TRACE_TASK_NOTIFY_GIVE_FROM_ISR,             \
                            TRACE_CLASS_TASK,                            \
                ...
                #define traceSTREAM_BUFFER_CREATE_FAILED( xIsMessageBuffer ) \
                    trcKERNEL_HOOKS_OBJECT_CREATE_FAILED(                    \
                        TRACE_GET_CLASS_EVENT_CODE( CREATE_OBJ,              \
                                                    ...
                                                    STREAMBUFFER,            \
                                                    xIsMessageBuffer ),      \
                        TRACE_GET_CLASS_TRACE_CLASS( STREAMBUFFER,           \
                                                     xIsMessageBuffer ) )
                ...
                #define traceSTREAM_BUFFER_RESET( xStreamBuffer )    \
                    trcKERNEL_HOOKS_KERNEL_SERVICE(                  \
                        ...
                        STREAMBUFFER,                                \
                        xStreamBuffer );                             \
                    trcKERNEL_HOOKS_SET_OBJECT_STATE( STREAMBUFFER,  \
                                                      xStreamBuffer, \
                                                      0 );
...
TraceHeapHandle_t xTraceKernelPortGetSystemHeapHandle( void );
        ...
        /*** Event codes for streaming - should match the Tracealyzer config
         * file *****/
         ...
         * idle */
        #undef traceINCREASE_TICK_COUNT
        /* Note: This can handle time adjustments of max 2^32 ticks, i.e., 35
         * seconds at 120 MHz. Thus, tick-less idle periods longer than 2^32
         * ticks will appear "compressed" on the time line.*/
        #define traceINCREASE_TICK_COUNT( xCount )                  \
            {                                                       \
                uint32_t uiTraceTickCount;                          \
                xTraceTimestampGetOsTickCount( &uiTraceTickCount ); \
                xTraceTimestampSetOsTickCount( uiTraceTickCount +   \
                                               ( xCount ) );        \
            }
            ...
            #define OS_TICK_EVENT( uxSchedulerSuspended, xTickCount )        \
                if( ( uxSchedulerSuspended ) ==                              \
                    ( unsigned portBASE_TYPE ) pdFALSE )                     \
                {                                                            \
                    prvTraceStoreEvent_Param( PSF_EVENT_NEW_TIME,            \
                                              ( uint32_t ) ( xTickCount ) ); \
                }
        ...
        #undef traceTASK_INCREMENT_TICK
            ...
            #define traceTASK_INCREMENT_TICK( xTickCount )               \
                if( uxSchedulerSuspended ==                              \
                    ...
                    xPendedTicks == 0 )                                  \
                {                                                        \
                    xTraceTimestampSetOsTickCount( ( xTickCount ) + 1 ); \
                }                                                        \
        ...
        /* Called for each task that becomes ready */
        #undef traceMOVED_TASK_TO_READY_STATE
        #define traceMOVED_TASK_TO_READY_STATE( pxTCB ) xTraceTaskReady( pxTCB )
        ...
        #undef traceTASK_CREATE
            ...
            #define traceTASK_CREATE( pxNewTCB )                               \
                if( ( pxNewTCB ) != 0 )                                        \
                {                                                              \
                    xTraceTaskRegisterWithoutHandle( ( void * ) ( pxNewTCB ),  \
                                                     ( pxNewTCB )->pcTaskName, \
                                                     ( pxNewTCB )              \
                                                         ->uxPriority );       \
                }
        ...
        /* Called on vTaskDelete */
        #undef traceTASK_DELETE // We don't allow for filtering out "delete"
                                // events. They are important and not very
                                // frequent. Moreover, we can't exclude create
                                // events, so this should be symmetrical.
        #define traceTASK_DELETE( pxTaskToDelete )             \
            xTraceTaskUnregisterWithoutHandle( pxTaskToDelete, \
                                               ( pxTaskToDelete )->uxPriority )
                ...
                #undef traceLOW_POWER_IDLE_BEGIN
                #define traceLOW_POWER_IDLE_BEGIN()                     \
                    prvTraceStoreEvent_Param( PSF_EVENT_LOWPOWER_BEGIN, \
                                              xExpectedIdleTime )
            ...
            #undef traceTASK_SUSPEND
            #define traceTASK_SUSPEND( pxTaskToSuspend )           \
                prvTraceStoreEvent_Handle( PSF_EVENT_TASK_SUSPEND, \
                                           pxTaskToSuspend )
            ...
            #define traceQUEUE_CREATE_FAILED( queueType )             \
                ...
                {                                                     \
                    case queueQUEUE_TYPE_BASE:                        \
                        prvTraceStoreEvent_HandleParam(               \
                            PSF_EVENT_QUEUE_CREATE_FAILED,            \
                            0,                                        \
                            uxQueueLength );                          \
                        break;                                        \
                    case queueQUEUE_TYPE_BINARY_SEMAPHORE:            \
                        prvTraceStoreEvent_HandleParam(               \
                            PSF_EVENT_SEMAPHORE_BINARY_CREATE_FAILED, \
                            0,                                        \
                            0 );                                      \
                        break;                                        \
            ...
            #undef traceQUEUE_DELETE // We don't allow for filtering out
                                     // "delete" events. They are important and
                                     // not very frequent. Moreover, we can't
                                     // exclude create events, so this should be
                                     // symmetrical.
            #define traceQUEUE_DELETE( pxQueue )              \
                ...
                {                                             \
                    ...
                    case queueQUEUE_TYPE_BINARY_SEMAPHORE:    \
                        xTraceObjectUnregisterWithoutHandle(  \
                            PSF_EVENT_SEMAPHORE_DELETE,       \
                            ( void * ) ( pxQueue ),           \
                            ( pxQueue )->uxMessagesWaiting ); \
                        break;                                \
                }

            /* Called in xQueueCreateCountingSemaphore, if the queue creation
             * fails */
            ...
            #define traceGIVE_MUTEX_RECURSIVE_FAILED( pxMutex ) \
                prvTraceStoreEvent_Handle(                      \
                    PSF_EVENT_MUTEX_GIVE_RECURSIVE_FAILED,      \
                    ( void * ) ( pxMutex ) )
            ...
            #define traceQUEUE_SEND( pxQueue )                               \
                ...
                {                                                            \
                    ...
                    case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                 \
                        prvTraceStoreEvent_HandleParam(                      \
                            PSF_EVENT_SEMAPHORE_GIVE,                        \
                            ( void * ) ( pxQueue ),                          \
                            ( pxQueue )->uxMessagesWaiting + 1 );            \
                        break;                                               \
                    case queueQUEUE_TYPE_MUTEX:                              \
                        prvTraceStoreEvent_Handle( PSF_EVENT_MUTEX_GIVE,     \
                                                   ( void * ) ( pxQueue ) ); \
                        break;                                               \
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                    \
                        prvTraceStoreEvent_Handle(                           \
                            PSF_EVENT_MUTEX_GIVE_RECURSIVE,                  \
                            ( void * ) ( pxQueue ) );                        \
                        break;                                               \
                }
            ...
            #define traceQUEUE_SEND_FAILED( pxQueue )                           \
                ...
                {                                                               \
                    ...
                    case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                    \
                        prvTraceStoreEvent_HandleParam(                         \
                            PSF_EVENT_SEMAPHORE_GIVE_FAILED,                    \
                            ( void * ) ( pxQueue ),                             \
                            ( pxQueue )->uxMessagesWaiting );                   \
                        break;                                                  \
                    case queueQUEUE_TYPE_MUTEX:                                 \
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                        prvTraceStoreEvent_Handle( PSF_EVENT_MUTEX_GIVE_FAILED, \
                                                   ( void * ) ( pxQueue ) );    \
                        break;                                                  \
                }
            ...
            #define traceBLOCKING_ON_QUEUE_SEND( pxQueue )                     \
                ...
                {                                                              \
                    ...
                    case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                   \
                        prvTraceStoreEvent_HandleParam(                        \
                            PSF_EVENT_SEMAPHORE_GIVE_BLOCK,                    \
                            ( void * ) ( pxQueue ),                            \
                            ( pxQueue )->uxMessagesWaiting );                  \
                        break;                                                 \
                    case queueQUEUE_TYPE_MUTEX:                                \
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                      \
                        prvTraceStoreEvent_Handle( PSF_EVENT_MUTEX_GIVE_BLOCK, \
                                                   ( void * ) ( pxQueue ) );   \
                        break;                                                 \
                }
            ...
            #define traceQUEUE_SEND_FROM_ISR( pxQueue )               \
                ...
                {                                                     \
                    ...
                    case queueQUEUE_TYPE_COUNTING_SEMAPHORE:          \
                        prvTraceStoreEvent_HandleParam(               \
                            PSF_EVENT_SEMAPHORE_GIVE_FROMISR,         \
                            ( void * ) ( pxQueue ),                   \
                            ( pxQueue )->uxMessagesWaiting + 1 );     \
                        break;                                        \
                }
                ...
                {                                                     \
                    ...
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:             \
                        prvTraceStoreEvent_HandleParam(               \
                            isQueueReceiveHookActuallyPeek            \
                                ? PSF_EVENT_MUTEX_PEEK                \
                                : PSF_EVENT_MUTEX_TAKE_RECURSIVE,     \
                            ( void * ) ( pxQueue ),                   \
                            xTicksToWait );                           \
                        break;                                        \
                }
                ...
                {                                                        \
                    ...
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                \
                        prvTraceStoreEvent_HandleParam(                  \
                            isQueueReceiveHookActuallyPeek               \
                                ? PSF_EVENT_MUTEX_PEEK_FAILED            \
                                : PSF_EVENT_MUTEX_TAKE_RECURSIVE_FAILED, \
                            ( void * ) ( pxQueue ),                      \
                            xTicksToWait );                              \
                        break;                                           \
                }
            ...
            #define traceBLOCKING_ON_QUEUE_RECEIVE( pxQueue )           \
                ...
                {                                                       \
                    ...
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:               \
                        prvTraceStoreEvent_HandleParam(                 \
                            isQueueReceiveHookActuallyPeek              \
                                ? PSF_EVENT_MUTEX_PEEK_BLOCK            \
                                : PSF_EVENT_MUTEX_TAKE_RECURSIVE_BLOCK, \
                            ( void * ) ( pxQueue ),                     \
                            xTicksToWait );                             \
                        break;                                          \
                }
                ...
                /* Called when the task is blocked due to a peek operation on an
                 * empty queue */
                #undef traceBLOCKING_ON_QUEUE_PEEK
                #define traceBLOCKING_ON_QUEUE_PEEK( pxQueue )    \
                    switch( ( pxQueue )->ucQueueType )            \
            ...
            #define traceQUEUE_RECEIVE_FROM_ISR( pxQueue )        \
                ...
                {                                                 \
                    case queueQUEUE_TYPE_BASE:                    \
                        prvTraceStoreEvent_HandleParam(           \
                            PSF_EVENT_QUEUE_RECEIVE_FROMISR,      \
                            ( void * ) ( pxQueue ),               \
                            ( pxQueue )->uxMessagesWaiting - 1 ); \
                        break;                                    \
                    case queueQUEUE_TYPE_BINARY_SEMAPHORE:        \
            ...
            #define traceQUEUE_PEEK( pxQueue )                                  \
                ...
                {                                                               \
                    ...
                    case queueQUEUE_TYPE_MUTEX:                                 \
                    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                        prvTraceStoreEvent_HandleParam( PSF_EVENT_MUTEX_PEEK,   \
                                                        ( void * ) ( pxQueue ), \
                                                        xTicksToWait );         \
                        break;                                                  \
                }
            ...
            #undef traceTASK_PRIORITY_INHERIT
            #define traceTASK_PRIORITY_INHERIT( pxTask, uxNewPriority )      \
                prvTraceStoreEvent_HandleParam( PSF_EVENT_TASK_PRIO_INHERIT, \
                                                ( void * ) ( pxTask ),       \
                                                uxNewPriority )
            ...
            #undef traceTASK_PRIORITY_DISINHERIT
            #define traceTASK_PRIORITY_DISINHERIT( pxTask, uxNewPriority )      \
                prvTraceStoreEvent_HandleParam( PSF_EVENT_TASK_PRIO_DISINHERIT, \
                                                ( void * ) ( pxTask ),          \
                                                uxNewPriority )
            ...
            #undef traceTASK_RESUME
            #define traceTASK_RESUME( pxTaskToResume )            \
                prvTraceStoreEvent_Handle( PSF_EVENT_TASK_RESUME, \
                                           ( void * ) ( pxTaskToResume ) )

            /* Called in vTaskResumeFromISR */
            #undef traceTASK_RESUME_FROM_ISR
            #define traceTASK_RESUME_FROM_ISR( pxTaskToResume )           \
                prvTraceStoreEvent_Handle( PSF_EVENT_TASK_RESUME_FROMISR, \
                                           ( void * ) ( pxTaskToResume ) )
                ...
                #define traceMALLOC( pvAddress, uiSize )                        \
                    ...
                    {                                                           \
                        xTraceHeapAlloc( xTraceKernelPortGetSystemHeapHandle(), \
                                         pvAddress,                             \
                                         uiSize );                              \
                    }
                ...
                #define traceFREE( pvAddress, uiSize )                         \
                    if( xTraceIsRecorderEnabled() )                            \
                    {                                                          \
                        xTraceHeapFree( xTraceKernelPortGetSystemHeapHandle(), \
                                        pvAddress,                             \
                                        uiSize );                              \
                    }
                ...
                #define traceTIMER_CREATE( tmr )               \
                    xTraceObjectRegisterWithoutHandle(         \
                        ...
                        ( void * ) ( tmr ),                    \
                        ( const char * ) ( tmr )->pcTimerName, \
                        ( uint32_t ) ( tmr )->xTimerPeriodInTicks )
                ...
                /* Note that xCommandID can never be tmrCOMMAND_EXECUTE_CALLBACK
                 * (-1) since the trace macro is not called in that case */
                #undef traceTIMER_COMMAND_SEND
                #define traceTIMER_COMMAND_SEND( tmr,                      \
                                                 xCommandID,               \
                                                 xOptionalValue,           \
                ...
                #define traceEVENT_GROUP_CREATE( eg )  \
                    xTraceObjectRegisterWithoutHandle( \
                        PSF_EVENT_EVENTGROUP_CREATE,   \
                        ( void * ) ( eg ),             \
                        0,                             \
                        ( uint32_t ) ( eg )->uxEventBits )

                #undef traceEVENT_GROUP_DELETE
                #define traceEVENT_GROUP_DELETE( eg )    \
                    xTraceObjectUnregisterWithoutHandle( \
                        PSF_EVENT_EVENTGROUP_DELETE,     \
                        ( void * ) ( eg ),               \
                        ( uint32_t ) ( eg )->uxEventBits )
                ...
                #define traceEVENT_GROUP_CREATE_FAILED() \
                    prvTraceStoreEvent_None(             \
                        PSF_EVENT_EVENTGROUP_CREATE_FAILED )
                ...
                #define traceEVENT_GROUP_SYNC_BLOCK( eg,             \
                                                     ...
                                                     bitsToWaitFor ) \
                    prvTraceStoreEvent_HandleParam(                  \
                        PSF_EVENT_EVENTGROUP_SYNC_BLOCK,             \
                        ( void * ) ( eg ),                           \
                        bitsToWaitFor )
                ...
                #define traceEVENT_GROUP_WAIT_BITS_BLOCK( eg, bitsToWaitFor ) \
                    prvTraceStoreEvent_HandleParam(                           \
                        PSF_EVENT_EVENTGROUP_WAITBITS_BLOCK,                  \
                        ( void * ) ( eg ),                                    \
                        bitsToWaitFor )
                ...
                #define traceEVENT_GROUP_CLEAR_BITS( eg, bitsToClear ) \
                    prvTraceStoreEvent_HandleParam(                    \
                        PSF_EVENT_EVENTGROUP_CLEARBITS,                \
                        ( void * ) ( eg ),                             \
                        bitsToClear )
                ...
                #define traceEVENT_GROUP_CLEAR_BITS_FROM_ISR( eg,           \
                                                              bitsToClear ) \
                    prvTraceStoreEvent_HandleParam(                         \
                        PSF_EVENT_EVENTGROUP_CLEARBITS_FROMISR,             \
                        ( void * ) ( eg ),                                  \
                        bitsToClear )
                ...
                #define traceEVENT_GROUP_SET_BITS_FROM_ISR( eg, bitsToSet ) \
                    prvTraceStoreEvent_HandleParam(                         \
                        PSF_EVENT_EVENTGROUP_SETBITS_FROMISR,               \
                        ( void * ) ( eg ),                                  \
                        bitsToSet )
            ...
            #undef traceTASK_NOTIFY
                ...
                #define traceTASK_NOTIFY( index )                     \
                    prvTraceStoreEvent_Handle( PSF_EVENT_TASK_NOTIFY, \
                                               ( void * ) xTaskToNotify )
            ...
            #undef traceTASK_NOTIFY_FROM_ISR
                ...
                #define traceTASK_NOTIFY_FROM_ISR( index )                     \
                    prvTraceStoreEvent_Handle( PSF_EVENT_TASK_NOTIFY_FROM_ISR, \
                                               ( void * ) xTaskToNotify )
            ...
            #undef traceTASK_NOTIFY_WAIT
                ...
                #define traceTASK_NOTIFY_WAIT( index )           \
                    prvTraceStoreEvent_HandleParam(              \
                        pxCurrentTCB->ucNotifyState[ index ] ==  \
                                taskNOTIFICATION_RECEIVED        \
                            ? PSF_EVENT_TASK_NOTIFY_WAIT         \
                            : PSF_EVENT_TASK_NOTIFY_WAIT_FAILED, \
                        ( void * ) pxCurrentTCB,                 \
                        xTicksToWait )
                ...
                #define traceTASK_NOTIFY_WAIT()                  \
                    prvTraceStoreEvent_HandleParam(              \
                        pxCurrentTCB->eNotifyState == eNotified  \
                            ? PSF_EVENT_TASK_NOTIFY_WAIT         \
                            : PSF_EVENT_TASK_NOTIFY_WAIT_FAILED, \
                        ( void * ) pxCurrentTCB,                 \
                        xTicksToWait )
            ...
            #undef traceTASK_NOTIFY_WAIT_BLOCK
                ...
                #define traceTASK_NOTIFY_WAIT_BLOCK( index ) \
                    prvTraceStoreEvent_HandleParam(          \
                        PSF_EVENT_TASK_NOTIFY_WAIT_BLOCK,    \
                        ( void * ) pxCurrentTCB,             \
                        xTicksToWait )
            ...
            #undef traceQUEUE_REGISTRY_ADD
            #define traceQUEUE_REGISTRY_ADD( object, name ) \
                xTraceObjectSetNameWithoutHandle( object,   \
                                                  ( const char * ) ( name ) );
