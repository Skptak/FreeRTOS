 ...
 *TRC_TICK_RATE_HZ. If using a free-running timer, this is often
...
uint32_t uiTraceTimerGetValue( void );
    ...
    #define TRC_HWTC_COUNT \
        ( ( TraceUnsignedBaseType_t ) uiTraceTimerGetValue() )
    #define TRC_HWTC_PERIOD  0
    #define TRC_HWTC_DIVISOR 1
    #define TRC_HWTC_FREQ_HZ \
        ( ( TraceUnsignedBaseType_t ) uiTraceTimerGetFrequency() )
...
void xTraceHardwarePortInitCortexM( void );
        ...
        /* Bit mask for NOPRFCNT bit in DWT_CTRL. If 1, DWT_EXCCNT is not
         * supported */
        #define TRC_DWT_CTRL_NOPRFCNT     ( 1 << 24 )

        /* Bit mask for NOCYCCNT bit in DWT_CTRL. If 1, DWT_CYCCNT is not
         * supported */
        #define TRC_DWT_CTRL_NOCYCCNT     ( 1 << 25 )

        /* Bit mask for EXCEVTENA_ bit in DWT_CTRL. Set to 1 to enable
         * DWT_EXCCNT */
        ...
        #define TRC_HWTC_PERIOD \
            ( ( *( ( volatile uint32_t * ) 0xE000E014 ) ) + 1 )
        #define TRC_HWTC_DIVISOR       4
        #define TRC_HWTC_FREQ_HZ       TRACE_CPU_CLOCK_HZ
        #define TRC_IRQ_PRIORITY_ORDER 0
    ...
    #define TRACE_ENTER_CRITICAL_SECTION()                         \
        {                                                          \
            __x_irq_status = TRC_KERNEL_PORT_SET_INTERRUPT_MASK(); \
        }
    #define TRACE_EXIT_CRITICAL_SECTION()                           \
        {                                                           \
            TRC_KERNEL_PORT_CLEAR_INTERRUPT_MASK( __x_irq_status ); \
        }
...
#elif( TRC_CFG_HARDWARE_PORT == \
       TRC_HARDWARE_PORT_TEXAS_INSTRUMENTS_TMS570_RM48 )
    ...
    #define TRC_HWTC_TYPE TRC_OS_TIMER_INCR
    #define TRC_HWTC_COUNT \
        ( ( uint32_t ) ( AT91C_BASE_PITC->PITC_PIIR & 0xFFFFF ) )
    ...
    #define TRC_HWTC_PERIOD \
        ( ( uint32_t ) ( sysreg_read( AVR32_COMPARE ) + 1 ) )
    #define TRC_HWTC_DIVISOR       1
    #define TRC_HWTC_FREQ_HZ       ( TRC_TICK_RATE_HZ * TRC_HWTC_PERIOD )
    #define TRC_IRQ_PRIORITY_ORDER 1
    ...
    #define TRC_HWTC_PERIOD \
        ( XTmrCtr_GetLoadReg( XPAR_TMRCTR_0_BASEADDR, 0 ) + 1 )
...
extern void cortex_a9_r5_exit_critical( int irq_already_masked_at_enter );
    ...
    #define TRC_HWTC_TYPE TRC_OS_TIMER_INCR
    #define TRC_HWTC_COUNT                                  \
        ( *( volatile uint32_t * ) ( configTIMER_BASEADDR + \
...
extern void cortex_a9_r5_exit_critical( int irq_already_masked_at_enter );
    ...
    /* INPUT YOUR PERIPHERAL BASE ADDRESS HERE (0xF8F00000 for Xilinx Zynq
     * 7000)*/
    ...
    #define TRC_CA9_MPCORE_PRIVCTR_PERIOD_REG                                 \
        ( *( volatile uint32_t * ) ( TRC_CA9_MPCORE_PERIPHERAL_BASE_ADDRESS + \
                                     TRC_CA9_MPCORE_PRIVATE_MEMORY_OFFSET +   \
                                     0x00 ) )
    ...
    used, the number of HW ticks between the trace events gets large, and
    thereby inefficient to store (sometimes extra events are needed). To improve
    efficiency, you may use the TRC_HWTC_DIVISOR as an additional prescaler.
    *****************************************************************************************/
    #define TRC_HWTC_DIVISOR       1
...
static inline uint32_t prvGetCPSR( void )
{
    ...
    #define TRC_IRQ_PRIORITY_ORDER \
        0 // Lower IRQ priority values are more significant

    #define TRC_PORT_SPECIFIC_INIT()

#elif( ( TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_XTensa_LX6 ) || \
       ( TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_XTensa_LX7 ) )
    /**
     * @note	When running with SMP FreeRTOS we cannot use the CCOUNT register
     * for timestamping, instead we use the external 40MHz timer for
     * synchronized timestamping between the cores.
     */
        ...
        #define TRC_HWTC_COUNT                                              \
            ...
            #define TRC_HWTC_FREQ_HZ \
                ( CONFIG_ESP32_DEFAULT_CPU_FREQ_MHZ * 1000000 )
        #elif defined( CONFIG_IDF_TARGET_ESP32S2 )
            #define TRC_HWTC_FREQ_HZ \
                ( CONFIG_ESP32S2_DEFAULT_CPU_FREQ_MHZ * 1000000 )
        #else
...
uint32_t prvGetSMPTimestamp();
        ...
        #error \
            "The XTensa LX6/LX7 trace hardware clock frequency is not defined."
    #endif
    ...
    #define TRC_HWTC_COUNT \
        PIT.TIMER[ configTICK_PIT_CHANNEL ].CVAL.R // must be the PIT channel
                                                   // used for the systick
    #define TRC_HWTC_PERIOD                             \
...
/* If Win32 port */
    ...
    #define TRACE_ALLOC_CRITICAL_SECTION() \
        TRC_KERNEL_PORT_ALLOC_CRITICAL_SECTION()
#endif
#ifndef TRACE_ENTER_CRITICAL_SECTION
    #define TRACE_ENTER_CRITICAL_SECTION() \
        TRC_KERNEL_PORT_ENTER_CRITICAL_SECTION()
#endif
#ifndef TRACE_EXIT_CRITICAL_SECTION
    #define TRACE_EXIT_CRITICAL_SECTION() \
        TRC_KERNEL_PORT_EXIT_CRITICAL_SECTION()
#endif
