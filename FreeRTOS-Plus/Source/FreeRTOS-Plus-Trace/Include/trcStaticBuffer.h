...
{
    TraceCoreStaticBuffer_t
        coreDummyEvents[ TRC_CFG_CORE_COUNT ]; /**< Temporary buffers used for
                                                  event or blob creation. */
} TraceStaticBufferTable_t;

        #define TRC_STATIC_BUFFER_BUFFER_SIZE \
            ( sizeof( TraceStaticBufferTable_t ) )
...
traceResult xTraceStaticBufferGet( void ** ppvBuffer );
            ...
            #define xTraceStaticBufferGet( ppvBuffer )                         \
                ( *ppvBuffer =                                                 \
                      ( void * ) &pxTraceStaticBufferTable                     \
                          ->coreDummyEvents[ TRC_CFG_GET_CURRENT_CORE() ]      \
                          .dummyEvents[ xTraceISRGetCurrentNestingReturned() + \
                                        1 ],                                   \
                  TRC_SUCCESS )
