        ...
        #define xTraceTaskRegister( pvTask, szName, uxPriority, pxTaskHandle ) \
            ( ( ( ( pvTask ) != 0 ) &&                                         \
                ( xTraceObjectRegister(                                        \
                      PSF_EVENT_TASK_CREATE,                                   \
                      pvTask,                                                  \
                      szName,                                                  \
                      uxPriority,                                              \
                      ( TraceObjectHandle_t * ) ( pxTaskHandle ) ) ==          \
                  TRC_SUCCESS ) )                                              \
                  ? ( xTraceStackMonitorAdd( pvTask ), TRC_SUCCESS )           \
                  : TRC_FAIL )
...
traceResult xTraceTaskSetPriority( TraceTaskHandle_t xTaskHandle,
        ...
        #define xTraceTaskRegisterWithoutHandle( pvTask, szName, uxPriority ) \
            ( ( ( ( pvTask ) != 0 ) &&                                        \
                ( xTraceObjectRegisterWithoutHandle( PSF_EVENT_TASK_CREATE,   \
                                                     pvTask,                  \
                                                     szName,                  \
                                                     uxPriority ) ==          \
                  TRC_SUCCESS ) )                                             \
        ...
        #define xTraceTaskUnregisterWithoutHandle( pvTask, uxPriority )     \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                             \
                ...
                xTraceObjectUnregisterWithoutHandle( PSF_EVENT_TASK_DELETE, \
                                                     pvTask,                \
                                                     uxPriority ) )
...
traceResult xTraceTaskReady( void * pvTask );
        #else
            #define xTraceTaskReady( p ) \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2( ( void ) p, TRC_SUCCESS )
        #endif
        ...
        #define xTraceTaskSetCurrent( pvTask )                              \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                             \
                pxTraceTaskInfo                                             \
                    ->coreTasks[ TRC_CFG_GET_CURRENT_CORE() ] = ( pvTask ), \
                TRC_SUCCESS )
        ...
        #define xTraceTaskGetCurrent( ppvTask )                               \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                               \
                *( ppvTask ) = pxTraceTaskInfo                                \
                                   ->coreTasks[ TRC_CFG_GET_CURRENT_CORE() ], \
                TRC_SUCCESS )
