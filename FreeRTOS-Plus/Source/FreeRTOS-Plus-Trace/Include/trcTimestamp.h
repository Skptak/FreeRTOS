 ...
 * @retval TRC_SUCCESS Success
 */
traceResult xTraceTimestampGetFrequency(
...
traceResult xTraceTimestampGetOsTickCount( uint32_t * puiOsTickCount );
             ...
             */
                ...
                #define xTraceTimestampGet( puiTimestamp )                     \
                    TRC_COMMA_EXPR_TO_STATEMENT_EXPR_4(                        \
                        *( puiTimestamp ) = TRC_HWTC_COUNT,                    \
                        ( *( puiTimestamp ) <                                  \
                          pxTraceTimestamp->latestTimestamp )                  \
                            ? pxTraceTimestamp->wraparounds++                  \
                            : 0,                                               \
                        pxTraceTimestamp->latestTimestamp = *( puiTimestamp ), \
                        TRC_SUCCESS )
            #elif( ( TRC_HWTC_TYPE == TRC_FREE_RUNNING_32BIT_DECR ) || \
                ...
                #define xTraceTimestampGet( puiTimestamp )                        \
                    TRC_COMMA_EXPR_TO_STATEMENT_EXPR_4(                           \
                        *( puiTimestamp ) = ( ( TRC_HWTC_COUNT ) &0x00FFFFFFU ) + \
                                            ( ( pxTraceTimestamp->osTickCount &   \
                                                0x000000FFU )                     \
                                              << 24 ),                            \
                        pxTraceTimestamp->wraparounds = pxTraceTimestamp          \
                                                            ->osTickCount,        \
                        pxTraceTimestamp->latestTimestamp = *( puiTimestamp ),    \
                        TRC_SUCCESS )
            #endif
            ...
            #define xTraceTimestampSetPeriod( uiPeriod )                     \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2( pxTraceTimestamp         \
                                                        ->period = uiPeriod, \
                                                    TRC_SUCCESS )
