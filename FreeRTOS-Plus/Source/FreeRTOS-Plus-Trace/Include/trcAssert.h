...
extern "C" {
         ...
         */
            ...
            #error \
                "TRC_CFG_USE_TRACE_ASSERT is not defined. Please define it in trcConfig.h"
        #endif
            ...
            #define TRC_ASSERT( __condition )                   \
                if( !( __condition ) )                          \
                {                                               \
                    prvTraceAssertCreate( __FILE__, __LINE__ ); \
                    return TRC_FAIL;                            \
                }
            ...
            #define TRC_ASSERT_CUSTOM_ON_FAIL_ALWAYS_EVALUATE \
                TRC_ASSERT_CUSTOM_ON_FAIL

            #if( defined( TRC_CFG_TEST_MODE ) && ( TRC_CFG_TEST_MODE ) == 1 )

                /* Asserts that two types have an equal size. Condition passed
                 * to function to avoid compilers warning about unreachable code
                 * due to constant value. */
                #define TRC_ASSERT_EQUAL_SIZE( x, y )               \
                    if( !prvTraceAssertCheckCondition(              \
                            ( TraceBaseType_t ) ( sizeof( x ) ==    \
                                                  sizeof( y ) ) ) ) \
                    {                                               \
                        prvTraceAssertCreate( __FILE__, __LINE__ ); \
                        return TRC_FAIL;                            \
...
/**
 * @brief Inlined condition check to get around some compiler warnings for
 * unused variables.
 *
...
traceResult xTraceAssertGet( TraceStringHandle_t * pxFileNameStringHandle,
            ...
            #define TRC_ASSERT_ALWAYS_EVALUATE( __condition ) \
                ( void ) ( __condition )
...
typedef struct TraceAssertBuffer
{
    uint32_t buffer[ 1 ];
            ...
            #define xTraceAssertInitialize( pxBuffer ) \
                ( ( void ) pxBuffer, TRC_SUCCESS )

            #define xTraceAssertGet( pxFileNameStringHandle, puxLineNumber ) \
                ( ( void ) pxFileNameStringHandle,                           \
                  ( void ) puxLineNumber,                                    \
                  TRC_FAIL )
