 ...
 *
 * This routine stores a name and priority level for an Interrupt Service
 *Routine, to allow for better visualization. Returns a TraceISRHandle_t used by
...
traceResult xTraceISRGetCurrent( TraceISRHandle_t * pxISRHandle );
            ...
            #define xTraceISRGetCurrentNesting( puiValue )                        \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                               \
                    *( puiValue ) = pxTraceISRInfo                                \
                                        ->coreInfos[ TRC_CFG_GET_CURRENT_CORE() ] \
                                        .stackIndex,                              \
                    TRC_SUCCESS )
            ...
            #define xTraceISRGetCurrentNestingReturned()                  \
                ( pxTraceISRInfo->coreInfos[ TRC_CFG_GET_CURRENT_CORE() ] \
                      .stackIndex )
            ...
            #define xTraceISRGetCurrent( pxISRHandle )                          \
                ( xTraceISRGetCurrentNestingReturned() >= 0                     \
                      ? ( *( pxISRHandle ) =                                    \
                              pxTraceISRInfo                                    \
                                  ->coreInfos[ TRC_CFG_GET_CURRENT_CORE() ]     \
                                  .handleStack                                  \
...
/** @internal Deprecated - Provides backwards-compability with older recorders
 * for now, will be removed in the future */
TraceISRHandle_t xTraceSetISRProperties( const char * szName,
                                         uint32_t uiPriority );

        /** @internal Deprecated - Provides backwards-compability with older
         * recorders for now, will be removed in the future */
        #define xTraceGetCurrentISRNesting( puiValue ) \
            xTraceISRGetCurrentNesting( puiValue )
        ...
        /** @internal Deprecated - Provides backwards-compability with older
         * recorders for now, will be removed in the future */
        #define vTraceStoreISREnd( xIsTaskSwitchRequired ) \
            xTraceISREnd( xIsTaskSwitchRequired )
