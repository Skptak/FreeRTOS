...
/**
 * @internal Initializes the internal trace event buffer used by certain stream
 * ports.
 *
...
traceResult xTraceInternalEventBufferClear( void );
            ...
            #define xTraceInternalEventBufferInitialize( puiBuffer, uiSize ) \
                ( ( void ) uiSize, puiBuffer != 0 ? TRC_SUCCESS : TRC_FAIL )
            #define xTraceInternalEventBufferPush( pvData,          \
                                                   uiSize,          \
                                                   piBytesWritten ) \
                ( ( void ) uiSize,                                  \
                  ( void ) piBytesWritten,                          \
                  pvData != 0 ? TRC_SUCCESS : TRC_FAIL )
            #define xTraceInternalEventBufferTransfer( piBytesWritten ) \
                ( ( void ) piBytesWritten, TRC_SUCCESS )
