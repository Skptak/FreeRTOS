 ...
 */
    ...
    #define TRC_ASSERT_VOID( _assert, _err ) \
        if( !( _assert ) )                   \
        {                                    \
            prvTraceError( _err );           \
            return;                          \
    ...
    /* Calls xTraceError if the _assert condition is false. For non-void
    functions, where a return value is to be provided. */
    #define TRC_ASSERT_RET( _assert, _err, _return ) \
        if( !( _assert ) )                           \
        {                                            \
            prvTraceError( _err );                   \
            return _return;                          \
...
typedef TraceISRHandle_t traceHandle;
    ...
    /* Header size */
    #define TRC_HEADER_BUFFER_SIZE                                       \
        ( sizeof( uint32_t ) + sizeof( uint16_t ) + sizeof( uint16_t ) + \
          sizeof( uint32_t ) + sizeof( uint32_t ) + sizeof( uint32_t ) + \
          ( sizeof( char ) * ( TRC_PLATFORM_CFG_LENGTH ) ) +             \
 ...
 *recorder if necessary and waits for a Start command from Tracealyzer ("Start
 ...
 * 		<RTOS init>
 * 		...
 * 		xTraceEnable(TRC_START); // e.g., in task context, at some relevant
 *event
 *
 ...
 *		<Create Queue Q3>
 *
 *		// Only include FilterGroup0 and FilterGroup2, exclude FilterGroup1 (Q1
 *and Q2) from the trace vTraceSetFilterMask( FilterGroup0 | FilterGroup2 );
 *
...
void vTraceSetFilterMask( uint16_t filterMask );
         ...
         * @param[in] value The starting value
         */
        #define TRACE_GET_HIGH16( value ) \
            ( ( uint16_t ) ( ( ( value ) >> 16 ) & 0x0000FFFF ) )
         ...
         */
        #define TRACE_SET_LOW16( current, value ) \
 ...
 *streaming mode you need to use xTraceStringRegister and use the returned
 *TraceStringHandle_t as the argument. In snapshot you simply provide a char* as
 *argument.
 *
...
traceResult xTracePrintF( TraceStringHandle_t chn, const char * fmt, ... );
        #else
            #define xTracePrintF( chn, fmt, ... )                          \
                ( ( void ) ( chn ),                                        \
                  ( void ) ( fmt ),                                        \
                  TRC_SUCCESS ) /* Comma operator is used to avoid "unused \
                                   variable" compiler warnings in a single \
                                   statement */
        #endif
...
traceResult xTraceVPrintF( TraceStringHandle_t eventLabel,
                           ...
                           va_list vl );
        #else
            #define xTraceVPrintF( chn, formatStr, vl )                    \
                ( ( void ) ( chn ),                                        \
                  ( void ) ( formatStr ),                                  \
                  ( void ) ( vl ),                                         \
                  TRC_SUCCESS ) /* Comma operator is used to avoid "unused \
            ...
            ( TRC_CFG_INCLUDE_USER_EVENTS == 1 )
/**
 * @brief A faster version of xTracePrintF, that only allows for logging a
 *string.
 *
 * Example:
 *
...
traceResult xTracePrint( TraceStringHandle_t chn, const char * str );
        #else
            #define xTracePrint( chn, str ) \
                ( ( void ) ( chn ), ( void ) ( str ), TRC_SUCCESS )
        #endif
...
void vTraceClear( void );
        ...
        #define trcKERNEL_HOOKS_TASK_DELETE( SERVICE,                          \
                                             SERVICE_NAME,                     \
                                             SERVICE_PROP,                     \
                                             pxTCB )                           \
            if( TRACE_GET_OBJECT_FILTER( TASK, TRACE_GET_CURRENT_TASK() ) &    \
                ...
                if( TRACE_GET_OBJECT_FILTER( TASK, pxTCB ) &                   \
                    ...
                    prvTraceStoreKernelCall( SERVICE,                          \
                                             TRACE_CLASS_TASK,                 \
                                             TRACE_GET_TASK_NUMBER( pxTCB ) ); \
            prvTraceStoreObjectNameOnCloseEvent( SERVICE_NAME,                 \
                                                 TRACE_GET_TASK_NUMBER(        \
                                                     pxTCB ),                  \
                                                 TRACE_CLASS_TASK );           \
            prvTraceStoreObjectPropertiesOnCloseEvent( SERVICE_PROP,           \
                                                       TRACE_GET_TASK_NUMBER(  \
                                                           pxTCB ),            \
                                                       TRACE_CLASS_TASK );     \
            prvTraceSetPriorityProperty( TRACE_CLASS_TASK,                     \
            ...
            prvTraceFreeObjectHandle( TRACE_CLASS_TASK,                        \
                                      TRACE_GET_TASK_NUMBER( pxTCB ) );
        ...
        #define trcKERNEL_HOOKS_OBJECT_CREATE( SERVICE, CLASS, pxObject )      \
            ...
            prvMarkObjectAsUsed( TRACE_GET_OBJECT_TRACE_CLASS( CLASS,          \
                                                               pxObject ),     \
                                 TRACE_GET_OBJECT_NUMBER( CLASS, pxObject ) ); \
            if( TRACE_GET_OBJECT_FILTER( TASK, TRACE_GET_CURRENT_TASK() ) &    \
                CurrentFilterMask )                                            \
                if( TRACE_GET_OBJECT_FILTER( CLASS, pxObject ) &               \
                    CurrentFilterMask )                                        \
                    prvTraceStoreKernelCall(                                   \
                        SERVICE,                                               \
                        TRACE_GET_OBJECT_TRACE_CLASS( CLASS, pxObject ),       \
                        TRACE_GET_OBJECT_NUMBER( CLASS, pxObject ) );          \
            prvTraceSetObjectState( TRACE_GET_OBJECT_TRACE_CLASS( CLASS,       \
                                                                  pxObject ),  \
                                    TRACE_GET_OBJECT_NUMBER( CLASS,            \
                                                             pxObject ),       \
                                    0 );
        ...
        #define trcKERNEL_HOOKS_OBJECT_DELETE( SERVICE,                         \
                                               SERVICE_NAME,                    \
                                               SERVICE_PROP,                    \
                                               CLASS,                           \
                                               pxObject )                       \
            ...
            prvTraceStoreObjectPropertiesOnCloseEvent(                          \
                ...
                TRACE_GET_OBJECT_NUMBER( CLASS, pxObject ),                     \
                TRACE_GET_OBJECT_TRACE_CLASS( CLASS, pxObject ) );              \
            prvTraceFreeObjectHandle( TRACE_GET_OBJECT_TRACE_CLASS( CLASS,      \
                                                                    pxObject ), \
                                      TRACE_GET_OBJECT_NUMBER( CLASS,           \
                                                               pxObject ) );

        /* This macro will create a call to a kernel service with a certain
         * result, with an object as parameter */
        ...
        /* This macro will create a call to a kernel service with a certain
         * result, with a null object as parameter */
        #undef trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT( SERVICE,     \
                                                            TRACECLASS ) \
        ...
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM( SERVICE,         \
                                                           CLASS,           \
                                                           pxObject,        \
                                                           param )          \
            if( TRACE_GET_OBJECT_FILTER( TASK, TRACE_GET_CURRENT_TASK() ) & \
        ...
        /* This macro will create a call to a kernel service with a certain
         * result, with a null object and other value as parameter */
        ...
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_NULL_OBJECT_WITH_PARAM( \
            ...
            if( TRACE_GET_TASK_FILTER( TRACE_GET_CURRENT_TASK() ) &    \
                CurrentFilterMask )                                    \
                prvTraceStoreKernelCallWithParam( SERVICE,             \
                                                  TRACECLASS,          \
                                                  0,                   \
                                                  param );
        ...
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(     \
            SERVICE,                                                        \
            param )                                                         \
            if( TRACE_GET_OBJECT_FILTER( TASK, TRACE_GET_CURRENT_TASK() ) & \
                CurrentFilterMask )                                         \
                prvTraceStoreKernelCallWithNumericParamOnly( SERVICE,       \
                                                             ( uint32_t )   \
                                                                 param );
        ...
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR( SERVICE,    \
                                                         CLASS,      \
                                                         pxObject )  \
            if( TRACE_GET_OBJECT_FILTER( CLASS, pxObject ) &         \
                CurrentFilterMask )                                  \
        ...
        #define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM_FROM_ISR( SERVICE,  \
                                                                    CLASS,    \
                                                                    pxObject, \
                                                                    param )   \
            if( TRACE_GET_OBJECT_FILTER( CLASS, pxObject ) &                  \
        ...
        #define trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED()                \
            ...
            /* This macro will create an event to indicate that a task became
             * Ready */
        ...
        #define trcKERNEL_HOOKS_INCREMENT_TICK()  \
            ...
            /* This macro will create an event indicating that the OS tick count
             * has increased */
        ...
        /* This macro will create a task switch event to the currently executing
         * task */
        ...
        /* This macro will create an event to indicate that the task has been
         * suspended */
        ...
        /* This macro will create an event to indicate that a task has gotten
         * its priority changed */
        #undef trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE
        #define trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE( SERVICE,               \
                                                      pxTCB,                 \
                                                      uxNewPriority )        \
            if( TRACE_GET_OBJECT_FILTER( TASK, pxTCB ) & CurrentFilterMask ) \
            {                                                                \
...
/**
 * @brief Stores a Kernel Service call event with an Object handle and a numeric
 * parameter
 *
 * @param[in] evtcode Event code
 * @param[in] objectClass Object class
...
void prvTraceStoreKernelCallWithParam( uint32_t evtcode,
            ...
            #define prvTraceStoreKernelCall( eventcode,   \
                                             objectClass, \
                                             byteParam )  \
                {                                         \
                }
            #define prvTraceStoreKernelCallWithNumericParamOnly( evtcode, \
                                                                 param )  \
                {                                                         \
                }
            #define prvTraceStoreKernelCallWithParam( evtcode,      \
                                                      objectClass,  \
                                                      objectNumber, \
                                                      param )       \
                {                                                   \
        ...
        #define TASK_STATE_INSTANCE_ACTIVE     1
            ...
            #define vTraceSetISRProperties( handle, name, priority )         \
                ( void ) ( handle ), ( void ) ( name ),                      \
                    ( void ) ( priority ) /* Comma operator is used to avoid \
            ...
            #define xTraceSetISRProperties( name, priority )                   \
                ( ( void ) ( name ),                                           \
                  ( void ) ( priority ),                                       \
                  ( traceHandle ) 0 ) /* Comma operator in parenthesis is used \
                                         to avoid "unused variable" compiler   \
                                         warnings and return 0 in a single     \
                                         statement */
...
/**
 * @brief Register a channel and fixed format string for use with the separate
 * User Event Buffer functions
 *
 * @param[in] channel Channel name handle
 * @param[in] formatStr Format string that will be used for all events on this
 * channel
 *
...
/**
 * @brief Creates a User Event using the channel, previously set format string
 * and data parameters
 *
...
void vTraceUBEvent( traceUBChannel channel );
            ...
            #define xTraceRegisterChannelFormat( eventLabel, formatStr ) \
                ( ( void ) ( eventLabel ), ( void ) ( formatStr ), 0 )
        ...
        #define trcCRITICAL_SECTION_END()      \
            ...
            #define trcCRITICAL_SECTION_BEGIN_ON_CORTEX_M_ONLY \
                trcCRITICAL_SECTION_BEGIN
            #define trcCRITICAL_SECTION_END_ON_CORTEX_M_ONLY \
...
typedef struct
{
    uint16_t indexOfNextAvailableHandle[ TRACE_NCLASSES ]; /**< For each object
                                                              class, the index
                                                              of the next handle
                                                              to allocate */
    uint16_t lowestIndexOfClass[ TRACE_NCLASSES ]; /**< The lowest index of this
                                                      class (constant) */
    uint16_t highestIndexOfClass[ TRACE_NCLASSES ]; /**< The highest index of
                                                       this class (constant) */
    uint16_t handleCountWaterMarksOfClass[ TRACE_NCLASSES ]; /**< The highest
                                                                use count for
                                                                this class (for
                                                                statistics) */
    traceHandle objectHandles[ TRACE_KERNEL_OBJECT_COUNT ]; /**< The free object
                                                               handles - a set
                                                               of stacks within
                                                               this array */
...
typedef struct
{
    ...
    uint32_t ObjectPropertyTableSizeInBytes; /**<  */
        ...
        #if( TRC_CFG_USE_16BIT_OBJECT_HANDLES == 1 )
    traceHandle
        NumberOfObjectsPerClass[ 2 * ( ( TRACE_NCLASSES + 1 ) / 2 ) ]; /** */
        #else
    traceHandle
        NumberOfObjectsPerClass[ 4 * ( ( TRACE_NCLASSES + 3 ) / 4 ) ]; /** */
        #endif
    ...
    uint8_t TotalPropertyBytesPerClass[ 4 * ( ( TRACE_NCLASSES + 3 ) /
                                              4 ) ]; /**< */
...
typedef struct
{
    ...
    uint16_t addr_low;
        ...
        /*******************************************************************************
         * The separate user event buffer structure. Can be enabled in
         *trcConfig.h.
         ******************************************************************************/
...
typedef struct
{
    ...
    symbolTableType SymbolTable;
            ...
            /* For inclusion of float support, and for endian detection of
            floats. The value should be (float)1 or (uint32_t)0 */
...
traceResult xTraceHeaderInitialize( TraceHeaderBuffer_t * pxBuffer );
        ...
        #define xTraceIsRecorderEnabled()     \
            ( xTraceIsRecorderInitialized() & \
              pxTraceRecorderData->uiRecorderEnabled )
        ...
        #define xTraceSetComponentInitialized( uiComponentBit ) \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                 \
        ...
        #define xTraceIsComponentInitialized( uiComponentBit ) \
            ( ( RecorderInitialized & ( uiComponentBit ) ) ? 1 : 0 )
         ...
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStateSet( uiState )                              \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                        \
                pxTraceRecorderData->uiTraceSystemState = ( uiState ), \
                TRC_SUCCESS )
         ...
         * @retval TRC_SUCCESS Success
         */
        #define xTraceStateGet( puiState )                               \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                          \
                *( puiState ) = pxTraceRecorderData->uiTraceSystemState, \
                TRC_SUCCESS )
...
traceResult xTraceTzCtrl( void );
        ...
        #define prvTraceStoreEvent_None( _eventID )                    \
            {                                                          \
                ...
                if( xTraceEventBegin( _eventID, 0, &_xEventHandle ) == \
                    TRC_SUCCESS )                                      \
                {                                                      \
        ...
        #define prvTraceStoreEvent_Handle( _eventID, _handle )          \
            {                                                           \
                TraceEventHandle_t _xEventHandle = 0;                   \
                if( xTraceEventBegin( _eventID,                         \
                                      sizeof( void * ),                 \
                                      &_xEventHandle ) == TRC_SUCCESS ) \
                {                                                       \
                    xTraceEventAddPointer( _xEventHandle,               \
                                           ( void * ) ( _handle ) );    \
                    xTraceEventEnd( _xEventHandle );                    \
                }                                                       \
            }
        ...
        #define prvTraceStoreEvent_Param( _eventID, _param1 )           \
            {                                                           \
                ...
                if( xTraceEventBegin( _eventID,                         \
                                      ...
                                      &_xEventHandle ) == TRC_SUCCESS ) \
                {                                                       \
                    xTraceEventAdd32( _xEventHandle,                    \
                                      ( uint32_t ) ( _param1 ) );       \
                    xTraceEventEnd( _xEventHandle );                    \
                }                                                       \
            }
...
traceResult xTraceGetEventBuffer( void ** ppvBuffer,
    ...
    #define xTraceStringRegister( x, y )                                       \
        ( ( void ) ( x ),                                                      \
          ( void ) y,                                                          \
          TRC_SUCCESS ) /* Comma operator in parenthesis is used to avoid      \
                           "unused variable" compiler warnings and return 0 in \
                           a single statement */
    #define xTracePrint( chn, ... ) ( ( void ) ( chn ), TRC_SUCCESS )
    #define xTracePrintF( chn, fmt, ... )                                    \
        ( ( void ) ( chn ),                                                  \
          ( void ) ( fmt ),                                                  \
          TRC_SUCCESS ) /* Comma operator is used to avoid "unused variable" \
    ...
    #define xTraceSetISRProperties( a, b )                                     \
        ( ( void ) ( a ),                                                      \
          ( void ) ( b ),                                                      \
          ( traceHandle ) 0 ) /* Comma operator in parenthesis is used to      \
                                 avoid "unused variable" compiler warnings and \
                                 return 0 in a single statement */
    #define xTraceRegisterChannelFormat( eventLabel, formatStr )        \
        ( ( void ) ( eventLabel ),                                      \
          ( void ) ( formatStr ),                                       \
          0 ) /* Comma operator in parenthesis is used to avoid "unused \
...
/**
 * @deprecated Backwards compatibility. Use xTraceTaskInstanceFinishedNow
 * instead.
 */
#define vTraceInstanceFinishedNow  ( void ) xTraceTaskInstanceFinishedNow

/**
 * @deprecated Backwards compatibility. Use xTraceTaskInstanceFinishedNext
 * instead.
 */
...
#define vTraceSetRecorderDataBuffer( pxBuffer ) \
    xTraceSetBuffer( ( TraceRecorderDataBuffer_t * ) ( pxBuffer ) )
