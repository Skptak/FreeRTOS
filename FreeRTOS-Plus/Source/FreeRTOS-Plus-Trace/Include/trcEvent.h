...
extern "C" {
         ...
         */
        #define TRC_EVENT_SET_PARAM_COUNT( id, n ) \
            ( ( ( uint16_t ) ( id ) ) |            \
              ( ( ( ( uint16_t ) ( n ) ) & 0xF ) << 12 ) )
        ...
        #define TRC_EVENT_GET_PARAM_COUNT( id ) ( ( ( id ) >> 12 ) & 0xF )
            ...
            #define TRC_EVENT_SET_EVENT_COUNT( c )                 \
                ( ( ( TRC_CFG_GET_CURRENT_CORE() & 0xF ) << 12 ) | \
                  ( ( uint16_t ) ( c ) &0xFFF ) )
        #else
        ...
        #define SET_BASE_EVENT_DATA( pxEvent, eventId, paramCount, eventCount ) \
            ( ( pxEvent )->EventID = TRC_EVENT_SET_PARAM_COUNT( eventId,        \
                                                                paramCount ),   \
              ( pxEvent )->EventCount = TRC_EVENT_SET_EVENT_COUNT(              \
                  eventCount ),                                                 \
        ...
        #define RESET_EVENT_DATA( p ) \
            ( ( p )->pvBlob = 0, ( p )->size = 0, ( p )->offset = 0 )

        /**
         * @internal Macro optimization for getting trace event size.
         */
        #define TRC_EVENT_GET_SIZE( pvAddress, puiSize )                      \
            ( *( uint32_t * ) ( puiSize ) =                                   \
                  sizeof( TraceBaseEvent_t ) +                                \
                  ( TRC_EVENT_GET_PARAM_COUNT(                                \
                      ( ( TraceBaseEvent_t * ) ( pvAddress ) )->EventID ) ) * \
                      sizeof( uint32_t ),                                     \
              TRC_SUCCESS )

        /**
         * @internal Macro optimization for getting trace event data pointer
         * with an offset.
         */
        #define TRC_EVENT_GET_RAW_DATA( xEventHandle,                       \
                                        uiOffset,                           \
                                        uiSize,                             \
                                        ppvData )                           \
            ( ( void ) ( uiSize ),                                          \
              *( void ** ) ( ppvData ) = ( void * ) &(                      \
                  ( uint8_t * ) ( ( TraceEventData_t * ) ( xEventHandle ) ) \
                      ->pvBlob )[ uiOffset ],                               \
              TRC_SUCCESS )

        /**
         * @internal Macro optimization for getting trace event payload pointer
         * with an offset.
         */
        #define TRC_EVENT_GET_PAYLOAD( xEventHandle,                           \
                                       uiOffset,                               \
                                       uiSize,                                 \
                                       ppvData )                               \
            ( ( void ) ( uiSize ),                                             \
              *( void ** ) ( ppvData ) = ( void * ) &(                         \
                  ( uint8_t * ) ( ( TraceEventData_t * ) ( xEventHandle ) )    \
                      ->pvBlob )[ sizeof( TraceBaseEvent_t ) + ( uiOffset ) ], \
              TRC_SUCCESS )

        /**
         * @internal Macro optimization for getting trace event remaining
         * payload size.
         */
        #define TRC_EVENT_PAYLOAD_REMAINING( xEventHandle, puiValue )    \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                          \
                *( uint32_t * ) ( puiValue ) =                           \
                    ( ( TraceEventData_t * ) ( xEventHandle ) )->size -  \
                    ( ( TraceEventData_t * ) ( xEventHandle ) )->offset, \
                TRC_SUCCESS )

        /**
         * @internal Macro optimization for getting trace event used payload
         * size.
         */
        #define TRC_EVENT_PAYLOAD_USED( xEventHandle, puiValue )        \
            ( *( uint32_t * ) ( puiValue ) =                            \
                  ( ( TraceEventData_t * ) ( xEventHandle ) )->offset - \
                  sizeof( TraceBaseEvent_t ),                           \
              TRC_SUCCESS )
         ...
         */
        #define TRC_EVENT_PAYLOAD_SIZE( xEventHandle, puiValue )      \
            ( *( uint32_t * ) ( puiValue ) =                          \
                  ( ( TraceEventData_t * ) ( xEventHandle ) )->size - \
                  sizeof( TraceBaseEvent_t ),                         \
              TRC_SUCCESS )
        ...
        /**
         * @internal Macro optimization for adding a pointer address to trace
         * event.
         */
        #define TRC_EVENT_ADD_POINTER( xEventHandle, value )                   \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_3(                                \
                ( ( void ** ) ( ( TraceEventData_t * ) ( xEventHandle ) )      \
                      ->pvBlob )[ ( ( TraceEventData_t * ) ( xEventHandle ) )  \
                                      ->offset /                               \
                                  sizeof( void * ) ] = ( value ),              \
                ( ( TraceEventData_t * ) ( xEventHandle ) )->offset += sizeof( \
                    void * ),                                                  \
                TRC_SUCCESS )

        /**
         * @internal Macro optimization for adding a unsigned base type to trace
         * event.
         */
        #define TRC_EVENT_ADD_UNSIGNED_BASE_TYPE( xEventHandle, value )                     \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_3(                                             \
                ( ( TraceUnsignedBaseType_t * ) ( ( TraceEventData_t * ) ( xEventHandle ) ) \
                      ->pvBlob )                                                            \
                    [ ( ( TraceEventData_t * ) ( xEventHandle ) )->offset /                 \
                      sizeof( TraceUnsignedBaseType_t ) ] = ( value ),                      \
                ( ( TraceEventData_t * ) ( xEventHandle ) )->offset += sizeof(              \
                    TraceUnsignedBaseType_t ),                                              \
                TRC_SUCCESS )

        /**
         * @internal Macro optimization for adding a 32-bit value to trace
         * event.
         */
        #define TRC_EVENT_ADD_32( xEventHandle, value )                        \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_3(                                \
                ( ( uint32_t * ) ( ( TraceEventData_t * ) ( xEventHandle ) )   \
                      ->pvBlob )[ ( ( TraceEventData_t * ) ( xEventHandle ) )  \
                                      ->offset /                               \
        ...
        /**
         * @internal Macro optimization for adding a 16-bit value to trace
         * event.
         */
        #define TRC_EVENT_ADD_16( xEventHandle, value )                        \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_3(                                \
                ( ( uint16_t * ) ( ( TraceEventData_t * ) ( xEventHandle ) )   \
                      ->pvBlob )[ ( ( TraceEventData_t * ) ( xEventHandle ) )  \
                                      ->offset /                               \
         ...
         */
        #define TRC_EVENT_BEGIN_OFFLINE( uiEventCode,                          \
                                         uiPayloadSize,                        \
                                         pxEventHandle )                       \
            ( ( xTraceEventBeginRawOffline( sizeof( TraceBaseEvent_t ) +       \
                                                ( uiPayloadSize ),             \
                                            pxEventHandle ) ) == TRC_SUCCESS   \
                  ? ( pxTraceEventDataTable                                    \
                          ->coreEventData[ TRC_CFG_GET_CURRENT_CORE() ]        \
                          .eventCounter++,                                     \
                      SET_BASE_EVENT_DATA(                                     \
                          ( TraceBaseEvent_t * ) ( ( ( TraceEventData_t * ) *( \
                                                         pxEventHandle ) )     \
                                                       ->pvBlob ),             \
                          uiEventCode,                                         \
                          ( ( ( TraceEventData_t * ) *( pxEventHandle ) )      \
                                ->size -                                       \
                            sizeof( TraceBaseEvent_t ) ) /                     \
                              sizeof( uint32_t ),                              \
                          pxTraceEventDataTable                                \
                              ...
                              .eventCounter ),                                 \
                      ( ( TraceEventData_t * ) *( pxEventHandle ) )            \
                          ->offset += sizeof( TraceBaseEvent_t ),              \
                      TRC_SUCCESS )                                            \
                  : TRC_FAIL )
        ...
        #define TRC_EVENT_END_OFFLINE( xEventHandle )                        \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_4(                              \
                xTraceStreamPortCommit(                                      \
                    ( ( TraceEventData_t * ) ( xEventHandle ) )->pvBlob,     \
                    ( ( TraceEventData_t * ) ( xEventHandle ) )->size,       \
                    &DUMMY_iTraceBytesCommitted ),                           \
...
{
    TraceCoreEventData_t coreEventData[ TRC_CFG_CORE_COUNT ]; /**< Holds data
                                                                 about current
                                                                 event for each
                                                                 core/isr depth
                                                               */
 ...
 *
 * This routine begins a trace event with specified size. Must call
 * xTraceEventEnd() to finalize event creation. Does not care about
 * RecorderEnabled.
 *
 ...
 * @retval TRC_SUCCESS Success
 */
traceResult xTraceEventBeginRawOfflineBlocking(
    uint32_t uiSize,
    TraceEventHandle_t * pxEventHandle );
        ...
        #define xTraceEventBegin( uiEventCode,                   \
            ...
            ( xTraceIsRecorderEnabled()                          \
                  ? xTraceEventBeginOffline( uiEventCode,        \
                                             uiTotalPayloadSize, \
                                             pxEventHandle )     \
                  : TRC_FAIL )
...
traceResult xTraceEventEndOfflineBlocking( TraceEventHandle_t xEventHandle );
         ...
         *
         * This routine ends the event that was begun by calling on
         * xTraceEventBegin(). Does not care about uiRecorderEnabled.
         *
         * @param[in] xEventHandle Pointer to initialized trace event.
         *
         * @retval TRC_FAIL Failure
 ...
 *
 * This routine gets a trace event payload pointer with an offset. It also
 * verifies that the size so it won't go outside its payload buffer.
 *
 * @param[in] xEventHandle Pointer to initialized trace event.
 * @param[in] uiOffset Offset.
 * @param[in] uiSize Size.
...
traceResult xTraceEventAdd8( TraceEventHandle_t xEventHandle, uint8_t value );
             ...
             * @brief Gets trace event data pointer with an offset.
             *
             * This routine gets a trace event data pointer with an offset. It
             * also verfies that the size so it won't go outside its buffer.
             *
             * @param[in] xEventHandle Pointer to initialized trace event.
             * @param[in] uiOffset Offset.
             * @param[in] uiSize Size.
             ...
             * @brief Gets trace event payload pointer with an offset.
             *
             * This routine gets a trace event payload pointer with an offset.
             * It also verifies that the size so it won't go outside its payload
             * buffer.
             *
             * @param[in] xEventHandle Pointer to initialized trace event.
             * @param[in] uiOffset Offset.
            ...
            #define xTraceEventAddUnsignedBaseType \
                TRC_EVENT_ADD_UNSIGNED_BASE_TYPE
