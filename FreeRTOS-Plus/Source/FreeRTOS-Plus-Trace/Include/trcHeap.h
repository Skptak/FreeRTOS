...
traceResult xTraceHeapGetMax( TraceHeapHandle_t xHeapHandle,
            ...
            #define xTraceHeapCreate( szName,          \
                                      uxCurrent,       \
                                      uxHighWaterMark, \
                                      uxMax,           \
                                      pxHeapHandle )   \
                ( ( void ) szName,                     \
                  ...
                  ( void ) uxMax,                      \
                  pxHeapHandle != 0 ? TRC_SUCCESS : TRC_FAIL )

            #define xTraceHeapAlloc( xHeapHandle, pvAddress, uxSize ) \
                ( ( void ) xHeapHandle,                               \
                  ( void ) pvAddress,                                 \
                  ( void ) uxSize,                                    \
                  TRC_SUCCESS )

            #define xTraceHeapFree( xHeapHandle, pvAddress, uxSize ) \
                ( ( void ) xHeapHandle,                              \
                  ( void ) pvAddress,                                \
                  ( void ) uxSize,                                   \
                  TRC_SUCCESS )
            ...
            #define xTraceHeapGetCurrent( xHeapHandle, puxCurrent ) \
                ( ( void ) xHeapHandle,                             \
                  puxCurrent != 0 ? * puxCurrent = 0 : 0,           \
                  puxCurrent != 0 ? TRC_SUCCESS : TRC_FAIL )

            #define xTraceHeapGetHighWaterMark( xHeapHandle,          \
                                                puxHighWaterMark )    \
                ( ( void ) xHeapHandle,                               \
                  puxHighWaterMark != 0 ? * puxHighWaterMark = 0 : 0, \
                  puxHighWaterMark != 0 ? TRC_SUCCESS : TRC_FAIL )

            #define xTraceHeapGetMax( xHeapHandle, puxMax ) \
                ( ( void ) xHeapHandle,                     \
                  puxMax != 0 ? * puxMax = 0 : 0,           \
                  puxMax != 0 ? TRC_SUCCESS : TRC_FAIL )
