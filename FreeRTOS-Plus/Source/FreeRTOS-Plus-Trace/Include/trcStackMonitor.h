...
extern "C" {
         ...
         */
            ...
            #define TRACE_STACK_MONITOR_BUFFER_SIZE                          \
...
traceResult xTraceStackMonitorReport( void );
        ...
        #else /* (((TRC_CFG_ENABLE_STACK_MONITOR) == 1) && \
                 ((TRC_CFG_SCHEDULING_ONLY) == 0)) */
...
typedef struct TraceStackMonitorBuffer
{
    uint32_t buffer[ 1 ];
            ...
            #define xTraceStackMonitorInitialize( pxBuffer ) \
                ( ( void ) pxBuffer, TRC_SUCCESS )

            #define xTraceStackMonitorDiagnosticsGet( xType, puiValue ) \
                ( ( void ) xType,                                       \
                  puiValue != 0 ? * puiValue = 0 : 0,                   \
                  puiValue != 0 ? TRC_SUCCESS : TRC_FAIL )

            #define xTraceStackMonitorDiagnosticsSet( xType, uiValue ) \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_3( ( void ) xType,    \
                                                    ( void ) uiValue,  \
                                                    TRC_SUCCESS )

            #define xTraceStackMonitorAdd( pvTask )                  \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2( ( void ) pvTask, \
                                                    TRC_SUCCESS )

            #define xTraceStackMonitorRemove( pvTask )               \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2( ( void ) pvTask, \
                                                    TRC_SUCCESS )

            #define xTraceStackMonitorGetAtIndex( uiIndex,                    \
                                                  ppvTask,                    \
                                                  puxLowWaterMark )           \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_4( ( void ) uiIndex,         \
                                                    ( void ) ppvTask,         \
                                                    ( void ) puxLowWaterMark, \
                                                    TRC_SUCCESS )

            #define xTraceStackMonitorReport() \
                TRC_COMMA_EXPR_TO_STATEMENT_EXPR_1( TRC_SUCCESS )
