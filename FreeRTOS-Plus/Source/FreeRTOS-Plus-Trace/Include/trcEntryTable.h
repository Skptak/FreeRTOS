...
extern "C" {
        ...
        #define TRC_ENTRY_CREATE_WITH_ADDRESS( _pvAddress, _pxEntryHandle ) \
            ( xTraceEntryCreate( _pxEntryHandle ) == TRC_SUCCESS            \
                  ? ( ( ( TraceEntry_t * ) *( _pxEntryHandle ) )            \
                          ->pvAddress = ( _pvAddress ),                     \
                      TRC_SUCCESS )                                         \
                  : TRC_FAIL )
        #define TRC_ENTRY_SET_STATE( xEntryHandle, uiStateIndex, uxState ) \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                            \
                ( ( TraceEntry_t * ) ( xEntryHandle ) )                    \
                    ->xStates[ uiStateIndex ] = ( uxState ),               \
                TRC_SUCCESS )
        #define TRC_ENTRY_SET_OPTIONS( xEntryHandle, uiMask ) \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(               \
                ( ( TraceEntry_t * ) ( xEntryHandle ) )       \
                    ->uiOptions |= ( uiMask ),                \
                TRC_SUCCESS )
        #define TRC_ENTRY_CLEAR_OPTIONS( xEntryHandle, uiMask )          \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                          \
                ( ( TraceEntry_t * ) ( xEntryHandle ) )->uiOptions &= ~( \
                    uiMask ),                                            \
                TRC_SUCCESS )
        #define TRC_ENTRY_GET_ADDRESS( xEntryHandle, ppvAddress )         \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                           \
                *( ppvAddress ) = ( ( TraceEntry_t * ) ( xEntryHandle ) ) \
                                      ->pvAddress,                        \
                TRC_SUCCESS )
        #define TRC_ENTRY_GET_SYMBOL( xEntryHandle, pszSymbol )          \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                          \
                *( pszSymbol ) = ( ( TraceEntry_t * ) ( xEntryHandle ) ) \
                                     ->szSymbol,                         \
                TRC_SUCCESS )
        #define TRC_ENTRY_GET_STATE( xEntryHandle, uiStateIndex, puxState ) \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                             \
                *( puxState ) = ( ( TraceEntry_t * ) ( xEntryHandle ) )     \
                                    ->xStates[ uiStateIndex ],              \
                TRC_SUCCESS )
        #define TRC_ENTRY_GET_OPTIONS( xEntryHandle, puiOptions )         \
            TRC_COMMA_EXPR_TO_STATEMENT_EXPR_2(                           \
                *( puiOptions ) = ( ( TraceEntry_t * ) ( xEntryHandle ) ) \
                                      ->uiOptions,                        \
                TRC_SUCCESS )

        #define TRC_ENTRY_TABLE_SLOTS       ( TRC_CFG_ENTRY_SLOTS )
        #define TRC_ENTRY_TABLE_STATE_COUNT ( 3 )
        #define TRC_ENTRY_TABLE_SYMBOL_LENGTH \
            ( TRC_CFG_ENTRY_SYMBOL_MAX_LENGTH )
        #define TRC_ENTRY_TABLE_SLOT_SYMBOL_SIZE                       \
            ( ( ( ( sizeof( char ) * TRC_ENTRY_TABLE_SYMBOL_LENGTH ) + \
                  ( sizeof( uint32_t ) - 1 ) ) /                       \
                sizeof( uint32_t ) ) *                                 \
              sizeof( uint32_t ) )
...
typedef struct TraceEntry
{
    ...
    char szSymbol[ TRC_ENTRY_TABLE_SLOT_SYMBOL_SIZE ];              /**< */
        ...
        #define TRC_ENTRY_TABLE_SIZE                                         \
            ( sizeof( uint32_t ) + sizeof( uint32_t ) + sizeof( uint32_t ) + \
              ( sizeof( TraceEntry_t ) * ( TRC_ENTRY_TABLE_SLOTS ) ) )
