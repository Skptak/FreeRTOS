...
#include <trcRecorder.h>
        ...
        #include <stdarg.h>
            ...
            #error \
                "CUSTOM timestamping mode is not (yet) supported in snapshot mode!"
        #endif
...
static int readyEventsEnabled = 1;
        #endif /*!defined TRC_CFG_INCLUDE_READY_EVENTS || \
                  TRC_CFG_INCLUDE_READY_EVENTS == 1*/
 ...
 * The stacks are empty initially, meaning that allocation of new handles will
 *be based on a counter (for each object class). Any delete operation will
...
int tasksNotIncluded TRC_CFG_RECORDER_DATA_ATTRIBUTE;
         ...
         * RecorderData
         *
         * The main data structure in snapshot mode, when using the default
         *static memory allocation (TRC_RECORDER_BUFFER_ALLOCATION_STATIC). The
         *recorder uses a pointer RecorderDataPtr to access the data, to also
         *allow for dynamic or custom data allocation (see
...
static TraceStringHandle_t prvTraceOpenSymbol(
    const char * name,
...
static TraceStringHandle_t prvTraceCreateSymbolTableEntry(
    const char * name,
    uint8_t crc6,
    uint8_t len,
    TraceStringHandle_t channel );

static TraceStringHandle_t prvTraceLookupSymbolTableEntry(
    const char * name,
    uint8_t crc6,
    uint8_t len,
    TraceStringHandle_t channel );
...
traceResult xTraceEnable( uint32_t uiStartOption )
{
    ...
    {
        prvTraceError(
            "xTraceEnable(TRC_START_AWAIT_HOST) not allowed in Snapshot mode" );
    ...
    {
        prvTraceError(
            "xTraceEnable(TRC_START_FROM_HOST) not allowed in Snapshot mode" );
...
void vTraceClear( void )
{
    ...
    ( void ) memset( RecorderDataPtr->eventData,
                     0,
                     RecorderDataPtr->maxEvents * 4 );
...
static void prvTraceStart( void )
{
    ...
    {
        TRACE_ASSERT( RecorderDataPtr != 0,
                      "Recorder not initialized. Use vTraceEnable() instead!",
                      TRC_UNUSED );
        return;
    }
    ...
    if( traceErrorMessage == 0 )
    {
        ...
        prvTraceStoreTaskswitch( handle ); /* Register the currently running
                                              task */
        trcCRITICAL_SECTION_END();
    }
}
 ...
 *		The first instance ends at xTraceTaskInstanceFinishedNow(), while the
...
traceHandle xTraceSetISRProperties( const char * name, uint8_t priority )
{
    ...
    TRACE_ASSERT( handle <= RecorderDataPtr->ObjectPropertyTable
                  ...
                  "xTraceSetISRProperties: Invalid value for handle",
                  0 );
...
void vTraceStoreISRBegin( traceHandle handle )
{
    ...
    if( recorder_busy )
    {
         ...
         *************************************************************************/
        prvTraceError(
            "vTraceStoreISRBegin - recorder busy! See code comment." );
        return;
    }
    trcCRITICAL_SECTION_BEGIN();
    ...
    if( RecorderDataPtr->recorderActive && handle_of_last_logged_task )
    {
        ...
        TRACE_ASSERT( handle != 0,
                      "vTraceStoreISRBegin: Invalid ISR handle (NULL)",
                      TRC_UNUSED );
        TRACE_ASSERT( handle <= RecorderDataPtr->ObjectPropertyTable
                                    .NumberOfObjectsPerClass[ TRACE_CLASS_ISR ],
...
void vTraceStoreISREnd( int pendingISR )
{
    ...
    trcCRITICAL_SECTION_BEGIN();
    isPendingContextSwitch |= pendingISR; /* Is there a pending context switch
                                             right now? If so, we will not
                                             create an event since we will get
                                             an event when that context switch
                                             is executed. */
    nISRactive--;
    if( nISRactive > 0 )
    {
        ...
        hnd8 = prvTraceGet8BitHandle(
            isrstack[ nISRactive - 1 ] ); /* isrstack[nISRactive] is the handle
                                             of the ISR we're currently exiting.
                                             isrstack[nISRactive - 1] is the
                                             handle of the ISR that was
                                        ...
                                        uint8_t byteOffset )
{
    ...
    while( formatStr[ formatStrIndex ] != '\0' )
    {
            ...
            if( formatStr[ formatStrIndex ] != '\0' )
            {
                switch( formatStr[ formatStrIndex ] )
                {
                    ...
                    case 's':
                    {
                        ...
                        xTraceStringRegister( ( char * ) va_arg( vl, char * ),
                                              &xString );

                        i = writeInt16( buffer, i, ( uint16_t ) xString );
                    }
                    break;
                    ...
                    case 'l':
                        ...
                        switch( formatStr[ formatStrIndex ] )
                        {
                            ...
                            va_arg and i consistent. */
                            case 'f':
                                i = writeInt32( buffer, /* In this case, the
                                                           value will not be
                                                           shown anyway */
                                                i,
                                                ( uint32_t ) va_arg( vl,
                                ...
                                i = writeInt32( buffer, /* Do it twice, to write
                                                           in total 8 bytes */
                                                i,
                                                ( uint32_t ) va_arg( vl,
                                                                     double ) );
                ...
                {
                    prvTraceError(
                        "xTracePrintF - Too many arguments, max 15 allowed!" );
                    return 0;
                }
            }
            else
        ...
        {
            prvTraceError(
                "xTracePrintF - Too large arguments, max 32 byte allowed!" );
            return 0;
        }
    }
...
static void prvTraceClearChannelBuffer( uint32_t count )
{
    ...
    TRACE_ASSERT( ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ) >= count,
                  "prvTraceClearChannelBuffer: "
                  "TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is too small to "
                  "handle this event.",
                  TRC_UNUSED );
        ...
        ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ) )
    {
        slots = ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ) -RecorderDataPtr
                    ->userEventBuffer.nextSlotToWrite; /* Number of slots before
                                                          end of buffer */
        ( void ) memset( &RecorderDataPtr->userEventBuffer
                              .channelBuffer[ RecorderDataPtr->userEventBuffer
                                                  .nextSlotToWrite ],
                         0,
                         slots );
        ( void ) memset( &RecorderDataPtr->userEventBuffer.channelBuffer[ 0 ],
                         0,
                         ( count - slots ) );
    }
    else
         ...
         *
         * Copies a number of items to the data buffer, starting from
         *nextSlotToWrite.
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) &&     \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) && \
...
static void prvTraceCopyToDataBuffer( uint32_t * data, uint32_t count )
{
        ...
        ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ) )
    {
        ...
        ( void ) memcpy(
            &RecorderDataPtr->userEventBuffer
                 .dataBuffer[ RecorderDataPtr->userEventBuffer.nextSlotToWrite *
                              4 ],
            data,
            slots * 4 );
        ( void ) memcpy( &RecorderDataPtr->userEventBuffer.dataBuffer[ 0 ],
                         data + slots,
                         ( count - slots ) * 4 );
    }
    else
    {
         ...
         * prvTraceUBHelper1
         *
         * Calls on prvTraceUserEventFormat() to do the actual formatting, then
         *goes on to the next helper function.
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) &&     \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) && \
             ( TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1 ) )
                               ...
                               va_list vl )
{
    ...
    if( channel == 0 )
    {
        ...
        byteOffset = ( uint8_t ) ( byteOffset + 4 ); /* Also need room for
                                                        channel and format */
        ( ( uint16_t * ) data )[ 2 ] = eventLabel;
        ( ( uint16_t * ) data )[ 3 ] = formatLabel;
    }

    noOfSlots = prvTraceUserEventFormat( ( char * ) &(
                                             RecorderDataPtr->SymbolTable
                                                 .symbytes[ formatLabel + 4 ] ),
                                         vl,
                                         ( uint8_t * ) data,
                                         byteOffset );
    ...
    prvTraceUBHelper2( channel, data, noOfSlots );
         ...
         * prvTraceUBHelper2
         *
         * This function simply copies the data buffer to the actual user event
         *buffer.
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) &&     \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) && \
                               ...
                               uint32_t noOfSlots )
{
    ...
    TRACE_ASSERT( ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ) >= noOfSlots,
                  "prvTraceUBHelper2: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE "
    ...
    prvTraceCopyToDataBuffer( data, noOfSlots ); /* Will wrap around the data if
                                                    necessary */

    old_nextSlotToWrite = RecorderDataPtr->userEventBuffer
                              .nextSlotToWrite; /* Save the index that we want
                                                   to write the channel data at
                                                   when we're done */
    RecorderDataPtr->userEventBuffer
        .nextSlotToWrite = ( RecorderDataPtr->userEventBuffer.nextSlotToWrite +
                             noOfSlots ) %
                           ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE ); /* Make
                                                                           sure
                                                                           we
                                                                           never
                                                                           end
    ...
    {
        RecorderDataPtr->userEventBuffer
            .channelBuffer[ old_nextSlotToWrite ] = channel;
    }
    else
    {
        /* 0xFF indicates that this is not a normal channel id */
        RecorderDataPtr->userEventBuffer
            .channelBuffer[ old_nextSlotToWrite ] = ( traceUBChannel ) 0xFF;
    }
    trcCRITICAL_SECTION_END();
}
        #endif
         ...
         *
         * Registers a channel for Separated User Events, i.e., those stored in
         *the separate user event buffer.
         *
         * Note: Only available if TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER is
                                        ...
                                        TraceStringHandle_t formatStr )
{
    ...
    TRACE_ASSERT( formatStr != 0,
                  "xTraceRegisterChannelFormat: formatStr == 0",
                  ( traceUBChannel ) 0 );

    trcCRITICAL_SECTION_BEGIN();
    for( i = 1; i <= ( TRC_CFG_UB_CHANNELS );
         i++ ) /* Size of the channels buffer is TRC_CFG_UB_CHANNELS + 1. Index
                  0 is unused. */
    {
            ...
            RecorderDataPtr->userEventBuffer.channels[ i ].defaultFormat == 0 )
        {
            ...
            RecorderDataPtr->userEventBuffer.channels[ i ]
    ...
    return retVal;
         ...
         *TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER is enabled in
         *trcSnapshotConfig.h
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) &&     \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) && \
...
void vTraceUBData( traceUBChannel channelPair, ... )
{
    ...
    TRACE_ASSERT( channelPair != 0,
                  "vTraceUBData: Not a valid traceUBChannel!",
                  TRC_UNUSED );
    ...
    va_end( vl );
        ...
        /* Extracts the channel name and format string from the traceUBChannel,
         * then calls prvTraceUBHelper1. */
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) &&     \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) && \
             ( TRC_CFG_USE_SEPARATE_USER_EVENT_BUFFER == 1 ) )
void vTraceUBData_Helper( traceUBChannel channelPair, va_list vl )
{
    ...
    TRACE_ASSERT( channelPair != 0,
                  "vTraceUBData_Helper: channelPair == 0",
                  TRC_UNUSED );
    ...
    formatStr = RecorderDataPtr->userEventBuffer.channels[ channelPair ]
                    .defaultFormat;
...
void vTraceUBEvent( traceUBChannel channelPair )
{
    ...
    TRACE_ASSERT( channelPair != 0,
                  "vTraceUBEvent: channelPair == 0",
                  TRC_UNUSED );
    TRACE_ASSERT( channelPair <= ( TRC_CFG_UB_CHANNELS ),
                  "vTraceUBEvent: ",
                  TRC_UNUSED );

    prvTraceUBHelper2( channelPair, data, 1 ); /* Only need one slot for
                                                  timestamp */
}
        #endif
         ...
         * xTracePrintF
         *
         * Generates User Event with formatted text and data, similar to a
         *"printf". It is very fast compared to a normal "printf" since this
         *function only stores the arguments. The actual formatting is done on
         *the host PC when the trace is displayed in the viewer tool.
         *
         ...
         *
         * Calling xTraceStringRegister multiple times will not create duplicate
         *entries, but it is of course faster to just do it once, and then keep
         *the handle for later use. If you don't have any data arguments, only a
         *text label/string, it is better to use xTracePrint - it is faster.
         *
         ...
         *
         * Up to 15 data arguments are allowed, with a total size of maximum 32
         *byte. In case this is exceeded, the user event is changed into an
         *error message.
         *
         * The data is stored in trace buffer, and is packed to allow storing
         *multiple smaller data entries in the same 4-byte record, e.g., four
         *8-bit values. A string requires two bytes, as the symbol table is
         *limited to 64K. Storing a double (%lf) uses two records, so this is
         *quite costly. Use float (%f) unless the higher precision is really
         *necessary.
         *
         * Note that the double-precision float (%lf) assumes a 64 bit double
         * representation. This does not seem to be the case on e.g. PIC24 and
         *PIC32. Before using a %lf argument on a 16-bit MCU, please verify that
         * "sizeof(double)" actually gives 8 as expected. If not, use %f
         *instead.
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) && \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) )
                           ...
                           va_list vl )
{
    ...
    if( RecorderDataPtr->recorderActive && handle_of_last_logged_task )
    {
        ...
        noOfSlots = prvTraceUserEventFormat( formatStr,
                                             vl,
        ...
        if( RecorderDataPtr->recorderActive )
        {
            ...
            if( RecorderDataPtr->recorderActive )
            {
                /* Check that the buffer to be overwritten does not contain any
                user events that would be partially overwritten. If so, they
                must be "killed" by replacing the user event and following data
                with NULL events (i.e., using a memset to zero).*/
                #if( TRC_CFG_SNAPSHOT_MODE == TRC_SNAPSHOT_MODE_RING_BUFFER )
                ...
                RecorderDataPtr->eventData[ RecorderDataPtr->nextFreeIndex *
                                            4 ] = ( uint8_t ) ( USER_EVENT +
                                                                noOfSlots - 1 );
    ...
    return TRC_SUCCESS;
         ...
         * Basic user event
         *
         * Generates a User Event with a text label. The label is created/looked
         *up in the symbol table using xTraceStringRegister.
         ******************************************************************************/
        #if( ( TRC_CFG_SCHEDULING_ONLY == 0 ) && \
             ( TRC_CFG_INCLUDE_USER_EVENTS == 1 ) )
traceResult xTracePrint( TraceStringHandle_t chn, const char * str )
{
    ...
    return TRC_SUCCESS;
         ...
         *
         * Register strings in the recorder, e.g. for names of user event
         *channels.
         *
         * Example:
...
traceResult xTraceInitialize()
{
    ...
    RecorderDataPtr = ( RecorderDataType * ) TRACE_MALLOC(
    ...
    RecorderDataPtr
        ->isrTailchainingThreshold = TRC_CFG_ISR_TAILCHAINING_THRESHOLD;
    ...
    RecorderDataPtr->userEventBuffer
        .numberOfSlots = ( TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE );
    RecorderDataPtr->userEventBuffer.numberOfChannels = ( TRC_CFG_UB_CHANNELS ) +
                                                        1;
        #endif
...
void prvTraceStoreTaskReady( traceHandle handle )
{
    ...
    TRACE_ASSERT( handle <= ( TRC_CFG_NTASK ),
                  "prvTraceStoreTaskReady: Invalid value for handle",
                  TRC_UNUSED );
...
void prvTraceStoreLowPower( uint32_t flag )
{
    ...
    TRACE_ASSERT( flag <= 1,
                  "prvTraceStoreLowPower: Invalid flag value",
                  TRC_UNUSED );
    ...
    trcCRITICAL_SECTION_END();
         ...
         *
         * This function stores malloc and free events. Each call requires two
         *records, for size and address respectively. The event code parameter
         *(ecode) is applied to the first record (size) and the following
         *address record gets event code "ecode + 1", so make sure this is
         *respected in the event code table. Note: On "free" calls, the
         *signed_size parameter should be negative.
         ******************************************************************************/
                              ...
                              int32_t signed_size )
{
    ...
    if( RecorderDataPtr->recorderActive )
    {
        ...
        if( ms != 0 )
        {
            ...
            prvTraceUpdateCounters();
                ...
                /* Storing a second record with address (signals "failed" if
                 * null) */
    ...
    trcCRITICAL_SECTION_END();
         ...
         * is called by the hooks in trcKernelHooks.h (see trcKernelPort.h for
         *event codes).
         ******************************************************************************/
        #if( TRC_CFG_SCHEDULING_ONLY == 0 )
                              ...
                              uint32_t objectNumber )
{
    ...
    TRACE_ASSERT( ecode < 0xFF,
                  "prvTraceStoreKernelCall: ecode >= 0xFF",
                  TRC_UNUSED );
    ...
    trcCRITICAL_SECTION_END();
         ...
         *
         * Used for storing kernel calls with a handle and a numeric parameter.
         *If the numeric parameter does not fit in one byte, and extra XPS event
         *is inserted before the kernel call event containing the three upper
         *bytes.
         ******************************************************************************/
        #if( TRC_CFG_SCHEDULING_ONLY == 0 )
                                       ...
                                       uint32_t param )
{
    ...
    TRACE_ASSERT( evtcode < 0xFF,
                  "prvTraceStoreKernelCallWithParam: evtcode >= 0xFF",
                  TRC_UNUSED );
    TRACE_ASSERT( objectClass < TRACE_NCLASSES,
                  "prvTraceStoreKernelCallWithParam: objectClass >= "
                  "TRACE_NCLASSES",
                  TRC_UNUSED );
    TRACE_ASSERT( objectNumber <= RecorderDataPtr->ObjectPropertyTable
                                      .NumberOfObjectsPerClass[ objectClass ],
                  "prvTraceStoreKernelCallWithParam: Invalid value for "
                  "objectNumber",
                  TRC_UNUSED );
    ...
    if( RecorderDataPtr->recorderActive && handle_of_last_logged_task )
    {
        ...
        hnd8 = prvTraceGet8BitHandle( ( traceHandle ) objectNumber );
        kse = ( KernelCallWithParamAndHandle * )
    ...
    trcCRITICAL_SECTION_END();
         ...
         * prvTraceGetParam
         *
         * Used for storing extra bytes for kernel calls with numeric
         *parameters.
         *
...
static uint32_t prvTraceGetParam( uint32_t param_max, uint32_t param )
{
    ...
    TRACE_ASSERT( param_max == 0xFF || param_max == 0xFFFF,
                  "prvTraceGetParam: Invalid value for param_max",
                  param );
    ...
    else
    {
        ...
        {
            xps->type = DIV_XPS;
            xps->xps_8 = ( uint8_t ) ( ( param & ( 0xFF00 & ~param_max ) ) >>
                                       8 );
            xps->xps_16 = ( uint16_t ) ( ( param &
                                           ( 0xFFFF0000 & ~param_max ) ) >>
                                         16 );
            prvTraceUpdateCounters();
        }
                                                  ...
                                                  uint32_t param )
{
    ...
    TRACE_ASSERT( evtcode < 0xFF,
                  "prvTraceStoreKernelCallWithNumericParamOnly: Invalid value "
                  "for evtcode",
                  TRC_UNUSED );
                                          ...
                                          traceObjectClass objectclass )
{
    ...
    TRACE_ASSERT( objectclass < TRACE_NCLASSES,
                  "prvTraceStoreObjectNameOnCloseEvent: objectclass >= "
                  "TRACE_NCLASSES",
                  TRC_UNUSED );
    TRACE_ASSERT( handle <= RecorderDataPtr->ObjectPropertyTable
                                .NumberOfObjectsPerClass[ objectclass ],
                  "prvTraceStoreObjectNameOnCloseEvent: Invalid value for "
                  "handle",
                  TRC_UNUSED );
                                                ...
                                                traceObjectClass objectclass )
{
    ...
    TRACE_ASSERT( handle <= RecorderDataPtr->ObjectPropertyTable
                                .NumberOfObjectsPerClass[ objectclass ],
                  "prvTraceStoreObjectPropertiesOnCloseEvent: Invalid value "
                  "for handle",
                  TRC_UNUSED );
...
{
    TRACE_ASSERT( objectclass < TRACE_NCLASSES,
                  "prvTraceGetPriorityProperty: objectclass >= TRACE_NCLASSES",
                  0 );
    TRACE_ASSERT( id <= RecorderDataPtr->ObjectPropertyTable
                            .NumberOfObjectsPerClass[ objectclass ],
                  "prvTraceGetPriorityProperty: Invalid value for id",
                  0 );
...
{
    TRACE_ASSERT( objectclass < TRACE_NCLASSES,
                  "prvTraceGetObjectState: objectclass >= TRACE_NCLASSES",
                  0 );
    TRACE_ASSERT( id <= RecorderDataPtr->ObjectPropertyTable
                            .NumberOfObjectsPerClass[ objectclass ],
                  "prvTraceGetObjectState: Invalid value for id",
                  0 );
...
uint16_t uiIndexOfObject( traceHandle objecthandle, uint8_t objectclass )
{
    uint16_t index;
    TRACE_ASSERT( objectclass < TRACE_NCLASSES,
                  "uiIndexOfObject: Invalid value for objectclass",
                  0 );
    TRACE_ASSERT( objecthandle > 0 &&
                      objecthandle <=
                          RecorderDataPtr->ObjectPropertyTable
                              .NumberOfObjectsPerClass[ objectclass ],
                  "uiIndexOfObject: Invalid value for objecthandle",
                  0 );
    ...
    if( ( objectclass < TRACE_NCLASSES ) && ( objecthandle > 0 ) &&
        ( objecthandle <= RecorderDataPtr->ObjectPropertyTable
                              .NumberOfObjectsPerClass[ objectclass ] ) )
    {
        index = ( uint16_t ) ( RecorderDataPtr->ObjectPropertyTable
...
traceHandle prvTraceGetObjectHandle( traceObjectClass objectclass )
{
    ...
    if( objectHandleStacks.objectHandles[ indexOfHandle ] == 0 )
    {
        ...
        objectHandleStacks.objectHandles[ indexOfHandle ] =
            ( traceHandle ) ( 1 + indexOfHandle -
                              objectHandleStacks
                                  .lowestIndexOfClass[ objectclass ] );
    }
    ...
    else
    {
        ...
        {
            objectHandleStacks
                .handleCountWaterMarksOfClass[ objectclass ] = ( traceHandle )
                hndCount;
        }
    }
    trcCRITICAL_SECTION_END();
                               ...
                               traceHandle handle )
{
    ...
    {
        objectHandleStacks.indexOfNextAvailableHandle[ objectclass ]--;
        indexOfHandle = objectHandleStacks
                            .indexOfNextAvailableHandle[ objectclass ];
                            ...
                            const char * name )
{
    ...
    else
    {
        ...
        if( traceErrorMessage == 0 )
        {
            prvStrncpy( ( char * ) &( RecorderDataPtr->ObjectPropertyTable
                                          .objbytes[ idx ] ),
                        name,
                        RecorderDataPtr->ObjectPropertyTable
                                        ...
                                        TraceStringHandle_t userEventChannel )
{
    ...
    TRACE_ASSERT( name != 0,
                  "prvTraceOpenSymbol: name == NULL",
                  ( TraceStringHandle_t ) 0 );
    ...
    if( !result )
    {
        result = prvTraceCreateSymbolTableEntry( name,
                                                 crc,
                                                 len,
                                                 userEventChannel );
    }
...
void vTraceSetFilterGroup( uint16_t filterGroup )
{
     ...
     * prvCheckDataToBeOverwrittenForMultiEntryEvents
     *
     * This checks if the next event to be overwritten is a multi-entry user
     *event, i.e., a USER_EVENT followed by data entries. Such data entries do
     *not have an event code at byte 0, as other events. All 4 bytes are user
     *data, so the first byte of such data events must not be interpreted as
     *type field. The number of data entries following a USER_EVENT is given in
     *the event code of the USER_EVENT. Therefore, when overwriting a USER_EVENT
     *(when using in ring-buffer mode) any data entries following must be
     *replaced with NULL events (code 0).
     *
...
void prvCheckDataToBeOverwrittenForMultiEntryEvents( uint8_t nofEntriesToCheck )
{
    ...
    while( i < nofEntriesToCheck )
    {
        ...
        {
            uint8_t nDataEvents = ( uint8_t ) ( RecorderDataPtr
                                                    ->eventData[ e * 4 ] -
                                                USER_EVENT );
            if( ( e + nDataEvents ) < RecorderDataPtr->maxEvents )
            {
                ( void ) memset( &RecorderDataPtr->eventData[ e * 4 ],
                                 0,
...
uint16_t prvTraceGetDTS( uint16_t param_maxDTS )
{
    ...
    TRACE_ASSERT( param_maxDTS == 0xFF || param_maxDTS == 0xFFFF,
                  "prvTraceGetDTS: Invalid value for param_maxDTS",
                  0 );
    ...
    if( RecorderDataPtr->frequency == 0 )
    {
        if( timestampFrequency != 0 )
        {
             ...
             * vTraceSetFrequency */
            RecorderDataPtr->frequency = timestampFrequency /
                                         ( TRC_HWTC_DIVISOR );
        }
        else if( init_hwtc_count != ( TRC_HWTC_COUNT ) )
        {
            ...
            */
            RecorderDataPtr->frequency = ( TRC_HWTC_FREQ_HZ ) /
                                         ( TRC_HWTC_DIVISOR );
        }
    ...
    if( RecorderDataPtr->frequency > 0 )
    {
        ...
        if( dts > RecorderDataPtr->frequency )
        {
            ...
            RecorderDataPtr->absTimeLastEventSecond += dts / RecorderDataPtr
                                                                 ->frequency;
                                                    ...
                                                    TraceStringHandle_t chn )
{
    ...
    TRACE_ASSERT( name != 0,
                  "prvTraceLookupSymbolTableEntry: name == NULL",
                  ( TraceStringHandle_t ) 0 );
    TRACE_ASSERT( len != 0,
                  "prvTraceLookupSymbolTableEntry: len == 0",
                  ( TraceStringHandle_t ) 0 );
    ...
    while( i != 0 )
    {
        ...
        {
            if( RecorderDataPtr->SymbolTable.symbytes[ i + 3 ] ==
                ( chn / 0x100 ) )
            {
                if( RecorderDataPtr->SymbolTable.symbytes[ i + 4 + len ] ==
                    '\0' )
                {
                    if( strncmp( ( char * ) ( &RecorderDataPtr->SymbolTable
                                                   .symbytes[ i + 4 ] ),
                                 name,
                                 len ) == 0 )
                    {
        ...
        i = ( uint16_t ) ( RecorderDataPtr->SymbolTable.symbytes[ i ] +
                           ( RecorderDataPtr->SymbolTable.symbytes[ i + 1 ] *
                             0x100 ) );
    }
    return i;
}
                                                    ...
                                                    TraceStringHandle_t channel )
{
    ...
    TRACE_ASSERT( name != 0,
                  "prvTraceCreateSymbolTableEntry: name == NULL",
                  0 );
    ...
    else
    {
        RecorderDataPtr->SymbolTable
            ...
            ( uint8_t ) ( RecorderDataPtr->SymbolTable
                              .latestEntryOfChecksum[ crc6 ] &
                          0x00FF );

        RecorderDataPtr->SymbolTable
            .symbytes[ RecorderDataPtr->SymbolTable.nextFreeSymbolIndex +
                       1 ] = ( uint8_t ) ( RecorderDataPtr->SymbolTable
                                               .latestEntryOfChecksum[ crc6 ] /
                                           0x100 );

        RecorderDataPtr->SymbolTable
            .symbytes[ RecorderDataPtr->SymbolTable.nextFreeSymbolIndex +
                       2 ] = ( uint8_t ) ( channel & 0x00FF );

        RecorderDataPtr->SymbolTable
            .symbytes[ RecorderDataPtr->SymbolTable.nextFreeSymbolIndex +
                       3 ] = ( uint8_t ) ( channel / 0x100 );
        ...
        RecorderDataPtr->SymbolTable
            .symbytes[ RecorderDataPtr->SymbolTable.nextFreeSymbolIndex + 4 +
                       len ] = '\0';

        /* store index of entry (for return value, and as head of LL[crc6]) */
        RecorderDataPtr->SymbolTable
            .latestEntryOfChecksum[ crc6 ] = ( uint16_t )
                                                 RecorderDataPtr->SymbolTable
                                                     .nextFreeSymbolIndex;

        RecorderDataPtr->SymbolTable.nextFreeSymbolIndex += ( uint32_t ) ( len +
                                                                           5 );
                          ...
                          uint8_t * plength )
{
    ...
    TRACE_ASSERT( pname != 0,
                  "prvTraceGetChecksum: pname == NULL",
                  TRC_UNUSED );
    TRACE_ASSERT( pcrc != 0, "prvTraceGetChecksum: pcrc == NULL", TRC_UNUSED );
    TRACE_ASSERT( plength != 0,
                  "prvTraceGetChecksum: plength == NULL",
                  TRC_UNUSED );
...
static uint8_t prvTraceGet8BitHandle( traceHandle handle )
{
    ...
    return ( uint8_t ) ( handle & 0xFF );
        ...
        /* If using DWT timestamping (default on ARM Cortex-M3, M4 and M7), make
         * sure the DWT unit is initialized. */
        #ifndef TRC_CFG_ARM_CM_USE_SYSTICK
            #if( ( TRC_CFG_HARDWARE_PORT ==            \
...
void prvTracePortGetTimeStamp( uint32_t * pTimestamp )
{
    ...
    if( trace_disable_timestamp == 1 )
    {
        ...
        /* Retrieve TRC_HWTC_COUNT only once since the same value should be used
    ...
    if( pTimestamp )
    {
         ...
         * unwanted overflows. */
        last_timestamp = traceTickCount *
                         ( ( TRC_HWTC_PERIOD ) / ( TRC_HWTC_DIVISOR ) );
        /* Increase timestamp by (hwtc_count + "lost hardware ticks from scaling
         * down period") / TRC_HWTC_DIVISOR. */
        last_timestamp += ( hwtc_count +
                            traceTickCount * ( ( TRC_HWTC_PERIOD ) %
                                               ( TRC_HWTC_DIVISOR ) ) ) /
                          ( TRC_HWTC_DIVISOR );
    }
    ...
    wrap at (2^32 / TRC_HWTC_DIVISOR) ticks. The scaled timestamp returned from
    this function is supposed to go from 0 -> 2^32, which in real time would
    represent (0 -> 2^32 * TRC_HWTC_DIVISOR) ticks. */
...
void prvReportStackUsage()
{
    ...
    int initial = i;  /* Used to make sure we break if we are back at the inital
                         value */
    ...
    do
    {
        ...
        if( tasksInStackMonitor[ i ].tcb != 0 )
        {
            ...
            xTraceKernelPortGetUnusedStack( tasksInStackMonitor[ i ].tcb,
                                            &unusedStackSpace );
