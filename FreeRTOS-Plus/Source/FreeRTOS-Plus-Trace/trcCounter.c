                                 ...
                                 TraceCounterHandle_t * pxCounterHandle )
{
    ...
    TRC_ASSERT(
        xTraceIsComponentInitialized( TRC_RECORDER_COMPONENT_COUNTER ) );
    ...
    if( xTraceObjectRegisterInternal( PSF_EVENT_COUNTER_CREATE,
                                      0,
                                      szName,
                                      3,
                                      uxStates,
                                      TRC_ENTRY_OPTION_COUNTER,
                              ...
                              TraceBaseType_t xValue )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE( xTraceCounterGet( xCounterHandle, &xCurrent ) ==
                                TRC_SUCCESS );

    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceCounterSet( xCounterHandle, xCurrent + xValue ) == TRC_SUCCESS );
                              ...
                              TraceBaseType_t xValue )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetState( ( TraceEntryHandle_t ) xCounterHandle,
                             TRC_COUNTER_VALUE_INDEX,
                             ( TraceUnsignedBaseType_t ) xValue ) ==
        TRC_SUCCESS );

    /* We need to check this */
    if( xTraceEventBegin( PSF_EVENT_COUNTER_CHANGE,
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceCounterGetLowerLimit( xCounterHandle, &xLowerLimit ) ==
        TRC_SUCCESS );
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceCounterGetUpperLimit( xCounterHandle, &xUpperLimit ) ==
        TRC_SUCCESS );
                              ...
                              TraceBaseType_t * pxValue )
{
    ...
    return xTraceEntryGetState( ( TraceEntryHandle_t ) xCounterHandle,
                                TRC_COUNTER_VALUE_INDEX,
                                  ...
                                  const char ** pszName )
{
    ...
    return xTraceEntryGetSymbol( ( TraceEntryHandle_t ) xCounterHandle,
                                 pszName );
}
