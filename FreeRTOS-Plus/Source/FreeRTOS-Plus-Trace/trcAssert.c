...
extern inline TraceBaseType_t prvTraceAssertCheckCondition(
    TraceBaseType_t condition );
                           ...
                           TraceUnsignedBaseType_t uxLineNumber )
{
    ...
    if( uxRecursionGuard == 0 )
    {
        ...
        xTraceEntryGetState( pxAssertInfo->xEntry,
                             TRC_ASSERT_STATE_INDEX_LINE_NUMBER,
                             &uxEntryLineNumber );
        ...
        if( uxEntryLineNumber == 0 )
        {
            ...
            for( i = xLength - 1; ( i >= 0 ) && ( ( szFilePath[ i ] != '\\' ) &&
                                                  ( szFilePath[ i ] != '/' ) );
                 i-- )
            {
            }
            ...
            xTraceObjectSetName( ( TraceObjectHandle_t ) pxAssertInfo->xEntry,
                                 &szFilePath[ i + 1 ] );
            xTraceObjectSetState( ( TraceObjectHandle_t ) pxAssertInfo->xEntry,
                                  uxLineNumber );
