...
traceResult xTraceISRInitialize( TraceISRInfoBuffer_t * pxBuffer )
{
    ...
    {
        TraceISRCoreInfo_t * pxCoreInfo = &pxTraceISRInfo
                                               ->coreInfos[ uiCoreIndex ];

        /* Initialize ISR stack */
        for( uiStackIndex = 0; uiStackIndex < ( TRC_CFG_MAX_ISR_NESTING );
             uiStackIndex++ )
        {
                               ...
                               TraceISRHandle_t * pxISRHandle )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE( xTraceEntrySetSymbol( xEntryHandle, szName ) ==
                                TRC_SUCCESS );

    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetState( xEntryHandle,
                             0,
    ...
    *pxISRHandle = ( TraceISRHandle_t ) xEntryHandle;
    ...
    /* We need to check this */
    if( xTraceEventBegin( PSF_EVENT_DEFINE_ISR,
                          uiLength + sizeof( void * ) + sizeof( uint32_t ),
...
traceResult xTraceISRBegin( TraceISRHandle_t xISRHandle )
{
    ...
    if( pxCoreInfo->stackIndex < ( TRC_CFG_MAX_ISR_NESTING ) -1 )
    {
        ...
        /* We need to check this */
        if( xTraceEventBegin( PSF_EVENT_ISR_BEGIN,
                              sizeof( void * ),
                              &xEventHandle ) == TRC_SUCCESS )
        {
...
traceResult xTraceISREnd( TraceBaseType_t xIsTaskSwitchRequired )
{
    ...
    if( pxCoreInfo->stackIndex > 0 )
    {
        ...
        /* We need to check this */
        if( xTraceEventBegin( PSF_EVENT_ISR_RESUME,
                              sizeof( void * ),
                              &xEventHandle ) == TRC_SUCCESS )
        {
            xTraceEventAddPointer(
                xEventHandle,
                ( void * ) pxCoreInfo->handleStack[ pxCoreInfo->stackIndex ] );
            xTraceEventEnd( xEventHandle );
        }
        #endif
    }
    else
    {
         ...
         * between. */
        if( ( pxCoreInfo->isPendingContextSwitch == 0 ) ||
            ( xTraceKernelPortIsSchedulerSuspended() ) )
        {
        #if( TRC_CFG_INCLUDE_ISR_TRACING == 1 )
            /* We need to check this */
            if( xTraceEventBegin( PSF_EVENT_TASK_ACTIVATE,
                                  sizeof( void * ),
                                  &xEventHandle ) == TRC_SUCCESS )
            {
