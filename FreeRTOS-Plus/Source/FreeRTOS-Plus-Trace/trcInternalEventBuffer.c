...
traceResult xTraceInternalEventBufferInitialize( uint8_t * puiBuffer,
                                                 uint32_t uiSize )
{
    /* uiSize must be larger than sizeof(TraceMultiCoreEventBuffer_t) or there
     * will be no room for any data */
    ...
    if( xTraceMultiCoreEventBufferInitialize(
            pxInternalEventBuffer,
            TRC_EVENT_BUFFER_OPTION_SKIP,
            &puiBuffer[ sizeof( TraceMultiCoreEventBuffer_t ) ],
            uiSize - sizeof( TraceMultiCoreEventBuffer_t ) ) == TRC_FAIL )
    {
        return TRC_FAIL;
    }

    xTraceSetComponentInitialized(
        TRC_RECORDER_COMPONENT_INTERNAL_EVENT_BUFFER );
...
{
    /* This should never fail */
    TRC_ASSERT( xTraceIsComponentInitialized(
        TRC_RECORDER_COMPONENT_INTERNAL_EVENT_BUFFER ) );

    return xTraceMultiCoreEventBufferPush( pxInternalEventBuffer,
                                           pvData,
                                           uiSize,
                                           piBytesWritten );
}
...
traceResult xTraceInternalEventBufferTransfer( int32_t * piBytesWritten )
{
    ...
    return xTraceMultiCoreEventBufferTransfer( pxInternalEventBuffer,
                                               piBytesWritten );
}
