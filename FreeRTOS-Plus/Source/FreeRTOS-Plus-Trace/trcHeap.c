                              ...
                              TraceHeapHandle_t * pxHeapHandle )
{
    ...
    if( xTraceObjectRegisterInternal( PSF_EVENT_HEAP_CREATE,
                                      0,
                                      szName,
                                      3,
                                      uxStates,
                                      TRC_ENTRY_OPTION_HEAP,
                             ...
                             TraceUnsignedBaseType_t uxSize )
{
    ...
    if( pvAddress != 0 )
    {
        ...
        TRC_ASSERT_ALWAYS_EVALUATE(
            xTraceEntryGetState( xHeapHandle,
                                 TRC_HEAP_STATE_INDEX_CURRENT,
                                 &uxCurrent ) == TRC_SUCCESS );

        /* This should never fail */
        TRC_ASSERT_ALWAYS_EVALUATE(
            xTraceEntryGetState( xHeapHandle,
                                 TRC_HEAP_STATE_INDEX_HIGHWATERMARK,
                                 &uxHighWaterMark ) == TRC_SUCCESS );
        ...
        if( uxCurrent > uxHighWaterMark )
        {
            ...
            /* This should never fail */
            TRC_ASSERT_ALWAYS_EVALUATE(
                xTraceEntrySetState( xHeapHandle,
                                     TRC_HEAP_STATE_INDEX_HIGHWATERMARK,
                                     uxHighWaterMark ) == TRC_SUCCESS );
        }
    ...
    /* We need to check this */
    if( xTraceEventBegin( pvAddress != 0 ? PSF_EVENT_MALLOC
                                         : PSF_EVENT_MALLOC_FAILED,
                            ...
                            TraceUnsignedBaseType_t uxSize )
{
    ...
    /* We need to check this */
    if( xTraceEventBegin( pvAddress != 0 ? PSF_EVENT_FREE
                                         : PSF_EVENT_FREE_FAILED,
                          sizeof( void * ) + sizeof( TraceUnsignedBaseType_t ),
                          &xEventHandle ) == TRC_SUCCESS )
    {
        xTraceEventAddPointer( xEventHandle, pvAddress );
                                  ...
                                  TraceUnsignedBaseType_t * puxCurrent )
{
    ...
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntryGetState( xHeapHandle,
                             TRC_HEAP_STATE_INDEX_CURRENT,
                             puxCurrent ) == TRC_SUCCESS );

    return TRC_SUCCESS;
}

traceResult xTraceHeapGetHighWaterMark(
                              ...
                              TraceUnsignedBaseType_t * puxMax )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE( xTraceEntryGetState( xHeapHandle,
                                                     TRC_HEAP_STATE_INDEX_MAX,
                                                     puxMax ) == TRC_SUCCESS );
