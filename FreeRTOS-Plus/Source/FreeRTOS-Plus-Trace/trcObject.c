                                          ...
                                          TraceObjectHandle_t * pxObjectHandle )
{
    ...
    if( pvObject != 0 )
    {
        ...
        if( xTraceEntryCreateWithAddress( pvObject, &xEntryHandle ) ==
            TRC_FAIL )
        {
    ...
    {
        /* This should never fail */
        TRC_ASSERT_ALWAYS_EVALUATE(
            xTraceEntrySetState( xEntryHandle, i, uxStates[ i ] ) ==
            TRC_SUCCESS );
    }

    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetOptions( xEntryHandle, uxOptions ) == TRC_SUCCESS );
    ...
    *pxObjectHandle = ( TraceObjectHandle_t ) xEntryHandle;
    ...
    if( szName != 0 && szName[ 0 ] != 0 )
    {
        ...
        /* This should never fail */
        TRC_ASSERT_ALWAYS_EVALUATE(
            xTraceObjectSetName( ( TraceObjectHandle_t ) xEntryHandle,
                                 szName ) == TRC_SUCCESS );
    }

    /* Send the create event, if possible */
    /*We need to check this */
    if( xTraceEventBegin( uiEventCode,
                          sizeof( void * ) +
                              uxStateCount * sizeof( TraceUnsignedBaseType_t ) +
                                  ...
                                  TraceObjectHandle_t * pxObjectHandle )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntrySetState( xEntryHandle, 0, uxState ) == TRC_SUCCESS );
    ...
    *pxObjectHandle = ( TraceObjectHandle_t ) xEntryHandle;
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        prvTraceObjectSendState( uiEventCode, pvObject, uxState ) ==
        TRC_SUCCESS );
                                    ...
                                    TraceUnsignedBaseType_t uxState )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntryGetAddress( ( TraceEntryHandle_t ) xObjectHandle,
                               &pvObject ) == TRC_SUCCESS );

    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        xTraceEntryGetSymbol( ( TraceEntryHandle_t ) xObjectHandle, &szName ) ==
        TRC_SUCCESS );

        #if( TRC_SEND_NAME_ONLY_ON_DELETE == 1 )
    /* Send name event because this is a delete */
    /* This should never fail */
    TRC_ASSERT( prvTraceObjectSendNameEvent( pvObject, szName ) ==
                TRC_SUCCESS );
                                 ...
                                 const char * szName )
{
    ...
    /* This should never fail */
    TRC_ASSERT_ALWAYS_EVALUATE(
        prvTraceObjectSendNameEvent( pvObject, szName ) == TRC_SUCCESS );
                                               ...
                                               TraceUnsignedBaseType_t uxState )
{
    ...
    return xTraceObjectRegister( uiEventCode,
                                 pvObject,
                                 szName,
                                 uxState,
                                 &xObjectHandle );
...
traceResult xTraceObjectUnregisterWithoutHandle(
    uint32_t uiEventCode,
    void * pvObject,
    TraceUnsignedBaseType_t uxState )
{
    ...
    xResult = xTraceObjectUnregister( ( TraceObjectHandle_t ) xEntryHandle,
                                      uiEventCode,
                                      uxState );
                                              ...
                                              const char * szName )
{
    ...
    xResult = xTraceObjectSetName( ( TraceObjectHandle_t ) xEntryHandle,
                                   szName );
...
traceResult xTraceObjectSetSpecificStateWithoutHandle(
    void * pvObject,
    uint32_t uiIndex,
    TraceUnsignedBaseType_t uxState )
{
    ...
    xResult = xTraceObjectSetSpecificState( ( TraceObjectHandle_t ) xEntryHandle,
                                            uiIndex,
                                            uxState );
                                                 ...
                                                 uint32_t uiMask )
{
    ...
    xResult = xTraceObjectSetOptions( ( TraceObjectHandle_t ) xEntryHandle,
                                      uiMask );
                                     ...
                                     TraceUnsignedBaseType_t uxState )
{
    ...
    if( xTraceEventBegin( uiEventCode,
                          sizeof( void * ) + sizeof( TraceUnsignedBaseType_t ),
                          &xEventHandle ) == TRC_SUCCESS )
    {
        xTraceEventAddPointer( xEventHandle, pvObject );
...
traceResult prvTraceObjectSendNameEvent( void * pvObject, const char * szName )
{
    ...
    for( i = 0;
         ( szName[ i ] != 0 ) && ( i < ( TRC_ENTRY_TABLE_SLOT_SYMBOL_SIZE ) );
         i++ )
    {
    }
    ...
    if( xTraceEventBegin( PSF_EVENT_OBJ_NAME,
                          sizeof( void * ) + uiLength,
                          &xEventHandle ) == TRC_SUCCESS )
    {
        xTraceEventAddPointer( xEventHandle, pvObject );
