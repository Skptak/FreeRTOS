...
traceResult xTraceConsoleChannelPrintF( const char * szFormat, ... )
{
    ...
    {
        if( xTraceStringRegister( "Debug Console",
                                  &pxPrintInfo->consoleChannel ) == TRC_FAIL )
        {
            return TRC_FAIL;
        }
    }
                                    ...
                                    va_list * pxVL )
{
    ...
    uint32_t uiValue;
    uint32_t uiEventCode = PSF_EVENT_USER_EVENT + 1 +
                           uiArgs; /* Add channel (1) */
    uint32_t uiSize = sizeof( void * ) +
                      uiArgs * sizeof( TraceUnsignedBaseType_t ) +
                      uiLength; /* Add channel (sizeof(void*)) */
    ...
    if( xChannel == 0 )
    {
        if( pxPrintInfo->defaultChannel == 0 )
        {
            ...
            if( xTraceStringRegister( "Default",
                                      &pxPrintInfo->defaultChannel ) ==
                TRC_FAIL )
            {
                return TRC_FAIL;
            }
    ...
    for( i = 0; i < uiArgs; i++ )
    {
        xTraceEventAddUnsignedBaseType( xEventHandle,
                                        va_arg( *pxVL,
