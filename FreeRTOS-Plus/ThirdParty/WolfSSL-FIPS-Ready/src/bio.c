...
static int wolfSSL_BIO_BASE64_read( WOLFSSL_BIO * bio, void * buf, int len )
{
    ...
    if( Base64_Decode( ( const byte * ) buf,
                       ( word32 ) len,
                       ( byte * ) buf,
                       &frmtSz ) != 0 )
    {
...
static int wolfSSL_BIO_MD_read( WOLFSSL_BIO * bio, void * buf, int sz )
{
    if( wolfSSL_EVP_MD_CTX_type( ( WOLFSSL_EVP_MD_CTX * ) bio->ptr ) ==
        NID_hmac )
    {
        if( wolfSSL_EVP_DigestSignUpdate( ( WOLFSSL_EVP_MD_CTX * ) bio->ptr,
                                          buf,
                                          sz ) != WOLFSSL_SUCCESS )
        {
            return WOLFSSL_FATAL_ERROR;
    ...
    {
        if( wolfSSL_EVP_DigestUpdate( ( WOLFSSL_EVP_MD_CTX * ) bio->ptr,
                                      buf,
                                      sz ) != WOLFSSL_SUCCESS )
        {
            return WOLFSSL_FATAL_ERROR;
                                     ...
                                     word32 * outLen )
{
    ...
    {
        if( Base64_Encode_NoNl( ( const byte * ) data, inLen, tmp, outLen ) <
            0 )
        {
    ...
    {
        if( Base64_Encode( ( const byte * ) data, inLen, tmp, outLen ) < 0 )
        {
            ret = WOLFSSL_FATAL_ERROR;
        }
    }
                                     ...
                                     int len )
{
    ...
    {
        bio->ptr = ( byte * )
            XREALLOC( bio->ptr, sz + len, bio->heap, DYNAMIC_TYPE_OPENSSL );
        if( bio->ptr == NULL )
        {
...
int wolfSSL_BIO_write( WOLFSSL_BIO * bio, const void * data, int len )
{
    ...
    while( bio != NULL && ret >= 0 )
    {
        ...
        if( bio->type == WOLFSSL_BIO_BASE64 )
        {
            ...
            if( bio->flags & WOLFSSL_BIO_FLAG_BASE64_NO_NL )
            {
                if( Base64_Encode_NoNl( ( const byte * ) data,
                                        len,
                                        NULL,
                                        &sz ) != LENGTH_ONLY_E )
                {
            ...
            {
                frmt = ( void * )
                    XREALLOC( frmt, sz, front->heap, DYNAMIC_TYPE_TMP_BUFFER );
                if( frmt == NULL )
                {
                    WOLFSSL_MSG( "Memory error" );
                    ret = SSL_FATAL_ERROR;
            ...
            if( ret >= 0 )
            {
                ...
                retB64 = wolfSSL_BIO_BASE64_write( bio,
                                                   data,
                                                   ( word32 ) len,
                                                   ( byte * ) frmt,
                                                   &frmtSz );
    ...
    /* Wrapper for other BIO type functions, expected to grow as OpenSSL
     * compatibility layer grows.
     *
...
int wolfSSL_BIO_gets( WOLFSSL_BIO * bio, char * buf, int sz )
{
    ...
    switch( bio->type )
    {
        ...
        /* call final on hash */
        case WOLFSSL_BIO_MD:
            if( wolfSSL_EVP_MD_CTX_size( ( WOLFSSL_EVP_MD_CTX * ) bio->ptr ) >
                sz )
            {
    ...
    if( bio->infoCb != NULL )
    {
        ret = ( int ) bio->infoCb( bio,
                                   WOLFSSL_BIO_CB_GETS | WOLFSSL_BIO_CB_RETURN,
                                   buf,
                                   sz,
                                   0,
                                   ret );
...
int wolfSSL_BIO_make_bio_pair( WOLFSSL_BIO * b1, WOLFSSL_BIO * b2 )
{
    ...
    if( b1->ptr == NULL &&
        wolfSSL_BIO_set_write_buf_size( b1, WOLFSSL_BIO_SIZE ) !=
            WOLFSSL_SUCCESS )
    {
        return WOLFSSL_FAILURE;
    }
    ...
    if( b2->ptr == NULL &&
        wolfSSL_BIO_set_write_buf_size( b2, WOLFSSL_BIO_SIZE ) !=
            WOLFSSL_SUCCESS )
    {
        return WOLFSSL_FAILURE;
    }
...
int wolfSSL_BIO_reset( WOLFSSL_BIO * bio )
{
    ...
    switch( bio->type )
    {
        ...
        case WOLFSSL_BIO_MD:
            if( bio->ptr != NULL )
            {
                const WOLFSSL_EVP_MD * md = wolfSSL_EVP_MD_CTX_md(
                    ( WOLFSSL_EVP_MD_CTX * ) bio->ptr );
...
long wolfSSL_BIO_set_nbio( WOLFSSL_BIO * bio, long on )
{
    ...
    #ifdef WOLFSSL_DTLS
            wolfSSL_dtls_set_using_nonblock( ( WOLFSSL * ) bio->ptr,
                                             ( int ) on );
    #endif
            break;
...
WOLFSSL_BIO_METHOD * wolfSSL_BIO_meth_new( int type, const char * name )
{
    ...
    meth = ( WOLFSSL_BIO_METHOD * ) XMALLOC( sizeof( WOLFSSL_BIO_METHOD ),
                                             NULL,
                                             DYNAMIC_TYPE_OPENSSL );
    if( meth == NULL )
    {
