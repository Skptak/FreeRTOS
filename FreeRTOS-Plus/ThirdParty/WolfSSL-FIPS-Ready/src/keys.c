...
int SetCipherSpecs( WOLFSSL * ssl )
{
    ...
    if( ssl->options.cipherSuite0 == CHACHA_BYTE )
    {
        switch( ssl->options.cipherSuite )
        {
            ...
            default:
                WOLFSSL_MSG(
                    "Unsupported cipher suite, SetCipherSpecs ChaCha" );
                return UNSUPPORTED_SUITE;
        }
    }
                    ...
                    int tls13 )
{
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_rc4 )
    {
        ...
        {
            enc->arc4 = ( Arc4 * ) XMALLOC( sizeof( Arc4 ),
                                            heap,
                                            DYNAMIC_TYPE_CIPHER );
            if( enc->arc4 == NULL )
                return MEMORY_E;
        ...
        {
            dec->arc4 = ( Arc4 * ) XMALLOC( sizeof( Arc4 ),
                                            heap,
                                            DYNAMIC_TYPE_CIPHER );
            if( dec->arc4 == NULL )
                return MEMORY_E;
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_chacha )
    {
        ...
        if( enc && enc->chacha == NULL )
            enc->chacha = ( ChaCha * ) XMALLOC( sizeof( ChaCha ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
        if( enc && enc->chacha == NULL )
            return MEMORY_E;
        if( dec && dec->chacha == NULL )
            dec->chacha = ( ChaCha * ) XMALLOC( sizeof( ChaCha ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
        if( dec && dec->chacha == NULL )
            return MEMORY_E;
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                chachaRet = wc_Chacha_SetKey( enc->chacha,
            ...
            if( dec )
            {
                chachaRet = wc_Chacha_SetKey( dec->chacha,
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_hc128 )
    {
        ...
        if( enc && enc->hc128 == NULL )
            enc->hc128 = ( HC128 * ) XMALLOC( sizeof( HC128 ),
                                              heap,
                                              DYNAMIC_TYPE_CIPHER );
        if( enc && enc->hc128 == NULL )
            return MEMORY_E;
        if( dec && dec->hc128 == NULL )
            dec->hc128 = ( HC128 * ) XMALLOC( sizeof( HC128 ),
                                              heap,
                                              DYNAMIC_TYPE_CIPHER );
        if( dec && dec->hc128 == NULL )
            return MEMORY_E;
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                hcRet = wc_Hc128_SetKey( enc->hc128,
                                         keys->client_write_key,
                                         keys->client_write_IV );
                if( hcRet != 0 )
                    return hcRet;
            ...
            if( dec )
            {
                hcRet = wc_Hc128_SetKey( dec->hc128,
                                         keys->server_write_key,
                                         keys->server_write_IV );
                if( hcRet != 0 )
                    return hcRet;
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_rabbit )
    {
        ...
        if( enc && enc->rabbit == NULL )
            enc->rabbit = ( Rabbit * ) XMALLOC( sizeof( Rabbit ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
        if( enc && enc->rabbit == NULL )
            return MEMORY_E;
        if( dec && dec->rabbit == NULL )
            dec->rabbit = ( Rabbit * ) XMALLOC( sizeof( Rabbit ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
        if( dec && dec->rabbit == NULL )
            return MEMORY_E;
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                rabRet = wc_RabbitSetKey( enc->rabbit,
                                          keys->client_write_key,
                                          keys->client_write_IV );
                if( rabRet != 0 )
                    return rabRet;
            ...
            if( dec )
            {
                rabRet = wc_RabbitSetKey( dec->rabbit,
                                          keys->server_write_key,
                                          keys->server_write_IV );
                if( rabRet != 0 )
                    return rabRet;
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_triple_des )
    {
        ...
        {
            if( enc->des3 == NULL )
                enc->des3 = ( Des3 * ) XMALLOC( sizeof( Des3 ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
            if( enc->des3 == NULL )
                return MEMORY_E;
        ...
        {
            if( dec->des3 == NULL )
                dec->des3 = ( Des3 * ) XMALLOC( sizeof( Des3 ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
            if( dec->des3 == NULL )
                return MEMORY_E;
        ...
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                desRet = wc_Des3_SetKey( enc->des3,
                                         keys->client_write_key,
                                         keys->client_write_IV,
                                         DES_ENCRYPTION );
                if( desRet != 0 )
            ...
            if( dec )
            {
                desRet = wc_Des3_SetKey( dec->des3,
                                         keys->server_write_key,
                                         keys->server_write_IV,
                                         DES_DECRYPTION );
                if( desRet != 0 )
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_aes )
    {
        ...
        {
            if( enc->aes == NULL )
                enc->aes = ( Aes * ) XMALLOC( sizeof( Aes ),
                                              heap,
                                              DYNAMIC_TYPE_CIPHER );
            if( enc->aes == NULL )
                return MEMORY_E;
        ...
        {
            if( dec->aes == NULL )
                dec->aes = ( Aes * ) XMALLOC( sizeof( Aes ),
                                              heap,
                                              DYNAMIC_TYPE_CIPHER );
            if( dec->aes == NULL )
                return MEMORY_E;
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_aes_gcm )
    {
        ...
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                gcmRet = wc_AesGcmSetKey( enc->aes,
                                          keys->client_write_key,
                                          specs->key_size );
                if( gcmRet != 0 )
                    return gcmRet;
            ...
            if( dec )
            {
                gcmRet = wc_AesGcmSetKey( dec->aes,
                                          keys->server_write_key,
                                          specs->key_size );
                if( gcmRet != 0 )
                    return gcmRet;
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_aes_ccm )
    {
        ...
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                CcmRet = wc_AesCcmSetKey( enc->aes,
                                          keys->client_write_key,
                                          specs->key_size );
                if( CcmRet != 0 )
                {
            ...
            ( ( !defined( HAVE_FIPS ) && !defined( HAVE_SELFTEST ) ) || \
              ( defined( HAVE_FIPS_VERSION ) && ( HAVE_FIPS_VERSION >= 2 ) ) )
                if( !tls13 )
                {
                    CcmRet = wc_AesCcmSetNonce( enc->aes,
                                                keys->client_write_IV,
                                                AEAD_MAX_IMP_SZ );
                    if( CcmRet != 0 )
                        return CcmRet;
            ...
            if( dec )
            {
                CcmRet = wc_AesCcmSetKey( dec->aes,
                                          keys->server_write_key,
                                          specs->key_size );
                if( CcmRet != 0 )
                {
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_camellia )
    {
        ...
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                camRet = wc_CamelliaSetKey( enc->cam,
                                            keys->client_write_key,
                                            specs->key_size,
                                            keys->client_write_IV );
                if( camRet != 0 )
            ...
            if( dec )
            {
                camRet = wc_CamelliaSetKey( dec->cam,
                                            keys->server_write_key,
                                            specs->key_size,
                                            keys->server_write_IV );
                if( camRet != 0 )
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_idea )
    {
        ...
        if( enc && enc->idea == NULL )
            enc->idea = ( Idea * ) XMALLOC( sizeof( Idea ),
                                            heap,
                                            DYNAMIC_TYPE_CIPHER );
        if( enc && enc->idea == NULL )
            return MEMORY_E;
        ...
        if( dec && dec->idea == NULL )
            dec->idea = ( Idea * ) XMALLOC( sizeof( Idea ),
                                            heap,
                                            DYNAMIC_TYPE_CIPHER );
        if( dec && dec->idea == NULL )
            return MEMORY_E;
        ...
        if( side == WOLFSSL_CLIENT_END )
        {
            if( enc )
            {
                ideaRet = wc_IdeaSetKey( enc->idea,
                                         keys->client_write_key,
                                         specs->key_size,
                                         keys->client_write_IV,
                                         IDEA_ENCRYPTION );
            ...
            if( dec )
            {
                ideaRet = wc_IdeaSetKey( dec->idea,
                                         keys->server_write_key,
                                         specs->key_size,
                                         keys->server_write_IV,
                                         IDEA_DECRYPTION );
    ...
    if( specs->bulk_cipher_algorithm == wolfssl_cipher_null )
    {
        ...
        if( tls13 )
        {
            ...
            {
                enc->hmac = ( Hmac * ) XMALLOC( sizeof( Hmac ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
                if( enc->hmac == NULL )
                    return MEMORY_E;
            ...
            {
                dec->hmac = ( Hmac * ) XMALLOC( sizeof( Hmac ),
                                                heap,
                                                DYNAMIC_TYPE_CIPHER );
                if( dec->hmac == NULL )
                    return MEMORY_E;
            ...
            if( side == WOLFSSL_CLIENT_END )
            {
                if( enc )
                {
                    XMEMCPY( keys->aead_enc_imp_IV,
                             keys->client_write_IV,
                             HMAC_NONCE_SZ );
                    hmacRet = wc_HmacSetKey( enc->hmac,
                                             hashType,
                                             keys->client_write_key,
                                             specs->key_size );
                    if( hmacRet != 0 )
                ...
                if( dec )
                {
                    XMEMCPY( keys->aead_dec_imp_IV,
                             keys->server_write_IV,
                             HMAC_NONCE_SZ );
                    hmacRet = wc_HmacSetKey( dec->hmac,
                                             hashType,
                                             keys->server_write_key,
                                             specs->key_size );
                    if( hmacRet != 0 )
                        ...
                        int devId )
{
    ...
    if( authentication && authentication->poly1305 == NULL )
        authentication->poly1305 = ( Poly1305 * )
            XMALLOC( sizeof( Poly1305 ), heap, DYNAMIC_TYPE_CIPHER );
    if( authentication && authentication->poly1305 == NULL )
        return MEMORY_E;
    if( authentication )
...
int SetKeysSide( WOLFSSL * ssl, enum encrypt_side side )
{
    ...
    switch( side )
    {
        case ENCRYPT_SIDE_ONLY:
            ...
            if( ssl->options.side == WOLFSSL_CLIENT_END )
            {
                WOLFSSL_BUFFER( ssl->keys.client_write_key,
                                ssl->specs.key_size );
            }
            else
            {
                WOLFSSL_BUFFER( ssl->keys.server_write_key,
                                ssl->specs.key_size );
            }
    #endif
...
int StoreKeys( WOLFSSL * ssl, const byte * keyData, int side )
{
        ...
        ssl->secure_renegotiation->cache_status == SCR_CACHE_NEEDED )
    {
        ...
        if( ssl->options.dtls )
        {
            ...
            ssl->secure_renegotiation->tmp_keys
                .dtls_epoch = ssl->keys.dtls_epoch + 1;
    ...
    if( ssl->options.haveMcast )
    {
        ...
        if( scr_copy )
        {
            XMEMCPY( ssl->keys.client_write_key, keys->client_write_key, sz );
            XMEMCPY( ssl->keys.server_write_key, keys->server_write_key, sz );
        }
        #endif
        ...
        if( scr_copy )
        {
            XMEMCPY( ssl->keys.client_write_IV, keys->client_write_IV, sz );
            XMEMCPY( ssl->keys.server_write_IV, keys->server_write_IV, sz );
        }
        #endif
...
{
    int length = 2 * ssl->specs.hash_size + 2 * ssl->specs.key_size +
                 2 * ssl->specs.iv_size;
    ...
    keyData = ( byte * ) XMALLOC( KEY_PREFIX * WC_MD5_DIGEST_SIZE,
                                  NULL,
                                  DYNAMIC_TYPE_TMP_BUFFER );
    md5 = ( wc_Md5 * ) XMALLOC( sizeof( wc_Md5 ),
                                NULL,
                                DYNAMIC_TYPE_TMP_BUFFER );
    sha = ( wc_Sha * ) XMALLOC( sizeof( wc_Sha ),
                                NULL,
                                DYNAMIC_TYPE_TMP_BUFFER );
...
int MakeMasterSecret( WOLFSSL * ssl )
{
    ...
    if( ssl->peerQSHKeyPresent )
    {
        ...
        {
            XMEMCPY( ssl->arrays->preMasterSecret + offset,
                     ssl->QSH_secret->CliSi->buffer,
                     ssl->QSH_secret->CliSi->length );
        }
