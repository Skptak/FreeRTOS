...
#include <wolfssl/wolfcrypt/settings.h>
    ...
    #ifdef _WIN32_WCE
        /* On WinCE winsock2.h must be included before windows.h for socket
         * stuff */
        #include <winsock2.h>
    #endif
 ...
 *
 * ssl  WOLFSSL struct passed in that has this function set as the send
 * callback. buf  buffer with data to write out sz   size of buf buffer ctx  a
 * user set context
 *
...
int EmbedReceiveFrom( WOLFSSL * ssl, char * buf, int sz, void * ctx )
{
    ...
    recvd = ( int ) RECVFROM_FUNCTION( sd,
                                       buf,
                                       sz,
                                       ssl->rflags,
                                       ( SOCKADDR * ) &peer,
    ...
    {
        if( dtlsCtx->peer.sz > 0 && peerSz != ( XSOCKLENT ) dtlsCtx->peer.sz &&
                  ...
                  int * fam )
{
    ...
    if( ssl == NULL || ip == NULL || ipSz == NULL || port == NULL ||
        fam == NULL )
    {
        return BAD_FUNC_ARG;
    }
                  ...
                  int fam )
{
    ...
    switch( addr.ss_family )
    {
        case WOLFSSL_IP4:
            ...
            if( ( ret = wolfSSL_dtls_set_peer( ssl,
                                               ( SOCKADDR_IN * ) &addr,
                                               sizeof( SOCKADDR_IN ) ) ) !=
                WOLFSSL_SUCCESS )
            {
                      ...
                      word16 * outPort )
{
    ...
    else
    {
        ...
        if( url[ cur ] == '[' )
        {
            cur++;
            /* copy until ']' */
            while( i < MAX_URL_ITEM_SIZE - 1 && cur < urlSz &&
                   url[ cur ] != 0 && url[ cur ] != ']' )
            {
                if( outName )
                    outName[ i ] = url[ cur ];
                                          ...
                                          void * heap )
{
    ...
    {
        WOLFSSL_MSG(
            "wolfIO_HttpProcessResponseBuf invalid chunk or length size" );
        return MEMORY_E;
    }
    ...
    {
        WOLFSSL_MSG(
            "wolfIO_HttpProcessResponseBuf new receive size overflow" );
        return MEMORY_E;
    }
                                ...
                                void * heap )
{
    ...
    *respBuf = NULL;
    ...
    do
    {
        ...
        {
            /* get chunk of data */
            result = wolfIO_HttpProcessResponseBuf( sfd,
                                                    respBuf,
                                                    &respBufSz,
                                                    chunkSz,
                                                    start,
        ...
        if( end == NULL )
        {
            ...
            {
                WOLFSSL_MSG(
                    "wolfIO_HttpProcessResponse recv http from peer failed" );
                return -1;
            }
        }
        end = XSTRSTR( start, "\r\n" ); /* locate end */
            ...
            switch( state )
            {
                ...
                case phr_have_type:
                 ...
                 next lines we're checking for. */
                        WOLFSSL_MSG( "wolfIO_HttpProcessResponse content type "
                                     "is too short." );
                        return -1;
                    }
                    ...
                    if( XSTRNCASECMP( start, "Content-Type:", 13 ) == 0 )
                    {
                        ...
                        {
                            if( XSTRNCASECMP( start,
                                              appStrList[ i ],
                                              XSTRLEN( appStrList[ i ] ) ) ==
                                0 )
                            {
                                break;
                            }
                        ...
                        {
                            WOLFSSL_MSG(
                                "wolfIO_HttpProcessResponse appstr mismatch" );
                            return -1;
                        }
                        state = ( state == phr_http_start ) ? phr_have_type
                                                            : phr_wait_end;
                    }
                    else if( XSTRNCASECMP( start, "Content-Length:", 15 ) == 0 )
                    {
                        ...
                        while( *start == ' ' )
                            start++;
                        chunkSz = XATOI( start );
                        state = ( state == phr_http_start ) ? phr_have_length
                                                            : phr_wait_end;
                    }
                    else if( XSTRNCASECMP( start, "Transfer-Encoding:", 18 ) ==
                             0 )
                    {
                        start += 18;
                        while( *start == ' ' )
...
int wolfIO_HttpBuildRequest( const char * reqType,
                             ...
                             int bufSize )
{
    return wolfIO_HttpBuildRequest_ex( reqType,
                                       domainName,
                                       path,
                                       pathLen,
                                       reqSz,
...
int wolfIO_HttpBuildRequest_ex( const char * reqType,
                                ...
                                int bufSize )
{
    word32 reqTypeLen, domainNameLen, reqSzStrLen, contentTypeLen, exHdrsLen,
        maxLen;
    char reqSzStr[ 6 ];
    char * req = ( char * ) buf;
    const char * blankStr = " ";
    ...
    maxLen = reqTypeLen + blankStrLen + pathLen + http11StrLen + hostStrLen +
             domainNameLen + contentLenStrLen + reqSzStrLen +
             contentTypeStrLen + contentTypeLen + singleCrLfStrLen + exHdrsLen +
             doubleCrLfStrLen + 1 /* null term */;
                                 ...
                                 int bufSize )
{
    ...
    return wolfIO_HttpBuildRequest_ex( "POST",
                                       domainName,
                                       path,
                                       ( int ) XSTRLEN( path ),
                                       ocspReqSz,
...
int wolfIO_HttpProcessResponseOcsp( int sfd,
                                    ...
                                    void * heap )
{
    const char * appStrList[] = { "application/ocsp-response", NULL };

    return wolfIO_HttpProcessResponse( sfd,
                                       appStrList,
                                       respBuf,
                                       httpBuf,
                                       httpBufSz,
                     ...
                     byte ** ocspRespBuf )
{
    ...
    char domainName[ MAX_URL_ITEM_SIZE ];
            ...
            #ifdef WOLFSSL_SMALL_STACK
    path = ( char * ) XMALLOC( MAX_URL_ITEM_SIZE,
                               NULL,
                               DYNAMIC_TYPE_TMP_BUFFER );
    if( path == NULL )
        return MEMORY_E;

    domainName = ( char * ) XMALLOC( MAX_URL_ITEM_SIZE,
                                     NULL,
                                     DYNAMIC_TYPE_TMP_BUFFER );
    if( domainName == NULL )
    {
    ...
    else
    {
        ...
        byte * httpBuf = ( byte * ) XMALLOC( httpBufSz,
                                             ctx,
                                             DYNAMIC_TYPE_OCSP );
        ...
        {
            httpBufSz = wolfIO_HttpBuildRequestOcsp( domainName,
                                                     path,
                                                     ocspReqSz,
                                                     httpBuf,
                                                     httpBufSz );
            ...
            {
                ret = wolfIO_HttpProcessResponseOcsp( sfd,
                                                      ocspRespBuf,
                                                      httpBuf,
...
{
    const char * cacheCtl = "Cache-Control: no-cache";
    return wolfIO_HttpBuildRequest_ex( "GET",
                                       domainName,
                                       url,
                                       urlSz,
                                       0,
                                   ...
                                   int httpBufSz )
{
    ...
    const char * appStrList[] = { "application/pkix-crl",
                                  "application/x-pkcs7-crl",
                                  NULL };

    result = wolfIO_HttpProcessResponse( sfd,
                                         appStrList,
                                         &respBuf,
                                         httpBuf,
                                         httpBufSz,
...
int EmbedCrlLookup( WOLFSSL_CRL * crl, const char * url, int urlSz )
{
    ...
    else
    {
        ...
        {
            httpBufSz = wolfIO_HttpBuildRequestCrl( url,
                                                    urlSz,
                                                    domainName,
                                                    httpBuf,
                                                    httpBufSz );
            ...
            {
                ret = wolfIO_HttpProcessResponseCrl( crl,
                                                     sfd,
                                                     httpBuf,
                                                     HTTP_SCRATCH_BUFFER_SIZE );
            }
...
int NetX_Receive( WOLFSSL * ssl, char * buf, int sz, void * ctx )
{
    ...
    {
        status = nx_tcp_socket_receive( nxCtx->nxSocket,
                                        &nxCtx->nxPacket,
                                        nxCtx->nxWait );
    ...
    if( nxCtx->nxPacket )
    {
        ...
        status = nx_packet_data_extract_offset( nxCtx->nxPacket,
                                                nxCtx->nxOffset,
                                                buf,
                                                sz,
                                                &copied );
...
int NetX_Send( WOLFSSL * ssl, char * buf, int sz, void * ctx )
{
    ...
    status = nx_packet_allocate( pool, &packet, NX_TCP_PACKET, nxCtx->nxWait );
    if( status != NX_SUCCESS )
    {
...
void * mynewt_ctx_new()
{
    ...
    rc = os_mbuf_pool_init( &mynewt_ctx->mnMbufpool,
                            &mynewt_ctx->mnMempool,
                            mem_buf_count,
                            mem_buf_size );
    if( rc != 0 )
    {
...
int Mynewt_Receive( WOLFSSL * ssl, char * buf, int sz, void * ctx )
{
    ...
    if( mynewt_ctx->mnPacket == NULL )
    {
        ...
        {
            rc = mn_recvfrom( mynewt_ctx->mnSocket,
                              &m,
...
int Mynewt_Send( WOLFSSL * ssl, char * buf, int sz, void * ctx )
{
    ...
    rc = mn_sendto( mynewt_ctx->mnSocket,
                    m,
                    ( struct mn_sockaddr * ) &mynewt_ctx->mnSockAddrIn );
    if( rc != 0 )
    {
                           ...
                           struct mn_sockaddr_in * mnSockAddrIn )
{
    if( ssl && ssl->mnCtx )
    {
        ...
        memcpy( &mynewt_ctx->mnSockAddrIn,
                mnSockAddrIn,
...
int uIPSend( WOLFSSL * ssl, char * buf, int sz, void * _ctx )
{
    ...
    do
    {
        ...
        ret = tcp_socket_send( &ctx->conn.tcp,
                               ( unsigned char * ) buf + total_written,
                               bytes_left );
        if( ret <= 0 )
            break;
        total_written += ret;
...
int uIPSendTo( WOLFSSL * ssl, char * buf, int sz, void * _ctx )
{
    ...
    int ret = 0;
    ( void ) ssl;
    ret = udp_socket_sendto( &ctx->conn.udp,
                             ( unsigned char * ) buf,
                             sz,
                             &ctx->peer_addr,
                             ctx->peer_port );
...
int uIPGenerateCookie( WOLFSSL * ssl, byte * buf, int sz, void * _ctx )
{
    ...
    XMEMCPY( token + sizeof( uip_ipaddr_t ),
             &ctx->peer_port,
             sizeof( word16 ) );
    ret = wc_ShaHash( token,
                      sizeof( uip_ipaddr_t ) + sizeof( word16 ),
                      digest );
    if( ret != 0 )
        return ret;
    if( sz > WC_SHA_DIGEST_SIZE )
...
int GNRC_SendTo( WOLFSSL * ssl, char * buf, int sz, void * _ctx )
{
    ...
    if( !ctx )
        return WOLFSSL_CBIO_ERR_GENERAL;
    ret = sock_udp_send( &ctx->conn.udp,
                         ( unsigned char * ) buf,
                         sz,
                         &ctx->peer_addr );
    if( ret == 0 )
