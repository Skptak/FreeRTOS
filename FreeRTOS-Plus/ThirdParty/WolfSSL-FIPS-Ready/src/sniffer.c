                                 ...
                                 const char * keyFile )
{
    if( TraceOn )
    {
        fprintf( TraceFile, "\tTrying to install a new Sniffer Server with\n" );
        fprintf( TraceFile,
                 "\tname: %s, server: %s, port: %d, keyFile: %s\n",
                 name,
                 srv,
                 port,
                 keyFile );
...
static void TracePacket( void )
{
    if( TraceOn )
    {
        ...
        fprintf( TraceFile,
                 "\tGot a Packet to decode, packet %u\n",
                 ++packetNumber );
    }
}
...
static void TraceRelativeSequence( word32 expected, word32 got )
{
    ...
    {
        fprintf( TraceFile,
                 "\tExpected sequence:%u, received sequence:%u\n",
                 expected,
                 got );
    }
}
...
static void TraceClientFin( word32 finSeq, word32 relSeq )
{
    ...
    {
        fprintf( TraceFile,
                 "\tClient FIN capture:%u, current SEQ:%u\n",
                 finSeq,
                 relSeq );
    }
}
...
static void TraceServerFin( word32 finSeq, word32 relSeq )
{
    ...
    {
        fprintf( TraceFile,
                 "\tServer FIN capture:%u, current SEQ:%u\n",
                 finSeq,
                 relSeq );
    }
}
...
static int IsServerRegistered6( byte * addr )
{
    ...
    while( sniffer )
    {
        if( sniffer->server.version == IPV6 &&
            0 == XMEMCMP( sniffer->server.ip6,
                          addr,
                          sizeof( sniffer->server.ip6 ) ) )
        {
            ret = 1;
            break;
        }
                        ...
                        const char * password )
{
    ...
    if( typeKey == WOLFSSL_FILETYPE_PEM )
    {
        ...
        if( saveBuf != NULL )
        {
            saveBufSz = wc_KeyPemToDer( loadBuf,
                                        ( int ) fileSz,
                                        saveBuf,
                                        ( int ) fileSz,
                                        password );
                               ...
                               int isEphemeralKey )
{
    ...
    if( name != NULL )
    {
        namedKey = ( NamedKey * ) XMALLOC( sizeof( NamedKey ),
                                           NULL,
                                           DYNAMIC_TYPE_SNIFFER_NAMED_KEY );
        if( namedKey == NULL )
        {
            SetError( MEMORY_STR, error, NULL, 0 );
            return -1;
    ...
    sniffer = ServerList;
    while( sniffer != NULL && ( !MatchAddr( sniffer->server, serverIp ) ||
                                sniffer->port != port ) )
    {
        sniffer = sniffer->next;
    }
    ...
    if( name == NULL )
    {
        if( password )
        {
            ...
            wolfSSL_CTX_set_default_passwd_cb_userdata( sniffer->ctx,
                                                        ( void * ) password );
        #endif
        }
        ...
        if( isEphemeralKey )
        {
            ...
            ret = wolfSSL_CTX_set_ephemeral_key( sniffer->ctx,
                                                 WC_PK_TYPE_NONE,
                                                 keyFile,
                                                 0,
                                                 type );
        ...
        #endif
        {
            if( keySz == 0 )
            {
                ret = wolfSSL_CTX_use_PrivateKey_file( sniffer->ctx,
                                                       keyFile,
                                                       type );
            }
            else
...
static int CheckIp6Hdr( Ip6Hdr * iphdr, IpInfo * info, int length, char * error )
{
    ...
    if( iphdr->next_header != TCP_PROTOCOL )
    {
        ...
        #ifndef WOLFSSL_SNIFFER_WATCH
    if( !IsServerRegistered6( iphdr->src ) &&
        !IsServerRegistered6( iphdr->dst ) )
    {
...
static int CheckTcpHdr( TcpHdr * tcphdr, TcpInfo * info, char * error )
{
    ...
    if( info->ack )
        ...
        #ifndef WOLFSSL_SNIFFER_WATCH
    if( !IsPortRegistered( info->srcPort ) &&
        !IsPortRegistered( info->dstPort ) )
    {
...
static void CopySessionInfo( SnifferSession * session, SSLInfo * sslInfo )
{
    if( NULL != sslInfo )
    {
        ...
        if( 0 != session->sslServer->options.cipherSuite )
        {
            ...
            sslInfo->serverCipherSuite0 = session->sslServer->options
                                              .cipherSuite0;
...
{
    PrintSecret( "server master secret",
                 session->sslServer->arrays->masterSecret,
                 SECRET_LEN );
    PrintSecret( "client master secret",
                      ...
                      DerBuffer * keyBuf )
{
    ...
    if( ksInfo == NULL )
    {
        ...
        if( ret == 0 )
        {
            ret = wc_RsaPrivateKeyDecode( keyBuf->buffer,
                                          &idx,
                                          &key,
                                          keyBuf->length );
            if( ret != 0 )
    ...
    if( ksInfo && ksInfo->dh_key_bits != 0 )
    {
        ...
        if( ret == 0 )
        {
            ...
            if( ret == 0 )
            {
                ret = wc_DhKeyDecode( keyBuf->buffer,
                                      &idx,
                                      &dhKey,
                                      keyBuf->length );
            }
    ...
    if( ksInfo && ksInfo->curve_id != 0 )
    {
        ...
        {
            pubKeyInit = 1;
            ret = wc_EccPrivateKeyDecode( keyBuf->buffer,
                                          &idx,
                                          &key,
                                          keyBuf->length );
            if( ret != 0 )
        ...
        if( ret == 0 )
        {
             ...
             * for the type byte. */
            if( IsTLS( session->sslServer ) &&
                !IsAtLeastTLSv1_3( session->sslServer->version ) )
            {
        ...
        if( ret == 0 )
        {
            ret = wc_ecc_import_x963_ex( input,
                                         length,
                                         &pubKey,
                                         ksInfo->curve_id );
            if( ret != 0 )
            {
                SetError( ECC_PUB_DECODE_STR,
                          error,
                          session,
                          FATAL_ERROR_STATE );
            }
    ...
    XMEMCPY( session->sslClient->arrays->preMasterSecret,
             session->sslServer->arrays->preMasterSecret,
             session->sslServer->arrays->preMasterSz );
    session->sslClient->arrays->preMasterSz = session->sslServer->arrays
                                                  ->preMasterSz;
    ...
    if( IsAtLeastTLSv1_3( session->sslServer->version ) )
    {
        ...
        ret += DeriveTls13Keys( session->sslServer,
                                handshake_key,
                                ENCRYPT_AND_DECRYPT_SIDE,
                                1 );
        ret += DeriveTls13Keys( session->sslClient,
                                handshake_key,
                                ENCRYPT_AND_DECRYPT_SIDE,
                                1 );
                            ...
                            word16 filter_group )
{
    ...
    while( index < len )
    {
        ...
        info->named_group = ( word16 ) ( ( input[ index ] << 8 ) |
                                         input[ index + 1 ] );
        index += OPAQUE16_LEN;
        info->key_len = ( word16 ) ( ( input[ index ] << 8 ) |
                                     input[ index + 1 ] );
        index += OPAQUE16_LEN;
                                  ...
                                  char * error )
{
    ...
    if( ret == 0 )
    {
        ...
        ret = ProcessKeyShare( &session->cliKs,
                               session->cliKeyShare,
                               session->cliKeyShareSz,
                               session->srvKs.named_group );
    }
    if( ret != 0 )
    {
    ...
    *sslBytes -= TICKET_HINT_LEN;
    ...
    if( IsAtLeastTLSv1_3( ssl->version ) )
    {
        ...
        *sslBytes -= OPAQUE8_LEN;
        ...
        if( len > 0 )
            XMEMCPY( &session->sslServer->session.ticketNonce.data,
                     input,
                     len );
            #endif
        input += len;
                               ...
                               char * error )
{
    ...
    *sslBytes -= ENUM_LEN;
    ...
    if( ( initialBytes - *sslBytes ) < msgSz )
    {
        ...
        {
            SetError( SERVER_HELLO_INPUT_STR,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
    ...
    if( session->sslServer->options.haveSessionId )
    {
        if( XMEMCMP( session->sslServer->arrays->sessionID,
                     session->sslClient->arrays->sessionID,
                     ID_LEN ) == 0 )
        {
            doResume = 1;
        }
    ...
    if( doResume )
    {
        ...
        if( IsAtLeastTLSv1_3( session->sslServer->version ) )
        {
            resume = GetSession( session->sslServer,
                                 session->sslServer->session.masterSecret,
                                 0 );
        }
        else
        {
        ...
        #endif
            SetError( BAD_SESSION_RESUME_STR,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
        ...
        if( IsAtLeastTLSv1_3( session->sslServer->version ) )
        {
            ...
            session->sslServer->arrays->psk_keySz = session->sslServer->specs
                                                        .hash_size;
            session->sslClient->arrays->psk_keySz = session->sslClient->specs
                                                        .hash_size;
            ret = DeriveResumptionPSK(
                session->sslServer,
    ...
    printf( "cipher suite = 0x%02x\n",
            session->sslServer->options.cipherSuite );
    PrintSecret( "server random",
                 session->sslServer->arrays->serverRandom,
                 RAN_LEN );
        #endif
    ...
    if( IsAtLeastTLSv1_3( session->sslServer->version ) )
    {
        ...
        if( session->sslServer->staticKE.key )
            ...
            #endif
        ret = SetupKeys( session->cliKs.key,
                         &session->cliKs.key_len,
                         session,
                         error,
                         &session->cliKs,
                         key );
                               ...
                               char * error )
{
        ...
        #ifdef HAVE_SNI
    {
        ...
        ret = wolfSSL_SNI_GetFromBuffer( input - HANDSHAKE_HEADER_SZ -
                                             RECORD_HEADER_SZ,
    ...
    *sslBytes -= ENUM_LEN;
    if( bLen )
    {
        if( ID_LEN > *sslBytes )
        {
            SetError( CLIENT_HELLO_INPUT_STR,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
        ...
        *sslBytes -= LENGTH_SZ;
        ...
        switch( extType )
        {
            ...
            case EXT_KEY_SHARE:
            {
                ...
                session->cliKeyShare = ( byte * )
                    XMALLOC( ksLen, NULL, DYNAMIC_TYPE_TMP_BUFFER );
            ...
            case EXT_PRE_SHARED_KEY:
            {
                ...
                /* Hash data up to binders for deriving binders in PSK
                 * extension. */
                HashRaw( session->sslServer,
                         inputHello - HANDSHAKE_HEADER_SZ,
                         inputHelloSz - bindersLen + HANDSHAKE_HEADER_SZ );
                HashRaw( session->sslClient,
                         inputHello - HANDSHAKE_HEADER_SZ,
                         inputHelloSz - bindersLen + HANDSHAKE_HEADER_SZ );
                ...
                {
                    /* we aren't decrypting the resumption, since we know the
                     * master secret */
                    /* ignore errors */
                }
                ...
                HashRaw( session->sslServer,
                         inputHello + inputHelloSz - bindersLen,
                         bindersLen );
                HashRaw( session->sslClient,
                         inputHello + inputHelloSz - bindersLen,
                         bindersLen );
            ...
            case EXT_TICKET_ID:
                ...
                if( extLen )
                {
                    ...
                    {
                        session->ticketID = ( byte * )
                            XMALLOC( ID_LEN,
                                     NULL,
                                     DYNAMIC_TYPE_SNIFFER_TICKET_ID );
    ...
    *sslBytes -= ( int ) inOutIdx;
    ...
    if( IsAtLeastTLSv1_3( ssl->version ) )
    {
        ...
        if( session->flags.side == WOLFSSL_SERVER_END )
        {
            ...
            #ifdef HAVE_SESSION_TICKET
            /* derive resumption secret for next session - on finished (from
             * client) */
            ret += DeriveResumptionSecret( session->sslClient,
                                           session->sslClient->session
                                               .masterSecret );
                        ...
                        char * error )
{
    ...
    *sslBytes -= HANDSHAKE_HEADER_SZ;
    ...
    if( !IsAtLeastTLSv1_3( ssl->version ) )
    {
        ...
        {
            SetError( NO_SECURE_RENEGOTIATION,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
    ...
    if( type != client_hello )
    {
        ...
        /* TLS v1.3 requires the hash for the handshake and transfer key
         * derivation */
    ...
    switch( type )
    {
        ...
        case client_key_exchange:
            ...
            {
                if( HashCopy( session->sslServer->hsHashes, session->hash ) ==
                        0 &&
        ...
        #endif
            if( ret == 0 )
                ret = ProcessClientKeyExchange( input,
                                                sslBytes,
                                                session,
                                                error );
            break;
...
static int Decrypt( WOLFSSL * ssl, byte * output, const byte * input, word32 sz )
{
    ...
    switch( ssl->specs.bulk_cipher_algorithm )
    {
        ...
        case wolfssl_aes_gcm:
            if( sz >=
                ( word32 ) ( AESGCM_EXP_IV_SZ + ssl->specs.aead_mac_size ) )
            {
                       ...
                       char * error )
{
    ...
    *rhSize = ( ( b0 & 0x7f ) << 8 ) | b1;
    ...
    ret = ProcessOldClientHello( session->sslServer,
                                 input,
                                 &idx,
                                 *sslBytes,
                                 ( word16 ) *rhSize );
    ...
    XMEMCPY( session->sslClient->arrays->clientRandom,
             session->sslServer->arrays->clientRandom,
             RAN_LEN );
                         ...
                         char * error )
{
    ...
    if( CheckTcpHdr( ( TcpHdr * ) ( packet + ipInfo->length ),
                     tcpInfo,
                     error ) != 0 )
        return -1;
                                    ...
                                    int * bytesLeft )
{
    ...
    pb->begin = *begin;
    pb->end = end;
    pb->data = ( byte * ) XMALLOC( added,
                                   NULL,
                                   DYNAMIC_TYPE_SNIFFER_PB_BUFFER );
                            ...
                            char * error )
{
    ...
    while( bytesLeft > 0 )
    {
        ...
        add = CreateBuffer( &seq,
                            seq + added - 1,
                            &sslFrame[ seq - startSeq ],
                            &bytesLeft );
        if( add == NULL )
        {
            SetError( MEMORY_STR, error, session, FATAL_ERROR_STATE );
                           ...
                           char * error )
{
    ...
    word32 * expected = ( session->flags.side == WOLFSSL_SERVER_END )
                            ? &session->cliExpected
                            : &session->srvExpected;
    PacketBuffer * reassemblyList = ( session->flags.side ==
                                      WOLFSSL_SERVER_END )
                                        ? session->cliReassemblyList
                                        : session->srvReassemblyList;
            ...
            *sslBytes -= overlap;
            ...
            if( reassemblyList )
            {
                ...
                if( newEnd > reassemblyList->end )
                {
                    ...
                    AddToReassembly( session->flags.side,
                                     reassemblyList->end + 1,
                                     *sslFrame + reassemblyList->end -
                                         *expected + 1,
                                     newEnd - reassemblyList->end,
                                     session,
                                     error );
                }
            }
    ...
    else if( real > *expected )
    {
        ...
        if( *sslBytes > 0 )
        {
            int addResult = AddToReassembly( session->flags.side,
                                             real,
                                             *sslFrame,
                                             *sslBytes,
                                             session,
    ...
    else if( *sslBytes > 0 )
    {
        ...
        else if( reassemblyList )
        {
            ...
            if( newEnd > reassemblyList->end )
            {
                ...
                AddToReassembly( session->flags.side,
                                 reassemblyList->end + 1,
                                 *sslFrame + reassemblyList->end - *expected +
                                     1,
                                 newEnd - reassemblyList->end,
                                 session,
                                 error );
    ...
    while( curr != NULL )
    {
        *expected = curr->end + 1;
        ...
        if( curr->data[ 0 ] == application_data &&
            curr->data[ 1 ] == pv.major && curr->data[ 2 ] == pv.minor )
        {
                           ...
                           char * error )
{
    ...
    if( vChain != NULL )
    {
        ...
        XMEMCPY( &ssl->buffers.inputBuffer.buffer[ length ],
                 ( byte * ) chain[ 0 ].iov_base + headerSz,
                 qty );
        offset = length;
        for( i = 1; i < chainSz; i++ )
        {
    ...
    if( ( *session )->flags.clientHello == 0 && **sslFrame != handshake )
    {
        ...
        int rhSize = ( ( ( *sslFrame )[ 0 ] & 0x7f ) << 8 ) |
                     ( ( *sslFrame )[ 1 ] );

        if( ( rhSize <= ( *sslBytes - 2 ) ) &&
            ( *sslFrame )[ 2 ] == OLD_HELLO_ID &&
            ( *sslFrame )[ 3 ] == SSLv3_MAJOR )
        {
        #ifdef OLD_HELLO_ALLOWED
            int ret = DoOldHello( *session,
                                  *sslFrame,
                                  &rhSize,
                                  sslBytes,
                                  error );
                          ...
                          char * error )
{
    ...
    word32 *
        bufferSize = ( session->flags.side == WOLFSSL_SERVER_END )
...
doMessage:
          ...
          session->flags.clientCipherOn ) )
    {
        ...
        {
            SetError( DECRYPT_KEYS_NOT_SETUP,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
...
doPart:
    ...
    switch( ( enum ContentType ) rh.type )
    {
                        ...
                        if( data != NULL )
                        {
                            ...
                            *data = tmpData;
                            XMEMCPY( *data + decoded,
                                     ssl->buffers.clearOutputBuffer.buffer,
                                     ret );
                        }
                        else
                        {
                            ...
                            {
                                SetError( STORE_DATA_CB_MISSING_STR,
                                          error,
                                          session,
                                          FATAL_ERROR_STATE );
                                return -1;
        ...
        case no_type:
        default:
            SetError( GOT_UNKNOWN_RECORD_STR,
                      error,
                      session,
                      FATAL_ERROR_STATE );
            return -1;
...
{
    int ret = 0;
    if( session->finCapture.cliFinSeq &&
        session->finCapture.cliFinSeq <= session->cliExpected )
    {
        if( session->finCapture.cliCounted == 0 )
        {
            session->flags.finCount += 1;
            session->finCapture.cliCounted = 1;
            TraceClientFin( session->finCapture.cliFinSeq,
                            session->cliExpected );
        }
    }

    if( session->finCapture.srvFinSeq &&
        session->finCapture.srvFinSeq <= session->srvExpected )
    {
        if( session->finCapture.srvCounted == 0 )
        {
            session->flags.finCount += 1;
            session->finCapture.srvCounted = 1;
            TraceServerFin( session->finCapture.srvFinSeq,
                            session->srvExpected );
        }
    }
...
int ssl_DecodePacketWithSessionInfo( const unsigned char * packet,
                                     ...
                                     char * error )
{
    return ssl_DecodePacketInternal( packet,
                                     length,
                                     NULL,
                                     0,
                                     data,
...
int ssl_DecodePacketWithChain( void * vChain,
                               ...
                               char * error )
{
    return ssl_DecodePacketInternal( NULL,
                                     0,
                                     vChain,
                                     chainSz,
                                     data,
                          ...
                          char * error )
{
    ...
    ret = ssl_SetWatchKey_buffer( vSniffer,
                                  keyBuf,
                                  keyBufSz,
                                  FILETYPE_DER,
                                  error );
