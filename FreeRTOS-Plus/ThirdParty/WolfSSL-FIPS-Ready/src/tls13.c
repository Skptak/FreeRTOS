                         ...
                         int hashAlgo )
{
    ...
    return HKDF_Expand_Label( output,
                              outputLen,
                              secret,
                              hashSz,
                              protocol,
...
static const byte binderKeyLabel[ BINDER_KEY_LABEL_SZ + 1 ] = "ext binder";
...
static const byte
    binderKeyResumeLabel[ BINDER_KEY_RESUME_LABEL_SZ + 1 ] = "res binder";
...
static const byte earlyTrafficLabel[ EARLY_TRAFFIC_LABEL_SZ + 1 ] = "c e "
                                                                    "traffic";
...
static int DeriveEarlyTrafficSecret( WOLFSSL * ssl, byte * key )
{
    ...
    ret = DeriveKey( ssl,
                     ...
                     ssl->arrays->secret,
                     earlyTrafficLabel,
                     EARLY_TRAFFIC_LABEL_SZ,
                     ssl->specs.mac_algorithm,
                     1 );
            #ifdef HAVE_SECRET_CALLBACK
    if( ret == 0 && ssl->tls13SecretCb != NULL )
    {
        ret = ssl->tls13SecretCb( ssl,
...
static const byte earlyExporterLabel[ EARLY_EXPORTER_LABEL_SZ + 1 ] = "e exp "
                                                                      "master";
...
static const byte
    clientHandshakeLabel[ CLIENT_HANDSHAKE_LABEL_SZ + 1 ] = "c hs traffic";
...
static const byte
    serverHandshakeLabel[ SERVER_HANDSHAKE_LABEL_SZ + 1 ] = "s hs traffic";
...
static int DeriveServerHandshakeSecret( WOLFSSL * ssl, byte * key )
{
    ...
    ret = DeriveKey( ssl,
                     ...
                     -1,
                     ssl->arrays->preMasterSecret,
                     serverHandshakeLabel,
                     SERVER_HANDSHAKE_LABEL_SZ,
                     ssl->specs.mac_algorithm,
                     1 );
        #ifdef HAVE_SECRET_CALLBACK
    if( ret == 0 && ssl->tls13SecretCb != NULL )
...
static const byte clientAppLabel[ CLIENT_APP_LABEL_SZ + 1 ] = "c ap traffic";
...
static const byte serverAppLabel[ SERVER_APP_LABEL_SZ + 1 ] = "s ap traffic";
...
static const byte exporterMasterLabel[ EXPORTER_MASTER_LABEL_SZ + 1 ] = "exp "
                                                                        "maste"
                                                                        "r";
...
static const byte resumeMasterLabel[ RESUME_MASTER_LABEL_SZ + 1 ] = "res "
                                                                    "master";
...
int DeriveResumptionSecret( WOLFSSL * ssl, byte * key )
{
    ...
    return DeriveKey( ssl,
                      key,
                      -1,
                      ssl->arrays->masterSecret,
                      resumeMasterLabel,
                      RESUME_MASTER_LABEL_SZ,
                      ssl->specs.mac_algorithm,
                      1 );
}
        #endif
...
static const byte appTrafficLabel[ APP_TRAFFIC_LABEL_SZ + 1 ] = "traffic upd";
...
int DeriveEarlySecret( WOLFSSL * ssl )
{
    ...
    if( ssl == NULL || ssl->arrays == NULL )
    {
        ...
        #if defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK )
    return Tls13_HKDF_Extract( ssl->arrays->secret,
                               NULL,
                               0,
                               ssl->arrays->psk_key,
...
/* The derived label. */
static const byte derivedLabel[ DERIVED_LABEL_SZ + 1 ] = "derived";
                         ...
                         byte * secret )
{
    ...
    return HKDF_Expand_Label( secret,
                              ssl->specs.hash_size,
                              ssl->session.masterSecret,
                              ssl->specs.hash_size,
                              protocol,
                              protocolLen,
                              resumptionLabel,
...
int DeriveTls13Keys( WOLFSSL * ssl, int secret, int side, int store )
{
    ...
    switch( secret )
    {
        ...
        case handshake_key:
            ...
            {
                ret = DeriveClientHandshakeSecret( ssl, ssl->clientSecret );
                if( ret != 0 )
                    goto end;
            }
            if( provision & PROVISION_SERVER )
            {
                ret = DeriveServerHandshakeSecret( ssl, ssl->serverSecret );
                if( ret != 0 )
                    goto end;
            }
            break;
                                 ...
                                 WOLFSSL * ssl )
{
    ...
    AddTls13HandShakeHeader( output + outputAdj,
                             length,
                             fragOffset,
                             fragSz,
                             type,
                                     ...
                                     byte * tag )
{
    ...
    ret = wc_Poly1305_MAC( ssl->auth.poly1305,
                           ( byte * ) aad,
                           aadSz,
                           output,
                           sz,
    ...
    switch( ssl->encrypt.state )
    {
        case CIPHER_STATE_BEGIN:
        {
            ...
            if( ssl->encrypt.nonce == NULL )
                ssl->encrypt.nonce = ( byte * )
                    XMALLOC( AEAD_NONCE_SZ,
                             ssl->heap,
                             DYNAMIC_TYPE_AES_BUFFER );
            if( ssl->encrypt.nonce == NULL )
                return MEMORY_E;

            BuildTls13Nonce( ssl,
                             ssl->encrypt.nonce,
                             ssl->keys.aead_enc_imp_IV,
                             CUR_ORDER );
        #endif
        ...
        case CIPHER_STATE_DO:
        {
            switch( ssl->specs.bulk_cipher_algorithm )
            {
                ...
                case wolfssl_aes_gcm:
                 ...
                 ( !defined( HAVE_FIPS_VERSION ) ||                      \
                   ( HAVE_FIPS_VERSION < 2 ) ) )
                    ...
                    if( ret == 0 )
                    {
                        ret = wc_AesGcmEncrypt_ex( ssl->encrypt.aes,
                                                   output,
                                                   input,
                                                   dataSz,
                                                   ssl->encrypt.nonce,
                ...
                case wolfssl_aes_ccm:
                 ...
                 ( !defined( HAVE_FIPS_VERSION ) ||                      \
                   ( HAVE_FIPS_VERSION < 2 ) ) )
                    ...
                    if( ret == 0 )
                    {
                        ret = wc_AesCcmEncrypt_ex( ssl->encrypt.aes,
                                                   output,
                                                   input,
                                                   dataSz,
                                                   ssl->encrypt.nonce,
                ...
                case wolfssl_cipher_null:
                    ret = Tls13IntegrityOnly_Encrypt( ssl,
                                                      output,
                                                      input,
                                                      dataSz,
                                                      ssl->encrypt.nonce,
                                     ...
                                     const byte * tagIn )
{
    ...
    if( ( ret = wc_Poly1305_MAC( ssl->auth.poly1305,
                                 ( byte * ) aad,
                                 aadSz,
                                 ( byte * ) input,
                                 sz,
                  ...
                  word16 aadSz )
{
    ...
    switch( ssl->decrypt.state )
    {
        ...
        case CIPHER_STATE_DO:
        {
            switch( ssl->specs.bulk_cipher_algorithm )
            {
                ...
                case wolfssl_aes_gcm:
                    ...
                    nonceSz = AESGCM_NONCE_SZ;
                    ret = wc_AesGcmDecrypt( ssl->decrypt.aes,
                                            output,
                                            input,
                                            dataSz,
                                            ssl->decrypt.nonce,
                ...
                case wolfssl_aes_ccm:
                    ...
                    nonceSz = AESCCM_NONCE_SZ;
                    ret = wc_AesCcmDecrypt( ssl->decrypt.aes,
                                            output,
                                            input,
                                            dataSz,
                                            ssl->decrypt.nonce,
        ...
        #if defined( HAVE_CHACHA ) && defined( HAVE_POLY1305 )
                case wolfssl_chacha:
                    ret = ChaCha20Poly1305_Decrypt( ssl,
                                                    output,
                                                    input,
                                                    dataSz,
                                                    ssl->decrypt.nonce,
        ...
        #ifdef HAVE_NULL_CIPHER
                case wolfssl_cipher_null:
                    ret = Tls13IntegrityOnly_Decrypt( ssl,
                                                      output,
                                                      input,
                                                      dataSz,
                                                      ssl->decrypt.nonce,
                       ...
                       int asyncOkay )
{
    ...
    typedef char
        args_test[ sizeof( ssl->async.args ) >= sizeof( *args ) ? 1 : -1 ];
    ...
    if( !psk->resumption )
    {
            ...
            psk->cipherSuite != cipherSuite )
        {
            return PSK_KEY_ERROR;
                    ...
                    #else
                            /* PSK information loaded during setting of default
                             * TLS extensions. */
                    #endif
    }
                #endif
...
static int WritePSKBinders( WOLFSSL * ssl, byte * output, word32 idx )
{
    ...
    ret = TLSX_PreSharedKey_WriteBinders( ( PreSharedKey * ) ext->data,
                                          output + idx,
                                          client_hello,
                                          &len );
    if( ret < 0 )
...
int SendTls13ClientHello( WOLFSSL * ssl )
{
    ...
    if( ( ret = TLSX_PopulateExtensions( ssl, 0 ) ) != 0 )
        return ret;
            #ifdef WOLFSSL_EARLY_DATA
                #ifndef NO_PSK
    if( !ssl->options.resuming && ssl->options.client_psk_tls13_cb == NULL &&
                        ...
                        byte * extMsgType )
{
    ...
    *inOutIdx = i;
    ...
    if( ssl->session.sessionIDSz != 0 )
    {
        ...
        return INVALID_PARAMETER;
            #else
    if( sessIdSz != ssl->session.sessionIDSz ||
        ( sessIdSz > 0 &&
          XMEMCMP( ssl->session.sessionID, sessId, sessIdSz ) != 0 ) )
    {
          ...
          ssl->options.cipherSuite == TLS_SHA384_SHA384 ) )
    {
        ...
        {
            WOLFSSL_MSG(
                "Server sent non-TLS13 cipher suite in TLS 1.3 packet" );
            return INVALID_PARAMETER;
        }
                                      ...
                                      word32 size )
{
     ...
     */
    certReqCtx = ( CertReqCtx * ) XMALLOC( sizeof( CertReqCtx ) + len - 1,
                                           ssl->heap,
    ...
    *inOutIdx += len;
    ...
    {
        if( PickHashSigAlgo( ssl,
                             peerSuites.hashSigAlgo,
    ...
    while( current != NULL )
    {
            ...
            WOLFSSL_TICKET_RET_OK )
        {
            ...
            if( ( ssl->options.server_psk_tls13_cb != NULL &&
                  ( ssl->arrays->psk_keySz = ssl->options.server_psk_tls13_cb(
                        ssl,
                        ssl->arrays->client_identity,
                        ssl->arrays->psk_key,
                        MAX_PSK_KEY_LEN,
                        &cipherName ) ) != 0 &&
                  GetCipherSuiteFromName( cipherName,
                                          &cipherSuite0,
                                          &cipherSuite ) == 0 ) ||
                ( ssl->options.server_psk_cb != NULL &&
                  ( ssl->arrays->psk_keySz = ssl->options.server_psk_cb(
                        ssl,
                        ssl->arrays->client_identity,
                        ssl->arrays->psk_key,
                        MAX_PSK_KEY_LEN ) ) != 0 ) )
...
static int CheckCookie( WOLFSSL * ssl, byte * cookie, byte cookieSz )
{
    ...
    return cookieSz;
                ...
                #define HRR_KEY_SHARE_SZ \
                    ( OPAQUE16_LEN + OPAQUE16_LEN + OPAQUE16_LEN )
                /* Length of the Supported Vresions Extension */
                #define HRR_VERSIONS_SZ \
                    ( OPAQUE16_LEN + OPAQUE16_LEN + OPAQUE16_LEN )
                /* Length of the Cookie Extension excluding cookie data */
                #define HRR_COOKIE_HDR_SZ \
                    ( OPAQUE16_LEN + OPAQUE16_LEN + OPAQUE16_LEN )
                /* PV | Random | Session Id | CipherSuite | Compression | Ext
                 * Len */
                #define HRR_BODY_SZ                                          \
                    ( VERSION_SZ + RAN_LEN + ENUM_LEN + ID_LEN + SUITE_LEN + \
                      COMP_LEN + OPAQUE16_LEN )
                /* HH | PV | CipherSuite | Ext Len | Key Share | Supported
                 * Version | Cookie */
                #define MAX_HRR_SZ                                           \
                    ( HANDSHAKE_HEADER_SZ + HRR_BODY_SZ + HRR_KEY_SHARE_SZ + \
                      HRR_VERSIONS_SZ + HRR_COOKIE_HDR_SZ )
                        ...
                        word32 helloSz )
{
    ...
    {
        ret = DoTls13SupportedVersions( ssl,
                                        input + begin,
                                        i - begin,
                                        helloSz,
                                        &wantDowngrade );
    ...
    if( ret >= 0 )
    {
        ...
        if( extension )
        {
            ssl->secure_renegotiation = ( SecureRenegotiation * )
                                            extension->data;
    ...
    ssl->options.side = WOLFSSL_SERVER_END;
            #endif /* OPENSSL_ALL || HAVE_STUNNEL || WOLFSSL_NGINX || \
                      WOLFSSL_HAPROXY */
    ...
    *inOutIdx = i;
    ...
    if( !usingPSK )
    {
        ...
        {
            WOLFSSL_MSG(
                "Client did not send a SignatureAlgorithms extension" );
            SendAlert( ssl, alert_fatal, missing_extension );
            return INCOMPLETE_DATA;
        }
...
int SendTls13ServerHello( WOLFSSL * ssl, byte extMsgType )
{
    ...
    if( extMsgType == server_hello )
    {
        ...
        if( ( ret = wc_RNG_GenerateBlock( ssl->rng, output + idx, RAN_LEN ) ) !=
            0 )
            return ret;
    }
    else
...
static WC_INLINE int GetMsgHash( WOLFSSL * ssl, byte * hash )
{
    ...
    return ret;
                ...
                /* The length of the certificate verification label - client and
                 * server. */
                #define CERT_VFY_LABEL_SZ 34
/* The server certificate verification label. */
static const byte serverCertVfyLabel[ CERT_VFY_LABEL_SZ ] = "TLS 1.3, server "
                                                            "CertificateVerify";
/* The client certificate verification label. */
static const byte clientCertVfyLabel[ CERT_VFY_LABEL_SZ ] = "TLS 1.3, client "
                                                            "CertificateVerify";
                ...
                #define MAX_SIG_DATA_SZ                            \
                    ( SIGNING_DATA_PREFIX_SZ + CERT_VFY_LABEL_SZ + \
                      WC_MAX_DIGEST_SIZE )
                              ...
                              word32 decSigSz )
{
    ...
    if( sigAlgo == rsa_pss_sa_algo )
    {
        ...
        ret = CreateRSAEncodedSig( sigData,
                                   sigData,
                                   sigDataSz,
                                   sigAlgo,
                                   hashAlgo );
        ...
        ret = wc_RsaPSS_CheckPadding( sigData,
                                      sigSz,
                                      decSig,
                                      decSigSz,
                                      hashType );
...
static int SendTls13Certificate( WOLFSSL * ssl )
{
    ...
    else
    {
        ...
        {
            ret = AllocDer( &ssl->buffers.certExts,
                            extSz,
                            CERT_TYPE,
                            ssl->heap );
            if( ret < 0 )
            ...
            extSz = 0;
            ret = TLSX_WriteResponse( ssl,
                                      ssl->buffers.certExts->buffer,
                                      certificate,
                                      &extSz );
            if( ret < 0 )
                return ret;
...
static int SendTls13CertificateVerify( WOLFSSL * ssl )
{
    ...
    switch( ssl->options.asyncState )
    {
        ...
        case TLS_ASYNC_BUILD:
        {
            ...
            args->verify = &args->output[ RECORD_HEADER_SZ +
                                          HANDSHAKE_HEADER_SZ ];
            ...
            if( ssl->hsType == DYNAMIC_TYPE_RSA )
            {
                ...
                if( args->length > MAX_SIG_DATA_SZ )
                    sigLen = args->length;
                args->sigData = ( byte * ) XMALLOC( sigLen,
                                                    ssl->heap,
                                                    DYNAMIC_TYPE_SIGNATURE );
            }
            else
            {
                args->sigData = ( byte * ) XMALLOC( MAX_SIG_DATA_SZ,
                                                    ssl->heap,
                                                    DYNAMIC_TYPE_SIGNATURE );
            }
            ...
            if( ssl->hsType == DYNAMIC_TYPE_RSA )
            {
                ...
                ret = CreateRSAEncodedSig( sig->buffer,
                                           args->sigData,
                                           args->sigDataSz,
                                           args->sigAlgo,
                                           ssl->suites->hashAlgo );
                                     ...
                                     word32 totalSz )
{
    ...
    switch( ssl->options.asyncState )
    {
        ...
        case TLS_ASYNC_BUILD:
        {
            ...
            sig->buffer = ( byte * ) XMALLOC( args->sz,
                                              ssl->heap,
                                              DYNAMIC_TYPE_SIGNATURE );
            if( sig->buffer == NULL )
            {
                     ...
                     int sniff )
{
    ...
    *inOutIdx += size + ssl->keys.padSz;
    ...
    if( ssl->options.side == WOLFSSL_SERVER_END && !ssl->options.handShakeDone )
    {
        ...
        if( ssl->earlyData != no_early_data )
        {
            if( ( ret = DeriveTls13Keys( ssl, no_key, DECRYPT_SIDE_ONLY, 1 ) ) !=
                0 )
                return ret;
        }
        #endif
...
static int SendTls13KeyUpdate( WOLFSSL * ssl )
{
     ...
     * I want a response.
     */
    ssl->keys.updateResponseReq = output[ i++ ] = !ssl->keys.updateResponseReq &&
...
static int ExpectedResumptionSecret( WOLFSSL * ssl )
{
    ...
    if( ( ret = DeriveResumptionSecret( ssl, ssl->session.masterSecret ) ) !=
        0 )
        return ret;
            ...
            if( ssl->options.side == WOLFSSL_CLIENT_END )
            {
                if( ssl->options.clientState < CLIENT_HELLO_COMPLETE )
                {
                    ...
                    return OUT_OF_ORDER_E;
                        ...
                        /* Must have seen certificate and verify from server
                         * except when using PSK. */
            #if defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK )
                if( ssl->arrays->psk_keySz != 0 )
                {
                    if( ssl->options.serverState !=
        ...
        if( ssl->options.side == WOLFSSL_CLIENT_END )
        {
            if( type == server_hello )
            {
                ...
                if( ( ret = SetKeysSide( ssl, DECRYPT_SIDE_ONLY ) ) != 0 )
                    return ret;
            #else
                if( ( ret = SetKeysSide( ssl, ENCRYPT_AND_DECRYPT_SIDE ) ) !=
                    0 )
                    return ret;
            #endif
            }
                         ...
                         word32 totalSz )
{
    ...
    if( ssl->arrays == NULL )
    {
        if( GetHandshakeHeader( ssl, input, inOutIdx, &type, &size, totalSz ) !=
            0 )
        {
            SendAlert( ssl, alert_fatal, unexpected_message );
            return PARSE_ERROR;
    ...
    if( ssl->arrays->pendingMsgSz == 0 )
    {
        ...
        if( inputLength - HANDSHAKE_HEADER_SZ < size )
        {
            ...
            ssl->arrays->pendingMsg = ( byte * )
                XMALLOC( size + HANDSHAKE_HEADER_SZ,
                         ssl->heap,
                         DYNAMIC_TYPE_ARRAYS );
                             ...
                             unsigned int secretSz )
{
    ...
    if( secretSz != ssl->buffers.tls13CookieSecret.length )
    {
        ...
        newSecret = ( byte * ) XMALLOC( secretSz,
                                        ssl->heap,
                                        DYNAMIC_TYPE_COOKIE_PWD );
        if( newSecret == NULL )
        {
...
int wolfSSL_accept_TLSv13( WOLFSSL * ssl )
{
    ...
    switch( ssl->options.acceptState )
    {
        ...
        case TLS13_ACCEPT_SECOND_REPLY_DONE:
            if( ( ssl->error = SendTls13ServerHello( ssl, server_hello ) ) !=
                0 )
            {
                WOLFSSL_ERROR( ssl->error );
                return WOLFSSL_FATAL_ERROR;
