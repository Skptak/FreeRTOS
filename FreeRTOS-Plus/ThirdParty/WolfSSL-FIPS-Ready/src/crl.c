                          ...
                          void * heap )
{
    ...
    if( !verified )
    {
        ...
        crle->toBeSigned = ( byte * ) XMALLOC( crle->tbsSz,
                                               heap,
                                               DYNAMIC_TYPE_CRL_ENTRY );
        if( crle->toBeSigned == NULL )
            return -1;
        crle->signature = ( byte * ) XMALLOC( crle->signatureSz,
                                              heap,
                                              DYNAMIC_TYPE_CRL_ENTRY );
                             ...
                             int * pFoundEntry )
{
    ...
    while( crle )
    {
        if( XMEMCMP( crle->issuerHash, cert->issuerHash, CRL_DIGEST_SIZE ) ==
            0 )
        {
            ...
            if( crle->verified == 0 )
            {
                ...
                ret = VerifyCRL_Signature( &sigCtx,
                                           tbs,
                                           tbsSz,
                                           sig,
                                           sigSz,
                ...
                while( crle )
                {
                                 ...
                                 CRL_DIGEST_SIZE ) == 0 )
                    {
                        ...
                        XFREE( crle->toBeSigned,
                               crl->heap,
                               DYNAMIC_TYPE_CRL_ENTRY );
                        crle->toBeSigned = NULL;
                        XFREE( crle->signature,
                               crl->heap,
                               DYNAMIC_TYPE_CRL_ENTRY );
                        crle->signature = NULL;
                        break;
            ...
            {
        #ifndef NO_ASN_TIME
                if( !XVALIDATE_DATE( crle->nextDate,
                                     crle->nextDateFormat,
                                     AFTER ) )
                {
                   ...
                   int verified )
{
    ...
    crle = ( CRL_Entry * ) XMALLOC( sizeof( CRL_Entry ),
                                    crl->heap,
                                    DYNAMIC_TYPE_CRL_ENTRY );
    if( crle == NULL )
    {
                   ...
                   int verify )
{
    ...
    if( type == WOLFSSL_FILETYPE_PEM )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    dcrl = ( DecodedCRL * ) XMALLOC( sizeof( DecodedCRL ),
                                     NULL,
...
static RevokedCert * DupRevokedCertList( RevokedCert * in, void * heap )
{
    ...
    while( current )
    {
        RevokedCert * tmp = ( RevokedCert * ) XMALLOC( sizeof( RevokedCert ),
                                                       heap,
                                                       DYNAMIC_TYPE_REVOKED );
        if( tmp != NULL )
        {
            XMEMCPY( tmp->serialNumber,
...
static CRL_Entry * DupCRL_Entry( const CRL_Entry * ent, void * heap )
{
    ...
    dup = ( CRL_Entry * ) XMALLOC( sizeof( CRL_Entry ),
                                   heap,
                                   DYNAMIC_TYPE_CRL_ENTRY );
    if( dup == NULL )
    {
    ...
    if( !ent->verified )
    {
        ...
        dup->toBeSigned = ( byte * ) XMALLOC( dup->tbsSz,
                                              heap,
                                              DYNAMIC_TYPE_CRL_ENTRY );
        ...
        dup->signature = ( byte * ) XMALLOC( dup->signatureSz,
                                             heap,
                                             DYNAMIC_TYPE_CRL_ENTRY );
...
static int DupX509_CRL( WOLFSSL_X509_CRL * dup, const WOLFSSL_X509_CRL * crl )
{
    ...
    if( crl->monitors[ 0 ].path )
    {
        ...
        dup->monitors[ 0 ].path = ( char * )
            XMALLOC( pathSz, dup->heap, DYNAMIC_TYPE_CRL_MONITOR );
        if( dup->monitors[ 0 ].path != NULL )
        {
            XSTRNCPY( dup->monitors[ 0 ].path,
                      crl->monitors[ 0 ].path,
                      pathSz );
        }
        else
    ...
    if( crl->monitors[ 1 ].path )
    {
        ...
        dup->monitors[ 1 ].path = ( char * )
            XMALLOC( pathSz, dup->heap, DYNAMIC_TYPE_CRL_MONITOR );
        if( dup->monitors[ 1 ].path != NULL )
        {
            XSTRNCPY( dup->monitors[ 1 ].path,
                      crl->monitors[ 1 ].path,
                      pathSz );
        }
        else
...
static int SwapLists( WOLFSSL_CRL * crl )
{
    ...
    WOLFSSL_CRL tmp[ 1 ];
            ...
            #ifdef WOLFSSL_SMALL_STACK
    tmp = ( WOLFSSL_CRL * ) XMALLOC( sizeof( WOLFSSL_CRL ),
                                     NULL,
                                     DYNAMIC_TYPE_TMP_BUFFER );
    if( tmp == NULL )
        return MEMORY_E;
    ...
    return 0;
                ...
                /* we need a unique kqueue user filter fd for crl in case user
                 * is doing custom events too */
...
static void * DoMonitor( void * arg )
{
    ...
    for( ;; )
    {
        ...
        result = select( max( notifyFd, crl->mfd ) + 1,
                         &readfds,
                         NULL,
                         NULL,
                         NULL );
...
int LoadCRL( WOLFSSL_CRL * crl, const char * path, int type, int monitor )
{
    ...
    if( crl == NULL )
        return BAD_FUNC_ARG;
            ...
            #ifdef WOLFSSL_SMALL_STACK
    readCtx = ( ReadDirCtx * ) XMALLOC( sizeof( ReadDirCtx ),
                                        crl->heap,
                                        DYNAMIC_TYPE_TMP_BUFFER );
    if( readCtx == NULL )
        return MEMORY_E;
    ...
    while( ret == 0 && name )
    {
        ...
        if( !skip &&
            ProcessFile( NULL, name, type, CRL_TYPE, NULL, 0, crl, VERIFY ) !=
                WOLFSSL_SUCCESS )
        {
    ...
    if( monitor & WOLFSSL_CRL_MONITOR )
    {
        ...
        pathBuf = ( char * ) XMALLOC( pathLen + 1,
                                      crl->heap,
                                      DYNAMIC_TYPE_CRL_MONITOR );
        if( pathBuf )
        {
