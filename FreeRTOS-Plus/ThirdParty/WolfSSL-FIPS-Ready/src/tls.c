...
static int TLSX_PopulateSupportedGroups( WOLFSSL * ssl, TLSX ** extensions );
        ...
        /* Digest enable checks */
                ...
                !defined( HAVE_FFDHE_3072 ) && !defined( HAVE_FFDHE_4096 ) && \
                !defined( HAVE_FFDHE_6144 ) && !defined( HAVE_FFDHE_8192 )
...
int BuildTlsHandshakeHash( WOLFSSL * ssl, byte * hash, word32 * hashLen )
{
    ...
    ret |= wc_ShaGetHash( &ssl->hsHashes->hashSha,
...
static const byte ext_master_label[ EXT_MASTER_LABEL_SZ + 1 ] = "extended "
                                                                "master secret";
            #endif
...
{
    int ret;
    int key_dig_len = 2 * ssl->specs.hash_size + 2 * ssl->specs.key_size +
                      2 * ssl->specs.iv_size;
...
int wolfSSL_MakeTlsMasterSecret( byte * ms,
                                 ...
                                 int hash_type )
{
    return _MakeTlsMasterSecret( ms,
                                 msLen,
                                 pms,
                                 pmsLen,
                                 cr,
...
int wolfSSL_MakeTlsExtendedMasterSecret( byte * ms,
                                         ...
                                         int hash_type )
{
    return _MakeTlsExtendedMasterSecret( ms,
                                         msLen,
                                         pms,
                                         pmsLen,
                                         sHash,
...
int MakeTlsMasterSecret( WOLFSSL * ssl )
{
    ...
    if( ssl->options.haveEMS )
    {
        word32 hashSz = HSHASH_SZ;
                #ifdef WOLFSSL_SMALL_STACK
        byte * handshake_hash = ( byte * ) XMALLOC( HSHASH_SZ,
                                                    ssl->heap,
                                                    DYNAMIC_TYPE_DIGEST );
        if( handshake_hash == NULL )
            return MEMORY_E;
        ...
        if( ret == 0 )
        {
            ret = _MakeTlsExtendedMasterSecret( ssl->arrays->masterSecret,
                                                SECRET_LEN,
    ...
    if( ret == 0 )
    {
        ...
        XSNPRINTF( &pmsBuf[ pmsPos ],
                   sizeof( pmsBuf ) - pmsPos,
                   "%s ",
                   CLIENT_RANDOM_LABEL );
                           ...
                           const char * label )
{
    ...
    XMEMCPY( seed + RAN_LEN, ssl->arrays->serverRandom, RAN_LEN );
            ...
            #ifdef WOLFSSL_HAVE_PRF
    ret = wc_PRF_TLS( ( byte * ) msk,
                      len,
                      ssl->arrays->masterSecret,
                      SECRET_LEN,
                      ( const byte * ) label,
                      ( word32 ) XSTRLEN( label ),
                      seed,
                      SEED_LEN,
                      IsAtLeastTLSv1_2( ssl ),
                      ssl->specs.mac_algorithm,
                                ...
                                byte * header )
{
    ...
    for( i = safeBlocks; i < blocks; i++ )
    {
        ...
        if( ret != 0 )
            return ret;
        for( j = 0; j < macLen; j++ )
            ( ( unsigned char * ) hmac->innerHash )[ j ] |= hashBlock[ j ] &
                                                            isOutBlock;
    }
              ...
              int epochOrder )
{
    ...
    return ret;
            ...
            /**
             * The TLSX semaphore is used to calculate the size of the
             * extensions to be sent from one peer to another.
             */
...
static WC_INLINE word16 TLSX_ToSemaphore( word16 type )
{
    switch( type )
    {
        ...
        default:
            if( type > 62 )
            {
                ...
                WOLFSSL_MSG(
                    "### TLSX semaphore collision or overflow detected!" );
            }
    }
    ...
    return type;
            ...
            /** Checks if a specific light (tls extension) is not set in the
             * semaphore. */
                                  ...
                                  byte isRequest )
{
    ...
    if( ssl->alpnSelect != NULL )
    {
                             ...
                             ssl->alpnSelectArg ) == 0 )
        {
            ...
            if( TLSX_UseALPN( &ssl->extensions,
                              ( char * ) out,
                              outLen,
                              0,
    ...
    {
        return isRequest ? 0 : TLSX_HandleUnsupportedExtension( ssl );
    }
    ...
    if( isRequest )
    {
        ...
        ssl->alpn_client_list = ( char * ) XMALLOC( size,
                                                    ssl->heap,
                                                    DYNAMIC_TYPE_ALPN );
        if( ssl->alpn_client_list == NULL )
            return MEMORY_ERROR;
    ...
    for( size = 0; offset < length; offset += size )
    {
        ...
        if( isRequest )
        {
            XMEMCPY( ssl->alpn_client_list + idx,
                     ( char * ) input + offset,
                     size );
            idx += size;
    ...
    if( sni )
    {
        ...
        switch( sni->type )
        {
            case WOLFSSL_SNI_HOST_NAME:
                sni->data.host_name = ( char * ) XMALLOC( size + 1,
                                                          heap,
                                                          DYNAMIC_TYPE_TLSX );
                if( sni->data.host_name )
                {
                    XSTRNCPY( sni->data.host_name,
                              ( const char * ) data,
                              size );
...
static void TLSX_SNI_SetStatus( TLSX * extensions, byte type, byte status )
{
    ...
    SNI * sni = TLSX_SNI_Find( extension ? ( SNI * ) extension->data : NULL,
                               type );
                           ...
                           byte isRequest )
{
    ...
    if( !isRequest )
    {
         ...
         * name was accepted.
         */
        TLSX_SNI_SetStatus( ssl->extensions,
    ...
    if( !extension || !extension->data )
    {
                    #if defined( WOLFSSL_ALWAYS_KEEP_SNI ) && \
        ...
        switch( type )
        {
            ...
            case WOLFSSL_TRUSTED_CA_CERT_SHA1:
                if( idSz == WC_SHA_DIGEST_SIZE &&
                    ( tca->id = ( byte * )
                          XMALLOC( idSz, heap, DYNAMIC_TYPE_TLSX ) ) )
                {
    ...
    if( isRequest )
    {
        ...
        switch( csr->status_type )
        {
            case WOLFSSL_CSR_OCSP:
                ...
                if( csr->request.ocsp.nonceSz )
                    length = ( word16 )
                        EncodeOcspRequestExtensions( &csr->request.ocsp,
                                                     output + offset +
                                                         OPAQUE16_LEN,
                                                     OCSP_NONCE_EXT_SZ );
                           ...
                           byte isRequest )
{
    ...
    if( !isRequest )
    {
        ...
        if( !csr )
        {
            ...
            csr = extension ? ( CertificateStatusRequest * ) extension->data
                            : NULL;
        ...
        switch( status_type )
        {
            ...
            default:
                return 0;
                    ...
                    /* if using status_request and already sending it, skip this
                     * one */
...
int TLSX_CSR_InitRequest( TLSX * extensions, DecodedCert * cert, void * heap )
{
    ...
    CertificateStatusRequest * csr = extension ? ( CertificateStatusRequest * )
                                                     extension->data
                                               : NULL;
    int ret = 0;
    ...
    if( csr )
    {
        switch( csr->status_type )
        {
            case WOLFSSL_CSR_OCSP:
            {
                ...
                if( ( ret = InitOcspRequest( &csr->request.ocsp,
                                             cert,
                                             0,
                                             heap ) ) != 0 )
                    return ret;
                    ...
                    case WOLFSSL_CSR2_OCSP_MULTI:
                        ...
                        if( csr2->request.ocsp[ 0 ].nonceSz )
                        {
                            ...
                            if( request )
                            {
                                ...
                                request->nonceSz = csr2->request.ocsp[ 0 ]
                                                       .nonceSz;
                            }
                        }
                        break;
                            ...
                            void * heap )
{
    ...
    CertificateStatusRequestItemV2 *
        csr2 = extension ? ( CertificateStatusRequestItemV2 * ) extension->data
                         : NULL;
    int ret = 0;
        ...
        switch( csr2->status_type )
        {
            ...
            case WOLFSSL_CSR2_OCSP_MULTI:
            {
                if( csr2->requests < 1 + MAX_CHAIN_DEPTH )
                {
                    ...
                    if( ( ret = InitOcspRequest( &csr2->request
                                                      .ocsp[ csr2->requests ],
                                                 cert,
                                                 0,
                                                 heap ) ) != 0 )
                                        ...
                                        int devId )
{
    ...
    csr2 = ( CertificateStatusRequestItemV2 * )
        XMALLOC( sizeof( CertificateStatusRequestItemV2 ),
                 heap,
                 DYNAMIC_TYPE_TLSX );
    if( !csr2 )
        return MEMORY_E;
    ...
    {
        CertificateStatusRequestItemV2 *
            last = ( CertificateStatusRequestItemV2 * ) extension->data;
    ...
    else if( ( ret = TLSX_Push( extensions,
                                TLSX_STATUS_REQUEST_V2,
                                csr2,
                                heap ) ) )
    {
    ...
    return WOLFSSL_SUCCESS;
        ...
        /******************************************************************************/
                    ...
                    !defined( HAVE_CURVE448 ) && !defined( HAVE_FFDHE )
                                    ...
                                    void * heap )
{
    ...
    *curve = ( SupportedCurve * ) XMALLOC( sizeof( SupportedCurve ),
                                           heap,
                                           DYNAMIC_TYPE_TLSX );
    if( *curve == NULL )
        return MEMORY_E;
                                 ...
                                 void * heap )
{
    ...
    *point = ( PointFormat * ) XMALLOC( sizeof( PointFormat ),
                                        heap,
                                        DYNAMIC_TYPE_TLSX );
    if( *point == NULL )
        return MEMORY_E;
...
{
                    #if defined( HAVE_FFDHE ) || defined( HAVE_ECC ) || \
                        defined( HAVE_CURVE25519 ) || defined( HAVE_CURVE448 )
    ( void ) semaphore;
                    #endif
...
int TLSX_SupportedCurve_CheckPriority( WOLFSSL * ssl )
{
    ...
    if( ( ret = TLSX_PopulateSupportedGroups( ssl, &priority ) ) !=
        WOLFSSL_SUCCESS )
        return ret;
...
int TLSX_ValidateSupportedCurves( WOLFSSL * ssl, byte first, byte second )
{
    ...
    for( curve = ( SupportedCurve * ) extension->data; curve && !( sig && key );
         curve = curve->next )
    {
                        #ifdef OPENSSL_EXTRA
...
int TLSX_UsePointFormat( TLSX ** extensions, byte format, void * heap )
{
    ...
    {
        ret = TLSX_PointFormat_Append( ( PointFormat * ) extension->data,
                                       format,
                                       heap );
        if( ret != 0 )
            return ret;
                                              ...
                                              int isRequest )
{
    ...
    if( data && data->enabled && data->verifySet )
    {
        ...
        if( !isRequest )
        {
            XMEMCPY( output + offset,
                     data->server_verify_data,
                     TLS_FINISHED_SZ );
            offset += TLS_FINISHED_SZ;
        }
    }
...
int TLSX_UseSecureRenegotiation( TLSX ** extensions, void * heap )
{
    ...
    data = ( SecureRenegotiation * ) XMALLOC( sizeof( SecureRenegotiation ),
                                              heap,
                                              DYNAMIC_TYPE_TLSX );
    if( data == NULL )
        return MEMORY_E;
...
static void TLSX_SessionTicket_ValidateRequest( WOLFSSL * ssl )
{
    ...
    SessionTicket * ticket = extension ? ( SessionTicket * ) extension->data
                                       : NULL;
                                     ...
                                     byte isRequest )
{
    ...
    else
    {
        ...
        if( length == 0 )
        {
            ...
            {
                ret = 0;
                TLSX_SetResponse( ssl, TLSX_SESSION_TICKET ); /* send blank
                                                                 ticket */
        ...
        else
        {
            ...
            else if( ret == WOLFSSL_TICKET_RET_CREATE )
            {
                ...
                ret = TLSX_UseSessionTicket( &ssl->extensions,
                                             NULL,
                                             ssl->heap );
                if( ret == WOLFSSL_SUCCESS )
                {
                                                         ...
                                                         void * heap )
{
    SessionTicket * ticket = ( SessionTicket * )
        XMALLOC( sizeof( SessionTicket ), heap, DYNAMIC_TYPE_TLSX );
    if( ticket )
    {
                           ...
                           void * heap )
{
    ...
    return WOLFSSL_SUCCESS;
                ...
                #define WOLF_STK_VALIDATE_REQUEST \
                ...
                #define WOLF_STK_FREE( stk, heap ) \
                    TLSX_SessionTicket_Free( ( SessionTicket * ) stk, ( heap ) )
                            ...
                            word16 pubLen )
{
    ...
    if( ( temp = ( QSHScheme * ) XMALLOC( sizeof( QSHScheme ),
                                          NULL,
                           ...
                           byte isRequest )
{
    ...
    while( ( offset_len < offset_pk ) && numKeys )
    {
        ...
        if( ( temp = ( QSHKey * ) XMALLOC( sizeof( QSHKey ),
                                           ssl->heap,
                                           DYNAMIC_TYPE_TLSX ) ) == NULL )
            return MEMORY_E;
        ...
        r = TLSX_UseQSHScheme( &ssl->extensions,
                               temp->name,
                               PK,
                               PKLen,
                               ssl->heap );
                          ...
                          byte isServer )
{
    ...
    buf->length = offset;
    buf->buffer = ( byte * ) XMALLOC( offset,
                                      ssl->heap,
...
int TLSX_ValidateQSHScheme( TLSX ** extensions, word16 theirs )
{
    ...
    for( format = ( QSHScheme * ) extension->data; format;
         format = format->next )
    {
...
static int TLSX_EncryptThenMac_Use( WOLFSSL * ssl )
{
    ...
    if( extension == NULL )
    {
        ...
        ret = TLSX_Push( &ssl->extensions,
                         TLSX_ENCRYPT_THEN_MAC,
                         NULL,
                         ssl->heap );
        if( ret != 0 )
            return ret;
                     ...
                     int resp )
{
    ...
    cookie = ( Cookie * ) XMALLOC( sizeof( Cookie ) + len + macSz - 1,
                                   ssl->heap,
                                   DYNAMIC_TYPE_TLSX );
    if( cookie == NULL )
        return MEMORY_E;
...
static word16 TLSX_SignatureAlgorithms_Write( void * data, byte * output )
{
    ...
    XMEMCPY( output + OPAQUE16_LEN,
             ssl->suites->hashSigAlgo,
             ssl->suites->hashSigAlgoSz );

    TLSX_SignatureAlgorithms_MapPss( ssl,
                                     output + OPAQUE16_LEN,
                                     ssl->suites->hashSigAlgoSz );
...
static int TLSX_KeyShare_GenDhKey( WOLFSSL * ssl, KeyShareEntry * kse )
{
    ...
    if( ret != 0 )
        goto end;
                    ...
                    #if defined( WOLFSSL_STATIC_EPHEMERAL ) && \
                    ...
                    #endif
    {
        ...
        ret = wc_DhGenerateKeyPair( dhKey,
                                    ssl->rng,
                                    ( byte * ) key,
                                    &keySz,
                                    keyData,
...
static int TLSX_KeyShare_GenX25519Key( WOLFSSL * ssl, KeyShareEntry * kse )
{
    ...
    /* Allocate space for the public key. */
    keyData = ( byte * ) XMALLOC( CURVE25519_KEYSIZE,
                                  ssl->heap,
                                  DYNAMIC_TYPE_PUBLIC_KEY );
    if( keyData == NULL )
    {
...
static int TLSX_KeyShare_GenX448Key( WOLFSSL * ssl, KeyShareEntry * kse )
{
    ...
    /* Allocate an ECC key to hold private key. */
    key = ( curve448_key * ) XMALLOC( sizeof( curve448_key ),
                                      ssl->heap,
                                      DYNAMIC_TYPE_PRIVATE_KEY );
    if( key == NULL )
    {
    ...
    /* Allocate space for the public key. */
    keyData = ( byte * ) XMALLOC( CURVE448_KEY_SIZE,
                                  ssl->heap,
                                  DYNAMIC_TYPE_PUBLIC_KEY );
    if( keyData == NULL )
    {
    ...
    /* Export public key. */
    if( wc_curve448_export_public_ex( key,
                                      keyData,
                                      &dataSize,
...
static int TLSX_KeyShare_GenEccKey( WOLFSSL * ssl, KeyShareEntry * kse )
{
    ...
    /* Allocate an ECC key to hold private key. */
    keyPtr = ( byte * ) XMALLOC( sizeof( ecc_key ),
                                 ssl->heap,
                                 DYNAMIC_TYPE_PRIVATE_KEY );
    if( keyPtr == NULL )
    {
    ...
    if( ssl->staticKE.key && ssl->staticKE.keyAlgo == WC_PK_TYPE_ECDH )
    {
        ...
        ret = wc_EccPrivateKeyDecode( keyDer->buffer,
                                      &idx,
                                      eccKey,
                                      keyDer->length );
    }
                                    ...
                                    KeyShareEntry * keyShareEntry )
{
    ...
    ret = wc_DhAgree( dhKey,
                      ssl->arrays->preMasterSecret,
                      &ssl->arrays->preMasterSz,
                      ( const byte * ) keyShareEntry->key,
                      keyShareEntry->keyLen,
                      keyShareEntry->ke,
                      keyShareEntry->keLen );
                    #ifdef WOLFSSL_ASYNC_CRYPT
    /* TODO: Make this function non-blocking */
                                        ...
                                        KeyShareEntry * keyShareEntry )
{
    ...
    peerX25519Key = ( curve25519_key * ) XMALLOC( sizeof( curve25519_key ),
                                                  ssl->heap,
                                                  DYNAMIC_TYPE_TLSX );
    ...
    if( wc_curve25519_check_public( keyShareEntry->ke,
                                    keyShareEntry->keLen,
                                    EC25519_LITTLE_ENDIAN ) != 0 )
    {
                                      ...
                                      KeyShareEntry * keyShareEntry )
{
    ...
    peerX448Key = ( curve448_key * ) XMALLOC( sizeof( curve448_key ),
                                              ssl->heap,
                                              DYNAMIC_TYPE_TLSX );
    if( peerX448Key == NULL )
    {
    ...
    if( wc_curve448_check_public( keyShareEntry->ke,
                                  keyShareEntry->keLen,
                                  EC448_LITTLE_ENDIAN ) != 0 )
    {
        ret = ECC_PEERKEY_ERROR;
    ...
    if( ret == 0 )
    {
        ...
        ret = wc_curve448_shared_secret_ex( key,
                                            peerX448Key,
                                            ssl->arrays->preMasterSecret,
                                            &ssl->arrays->preMasterSz,
                                            EC448_LITTLE_ENDIAN );
                                     ...
                                     KeyShareEntry * keyShareEntry )
{
    ...
    if( wc_ecc_import_x963_ex( keyShareEntry->ke,
                               keyShareEntry->keLen,
                               ssl->peerEccKey,
                               curveId ) != 0 )
    {
    ...
    do
    {
        ...
        ret = wc_AsyncWait( ret,
                            &keyShareKey->asyncDev,
                            WC_ASYNC_FLAG_CALL_AGAIN );
                    #endif
        if( ret >= 0 )
            ret = wc_ecc_shared_secret( keyShareKey,
                                        ssl->peerEccKey,
                                        ssl->arrays->preMasterSecret,
                                        &ssl->arrays->preMasterSz );
...
static int TLSX_SupportedGroups_Find( WOLFSSL * ssl, word16 name )
{
    ...
    if( ( extension = TLSX_Find( ssl->extensions, TLSX_SUPPORTED_GROUPS ) ) ==
        NULL )
    {
    ...
    for( curve = ( SupportedCurve * ) extension->data; curve;
         curve = curve->next )
    {
        if( curve->name == name )
            return 1;
                                ...
                                byte msgType )
{
    ...
    if( msgType == client_hello )
    {
        ...
        {
            ret = TLSX_KeyShareEntry_Parse( ssl,
                                            &input[ offset ],
                                            length - offset,
                                            &keyShareEntry );
            if( ret < 0 )
                              ...
                              KeyShareEntry ** keyShareEntry )
{
    ...
    kse = ( KeyShareEntry * ) XMALLOC( sizeof( KeyShareEntry ),
                                       heap,
                                       DYNAMIC_TYPE_TLSX );
    if( kse == NULL )
        return MEMORY_E;
                       ...
                       KeyShareEntry ** kse )
{
    ...
    {
        ret = TLSX_KeyShare_New( ( KeyShareEntry ** ) &extension->data,
                                 group,
                                 ssl->heap,
                                 &keyShareEntry );
        if( ret != 0 )
...
int TLSX_KeyShare_Establish( WOLFSSL * ssl )
{
    ...
    for( clientKSE = list; clientKSE != NULL; clientKSE = clientKSE->next )
    {
        ...
        if( ( clientKSE->group & NAMED_DH_MASK ) == 0 )
        {
            ...
            if( ssl->ctx->disabledCurves &
                ( ( word32 ) 1 << clientKSE->group ) )
                continue;
                #endif
        }
                                  ...
                                  PreSharedKey ** preSharedKey )
{
    ...
    psk = ( PreSharedKey * ) XMALLOC( sizeof( PreSharedKey ),
                                      heap,
                                      DYNAMIC_TYPE_TLSX );
    if( psk == NULL )
        return MEMORY_E;
                           ...
                           PreSharedKey ** preSharedKey )
{
    ...
    if( psk == NULL )
    {
        ret = TLSX_PreSharedKey_New( ( PreSharedKey ** ) &extension->data,
                                     identity,
                                     len,
                                     ssl->heap,
                                     &psk );
    ...
    while( ( extension = list ) )
    {
        ...
        switch( extension->type )
        {
            ...
            case TLSX_STATUS_REQUEST:
                CSR_FREE_ALL( ( CertificateStatusRequest * ) extension->data,
                              heap );
                break;

            case TLSX_STATUS_REQUEST_V2:
                CSR2_FREE_ALL( ( CertificateStatusRequestItemV2 * )
                                   extension->data,
                               heap );
                break;
                         ...
                         word16 * pLength )
{
    ...
    while( ( extension = list ) )
    {
        ...
        switch( extension->type )
        {
            ...
            case TLSX_STATUS_REQUEST:
                length += CSR_GET_SIZE( ( CertificateStatusRequest * )
                                            extension->data,
                                        isRequest );
                break;

            case TLSX_STATUS_REQUEST_V2:
                length += CSR2_GET_SIZE( ( CertificateStatusRequestItemV2 * )
                                             extension->data,
                                         isRequest );
                break;

            case TLSX_RENEGOTIATION_INFO:
                length += SCR_GET_SIZE( ( SecureRenegotiation * )
                                            extension->data,
                                        isRequest );
                break;
            ...
            case TLSX_QUANTUM_SAFE_HYBRID:
                length += QSH_GET_SIZE( ( QSHScheme * ) extension->data,
                                        isRequest );
                break;
            ...
            case TLSX_COOKIE:
                ret = CKE_GET_SIZE( ( Cookie * ) extension->data,
                                    msgType,
                                    &length );
                break;
                ...
                #if defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK )
            case TLSX_PRE_SHARED_KEY:
                ret = PSK_GET_SIZE( ( PreSharedKey * ) extension->data,
                                    msgType,
                                    &length );
                break;
            ...
            case TLSX_KEY_SHARE:
                length += KS_GET_SIZE( ( KeyShareEntry * ) extension->data,
                                       msgType );
                break;
            #endif
        }
    ...
    while( ( extension = list ) )
    {
        ...
        switch( extension->type )
        {
            ...
            case TLSX_TRUSTED_CA_KEYS:
                ...
                if( isRequest )
                {
                    offset += TCA_WRITE( ( TCA * ) extension->data,
                                         output + offset );
                }
                break;
            ...
            case TLSX_STATUS_REQUEST_V2:
                WOLFSSL_MSG(
                    "Certificate Status Request v2 extension to write" );
                offset += CSR2_WRITE( ( CertificateStatusRequestItemV2 * )
                                          extension->data,
                                      output + offset,
                                      isRequest );
                break;
            ...
            case TLSX_QUANTUM_SAFE_HYBRID:
                ...
                if( isRequest )
                {
                    offset += QSH_WRITE( ( QSHScheme * ) extension->data,
                                         output + offset );
                }
                offset += QSHPK_WRITE( ( QSHScheme * ) extension->data,
                                       output + offset );
            ...
            case TLSX_APPLICATION_LAYER_PROTOCOL:
                ...
                offset += ALPN_WRITE( ( ALPN * ) extension->data,
                                      output + offset );
                break;
            #if !defined( WOLFSSL_NO_SIGALG )
            case TLSX_SIGNATURE_ALGORITHMS:
            ...
            case TLSX_COOKIE:
                ...
                ret = CKE_WRITE( ( Cookie * ) extension->data,
                                 output + offset,
                                 msgType,
                                 &offset );
                break;
            ...
            case TLSX_POST_HANDSHAKE_AUTH:
                WOLFSSL_MSG(
                    "Post-Handshake Authentication extension to write" );
...
static word32 GetEntropy( unsigned char * out, word32 num_bytes )
{
    ...
    if( gRng == NULL )
    {
        if( ( gRng = ( WC_RNG * ) XMALLOC( sizeof( WC_RNG ),
                                           NULL,
                                           DYNAMIC_TYPE_TLSX ) ) == NULL )
    ...
    if( gRngMutex == NULL )
    {
        if( ( gRngMutex = ( wolfSSL_Mutex * ) XMALLOC( sizeof( wolfSSL_Mutex ),
                                                       NULL,
                                                       DYNAMIC_TYPE_TLSX ) ) ==
            NULL )
            return DRBG_OUT_OF_MEMORY;
...
int TLSX_CreateNtruKey( WOLFSSL * ssl, int type )
{
    ...
    if( ( ret = ntru_crypto_ntru_encrypt_keygen( drbg,
                                                 ntruType,
                                                 &public_key_len,
                                                 NULL,
                                                 &private_key_len,
    ...
    temp->pub.buffer = ( byte * ) XMALLOC( public_key_len,
                                           ssl->heap,
                                           DYNAMIC_TYPE_PUBLIC_KEY );
    XMEMCPY( temp->pub.buffer, public_key, public_key_len );
    temp->pri.length = private_key_len;
    temp->pri.buffer = ( byte * ) XMALLOC( private_key_len,
                                           ssl->heap,
                                           DYNAMIC_TYPE_ARRAYS );
...
static int TLSX_PopulateSupportedGroups( WOLFSSL * ssl, TLSX ** extensions )
{
    ...
    {
        return TLSX_UseSupportedCurve( extensions,
                                       ssl->session.namedGroup,
                                       ssl->heap );
    }
                    #endif
    ...
    if( ssl->numGroups != 0 )
    {
        ...
        return WOLFSSL_SUCCESS;
                ...
                #if defined( HAVE_ECC ) && defined( HAVE_SUPPORTED_CURVES )
                    /* list in order by strength, since not all servers choose
                     * by strength */
                    #if defined( HAVE_ECC521 ) || defined( HAVE_ALL_CURVES )
                        #ifndef NO_ECC_SECP
    ret = TLSX_UseSupportedCurve( extensions,
...
int TLSX_PopulateExtensions( WOLFSSL * ssl, byte isServer )
{
    ...
    if( !isServer )
    {
        ...
        if( !ssl->user_set_QSHSchemes )
        {
            ...
            {
                if( ( ret = TLSX_CreateQSHKey( ssl, WOLFSSL_NTRU_EESS743 ) ) !=
                    0 )
                {
                    WOLFSSL_MSG( "Error creating ntru keys" );
                    return ret;
                }
                if( ( ret = TLSX_CreateQSHKey( ssl, WOLFSSL_NTRU_EESS593 ) ) !=
                    0 )
                {
                    WOLFSSL_MSG( "Error creating ntru keys" );
                    return ret;
                }
                if( ( ret = TLSX_CreateQSHKey( ssl, WOLFSSL_NTRU_EESS439 ) ) !=
                    0 )
                {
                    WOLFSSL_MSG( "Error creating ntru keys" );
                    return ret;
                ...
                public_key = TLSX_QSHKeyFind_Pub( ssl->QSH_Key,
                                                  &public_key_len,
                                                  WOLFSSL_NTRU_EESS743 );
            }
            if( TLSX_UseQSHScheme( &ssl->extensions,
                                   WOLFSSL_NTRU_EESS743,
                                   public_key,
                                   public_key_len,
                                   ssl->heap ) != WOLFSSL_SUCCESS )
        ...
        {
            if( TLSX_Find( ssl->ctx->extensions, TLSX_SUPPORTED_GROUPS ) ==
                NULL )
            {
            ...
            TLSX_Find( ssl->extensions, TLSX_EC_POINT_FORMATS ) == NULL )
        {
            ...
            #endif /* (HAVE_ECC || CURVE25519 || CURVE448) && \
                      HAVE_SUPPORTED_CURVES */
    }              /* is not server */

            #if !defined( WOLFSSL_NO_SIGALG )
    WOLFSSL_MSG( "Adding signature algorithms extension" );
    if( ( ret = TLSX_SetSignatureAlgorithms( &ssl->extensions,
                                             ssl,
                                             ssl->heap ) ) != 0 )
    {
        return ret;
    ...
    if( !isServer && IsAtLeastTLSv1_3( ssl->version ) )
    {
        ...
        if( ( ret = TLSX_SetSupportedVersions( &ssl->extensions,
                                               ssl,
                                               ssl->heap ) ) != 0 )
        {
            return ret;
        ...
        if( TLSX_Find( ssl->extensions, TLSX_KEY_SHARE ) == NULL )
        {
                ...
                #endif
            {
                ...
                namedGroup = WOLFSSL_ECC_X448;
                    ...
                    ( !defined( NO_ECC384 ) || defined( HAVE_ALL_CURVES ) ) && \
                    !defined( NO_ECC_SECP )
                namedGroup = WOLFSSL_ECC_SECP384R1;
                #elif defined( HAVE_ECC ) &&                                   \
                    ( !defined( NO_ECC521 ) || defined( HAVE_ALL_CURVES ) ) && \
                    !defined( NO_ECC_SECP )
            ...
            ssl->options.client_psk_tls13_cb != NULL )
        {
            ...
            if( ssl->options.client_psk_tls13_cb != NULL )
            {
                ssl->arrays->psk_keySz = ssl->options.client_psk_tls13_cb(
                    ...
                    MAX_PSK_KEY_LEN,
                    &cipherName );
                if( GetCipherSuiteFromName( cipherName,
                                            &cipherSuite0,
                                            &cipherSuite ) != 0 )
                {
                    return PSK_KEY_ERROR;
            ...
            {
                ssl->arrays->psk_keySz = ssl->options.client_psk_cb(
                    ssl,
                    ssl->arrays->server_hint,
                    ssl->arrays->client_identity,
                    MAX_PSK_ID_LEN,
        ...
        switch( msgType )
        {
            ...
            case server_hello:
                ...
                if( IsAtLeastTLSv1_3( ssl->version ) )
                {
                    ...
                    if( !ssl->options.noPskDheKe )
                        TURN_OFF( semaphore,
                                  TLSX_ToSemaphore( TLSX_KEY_SHARE ) );
                        #if defined( HAVE_SESSION_TICKET ) || !defined( NO_PSK )
                    TURN_OFF( semaphore,
                              TLSX_ToSemaphore( TLSX_PRE_SHARED_KEY ) );
                        #endif
                }
                else
                {
            ...
            case encrypted_extensions:
                ...
                TURN_ON( semaphore, TLSX_ToSemaphore( TLSX_STATUS_REQUEST ) );
                        ...
                        #ifdef HAVE_CERTIFICATE_STATUS_REQUEST_V2
                TURN_ON( semaphore,
                         TLSX_ToSemaphore( TLSX_STATUS_REQUEST_V2 ) );
                        #endif
                        #if defined( HAVE_SECURE_RENEGOTIATION )
                TURN_ON( semaphore,
                         TLSX_ToSemaphore( TLSX_RENEGOTIATION_INFO ) );
                        #endif
                break;
                ...
                Suites * suites )
{
    ...
    while( ret == 0 && offset < length )
    {
        ...
        switch( type )
        {
            ...
            #ifdef WOLFSSL_TLS13
                if( IsAtLeastTLSv1_3( ssl->version ) &&
                    msgType != client_hello && msgType != certificate_request &&
                    msgType != certificate )
                {
                    break;
                }
            ...
            case TLSX_STATUS_REQUEST_V2:
                WOLFSSL_MSG(
                    "Certificate Status Request v2 extension received" );
            #ifdef WOLFSSL_DEBUG_TLS
                WOLFSSL_BUFFER( input + offset, size );
            #endif
            ...
            case TLSX_POST_HANDSHAKE_AUTH:
                WOLFSSL_MSG(
                    "Post Handshake Authentication extension received" );
                    #ifdef WOLFSSL_DEBUG_TLS
                WOLFSSL_BUFFER( input + offset, size );
                    #endif
