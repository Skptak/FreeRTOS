...
static const char * client_showx509_msg[][ 5 ] = {
    /* English */
    { "issuer", "subject", "altname", "serial number", NULL },
    #ifndef NO_MULTIBYTE_PRINT
    /* Japanese */
    { "発行者", "サブジェクト", "代替名", "シリアル番号", NULL },
    #endif
};
                                  ...
                                  int lng_index )
{
    ...
    issuer = wolfSSL_X509_NAME_oneline( wolfSSL_X509_get_issuer_name( x509 ),
                                        0,
                                        0 );
                                     ...
                                     const char * hdr )
{
    ...
    for( i = 0; i < count; i++ )
    {
        wolfSSL_get_chain_cert_pem( chain,
                                    i,
                                    buffer,
                                    sizeof( buffer ),
                                    &length );
                                  ...
                                  int sctp )
{
    ...
    /* don't use INADDR_ANY by default, firewall may block, make user switch
       on */
    build_addr( &addr,
                ( useAnyAddr ? INADDR_ANY : wolfSSLIP ),
                *port,
                udp,
                sctp );
    tcp_socket( sockfd, udp, sctp );
                                                      ...
                                                      const char ** ciphersuite )
{
    int i;
    int b = 0x01;
    const char * userCipher = ( const char * ) wolfSSL_get_psk_callback_ctx(
        ssl );
                                       ...
                                       int type )
{
    ...
    {
        if( wolfSSL_use_certificate_buffer( ssl, buff, ( long ) sz, format ) !=
            WOLFSSL_SUCCESS )
            err_sys( "can't load buffer cert file" );
    }
    else if( type == WOLFSSL_KEY )
    {
        if( wolfSSL_use_PrivateKey_buffer( ssl, buff, ( long ) sz, format ) !=
            WOLFSSL_SUCCESS )
            err_sys( "can't load buffer key file" );
    }
    else if( type == WOLFSSL_CERT_CHAIN )
 ...
 * Normal cases of the verify callback only occur on certificate failures when
 * the wolfSSL_set_verify(ssl, SSL_VERIFY_PEER, myVerifyCb); is called
 */
...
static WC_INLINE int myVerify( int preverify, WOLFSSL_X509_STORE_CTX * store )
{
     ...
     * store->current_cert: Current WOLFSSL_X509 object (only with
     OPENSSL_EXTRA)
    ...
    if( peer )
    {
        char * issuer = wolfSSL_X509_NAME_oneline( wolfSSL_X509_get_issuer_name(
                                                       peer ),
                                                   0,
                                                   0 );
        char * subject = wolfSSL_X509_NAME_oneline(
            wolfSSL_X509_get_subject_name( peer ),
            0,
            0 );
...
#endif /* !defined(WOLFSSL_MDK_ARM) && !defined(WOLFSSL_KEIL_FS) && \
          !defined(WOLFSSL_TIRTOS) */
...
static WC_INLINE int StackSizeCheck( func_args * args, thread_func tf )
{
    ...
    ret = posix_memalign( ( void ** ) &myStack,
    ...
    /* good settings
       --enable-debug --disable-shared C_EXTRA_FLAGS="-DUSER_TIME
                                     ...
                                     void * ctx )
{
    ...
    ret = wc_HmacSetKey( &hmac,
                         ...
                         wolfSSL_GetMacSecret( ssl, macVerify ),
                         wolfSSL_GetHmacSize( ssl ) );
    if( ret != 0 )
        return ret;
                                        ...
                                        void * ctx )
{
    ...
    *padSz = wolfSSL_GetHmacSize( ssl ) + pad + padByte;
    ...
    if( XMEMCMP( verify,
                 decOut + decSz - digestSz - pad - padByte,
                 digestSz ) != 0 )
    {
...
static WC_INLINE void FreeAtomicUser( WOLFSSL * ssl )
{
    ...
    AtomicDecCtx * decCtx = ( AtomicDecCtx * ) wolfSSL_GetDecryptVerifyCtx(
        ssl );
                                        ...
                                        void * ctx )
{
    ...
    else if( side == WOLFSSL_SERVER_END )
    {
        ...
        ret = wc_ecc_import_x963_ex( pubKeyDer,
                                     *pubKeySz,
                                     pubKey,
                                     otherKey->dp->id );
    }
                                           ...
                                           void * ctx )
{
    ...
    else if( side == WOLFSSL_SERVER_END )
    {
        ...
        ret = wc_curve25519_import_public_ex( pubKeyDer,
                                              *pubKeySz,
                                              pubKey,
                                              EC25519_LITTLE_ENDIAN );
    }
    ...
    if( ret == 0 )
    {
        ret = wc_curve25519_shared_secret_ex( privKey,
                                              pubKey,
                                              out,
                                              outlen,
                                              EC25519_LITTLE_ENDIAN );
                                         ...
                                         void * ctx )
{
    ...
    if( side == WOLFSSL_CLIENT_END )
    {
        ...
        if( ret == 0 )
        {
            ...
            if( ret == 0 )
            {
                ret = wc_curve448_export_public_ex( privKey,
                                                    pubKeyDer,
                                                    pubKeySz,
                                                    EC448_LITTLE_ENDIAN );
            }
    ...
    else if( side == WOLFSSL_SERVER_END )
    {
        ...
        ret = wc_curve448_import_public_ex( pubKeyDer,
                                            *pubKeySz,
                                            pubKey,
                                            EC448_LITTLE_ENDIAN );
    }
    ...
    if( ret == 0 )
    {
        ret = wc_curve448_shared_secret_ex( privKey,
                                            pubKey,
                                            out,
                                            outlen,
                                            EC448_LITTLE_ENDIAN );
                                     ...
                                     void * ctx )
{
    ...
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            ret = wc_RsaPSS_VerifyInline( sig,
                                          sigSz,
                                          out,
                                          hashType,
                                          mgf,
                                        ...
                                        void * ctx )
{
    ...
    WOLFSSL_PKMSG( "PK RSA PSS SignCheck: sigSz %d, hash %d, mgf %d, keySz "
                   "%d\n",
                   sigSz,
                   hash,
                   mgf,
...
static WC_INLINE void SetupPkCallbackContexts( WOLFSSL * ssl, void * myCtx )
{
    ...
    defined( WOLFSSL_TIRTOS ) || defined( _MSC_VER )
                                    ...
                                    void * userCtx )
{
    ...
    if( enc )
    {
        ...
        ret = wc_ChaCha20Poly1305_Encrypt( myKey_ctx.key,
                                           iv,
                                           aad,
                                           aadSz,
                                           ticket,
    ...
    else
    {
        ...
        ret = wc_ChaCha20Poly1305_Decrypt( myKey_ctx.key,
                                           iv,
                                           aad,
                                           aadSz,
                                           ticket,
...
static WC_INLINE word16 GetRandomPort( void )
{
    ...
    if( wc_InitRng( &rng ) == 0 )
    {
        if( wc_RNG_GenerateBlock( &rng, ( byte * ) &port, sizeof( port ) ) ==
            0 )
        {
