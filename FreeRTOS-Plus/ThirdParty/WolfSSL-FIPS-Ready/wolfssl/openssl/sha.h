...
typedef struct WOLFSSL_SHA_CTX
{
    ...
    void * cryptocb_holder[ ( sizeof( int ) + sizeof( void * ) + 4 ) /
                            sizeof( void * ) ];
#endif
} WOLFSSL_SHA_CTX;
...
typedef WOLFSSL_SHA384_CTX SHA384_CTX;
    ...
    #if defined( NO_OLD_SHA_NAMES ) && !defined( HAVE_FIPS ) && \
        !defined( HAVE_SELFTEST )
        /* SHA384 is only available in non-fips mode because of SHA384 enum in
         * FIPS build. */
...
typedef WOLFSSL_SHA512_CTX SHA512_CTX;
    ...
    #if defined( NO_OLD_SHA_NAMES ) && !defined( HAVE_FIPS ) && \
        !defined( HAVE_SELFTEST )
        /* SHA512 is only available in non-fips mode because of SHA512 enum in
         * FIPS build. */
