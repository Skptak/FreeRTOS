...
int ebsnet_fseek( int a, long b, int c ); /* Not prototyped in vfile.h per
        ...
        #define XFILE int
        #define XFOPEN( NAME, MODE ) \
            vf_open( ( const char * ) NAME, VO_RDONLY, 0 );
        ...
        #define XFWRITE( BUF, SZ, AMT, F ) \
            fs_write( F, ( char * ) BUF, SZ * AMT )
        ...
        #warning \
            "TODO - DDC-I Certifiable Fast File System for Deos is not integrated"
        // #define XFILE      bfd *
...
int z_fs_close( XFILE file );
        ...
        #define XREWIND( F )         ( void ) F
        #define XFREAD( BUF, SZ, AMT, F ) \
            m2mb_fs_read( ( F ), ( BUF ), ( SZ ) * ( AMT ) )
        #define XFWRITE( BUF, SZ, AMT, F ) \
            m2mb_fs_write( ( F ), ( BUF ), ( SZ ) * ( AMT ) )
