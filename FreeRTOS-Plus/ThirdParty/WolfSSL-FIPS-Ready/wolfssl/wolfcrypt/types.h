...
#define XSTR_SIZEOF( x )                                 \
    ( sizeof( x ) - 1 ) /* -1 to not count the null char \
                         */
...
WOLFSSL_API void XFREE( void * p, void * heap, int type );
    ...
    defined( HAVE_INTEL_QA_SYNC )
        ...
        #ifdef WOLFSSL_DEBUG_MEMORY
            #define XMALLOC( s, h, t )               \
                wc_CryptoCb_IntelQaMalloc( ( s ),    \
                                           ( h ),    \
                                           ( t ),    \
                                           __func__, \
                                           __LINE__ )
            #define XFREE( p, h, t )               \
                wc_CryptoCb_IntelQaFree( ( p ),    \
                                         ( h ),    \
                                         ( t ),    \
                                         __func__, \
                                         __LINE__ )
            #define XREALLOC( p, n, h, t )            \
                wc_CryptoCb_IntelQaRealloc( ( p ),    \
                                            ( n ),    \
                                            ( h ),    \
                                            ( t ),    \
                                            __func__, \
...
WOLFSSL_API void xfree( void * p,
    ...
    #define XMALLOC( s, h, t ) \
        ( ( void ) h, ( void ) t, m2mb_os_malloc( ( s ) ) )
    #define XFREE( p, h, t )            \
        {                               \
            void * xp = ( p );          \
            if( ( xp ) )                \
    ...
    #include <wolfssl/wolfcrypt/memory.h>
            ...
            #define XFREE( p, h, t )                                \
                {                                                   \
                    void * xp = ( p );                              \
                    if( ( xp ) )                                    \
                        wolfSSL_Free( ( xp ), __func__, __LINE__ ); \
                }
            #define XREALLOC( p, n, h, t ) \
                wolfSSL_Realloc( ( p ), ( n ), __func__, __LINE__ )
        #else
            #define XMALLOC( s, h, t ) \
                ( ( void ) h, ( void ) t, wolfSSL_Malloc( ( s ) ) )
            #define XFREE( p, h, t )            \
                {                               \
                    void * xp = ( p );          \
                    if( ( xp ) )                \
...
/* declare/free variable handling for async */
    ...
    #define DECLARE_ARRAY( VAR_NAME, VAR_TYPE, VAR_ITEMS, VAR_SIZE, HEAP )   \
        ...
        {                                                                    \
            VAR_NAME[ idx##VAR_NAME ] = ( VAR_TYPE * )                       \
                XMALLOC( VAR_SIZE, ( HEAP ), DYNAMIC_TYPE_WOLF_BIGINT );     \
        }
    ...
    #define FREE_ARRAY( VAR_NAME, VAR_ITEMS, HEAP )                          \
        for( idx##VAR_NAME = 0; idx##VAR_NAME < VAR_ITEMS; idx##VAR_NAME++ ) \
        {                                                                    \
            XFREE( VAR_NAME[ idx##VAR_NAME ],                                \
                   ( HEAP ),                                                 \
                   DYNAMIC_TYPE_WOLF_BIGINT );                               \
        }

    #define DECLARE_ARRAY_DYNAMIC_DEC( VAR_NAME,  \
                                       VAR_TYPE,  \
                                       VAR_ITEMS, \
                                       VAR_SIZE,  \
                                       HEAP )     \
        DECLARE_ARRAY( VAR_NAME, VAR_TYPE, VAR_ITEMS, VAR_SIZE, HEAP )
    #define DECLARE_ARRAY_DYNAMIC_EXE( VAR_NAME,  \
                                       VAR_TYPE,  \
                                       VAR_ITEMS, \
                                       VAR_SIZE,  \
                                       HEAP )
    ...
    #define FREE_ARRAY( VAR_NAME, VAR_ITEMS, HEAP )    /* nothing to free, its \
                                                          stack */
    ...
    #define DECLARE_ARRAY_DYNAMIC_EXE( VAR_NAME,                             \
        ...
        {                                                                    \
            VAR_NAME[ idx##VAR_NAME ] = ( VAR_TYPE * )                       \
                XMALLOC( VAR_SIZE, ( HEAP ), DYNAMIC_TYPE_TMP_BUFFER );      \
        }
    #define FREE_ARRAY_DYNAMIC( VAR_NAME, VAR_ITEMS, HEAP )                  \
        for( idx##VAR_NAME = 0; idx##VAR_NAME < VAR_ITEMS; idx##VAR_NAME++ ) \
        {                                                                    \
    ...
    /* snprintf is used in asn.c for GetTimeString, PKCS7 test, and when
       debugging is turned on */
            ...
            defined( __MINGW32__ )
                ...
                #endif
static WC_INLINE int xsnprintf( char * buffer,
                                size_t bufsize,
                                const char * format,
                                ... )
{
...
enum
{
#if !defined( USE_FAST_MATH ) && !defined( SIZEOF_LONG ) && \
