...
extern "C" {
    ...
    #if defined( FP_64BIT ) && defined( __INTEL_COMPILER ) && \
 ...
 * changing the build from the make process */
    ...
    /* do we want the large set of small multiplications ?
       ...
       multiplications say in ECC Or if you're on a 64-bit machine doing RSA as
       a 1024-bit integer == 16 digits ;-)
     */
    /* need to refactor the function */
    /*#define TFM_SMALL_SET */

    /* do we want huge code
       Enable these if you are doing 20, 24, 28, 32, 48, 64 digit
       multiplications (useful for RSA) Less important on 64-bit machines as 32
       digits == 2048 bits
     */
    #if 0
        #define TFM_MUL3
     ...
     * This makes it slower but also timing invariant with respect to the
     * exponent
     */
...
/* callback for fp_prime_random, should fill dst with random bytes and return
 * how many read [up to len] */
/*typedef int tfm_prime_callback(unsigned char *dst, int len, void *dat);*/

/*#define fp_prime_random(a, t, size, bbs, cb, dat) fp_prime_random_ex(a, t,
 * ((size) * 8) + 1, (bbs==1)?TFM_PRIME_BBS:0, cb, dat)*/

/*int fp_prime_random_ex(fp_int *a, int t, int size, int flags,
 * tfm_prime_callback cb, void *dat);*/
