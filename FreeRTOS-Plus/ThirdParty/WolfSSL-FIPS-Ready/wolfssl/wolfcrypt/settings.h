...
#if defined( _WIN32 ) && !defined( _M_X64 ) && defined( HAVE_AESGCM ) && \
...
/* Micrium will use Visual Studio for compilation but not the Win32 API */
    ...
    !defined( WOLFSSL_EROAD ) && !defined( WOLFSSL_UTASKER ) &&         \
    !defined( INTIME_RTOS )
    #define USE_WINDOWS_API
#endif
...
static char * fgets( char * buff, int sz, XFILE fp )
{
    ...
    *p = '\0';
    ...
    #define XMALLOC( s, h, type ) \
        ( ( void * ) rtp_malloc( ( s ), SSL_PRO_MALLOC ) )
    ...
    #define XREALLOC( p, n, h, t ) ( rtp_realloc( ( p ), ( n ) ) )
        ...
        #define XSTRNCASECMP( s1, s2, n ) _strnicmp( ( s1 ), ( s2 ), ( n ) )
    #else
        #sslpro : settings.h - please implement XSTRNCASECMP - \
            needed for HAVE_ECC
    #endif
...
/* To support storing some of the large constant tables in flash memory rather
   than SRAM. Useful for processors that have limited SRAM, such as the AVR
   family of microtrollers. */
    ...
    #define XMEMCPY_P( pdest, psrc, size ) \
    ...
    #define XREALLOC( p, n, h, t ) \
        _mem_realloc( ( p ), ( n ) ) /* since MQX 4.1.2 */
    ...
    #if defined( FSL_FEATURE_SOC_TRNG_COUNT ) && \
        ( FSL_FEATURE_SOC_TRNG_COUNT > 0 )
        #define FREESCALE_KSDK_2_0_TRNG
    #elif defined( FSL_FEATURE_SOC_RNG_COUNT ) && \
        ( FSL_FEATURE_SOC_RNG_COUNT > 0 )
        #ifdef FREESCALE_KSDK_1_3
    ...
    /* if case manual selection is required, for example for benchmarking
     * purposes, just define FREESCALE_USE_MMCAU or FREESCALE_USE_LTC or none of
     * these two macros (for software only) both can be enabled simultaneously
     * as LTC has priority over MMCAU in source code.
     */
    /* #define FSL_HW_CRYPTO_MANUAL_SELECTION */
    #ifndef FSL_HW_CRYPTO_MANUAL_SELECTION
        #if defined( FSL_FEATURE_SOC_MMCAU_COUNT ) && \
...
/* KSDK mmCAU library */
    ...
    #define FREESCALE_MMCAU_SHA
        ...
        #define LTC_BASE LTC0
            ...
            /* the LTC PKHA hardware limit is 2048 bits (256 bytes) for integer
               arithmetic. the LTC_MAX_INT_BYTES defines the size of local
               variables that hold big integers. */
             ...
             * for TFM is linked in. The decision for which algorithm is used is
             * determined at runtime from size of inputs. If inputs and result
             * can fit into LTC (see LTC_MAX_INT_BYTES) then we call hardware
             * algorithm, otherwise we call software algorithm.
             *
             * Chinese reminder theorem is used to break RSA 4096
             * exponentiations (both public and private key) into several
             * computations with 2048-bit modulus and exponents.
             */
            /* #define FREESCALE_LTC_TFM_RSA_4096_ENABLE */

            /* ECC-384, ECC-256, ECC-224 and ECC-192 have been enabled with LTC
             * PKHA acceleration */
            #ifdef HAVE_ECC
                #undef ECC_TIMING_RESISTANT
                #define ECC_TIMING_RESISTANT
                ...
                /* the LTC PKHA hardware limit is 512 bits (64 bytes) for ECC.
                   the LTC_MAX_ECC_BITS defines the size of local variables that
                   hold ECC parameters and point coordinates */
...
/* if LTC has AES engine but doesn't have GCM, use software with LTC AES ECB
 * mode */
...
/* if using hardware crypto and have alignment requirements, specify the
   ...
   This hint tries to help as much as possible.  */
    ...
    #pragma warning( disable : 2259 ) /* explicit casts to smaller sizes, \
                                         disable */
#endif
...
/* restriction with static memory */
    ...
    #if defined( HAVE_IO_POOL ) || defined( XMALLOC_USER ) || \
        defined( NO_WOLFSSL_MEMORY )
...
/* warning for not using harden build options (default with ./configure) */
        ...
        ( !defined( NO_RSA ) && !defined( WC_RSA_BLINDING ) &&             \
          !defined( HAVE_FIPS ) && !defined( WC_NO_RNG ) )
...
/* RAW hash function APIs are not implemented with ARMv8 hardware acceleration*/
    ...
    defined( NO_AES ) && !defined( WOLFSSL_SHA3 )
...
#if defined( WOLFCRYPT_ONLY ) && defined( NO_AES ) &&           \
    ...
    defined( WC_NO_RNG ) && defined( WOLFSSL_SP_MATH ) &&       \
