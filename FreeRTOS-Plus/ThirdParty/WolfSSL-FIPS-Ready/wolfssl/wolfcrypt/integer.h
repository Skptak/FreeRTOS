...
extern "C" {
    ...
    #if defined( MP_64BIT ) && defined( __INTEL_COMPILER ) && \
...
typedef ulong64 mp_word;
                ...
                #define DIGIT_BIT 31
            #else
                /* default case is 28-bit digits, defines MP_28BIT as a handy
                 * test macro */
                #define DIGIT_BIT 28
                #define MP_28BIT
            #endif
...
typedef int mp_err;
    ...
    /* size of comba arrays, should be at least 2 * 2**(BITS_PER_WORD -
       BITS_PER_DIGIT*2) */
    #define MP_WARRAY                                                       \
        ( ( mp_word ) 1 << ( sizeof( mp_word ) * CHAR_BIT - 2 * DIGIT_BIT + \
                             1 ) )
...
typedef int ltm_prime_callback( unsigned char * dst, int len, void * dat );
    ...
    #define mp_isone( a )                                                   \
        ( ( ( ( ( a )->used == 1 ) ) && ( ( a )->dp[ 0 ] == 1u ) ) ? MP_YES \
                                                                   : MP_NO )
    #define mp_iseven( a )                                                  \
        ( ( ( a )->used > 0 && ( ( ( a )->dp[ 0 ] & 1u ) == 0u ) ) ? MP_YES \
                                                                   : MP_NO )
    #define mp_isodd( a )                                                   \
        ( ( ( a )->used > 0 && ( ( ( a )->dp[ 0 ] & 1u ) == 1u ) ) ? MP_YES \
                                                                   : MP_NO )
    ...
    #define mp_prime_random( a, t, size, bbs, cb, dat )       \
        mp_prime_random_ex( a,                                \
                            t,                                \
                            ( ( size ) *8 ) + 1,              \
                            ( bbs == 1 ) ? LTM_PRIME_BBS : 0, \
                            cb,                               \
                            dat )
...
MP_API int mp_montgomery_reduce( mp_int * x, mp_int * n, mp_digit rho );
    #define mp_montgomery_reduce_ex( x, n, rho, ct ) \
        mp_montgomery_reduce( x, n, rho )
