...
extern "C" {
    ...
    /* calculate max ECC bytes */
        ...
        /* ECC maximum padding size (when MSB is set extra byte required for R
         * and S) */
...
enum
{
    ...
    SIG_HEADER_SZ = 7,    /* ECC signature header size (30 81 87 02 42 [R] 02 42
                             [S]) */
    ...
    ECC_MAX_OID_LEN = 16,
    ECC_MAX_SIG_SIZE = ( ( MAX_ECC_BYTES * 2 ) + ECC_MAX_PAD_SZ +
                         SIG_HEADER_SZ ),
...
typedef byte ecc_oid_t;
         ...
         * GCC, and it causes the FIPS checksum to fail. In the case of windows
         * builds, store everything as arrays instead of pointers to strings. */
...
typedef struct ecc_set_type
{
    ...
    const char prime[ MAX_ECC_STRING ]; /* prime that defines the field, curve
                                           is in (hex) */
    ...
    const char Gx[ MAX_ECC_STRING ]; /* x coordinate of the base point on curve
                                        (hex) */
    const char Gy[ MAX_ECC_STRING ]; /* y coordinate of the base point on curve
                                        (hex) */
    ...
    int cofactor;
         ...
         * The fast math code uses an array of a fixed size to store the big
         * integers. By default, the array is big enough for RSA keys. There is
         * a size, FP_MAX_BITS which can be used to make the array smaller when
         * one wants ECC but not RSA. Some people want fast math sized for both
         * RSA and ECC, where ECC won't use as much as RSA. The flag
         * ALT_ECC_SIZE switches in an alternate ecc_point structure that uses
         * an alternate fp_int that has a shorter array of fp_digits.
         *
         * Now, without ALT_ECC_SIZE, the ecc_point has three single item arrays
         * of mp_ints for the components of the point. With ALT_ECC_SIZE, the
         * components of the point are pointers that are set to each of a three
         * item array of alt_fp_ints. While an mp_int will have 4096 bits of
         * digit inside the structure, the alt_fp_int will only have 512 bits
         * for ECC 256-bit and 1056-bits for ECC 521-bit. A size value was added
         * in the ALT case, as well, and is set by mp_init() and alt_fp_init().
         * The functions fp_zero() and fp_copy() use the size parameter. An int
         * needs to be initialized before using it instead of just fp_zeroing
         * it, the init will call zero. The FP_MAX_BITS_ECC defaults to
         * calculating based on MAX_ECC_BITS, but can be set to change the
         * number of bits used in the alternate FP_INT.
         *
         * The ALT_ECC_SIZE option only applies to stack based fast math
         * USE_FAST_MATH.
         */
        ...
        /* determine max bits required for ECC math */
            ...
            #define FP_MAX_BITS_ECC                                      \
                ( 2 * ( ( MAX_ECC_BITS + DIGIT_BIT - 1 ) / DIGIT_BIT ) * \
                  DIGIT_BIT )
...
WOLFSSL_ABI WOLFSSL_API int wc_ecc_make_key_ex( WC_RNG * rng,
                                                int keysize,
                                                ecc_key * key,
                                                int curve_id );
WOLFSSL_API
...
int wc_ecc_shared_secret_ex( ecc_key * private_key,
        ...
        #else
            #define wc_ecc_shared_secret_ssh \
    ...
    #ifdef HAVE_ECC_SIGN
WOLFSSL_ABI WOLFSSL_API int wc_ecc_sign_hash( const byte * in,
                                              word32 inlen,
                                              byte * out,
                                              word32 * outlen,
                                              WC_RNG * rng,
...
WOLFSSL_ABI WOLFSSL_API int wc_ecc_init_ex( ecc_key * key,
                                            void * heap,
                                            int devId );
    #ifdef HAVE_PKCS11
WOLFSSL_API
...
WOLFSSL_ABI WOLFSSL_API int wc_ecc_import_x963( const byte * in,
                                                word32 inLen,
                                                ecc_key * key );
WOLFSSL_API
