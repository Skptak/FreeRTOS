...
static int printfk( const char * fmt, ... )
{
    ...
    return ret;
        ...
        #endif
        #define TEST_SLEEP() \
            m2mb_os_taskSleep( M2MB_OS_MS2TICKS( TEST_SLEEP_MS ) )
...
static WOLFSSL_HEAP_HINT * HEAP_HINT;
    ...
    #define BENCH_SHA2 \
        ( BENCH_SHA224 | BENCH_SHA256 | BENCH_SHA384 | BENCH_SHA512 )
    ...
    #define BENCH_HMAC                                          \
        ( BENCH_HMAC_MD5 | BENCH_HMAC_SHA | BENCH_HMAC_SHA224 | \
          BENCH_HMAC_SHA256 | BENCH_HMAC_SHA384 | BENCH_HMAC_SHA512 )
...
static const bench_alg bench_cipher_opt[] = { { "-cipher", -1 },
            #ifdef HAVE_AES_CBC
...
static const bench_alg bench_digest_opt[] = { { "-digest", -1 },
            #ifndef NO_MD5
                                              { "-md5", BENCH_MD5 },
            #endif
            #ifdef HAVE_POLY1305
 ...
 */
static const bench_alg bench_other_opt[] = { { "-other", -1 },
            #ifndef WC_NO_RNG
                                             { "-rng", BENCH_RNG },
            #endif
            #ifdef HAVE_SCRYPT
...
static const char * bench_result_words1[][ 4 ] = {
    ...
    { "を", "秒で処理", "1バイトあたりのサイクル数", NULL }, /* 1 Japanese */
        ...
        defined( HAVE_NTRU ) || defined( HAVE_ECC ) || !defined( NO_DH ) ||    \
        defined( HAVE_ECC_ENCRYPT ) || defined( HAVE_CURVE25519 ) ||           \
        defined( HAVE_CURVE25519_SHARED_SECRET ) || defined( HAVE_ED25519 ) || \
        defined( HAVE_CURVE448 ) || defined( HAVE_CURVE448_SHARED_SECRET ) ||  \
        defined( HAVE_ED448 )
...
static THREAD_LS_T struct perf_event_attr atr;
        ...
        #define INIT_CYCLE_COUNTER                                       \
            ...
            {                                                            \
                ...
                cycles = ( int )                                         \
                    syscall( __NR_perf_event_open, &atr, 0, -1, -1, 0 ); \
            } while( 0 );

        #define BEGIN_INTEL_CYCLES \
            read( cycles, &begin_cycles, sizeof( begin_cycles ) );
        #define END_INTEL_CYCLES                                       \
            do                                                         \
            {                                                          \
...
WOLFSSL_API void wolfSSL_Debugging_OFF( void );
        ...
        !defined( NO_DH ) || defined( WOLFSSL_KEY_GEN ) ||              \
                               ...
                               int * pending )
{
    ...
    if( *ret == WC_PENDING_E )
    {
        ...
        {
            ( *pending )++;
            *ret = wc_AsyncHandle( asyncDev,
                                   &eventQueue,
                                   callAgain ? WC_ASYNC_FLAG_CALL_AGAIN
                                             : WC_ASYNC_FLAG_NONE );
        }
    }
    else if( *ret >= 0 )
...
static WC_INLINE int bench_async_poll( int * pending )
{
    ...
    ret = wolfAsync_EventQueuePoll( &eventQueue,
                                    NULL,
                                    NULL,
                                    0,
...
static const XGEN_ALIGN byte bench_key_buf[] = {
    ...
    0x45, 0x67, 0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef
};
                                        ...
                                        int ret )
{
    ...
    for( bstat = bench_stats_head; bstat != NULL; bstat = bstat->next )
    {
        ...
        if( bstat->algo == algo && bstat->strength == strength &&
            bstat->desc == desc && bstat->doAsync == doAsync )
        {
            break;
        }
    }
    ...
    if( bstat == NULL )
    {
        ...
        bstat = ( bench_stats_t * ) XMALLOC( sizeof( bench_stats_t ),
                                             NULL,
                                             DYNAMIC_TYPE_INFO );
        if( bstat )
        {
    ...
    for( bstat = bench_stats_head; bstat != NULL; )
    {
        if( bstat->type == BENCH_STAT_SYM )
        {
            printf( "%-16s%s %8.3f %s/s\n",
                    bstat->desc,
                    BENCH_ASYNC_GET_NAME( bstat->doAsync ),
                    bstat->perfsec,
                    base2 ? "MB" : "mB" );
        }
        else
                                    ...
                                    int ret )
{
    ...
    else
    {
        XSNPRINTF( msg,
                   sizeof( msg ),
                   "%-16s%s %5.0f %s %s %5.3f %s, %8.3f %s/s",
                   desc,
                   BENCH_ASYNC_GET_NAME( doAsync ),
                   blocks,
                   blockType,
                   word[ 0 ],
                   total,
                                     ...
                                     int ret )
{
    ...
    else
    {
        XSNPRINTF( msg,
                   sizeof( msg ),
                   "%-6s %5d %-9s %s %6d %s %5.3f %s, %s %5.3f ms,"
                   " %.3f %s\n",
                   algo,
                   strength,
                   desc,
    ...
    /* Add to thread stats */
    bench_stats_add( BENCH_STAT_ASYM,
                     algo,
                     strength,
                     desc,
                     doAsync,
...
static void * benchmarks_do( void * args )
{
    ...
    bench_plain = ( byte * ) aligned_alloc( 64,
                                            ( size_t ) bench_buf_size + 16 );
    bench_cipher = ( byte * ) aligned_alloc( 64,
                                             ( size_t ) bench_buf_size + 16 );
    #else
    ...
    if( bench_all || ( bench_cipher_algs & BENCH_AES_CBC ) )
    {
        ...
        bench_aescbc( 0 );
                   ...
                   defined( WC_ASYNC_ENABLE_3DES ) ) ||  \
                 defined( HAVE_INTEL_QA_SYNC ) ||        \
                 defined( HAVE_CAVIUM_OCTEON_SYNC ) ) && \
                !defined( NO_HW_BENCH )
        bench_aescbc( 1 );
            #endif
    }
    ...
    if( bench_all || ( bench_cipher_algs & BENCH_AES_ECB ) )
    {
        ...
        bench_aesecb( 0 );
            #endif
            #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                defined( WC_ASYNC_ENABLE_AES ) && !defined( NO_HW_BENCH )
        bench_aesecb( 1 );
            #endif
    }
        #endif
    ...
    if( bench_all || ( bench_cipher_algs & BENCH_ARC4 ) )
    {
        ...
        #endif
        #if defined( WOLFSSL_ASYNC_CRYPT ) && \
            defined( WC_ASYNC_ENABLE_ARC4 ) && !defined( NO_HW_BENCH )
        bench_arc4( 1 );
        #endif
    }
    #endif
    ...
    if( bench_all || ( bench_digest_algs & BENCH_SHA3_224 ) )
    {
        ...
        bench_sha3_224( 0 );
            #endif
            #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                defined( WC_ASYNC_ENABLE_SHA3 ) && !defined( NO_HW_BENCH )
        bench_sha3_224( 1 );
            #endif
    }
    ...
    if( bench_all || ( bench_asym_algs & BENCH_RSA ) )
    {
        ...
        #endif
        #if defined( WOLFSSL_ASYNC_CRYPT ) && \
            defined( WC_ASYNC_ENABLE_RSA ) && !defined( NO_HW_BENCH )
        bench_rsa( 1 );
        #endif
    }
    ...
    if( bench_all || ( bench_asym_algs & BENCH_ECC_MAKEKEY ) )
    {
        ...
        #endif
        #if defined( WOLFSSL_ASYNC_CRYPT ) && \
            defined( WC_ASYNC_ENABLE_ECC ) && !defined( NO_HW_BENCH )
        bench_eccMakeKey( 1 );
        #endif
    }
...
int benchmark_init( void )
{
    ...
    #ifdef WOLFSSL_STATIC_MEMORY
    ret = wc_LoadStaticMemory( &HEAP_HINT,
                               gBenchMemory,
                               sizeof( gBenchMemory ),
                               WOLFMEM_GENERAL,
                               1 );
    ...
    #endif
{
    ...
    #if defined( WOLFSSL_ASYNC_CRYPT ) && !defined( WC_NO_ASYNC_THREADING )
    {
        ...
        g_threadData = ( ThreadData * ) XMALLOC( sizeof( ThreadData ) *
                                                     g_threadCount,
                                                 HEAP_HINT,
    ...
    do
    {
        for( i = 0; i < numBlocks; i++ )
        {
            ...
            while( remain > 0 )
            {
                ...
                ret = wc_RNG_GenerateBlock( &myrng,
                                            &bench_plain[ pos ],
                                            ( word32 ) len );
                if( ret < 0 )
                    goto exit_rng;
        ...
        for( times = 0; times < numBlocks || pending > 0; )
        {
            ...
            for( i = 0; i < BENCH_MAX_PENDING; i++ )
            {
                if( bench_async_check( &ret,
                                       BENCH_ASYNC_GET_DEV( &enc[ i ] ),
                                       0,
                                       &times,
                                       numBlocks,
                                       &pending ) )
                {
                    ret = wc_AesCbcEncrypt( &enc[ i ],
                                            bench_plain,
                                            bench_cipher,
                                            BENCH_SIZE );
...
exit_aes_gcm:
    ...
    do
    {
        for( times = 0; times < numBlocks || pending > 0; )
        {
            ...
            {
                if( bench_async_check( &ret,
                                       BENCH_ASYNC_GET_DEV( &dec[ i ] ),
                                       0,
                                       &times,
                                       numBlocks,
                                       &pending ) )
                {
                    ret = wc_AesGcmDecrypt( &dec[ i ],
                                            bench_plain,
                                            bench_cipher,
                                            BENCH_SIZE,
                                            iv,
...
exit_aes_enc:
    bench_stats_sym_finish( encLabel,
                            doAsync,
                            count,
                            AES_BLOCK_SIZE,
                            start,
...
exit_aes_dec:
    bench_stats_sym_finish( decLabel,
                            doAsync,
                            count,
                            AES_BLOCK_SIZE,
                            start,
...
void bench_aesxts( void )
{
    ...
    static unsigned char k1[] = { 0xa1, 0xb9, 0x0c, 0xba, 0x3f, 0x06, 0xac,
                                  0x35, 0x3b, 0x2c, 0x34, 0x38, 0x76, 0x08,
                                  0x17, 0x62, 0x09, 0x09, 0x23, 0x02, 0x6e,
                                  0x91, 0x77, 0x18, 0x15, 0xf2, 0x9d, 0xab,
                                  0x01, 0x93, 0x2f, 0x2f };

    static unsigned char i1[] = { 0x4f, 0xae, 0xf7, 0x11, 0x7c, 0xda,
                                  0x59, 0xc6, 0x6e, 0x4b, 0x92, 0x01,
                                  0x3e, 0x76, 0x8a, 0xd5 };

    ret = wc_AesXtsSetKey( &aes,
                           k1,
                           sizeof( k1 ),
                           AES_ENCRYPTION,
                           HEAP_HINT,
...
void bench_chacha20_poly1305_aead( void )
{
    ...
    do
    {
        for( i = 0; i < numBlocks; i++ )
        {
            ret = wc_ChaCha20Poly1305_Encrypt( bench_key,
                                               bench_iv,
                                               NULL,
                                               0,
                                               bench_plain,
...
void bench_pbkdf2( void )
{
    ...
    do
    {
        ret = wc_PBKDF2( derived,
                         ( const byte * ) passwd32,
                         ( int ) XSTRLEN( passwd32 ),
                         salt32,
                         ( int ) sizeof( salt32 ),
                         1000,
                         32,
...
static unsigned char rsa_2048_sig[] = {
    
};
            #elif defined( USE_CERT_BUFFERS_3072 )
static unsigned char rsa_3072_sig[] = {
    
};
            #else
                              ...
                              int rsaKeySz )
{
    ...
    /* MDK5 compiler regard this as a executable statement, and does not allow
     * declarations after the line. */
    DECLARE_ARRAY_DYNAMIC_DEC( enc,
                               byte,
                               BENCH_MAX_PENDING,
                               rsaKeySz,
                               HEAP_HINT );
            ...
            for( times = 0; times < ntimes || pending > 0; )
            {
                ...
                for( i = 0; i < BENCH_MAX_PENDING; i++ )
                {
                    if( bench_async_check( &ret,
                                           ...
                                           &pending ) )
                    {
                        ret = wc_RsaPublicEncrypt( message,
                                                   ( word32 ) len,
                                                   enc[ i ],
                                                   rsaKeySz / 8,
                                                   &rsaKey[ i ],
            ...
            for( times = 0; times < ntimes || pending > 0; )
            {
                ...
                for( i = 0; i < BENCH_MAX_PENDING; i++ )
                {
                    if( bench_async_check( &ret,
                                           ...
                                           &pending ) )
                    {
                        ret = wc_RsaPrivateDecrypt( enc[ i ],
                                                    idx,
                                                    out[ i ],
                                                    rsaKeySz / 8,
                                                    &rsaKey[ i ] );
        ...
        do
        {
            for( times = 0; times < ntimes || pending > 0; )
            {
                ...
                !defined( WOLFSSL_RSA_PUBLIC_ONLY )
                        ...
                        out[ i ] = NULL;
                        ret = wc_RsaSSL_VerifyInline( enc[ i ],
                                                      idx,
                                                      &out[ i ],
                                                      &rsaKey[ i ] );
                        if( ret > 0 )
...
void bench_rsa( int doAsync )
{
    ...
    for( i = 0; i < BENCH_MAX_PENDING; i++ )
    {
        ...
        if( ( ret = wc_InitRsaKey_ex( &rsaKey[ i ],
                                      HEAP_HINT,
                                      doAsync ? devId : INVALID_DEVID ) ) < 0 )
        {
            goto exit_bench_rsa;
        ...
        idx = 0;
        if( ( ret = wc_RsaPrivateKeyDecode( tmp,
                                            &idx,
                                            &rsaKey[ i ],
                                            ( word32 ) bytes ) ) != 0 )
        {
...
void bench_rsa_key( int doAsync, int rsaKeySz )
{
    ...
    do
    {
        ...
        {
            if( !isPending[ i ] )
            { /* if making the key is pending then just call
               * wc_MakeRsaKey again */
                /* setup an async context for each key */
                if( ( ret = wc_InitRsaKey_ex( &rsaKey[ i ],
                                              HEAP_HINT,
    ...
    do
    {
        ...
        for( times = 0; times < genTimes || pending > 0; )
        {
            ...
            for( i = 0; i < BENCH_MAX_PENDING; i++ )
            {
                ...
                {
                    privSz[ i ] = 0;
                    ret = wc_DhGenerateKeyPair( &dhKey[ i ],
                                                &gRng,
                                                priv[ i ],
                                                &privSz[ i ],
                                                pub[ i ],
...
exit_dh_gen:
    ...
    ret = wc_DhGenerateKeyPair( &dhKey[ 0 ],
                                &gRng,
                                priv2,
                                &privSz2,
                                pub2,
...
void bench_ntru( void )
{
    ...
    DRBG_HANDLE drbg;
    static byte const aes_key[] = { 0xf3, 0xe9, 0x87, 0xbb, 0x18, 0x08,
                                    0x3c, 0xaa, 0x7b, 0x12, 0x49, 0x88,
                                    0xaf, 0xb3, 0x22, 0xd8 };

    static byte const wolfsslStr[] = { 'w', 'o', 'l', 'f', 'S', 'S',
                                       'L', ' ', 'N', 'T', 'R', 'U' };
    ...
    for( ntruBits = 128; ntruBits < 257; ntruBits += 64 )
    {
        ...
        ret = ntru_crypto_drbg_instantiate( ntruBits,
                                            wolfsslStr,
                                            sizeof( wolfsslStr ),
                                            ( ENTROPY_FN ) GetEntropy,
                                            &drbg );
        if( ret != DRBG_OK )
        {
        ...
        /* set key sizes */
        ret = ntru_crypto_ntru_encrypt_keygen( drbg,
                                               type,
                                               &public_key_len,
                                               NULL,
                                               &private_key_len,
        ...
        ret = ntru_crypto_ntru_encrypt( drbg,
                                        public_key_len,
                                        public_key,
                                        sizeof( aes_key ),
                                        aes_key,
        ...
        ret = ntru_crypto_ntru_decrypt( private_key_len,
                                        private_key,
                                        ciphertext_len,
                                        ciphertext,
                                        &plaintext_len,
...
void bench_ntruKeyGen( void )
{
    ...
    DRBG_HANDLE drbg;
    static uint8_t const pers_str[] = { 'w', 'o', 'l', 'f', 'S', 'S',
                                        'L', ' ', 't', 'e', 's', 't' };
...
void bench_ecc( int doAsync )
{
    ...
    for( i = 0; i < BENCH_MAX_PENDING; i++ )
    {
        ...
        if( ( ret = wc_ecc_init_ex( &genKey[ i ],
                                    HEAP_HINT,
                                    doAsync ? devId : INVALID_DEVID ) ) < 0 )
        {
            goto exit;
        ...
        #ifdef HAVE_ECC_DHE
        if( ( ret = wc_ecc_init_ex( &genKey2[ i ], HEAP_HINT, INVALID_DEVID ) ) <
            0 )
        {
            goto exit;
        }
    ...
    do
    {
        for( times = 0; times < agreeTimes || pending > 0; )
        {
            ...
            for( i = 0; i < BENCH_MAX_PENDING; i++ )
            {
                ...
                {
                    x[ i ] = ( word32 ) keySize;
                    ret = wc_ecc_shared_secret( &genKey[ i ],
                                                &genKey2[ i ],
                                                shared[ i ],
                                                &x[ i ] );
                    if( !bench_async_handle( &ret,
...
double current_time( int reset )
{
    ...
    return ( ns / CLOCK * 2.0 );
            ...
            defined( WOLFSSL_CURRTIME_REMAP )
...
static void Usage( void )
{
    ...
    printf( "%s", bench_Usage_msg1[ lng_index ][ 13 ] ); /* option <num> */
            #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                !defined( WC_NO_ASYNC_THREADING )
    printf( "%s", bench_Usage_msg1[ lng_index ][ 14 ] ); /* option -threads
                                                            <num> */
            #endif
