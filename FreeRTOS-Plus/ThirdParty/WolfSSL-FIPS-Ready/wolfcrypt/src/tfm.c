...
int fp_invmod_mont_ct( fp_int * a, fp_int * b, fp_int * c, fp_digit mp )
{
    ...
    #ifdef WOLFSSL_SMALL_STACK
    t = ( fp_int * ) XMALLOC( sizeof( fp_int ) * ( 2 + CT_INV_MOD_PRE_CNT ),
                              NULL,
                              DYNAMIC_TYPE_BIGINT );
    if( t == NULL )
        return FP_MEM;
...
static const word32 exptModNbInst[ TFM_EXPTMOD_NB_COUNT ] = {
                    #ifdef TFM_PPC32
                        #ifdef _DEBUG
    11098, 8701,   3971,   178394, 858093, 1040,
    822,   178056, 181574, 90883,  184339, 236813
                        #else
...
static int fp_exptmod_nb_checktime( exptModNb_t * nb )
{
    ...
    return TFM_EXPTMOD_NB_STOP;
                    ...
                    #define FP_EXPTMOD_NB_CHECKTIME( nb ) \
                        fp_exptmod_nb_checktime( ( nb ) )
                   ...
                   fp_int * Y )
{
    ...
    do
    {
        ...
        switch( nb->state )
        {
            ...
            #else
            fp_copy( ( fp_int * ) ( ( ( wolfssl_word ) &nb->R[ 0 ] &
                                      wc_off_on_addr[ nb->y ^ 1 ] ) +
                                    ( ( wolfssl_word ) &nb->R[ 1 ] &
                                      wc_off_on_addr[ nb->y ] ) ),
                     &nb->R[ 2 ] );
                           ...
                           fp_int * Y )
{
    ...
    */
    ...
    for( ;; )
    {
        ...
        /* instead of using R[y^1] for mul, which leaks key bit to cache
         * monitor, use R[2] as temp, make sure address calc is constant, keep
         * &R[0] and &R[1] in cache */
        fp_copy( &R[ 2 ],
                 ( fp_int * ) ( ( ( wolfssl_word ) &R[ 0 ] &
                                  wc_off_on_addr[ y ] ) +
                                ( ( wolfssl_word ) &R[ 1 ] &
                                  wc_off_on_addr[ y ^ 1 ] ) ) );
         ...
         * &R[0] and &R[1] in cache */
        fp_copy( ( fp_int * ) ( ( ( wolfssl_word ) &R[ 0 ] &
                                  wc_off_on_addr[ y ^ 1 ] ) +
                                ( ( wolfssl_word ) &R[ 1 ] &
                                  wc_off_on_addr[ y ] ) ),
                 &R[ 2 ] );
...
static int _fp_exptmod_nct( fp_int * G, fp_int * X, fp_int * P, fp_int * Y )
{
    ...
    M = ( fp_int * ) XMALLOC( sizeof( fp_int ) * ( ( 1 << winsize ) + 1 ),
                              NULL,
                              DYNAMIC_TYPE_BIGINT );
    if( M == NULL )
    {
     ...
     *
     * The first half of the table is not computed though except for M[0] and
     * M[1]
     */
    ...
    {
        fp_sqr( &M[ ( word32 ) ( 1 << ( winsize - 1 ) ) ],
                &M[ ( word32 ) ( 1 << ( winsize - 1 ) ) ] );
        err = fp_montgomery_reduce_ex( &M[ ( word32 ) ( 1 << ( winsize - 1 ) ) ],
                                       P,
                                       mp,
                                       0 );
        if( err != FP_OKAY )
 ...
 * Some restrictions... x must be positive and < b
 */
static int _fp_exptmod_base_2( fp_int * X, int digits, fp_int * P, fp_int * Y )
{
    fp_digit buf, mp;
    ...
    fp_int tmp[ 1 ];
        ...
        #ifdef WOLFSSL_SMALL_STACK
    res = ( fp_int * ) XMALLOC( 2 * sizeof( fp_int ),
                                NULL,
...
static int _fp_exptmod_base_2( fp_int * X, int digits, fp_int * P, fp_int * Y )
{
    ...
    if( ( err = fp_montgomery_setup( P, &mp ) ) != FP_OKAY )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    res = ( fp_int * ) XMALLOC( sizeof( fp_int ),
                                NULL,
                                DYNAMIC_TYPE_TMP_BUFFER );
    if( res == NULL )
    {
...
int fp_exptmod( fp_int * G, fp_int * X, fp_int * P, fp_int * Y )
{
    ...
    if( X->sign == FP_NEG )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
        tmp = ( fp_int * ) XMALLOC( sizeof( fp_int ) * 2,
                                    NULL,
                                    DYNAMIC_TYPE_BIGINT );
        if( tmp == NULL )
            return FP_MEM;
...
int fp_montgomery_setup( fp_int * a, fp_digit * rho )
{
    ...
    /* rho = -1/m mod b */
    *rho = ( fp_digit ) ( ( ( fp_word ) 1 << ( ( fp_word ) DIGIT_BIT ) ) -
                          ( ( fp_word ) x ) );
                                      ...
                                      int ct )
{
    ...
    if( m->used <= 16 )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    /* only allocate space for what's needed for window plus res */
    c = ( fp_digit * ) XMALLOC( sizeof( fp_digit ) * ( FP_SIZE + 1 ),
                                NULL,
                                DYNAMIC_TYPE_BIGINT );
    if( c == NULL )
    {
...
int fp_montgomery_reduce_ex( fp_int * a, fp_int * m, fp_digit mp, int ct )
{
    ...
    IF_HAVE_INTEL_MULX( err = fp_montgomery_reduce_mulx( a, m, mp, ct ),
                        return err );
    ( void ) err;
...
int mp_set_bit( mp_int * a, mp_digit b )
{
    return fp_set_bit( a, b );
        ...
        !defined( NO_DH ) || !defined( NO_DSA ) || !defined( NO_RSA )
...
int mp_montgomery_calc_normalization( mp_int * a, mp_int * b )
{
    ...
    #if defined( WC_MP_TO_RADIX ) || !defined( NO_DH ) || \
...
static int fp_prime_miller_rabin( fp_int * a, fp_int * b, int * result )
{
    ...
    fp_int *n1, *y, *r;
        ...
        #ifdef WOLFSSL_SMALL_STACK
    n1 = ( fp_int * ) XMALLOC( sizeof( fp_int ) * 3,
                               NULL,
                               DYNAMIC_TYPE_BIGINT );
    if( n1 == NULL )
    {
...
int mp_prime_is_prime_ex( mp_int * a, int t, int * result, WC_RNG * rng )
{
     ...
     * give a (1/4)^t chance of a false prime. */
    {
        ...
        b = ( fp_int * ) XMALLOC( sizeof( fp_int ) * 5,
                                  NULL,
                                  DYNAMIC_TYPE_BIGINT );
        if( b == NULL )
        {
...
static int fp_read_radix( fp_int * a, const char * str, int radix )
{
    ...
    while( *str )
    {
         ...
         * [e.g. in hex]
         */
        ch = ( char ) ( ( radix <= 36 ) ? XTOUPPER( ( unsigned char ) *str )
                                        : *str );
        for( y = 0; y < 64; y++ )
        {
            if( ch == fp_s_rmap[ y ] )
