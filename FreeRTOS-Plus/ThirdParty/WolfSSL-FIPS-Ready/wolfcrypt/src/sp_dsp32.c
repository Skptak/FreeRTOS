...
static const sp_digit p256_mod[ 10 ] __attribute__( ( aligned(
    128 ) ) ) = { 0x3ffffff, 0x3ffffff, 0x3ffffff, 0x003ffff, 0x0000000,
                  0x0000000, 0x0000000, 0x0000400, 0x3ff0000, 0x03fffff };
                #ifndef WOLFSSL_SP_SMALL
/* The Montogmery normalizer for modulus of the curve P256. */
static const sp_digit p256_norm_mod[ 10 ] __attribute__( ( aligned(
    128 ) ) ) = { 0x0000001, 0x0000000, 0x0000000, 0x3fc0000, 0x3ffffff,
                  0x3ffffff, 0x3ffffff, 0x3fffbff, 0x000ffff, 0x0000000 };
                #endif /* WOLFSSL_SP_SMALL */
/* The Montogmery multiplier for modulus of the curve P256. */
static const sp_digit p256_mp_mod
    __attribute__( ( aligned( 128 ) ) ) = 0x000001;
                #if defined( WOLFSSL_VALIDATE_ECC_KEYGEN ) || \
                    defined( HAVE_ECC_SIGN ) || defined( HAVE_ECC_VERIFY )
/* The order of the curve P256. */
static const sp_digit p256_order[ 10 ] __attribute__( ( aligned(
    128 ) ) ) = { 0x0632551, 0x272b0bf, 0x1e84f3b, 0x2b69c5e, 0x3bce6fa,
                  0x3ffffff, 0x3ffffff, 0x00003ff, 0x3ff0000, 0x03fffff };
                #endif
                #if defined( HAVE_ECC_SIGN ) || defined( HAVE_ECC_VERIFY )
/* The Montogmery normalizer for order of the curve P256. */
static const sp_digit p256_norm_order[ 10 ] __attribute__( ( aligned(
    128 ) ) ) = { 0x39cdaaf, 0x18d4f40, 0x217b0c4, 0x14963a1, 0x0431905,
                  0x0000000, 0x0000000, 0x3fffc00, 0x000ffff, 0x0000000 };
                #endif
                #if defined( HAVE_ECC_SIGN ) || defined( HAVE_ECC_VERIFY )
/* The Montogmery multiplier for order of the curve P256. */
static const sp_digit p256_mp_order
    __attribute__( ( aligned( 128 ) ) ) = 0x200bc4f;
                #endif
    ...
    *p = sp;
    ...
    return ret;
                    ...
                    #define sp_ecc_point_new( heap, sp, p ) \
                        sp_ecc_point_new_ex( ( heap ), NULL, &( p ) )
                #else
                    /* Set pointer to data and return no error. */
                    #define sp_ecc_point_new( heap, sp, p ) \
                        sp_ecc_point_new_ex( ( heap ), &( sp ), &( p ) )
                #endif
                                   ...
                                   const sp_digit * m )
{
    ...
    ( void ) m;
                ...
                #if defined( WOLFSSL_SP_SMALL ) || \
                    defined( WOLFSSL_SMALL_STACK )
    td = ( int64_t * ) XMALLOC( sizeof( int64_t ) * 2 * 8,
                                NULL,
                                DYNAMIC_TYPE_ECC );
    if( td == NULL )
    {
    ...
    if( err == MP_OKAY )
    {
        ...
        /*  1  1  0 -1 -1 -1 -1  0 */
        t[ 0 ] = 0 + a32[ 0 ] + a32[ 1 ] - a32[ 3 ] - a32[ 4 ] - a32[ 5 ] -
                 a32[ 6 ];
        /*  0  1  1  0 -1 -1 -1 -1 */
        t[ 1 ] = 0 + a32[ 1 ] + a32[ 2 ] - a32[ 4 ] - a32[ 5 ] - a32[ 6 ] -
                 a32[ 7 ];
        /*  0  0  1  1  0 -1 -1 -1 */
        t[ 2 ] = 0 + a32[ 2 ] + a32[ 3 ] - a32[ 5 ] - a32[ 6 ] - a32[ 7 ];
        /* -1 -1  0  2  2  1  0 -1 */
        t[ 3 ] = 0 - a32[ 0 ] - a32[ 1 ] + 2 * a32[ 3 ] + 2 * a32[ 4 ] +
                 a32[ 5 ] - a32[ 7 ];
        /*  0 -1 -1  0  2  2  1  0 */
        t[ 4 ] = 0 - a32[ 1 ] - a32[ 2 ] + 2 * a32[ 4 ] + 2 * a32[ 5 ] +
                 a32[ 6 ];
        /*  0  0 -1 -1  0  2  2  1 */
        t[ 5 ] = 0 - a32[ 2 ] - a32[ 3 ] + 2 * a32[ 5 ] + 2 * a32[ 6 ] +
                 a32[ 7 ];
        /* -1 -1  0  0  0  1  3  2 */
        t[ 6 ] = 0 - a32[ 0 ] - a32[ 1 ] + a32[ 5 ] + 3 * a32[ 6 ] +
                 2 * a32[ 7 ];
        /*  1  0 -1 -1 -1 -1  0  3 */
        t[ 7 ] = 0 + a32[ 0 ] - a32[ 2 ] - a32[ 3 ] - a32[ 4 ] - a32[ 5 ] +
                 3 * a32[ 7 ];
                                   ...
                                   sp_digit mp )
{
    ...
    sp_256_cond_sub_10( a,
                        a,
                        m,
                        0 - ( ( ( a[ 9 ] >> 22 ) > 0 ) ? ( sp_digit ) 1
                                                       : ( sp_digit ) 0 ) );
    sp_256_norm_10( a );
}
                                       ...
                                       const sp_digit * b )
{
    ...
    int64_t t1 = Q6_P_mpy_RR( a[ 0 ], b[ 1 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 0 ] );
    int64_t t2 = Q6_P_mpy_RR( a[ 0 ], b[ 2 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 1 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 0 ] );
    int64_t t3 = Q6_P_mpy_RR( a[ 0 ], b[ 3 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 2 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 1 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 0 ] );
    int64_t t4 = Q6_P_mpy_RR( a[ 0 ], b[ 4 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 3 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 2 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 1 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 0 ] );
    int64_t t5 = Q6_P_mpy_RR( a[ 0 ], b[ 5 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 4 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 3 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 2 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 1 ] ) + Q6_P_mpy_RR( a[ 5 ], b[ 0 ] );
    int64_t t6 = Q6_P_mpy_RR( a[ 0 ], b[ 6 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 5 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 4 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 3 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 2 ] ) + Q6_P_mpy_RR( a[ 5 ], b[ 1 ] ) +
                 Q6_P_mpy_RR( a[ 6 ], b[ 0 ] );
    int64_t t7 = Q6_P_mpy_RR( a[ 0 ], b[ 7 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 6 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 5 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 4 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 3 ] ) + Q6_P_mpy_RR( a[ 5 ], b[ 2 ] ) +
                 Q6_P_mpy_RR( a[ 6 ], b[ 1 ] ) + Q6_P_mpy_RR( a[ 7 ], b[ 0 ] );
    int64_t t8 = Q6_P_mpy_RR( a[ 0 ], b[ 8 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 7 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 6 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 5 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 4 ] ) + Q6_P_mpy_RR( a[ 5 ], b[ 3 ] ) +
                 Q6_P_mpy_RR( a[ 6 ], b[ 2 ] ) + Q6_P_mpy_RR( a[ 7 ], b[ 1 ] ) +
                 Q6_P_mpy_RR( a[ 8 ], b[ 0 ] );
    int64_t t9 = Q6_P_mpy_RR( a[ 0 ], b[ 9 ] ) + Q6_P_mpy_RR( a[ 1 ], b[ 8 ] ) +
                 Q6_P_mpy_RR( a[ 2 ], b[ 7 ] ) + Q6_P_mpy_RR( a[ 3 ], b[ 6 ] ) +
                 Q6_P_mpy_RR( a[ 4 ], b[ 5 ] ) + Q6_P_mpy_RR( a[ 5 ], b[ 4 ] ) +
                 Q6_P_mpy_RR( a[ 6 ], b[ 3 ] ) + Q6_P_mpy_RR( a[ 7 ], b[ 2 ] ) +
                 Q6_P_mpy_RR( a[ 8 ], b[ 1 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 0 ] );
    int64_t t10 = Q6_P_mpy_RR( a[ 1 ], b[ 9 ] ) +
                  ...
                  Q6_P_mpy_RR( a[ 6 ], b[ 4 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 3 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 2 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 1 ] );
    int64_t t11 = Q6_P_mpy_RR( a[ 2 ], b[ 9 ] ) +
                  ...
                  Q6_P_mpy_RR( a[ 6 ], b[ 5 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 4 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 3 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 2 ] );
    int64_t t12 = Q6_P_mpy_RR( a[ 3 ], b[ 9 ] ) +
                  Q6_P_mpy_RR( a[ 4 ], b[ 8 ] ) +
                  Q6_P_mpy_RR( a[ 5 ], b[ 7 ] ) +
                  Q6_P_mpy_RR( a[ 6 ], b[ 6 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 5 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 4 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 3 ] );
    int64_t t13 = Q6_P_mpy_RR( a[ 4 ], b[ 9 ] ) +
                  Q6_P_mpy_RR( a[ 5 ], b[ 8 ] ) +
                  Q6_P_mpy_RR( a[ 6 ], b[ 7 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 6 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 5 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 4 ] );
    int64_t t14 = Q6_P_mpy_RR( a[ 5 ], b[ 9 ] ) +
                  Q6_P_mpy_RR( a[ 6 ], b[ 8 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 7 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 6 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 5 ] );
    int64_t t15 = Q6_P_mpy_RR( a[ 6 ], b[ 9 ] ) +
                  Q6_P_mpy_RR( a[ 7 ], b[ 8 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 7 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 6 ] );
    int64_t t16 = Q6_P_mpy_RR( a[ 7 ], b[ 9 ] ) +
                  Q6_P_mpy_RR( a[ 8 ], b[ 8 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 7 ] );
    int64_t t17 = Q6_P_mpy_RR( a[ 8 ], b[ 9 ] ) + Q6_P_mpy_RR( a[ 9 ], b[ 8 ] );
...
static const uint32_t p256_mod_2[ 8 ] = { 0xfffffffdU, 0xffffffffU, 0xffffffffU,
...
static void sp_256_map_10( sp_point * r, const sp_point * p, sp_digit * t )
{
    ...
    sp_256_cond_sub_10( r->x,
                        r->x,
                        p256_mod,
                        0 - ( ( n >= 0 ) ? ( sp_digit ) 1 : ( sp_digit ) 0 ) );
    sp_256_norm_10( r->x );
                                      ...
                                      const sp_digit * b )
{
    ...
    r[ 9] = Q6_R_add_RR(a[9], b[9]);
                #endif
                #if 1
    __asm__ __volatile__( "{ r1 = memw(%[a]+#0)  \n"
                          "  r2 = memw(%[b]+#0) }\n"
                          "{ r3 = memw(%[a]+#4)  \n"
                          "  r19 = add(r1,r2)    \n"
                          "  r4 = memw(%[b]+#4) }\n"
                                ...
                                const sp_digit * m )
{
    ...
    sp_256_norm_10( r );
    sp_256_cond_sub_10( r,
                        r,
                        m,
                        0 - ( ( ( r[ 9 ] >> 22 ) > 0 ) ? ( sp_digit ) 1
                                                       : ( sp_digit ) 0 ) );
    sp_256_norm_10( r );
}
                                      ...
                                      sp_digit * t )
{
    ...
    if( ( sp_256_cmp_equal_10( p->x, q->x ) &
          ...
          ( sp_256_cmp_equal_10( p->y, q->y ) |
            sp_256_cmp_equal_10( p->y, t1 ) ) ) != 0 )
    {
                                 ...
                                 void * heap )
{
    ...
    td = ( sp_point * ) XMALLOC( sizeof( sp_point ) * 3,
                                 heap,
                                 DYNAMIC_TYPE_ECC );
    if( td == NULL )
        err = MEMORY_E;
    tmp = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * 2 * 10 * 5,
                                  heap,
                                  DYNAMIC_TYPE_ECC );
    if( tmp == NULL )
        err = MEMORY_E;
                                      ...
                                      void * heap )
{
    ...
    err = sp_ecc_point_new( heap, rtd, rt );
                    #if defined( WOLFSSL_SP_SMALL ) || \
                        defined( WOLFSSL_SMALL_STACK )
    t = ( sp_point * ) XMALLOC( sizeof( sp_point ) * 16,
                                heap,
                                DYNAMIC_TYPE_ECC );
    if( t == NULL )
        err = MEMORY_E;
                                        ...
                                        void * heap )
{
    ...
    if( err == MP_OKAY )
    {
        err = sp_ecc_point_new( heap, pd, p );
                    ...
                    #if defined( WOLFSSL_SP_SMALL ) || \
                        defined( WOLFSSL_SMALL_STACK )
    t = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * 2 * 10 * 5,
                                heap,
                                DYNAMIC_TYPE_ECC );
    if( t == NULL )
    {
                                 ...
                                 void * heap )
{
    ...
    if( err == MP_OKAY )
    {
        ...
        {
            err = sp_256_ecc_mulmod_stripe_10( r,
                                               g,
                                               cache->table,
                                               k,
                                               map,
...
static int sp_256_ecc_mulmod_base_10( sp_point * r,
                                      ...
                                      void * heap )
{
    return sp_256_ecc_mulmod_stripe_10( r,
                                        &p256_base,
                                        p256_table,
                                        k,
                                        map,
                          ...
                          sp_digit * r )
{
    ...
    ( void ) m;
                    ...
                    #if defined( WOLFSSL_SP_SMALL ) || \
                        defined( WOLFSSL_SMALL_STACK )
    td = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * ( 3 * 10 + 1 ),
                                 NULL,
...
static const uint32_t p256_order_2[ 8 ] = { 0xfc63254fU, 0xf3b9cac2U,
                                            0xa7179e84U, 0xbce6faadU,
                                            0xffffffffU, 0xffffffffU,
                                            0x00000000U, 0xffffffffU };
                    #else
/* The low half of the order-2 of the P256 curve. */
static const uint32_t p256_order_low[ 4 ] = { 0xfc63254fU,
                                              0xf3b9cac2U,
                                              0xa7179e84U,
                                              0xbce6faadU };
                    #endif /* WOLFSSL_SP_SMALL */
                                      ...
                                      sp_digit * td )
{
    ...
    for( i = 254; i >= 0; i-- )
    {
        ...
        if( ( p256_order_2[ i / 32 ] & ( ( sp_int_digit ) 1 << ( i % 32 ) ) ) !=
            0 )
        {
    ...
    for( i = 127; i >= 112; i-- )
    {
        ...
        if( ( ( sp_digit ) p256_order_low[ i / 32 ] &
              ( ( sp_int_digit ) 1 << ( i % 32 ) ) ) != 0 )
        {
                               ...
                               mp_int * rZ )
{
    ...
    if( err == MP_OKAY )
    {
        tmp = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * 2 * 10 * 2,
                                      NULL,
                                      DYNAMIC_TYPE_ECC );
        if( tmp == NULL )
        {
...
int sp_ecc_map_256( mp_int * pX, mp_int * pY, mp_int * pZ )
{
    ...
    if( err == MP_OKAY )
    {
        tmp = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * 2 * 10 * 4,
                                      NULL,
                                      DYNAMIC_TYPE_ECC );
        if( tmp == NULL )
        {
...
static int sp_256_mont_sqrt_10( sp_digit * y )
{
    ...
    int err = MP_OKAY;
                    ...
                    #if defined( WOLFSSL_SP_SMALL ) || \
                        defined( WOLFSSL_SMALL_STACK )
    d = ( sp_digit * ) XMALLOC( sizeof( sp_digit ) * 4 * 10,
                                NULL,
                                DYNAMIC_TYPE_ECC );
    if( d == NULL )
    {
