                      ...
                      const byte keylen )
{
...

    {
        ...
        block = ( byte * ) XMALLOC( BLAKE2B_BLOCKBYTES,
                                    NULL,
                             ...
                             const byte block[ BLAKE2B_BLOCKBYTES ] )
{
    ...
    m = ( word64 * ) XMALLOC( sizeof( word64 ) * 16,
                              NULL,
                              DYNAMIC_TYPE_TMP_BUFFER );
    ...
    v = ( word64 * ) XMALLOC( sizeof( word64 ) * 16,
                              NULL,
                              DYNAMIC_TYPE_TMP_BUFFER );
...
int blake2b_final( blake2b_state * S, byte * out, byte outlen )
{
    ...
    XMEMSET( S->buf + S->buflen,
             0,
             ( wolfssl_word ) ( 2 * BLAKE2B_BLOCKBYTES - S->buflen ) );
    /* Padding */
    if( blake2b_compress( S, S->buf ) < 0 )
        return -1;
...
int main( int argc, char ** argv )
{
    ...
    for( word32 i = 0; i < KAT_LENGTH; ++i )
    {
        ...
        if( blake2b( hash, buf, key, BLAKE2B_OUTBYTES, i, BLAKE2B_KEYBYTES ) <
            0 )
        {
            puts( "error" );
            return -1;
