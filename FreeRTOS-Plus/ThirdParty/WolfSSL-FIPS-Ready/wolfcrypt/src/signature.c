                            ...
                            word32 key_len )
{
    ...
    /* Check arguments */
    if( hash_data == NULL || hash_len == 0 || sig == NULL || sig_len == 0 ||
        key == NULL || key_len == 0 )
    {
        return BAD_FUNC_ARG;
    }
    ...
    switch( sig_type )
    {
        ...
        case WC_SIGNATURE_TYPE_RSA:
        {
            ...
            {
                ret = cc310_RsaSSL_Verify( hash_data,
                                           hash_len,
                                           ( byte * ) sig,
                                           key,
            ...
            #if defined( WOLFSSL_SMALL_STACK ) && !defined( WOLFSSL_NO_MALLOC )
            plain_data = ( byte * ) XMALLOC( plain_len,
                                             NULL,
            ...
            #endif
                    if( ret >= 0 )
                        ret = wc_RsaSSL_VerifyInline( plain_data,
                                                      sig_len,
                                                      &plain_ptr,
                                                      ( RsaKey * ) key );
                } while( ret == WC_PENDING_E );
                        ...
                        word32 key_len )
{
    ...
    hash_data = ( byte * ) XMALLOC( hash_enc_len,
                                    NULL,
                                    DYNAMIC_TYPE_TMP_BUFFER );
    if( hash_data == NULL )
    {
    ...
    if( ret == 0 )
    {
        ...
        if( sig_type == WC_SIGNATURE_TYPE_RSA_W_ENC )
        {
            ...
            ret = SIG_TYPE_E;
    #else
            ret = wc_SignatureDerEncode( hash_type,
                                         hash_data,
                                         hash_len,
                                         &hash_enc_len );
    #endif
        ...
        {
            /* Verify signature using hash */
            ret = wc_SignatureVerifyHash( hash_type,
                                          sig_type,
                                          hash_data,
                                          hash_enc_len,
                                          sig,
...
int wc_SignatureGenerateHash( enum wc_HashType hash_type,
                              ...
                              WC_RNG * rng )
{
    return wc_SignatureGenerateHash_ex( hash_type,
                                        sig_type,
                                        hash_data,
                                        hash_len,
                                        sig,
                                 ...
                                 int verify )
{
    ...
    /* Check arguments */
    if( hash_data == NULL || hash_len == 0 || sig == NULL || sig_len == NULL ||
    ...
    switch( sig_type )
    {
        ...
        #endif
                if( ret >= 0 )
                    ret = wc_ecc_sign_hash( hash_data,
                                            hash_len,
                                            sig,
                                            sig_len,
                                            rng,
        ...
        #ifdef WOLFSSL_CRYPTOCELL
            /* use expected signature size (incoming sig_len could be larger
             * buffer */
            *sig_len = wc_SignatureGetSize( sig_type, key, key_len );
            if( sig_type == WC_SIGNATURE_TYPE_RSA_W_ENC )
            {
                ret = cc310_RsaSSL_Sign( hash_data,
                                         hash_len,
                                         sig,
                                         *sig_len,
                                         key,
...
int wc_SignatureGenerate( enum wc_HashType hash_type,
                          ...
                          WC_RNG * rng )
{
    return wc_SignatureGenerate_ex( hash_type,
                                    sig_type,
                                    data,
                                    data_len,
                                    sig,
                             ...
                             int verify )
{
    ...
    if( ret == 0 )
    {
        ...
        if( ret == 0 )
        {
            ...
            ret = wc_SignatureGenerateHash( hash_type,
                                            sig_type,
                                            hash_data,
                                            hash_enc_len,
                                            sig,
