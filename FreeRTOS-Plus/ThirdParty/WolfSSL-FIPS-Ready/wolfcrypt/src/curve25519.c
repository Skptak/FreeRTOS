                            ...
                            const byte * priv )
{
    ...
    /* check clamping */
    if( ( priv[ 0 ] & ~248 ) || ( priv[ CURVE25519_KEYSIZE - 1 ] & 128 ) )
    {
        return ECC_BAD_ARG_E;
    }
    ...
    #ifdef FREESCALE_LTC_ECC
    {
        ...
        ECPoint wc_pub;
        ret = nxp_ltc_curve25519( &wc_pub,
                                  priv,
                                  basepoint,
                                  kLTC_Weierstrass ); /* input basepoint on
                                                         Weierstrass curve */
        if( ret == 0 )
...
int wc_curve25519_make_key( WC_RNG * rng, int keysize, curve25519_key * key )
{
    ...
    key->k.point[ 0 ] &= 248;
    key->k.point[ CURVE25519_KEYSIZE - 1 ] &= 63; /* same &=127 because |=64
                                                     after */
    key->k.point[ CURVE25519_KEYSIZE - 1 ] |= 64;

    return wc_curve25519_make_pub( ( int ) sizeof key->p.point,
                                   key->p.point,
                                   sizeof key->k.point,
                                   key->k.point );
}
...
int wc_curve25519_shared_secret( curve25519_key * private_key,
                                 ...
                                 word32 * outlen )
{
    return wc_curve25519_shared_secret_ex( private_key,
                                           public_key,
                                           out,
                                           outlen,
                                           EC25519_BIG_ENDIAN );
                                    ...
                                    int endian )
{
    ...
    if( private_key == NULL || public_key == NULL || out == NULL ||
        outlen == NULL || *outlen < CURVE25519_KEYSIZE )
        return BAD_FUNC_ARG;
    ...
    if( public_key->p.point[ CURVE25519_KEYSIZE - 1 ] > 0x7F )
        ...
        #ifdef FREESCALE_LTC_ECC
    ret = nxp_ltc_curve25519(
        &o,
        private_key->k.point,
        &public_key->p,
        kLTC_Curve25519 /* input point P on Curve25519 */ );
        #else
...
int wc_curve25519_export_public( curve25519_key * key,
                                 ...
                                 word32 * outLen )
{
    return wc_curve25519_export_public_ex( key,
                                           out,
                                           outLen,
                                           EC25519_BIG_ENDIAN );
}
...
int wc_curve25519_export_private_raw( curve25519_key * key,
                                      ...
                                      word32 * outLen )
{
    return wc_curve25519_export_private_raw_ex( key,
                                                out,
                                                outLen,
                                                EC25519_BIG_ENDIAN );
}
...
int wc_curve25519_export_key_raw( curve25519_key * key,
                                  ...
                                  word32 * pubSz )
{
    return wc_curve25519_export_key_raw_ex( key,
                                            priv,
                                            privSz,
                                            pub,
                                            pubSz,
...
int wc_curve25519_import_private_raw( const byte * priv,
                                      ...
                                      curve25519_key * key )
{
    return wc_curve25519_import_private_raw_ex( priv,
                                                privSz,
                                                pub,
                                                pubSz,
                                                key,
...
int wc_curve25519_import_private( const byte * priv,
                                  ...
                                  curve25519_key * key )
{
    return wc_curve25519_import_private_ex( priv,
                                            privSz,
                                            key,
                                            EC25519_BIG_ENDIAN );
}
