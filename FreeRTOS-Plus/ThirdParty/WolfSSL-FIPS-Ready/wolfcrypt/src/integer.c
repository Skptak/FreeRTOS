...
int mp_lshd( mp_int * a, int b )
{
    ...
    return MP_OKAY;
            ...
            /* this is a shell function that calls either the normal or
             * Montgomery exptmod functions.  Originally the call to the
             * montgomery code was embedded in the normal function but that
             * wasted a lot of stack space for nothing (since 99% of the time
             * the Montgomery code would be called)
             */
            #if defined( FREESCALE_LTC_TFM )
    ...
    if( G->used == 1 && G->dp[ 0 ] == 2 )
    {
        return mp_exptmod_base_2( X, P, Y );
            ...
            /* modified diminished radix reduction */
                ...
                defined( BN_MP_REDUCE_2K_L_C ) && defined( BN_S_MP_EXPTMOD_C )
...
int mp_invmod_slow( mp_int * a, mp_int * b, mp_int * c )
{
    ...
    /* init temps */
    if( ( res = mp_init_multi( &x, &y, &u, &v, &A, &B ) ) != MP_OKAY )
    {
        return res;
    }
...
int mp_is_bit_set( mp_int * a, mp_digit b )
{
    ...
    return ( int ) ( ( a->dp[ b / DIGIT_BIT ] >> b % DIGIT_BIT ) &
                     ( mp_digit ) 1 );
}
                     ...
                     int redmode )
{
    ...
    if( redmode == 0 )
    {
        ...
        goto LBL_M;
                ...
                /* automatically pick the comba one if available (saves quite a
                   few calls/ifs) */
            #ifdef BN_FAST_MP_MONTGOMERY_REDUCE_C
        if( ( ( P->used * 2 + 1 ) < ( int ) MP_WARRAY ) &&
            P->used < ( 1L << ( ( CHAR_BIT * sizeof( mp_word ) ) -
                                ( 2 * DIGIT_BIT ) ) ) )
        {
    ...
    for( x = 0; x < ( winsize - 1 ); x++ )
    {
        ...
        if( ( err = redux( &M[ ( mp_digit ) ( 1 << ( winsize - 1 ) ) ],
                           P,
                           mp ) ) != MP_OKAY )
        {
            goto LBL_RES;
...
int mp_exptmod_base_2( mp_int * X, mp_int * P, mp_int * Y )
{
                ...
                #endif
    {
        ...
        return MP_VAL;
                ...
                #ifdef WOLFSSL_SMALL_STACK
    res = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                NULL,
...
int mp_montgomery_setup( mp_int * n, mp_digit * rho )
{
    ...
    x *= 2 - b * x; /* here x*a==1 mod 2**16 */
            #endif
            #if defined( MP_64BIT ) || \
                !( defined( MP_8BIT ) || defined( MP_16BIT ) )
    ...
    *rho = ( mp_digit ) ( ( ( ( mp_digit ) 1 << ( ( mp_digit ) DIGIT_BIT ) ) -
                            x ) &
                          MP_MASK );
...
int mp_montgomery_reduce( mp_int * x, mp_int * n, mp_digit rho )
{
    ...
    for( ix = 0; ix < n->used; ix++ )
    {
         ...
         */
        mu = ( mp_digit ) ( ( ( mp_word ) x->dp[ ix ] ) * ( ( mp_word ) rho ) &
                            MP_MASK );
...
int mp_mul_2( mp_int * a, mp_int * b )
{
...

    {
        ...
        for( x = 0; x < a->used; x++ )
        {
            ...
            *tmpb++ = ( mp_digit ) ( ( ( *tmpa++ << ( ( mp_digit ) 1 ) ) | r ) &
                                     MP_MASK );
...
int mp_div_3( mp_int * a, mp_int * c, mp_digit * d )
{
    ...
    /* b = 2**DIGIT_BIT / 3 */
    b = ( mp_digit ) ( ( ( ( mp_word ) 1 ) << ( ( mp_word ) DIGIT_BIT ) ) /
                       ( ( mp_word ) 3 ) );
...
int mp_init_size( mp_int * a, int size )
{
    ...
    /* alloc mem */
    a->dp = OPT_CAST( mp_digit )
        XMALLOC( sizeof( mp_digit ) * size, NULL, DYNAMIC_TYPE_BIGINT );
    if( a->dp == NULL )
    {
        return MP_MEM;
    }
...
int fast_s_mp_mul_digs( mp_int * a, mp_int * b, mp_int * c, int digs )
{
...

    {
        ...
        tmpc = c->dp;
        for( ix = 0; ix < pa; ix++ )
        { /* JRB, +1 could read uninitialized data */
...
int fast_s_mp_mul_high_digs( mp_int * a, mp_int * b, mp_int * c, int digs )
{
    ...
    if( a->dp == NULL )
    { /* JRB, avoid reading uninitialized values */
        return MP_VAL;
    }
    ...
    { /* JRB, have a->dp check at top of function*/
        int tx, ty, iy;
        mp_digit *tmpx, *tmpy;
...

    {
        ...
        tmpc = c->dp + digs;
        for( ix = digs; ix < pa; ix++ )
        { /* TAO, <= could potentially overwrite */
            /* now extract the previous digit [below the carry] */
            *tmpc++ = W[ ix ];
        }
...
int mp_set_int( mp_int * a, unsigned long b )
{
    ...
    return MP_OKAY;
                ...
                !defined( NO_RSA ) || !defined( NO_DSA ) | !defined( NO_DH )
...
int mp_sub_d( mp_int * a, mp_digit b, mp_int * c )
{
    ...
    return MP_OKAY;
                ...
                defined( HAVE_ECC ) || defined( DEBUG_WOLFSSL ) ||        \
                !defined( NO_RSA ) || !defined( NO_DSA ) || !defined( NO_DH )
...
int mp_mod_d( mp_int * a, mp_digit b, mp_digit * c )
{
    return mp_div_d( a, b, NULL, c );
            ...
            #endif /* WOLFSSL_KEY_GEN || HAVE_COMP_KEY || HAVE_ECC || \
                      DEBUG_WOLFSSL */
...
int mp_read_radix( mp_int * a, const char * str, int radix )
{
    ...
    while( *str != '\0' )
    {
         ...
         * [e.g. in hex]
         */
        ch = ( radix <= 36 ) ? ( char ) XTOUPPER( ( unsigned char ) *str )
                             : *str;
        for( y = 0; y < 64; y++ )
        {
            if( ch == mp_s_rmap[ y ] )
...
void mp_dump( const char * desc, mp_int * a, byte verbose )
{
    ...
    buffer = ( char * ) XMALLOC( size * sizeof( mp_digit ) * 2,
                                 NULL,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    if( buffer == NULL )
    {
