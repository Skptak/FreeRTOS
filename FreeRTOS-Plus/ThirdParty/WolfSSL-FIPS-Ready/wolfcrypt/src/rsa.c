...
#include <wolfssl/wolfcrypt/settings.h>
     ...
     * NO_RSA:              Overall control of RSA                      default:
    on (not defined)
     * WC_RSA_BLINDING:     Uses Blinding w/ Private Ops                default:
    off Note: slower by ~20%
     * WOLFSSL_KEY_GEN:     Allows Private Key Generation               default:
    off
     * RSA_LOW_MEM:         NON CRT Private Operations, less memory     default:
    off
     * WC_NO_RSA_OAEP:      Disables RSA OAEP padding                   default:
    on (not defined)
     * WC_RSA_NONBLOCK:     Enables support for RSA non-blocking        default:
    off
     * WC_RSA_NONBLOCK_TIME:Enables support for time based blocking     default:
    off
     *                      time calculation.
    */
     ...
     * FP_MAX_BITS:         With USE_FAST_MATH only                     default:
    4096 If USE_FAST_MATH then use this to override default. Value is key size
    * 2. Example: RSA 3072 = 6144
    */
...
int wc_InitRsaHw( RsaKey * key )
{
    ...
    if( mp_to_unsigned_bin( &( key->e ),
                            ( byte * ) &e + ( MAX_E_SIZE - eSz ) ) != MP_OKAY )
    {
        XFREE( m, key->heap, DYNAMIC_TYPE_KEY );
    ...
    if( XSecure_RsaInitialize( &( key->xRsa ),
                               key->mod,
                               NULL,
...
int wc_CheckRsaKey( RsaKey * key )
{
    ...
    if( ret == 0 )
    {
        switch( mp_count_bits( &key->n ) )
        {
            ...
            default:
                    /* If using only single prcsision math then issue key size
                    ...
                    void * heap )
{
    ...
    if( ( seedSz + 4 ) > sizeof( tmpA ) || ( word32 ) hLen > sizeof( tmpA ) )
    {
        ...
        tmpSz = ( ( seedSz + 4 ) > ( word32 ) hLen ) ? seedSz + 4
                                                     : ( word32 ) hLen;
                        ...
                        void * heap )
{
    ...
    {
        WOLFSSL_MSG(
            "OAEP hash type possibly not supported or lHash to small" );
                    #ifdef WOLFSSL_SMALL_STACK
        XFREE( lHash, heap, DYNAMIC_TYPE_RSA_BUFFER );
    ...
    { /*make sure not writing over end of buffer */
                    #ifdef WOLFSSL_SMALL_STACK
        XFREE( lHash, heap, DYNAMIC_TYPE_RSA_BUFFER );
        XFREE( seed, heap, DYNAMIC_TYPE_RSA_BUFFER );
                    #endif
    ...
    dbMask = ( byte * ) XMALLOC( pkcsBlockLen - hLen - 1,
                                 heap,
                                 DYNAMIC_TYPE_RSA );
    if( dbMask == NULL )
    {
                       ...
                       void * heap )
{
    ...
    byte * m;
    byte * s;
                    #if defined( WOLFSSL_PSS_LONG_SALT ) || \
    ...
    maskLen = pkcsBlockLen - 1 - hLen;
                        ...
                        #if !defined( WOLFSSL_NO_MALLOC ) || \
                            defined( WOLFSSL_STATIC_MEMORY )
    salt = ( byte * ) XMALLOC( RSA_PSS_PAD_SZ + inputLen + saltLen,
                               heap,
                  ...
                  void * heap )
{
    ...
    switch( padType )
    {
        ...
        case WC_RSA_NO_PAD:
             ...
             * the RSA key length */
            if( bits <= 0 ||
                inputLen != ( ( word32 ) bits / WOLFSSL_BIT_SIZE ) )
            {
                          ...
                          void * heap )
{
    ...
    {
        pkcsBlock[ hLen + 1 + idx ] = pkcsBlock[ hLen + 1 + idx ] ^
                                      tmp[ idx + hLen ];
    }
                         ...
                         void * heap )
{
    ...
    if( ( ret = RsaMGF( mgf, pkcsBlock + maskLen, hLen, tmp, maskLen, heap ) ) !=
        0 )
    {
        XFREE( tmp, heap, DYNAMIC_TYPE_RSA_BUFFER );
        return ret;
                    ...
                    void * heap )
{
    ...
    switch( padType )
    {
        ...
        case WC_RSA_OAEP_PAD:
            ...
            ret = RsaUnPad_OAEP( ( byte * ) pkcsBlock,
                                 pkcsBlockLen,
                                 out,
                                 hType,
                                 mgf,
        ...
        case WC_RSA_PSS_PAD:
            ...
            ret = RsaUnPad_PSS( ( byte * ) pkcsBlock,
                                pkcsBlockLen,
                                out,
                                hType,
                                mgf,
        ...
        case WC_RSA_NO_PAD:
             ...
             * the RSA key length */
            if( bits <= 0 ||
                pkcsBlockLen != ( ( word32 ) ( bits + WOLFSSL_BIT_SIZE - 1 ) /
                                  WOLFSSL_BIT_SIZE ) )
            {
                WOLFSSL_MSG( "Bad input size" );
                ret = RSA_PAD_E;
                                   ...
                                   RsaKey * key )
{
    ...
    if( key->nb->exptmod.state == TFM_EXPTMOD_NB_INIT )
    {
        ...
        if( ret == 0 )
        {
            if( mp_read_unsigned_bin( &key->nb->tmp, ( byte * ) in, inLen ) !=
                MP_OKAY )
            {
                ret = MP_READ_E;
            }
    ...
    if( ret == 0 )
    {
        switch( type )
        {
            ...
            case RSA_PRIVATE_ENCRYPT:
                ret = fp_exptmod_nb( &key->nb->exptmod,
                                     &key->nb->tmp,
                                     &key->d,
                                     &key->n,
                                     &key->nb->tmp );
                               ...
                               WC_RNG * rng )
{
    ...
    switch( type )
    {
        ...
        case RSA_PRIVATE_ENCRYPT:
             ...
             * leading to private key operations with Xsecure_RsaDecrypt not
             * being supported */
            ret = RSA_WRONG_TYPE_E;
            #else
        {
        ...
        case RSA_PUBLIC_DECRYPT:
            ...
            #else
            /* starting at Xilinx release 2019 the function XSecure_RsaDecrypt
             * was removed */
            if( XSecure_RsaPublicEncrypt( &( key->xRsa ),
                                          ( u8 * ) in,
                                          inLen,
                                          out ) != XST_SUCCESS )
            {
                WOLFSSL_MSG( "Error happened when calling hardware RSA public "
                             "operation" );
                ret = BAD_STATE_E;
            }
            #endif
                               ...
                               WC_RNG * rng )
{
    ...
    char cbuf[ CMSG_SPACE( 4 ) +
               CMSG_SPACE( sizeof( struct af_alg_iv ) + 1 ) ] = { 0 };
    int ret = 0;
    ...
    if( ( keyBuf = ( byte * )
              XMALLOC( keyLen * 2, key->heap, DYNAMIC_TYPE_KEY ) ) == NULL )
    {
        ERROR_OUT( MEMORY_E );
    }
    ...
    if( ( key->rdFd = wc_Afalg_CreateRead( key->alFd,
                                           WC_TYPE_ASYMKEY,
                                           WC_NAME_RSA ) ) < 0 )
    {
    ...
    if( ( ret = wc_Afalg_SetIv( cmsg,
                                ( byte * ) XILINX_RSA_FLAG,
                               ...
                               WC_RNG * rng )
{
    ...
    if( tmp == NULL )
        return MEMORY_E;
                    #ifdef WC_RSA_BLINDING
    rnd = ( mp_int * ) XMALLOC( sizeof( mp_int ) * 2,
                                key->heap,
                                DYNAMIC_TYPE_RSA );
    if( rnd == NULL )
    {
    ...
    if( ret == 0 &&
        mp_read_unsigned_bin( tmp, ( byte * ) in, inLen ) != MP_OKAY )
        ret = MP_READ_E;
    ...
    if( ret == 0 )
    {
        switch( type )
        {
            ...
            case RSA_PRIVATE_ENCRYPT:
            {
                ...
                /* rnd = rnd^e */
                if( ret == 0 &&
                    mp_exptmod( rnd, &key->e, &key->n, rnd ) != MP_OKAY )
                    ret = MP_EXPTMOD_E;
                ...
                if( ret == 0 && mp_mulmod( tmp, rnd, &key->n, tmp ) != MP_OKAY )
                    ret = MP_MULMOD_E;
                        ...
                        #ifdef RSA_LOW_MEM /* half as much memory but twice as \
                                              slow */
                if( ret == 0 &&
                    mp_exptmod( tmp, &key->d, &key->n, tmp ) != MP_OKAY )
                    ret = MP_EXPTMOD_E;
                        #else
                if( ret == 0 )
                {
                    ...
                    /* tmpa = tmp^dP mod p */
                    if( ret == 0 &&
                        mp_exptmod( tmp, &key->dP, &key->p, tmpa ) != MP_OKAY )
                        ret = MP_EXPTMOD_E;

                    /* tmpb = tmp^dQ mod q */
                    if( ret == 0 &&
                        mp_exptmod( tmp, &key->dQ, &key->q, tmpb ) != MP_OKAY )
                        ret = MP_EXPTMOD_E;
                    ...
                    if( ret == 0 &&
                        mp_mulmod( tmp, &key->u, &key->p, tmp ) != MP_OKAY )
                        ret = MP_MULMOD_E;
                ...
                /* unblind */
                if( ret == 0 &&
                    mp_mulmod( tmp, rndi, &key->n, tmp ) != MP_OKAY )
                    ret = MP_MULMOD_E;
                        #endif /* WC_RSA_BLINDING */
    ...
    switch( type )
    {
        ...
        case RSA_PRIVATE_ENCRYPT:
            ...
            ret = NitroxRsaExptMod( in,
                    ...
                    #ifdef RSA_LOW_MEM
            ret = IntelQaRsaPrivate( &key->asyncDev,
                                     in,
                                     inLen,
                                     &key->d.raw,
                                     &key->n.raw,
                                     ...
                                     outLen );
                    #else
            ret = IntelQaRsaCrtPrivate( &key->asyncDev,
                                        in,
                                        inLen,
                                        &key->p.raw,
                                        &key->q.raw,
        ...
        case RSA_PUBLIC_DECRYPT:
            ...
            ret = NitroxRsaExptMod( in,
                                    ...
                                    key );
            #elif defined( HAVE_INTEL_QA )
            ret = IntelQaRsaPublic( &key->asyncDev,
                                    in,
                                    inLen,
                                    &key->e.raw,
                                    &key->n.raw,
                    ...
                    WC_RNG * rng )
{
    ...
    if( type == RSA_PRIVATE_DECRYPT && key->state == RSA_STATE_DECRYPT_EXPTMOD )
    {
        ...
        mp_int c[ 1 ];
                ...
                #ifdef WOLFSSL_SMALL_STACK
        c = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                  key->heap,
                                  DYNAMIC_TYPE_RSA );
        if( c == NULL )
            ret = MEMORY_E;
    ...
    )
    {
        if( ret == MP_EXPTMOD_E )
        {
            /* This can happen due to incorrectly set FP_MAX_BITS or missing
             * XREALLOC */
                               ...
                               WC_RNG * rng )
{
    ...
    switch( key->state )
    {
        ...
        case RSA_STATE_ENCRYPT_PAD:
                ...
                defined( WC_ASYNC_ENABLE_RSA ) && defined( HAVE_CAVIUM )
                                ...
                                WC_RNG * rng )
{
    ...
    switch( key->state )
    {
        case RSA_STATE_NONE:
                ...
                pad_type != WC_RSA_PSS_PAD )
            {
                    ...
                    pad_value == RSA_BLOCK_TYPE_2 )
                {
                    ...
                    key->data = NULL;
                    return NitroxRsaPrivateDecrypt( in,
                                                    inLen,
                                                    out,
                                                    &key->dataLen,
                                                    key );
            ...
            !defined( WOLFSSL_RSA_VERIFY_INLINE )
        ...
        #if !defined( WOLFSSL_RSA_VERIFY_ONLY )
                ret = ctMaskSelInt( ctMaskLTE( ret, outLen ),
                                    ret,
                                    RSA_BUFFER_E );
                          ...
                          RsaKey * key )
{
    ...
    rng = NULL;
            #endif
    return RsaPrivateDecryptEx( ( byte * ) in,
                                inLen,
                                out,
                                outLen,
                                NULL,
...
{
            #ifndef WOLFSSL_PSS_SALT_LEN_DISCOVER
    return wc_RsaPSS_VerifyInline_ex( in,
                                      inLen,
                                      out,
                                      hash,
                                      mgf,
                                 ...
                                 RsaKey * key )
{
    ...
    verify = wc_RsaPSS_VerifyInline_ex( in,
                                        inLen,
                                        out,
                                        hash,
                                        mgf,
                                        ...
                                        key );
    if( verify > 0 )
        ret = wc_RsaPSS_CheckPadding_ex( digest,
                                         digestLen,
                                         *out,
                                         verify,
                                         hash,
                       ...
                       WC_RNG * rng )
{
    return RsaPublicEncryptEx( in,
        ...
        #if !defined( WOLFSSL_RSA_VERIFY_ONLY ) || \
            !defined( WOLFSSL_SP_MATH ) || defined( WC_RSA_PSS )
int wc_RsaEncryptSize( RsaKey * key )
{
    int ret;
                              ...
                              WC_RNG * rng )
{
    ...
    if( pRaw == NULL || pRawSz == 0 || eRaw == NULL || eRawSz == 0 ||
        isPrime == NULL )
    {
        return BAD_FUNC_ARG;
    }
...
int wc_CheckProbablePrime( const byte * pRaw,
                           ...
                           int * isPrime )
{
    return wc_CheckProbablePrime_ex( pRaw,
                                     pRawSz,
                                     qRaw,
                                     qRawSz,
                                     eRaw,
