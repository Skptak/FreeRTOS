...
static int wc_PKCS7_CreateStream( PKCS7 * pkcs7 )
{
    ...
    pkcs7->stream = ( PKCS7State * ) XMALLOC( sizeof( PKCS7State ),
                                              pkcs7->heap,
                                              DYNAMIC_TYPE_PKCS7 );
                                     ...
                                     word32 * idx )
{
    ...
    if( inSz - rdSz > 0 && pkcs7->stream->length < expected )
    {
        ...
        XMEMCPY( pkcs7->stream->buffer + pkcs7->stream->length,
                 in + rdSz,
                 len );
                                   ...
                                   word32 * idx )
{
    ...
    if( pkcs7->stream->length > 0 )
    {
        ...
        {
            XMEMMOVE( pkcs7->stream->buffer,
                      pkcs7->stream->buffer + *idx,
...
static void wc_PKCS7_ChangeState( PKCS7 * pkcs7, int newState )
{
    ...
    printf( "\tChanging from state [%02d] %s to [%02d] %s\n",
            pkcs7->state,
            wc_PKCS7_GetStateName( pkcs7->state ),
            newState,
            wc_PKCS7_GetStateName( newState ) );
    #endif
    pkcs7->state = newState;
}

    #define MAX_PKCS7_DIGEST_SZ \
        ( MAX_SEQ_SZ + MAX_ALGO_SZ + MAX_OCTET_STR_SZ + WC_MAX_DIGEST_SIZE )
...
static int wc_SetContentType( int pkcs7TypeOID, byte * output, word32 outputSz )
{
    ...
    const byte pkcs7[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x07 };
    const byte data[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x07, 0x01 };
    const byte signedData[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7,
                                0x0D, 0x01, 0x07, 0x02 };
    const byte envelopedData[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7,
                                   0x0D, 0x01, 0x07, 0x03 };
    const byte authEnvelopedData[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D,
                                       0x01, 0x09, 0x10, 0x01, 0x17 };
...
{
    PKCS7 * pkcs7 = ( PKCS7 * ) XMALLOC( sizeof( PKCS7 ),
                                         heap,
                                         DYNAMIC_TYPE_PKCS7 );
    if( pkcs7 )
    {
...
int wc_PKCS7_InitWithCert( PKCS7 * pkcs7, byte * derCert, word32 derCertSz )
{
    ...
    if( derCert != NULL && derCertSz > 0 )
    {
        ...
        dCert = ( DecodedCert * ) XMALLOC( sizeof( DecodedCert ),
                                           pkcs7->heap,
                                           DYNAMIC_TYPE_DCERT );
        if( dCert == NULL )
            return MEMORY_E;
        ...
        cert = ( Pkcs7Cert * ) XMALLOC( sizeof( Pkcs7Cert ),
                                        pkcs7->heap,
                                        DYNAMIC_TYPE_PKCS7 );
...
static int wc_PKCS7_SignerInfoNew( PKCS7 * pkcs7 )
{
    ...
    pkcs7->signerInfo = ( PKCS7SignerInfo * )
        XMALLOC( sizeof( PKCS7SignerInfo ), pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
    if( pkcs7->signerInfo == NULL )
    {
...
static int wc_PKCS7_SignerInfoSetSID( PKCS7 * pkcs7, byte * in, int inSz )
{
    ...
    pkcs7->signerInfo->sid = ( byte * ) XMALLOC( inSz,
                                                 pkcs7->heap,
                                                 DYNAMIC_TYPE_PKCS7 );
    if( pkcs7->signerInfo->sid == NULL )
    {
...
{
    static const byte oid[] = { ASN_OBJECT_ID, 0x09, 0x2A, 0x86, 0x48, 0x86,
...
typedef struct ESD
{
    ...
    byte contentDigest[ WC_MAX_DIGEST_SIZE + 2 ]; /* content only + ASN.1
                                                     heading */
    ...
    word32 signerInfoSetSz, signerInfoSeqSz, signerVersionSz, issuerSnSeqSz,
        issuerNameSz, issuerSnSz, issuerSKIDSz, issuerSKIDSeqSz,
...
{
    FlatAttrib * fb = ( FlatAttrib * ) XMALLOC( sizeof( FlatAttrib ),
                                                heap,
                                  ...
                                  int eaSz )
{
    ...
    for( i = 0; i < eaSz; i++ )
    {
        sz = ea[ i ].valueSeqSz + ea[ i ].oidSz + ea[ i ].valueSetSz +
             ea[ i ].valueSz;
                              ...
                              int eaSz )
{
    ...
    derArr = ( FlatAttrib ** ) XMALLOC( eaSz * sizeof( FlatAttrib * ),
                                        pkcs7->heap,
                                        DYNAMIC_TYPE_TMP_BUFFER );
    if( derArr == NULL )
    {
...
static int wc_PKCS7_RsaSign( PKCS7 * pkcs7, byte * in, word32 inSz, ESD * esd )
{
    ...
    if( pkcs7 == NULL || pkcs7->rng == NULL || in == NULL || esd == NULL )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    privKey = ( RsaKey * ) XMALLOC( sizeof( RsaKey ),
                                    pkcs7->heap,
                                    DYNAMIC_TYPE_TMP_BUFFER );
    if( privKey == NULL )
        return MEMORY_E;
    ...
    if( ret == 0 )
    {
        ...
        {
            idx = 0;
            ret = wc_RsaPrivateKeyDecode( pkcs7->privateKey,
                                          &idx,
                                          privKey,
                                          pkcs7->privateKeySz );
        }
...
static int wc_PKCS7_EcdsaSign( PKCS7 * pkcs7, byte * in, word32 inSz, ESD * esd )
{
    ...
    if( pkcs7 == NULL || pkcs7->rng == NULL || in == NULL || esd == NULL )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    privKey = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                     pkcs7->heap,
                                     DYNAMIC_TYPE_TMP_BUFFER );
    if( privKey == NULL )
        return MEMORY_E;
    ...
    if( ret == 0 )
    {
        ...
        {
            idx = 0;
            ret = wc_EccPrivateKeyDecode( pkcs7->privateKey,
                                          &idx,
                                          privKey,
                                          pkcs7->privateKeySz );
        }
                                           ...
                                           word32 signingTimeSz )
{
    ...
    if( pkcs7->skipDefaultSignedAttribs == 0 )
    {
        ...
        esd->signedAttribsSz += EncodeAttributes( &esd->signedAttribs[ 0 ],
                                                  3,
                                                  cannedAttribs,
                                                  cannedAttribsCount );
    }
    ...
    #ifdef NO_ASN_TIME
        esd->signedAttribsSz += EncodeAttributes( &esd->signedAttribs[ 2 ],
                                                  4,
                                                  pkcs7->signedAttribs,
                                                  pkcs7->signedAttribsSz );
    #else
        esd->signedAttribsSz += EncodeAttributes( &esd->signedAttribs[ 3 ],
                                                  4,
                                                  pkcs7->signedAttribs,
                                                  pkcs7->signedAttribsSz );
    #endif
                                     ...
                                     word32 * digestInfoSz )
{
    ...
    if( flatSignedAttribsSz != 0 )
    {
        ...
        ret = wc_HashUpdate( &esd->hash, esd->hashType, attribSet, attribSetSz );
        if( ret == 0 )
            ret = wc_HashUpdate( &esd->hash,
                                 esd->hashType,
                                 flatSignedAttribs,
                                 flatSignedAttribsSz );
        if( ret == 0 )
            ret = wc_HashFinal( &esd->hash,
                                esd->hashType,
                                              ...
                                              ESD * esd )
{
    ...
    #ifdef WOLFSSL_SMALL_STACK
    digestInfo = ( byte * ) XMALLOC( digestInfoSz,
                                     pkcs7->heap,
                                     DYNAMIC_TYPE_TMP_BUFFER );
    if( digestInfo == NULL )
    {
    ...
    ret = wc_PKCS7_BuildDigestInfo( pkcs7,
                                    flatSignedAttribs,
                                    flatSignedAttribsSz,
                                    esd,
                                    digestInfo,
                               ...
                               word32 * output2Sz )
{
    ...
    const byte contentTypeOid[] = { ASN_OBJECT_ID, 0x09, 0x2a, 0x86, 0x48, 0x86,
                                    0xF7,          0x0d, 0x01, 0x09, 0x03 };

    /* messageDigest OID (1.2.840.113549.1.9.4) */
    const byte messageDigestOid[] = { ASN_OBJECT_ID, 0x09, 0x2a, 0x86,
                                      0x48,          0x86, 0xf7, 0x0d,
                                      0x01,          0x09, 0x04 };

    /* signingTime OID () */
    byte signingTimeOid[] = { ASN_OBJECT_ID, 0x09, 0x2a, 0x86, 0x48, 0x86,
                              0xf7,          0x0d, 0x01, 0x09, 0x05 };
    ...
    if( pkcs7 == NULL || pkcs7->contentSz == 0 || pkcs7->encryptOID == 0 ||
        pkcs7->hashOID == 0 || pkcs7->rng == 0 || output == NULL ||
        outputSz == NULL || *outputSz == 0 || hashSz == 0 || hashBuf == NULL )
    {
        return BAD_FUNC_ARG;
    }
    ...
    if( pkcs7->contentTypeSz == 0 )
    {
        ...
        ret = wc_SetContentType( pkcs7->contentOID,
                                 pkcs7->contentType,
    ...
    ret = wc_SetContentType( SIGNED_DATA,
                             signedDataOid,
    ...
    {
        esd->innerOctetsSz = SetOctetString( pkcs7->contentSz,
                                             esd->innerOctets );
    ...
    {
        /* IssuerAndSerialNumber */
        esd->issuerSnSz = SetSerialNumber( pkcs7->issuerSn,
                                           pkcs7->issuerSnSz,
                                           esd->issuerSn,
                                           MAX_SN_SZ,
                                           MAX_SN_SZ );
    ...
    else if( pkcs7->sidType == CMS_SKID )
    {
        ...
        esd->issuerSKIDSeqSz = SetExplicit( 0,
                                            esd->issuerSKIDSz + KEYID_SIZE,
                                            esd->issuerSKIDSeq );
    ...
    if( pkcs7->sidType != DEGENERATE_SID )
    {
        ...
        esd->signerDigAlgoIdSz = SetAlgoID( pkcs7->hashOID,
                                            esd->signerDigAlgoId,
                                            oidHashType,
                                            0 );
        ...
        esd->digEncAlgoIdSz = SetAlgoID( digEncAlgoId,
                                         esd->digEncAlgoId,
                                         digEncAlgoType,
                                         0 );
        ...
        ret = wc_PKCS7_BuildSignedAttributes( pkcs7,
                                              esd,
                                              pkcs7->contentType,
                                              pkcs7->contentTypeSz,
                                              contentTypeOid,
        ...
        {
            flatSignedAttribs = ( byte * ) XMALLOC( esd->signedAttribsSz,
                                                    pkcs7->heap,
                                                    DYNAMIC_TYPE_PKCS7 );
            ...
            FlattenAttributes( pkcs7,
                               flatSignedAttribs,
                               esd->signedAttribs,
                               esd->signedAttribsCount );
            esd->signedAttribSetSz = SetImplicit( ASN_SET,
                                                  0,
                                                  esd->signedAttribsSz,
                                                  esd->signedAttribSet );
        }
        ...
        ret = wc_PKCS7_SignedDataBuildSignature( pkcs7,
                                                 flatSignedAttribs,
                                                 flatSignedAttribsSz,
                                                 esd );
        if( ret < 0 )
    ...
    if( pkcs7->sidType != DEGENERATE_SID )
    {
        esd->singleDigAlgoIdSz = SetAlgoID( pkcs7->hashOID,
                                            esd->singleDigAlgoId,
                                            oidHashType,
                                            0 );
    }
    ...
    esd->outerContentSz = SetExplicit( 0,
                                       totalSz + total2Sz,
                                       esd->outerContent );
                                       ...
                                       word32 outputSz )
{
    ...
    encrypted = ( byte * ) XMALLOC( encryptedSz,
                                    pkcs7->heap,
                                    DYNAMIC_TYPE_PKCS7 );
    if( encrypted == NULL )
    {
                                        ...
                                        word32 outputSz )
{
    ...
    compressed = ( byte * ) XMALLOC( compressedSz,
                                     pkcs7->heap,
                                     DYNAMIC_TYPE_PKCS7 );
    if( compressed == NULL )
    {
                               ...
                               word32 hashSz )
{
    ...
    if( pkcs7 == NULL || sig == NULL || hash == NULL )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    digest = ( byte * ) XMALLOC( MAX_PKCS7_DIGEST_SZ,
                                 pkcs7->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    if( digest == NULL )
        return MEMORY_E;

    key = ( RsaKey * ) XMALLOC( sizeof( RsaKey ),
                                pkcs7->heap,
                                DYNAMIC_TYPE_TMP_BUFFER );
    if( key == NULL )
    {
    ...
    for( i = 0; i < MAX_PKCS7_CERTS; i++ )
    {
        ...
        if( wc_RsaPublicKeyDecode( dCert->publicKey,
                                   &scratch,
                                   key,
                                   dCert->pubKeySize ) < 0 )
        {
                                 ...
                                 word32 hashSz )
{
    ...
    key = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                 pkcs7->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    if( key == NULL )
    {
    ...
    for( i = 0; i < MAX_PKCS7_CERTS; i++ )
    {
        ...
        if( wc_EccPublicKeyDecode( pkcs7->publicKey,
                                   &idx,
                                   key,
                                   pkcs7->publicKeySz ) < 0 )
        {
                                           ...
                                           word32 hashBufSz )
{
    ...
    /* check arguments */
    if( pkcs7 == NULL || pkcs7Digest == NULL || pkcs7DigestSz == NULL ||
        plainDigest == NULL )
    {
        return BAD_FUNC_ARG;
    }
                                                ...
                                                word32 hashSz )
{
    ...
    if( ( innerAttribSz != digestSz ) ||
        ( XMEMCMP( attrib->value + idx, digestBuf, ( word32 ) digestSz ) !=
          0 ) )
    {
        WOLFSSL_MSG(
            "Content digest does not match messageDigest attrib value" );
    #ifdef WOLFSSL_SMALL_STACK
        XFREE( digest, pkcs7->heap, DYNAMIC_TYPE_TMP_BUFFER );
    #endif
        return SIG_VERIFY_E;
                                               ...
                                               word32 hashSz )
{
    ...
    #ifdef WOLFSSL_SMALL_STACK
    pkcs7Digest = ( byte * ) XMALLOC( pkcs7DigestSz,
                                      pkcs7->heap,
                                      DYNAMIC_TYPE_TMP_BUFFER );
    if( pkcs7Digest == NULL )
        return MEMORY_E;
    ...
    ret = wc_PKCS7_BuildSignedDataDigest( pkcs7,
                                          signedAttrib,
                                          signedAttribSz,
                                          pkcs7Digest,
                                          &pkcs7DigestSz,
    ...
    if( ret == 0 )
    {
        ...
        if( !haveCert )
        {
            ...
            pkcs7->signature = NULL;
            pkcs7->signatureSz = 0;
            pkcs7->signature = ( byte * ) XMALLOC( sigSz,
                                                   pkcs7->heap,
            ...
            pkcs7->plainDigest = NULL;
            pkcs7->plainDigestSz = 0;
            pkcs7->plainDigest = ( byte * ) XMALLOC( plainDigestSz,
                                                     pkcs7->heap,
                                                     DYNAMIC_TYPE_DIGEST );
            ...
            pkcs7->pkcs7Digest = NULL;
            pkcs7->pkcs7DigestSz = 0;
            pkcs7->pkcs7Digest = ( byte * ) XMALLOC( pkcs7DigestSz,
                                                     pkcs7->heap,
                                                     DYNAMIC_TYPE_DIGEST );
    ...
    #ifdef HAVE_ECC
        case ECDSAk:
            ret = wc_PKCS7_EcdsaVerify( pkcs7,
                                        sig,
                                        sigSz,
                                        plainDigest,
                                        plainDigestSz );
...
static int wc_PKCS7_ParseAttribs( PKCS7 * pkcs7, byte * in, int inSz )
{
    ...
    while( idx < ( word32 ) inSz )
    {
        ...
        oidIdx = idx;
        if( GetObjectId( in, &idx, &oid, oidIgnoreType, inSz ) < 0 )
        {
            XFREE( attrib, pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
            return ASN_PARSE_E;
        }
        attrib->oidSz = idx - oidIdx;
        attrib->oid = ( byte * ) XMALLOC( attrib->oidSz,
                                          pkcs7->heap,
                                          DYNAMIC_TYPE_PKCS7 );
        if( attrib->oid == NULL )
        {
        ...
        attrib->value = ( byte * ) XMALLOC( attrib->valueSz,
                                            pkcs7->heap,
                                            DYNAMIC_TYPE_PKCS7 );
...
void wc_PKCS7_AllowDegenerate( PKCS7 * pkcs7, word16 flag )
{
    ...
    {
        if( flag )
        { /* flag of 1 turns on support for degenerate */
            pkcs7->noDegenerate = 0;
        }
        else
        { /* flag of 0 turns off support */
    ...
    if( inSz > 0 && degenerate == 0 )
    {
        ...
        else if( ret == 0 && version == 3 )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        WOLFSSL_MSG(
                            "Unexpected signer info found with version" );
                        ret = ASN_PARSE_E;
                    }
            ...
            tag == ( ASN_CONSTRUCTED | ASN_CONTEXT_SPECIFIC | 0 ) )
        {
            ...
            *signedAttribSz = length;
            ...
            if( ret == 0 &&
                wc_PKCS7_ParseAttribs( pkcs7, *signedAttrib, *signedAttribSz ) <
                    0 )
            {
                                   ...
                                   word32 in2Sz )
{
    ...
    switch( pkcs7->state )
    {
        case WC_PKCS7_START:
    #ifndef NO_PKCS7_STREAM
            if( ( ret = wc_PKCS7_AddDataToStream( pkcs7,
                                                  in,
                                                  inSz,
                                                  MAX_SEQ_SZ + MAX_VERSION_SZ +
                                                      MAX_SEQ_SZ +
                                                      MAX_LENGTH_SZ +
                                                      ASN_TAG_SZ + MAX_OID_SZ +
                                                      MAX_SEQ_SZ,
                                                  &pkiMsg,
                                                  &idx ) ) != 0 )
            {
            ...
            if( ret == 0 &&
                GetSequence_ex( pkiMsg, &idx, &length, totalSz, NO_USER_CHECK ) <
                    0 )
                ret = ASN_PARSE_E;
            ...
            if( ret == 0 &&
                wc_GetContentType( pkiMsg, &idx, &outerContentType, pkiMsgSz ) <
                    0 )
                ret = ASN_PARSE_E;
            ...
            if( ret == 0 &&
                GetLength_ex( pkiMsg, &idx, &length, totalSz, NO_USER_CHECK ) <
                    0 )
                ret = ASN_PARSE_E;
            ...
            /* Get the version */
            if( ret == 0 &&
                GetMyVersion( pkiMsg, &idx, &version, pkiMsgSz ) < 0 )
                ret = ASN_PARSE_E;
    ...
    #ifndef NO_PKCS7_STREAM
            if( ( ret = wc_PKCS7_StreamEndCase( pkcs7, &stateIdx, &idx ) ) !=
                0 )
            {
                break;
            }
        ...
        case WC_PKCS7_VERIFY_STAGE2:
            ...
            if( ret == 0 &&
                GetASNTag( pkiMsg, &localIdx, &tag, pkiMsgSz ) != 0 )
                ret = ASN_PARSE_E;
            ...
            /* save contentType */
            pkcs7->stream->nonce = ( byte * ) XMALLOC( contentTypeSz,
                                                       pkcs7->heap,
                                                       DYNAMIC_TYPE_PKCS7 );
        ...
        case WC_PKCS7_VERIFY_STAGE3:
            ...
            if( ret == 0 && multiPart )
            {
                ...
                if( keepContent )
                {
                    ...
                    pkcs7->contentDynamic = ( byte * )
                        XMALLOC( contentLen, pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
                    if( pkcs7->contentDynamic == NULL )
                        ret = MEMORY_E;
                }
                ...
                while( ret == 0 && localIdx < start + contentLen )
                {
                    ...
                    if( ret == 0 &&
                        GetLength( pkiMsg, &localIdx, &length, totalSz ) < 0 )
                        ret = ASN_PARSE_E;
            ...
            if( ret == 0 && length > 0 )
            {
                ...
                if( pkiMsg2 && pkiMsg2Sz > 0 && hashBuf && hashSz > 0 )
                {
                    ...
                    {
                        WOLFSSL_MSG(
                            "Data signed does not match contentSz provided" );
                        ret = BUFFER_E;
                    }
                }
                else
             ...
             * return the error case. Otherwise with a degenerate it is ok if
             * the content info was omitted */
            if( !degenerate && !detached && ( ret != 0 ) )
            {
                break;
            }
            ...
            if( content != NULL )
            {
                XFREE( pkcs7->stream->content,
                       pkcs7->heap,
                       DYNAMIC_TYPE_PKCS7 );
                pkcs7->stream->content = ( byte * )
                    XMALLOC( contentSz, pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
            ...
            localIdx = idx;
            if( ret == 0 &&
                GetASNTag( pkiMsg2, &localIdx, &tag, pkiMsg2Sz ) == 0 &&
                tag == ( ASN_CONSTRUCTED | ASN_CONTEXT_SPECIFIC | 0 ) )
            {
                idx++;
                if( GetLength_ex( pkiMsg2,
                                  &idx,
                                  &length,
                                  maxIdx,
                                  NO_USER_CHECK ) < 0 )
        ...
        case WC_PKCS7_VERIFY_STAGE4:
            ...
            if( length > 0 && in2Sz == 0 )
            {
                ...
                XFREE( pkcs7->stream->tmpCert,
                       pkcs7->heap,
                       DYNAMIC_TYPE_PKCS7 );
                pkcs7->stream->tmpCert = ( byte * )
                    XMALLOC( length, pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
            ...
            if( length > 0 )
            {
                ...
                if( ret == 0 && MAX_PKCS7_CERTS > 0 )
                {
                    ...
                    for( i = 1;
                         i < MAX_PKCS7_CERTS && certIdx + 1 < pkiMsg2Sz &&
                         certIdx + 1 < ( word32 ) length;
                         i++ )
                    {
                        localIdx = certIdx;

                        if( ret == 0 &&
                            GetASNTag( pkiMsg2, &certIdx, &tag, pkiMsg2Sz ) <
                                0 )
                        {
                            ret = ASN_PARSE_E;
                            break;
                        ...
                        {
                            if( GetLength( pkiMsg2, &certIdx, &sz, pkiMsg2Sz ) <
                                0 )
                            {
                                ret = ASN_PARSE_E;
                                break;
        ...
        case WC_PKCS7_VERIFY_STAGE5:
            ...
            if( ret == 0 &&
                wc_PKCS7_SetContentType( pkcs7, contentType, contentTypeSz ) <
                    0 )
                ret = ASN_PARSE_E;
            ...
            if( ret == 0 &&
                GetSet_ex( pkiMsg2, &idx, &length, maxIdx, NO_USER_CHECK ) < 0 )
                ret = ASN_PARSE_E;
        ...
        case WC_PKCS7_VERIFY_STAGE6:
            ...
            ret = wc_PKCS7_ParseSignerInfo( pkcs7,
                                            pkiMsg2,
                                            pkiMsg2Sz,
                                            &idx,
                                            degenerate,
            ...
            if( ret == 0 && length > 0 && degenerate == 0 )
            {
                ...
                if( ret == 0 )
                {
                    ret = wc_PKCS7_SignedDataVerifySignature( pkcs7,
                                                              sig,
                                                              sigSz,
                                                              signedAttrib,
                                                              signedAttribSz,
...
int wc_PKCS7_VerifySignedData_ex( PKCS7 * pkcs7,
                                  ...
                                  word32 pkiMsgFootSz )
{
    return PKCS7_VerifySignedData( pkcs7,
                                   hashBuf,
                                   hashSz,
                                   pkiMsgHead,
                                   pkiMsgHeadSz,
                             ...
                             int direction )
{
    ...
    switch( keyWrapAlgo )
    {
        ...
        case AES256_WRAP:
            ...
            {
                ret = wc_AesKeyWrap( kek, kekSz, cek, cekSz, out, outSz, NULL );
            }
            else if( direction == AES_DECRYPTION )
            {
                ret = wc_AesKeyUnWrap( kek,
                                       kekSz,
                                       cek,
                                       cekSz,
                                       out,
...
static WC_PKCS7_KARI * wc_PKCS7_KariNew( PKCS7 * pkcs7, byte direction )
{
    ...
    kari->decoded = ( DecodedCert * ) XMALLOC( sizeof( DecodedCert ),
                                               pkcs7->heap,
                                               DYNAMIC_TYPE_PKCS7 );
    ...
    kari->recipKey = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                            pkcs7->heap,
                                            DYNAMIC_TYPE_PKCS7 );
    ...
    kari->senderKey = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                             pkcs7->heap,
                                             DYNAMIC_TYPE_PKCS7 );
                                        ...
                                        word32 keySz )
{
    ...
    if( kari == NULL || kari->decoded == NULL || cert == NULL || certSz == 0 )
        return BAD_FUNC_ARG;
    ...
    {
        idx = 0;
        ret = wc_EccPublicKeyDecode( kari->decoded->publicKey,
                                     &idx,
                                     kari->recipKey,
...
static int wc_PKCS7_KariGenerateEphemeralKey( WC_PKCS7_KARI * kari )
{
    ...
    if( kari == NULL || kari->decoded == NULL || kari->recipKey == NULL ||
        kari->recipKey->dp == NULL )
        return BAD_FUNC_ARG;
    ...
    ret = wc_ecc_export_x963( kari->senderKey,
                              kari->senderKeyExport,
                                            ...
                                            int keyWrapOID )
{
    ...
    /* suppPubInfo */
    suppPubInfoSeqSz = SetImplicit( ASN_SEQUENCE,
                                    2,
    ...
    if( kari->ukmSz > 0 )
    {
        ...
        entityUInfoExplicitSz = SetExplicit( 0,
                                             entityUInfoOctetSz + kari->ukmSz,
    ...
    kari->sharedInfo = ( byte * ) XMALLOC( sharedInfoSz,
                                           kari->heap,
                                           DYNAMIC_TYPE_PKCS7 );
                                     ...
                                     int keyEncOID )
{
    ...
    if( kari == NULL || kari->recipKey == NULL || kari->senderKey == NULL ||
        kari->senderKey->dp == NULL )
        return BAD_FUNC_ARG;
    ...
    if( kari->direction == WC_PKCS7_ENCODE )
    {
        ret = wc_ecc_shared_secret( kari->senderKey,
                                    kari->recipKey,
                                    secret,
                                    &secretSz );
    }
    ...
    {
        ret = wc_ecc_shared_secret( kari->recipKey,
                                    kari->senderKey,
                                    secret,
                                    &secretSz );
    }
                                ...
                                int options )
{
    ...
    keySz = wc_PKCS7_KeyWrap( pkcs7->cek,
                              pkcs7->cekSz,
                              kari->kek,
                              kari->kekSz,
                              encryptedKey,
    ...
    recipKeyIdSeqSz = SetImplicit( ASN_SEQUENCE,
                                   0,
                                   subjKeyIdOctetSz + KEYID_SIZE,
                                   recipKeyIdSeq );
    ...
    if( kari->ukmSz > 0 )
    {
        ...
        ukmExplicitSz = SetExplicit( 1,
                                     ukmOctetSz + kari->ukmSz,
                                     ukmExplicitSeq );
    ...
    keyEncryptAlgoIdSz = SetAlgoID( keyAgreeOID,
                                    keyEncryptAlgoId,
                                    oidCmsKeyAgreeType,
                                    keyWrapAlgSz );
    ...
    origPubKeySeqSz = SetImplicit( ASN_SEQUENCE,
                                   1,
    ...
    origIdOrKeySeqSz = SetImplicit( ASN_SEQUENCE,
                                    0,
                                ...
                                int options )
{
    ...
    serial = ( byte * ) XMALLOC( MAX_SN_SZ,
                                 pkcs7->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    keyAlgArray = ( byte * ) XMALLOC( MAX_SN_SZ,
                                      pkcs7->heap,
                                      DYNAMIC_TYPE_TMP_BUFFER );
    encryptedKey = ( byte * ) XMALLOC( MAX_ENCRYPTED_KEY_SZ,
                                       pkcs7->heap,
    ...
    if( decoded == NULL || serial == NULL || encryptedKey == NULL ||
        keyAlgArray == NULL )
    {
        if( serial )
    ...
    if( sidType == CMS_ISSUER_AND_SERIAL_NUMBER )
    {
        ...
        snSz = SetSerialNumber( decoded->serial,
                                decoded->serialSz,
                                serial,
                                MAX_SN_SZ,
                                MAX_SN_SZ );
    ...
    else if( sidType == CMS_SKID )
    {
        ...
        issuerSKIDSeqSz = SetExplicit( 0,
                                       issuerSKIDSz + KEYID_SIZE,
                                       issuerSKIDSeq );
    }
    else
    {
    ...
    if( keyEncAlgSz == 0 )
    {
        ...
        #ifdef WOLFSSL_SMALL_STACK
    pubKey = ( RsaKey * ) XMALLOC( sizeof( RsaKey ),
                                   pkcs7->heap,
                                   DYNAMIC_TYPE_TMP_BUFFER );
    if( pubKey == NULL )
    {
    ...
    if( wc_RsaPublicKeyDecode( decoded->publicKey,
                               &idx,
                               pubKey,
    ...
    ret = wc_RsaPublicEncrypt( pkcs7->cek,
                               pkcs7->cekSz,
                               encryptedKey,
                               encryptedKeySz,
                               pubKey,
    ...
    if( sidType == CMS_ISSUER_AND_SERIAL_NUMBER )
    {
        recipSeqSz = SetSequence( verSz + issuerSerialSeqSz + issuerSeqSz +
                                      ...
                                      encKeyOctetStrSz + encryptedKeySz,
                                  recipSeq );

        if( recipSeqSz + verSz + issuerSerialSeqSz + issuerSeqSz + snSz +
                keyEncAlgSz + encKeyOctetStrSz + encryptedKeySz >
            MAX_RECIP_SZ )
        {
                                    ...
                                    byte * out )
{
    ...
    {
        return pkcs7->decryptionCb( pkcs7,
                                    encryptOID,
                                    iv,
                                    ivSz,
                                    aad,
                                   ...
                                   word32 outSz )
{
    ...
    if( rng == NULL )
    {
        rnd = ( WC_RNG * ) XMALLOC( sizeof( WC_RNG ),
                                    pkcs7->heap,
                                    DYNAMIC_TYPE_RNG );
        if( rnd == NULL )
            return MEMORY_E;
...
int wc_PKCS7_SetSignerIdentifierType( PKCS7 * pkcs7, int type )
{
    ...
    if( type != CMS_ISSUER_AND_SERIAL_NUMBER && type != CMS_SKID &&
        type != DEGENERATE_SID )
    {
        return BAD_FUNC_ARG;
    }
                      ...
                      word32 blockSz )
{
    ...
    if( in == NULL || inSz == 0 || out == NULL || outSz == 0 )
        return BAD_FUNC_ARG;
                               ...
                               int options )
{
    ...
    recipSeqSz = SetImplicit( ASN_SEQUENCE,
                              4,
                              1 + oriTypeLenSz + oriTypeSz + oriValueSz,
                              recipSeq );
                                     ...
                                     int algID )
{
    ...
    {
        /* encrypt, normal */
        ret = wc_PKCS7_EncryptContent( algID,
                                       ( byte * ) kek,
                                       kekSz,
                                       ( byte * ) iv,
                                       ivSz,
                                       ...
                                       int algID )
{
    ...
    if( pkcs7 == NULL || kek == NULL || in == NULL || out == NULL ||
        iv == NULL )
    {
        return BAD_FUNC_ARG;
    }
    ...
    /* decrypt last block */
    ret = wc_PKCS7_DecryptContent( pkcs7,
                                   algID,
                                   ( byte * ) kek,
                                   kekSz,
                                   tmpIv,
                                ...
                                int options )
{
    ...
    if( pkcs7 == NULL || passwd == NULL || pLen == 0 || salt == NULL ||
        saltSz == 0 )
    {
        return BAD_FUNC_ARG;
    }
    ...
    ret = wc_PKCS7_GenerateKEK_PWRI( pkcs7,
                                     passwd,
                                     pLen,
                                     salt,
                                     saltSz,
    ...
    ret = wc_PKCS7_PwriKek_KeyWrap( pkcs7,
                                    kek,
                                    kekKeySz,
                                    pkcs7->cek,
                                    pkcs7->cekSz,
    ...
    pwriEncAlgoIdSz = SetAlgoID( encryptOID,
                                 pwriEncAlgoId,
                                 oidBlkType,
    ...
    ret = wc_SetContentType( PWRI_KEK_WRAP,
                             keyEncAlgoId,
                             sizeof( keyEncAlgoId ) );
    if( ret <= 0 )
    {
    ...
    kdfAlgoIdSeqSz = SetExplicit( 0,
                                  kdfAlgoIdSz + kdfParamsSeqSz +
                                      kdfSaltOctetStrSz + saltSz +
                                      kdfIterationsSz,
                                  kdfAlgoIdSeq );
                                 ...
                                 int options )
{
    ...
    encryptedKeySz = wc_PKCS7_KeyWrap( pkcs7->cek,
                                       pkcs7->cekSz,
                                       kek,
                                       kekSz,
                                       encryptedKey,
    ...
    kekIdSeqSz = SetSequence( kekIdOctetStrSz + keyIdSz + timeSz +
                                  otherAttSeqSz + otherOIDSz + otherSz,
                              kekIdSeq );
    totalSz += kekIdSeqSz;
                                  ...
                                  word32 outputSz )
{
    ...
    {
        /* outer content type */
        ret = wc_SetContentType( ENVELOPED_DATA,
                                 outerContentType,
    ...
    #ifndef NO_RSA
            case RSAk:
                ret = wc_PKCS7_AddRecipient_KTRI( pkcs7,
                                                  pkcs7->singleCert,
                                                  pkcs7->singleCertSz,
                                                  0 );
                break;
    ...
    plain = ( byte * ) XMALLOC( encryptedOutSz,
                                pkcs7->heap,
                                DYNAMIC_TYPE_PKCS7 );
    if( plain == NULL )
        return MEMORY_E;

    ret = wc_PKCS7_PadData( pkcs7->content,
                            pkcs7->contentSz,
                            plain,
                            encryptedOutSz,
                            blockSz );
    ...
    encryptedContent = ( byte * ) XMALLOC( encryptedOutSz,
                                           pkcs7->heap,
                                           DYNAMIC_TYPE_PKCS7 );
    ...
    contentEncAlgoSz = SetAlgoID( pkcs7->encryptOID,
                                  contentEncAlgo,
                                  oidBlkType,
    ...
    /* encrypt content */
    ret = wc_PKCS7_EncryptContent( pkcs7->encryptOID,
                                   pkcs7->cek,
                                   pkcs7->cekSz,
                                   tmpIv,
                                   blockSz,
    ...
    encContentOctetSz = SetImplicit( ASN_OCTET_STRING,
                                     0,
                                     encryptedOutSz,
                                     encContentOctet );
    ...
    totalSz = verSz + recipSetSz + recipSz + encContentSeqSz + contentTypeSz +
              contentEncAlgoSz + ivOctetStringSz + blockSz + encContentOctetSz +
              encryptedOutSz;
                                 ...
                                 int * recipFound )
{
    ...
    switch( pkcs7->state )
    {
        ...
        case WC_PKCS7_DECRYPT_KTRI_2:
        ...
        #ifdef WOLFSSL_SMALL_STACK
                serialNum = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                                  pkcs7->heap,
                                                  DYNAMIC_TYPE_TMP_BUFFER );
                if( serialNum == NULL )
                    return MEMORY_E;
            ...
            else
            {
                ...
                if( XMEMCMP( pkiMsg + ( *idx ),
                             pkcs7->issuerSubjKeyId,
                             KEYID_SIZE ) == 0 )
                {
                    *recipFound = 1;
                }
        ...
        #ifdef WOLFSSL_SMALL_STACK
            encryptedKey = ( byte * ) XMALLOC( encryptedKeySz,
                                               pkcs7->heap,
                                               DYNAMIC_TYPE_TMP_BUFFER );
            if( encryptedKey == NULL )
                return MEMORY_E;
                                                      ...
                                                      word32 * idx )
{
    ...
    ret = wc_ecc_import_x963_ex( pkiMsg + ( *idx ),
                                 length - 1,
                                 kari->senderKey,
                                 curve_id );
    if( ret != 0 )
    {
        ret = wc_EccPublicKeyDecode( pkiMsg,
                                     idx,
                                     kari->senderKey,
                                     *idx + length - 1 );
        if( ret != 0 )
                                                     ...
                                                     word32 * keyWrapOID )
{
    ...
    if( kari == NULL || pkiMsg == NULL || idx == NULL || keyAgreeOID == NULL ||
        keyWrapOID == NULL )
        return BAD_FUNC_ARG;
                                                  ...
                                                  byte * rid )
{
    ...
    if( XMEMCMP( rid, kari->decoded->issuerHash, KEYID_SIZE ) == 0 )
    {
        *recipFound = 1;
        ...
        #ifdef WOLFSSL_SMALL_STACK
    serial = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                   kari->heap,
                                   DYNAMIC_TYPE_TMP_BUFFER );
    if( serial == NULL )
        return MEMORY_E;

    recipSerial = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                        kari->heap,
                                        DYNAMIC_TYPE_TMP_BUFFER );
    if( recipSerial == NULL )
    {
    ...
    ret = mp_read_unsigned_bin( recipSerial,
                                kari->decoded->serial,
                                                   ...
                                                   byte * rid )
{
    ...
    if( tag == ( ASN_CONSTRUCTED | ASN_CONTEXT_SPECIFIC | 0 ) )
    {
        ...
        ret = wc_PKCS7_KariGetSubjectKeyIdentifier( kari,
                                                    pkiMsg,
                                                    pkiMsgSz,
                                                    idx,
                                                    recipFound,
    ...
    else
    {
        ...
        ret = wc_PKCS7_KariGetIssuerAndSerialNumber( kari,
                                                     pkiMsg,
                                                     pkiMsgSz,
                                                     idx,
                                                     recipFound,
    ...
    switch( pkcs7->state )
    {
        case WC_PKCS7_DECRYPT_PWRI:
            ...
            *idx += saltSz;
            ...
            /* get pwriEncAlgoId */
            if( GetAlgoId( pkiMsg, idx, &pwriEncAlgoId, oidBlkType, pkiMsgSz ) <
                0 )
            {
                XFREE( salt, pkcs7->heap, DYNAMIC_TYPE_PKCS7 );
                return ASN_PARSE_E;
            ...
            *idx += length;
            ...
            ret = wc_PKCS7_PwriKek_KeyUnWrap( pkcs7,
                                              kek,
                                              kekKeySz,
                                              pkiMsg + ( *idx ),
                                              length,
    ...
    switch( pkcs7->state )
    {
        case WC_PKCS7_DECRYPT_KEKRI:
            ...
            *idx += keyIdSz;
            ...
            if( ( *idx < kekIdSz ) &&
                GetASNTag( pkiMsg, &localIdx, &tag, pkiMsgSz ) == 0 &&
                tag == ASN_GENERALIZED_TIME )
            {
                if( wc_GetDateInfo( pkiMsg + *idx,
                                    pkiMsgSz,
                                    &datePtr,
                                    &dateFormat,
                                    &dateLen ) != 0 )
            ...
            {
                keySz = wc_PKCS7_KeyWrap( pkiMsg + *idx,
                                          length,
                                          pkcs7->privateKey,
                                          pkcs7->privateKeySz,
                                          decryptedKey,
                                 ...
                                 int * recipFound )
{
    ...
    switch( pkcs7->state )
    {
        case WC_PKCS7_DECRYPT_KARI:
        {
            ...
            /* remove OriginatorIdentifierOrKey */
            ret = wc_PKCS7_KariGetOriginatorIdentifierOrKey( kari,
                                                             pkiMsg,
                                                             pkiMsgSz,
                                                             idx );
            if( ret != 0 )
            ...
            ret = wc_PKCS7_KariGetUserKeyingMaterial( kari,
                                                      pkiMsg,
                                                      pkiMsgSz,
                                                      idx );
            if( ret != 0 )
            ...
            /* remove KeyEncryptionAlgorithmIdentifier */
            ret = wc_PKCS7_KariGetKeyEncryptionAlgorithmId( kari,
                                                            pkiMsg,
                                                            pkiMsgSz,
                                                            idx,
                                                            &keyAgreeOID,
            ...
            if( pkcs7->wrapCEKCb )
            {
                ...
                tmpKeyDer = ( byte * ) XMALLOC( tmpKeySz,
                                                pkcs7->heap,
                                                DYNAMIC_TYPE_TMP_BUFFER );
                if( tmpKeyDer == NULL )
                {
                ...
                ret = wc_EccPublicKeyToDer( kari->senderKey,
                                            tmpKeyDer,
                                            tmpKeySz,
                                            1 );
                if( ret < 0 )
            ...
            else
            {
                ...
                /* decrypt CEK with KEK */
                keySz = wc_PKCS7_KeyWrap( encryptedKey,
                                          encryptedKeySz,
                                          kari->kek,
                                          kari->kekSz,
                                          decryptedKey,
                                           ...
                                           int * recipFound )
{
    ...
    if( pkcs7 == NULL || pkiMsg == NULL || idx == NULL ||
        decryptedKey == NULL || decryptedKeySz == NULL || recipFound == NULL )
    {
        return BAD_FUNC_ARG;
    }
    ...
    switch( pkcs7->state )
    {
        ...
        case WC_PKCS7_DECRYPT_KTRI_3:
    #ifndef NO_RSA
            ret = wc_PKCS7_DecryptKtri( pkcs7,
                                        in,
                                        inSz,
                                        idx,
                                        decryptedKey,
        ...
        case WC_PKCS7_DECRYPT_KARI:
            ret = wc_PKCS7_DecryptKari( pkcs7,
                                        in,
                                        inSz,
                                        idx,
                                        decryptedKey,
        ...
        case WC_PKCS7_DECRYPT_KEKRI:
            ret = wc_PKCS7_DecryptKekri( pkcs7,
                                         in,
                                         inSz,
                                         idx,
                                         decryptedKey,
        ...
        case WC_PKCS7_DECRYPT_PWRI:
    #if !defined( NO_PWDBASED ) && !defined( NO_SHA )
            ret = wc_PKCS7_DecryptPwri( pkcs7,
                                        in,
                                        inSz,
                                        idx,
                                        decryptedKey,
            ...
            *idx = savedIdx;
            ...
            if( tag == ( ASN_CONSTRUCTED | ASN_CONTEXT_SPECIFIC | 1 ) )
            {
                ...
                if( version != 3 )
                    ...
                    /* found kari */
    #ifndef NO_PKCS7_STREAM
                if( ( ret = wc_PKCS7_StreamEndCase( pkcs7, &tmpIdx, idx ) ) !=
                    0 )
                {
                    break;
                }
                                             ...
                                             int type )
{
    ...
    switch( pkcs7->state )
    {
        ...
        case WC_PKCS7_INFOSET_STAGE1:
            ...
            {
                if( ret == 0 &&
                    wc_GetContentType( pkiMsg, idx, &contentType, pkiMsgSz ) <
                        0 )
                    ret = ASN_PARSE_E;
                ...
                if( ret == 0 )
                {
                    ...
                    {
                        WOLFSSL_MSG(
                            "PKCS#7 input not of type AuthEnvelopedData" );
                        ret = PKCS7_OID_E;
                    }
                }
        ...
        case WC_PKCS7_INFOSET_STAGE2:
            ...
            {
                if( ret == 0 &&
                    GetSequence( pkiMsg, idx, &length, pkiMsgSz ) < 0 )
                    ret = ASN_PARSE_E;
            }
                                              ...
                                              word32 outputSz )
{
    ...
    if( pkiMsg == NULL || pkiMsgSz == 0 || output == NULL || outputSz == 0 )
        return BAD_FUNC_ARG;
    ...
    switch( pkcs7->state )
    {
        ...
        case WC_PKCS7_INFOSET_STAGE2:
        case WC_PKCS7_INFOSET_END:
            ret = wc_PKCS7_ParseToRecipientInfoSet( pkcs7,
                                                    pkiMsg,
                                                    pkiMsgSz,
                                                    &idx,
                                                    ENVELOPED_DATA );
            ...
            decryptedKey = ( byte * ) XMALLOC( MAX_ENCRYPTED_KEY_SZ,
                                               pkcs7->heap,
                                               DYNAMIC_TYPE_PKCS7 );
        ...
        case WC_PKCS7_DECRYPT_ORI:
            ...
            ret = wc_PKCS7_DecryptRecipientInfos( pkcs7,
                                                  in,
                                                  inSz,
                                                  &idx,
                                                  decryptedKey,
        ...
        case WC_PKCS7_ENV_3:
            ...
            if( ret == 0 &&
                wc_GetContentType( pkiMsg, &idx, &contentType, pkiMsgSz ) < 0 )
            {
                ret = ASN_PARSE_E;
            }

            if( ret == 0 &&
                GetAlgoId( pkiMsg, &idx, &encOID, oidBlkType, pkiMsgSz ) < 0 )
            {
                ret = ASN_PARSE_E;
            }
        ...
        case WC_PKCS7_ENV_4:
            ...
            if( ret == 0 &&
                GetLength( pkiMsg, &idx, &encryptedContentTotalSz, pkiMsgSz ) <=
                    0 )
            {
                ret = ASN_PARSE_E;
            }
        ...
        case WC_PKCS7_ENV_5:
            ...
            if( explicitOctet )
            {
                ...
                while( idx < ( localIdx + encryptedContentTotalSz ) )
                {
                    ...
                    if( ret == 0 )
                    {
                        ret = PKCS7_CacheEncryptedContent( pkcs7,
                                                           &pkiMsg[ idx ],
                                                           encryptedContentSz );
                    }
                                      ...
                                      word32 outputSz )
{
    ...
    /* outer content type */
    ret = wc_SetContentType( AUTH_ENVELOPED_DATA,
                             outerContentType,
                             sizeof( outerContentType ) );
    if( ret < 0 )
        return ret;
    ...
    {
        flatAuthAttribs = ( byte * ) XMALLOC( authAttribsSz,
                                              pkcs7->heap,
                                              DYNAMIC_TYPE_PKCS7 );
        ...
        authAttribsSetSz = SetImplicit( ASN_SET,
                                        1,
                                        authAttribsSz,
                                        authAttribSet );
    ...
    if( pkcs7->unauthAttribsSz > 0 )
    {
        unauthAttribsSz = EncodeAttributes( unauthAttribs + unauthAttribsCount,
                                            MAX_UNAUTH_ATTRIBS_SZ -
                                                unauthAttribsCount,
                                            pkcs7->unauthAttribs,
                                            pkcs7->unauthAttribsSz );
        unauthAttribsCount = pkcs7->unauthAttribsSz;

        flatUnauthAttribs = ( byte * ) XMALLOC( unauthAttribsSz,
                                                pkcs7->heap,
                                                DYNAMIC_TYPE_PKCS7 );
        ...
        FlattenAttributes( pkcs7,
                           ...
                           unauthAttribs,
                           unauthAttribsCount );
        unauthAttribsSetSz = SetImplicit( ASN_SET,
                                          2,
                                          unauthAttribsSz,
                                          unauthAttribSet );
    }
    ...
    #else
    WOLFSSL_MSG(
        "AuthEnvelopedData requires AES-GCM or AES-CCM to be enabled" );
    ( void ) pkcs7;
    ( void ) output;
    ( void ) outputSz;
                                                  ...
                                                  word32 outputSz )
{
    ...
    switch( pkcs7->state )
    {
        ...
        case WC_PKCS7_AUTHENV_4:
            ...
            if( ret == 0 &&
                GetLength( pkiMsg, &idx, &encryptedContentSz, pkiMsgSz ) <= 0 )
            {
                ret = ASN_PARSE_E;
            }
            ...
            if( explicitOctet )
            {
                ...
                if( ret == 0 &&
                    GetLength( pkiMsg, &idx, &encryptedContentSz, pkiMsgSz ) <=
                        0 )
                {
                    ret = ASN_PARSE_E;
                }
            ...
            if( nonceSz > 0 )
            {
                ...
                pkcs7->stream->nonce = ( byte * ) XMALLOC( nonceSz,
                                                           pkcs7->heap,
                                                           DYNAMIC_TYPE_PKCS7 );
                if( pkcs7->stream->nonce == NULL )
                {
        ...
        case WC_PKCS7_AUTHENV_5:
            ...
            encryptedContent = ( byte * ) XMALLOC( encryptedContentSz,
                                                   pkcs7->heap,
                                                   DYNAMIC_TYPE_PKCS7 );
                ...
                tag == ( ASN_CONSTRUCTED | ASN_CONTEXT_SPECIFIC | 1 ) )
            {
                ...
                if( encodedAttribSz > 0 )
                {
                    ...
                    pkcs7->stream->aad = ( byte * )
                        XMALLOC( encodedAttribSz,
                                 pkcs7->heap,
                                 DYNAMIC_TYPE_PKCS7 );
        ...
        case WC_PKCS7_AUTHENV_ATRB:
            ...
            if( ret == 0 &&
                wc_PKCS7_ParseAttribs( pkcs7, authAttrib, authAttribSz ) < 0 )
            {
        ...
        case WC_PKCS7_AUTHENV_ATRBEND:
            ...
            if( ret == 0 &&
                GetLength( pkiMsg, &idx, &authTagSz, pkiMsgSz ) < 0 )
            {
                ret = ASN_PARSE_E;
            }
                                  ...
                                  word32 outputSz )
{
    ...
    {
        /* outer content type */
        ret = wc_SetContentType( ENCRYPTED_DATA,
                                 outerContentType,
                                 sizeof( outerContentType ) );
        if( ret < 0 )
            return ret;
    ...
    if( pkcs7->unprotectedAttribsSz != 0 )
    {
        ...
        attribs = ( EncodedAttrib * ) XMALLOC( sizeof( EncodedAttrib ) *
        ...
        attribsSz = EncodeAttributes( attribs,
                                      pkcs7->unprotectedAttribsSz,
                                      pkcs7->unprotectedAttribs,
                                      pkcs7->unprotectedAttribsSz );

        flatAttribs = ( byte * ) XMALLOC( attribsSz,
                                          pkcs7->heap,
                                          DYNAMIC_TYPE_PKCS7 );
        if( flatAttribs == NULL )
        {
                                                 ...
                                                 word32 * inOutIdx )
{
    ...
    if( pkcs7 == NULL || pkiMsg == NULL || pkiMsgSz == 0 || inOutIdx == NULL )
        return BAD_FUNC_ARG;
                                  ...
                                  word32 outputSz )
{
    ...
    switch( pkcs7->state )
    {
        case WC_PKCS7_START:
            ...
            if( pkcs7->version != 3 )
            { /* ContentInfo not in firmware bundles */
        ...
        case WC_PKCS7_STAGE2:
            ...
            if( pkcs7->version != 3 )
            {
                ...
                if( ret == 0 &&
                    GetLength( pkiMsg, &idx, &length, pkiMsgSz ) < 0 )
                    ret = ASN_PARSE_E;
        ...
        case WC_PKCS7_STAGE3:
            ...
            if( ret == 0 && ( ret = GetAlgoId( pkiMsg,
                ...
                ret = ASN_PARSE_E;
            if( ret == 0 &&
                ( expBlockSz = wc_PKCS7_GetOIDBlockSize( encOID ) ) < 0 )
                ret = expBlockSz;
        ...
        case WC_PKCS7_STAGE6:
            ...
            encOID = 0;
        #endif
            if( ret == 0 && ( encryptedContent = ( byte * )
                                  XMALLOC( encryptedContentSz,
                                           pkcs7->heap,
            ...
            if( ret == 0 )
            {
        ...
        #endif
                {
                    ...
                    ret = wc_PKCS7_DecodeUnprotectedAttributes( pkcs7,
                                                                pkiMsg,
                                                                pkiMsgSz,
                                                                &idx );
                    if( ret != 0 )
        ...
        default:
            WOLFSSL_MSG(
                "Error in unknown PKCS#7 Decode Encrypted Data state" );
            return BAD_STATE_E;
    }
                                   ...
                                   word32 outputSz )
{
    ...
    compressedSz = ( pkcs7->contentSz +
                     ( word32 ) ( pkcs7->contentSz * 0.001 ) + 12 );
    compressed = ( byte * ) XMALLOC( compressedSz,
                                     pkcs7->heap,
                                     DYNAMIC_TYPE_PKCS7 );
    if( compressed == NULL )
    ...
    {
        ret = wc_SetContentType( COMPRESSED_DATA,
                                 contentInfoTypeOid,
                                   ...
                                   word32 outputSz )
{
    ...
    if( pkcs7 == NULL || pkiMsg == NULL || pkiMsgSz == 0 || output == NULL ||
        outputSz == 0 )
    {
        return BAD_FUNC_ARG;
    }
    ...
    decompressed = ( byte * ) XMALLOC( length,
                                       pkcs7->heap,
                                       DYNAMIC_TYPE_PKCS7 );
