                   ...
                   int dir )
{
    ...
    for( i = IDEA_ROUNDS; i < IDEA_SK_NUM; i++ )
    {
        ...
        t |= ( word32 ) idea->skey[ ( ( i + 2 ) & 7 ) < 2 ? i - 14 : i - 6 ] >>
             7;
    ...
    if( dir == IDEA_DECRYPTION )
    {
        ...
        idea->skey[ 6 * IDEA_ROUNDS + 1 ] = ( IDEA_2EXP16 - enckey[ idx++ ] ) &
                                            IDEA_MASK;
        idea->skey[ 6 * IDEA_ROUNDS + 2 ] = ( IDEA_2EXP16 - enckey[ idx++ ] ) &
                                            IDEA_MASK;
        ...
        for( i = 6 * ( IDEA_ROUNDS - 1 ); i >= 0; i -= 6 )
        {
            ...
            if( i )
            {
                idea->skey[ i + 2 ] = ( IDEA_2EXP16 - enckey[ idx++ ] ) &
                                      IDEA_MASK;
                idea->skey[ i + 1 ] = ( IDEA_2EXP16 - enckey[ idx++ ] ) &
                                      IDEA_MASK;
            }
            else
            {
...
int wc_IdeaCipher( Idea * idea, byte * out, const byte * in )
{
    ...
    for( i = 0; i < IDEA_ROUNDS; i++ )
    {
        ...
        x[ 1 ] = ( ( word32 ) x[ 1 ] + ( word32 ) idea->skey[ skey_idx++ ] ) &
                 IDEA_MASK;
        x[ 2 ] = ( ( word32 ) x[ 2 ] + ( word32 ) idea->skey[ skey_idx++ ] ) &
                 IDEA_MASK;
