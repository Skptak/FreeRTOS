...
#ifdef WOLFSSL_SCE
        ret = ( int ) WOLFSSL_SCE_GSCE_HANDLE.p_api
                  ->open( WOLFSSL_SCE_GSCE_HANDLE.p_ctrl,
...
{
    wolfSSL_Mutex * m = ( wolfSSL_Mutex * ) XMALLOC( sizeof( wolfSSL_Mutex ),
                                                     NULL,
                                                     DYNAMIC_TYPE_MUTEX );
    if( m != NULL )
    {
...
int wc_InitMutex( wolfSSL_Mutex * m )
{
    mutexStatus mutStat;
    /*
    The empty string "" denotes an anonymous mutex, so objects do not cause name
    collisions. `protectWolfSSLTemp` in an XML configuration element template
    describing a mutex.
    */
    if( m )
    {
...
static const osMutexDef_t * CMSIS_mutex[] = {
    osMutex( wolfSSL_mt0 ), osMutex( wolfSSL_mt1 ), osMutex( wolfSSL_mt2 ),
    osMutex( wolfSSL_mt3 ), osMutex( wolfSSL_mt4 ), osMutex( wolfSSL_mt5 ),
    osMutex( wolfSSL_mt6 ), osMutex( wolfSSL_mt7 ), osMutex( wolfSSL_mt8 ),
    osMutex( wolfSSL_mt9 )
};
...
int wc_InitMutex( wolfSSL_Mutex * m )
{
    static const osMutexAttr_t attr = { "wolfSSL_mutex",
                                        osMutexRecursive,
                                        NULL,
                                        0 };
...
int wc_InitMutex( wolfSSL_Mutex * m )
{
    ...
    *m = CreateRtSemaphore( 1,               /* initial unit count */
                            1,               /* maximum unit count */
                            PRIORITY_QUEUING /* creation flags: FIFO_QUEUING or
                                                PRIORITY_QUEUING */
    );
    if( *m == BAD_RTHANDLE )
    {
...
int wc_FreeMutex( wolfSSL_Mutex * m )
{
    ...
    del = DeleteRtSemaphore( *m /* handle for RT semaphore */
    );
    if( del != TRUE )
        ret = BAD_MUTEX_E;
...
int wc_LockMutex( wolfSSL_Mutex * m )
{
    ...
    lck = WaitForRtSemaphore( *m,          /* handle for RT semaphore */
                              1,           /* number of units to wait for */
                              WAIT_FOREVER /* number of milliseconds to wait for
                                              units */
    );
    if( lck == WAIT_FAILED )
    {
...
int wc_UnLockMutex( wolfSSL_Mutex * m )
{
    ...
    rel = ReleaseRtSemaphore( *m, /* handle for RT semaphore */
...
int wc_InitMutex( wolfSSL_Mutex * m )
{
    ...
    if( NU_Create_Semaphore( m, "WOLFSSL_MTX", 1, NU_PRIORITY ) == NU_SUCCESS )
    {
        return 0;
    }
...
int wc_InitMutex( wolfSSL_Mutex * m )
{
    ...
    osRes = m2mb_os_mtx_setAttrItem( &mtxAttrHandle,
                                     CMDS_ARGS( M2MB_OS_MTX_SEL_CMD_CREATE_ATTR,
                                                NULL,
