...
int wc_SrpInit( Srp * srp, SrpType type, SrpSide side )
{
    ...
    if( ( r = mp_init_multi( &srp->N, &srp->g, &srp->auth, &srp->priv, 0, 0 ) ) !=
        0 )
        return r;
...
int wc_SrpGetPublic( Srp * srp, byte * pub, word32 * size )
{
    ...
    else
    {
        ...
        {
            if( !r )
                r = mp_read_unsigned_bin( &i,
                                          srp->k,
                                          SrpHashSize( srp->type ) );
            if( !r )
                      ...
                      word32 serverPubKeySz )
{
    ...
    if( !srp || !clientPubKey || clientPubKeySz == 0 || !serverPubKey ||
        serverPubKeySz == 0 )
        return BAD_FUNC_ARG;
    ...
    if( ( secret = ( byte * )
              XMALLOC( secretSz, srp->heap, DYNAMIC_TYPE_SRP ) ) == NULL )
        return MEMORY_E;
...
int wc_SrpGetProof( Srp * srp, byte * proof, word32 * size )
{
    ...
    if( ( r = SrpHashFinal( srp->side == SRP_CLIENT_SIDE ? &srp->client_proof
                                                         : &srp->server_proof,
                            proof ) ) != 0 )
        return r;
