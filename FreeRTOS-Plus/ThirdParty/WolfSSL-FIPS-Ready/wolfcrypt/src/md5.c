...
int wc_Md5Update( wc_Md5 * md5, const byte * data, word32 len )
{
    ...
    if( ret == 0 )
    {
        ret = wc_Stm32_Hash_Update( &md5->stmCtx,
...
int wc_Md5Final( wc_Md5 * md5, byte * hash )
{
    ...
    if( ret == 0 )
    {
        ret = wc_Stm32_Hash_Final( &md5->stmCtx,
                                   HASH_AlgoSelection_MD5,
                                   hash,
                                   WC_MD5_DIGEST_SIZE );
        wolfSSL_CryptHwMutexUnLock();
...
static int Transform_Len( wc_Md5 * md5, const byte * data, word32 len )
{
    ...
    if( ret == 0 )
    {
        ...
        {
            #ifdef FREESCALE_MMCAU_CLASSIC_SHA
            cau_md5_hash_n( ( byte * ) data,
                            len / WC_MD5_BLOCK_SIZE,
                            ( unsigned char * ) md5->digest );
            #else
            MMCAU_MD5_HashN( ( byte * ) data,
                             len / WC_MD5_BLOCK_SIZE,
                             ( uint32_t * ) md5->digest );
            #endif
        }
...
int wc_InitMd5_ex( wc_Md5 * md5, void * heap, int devId )
{
    ...
    ret = wolfAsync_DevCtxInit( &md5->asyncDev,
                                WOLFSSL_ASYNC_MARKER_MD5,
                                md5->heap,
                                devId );
            #else
    ( void ) devId;
...
int wc_Md5Update( wc_Md5 * md5, const byte * data, word32 len )
{
    ...
    while( len >= WC_MD5_BLOCK_SIZE )
    {
        word32 * local32 = md5->buffer;
                    /* optimization to avoid memcpy if data pointer is properly
                     * aligned */
...
int wc_Md5Final( wc_Md5 * md5, byte * hash )
{
    ...
    XMEMCPY( &local[ WC_MD5_PAD_SIZE + sizeof( word32 ) ],
             &md5->hiLen,
             sizeof( word32 ) );
