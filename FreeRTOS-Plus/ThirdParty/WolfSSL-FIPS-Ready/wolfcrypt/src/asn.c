...
objects. ASN.1 is a standard interface description language for defining data
...
{
    items->len[ items->idx ]
        .headerLen += SetLength( items->len[ items->idx ].len, NULL );
}
    ...
    for( i = 0; i < berSz; )
    {
        ...
        if( indef )
        {
                ...
                tag != ( ASN_SET | ASN_CONSTRUCTED ) )
            {
                ...
                {
                    ( void )
                        GetBerHeader( ber, &i, berSz, &tag, &length, &indef );
                    if( tag == ASN_EOC )
                    {
                        break;
                    }
...
static const byte wrapPwriKekOid[] = {
    ...
    /* cmsKeyAgreeType */
    #ifndef NO_SHA
static const byte dhSinglePass_stdDH_sha1kdf_Oid[] = { 43, 129, 5, 16, 134,
                                                       72, 63,  0, 2 };
    #endif
    #ifdef WOLFSSL_SHA224
                       ...
                       word32 oidSz )
{
    ...
    {
        if( *outSz <
            ( keySz + MAX_SEQ_SZ + MAX_VERSION_SZ + MAX_ALGO_SZ +
...
int wc_CheckPrivateKey( byte * key, word32 keySz, DecodedCert * der )
{
    ...
    if( der->keyOID == RSAk )
    {
        ...
        if( ( ret = wc_RsaPrivateKeyDecode( key, &keyIdx, a, keySz ) ) == 0 )
        {
            ...
            if( ( ret = wc_RsaPublicKeyDecode( der->publicKey,
                                               &keyIdx,
                                               b,
                                               der->pubKeySize ) ) == 0 )
            {
        ...
        if( der->keyOID == ECDSAk )
    {
        ...
        word32 keyIdx = 0;
            ...
            #ifdef WOLFSSL_SMALL_STACK
        key_pair = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                          NULL,
                                          DYNAMIC_TYPE_ECC );
        if( key_pair == NULL )
            return MEMORY_E;
        privDer = ( byte * ) XMALLOC( MAX_ECC_BYTES,
                                      NULL,
        ...
        if( ( ret = wc_EccPrivateKeyDecode( key, &keyIdx, key_pair, keySz ) ) ==
            0 )
        {
            WOLFSSL_MSG( "Checking ECC key pair" );

            if( ( ret = wc_ecc_export_private_only( key_pair,
                                                    privDer,
                                                    &privSz ) ) == 0 )
            {
        ...
        if( der->keyOID == ED25519k )
    {
        ...
        word32 keyIdx = 0;
            ...
            #ifdef WOLFSSL_SMALL_STACK
        key_pair = ( ed25519_key * ) XMALLOC( sizeof( ed25519_key ),
                                              NULL,
        ...
        if( ( ret = wc_Ed25519PrivateKeyDecode( key,
                                                &keyIdx,
                                                key_pair,
                                                keySz ) ) == 0 )
        {
        ...
        if( der->keyOID == ED448k )
    {
        ...
        word32 keyIdx = 0;
            ...
            #ifdef WOLFSSL_SMALL_STACK
        key_pair = ( ed448_key * ) XMALLOC( sizeof( ed448_key ),
                                            NULL,
                                            DYNAMIC_TYPE_ED448 );
        if( key_pair == NULL )
            return MEMORY_E;
        ...
        if( ( ret = wc_Ed448PrivateKeyDecode( key,
                                              &keyIdx,
                                              key_pair,
                                              keySz ) ) == 0 )
        {
            ...
            keyIdx = 0;
            if( ( ret = wc_ed448_import_public( der->publicKey,
                                                der->pubKeySize,
                                                key_pair ) ) == 0 )
            {
                /* public and private extracted successfully no check if is
                      ...
                      void * heap )
{
    ...
    if( out == NULL )
    {
        ...
        /* plus 3 for tags */
        *outSz = tmpSz + MAX_ALGO_SZ + MAX_LENGTH_SZ + MAX_LENGTH_SZ +
                 MAX_SEQ_SZ + MAX_LENGTH_SZ + MAX_SEQ_SZ + 3;
            #ifdef WOLFSSL_SMALL_STACK
        if( saltTmp != NULL )
            XFREE( saltTmp, heap, DYNAMIC_TYPE_TMP_BUFFER );
            #endif
    ...
    tmp = ( byte * ) XMALLOC( tmpSz + ( blockSz - 1 ),
                              heap,
                              DYNAMIC_TYPE_TMP_BUFFER );
    if( tmp == NULL )
    {
                    ...
                    void * heap )
{
    ...
    if( ret == 0 )
    {
        pkcs8Key = ( byte * ) XMALLOC( pkcs8KeySz,
                                       NULL,
                                       DYNAMIC_TYPE_TMP_BUFFER );
        if( pkcs8Key == NULL )
            ret = MEMORY_E;
    ...
    if( ret == 0 )
    {
        ret = wc_CreatePKCS8Key( pkcs8Key,
                                 &pkcs8KeySz,
                                 key,
                                 keySz,
                                 algId,
    ...
    if( ret == 0 )
    {
        padSz = ( blockSz - ( pkcs8KeySz & ( blockSz - 1 ) ) ) &
                ( blockSz - 1 );
                      ...
                      word32 * algId )
{
    ...
    {
        ret = DecryptContent( input + inOutIdx,
                              sz - inOutIdx,
                              password,
                              passwordSz );
        if( ret > 0 )
                              ...
                              RsaKey * key )
{
    ...
    if( mp_read_unsigned_bin( &key->n, n, nSz ) != 0 )
    {
        ...
        return ASN_GETINT_E;
            ...
            #ifdef HAVE_WOLF_BIGINT
    if( ( int ) nSz > 0 &&
        wc_bigint_from_unsigned_bin( &key->n.raw, n, nSz ) != 0 )
    {
        mp_clear( &key->n );
        return ASN_GETINT_E;
    }
    ...
    if( mp_read_unsigned_bin( &key->e, e, eSz ) != 0 )
    {
        ...
        return ASN_GETINT_E;
            ...
            #ifdef HAVE_WOLF_BIGINT
    if( ( int ) eSz > 0 &&
        wc_bigint_from_unsigned_bin( &key->e.raw, e, eSz ) != 0 )
    {
        mp_clear( &key->n );
        mp_clear( &key->e );
        return ASN_GETINT_E;
...
/* The wc_DhParamsLoad function also loads DH params, but directly into buffers,
 * not DhKey */
                        ...
                        int with_header )
{
    ...
    if( with_header )
    {
        ...
        /* check output size */
        if( ( idx + algoSz + bitStringSz + innerSeqSz + pSz + qSz + gSz +
              ySz ) > outLen )
        {
            #ifdef WOLFSSL_SMALL_STACK
            XFREE( p, key->heap, DYNAMIC_TYPE_TMP_BUFFER );
...
int wc_DsaKeyToDer( DsaKey * key, byte * output, word32 inLen )
{
    ...
    for( i = 0; i < DSA_INTS; i++ )
    {
        ...
        tmps[ i ] = ( byte * ) XMALLOC( rawLen + MAX_SEQ_SZ,
                                        key->heap,
                                        DYNAMIC_TYPE_DSA );
        if( tmps[ i ] == NULL )
        {
...
static int GetCertHeader( DecodedCert * cert )
{
    ...
    if( GetExplicitVersion( cert->source,
                            &cert->srcIdx,
                            &cert->version,
                            cert->sigIndex ) < 0 )
        return ASN_PARSE_E;

    if( GetSerialNumber( cert->source,
                         &cert->srcIdx,
                         cert->serial,
                         &cert->serialSz,
                         cert->sigIndex ) < 0 )
...
static int GetKey( DecodedCert * cert )
{
    ...
    switch( cert->keyOID )
    {
        ...
        case RSAk:
        {
            ...
            ret = CheckBitString( cert->source,
                                  &cert->srcIdx,
                                  &length,
                                  cert->maxIdx,
                                  1,
        ...
        #ifdef HAVE_OCSP
            ret = CalcHashId( cert->source + cert->srcIdx,
                              length,
                              cert->subjectKeyHash );
            if( ret != 0 )
                return ret;
        ...
        #endif
            rc = ntru_crypto_ntru_encrypt_subjectPublicKeyInfo2PublicKey(
                key,
                &keyLen,
                NULL,
                &next,
        ...
        #ifdef WOLFSSL_SMALL_STACK
            keyBlob = ( byte * ) XMALLOC( MAX_NTRU_KEY_SZ,
                                          cert->heap,
                                          DYNAMIC_TYPE_TMP_BUFFER );
            if( keyBlob == NULL )
                return MEMORY_E;
...
int wc_OBJ_sn2nid( const char * sn )
{
    const struct
    {
        const char * sn;
        int nid;
    } sn2nid[] = { { WOLFSSL_COMMON_NAME, NID_commonName },
...
int GetName( DecodedCert * cert, int nameType, int maxIdx )
{
    ...
    while( cert->srcIdx < ( word32 ) length )
    {
        ...
        if( joint[ 0 ] == 0x55 && joint[ 1 ] == 0x04 )
        {
            cert->srcIdx += 3;
            id = joint[ 2 ];
            if( GetHeader( cert->source, &b, &cert->srcIdx, &strLen, maxIdx, 1 ) <
                0 )
            {
    #if( defined( OPENSSL_EXTRA ) || defined( OPENSSL_EXTRA_X509_SMALL ) ) && \
        !defined( WOLFCRYPT_ONLY )
    ...
    #ifdef WOLFSSL_CERT_EXT
        else if( ( cert->srcIdx + ASN_JOI_PREFIX_SZ + 2 <=
                   ( word32 ) maxIdx ) &&
                 ( 0 == XMEMCMP( &cert->source[ cert->srcIdx ],
                                 ASN_JOI_PREFIX,
                                 ASN_JOI_PREFIX_SZ ) ) &&
                 ( ( cert->source[ cert->srcIdx + ASN_JOI_PREFIX_SZ ] ==
                     ASN_JOI_C ) ||
                   ( cert->source[ cert->srcIdx + ASN_JOI_PREFIX_SZ ] ==
                     ASN_JOI_ST ) ) )
        {
        ...
        !defined( WOLFCRYPT_ONLY )
            ...
            !defined( WOLFCRYPT_ONLY )
                    ...
                    emailName->type = 0;
                    emailName->name = ( char * )
                        XMALLOC( strLen + 1, cert->heap, DYNAMIC_TYPE_ALTNAME );
        ...
        if( nid != NID_undef )
        {
            if( wolfSSL_X509_NAME_add_entry_by_NID(
                    dName,
                    nid,
                    MBSTRING_UTF8,
                    &cert->source[ cert->srcIdx ],
                 ...
                 int * idx )
{
    ...
    return 1;
            ...
            defined( OPENSSL_EXTRA ) || defined( WOLFSSL_NGINX ) ||          \
...
int GetTimeString( byte * date, int format, char * buf, int len )
{
    ...
    return 1;
        ...
        #endif /* OPENSSL_ALL || WOLFSSL_MYSQL_COMPATIBLE || WOLFSSL_NGINX || \
                  WOLFSSL_HAPROXY */
...
int wc_GetCertDates( Cert * cert, struct tm * before, struct tm * after )
{
    ...
    {
        ret = wc_GetDateInfo( cert->beforeDate,
                              cert->beforeDateSz,
                              &date,
                              &format,
                              &length );
    ...
    {
        ret = wc_GetDateInfo( cert->afterDate,
                              cert->afterDateSz,
                              &date,
                              &format,
                              &length );
...
word32 SetLength( word32 length, byte * output )
{
    ...
    {
        if( output )
            output[ i ] = ( byte ) ( BytePrecision( length ) |
                                     ASN_LONG_LENGTH );
        i++;
...
{
    output[ 0 ] = ( ( tag == ASN_SEQUENCE || tag == ASN_SET ) ? ASN_CONSTRUCTED
                                                              : 0 ) |
                             ...
                             byte * rsaKeyIdx )
{
    ...
    switch( sigCtx->state )
    {
        case SIG_STATE_BEGIN:
        {
            ...
            sigCtx->digest = ( byte * ) XMALLOC( WC_MAX_DIGEST_SIZE,
                                                 sigCtx->heap,
                                                 DYNAMIC_TYPE_DIGEST );
        ...
        case SIG_STATE_KEY:
        {
            switch( keyOID )
            {
                ...
                case RSAk:
                {
                    ...
                    if( ( ret = wc_InitRsaKey_ex( sigCtx->key.rsa,
                                                  sigCtx->heap,
                                                  sigCtx->devId ) ) != 0 )
                    {
                        goto exit_cs;
                    ...
                    if( ( ret = wc_RsaPublicKeyDecode( key,
                                                       &idx,
                                                       sigCtx->key.rsa,
                                                       keySz ) ) != 0 )
                    {
                ...
                case ECDSAk:
                {
                    ...
                    if( ( ret = wc_ecc_init_ex( sigCtx->key.ecc,
                                                sigCtx->heap,
                                                sigCtx->devId ) ) < 0 )
                    {
                        goto exit_cs;
                    }
                    ret = wc_EccPublicKeyDecode( key,
                                                 &idx,
                                                 sigCtx->key.ecc,
                                                 keySz );
                    if( ret < 0 )
                ...
                {
                    sigCtx->verify = 0;
                    sigCtx->key.ed25519 = ( ed25519_key * )
                        XMALLOC( sizeof( ed25519_key ),
                                 sigCtx->heap,
                                 DYNAMIC_TYPE_ED25519 );
                    ...
                    if( ( ret = wc_ed25519_import_public( key,
                                                          keySz,
                                                          sigCtx->key
                                                              .ed25519 ) ) < 0 )
                    {
                ...
                {
                    sigCtx->verify = 0;
                    sigCtx->key.ed448 = ( ed448_key * )
                        XMALLOC( sizeof( ed448_key ),
                                 sigCtx->heap,
                                 DYNAMIC_TYPE_ED448 );
                    ...
                    if( ( ret = wc_ed448_import_public( key,
                                                        keySz,
                                                        sigCtx->key.ed448 ) ) <
                        0 )
                    {
            ...
            #ifdef WOLFSSL_ASYNC_CRYPT
            if( sigCtx->devId != INVALID_DEVID && sigCtx->asyncDev &&
                sigCtx->asyncCtx )
            {
                /* make sure event is initialized */
                WOLF_EVENT * event = &sigCtx->asyncDev->event;
                ret = wolfAsync_EventInit( event,
        ...
        case SIG_STATE_DO:
        {
            switch( keyOID )
            {
                ...
                case RSAk:
                {
                    ...
                    if( sigCtx->pkCbRsa )
                    {
                        ret = sigCtx->pkCbRsa( sigCtx->plain,
                                               sigSz,
                                               &sigCtx->out,
                                               key,
                                               keySz,
                ...
                #endif /* HAVE_PK_CALLBACKS */
                    {
                        ...
                        if( rsaKeyIdx != NULL )
                        {
                            ret = tsip_tls_CertVerify( buf,
                                                       bufSz,
                                                       sigCtx->plain,
                                                       sigSz,
                                                       sigCtx->pubkey_n_start -
                                                           sigCtx->certBegin,
                                                       sigCtx->pubkey_n_len - 1,
                                                       sigCtx->pubkey_e_start -
                                                           sigCtx->certBegin,
                            ...
                            {
                                WOLFSSL_MSG(
                                    "RSA Verify by tsip didn't match" );
                                ret = ASN_SIG_CONFIRM_E;
                            }
                        }
                        else
                #endif
                            ret = wc_RsaSSL_VerifyInline( sigCtx->plain,
                                                          sigSz,
                                                          &sigCtx->out,
                                                          sigCtx->key.rsa );
                    }
        ...
        case SIG_STATE_CHECK:
        {
            switch( keyOID )
            {
                ...
                #endif
                #ifdef WOLFSSL_SMALL_STACK
                    byte * encodedSig = ( byte * )
                        XMALLOC( MAX_ENCODED_SIG_SZ,
                                 sigCtx->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
                    if( encodedSig == NULL )
                    {
                          ...
                          int baseSz )
{
    ...
    if( ( type == ASN_DNS_TYPE || type == ASN_RFC822_TYPE ) &&
        base[ 0 ] == '.' )
    {
...
static int DecodeAltNames( const byte * input, int sz, DecodedCert * cert )
{
    ...
    while( length > 0 )
    {
        ...
        if( b == ( ASN_CONTEXT_SPECIFIC | ASN_DNS_TYPE ) )
        {
            ...
            dnsEntry = ( DNS_entry * ) XMALLOC( sizeof( DNS_entry ),
                                                cert->heap,
                                                DYNAMIC_TYPE_ALTNAME );
            if( dnsEntry == NULL )
            {
            ...
            dnsEntry->name = ( char * ) XMALLOC( strLen + 1,
                                                 cert->heap,
                                                 DYNAMIC_TYPE_ALTNAME );
        ...
        else if( b == ( ASN_CONTEXT_SPECIFIC | ASN_RFC822_TYPE ) )
        {
            ...
            emailEntry = ( DNS_entry * ) XMALLOC( sizeof( DNS_entry ),
                                                  cert->heap,
                                                  DYNAMIC_TYPE_ALTNAME );
            if( emailEntry == NULL )
            {
            ...
            emailEntry->name = ( char * ) XMALLOC( strLen + 1,
                                                   cert->heap,
                                                   DYNAMIC_TYPE_ALTNAME );
        ...
        else if( b == ( ASN_CONTEXT_SPECIFIC | ASN_URI_TYPE ) )
        {
            ...
            uriEntry = ( DNS_entry * ) XMALLOC( sizeof( DNS_entry ),
                                                cert->heap,
                                                DYNAMIC_TYPE_ALTNAME );
            if( uriEntry == NULL )
            {
            ...
            uriEntry->name = ( char * ) XMALLOC( strLen + 1,
                                                 cert->heap,
                                                 DYNAMIC_TYPE_ALTNAME );
        ...
        else if( b == ( ASN_CONTEXT_SPECIFIC | ASN_IP_TYPE ) )
        {
            ...
            ipAddr = ( DNS_entry * ) XMALLOC( sizeof( DNS_entry ),
                                              cert->heap,
                                              DYNAMIC_TYPE_ALTNAME );
            if( ipAddr == NULL )
            {
            ...
            ipAddr->name = ( char * ) XMALLOC( strLen + 1,
                                               cert->heap,
                                               DYNAMIC_TYPE_ALTNAME );
            if( ipAddr->name == NULL )
            {
                 ...
                 ( ASN_CONTEXT_SPECIFIC | ASN_CONSTRUCTED | ASN_OTHER_TYPE ) )
        {
            ...
            cert->hwType = ( byte * ) XMALLOC( strLen,
                                               cert->heap,
            ...
            cert->hwSerialNum = ( byte * ) XMALLOC( strLen + 1,
                                                    cert->heap,
                                                    DYNAMIC_TYPE_X509_EXT );
...
static int DecodeCrlDist( const byte * input, int sz, DecodedCert * cert )
{
    ...
    localIdx = idx;
    if( idx < ( word32 ) sz && GetASNTag( input, &localIdx, &tag, sz ) == 0 &&
                          ...
                          void * heap )
{
    ...
    while( idx < ( word32 ) sz )
    {
            ...
            bType == ASN_DIR_TYPE )
        {
            ...
            entry = ( Base_entry * ) XMALLOC( sizeof( Base_entry ),
                                              heap,
                                              DYNAMIC_TYPE_ALTNAME );
            if( entry == NULL )
            {
            ...
            entry->name = ( char * ) XMALLOC( strLength,
                                              heap,
                                              DYNAMIC_TYPE_ALTNAME );
            if( entry->name == NULL )
            {
...
static int DecodeCertPolicy( const byte * input, int sz, DecodedCert * cert )
{
    ...
    do
    {
        ...
        if( length > 0 )
        {
            ...
            cert->deviceType = ( byte * ) XMALLOC( length,
                                                   cert->heap,
                                                   DYNAMIC_TYPE_X509_EXT );
        ...
        switch( oid )
        {
            ...
            case AUTH_KEY_OID:
                ...
                if( critical )
                {
                    ...
                    WOLFSSL_MSG(
                        "Use macro WOLFSSL_ALLOW_CRIT_SKID if wanted" );
            ...
            case NETSCAPE_CT_OID:
                WOLFSSL_MSG(
                    "Netscape certificate type extension not supported "
                    "yet." );
                if( CheckBitString( input,
                                    &idx,
                                    &length,
                                    idx + length,
                                    0,
...
int ParseCert( DecodedCert * cert, int type, int verify, void * cm )
{
    ...
    {
        ptr = ( char * ) XMALLOC( cert->subjectCNLen + 1,
                                  cert->heap,
    ...
    if( cert->keyOID == RSAk && cert->publicKey != NULL &&
        cert->pubKeySize > 0 )
    {
        ptr = ( char * ) XMALLOC( cert->pubKeySize,
                                  cert->heap,
                                  DYNAMIC_TYPE_PUBLIC_KEY );
        if( ptr == NULL )
            return MEMORY_E;
                                  ...
                                  int pubKeyOID )
{
    ...
    if( cert == NULL )
    {
        return BAD_FUNC_ARG;
            ...
            #ifdef WOLFSSL_SMALL_STACK
    sigCtx = ( SignatureCtx * ) XMALLOC( sizeof( *sigCtx ),
                                         heap,
                                         DYNAMIC_TYPE_SIGNATURE );
    if( sigCtx == NULL )
        return MEMORY_E;
        ...
        if( ret == 0 )
        {
            ...
            while( ret == 0 && idx < extEndIdx )
            {
                ...
                if( ret == 0 )
                {
                    ...
                    {
                        case AUTH_KEY_OID:
                            if( GetSequence( cert, &extIdx, &extLen, certSz ) <
                                0 )
                                ret = ASN_PARSE_E;
...
int CheckCertSignaturePubKey( const byte * cert,
                              ...
                              int pubKeyOID )
{
    return CheckCertSignature_ex( cert,
                                  certSz,
                                  heap,
                                  NULL,
                                  pubKey,
...
int ParseCertRelative( DecodedCert * cert, int type, int verify, void * cm )
{
                                   ...
                                   type != TRUSTED_PEER_TYPE ) )
        {
            ...
            if( cert->ca == NULL )
            {
                ...
                if( cert->ca && cert->extAuthKeyIdSet )
                {
                    ...
                    /* OCSP Only: alt lookup using subject and pub key w/o sig
                     * check */
        ...
        else
        {
                ...
                type != TRUSTED_PEER_TYPE )
            {
                if( cert->ca )
                {
                    ...
                    XMEMCPY( cert->issuerKeyHash,
                             cert->ca->subjectKeyHash,
                             KEYID_SIZE );
                }
            }
        #endif /* HAVE_OCSP */
    ...
    if( cert->keyOID == RSAk )
    {
        ...
        if( ( ret = RsaPublicKeyDecodeRawIndex( cert->publicKey,
                                                ( word32 * ) &idx,
                                                cert->pubKeySize,
        ...
        cert->sigCtx.pubkey_n_len == 256 )
    {
        ...
        if( !cert->tsip_encRsaKeyIdx )
            cert->tsip_encRsaKeyIdx = ( byte * )
                XMALLOC( TSIP_TLS_ENCPUBKEY_SZ_BY_CERTVRFY,
                         cert->heap,
                         DYNAMIC_TYPE_RSA );
        if( cert->tsip_encRsaKeyIdx == NULL )
            return MEMORY_E;
...
int AllocDer( DerBuffer ** pDer, word32 length, int type, void * heap )
{
    ...
    if( pDer )
    {
        ...
        /* Setup new buffer */
        *pDer = ( DerBuffer * ) XMALLOC( sizeof( DerBuffer ) + length,
                                         heap,
                                         dynType );
        if( *pDer == NULL )
        {
...
wcchar END_EC_PRIV = "-----END EC PRIVATE KEY-----";
                ...
                defined( HAVE_ED448 ) || !defined( NO_DSA )
...
int wc_EncryptedInfoGet( EncryptedInfo * info, const char * cipherInfo )
{
    ...
    else if( XSTRNCMP( cipherInfo, kEncTypeDes3, XSTRLEN( kEncTypeDes3 ) ) ==
             0 )
    {
        ...
        if( info->ivSz == 0 )
            info->ivSz = DES_IV_SIZE;
                    ...
                    #if !defined( NO_AES ) && defined( HAVE_AES_CBC ) && \
                        defined( WOLFSSL_AES_128 )
        if( XSTRNCMP( cipherInfo,
                      kEncTypeAesCbc128,
                      XSTRLEN( kEncTypeAesCbc128 ) ) == 0 )
    {
...
static int wc_EncryptedInfoAppend( char * dest, int destSz, char * cipherInfo )
{
    if( cipherInfo != NULL )
    {
        ...
        if( destSz - ( int ) XSTRLEN( dest ) >=
            cipherInfoStrLen + ( 9 + 14 + 8 + 2 + 2 + 1 ) )
        {
                   ...
                   int type )
{
    ...
    if( ( err = Base64_Encode( der, derSz, output + i, ( word32 * ) &outLen ) ) <
        0 )
    {
                #ifdef WOLFSSL_SMALL_STACK
        XFREE( footer, NULL, DYNAMIC_TYPE_TMP_BUFFER );
              ...
              int * keyFormat )
{
    ...
    { /* handle no end of line on last line */
        /* eat end of line characters */
    ...
    if( Base64_Decode( ( byte * ) headerEnd,
                       ( word32 ) neededSz,
                       der->buffer,
                       &der->length ) < 0 )
        return BUFFER_E;
    ...
    if( encrypted_key || header == BEGIN_ENC_PRIV_KEY )
    {
        ...
        if( ret >= 0 )
        {
            ...
            {
                    #ifndef NO_PWDBASED
                ret = ToTraditionalEnc( der->buffer,
                                        der->length,
                                        password,
                                        passwordSz,
                                        &algId );
                    ...
                    const char * pass )
{
    ...
    if( pem == NULL || buff == NULL || buffSz <= 0 )
    {
        ...
        return BAD_FUNC_ARG;
                ...
                #ifdef WOLFSSL_SMALL_STACK
    info = ( EncryptedInfo * ) XMALLOC( sizeof( EncryptedInfo ),
                                        NULL,
...
typedef struct DerCert
{
    ...
    byte serial[ ( int ) CTC_SERIAL_SIZE + ( int ) MAX_LENGTH_SZ ]; /* serial
                                                                       number
                                                                       encoded
                                                                     */
    ...
    byte validity[ MAX_DATE_SIZE * 2 + MAX_SEQ_SZ * 2 ]; /* before and after
                                                            dates */
    ...
    byte certPolicies[ MAX_CERTPOL_NB * MAX_CERTPOL_SZ ]; /* Certificate
                                                             Policies */
            #endif
            #ifdef WOLFSSL_CERT_REQ
...
static int wc_SetCert_LoadDer( Cert * cert, const byte * der, word32 derSz )
{
    ...
    else
    {
        ...
        cert->decodedCert = ( void * ) XMALLOC( sizeof( DecodedCert ),
                                                cert->heap,
                                                DYNAMIC_TYPE_DCERT );
        ...
        else
        {
            ...
            InitDecodedCert( ( DecodedCert * ) cert->decodedCert,
                             der,
                             derSz,
                             cert->heap );
...
static int SetEccPublicKey( byte * output, ecc_key * key, int with_header )
{
    ...
    {
            #ifdef WOLFSSL_SMALL_STACK
        curve = ( byte * ) XMALLOC( MAX_ALGO_SZ,
                                    key->heap,
                                    DYNAMIC_TYPE_TMP_BUFFER );
        if( curve == NULL )
        {
...
static int CopyValidity( byte * output, Cert * cert )
{
    ...
    if( output )
    {
        ...
        XMEMCPY( output + seqSz + cert->beforeDateSz,
                 cert->afterDate,
                 cert->afterDateSz );
    }
...
{
    const byte ca[] = { 0x30, 0x0c, 0x06, 0x03, 0x55, 0x1d, 0x13,
                    ...
                    word32 length )
{
    ...
    idx = SetSequence( length + sizeof( skid_oid ) + skid_lenSz +
                           skid_enc_lenSz,
                       output );
...
static int SetExtKeyUsage( Cert * cert, byte * output, word32 outSz, byte input )
{
    ...
    {
        ret |= SetOjectIdValue( output,
                                outSz,
                                &idx,
                                extExtKeyUsageAnyOid,
    ...
    else
    {
        if( input & EXTKEYUSE_SERVER_AUTH )
            ret |= SetOjectIdValue( output,
                                    ...
                                    sizeof( extExtKeyUsageServerAuthOid ) );
        if( input & EXTKEYUSE_CLIENT_AUTH )
            ret |= SetOjectIdValue( output,
                                    outSz,
                                    &idx,
                                    extExtKeyUsageClientAuthOid,
                                    sizeof( extExtKeyUsageClientAuthOid ) );
        if( input & EXTKEYUSE_CODESIGN )
            ret |= SetOjectIdValue( output,
                                    outSz,
                                    &idx,
                                    extExtKeyUsageCodeSigningOid,
                                    sizeof( extExtKeyUsageCodeSigningOid ) );
        if( input & EXTKEYUSE_EMAILPROT )
            ret |= SetOjectIdValue( output,
                                    outSz,
                                    &idx,
                                    extExtKeyUsageEmailProtectOid,
                                    sizeof( extExtKeyUsageEmailProtectOid ) );
        if( input & EXTKEYUSE_TIMESTAMP )
            ret |= SetOjectIdValue( output,
                                    outSz,
                                    &idx,
                                    extExtKeyUsageTimestampOid,
                                    sizeof( extExtKeyUsageTimestampOid ) );
        if( input & EXTKEYUSE_OCSP_SIGN )
            ret |= SetOjectIdValue( output,
                                    outSz,
                                    &idx,
                                    extExtKeyUsageOcspSignOid,
                                    sizeof( extExtKeyUsageOcspSignOid ) );
 ...
 * RFC5280 : non-critical */
static int SetCertificatePolicies(
    byte * output,
    word32 outputSz,
    ...
    void * heap )
{
    byte oid[ MAX_OID_SZ ], der_oid[ MAX_CERTPOL_NB ][ MAX_OID_SZ ],
        out[ MAX_CERTPOL_SZ ];
    word32 oidSz;
                          ...
                          byte type )
{
    ...
    if( nameStr )
    {
        ...
        switch( type )
        {
            case ASN_EMAIL_NAME:
            {
                const byte EMAIL_OID[] = { 0x2a, 0x86, 0x48, 0x86, 0xf7,
                                           0x0d, 0x01, 0x09, 0x01, 0x16 };
                /* email joint id */
            ...
            case ASN_DOMAIN_COMPONENT:
            {
                const byte PILOT_OID[] = { 0x09, 0x92, 0x26, 0x89, 0x93,
                                           0xF2, 0x2C, 0x64, 0x01 };
...
int SetName( byte * output, word32 outputSz, CertName * name )
{
    ...
    if( outputSz < 3 )
        return BUFFER_E;
            ...
            #ifdef WOLFSSL_SMALL_STACK
    names = ( EncodedName * ) XMALLOC( sizeof( EncodedName ) * NAME_ENTRIES,
                                       NULL,
                                       DYNAMIC_TYPE_TMP_BUFFER );
    if( names == NULL )
        return MEMORY_E;
                       ...
                       ed448_key * ed448Key )
{
    ...
    der->serialSz = SetSerialNumber( cert->serial,
                                     cert->serialSz,
                                     der->serial,
                                     sizeof( der->serial ),
                                     CTC_SERIAL_SIZE );
    ...
    if( cert->keyType == RSA_KEY )
    {
        if( rsaKey == NULL )
            return PUBLIC_KEY_E;
        der->publicKeySz = SetRsaPublicKey( der->publicKey,
                                            rsaKey,
            ...
            #endif
    {
        ...
        der->subjectSz = SetName( der->subject,
                                  sizeof( der->subject ),
                                  &cert->subject );
    }
    if( der->subjectSz <= 0 )
    ...
    {
        /* Use the name structure */
        der->issuerSz = SetName( der->issuer,
                                 sizeof( der->issuer ),
                                 cert->selfSigned ? &cert->subject
                                                  : &cert->issuer );
    }
    if( der->issuerSz <= 0 )
        return ISSUER_E;
    ...
    if( cert->akidSz )
    {
        ...
        if( cert->akidSz >
            ( int ) min( CTC_MAX_AKID_SIZE, sizeof( der->akid ) ) )
            return AKID_E;

        der->akidSz = SetAKID( der->akid,
                               sizeof( der->akid ),
                               cert->akid,
                               cert->akidSz,
                               cert->heap );
    ...
    if( cert->keyUsage != 0 )
    {
        der->keyUsageSz = SetKeyUsage( der->keyUsage,
                                       sizeof( der->keyUsage ),
                                       cert->keyUsage );
        if( der->keyUsageSz <= 0 )
            return KEYUSAGE_E;
    ...
    {
        der->extKeyUsageSz = SetExtKeyUsage( cert,
                                             der->extKeyUsage,
                                             sizeof( der->extKeyUsage ),
                                             cert->extKeyUsage );
    ...
    if( der->extensionsSz > 0 )
    {
        ...
        if( der->caSz )
        {
            ret = SetExtensions( der->extensions,
                                 sizeof( der->extensions ),
                                 &der->extensionsSz,
                                 der->ca,
                                 der->caSz );
    ...
    der->total = der->versionSz + der->serialSz + der->sigAlgoSz +
                 der->publicKeySz + der->validitySz + der->subjectSz +
                 der->issuerSz + der->extensionsSz;
...
static int WriteCertBody( DerCert * der, byte * buf )
{
    ...
    if( der->extensionsSz )
    {
        ...
        XMEMCPY( buf + idx,
                 der->extensions,
                 min( der->extensionsSz, ( int ) sizeof( der->extensions ) ) );
                          ...
                          void * heap )
{
    ...
    switch( certSignCtx->state )
    {
        ...
        case CERTSIGN_STATE_DIGEST:
            ...
            certSignCtx->digest = ( byte * ) XMALLOC( WC_MAX_DIGEST_SIZE,
                                                      heap,
                                                      DYNAMIC_TYPE_TMP_BUFFER );
            ...
            ret = HashForSignature( buf,
                                    ...
                                    &digestSz,
                                    0 );
            /* set next state, since WC_PENDING_E rentry for these are not "call
             * again" */
        ...
        case CERTSIGN_STATE_ENCODE:
            ...
            if( rsaKey )
            {
                certSignCtx->encSig = ( byte * )
                    XMALLOC( MAX_DER_DIGEST_SZ, heap, DYNAMIC_TYPE_TMP_BUFFER );
        ...
        case CERTSIGN_STATE_DO:
            ...
            if( !rsaKey && eccKey )
            {
                ...
                ret = wc_ecc_sign_hash( certSignCtx->digest,
                                        digestSz,
                                        sig,
                                        &outSz,
                                        rng,
                        ...
                        ed448_key * ed448Key )
{
    ...
    cert->keyType = eccKey
            ...
            #ifdef WOLFSSL_SMALL_STACK
    der = ( DerCert * ) XMALLOC( sizeof( DerCert ),
                                 cert->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    if( der == NULL )
        return MEMORY_E;
...
static int SetReqAttrib( byte * output,
                         ...
                         int extSz )
{
    const byte cpOid[] = { ASN_OBJECT_ID, 0x09, 0x2a, 0x86, 0x48, 0x86,
                           0xf7,          0x0d, 0x01, 0x09, 0x07 };
    const byte erOid[] = { ASN_OBJECT_ID, 0x09, 0x2a, 0x86, 0x48, 0x86,
    ...
    if( pw && pw[ 0 ] )
    {
        ...
        cpSeqSz = SetSequence( sizeof( cpOid ) + cpSetSz + cpStrSz + pwSz,
                               cpSeq );
                     ...
                     WC_RNG * rng )
{
    ...
    {
        certSignCtx->sig = ( byte * ) XMALLOC( MAX_ENCODED_SIG_SZ,
                                               heap,
                                               DYNAMIC_TYPE_TMP_BUFFER );
        if( certSignCtx->sig == NULL )
            return MEMORY_E;
    ...
    sigSz = MakeSignature( certSignCtx,
                           buf,
                           requestSz,
                           certSignCtx->sig,
                           MAX_ENCODED_SIG_SZ,
                     ...
                     WC_RNG * rng )
{
    ...
    return wc_SignCert( cert->bodySz,
                        cert->sigType,
                        buf,
                        buffSz,
                        key,
                                  ...
                                  int kid_type )
{
    ...
    buf = ( byte * ) XMALLOC( MAX_PUBLIC_KEY_SZ,
                              cert->heap,
                              DYNAMIC_TYPE_TMP_BUFFER );
    if( buf == NULL )
        return MEMORY_E;
...
int wc_SetSubjectKeyIdFromPublicKey_ex( Cert * cert, int keyType, void * key )
{
    ...
    return SetKeyIdFromPublicKey( cert,
                                  rsaKey,
                                  eccKey,
                                  NULL,
                                  0,
...
int wc_SetSubjectKeyId( Cert * cert, const char * file )
{
    ...
    der = ( byte * ) XMALLOC( MAX_PUBLIC_KEY_SZ,
                              cert->heap,
                              DYNAMIC_TYPE_CERT );
    if( der == NULL )
    {
    ...
    if( derSz <= 0 )
    {
        ...
        return derSz;
                    ...
                    #ifndef NO_RSA
    rsakey = ( RsaKey * ) XMALLOC( sizeof( RsaKey ),
                                   cert->heap,
                                   DYNAMIC_TYPE_RSA );
    if( rsakey == NULL )
    {
                    ...
                    #endif
    {
        ...
        eckey = ( ecc_key * ) XMALLOC( sizeof( ecc_key ),
                                       cert->heap,
                                       DYNAMIC_TYPE_ECC );
        if( eckey == NULL )
        {
...
int wc_SetAuthKeyIdFromCert( Cert * cert, const byte * der, int derSz )
{
    ...
    else
    {
        ...
        if( ret >= 0 )
        {
            ret = SetAuthKeyIdFromDcert( cert,
...
static int SetAltNamesFromDcert( Cert * cert, DecodedCert * decoded )
{
    ...
    if( decoded->extensions )
    {
        ...
        else if( GetLength( decoded->source,
                            &decoded->srcIdx,
                            &length,
                            decoded->maxIdx ) < 0 )
        {
        ...
        else if( GetSequence( decoded->source,
                              &decoded->srcIdx,
                              &length,
                              decoded->maxIdx ) < 0 )
        {
            ...
            while( decoded->srcIdx < maxExtensionsIdx )
            {
                ...
                if( oid == ALT_NAMES_OID )
                {
                    ...
                    if( cert->altNamesSz < ( int ) sizeof( cert->altNames ) )
                        XMEMCPY( cert->altNames,
                                 &decoded->source[ startIdx ],
                                 cert->altNamesSz );
                    else
                    {
                        cert->altNamesSz = 0;
...
static int SetDatesFromDcert( Cert * cert, DecodedCert * decoded )
{
    ...
    {
        XMEMCPY( cert->beforeDate,
                 decoded->beforeDate,
                 decoded->beforeDateLen );
...
int StoreECC_DSA_Sig( byte * out, word32 * outLen, mp_int * r, mp_int * s )
{
    ...
    if( *outLen < ( rLen + rLeadingZero + sLen + sLeadingZero + headerSz +
                    2 ) ) /* SEQ_TAG + LEN(ENUM) */
        return BUFFER_E;

    idx = SetSequence( rLen + rLeadingZero + sLen + sLeadingZero + headerSz,
                       out );
                            ...
                            word32 inSz )
{
    ...
    *inOutIdx += 1;
    ...
    if( length > ECC_MAXSIZE )
        ...
        #ifdef WOLFSSL_SMALL_STACK
    priv = ( byte * ) XMALLOC( ECC_MAXSIZE + 1,
                               key->heap,
                               DYNAMIC_TYPE_TMP_BUFFER );
    if( priv == NULL )
        return MEMORY_E;

    pub = ( byte * ) XMALLOC( 2 * ( ECC_MAXSIZE + 1 ),
                              key->heap,
                              DYNAMIC_TYPE_TMP_BUFFER );
    if( pub == NULL )
    {
    ...
    *inOutIdx += length;
    ...
    if( ret == 0 )
    {
        ret = wc_ecc_import_private_key_ex( priv,
                                            privSz,
                                            pubData,
                                            pubSz,
                                            key,
                           ...
                           word32 inSz )
{
        ...
        tag == ( ASN_SEQUENCE | ASN_CONSTRUCTED ) )
    {
        ...
        curve = ( ecc_set_type * ) XMALLOC( sizeof( *curve ),
                                            key->heap,
        ...
        if( ret == 0 )
        {
            localIdx = *inOutIdx;
            if( *inOutIdx < inSz &&
                GetASNTag( input, &localIdx, &tag, inSz ) == 0 &&
                tag == ASN_BIT_STRING )
            {
                len = 0;
                ret = GetASNHeader( input,
                                    ASN_BIT_STRING,
                                    inOutIdx,
                                    &len,
                                    inSz );
        ...
        {
            #ifndef WOLFSSL_ECC_CURVE_STATIC
            curve->Gx = ( const char * ) XMALLOC( curve->size * 2 + 2,
                                                  key->heap,
                                                  DYNAMIC_TYPE_ECC_BUFFER );
            curve->Gy = ( const char * ) XMALLOC( curve->size * 2 + 2,
                                                  key->heap,
                                                  DYNAMIC_TYPE_ECC_BUFFER );
        ...
        if( ret == 0 )
        {
            ...
            XMEMCPY( ( char * ) curve->Gy,
                     point + curve->size * 2 + 2,
                     curve->size * 2 );
...
int wc_EccPrivateKeyToPKCS8( ecc_key * key, byte * output, word32 * outLen )
{
    ...
    tmpDer = ( byte * ) XMALLOC( ECC_BUFSIZE,
                                 key->heap,
                                 DYNAMIC_TYPE_TMP_BUFFER );
    if( tmpDer == NULL )
        return MEMORY_E;
                               ...
                               word32 inSz )
{
    ...
    /* This is the raw point data compressed or uncompressed. */
    if( wc_ed25519_import_public( input + *inOutIdx, inSz - *inOutIdx, key ) !=
        0 )
        return ASN_ECC_KEY_E;
                             ...
                             word32 inSz )
{
    ...
    /* This is the raw point data compressed or uncompressed. */
    if( wc_ed448_import_public( input + *inOutIdx, inSz - *inOutIdx, key ) !=
        0 )
        return ASN_ECC_KEY_E;
                                 ...
                                 word32 size )
{
    ...
    XMEMCPY( cs->thisDateParsed.data,
             ...
             cs->thisDateParsed.length );
        #endif
    if( GetBasicDate( source, &idx, cs->thisDate, &cs->thisDateFormat, size ) <
        0 )
        return ASN_PARSE_E;
                                     ...
                                     word32 sz )
{
    ...
    while( idx < ( word32 ) ext_bound )
    {
        ...
        localIdx = idx;
        if( GetASNTag( source, &localIdx, &tag, sz ) == 0 &&
            tag == ASN_BOOLEAN )
        {
                                    ...
                                    int noVerify )
{
    ...
    idx += sigLength;
        ...
        /*
         * Check the length of the BasicOcspResponse against the current index
         * to see if there are certificates, they are optional.
         */
        #ifndef WOLFSSL_NO_OCSP_OPTIONAL_CERTS
    if( idx < end_index )
    {
        ...
        ret = ParseCertRelative( &cert,
                                 CERT_TYPE,
                                 noVerify ? NO_VERIFY : VERIFY_OCSP,
                                 cm );
        if( ret < 0 )
        {
        ...
        {
            if( XMEMCMP( cert.subjectHash, resp->issuerHash, KEYID_SIZE ) == 0 )
            {
        ...
        #endif /* WOLFSSL_NO_OCSP_OPTIONAL_CERTS */
    {
        ...
        if( ca )
        {
            ...
            /* ConfirmSignature is blocking here */
            sigValid = ConfirmSignature( &sigCtx,
                                         resp->response,
                                         resp->responseSz,
                                         ca->publicKey,
                                         ca->pubKeySize,
                        ...
                        int noVerify )
{
    ...
    ret = DecodeBasicOcspResponse( source,
                                   &idx,
                                   resp,
                                   size,
                                   cm,
...
word32 EncodeOcspRequestExtensions( OcspRequest * req,
                                    ...
                                    word32 size )
{
    const byte NonceObjId[] = { 0x2b, 0x06, 0x01, 0x05, 0x05,
                                0x07, 0x30, 0x01, 0x02 };
    ...
    totalSz += seqSz[ 1 ] = SetOctetString( req->nonceSz + seqSz[ 0 ],
                                            seqArray[ 1 ] );
...
int EncodeOcspRequest( OcspRequest * req, byte * output, word32 size )
{
    ...
    snSz = SetSerialNumber( req->serial,
                            req->serialSz,
                            snArray,
                            MAX_SN_SZ,
                            MAX_SN_SZ );
    ...
    if( req->nonceSz )
    {
         ...
         */
        extSz = EncodeOcspRequestExtensions( req,
                                             extArray + 2,
                                             OCSP_NONCE_EXT_SZ );
                     ...
                     void * heap )
{
    ...
    if( cert )
    {
        ...
        req->serial = ( byte * ) XMALLOC( cert->serialSz,
                                          req->heap,
        ...
        {
            req->url = ( byte * ) XMALLOC( cert->extAuthInfoSz + 1,
                                           req->heap,
                                           DYNAMIC_TYPE_OCSP_REQUEST );
            if( req->url == NULL )
            {
    ...
    if( useNonce )
    {
        ...
        {
            if( wc_RNG_GenerateBlock( &rng, req->nonce, MAX_OCSP_NONCE_SZ ) !=
                0 )
                       ...
                       int maxIdx )
{
    ...
    rc = ( RevokedCert * ) XMALLOC( sizeof( RevokedCert ),
                                    dcrl->heap,
    ...
    if( GetSerialNumber( buff, idx, rc->serialNumber, &rc->serialSz, maxIdx ) <
        0 )
    {
                              ...
                              int sz )
{
    ...
    if( GetBasicDate( buf, &idx, dcrl->lastDate, &dcrl->lastDateFormat, sz ) <
        0 )
        return ASN_PARSE_E;

    dateIdx = idx;

    if( GetBasicDate( buf, &idx, dcrl->nextDate, &dcrl->nextDateFormat, sz ) <
        0 )
    {
...
int ParseCRL( DecodedCRL * dcrl, const byte * buff, word32 sz, void * cm )
{
    ...
    if( ca != NULL &&
        XMEMCMP( dcrl->issuerHash, ca->subjectNameHash, KEYID_SIZE ) != 0 )
    {
        ca = NULL;
    }
    if( ca == NULL )
    ...
    return VerifyCRL_Signature( &sigCtx,
                                buff + dcrl->certBegin,
...
int wc_ParseCertPIV( wc_CertPIV * piv, const byte * buf, word32 totalSz )
{
    ...
    if( GetASNHeader( buf, ASN_PIV_CERT, &idx, &length, totalSz ) >= 0 )
    {
        ...
        if( GetASNHeader( buf, ASN_PIV_SIGNED_NONCE, &idx, &length, totalSz ) >=
            0 )
        {
    ...
    if( GetASNHeader( buf,
                      ASN_APPLICATION | ASN_PRINTABLE_STRING,
                      &idx,
                      &length,
                      totalSz ) < 0 )
    {
    ...
    if( GetASNHeader( buf, ASN_PIV_TAG_CERT_INFO, &idx, &length, totalSz ) >=
        0 )
    {
        if( length >= 1 )
        {
