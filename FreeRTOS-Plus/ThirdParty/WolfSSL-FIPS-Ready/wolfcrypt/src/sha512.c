...
#include <wolfssl/wolfcrypt/settings.h>
    ...
    !defined( WOLFSSL_ARMASM ) && !defined( WOLFSSL_PSOC6_CRYPTO )
...
void wc_Sha384Free( wc_Sha384 * sha )
{
    ...
    /* Not supported in FIPS */
        ...
        #if defined( HAVE_BYTEREVERSE64 ) && !defined( HAVE_INTEL_AVX1 ) && \
            !defined( HAVE_INTEL_AVX2 )
            #define ByteReverseWords64( out, in, size ) \
                ByteReverseWords64_1( out, size )
    ...
    return 0;
            ...
            /* Hardware Acceleration */
                ...
                */
                ...
                /* Each platform needs to query info type 1 from cpuid to see if
                 * aesni is supported. Also, let's setup a macro for proper
                 * linkage w/o ABI conflicts
                 */
...
extern int Transform_Sha512_AVX1_RORX_Len( wc_Sha512 * sha512, word32 len );
extern int Transform_Sha512_AVX2_RORX( wc_Sha512 * sha512 );
extern int Transform_Sha512_AVX2_RORX_Len( wc_Sha512 * sha512, word32 len );
                        #endif
                    #endif
...
static const word64 K512[ 80 ] = {
    ...
    W64LIT( 0x5fcb6fab3ad6faec ), W64LIT( 0x6c44198c4a475817 )
            ...
            #define S0( x )                                     \
                ( rotrFixed64( x, 28 ) ^ rotrFixed64( x, 34 ) ^ \
                  rotrFixed64( x, 39 ) )
            #define S1( x )                                     \
                ( rotrFixed64( x, 14 ) ^ rotrFixed64( x, 18 ) ^ \
                  rotrFixed64( x, 41 ) )
            #define s0( x ) \
                ( rotrFixed64( x, 1 ) ^ rotrFixed64( x, 8 ) ^ ( x >> 7 ) )
            #define s1( x ) \
                ( rotrFixed64( x, 19 ) ^ rotrFixed64( x, 61 ) ^ ( x >> 6 ) )

            #define R( i )                                              \
                h( i ) += S1( e( i ) ) + Ch( e( i ), f( i ), g( i ) ) + \
                          K[ i + j ] + ( j ? blk2( i ) : blk0( i ) );   \
                d( i ) += h( i );                                       \
...
static int _Transform_Sha512( wc_Sha512 * sha512 )
{
    ...
    if( W == NULL )
    {
        W = ( word64 * ) XMALLOC( sizeof( word64 ) * 16,
                                  NULL,
...
static WC_INLINE int Sha512Final( wc_Sha512 * sha512 )
{
    ...
    XMEMSET( &local[ sha512->buffLen ],
             0,
             WC_SHA512_PAD_SIZE - sha512->buffLen );
    ...
    if( !IS_INTEL_AVX1( intel_flags ) && !IS_INTEL_AVX2( intel_flags ) )
        ...
        #if !defined( WOLFSSL_ESP32WROOM32_CRYPT ) || \
            defined( NO_WOLFSSL_ESP32WROOM32_CRYPT_HASH )
    sha512->buffer[ WC_SHA512_BLOCK_SIZE / sizeof( word64 ) - 2 ] = sha512
                                                                        ->hiLen;
    sha512->buffer[ WC_SHA512_BLOCK_SIZE / sizeof( word64 ) - 1 ] = sha512
                                                                        ->loLen;
        #endif
...
int wc_Sha512FinalRaw( wc_Sha512 * sha512, byte * hash )
{
    ...
    if( sha512 == NULL || hash == NULL )
    {
        return BAD_FUNC_ARG;
            ...
            #ifdef LITTLE_ENDIAN_ORDER
    ByteReverseWords64( ( word64 * ) digest,
                        ( word64 * ) sha512->digest,
...
int wc_Sha512Final( wc_Sha512 * sha512, byte * hash )
{
    ...
    {
                #if defined( HAVE_INTEL_QA )
        return IntelQaSymSha512( &sha512->asyncDev,
                                 hash,
                                 NULL,
                                 WC_SHA512_DIGEST_SIZE );
                #endif
...
int wc_Sha384Update( wc_Sha384 * sha384, const byte * data, word32 len )
{
    if( sha384 == NULL || ( data == NULL && len > 0 ) )
    {
        return BAD_FUNC_ARG;
                ...
                #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                    defined( WC_ASYNC_ENABLE_SHA384 )
...
int wc_Sha384Final( wc_Sha384 * sha384, byte * hash )
{
    ...
    {
                    #if defined( HAVE_INTEL_QA )
        return IntelQaSymSha384( &sha384->asyncDev,
                                 hash,
                                 NULL,
                                 WC_SHA384_DIGEST_SIZE );
                    #endif
...
int wc_InitSha384_ex( wc_Sha384 * sha384, void * heap, int devId )
{
    ...
    sha384->W = NULL;
                ...
                #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                    defined( WC_ASYNC_ENABLE_SHA384 )
    ret = wolfAsync_DevCtxInit( &sha384->asyncDev,
                                WOLFSSL_ASYNC_MARKER_SHA384,
                                sha384->heap,
                                devId );
                #else
    ( void ) devId;
