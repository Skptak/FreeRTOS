...
static const byte WC_PKCS12_ENCRYPTED_OID[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7,
                                                0x0D, 0x01, 0x07, 0x06 };
static const byte WC_PKCS12_DATA_OID[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7,
                                           0x0D, 0x01, 0x07, 0x01 };
static const byte WC_PKCS12_CertBag_Type1_OID[] = { 0x2A, 0x86, 0x48, 0x86,
                                                    0xF7, 0x0D, 0x01, 0x09,
                                                    0x16, 0x01 };
static const byte WC_PKCS12_CertBag_OID[] = { 0x2A, 0x86, 0x48, 0x86,
                                              0xF7, 0x0D, 0x01, 0x0c,
                                              0x0a, 0x01, 0x03 };
static const byte WC_PKCS12_KeyBag_OID[] = { 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D,
                                             0x01, 0x0c, 0x0a, 0x01, 0x01 };
static const byte WC_PKCS12_ShroudedKeyBag_OID[] = { 0x2A, 0x86, 0x48, 0x86,
                                                     0xF7, 0x0D, 0x01, 0x0c,
                                                     0x0a, 0x01, 0x02 };
                           ...
                           int maxIdx )
{
    ...
    safe = ( AuthenticatedSafe * ) XMALLOC( sizeof( AuthenticatedSafe ),
                                            pkcs12->heap,
                                            DYNAMIC_TYPE_PKCS );
    if( safe == NULL )
    {
    ...
    *idx = localIdx;
    ...
    input = safe->data;
    {
        ...
        while( ( int ) localIdx < CISz )
        {
            ...
            ci = ( ContentInfo * ) XMALLOC( sizeof( ContentInfo ),
                                            pkcs12->heap,
                                            DYNAMIC_TYPE_PKCS );
            if( ci == NULL )
            {
                        ...
                        word32 totalSz )
{
    ...
    mac = ( MacData * ) XMALLOC( sizeof( MacData ),
                                 pkcs12->heap,
                                 DYNAMIC_TYPE_PKCS );
    if( mac == NULL )
    {
    ...
    mac->digestSz = size;
    mac->digest = ( byte * ) XMALLOC( mac->digestSz,
                                      pkcs12->heap,
                                      DYNAMIC_TYPE_DIGEST );
    ...
    mac->saltSz = size;
    mac->salt = ( byte * ) XMALLOC( mac->saltSz,
                                    pkcs12->heap,
                                    DYNAMIC_TYPE_SALT );
                             ...
                             word32 pswSz )
{
    ...
    if( ( ret = wc_PKCS12_create_mac( pkcs12,
                                      data,
                                      dataSz,
                                      psw,
                                      pswSz,
                     ...
                     WC_DerCertList ** ca )
{
    ...
    *pkey = NULL;
    ...
    if( pkcs12->signData != NULL )
    {
        if( ( ret = wc_PKCS12_verify( pkcs12,
                                      pkcs12->safe->data,
                                      pkcs12->safe->dataSz,
                                      ( byte * ) psw,
                                      pswSz ) ) != 0 )
    ...
    for( i = 0; i < pkcs12->safe->numCI; i++ )
    {
        ...
        if( ci->type == WC_PKCS12_ENCRYPTED_DATA )
        {
            ...
            {
                WOLFSSL_MSG(
                    "Not PKCS12 DATA object or get object parse error" );
                ERROR_OUT( ASN_PARSE_E, exit_pk12par );
            }
        ...
        while( ( int ) idx < totalSz )
        {
            ...
            switch( oid )
            {
                ...
                case WC_PKCS12_CertBag: /* 669 */
                {
                    ...
                    /* get cert bag type */
                    if( ( ret = GetSequence( data, &idx, &size, ci->dataSz ) ) <
                        0 )
                    {
                        goto exit_pk12par;
                    }
                    ...
                    node = ( WC_DerCertList * ) XMALLOC( sizeof(
                                                             WC_DerCertList ),
                                                         pkcs12->heap,
                                                         DYNAMIC_TYPE_PKCS );
                    if( node == NULL )
                                 ...
                                 int itt )
{
    ...
    if( vAlgo < 0 )
    {
        ...
        ret = wc_CreatePKCS8Key( out + tmpIdx,
                                 &sz,
                                 key,
                                 keySz,
                                 algoID,
                                     ...
                                     int passSz )
{
    ...
    ret = wc_PKCS12_shroud_key( pkcs12,
                                rng,
                                NULL,
                                &length,
                                key,
                                         ...
                                         int iter )
{
    ...
    *keyCiSz = 0;
    ...
    ret = wc_PKCS12_create_key_bag( pkcs12,
                                    rng,
                                    NULL,
                                    &keyBufSz,
                                    key,
    ...
    #endif
    ret = wc_PKCS12_encrypt_content( pkcs12,
                                     rng,
                                     NULL,
                                     keyCiSz,
                                     NULL,
                                          ...
                                          int iter )
{
    ...
    if( ca != NULL )
    {
        ...
        while( current != NULL )
        {
            ret = wc_PKCS12_create_cert_bag( pkcs12,
                                             NULL,
                                             &curBufSz,
                                             current->buffer,
                                             current->bufferSz );
    ...
    sz = certBufSz - idx;
    if( ( ret = wc_PKCS12_create_cert_bag( pkcs12,
                                           certBuf + idx,
                                           &sz,
                                           cert,
                                           certSz ) ) < 0 )
                               ...
                               int iter )
{
    ...
    safeData = ( byte * ) XMALLOC( safeDataSz,
                                   pkcs12->heap,
    ...
    innerData = ( byte * ) XMALLOC( innerDataSz,
                                    pkcs12->heap,
                                    DYNAMIC_TYPE_PKCS );
    if( innerData == NULL )
    {
                              ...
                              void * heap )
{
    ...
    /**** add private key bag ****/
    keyCi = PKCS12_create_key_content( pkcs12,
                                       nidKey,
                                       &keyCiSz,
                                       &rng,
                                       pass,
    ...
    certCi = PKCS12_create_cert_content( pkcs12,
                                         nidCert,
                                         ca,
                                         cert,
                                         certSz,
    ...
    if( macIter > 0 )
    {
        ...
        if( ( ret = wc_RNG_GenerateBlock( &rng, mac->salt, mac->saltSz ) ) !=
            0 )
        {
        ...
        ret = wc_PKCS12_create_mac( pkcs12,
                                    pkcs12->safe->data,
                                    pkcs12->safe->dataSz,
                                    ( const byte * ) pass,
                                    passSz,
