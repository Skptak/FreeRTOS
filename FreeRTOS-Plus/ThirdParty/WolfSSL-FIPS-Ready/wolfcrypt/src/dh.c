 ...
 * NO_RSA:              Overall control of DH                 default: on (not
defined)
...
int wc_InitDhKey_ex( DhKey * key, void * heap, int devId )
{
    ...
    if( mp_init_multi( &key->p, &key->g, &key->q, &key->pub, &key->priv, NULL ) !=
        MP_OKAY )
    #else
    if( mp_init_multi( &key->p, &key->g, &key->q, NULL, NULL, NULL ) !=
        MP_OKAY )
    #endif
        return MEMORY_E;
...
static word32 DiscreteLogWorkFactor( word32 n )
{
    ...
    if( n < 5 )
        ...
        return ( word32 ) ( 2.4 * XPOW( ( double ) n, 1.0 / 3.0 ) *
                                XPOW( XLOG( ( double ) n ), 2.0 / 3.0 ) -
                            5 );
}
        #endif /* WOLFSSL_DH_CONST*/

        /* if not using fixed points use DiscreteLogWorkFactor function for
           unusual size otherwise round up on size needed */
        #ifndef WOLFSSL_DH_CONST
                                      ...
                                      word32 * pubSz )
{
    ...
    return ( ret != 0 ) ? ret
                        : GeneratePublicDh( key, priv, *privSz, pub, pubSz );
}
                                       ...
                                       word32 * pubSz )
{
    ...
    if( pBits == 768 || pBits == 1024 || pBits == 1536 || pBits == 2048 ||
        pBits == 3072 || pBits == 4096 )
    {
        mp_int x;
        ...
        if( ret == MP_OKAY )
            ret = IntelQaDhKeyGen( &key->asyncDev,
                                   &key->p.raw,
                                   &key->g.raw,
                                   &x.raw,
                                   pub,
                       ...
                       word32 privSz )
{
    ...
    #ifdef WOLFSSL_SMALL_STACK
    publicKey = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                      key->heap,
                                      DYNAMIC_TYPE_DH );
    if( publicKey == NULL )
        return MEMORY_E;
    privateKey = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                       key->heap,
                                       DYNAMIC_TYPE_DH );
    if( privateKey == NULL )
    {
    ...
    checkKey = ( mp_int * ) XMALLOC( sizeof( mp_int ),
                                     key->heap,
                                     DYNAMIC_TYPE_DH );
    if( checkKey == NULL )
    {
    ...
    if( mp_init_multi( publicKey, privateKey, checkKey, NULL, NULL, NULL ) !=
        MP_OKAY )
    {
    #ifdef WOLFSSL_SMALL_STACK
        XFREE( privateKey, key->heap, DYNAMIC_TYPE_DH );
    ...
    if( ret == 0 )
    {
        ...
        {
    #ifndef WOLFSSL_SP_MATH
            if( mp_exptmod( &key->g, privateKey, &key->p, checkKey ) !=
                MP_OKAY )
                ret = MP_EXPTMOD_E;
    #else
            ret = WC_KEY_SIZE_E;
                             ...
                             word32 pubSz )
{
        ...
        pBits == 3072 || pBits == 4096 )
    {
        ret = wc_mp_to_bigint( &key->p, &key->p.raw );
        if( ret == MP_OKAY )
            ret = IntelQaDhAgree( &key->asyncDev,
                                  &key->p.raw,
                                  agree,
                                  agreeSz,
                                  priv,
...
{
    mp_int tmp, tmp2;
    int groupSz = 0, bufSz = 0, primeCheckCount = 0, primeCheck = MP_NO,
        ret = 0;
    ...
    if( ret == 0 )
    {
        ...
        if( mp_init_multi( &tmp, &tmp2, &dh->p, &dh->q, &dh->g, 0 ) != MP_OKAY )
        {
            ret = MP_INIT_E;
        }
    }
