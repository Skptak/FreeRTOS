                             ...
                             byte contextLen )
{
    ...
    if( ret == 0 )
    {
         ...
         */
        ret = ge448_double_scalarmult_vartime( &R,
                                               h,
                                               &A,
                           ...
                           byte contextLen )
{
    ...
    if( ret == 0 )
    {
        ret = wc_ed448ph_verify_hash( sig,
                                      sigLen,
                                      hash,
                                      sizeof( hash ),
                                      res,
...
int wc_ed448_import_public( const byte * in, word32 inLen, ed448_key * key )
{
    ...
    if( ret == 0 )
    {
        ...
        else if( in[ 0 ] == 0x04 && inLen > 2 * ED448_PUB_KEY_SIZE )
        {
            ...
            ret = ge448_compress_key( key->p,
                                      in + 1,
                                      in + 1 + ED448_PUB_KEY_SIZE );
            if( ret == 0 )
                key->pubKeySet = 1;
        }
                                 ...
                                 ed448_key * key )
{
    ...
    /* key size check */
    if( ( ret == 0 ) &&
        ( ( privSz < ED448_KEY_SIZE ) || ( pubSz < ED448_PUB_KEY_SIZE ) ) )
    {
        ret = BAD_FUNC_ARG;
    }
