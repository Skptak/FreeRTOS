...
static CK_OBJECT_CLASS privKeyClass = CKO_PRIVATE_KEY;
         ...
         ( defined( HAVE_AESGCM ) || defined( HAVE_AES_CBC ) ) ) || \
        !defined( NO_HMAC ) ||                                      \
        ( defined( HAVE_ECC ) && !defined( NO_PKCS11_ECDH ) )
                         ...
                         int userPinSz )
{
    ...
    if( ret == 0 )
    {
        if( slotId < 0 )
        {
            ...
            if( ret == 0 )
            {
                slot = ( CK_SLOT_ID * ) XMALLOC( slotCnt * sizeof( *slot ),
                                                 dev->heap,
                              ...
                              int readWrite )
{
    ...
    if( ret == 0 )
    {
        ...
        else
        {
            ...
            {
                rv = token->func->C_Login( session->handle,
                                           CKU_USER,
                                           token->userPin,
                                           token->userPinSz );
                if( rv != CKR_OK )
                                  ...
                                  int idLen )
{
    ...
    rv = session->func->C_CreateObject( session->handle,
                                        keyTemplate,
                                        keyTmplCnt,
                                        key );
    if( rv != CKR_OK )
    ...
    if( ret == 0 )
    {
        switch( type )
        {
            ...
            case PKCS11_KEY_TYPE_AES_GCM:
            {
                ...
                if( ret == 0 )
                {
                    ret = Pkcs11CreateSecretKey( &privKey,
                                                 &session,
                                                 CKK_AES,
            ...
            case PKCS11_KEY_TYPE_RSA:
            {
                ...
                if( ret == 0 )
                    ret = Pkcs11CreateRsaPrivateKey( &privKey,
                                                     &session,
                                                     rsaKey );
                if( ret == 0 && clear )
                {
            ...
            case PKCS11_KEY_TYPE_EC:
            {
                ...
                if( ret == 0 )
                {
                    ret = Pkcs11CreateEccPrivateKey( &privKey,
                                                     &session,
                                                     eccKey,
                                                     CKA_DERIVE );
                }
                ...
                if( ret == 0 || ret == NOT_COMPILED_IN )
                {
                    ...
                    if( ret2 == 0 )
                    {
                        ret2 = Pkcs11CreateEccPrivateKey( &privKey,
                                                          &session,
                                                          eccKey,
                                                          CKA_SIGN );
                    }
                              ...
                              int idLen )
{
    ...
    rv = session->func->C_FindObjectsInit( session->handle,
                                           keyTemplate,
                                           keyTmplCnt );
    if( rv != CKR_OK )
        ret = WC_HW_E;
...
static int Pkcs11RsaPublic( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    if( ret == 0 )
    {
        ...
        {
            ret = Pkcs11FindKeyById( &publicKey,
                                     CKO_PUBLIC_KEY,
                                     CKK_RSA,
                                     session,
                                     info->pk.rsa.key->id,
...
static int Pkcs11RsaPrivate( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    if( ret == 0 )
    {
        ...
        {
            ret = Pkcs11CreateRsaPrivateKey( &privateKey,
                                             session,
                                             info->pk.rsa.key );
        }
        else if( info->pk.rsa.key->idLen > 0 )
        {
            ret = Pkcs11FindKeyById( &privateKey,
                                     CKO_PRIVATE_KEY,
                                     CKK_RSA,
                                     session,
                                     info->pk.rsa.key->id,
        ...
        {
            ret = Pkcs11FindRsaKey( &privateKey,
                                    CKO_PRIVATE_KEY,
                                    session,
                                    info->pk.rsa.key );
        }
...
static int Pkcs11Rsa( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    rv = session->func->C_GetMechanismInfo( session->slotId,
                                            CKM_RSA_X_509,
                                            &mechInfo );
    if( rv != CKR_OK )
        ret = NOT_COMPILED_IN;
                                  ...
                                  CK_OBJECT_HANDLE pubKey )
{
    ...
    int modSz, expSz;
    CK_ATTRIBUTE tmpl[] = { { CKA_MODULUS, NULL_PTR, 0 },
                            { CKA_PUBLIC_EXPONENT, NULL_PTR, 0 } };
    CK_ULONG tmplCnt = sizeof( tmpl ) / sizeof( *tmpl );
    CK_RV rv;

    rv = session->func->C_GetAttributeValue( session->handle,
                                             pubKey,
                                             tmpl,
                                             tmplCnt );
    if( rv != CKR_OK )
    ...
    if( ret == 0 )
    {
        ...
        mod = ( unsigned char * ) XMALLOC( modSz,
                                           key->heap,
                                           DYNAMIC_TYPE_TMP_BUFFER );
        if( mod == NULL )
            ret = MEMORY_E;
    ...
    if( ret == 0 )
    {
        exp = ( unsigned char * ) XMALLOC( expSz,
                                           key->heap,
                                           DYNAMIC_TYPE_TMP_BUFFER );
        if( exp == NULL )
            ret = MEMORY_E;
...
static int Pkcs11RsaKeyGen( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    CK_ATTRIBUTE privKeyTmpl[] = { { CKA_DECRYPT, &ckTrue, sizeof( ckTrue ) },
    ...
    if( ret == 0 )
    {
        ...
        rv = session->func->C_GenerateKeyPair( session->handle,
                                               &mech,
                                               pubKeyTmpl,
                                               pubTmplCnt,
                                               privKeyTmpl,
    ...
    if( ret != 0 && privKey != NULL_PTR )
        ret = ( int ) session->func->C_DestroyObject( session->handle,
                                                      privKey );
                             ...
                             ecc_key * eccKey )
{
    ...
    if( ret == 0 && keyClass == CKO_PUBLIC_KEY )
    {
        ...
        ecPoint = ( unsigned char * ) XMALLOC( len,
                                               eccKey->heap,
                                               DYNAMIC_TYPE_ECC );
        if( ecPoint == NULL )
            ret = MEMORY_E;
                                  ...
                                  CK_OBJECT_HANDLE pubKey )
{
    ...
    if( ret == 0 )
    {
        ...
        point = ( unsigned char * ) XMALLOC( pointSz,
                                             key->heap,
                                             DYNAMIC_TYPE_ECC );
        if( point == NULL )
            ret = MEMORY_E;
    ...
    if( ret == 0 )
    {
        ...
        ret = wc_ecc_import_point_der( point + i,
                                       pointSz - i,
                                       curveIdx,
                                       &key->pubkey );
    }
...
{
    int ret = 0;
    CK_ATTRIBUTE tmpl[] = { { CKA_VALUE, NULL_PTR, 0 } };
    CK_ULONG tmplCnt = sizeof( tmpl ) / sizeof( *tmpl );
    CK_RV rv;
...
static int Pkcs11ECDH( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    if( ret == 0 )
    {
        ...
        if( ( sessionKey = !mp_iszero( &info->pk.ecdh.private_key->k ) ) )
            ret = Pkcs11CreateEccPrivateKey( &privateKey,
                                             session,
                                             info->pk.ecdh.private_key,
                                             CKA_DERIVE );
        else if( info->pk.ecdh.private_key->idLen > 0 )
        {
            ret = Pkcs11FindKeyById( &privateKey,
                                     ...
                                     CKK_EC,
                                     session,
                                     info->pk.ecdh.private_key->id,
                                     info->pk.ecdh.private_key->idLen );
        }
        else
        {
            ret = Pkcs11FindEccKey( &privateKey,
                                    CKO_PRIVATE_KEY,
                                    session,
    ...
    if( ret == 0 )
    {
        ...
        if( ret == LENGTH_ONLY_E )
        {
            ...
            ret = wc_ecc_export_x963( info->pk.ecdh.public_key,
                                      point,
                                      &pointLen );
        }
    }
    ...
    if( ret == 0 )
    {
        ...
        rv = session->func->C_DeriveKey( session->handle,
                                         &mech,
                                         privateKey,
                                         tmpl,
                                         tmplCnt,
    ...
    if( ret == 0 )
    {
        ret = Pkcs11ExtractSecret( session,
                                   secret,
                                   info->pk.ecdh.out,
                                   info->pk.ecdh.outlen );
    }
                               ...
                               ecc_key * key )
{
    ...
    if( ret == 0 )
    {
        ...
        curveId = wc_ecc_get_curve_id_from_oid(
            oid + 2,
...
static int Pkcs11ECDSA_Verify( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    if( ret == 0 )
    {
        ...
        ret = Pkcs11CreateEccPublicKey( &publicKey,
                                        session,
...
static int Pkcs11RandomBlock( Pkcs11Session * session, wc_CryptoInfo * info )
{
    ...
    rv = session->func->C_GenerateRandom( session->handle,
                                          info->rng.out,
                                          info->rng.sz );
    if( rv != CKR_OK )
        ret = WC_HW_E;
