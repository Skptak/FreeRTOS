...
static double wc_func_time[ WC_FUNC_COUNT ] = {
    0,
};
    ...
    #endif
{
    ...
    #endif
    {
        ...
        if( wc_LockMutex( &debug_mutex ) != 0 )
        {
            ...
            XSNPRINTF( buffer,
                       sizeof( buffer ),
                       "wolfSSL error occurred, error = %d",
                       error );
        }
        else
        {
            ...
            if( error != WANT_READ && error != WANT_WRITE )
            {
                ...
                XSNPRINTF( buffer,
                           ...
                           line,
                           file );
                if( wc_AddErrorNode( error, line, buffer, ( char * ) file ) !=
                    0 )
                {
...
int wc_AddErrorNode( int error, int line, char * buf, char * file )
{
    ...
    #else
    struct wc_error_queue * err;
    err = ( struct wc_error_queue * ) XMALLOC( sizeof( struct wc_error_queue ),
                                               wc_error_heap,
                                               DYNAMIC_TYPE_LOG );
    if( err == NULL )
    {
 ...
 * function. */
void wc_ERR_print_errors_cb( int ( *cb )( const char * str,
                                          size_t len,
                                          void * u ),
                             void * u )
{
