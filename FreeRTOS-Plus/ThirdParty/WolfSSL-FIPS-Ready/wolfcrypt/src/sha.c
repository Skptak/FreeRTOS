...
int wc_ShaUpdate( wc_Sha * sha, const byte * data, word32 len )
{
    ...
    if( ret == 0 )
    {
        ret = wc_Stm32_Hash_Update( &sha->stmCtx,
...
int wc_ShaFinal( wc_Sha * sha, byte * hash )
{
    ...
    if( ret == 0 )
    {
        ret = wc_Stm32_Hash_Final( &sha->stmCtx,
                                   HASH_AlgoSelection_SHA1,
                                   hash,
                                   WC_SHA_DIGEST_SIZE );
        wolfSSL_CryptHwMutexUnLock();
...
int wc_ShaFinal( wc_Sha * sha, byte * hash )
{
    ...
    return wc_InitSha( sha ); /* reset state */
                ...
                #define USE_SHA_SOFTWARE_IMPL    /* Only for API's, actual \
                                                    transform is here */

                #define XTRANSFORM( S, B )       Transform( ( S ), ( B ) )
                #define XTRANSFORM_LEN( S, B, L ) \
                    Transform_Len( ( S ), ( B ), ( L ) )
...
static int Transform_Len( wc_Sha * sha, const byte * data, word32 len )
{
    ...
    if( ret == 0 )
    {
        ...
        {
                #ifdef FREESCALE_MMCAU_CLASSIC_SHA
            cau_sha1_hash_n( ( byte * ) data,
                             len / WC_SHA_BLOCK_SIZE,
                             sha->digest );
                #else
            MMCAU_SHA1_HashN( ( byte * ) data,
                              len / WC_SHA_BLOCK_SIZE,
                              ( uint32_t * ) sha->digest );
                #endif
        }
    ...
    if( ( sha->loLen += len ) < tmp )
        sha->hiLen++; /* carry low to high */
                ...
                /* Check if custom wc_Sha transform is used */
                    ...
                    /* (R0+R1), R2, R3, R4 are the different operations used in
                     * SHA1 */
                    #define R0( v, w, x, y, z, i )                           \
                        ( z ) += f1( ( w ), ( x ), ( y ) ) + blk0( ( i ) ) + \
                                 0x5A827999 + rotlFixed( ( v ), 5 );         \
                        ( w ) = rotlFixed( ( w ), 30 );
                    ...
                    #define R2( v, w, x, y, z, i )                           \
                        ( z ) += f2( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + \
                                 0x6ED9EBA1 + rotlFixed( ( v ), 5 );         \
                        ( w ) = rotlFixed( ( w ), 30 );
                    #define R3( v, w, x, y, z, i )                           \
                        ( z ) += f3( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + \
                                 0x8F1BBCDC + rotlFixed( ( v ), 5 );         \
                        ( w ) = rotlFixed( ( w ), 30 );
                    #define R4( v, w, x, y, z, i )                           \
                        ( z ) += f4( ( w ), ( x ), ( y ) ) + blk1( ( i ) ) + \
                                 0xCA62C1D6 + rotlFixed( ( v ), 5 );         \
                        ( w ) = rotlFixed( ( w ), 30 );
...
int wc_InitSha_ex( wc_Sha * sha, void * heap, int devId )
{
    ...
    if( ret != 0 )
        return ret;
                    ...
                    defined( WC_ASYNC_ENABLE_SHA )
    ret = wolfAsync_DevCtxInit( &sha->asyncDev,
                                WOLFSSL_ASYNC_MARKER_SHA,
                                sha->heap,
                                devId );
                #else
    ( void ) devId;
...
int wc_ShaUpdate( wc_Sha * sha, const byte * data, word32 len )
{
    ...
    while( len >= WC_SHA_BLOCK_SIZE )
    {
        word32 * local32 = sha->buffer;
                         ...
                         * properly aligned */
                        /* Little Endian requires byte swap, so can't use data
                         * directly */
                    #if defined( WC_HASH_DATA_ALIGNMENT ) && \
...
int wc_ShaFinal( wc_Sha * sha, byte * hash )
{
    ...
    XMEMCPY( &local[ WC_SHA_PAD_SIZE + sizeof( word32 ) ],
             &sha->loLen,
             sizeof( word32 ) );
