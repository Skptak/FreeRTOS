...
int wc_Chacha_SetIV( ChaCha * ctx, const byte * inIv, word32 counter )
{
    ...
    ctx->X[ CHACHA_MATRIX_CNT_IV + 1 ] = LITTLE32(
        temp[ 0 ] ); /* fixed variable from nonce */
    ctx->X[ CHACHA_MATRIX_CNT_IV + 2 ] = LITTLE32( temp[ 1 ] ); /* counter from
                                                                   nonce */
    ctx->X[ CHACHA_MATRIX_CNT_IV + 3 ] = LITTLE32( temp[ 2 ] ); /* counter from
                                                                   nonce */
...
static const word32 sigma[ 4 ] = { 0x61707865,
                                   0x3320646e,
                                   0x79622d32,
                                   0x6b206574 };
/* "expand 16-byte k" as unsigned 16 byte */
static const word32 tau[ 4 ] = { 0x61707865,
                                 0x3120646e,
                                 0x79622d36,
                                 0x6b206574 };
 ...
 */
static WC_INLINE void wc_Chacha_wordtobyte(
    word32 output[ CHACHA_CHUNK_WORDS ],
                                     ...
                                     word32 bytes )
{
    ...
    if( bytes > 0 && ctx->left > 0 )
    {
        ...
        if( ctx->left == 0 )
        {
            ctx->X[ CHACHA_MATRIX_CNT_IV ] = PLUSONE(
