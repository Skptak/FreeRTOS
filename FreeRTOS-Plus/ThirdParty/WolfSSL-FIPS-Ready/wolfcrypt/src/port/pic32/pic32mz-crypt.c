                        ...
                        int ivLen )
{
    ...
    if( ( ( size_t ) in % sizeof( word32 ) ) ||
        ( ( size_t ) out % sizeof( word32 ) ) )
    {
    ...
    if( key )
    {
        ...
        dst = ( word32 * ) KVA0_TO_KVA1(
            sa.SA_ENCKEY + ( sizeof( sa.SA_ENCKEY ) / sizeof( word32 ) ) -
            ( keyLen / sizeof( word32 ) ) );
        ...
        if( iv && ivLen > 0 )
        {
            ...
            dst = ( word32 * ) KVA0_TO_KVA1(
                sa.SA_ENCIV + ( sizeof( sa.SA_ENCIV ) / sizeof( word32 ) ) -
                ( ivLen / sizeof( word32 ) ) );
    ...
    else
    {
        ...
        dst = ( word32 * ) KVA0_TO_KVA1(
            sa.SA_AUTHIV + ( sizeof( sa.SA_AUTHIV ) / sizeof( word32 ) ) -
            ( outLen / sizeof( word32 ) ) );
...
typedef struct
{
    ...
    uint32_t dbPtr;
    int engine_ready;
    volatile bufferDescriptor bd[ PIC32MZ_MAX_BD ]
        __attribute__( ( aligned( 8 ) ) );
    securityAssociation sa __attribute__( ( aligned( 8 ) ) );
} pic32mz_desc;

static pic32mz_desc gLHDesc __attribute__( ( coherent ) );
static uint8_t gLHDataBuf[ PIC32MZ_MAX_BD ][ PIC32_BLOCK_SIZE ]
    __attribute__( ( aligned( 4 ), coherent ) );
...
static void reset_engine( int algo )
{
    ...
    gLHDesc.bd[ 0 ].BD_CTRL.SA_FETCH_EN = 1; /* Fetch the security association
                                                on the first BD */
    gLHDesc.dbPtr = 0;
    gLHDesc.currBd = 0;
    gLHDesc.msgSize = 0;
...
static void update_engine( const byte * input, word32 len, word32 * hash )
{
    ...
    while( len )
    {
        ...
        {
            /* Wrap up the buffer descriptor and enable it so the engine can
             * process */
        ...
        else
        {
            ...
            {
                /* We have more data than can be put in the buffer. Fill what we
                 * can.*/
                total = PIC32_BLOCK_SIZE - gLHDesc.dbPtr;
                XMEMCPY( &gLHDataBuf[ gLHDesc.currBd ][ gLHDesc.dbPtr ],
                         input,
                         total );
                len -= total;
        ...
        #endif
    {
        if( cache->updLen == 0 )
        {
            ...
            switch( algo )
            {
                case PIC32_ALGO_SHA256:
                {
                    const char * sha256EmptyHash = "\xe3\xb0\xc4\x42\x98\xfc"
                                                   "\x1c\x14\x9a\xfb\xf4\xc8"
                                                   "\x99\x6f\xb9"
                                                   "\x24\x27\xae\x41\xe4\x64"
                                                   "\x9b\x93\x4c\xa4\x95\x99"
                                                   "\x1b\x78\x52"
                                                   "\xb8\x55";
                ...
                case PIC32_ALGO_SHA1:
                {
                    const char * shaEmptyHash = "\xda\x39\xa3\xee\x5e\x6b\x4b"
                                                "\x0d\x32\x55\xbf\xef\x95\x60"
                                                "\x18"
                                                "\x90\xaf\xd8\x07\x09";
                ...
                case PIC32_ALGO_MD5:
                {
                    const char * md5EmptyHash = "\xd4\x1d\x8c\xd9\x8f\x00\xb2"
                                                "\x04\xe9\x80\x09\x98\xec\xf8"
                                                "\x42"
                                                "\x7e";
...
int wc_Md5Update( wc_Md5 * md5, const byte * data, word32 len )
{
    if( md5 == NULL || ( data == NULL && len > 0 ) )
        return BAD_FUNC_ARG;
    return wc_Pic32HashUpdate( &md5->cache,
                               ( byte * ) md5->buffer,
                               sizeof( md5->buffer ),
                               md5->digest,
                               MD5_DIGEST_SIZE,
...
int wc_Md5Final( wc_Md5 * md5, byte * hash )
{
    ...
    ret = wc_Pic32HashFinal( &md5->cache,
                             ( byte * ) md5->buffer,
                             md5->digest,
                             hash,
                             MD5_DIGEST_SIZE,
...
int wc_ShaUpdate( wc_Sha * sha, const byte * data, word32 len )
{
    if( sha == NULL || ( data == NULL && len > 0 ) )
        return BAD_FUNC_ARG;
    return wc_Pic32HashUpdate( &sha->cache,
                               ( byte * ) sha->buffer,
                               sizeof( sha->buffer ),
                               sha->digest,
                               SHA_DIGEST_SIZE,
...
int wc_ShaFinal( wc_Sha * sha, byte * hash )
{
    ...
    ret = wc_Pic32HashFinal( &sha->cache,
                             ( byte * ) sha->buffer,
                             sha->digest,
                             hash,
                             SHA_DIGEST_SIZE,
...
int wc_Sha256Update( wc_Sha256 * sha256, const byte * data, word32 len )
{
    if( sha256 == NULL || ( data == NULL && len > 0 ) )
        return BAD_FUNC_ARG;
    return wc_Pic32HashUpdate( &sha256->cache,
                               ( byte * ) sha256->buffer,
                               sizeof( sha256->buffer ),
                               sha256->digest,
                               SHA256_DIGEST_SIZE,
                               data,
                               len,
...
int wc_Sha256Final( wc_Sha256 * sha256, byte * hash )
{
    ...
    ret = wc_Pic32HashFinal( &sha256->cache,
                             ( byte * ) sha256->buffer,
                             sha256->digest,
                             hash,
                             SHA256_DIGEST_SIZE,
