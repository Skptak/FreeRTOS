                       ...
                       word32 mode )
{
    ...
    ROM_AESConfigSet( AES_BASE,
                      ( aes->keylen | dir |
                        ( mode == AES_CFG_MODE_CTR_NOCTR ? AES_CFG_MODE_CTR
                                                         : mode ) ) );
    ...
    else
    {
        ...
        b[ AES_BLOCK_SIZE - 1 ] = 1;
            ...
            #define FREE_ALL                                             \
                ...
                if( nonce_save )                                         \
                    XFREE( nonce_save, NULL, DYNAMIC_TYPE_TMP_BUFFER );
                           ...
                           int mode )
{
    ...
    {
        if( ( in_save = XMALLOC( RoundUp16( inSz ),
                                 NULL,
        ...
        if( ( out_save = XMALLOC( RoundUp16( inSz ),
                                  NULL,
                                  DYNAMIC_TYPE_TMP_BUFFER ) ) == NULL )
        {
            FREE_ALL;
    ...
    {
        if( ( authIn_save = XMALLOC( RoundUp16( authInSz ),
                                     NULL,
                                     DYNAMIC_TYPE_TMP_BUFFER ) ) == NULL )
        {
            FREE_ALL;
    ...
    {
        if( ( nonce_save = XMALLOC( RoundUp16( nonceSz ),
                                    NULL,
                                    DYNAMIC_TYPE_TMP_BUFFER ) ) == NULL )
        {
            FREE_ALL;
    ...
    ROM_AESReset( AES_BASE );
    ROM_AESConfigSet( AES_BASE,
                      ( aes->keylen | AES_CFG_DIR_ENCRYPT |
                        AES_CFG_CTR_WIDTH_128 | mode |
                           ...
                           int mode )
{
    ...
    ROM_AESReset( AES_BASE );
    ROM_AESConfigSet( AES_BASE,
                      ( aes->keylen | AES_CFG_DIR_DECRYPT |
                        AES_CFG_CTR_WIDTH_128 | mode |
                        ( ( mode == AES_CFG_MODE_CCM ) ? ( L | M ) : 0 ) ) );
    ROM_AESIVSet( AES_BASE, aes->reg );
    ROM_AESKey1Set( AES_BASE, aes->key, aes->keylen );
...
WOLFSSL_API int wc_GmacUpdate( Gmac * gmac,
                               ...
                               word32 authTagSz )
{
    return AesAuthEncrypt( &gmac->aes,
                           NULL,
                           NULL,
                           0,
                           iv,
