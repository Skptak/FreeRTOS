 ...
 */
    ...
    /* Check for if compiling misc.c when not needed. */
        ...
        /* #warning misc.c does not need to be compiled when using inline
         * (NO_INLINE not defined)*/
        #endif
...
WC_STATIC WC_INLINE word32 ByteReverseWord32( word32 value )
{
    ...
    return ( word32 ) __rev( value );
        #elif defined( WOLF_ALLOW_BUILTIN ) && defined( __GNUC_PREREQ ) && \
            __GNUC_PREREQ( 4, 3 )
...
{
            #if defined( WOLF_ALLOW_BUILTIN ) && defined( __GNUC_PREREQ ) && \
                __GNUC_PREREQ( 4, 3 )
    return ( word64 ) __builtin_bswap64( value );
            #elif defined( WOLFCRYPT_SLOW_WORD64 )
    return ( word64 ) ( ( word64 ) ByteReverseWord32( ( word32 ) value ) )
               << 32 |
...
{
    if( ( ( wolfssl_word ) buf | ( wolfssl_word ) mask | count ) %
            WOLFSSL_WORD_SIZE ==
        0 )
        XorWords( ( wolfssl_word * ) buf,
                  ( const wolfssl_word * ) mask,
...
WC_STATIC WC_INLINE void ForceZero( const void * mem, word32 len )
{
    ...
    word32 l = ( sizeof( word64 ) -
                 ( ( size_t ) z & ( sizeof( word64 ) - 1 ) ) ) &
               ( sizeof( word64 ) - 1 );
                                         ...
                                         int length )
{
    ...
    return compareSum;
            ...
            #define WOLFSSL_HAVE_MIN
                ...
                !defined( min ) /* so ifdef check passes */
                #define min min
            #endif
...
WC_STATIC WC_INLINE word32 min( word32 a, word32 b )
{
    return a > b ? b : a;
            ...
            #if defined( HAVE_FIPS ) && \
                !defined( max ) /* so ifdef check passes */
                #define max max
            #endif
...
{
    *wc_u32 = ( ( word32 ) wc_u24[ 0 ] << 16 ) | ( wc_u24[ 1 ] << 8 ) |
              wc_u24[ 2 ];
}
