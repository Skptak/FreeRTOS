    ...
    byte outAuthTag[ CHACHA20_POLY1305_AEAD_AUTHTAG_SIZE ] )
{
    ...
    if( !inKey || !inIV || !inPlaintext || !inPlaintextLen || !outCiphertext ||
        !outAuthTag )
    {
        return BAD_FUNC_ARG;
    }

    ret = wc_ChaCha20Poly1305_Init( &aead,
                                    inKey,
                                    inIV,
    ...
    if( ret == 0 )
        ret = wc_ChaCha20Poly1305_UpdateData( &aead,
                                              inPlaintext,
                                              outCiphertext,
                                              inPlaintextLen );
    if( ret == 0 )
    ...
    byte * outPlaintext )
{
    ...
    /* Validate function arguments */
    if( !inKey || !inIV || !inCiphertext || !inCiphertextLen || !inAuthTag ||
        !outPlaintext )
    {
        return BAD_FUNC_ARG;
    }
                              ...
                              int isEncrypt )
{
    ...
    ret = wc_Chacha_SetKey( &aead->chacha,
                            inKey,
                            CHACHA20_POLY1305_AEAD_KEYSIZE );
    if( ret == 0 )
    {
        ret = wc_Chacha_SetIV( &aead->chacha,
                               inIV,
    ...
    if( ret == 0 )
    {
        ret = wc_Chacha_Process( &aead->chacha,
                                 authKey,
                                 authKey,
                                 CHACHA20_POLY1305_AEAD_KEYSIZE );
    }
    ...
    if( ret == 0 )
    {
        ret = wc_Poly1305SetKey( &aead->poly,
                                 authKey,
                                 CHACHA20_POLY1305_AEAD_KEYSIZE );
    }
    ...
    byte outAuthTag[ CHACHA20_POLY1305_AEAD_AUTHTAG_SIZE ] )
{
    ...
    if( ret == 0 )
    {
        ret = wc_Poly1305_EncodeSizes( &aead->poly,
                                       aead->aadLen,
                                       aead->dataLen );
    }
