        ...
        /* wc_port.c includes wolfcrypt/src/port/arm/cryptoCellHash.c */
            ...
            #include <wolfssl/wolfcrypt/logging.h>
                ...
                ( !defined( WOLFSSL_RENESAS_TSIP_CRYPT ) ||          \
                  defined( NO_WOLFSSL_RENESAS_TSIP_HASH ) ) &&       \
...
static int InitSha256( wc_Sha256 * sha256 )
{
    ...
    return ret;
            ...
            /* Hardware Acceleration */
                ...
                /* in case intel instructions aren't available, plus we need the
                 * K[] global */
...
static word32 intel_flags;
                ...
                #define XTRANSFORM( S, D ) \
                    ( *Transform_Sha256_p )( ( S ), ( D ) )
                #define XTRANSFORM_LEN( S, D, L ) \
                    ( *Transform_Sha256_Len_p )( ( S ), ( D ), ( L ) )
...
int wc_InitSha256_ex( wc_Sha256 * sha256, void * heap, int devId )
{
    ...
    Sha256_SetTransform();
                    ...
                    defined( WC_ASYNC_ENABLE_SHA256 )
                                 ...
                                 word32 len )
{
    ...
    if( ret == 0 )
    {
                #if defined( WC_HASH_DATA_ALIGNMENT ) && \
                    WC_HASH_DATA_ALIGNMENT > 0
        ...
        {
                #ifdef FREESCALE_MMCAU_CLASSIC_SHA
            cau_sha256_hash_n( ( byte * ) data,
                               len / WC_SHA256_BLOCK_SIZE,
                               sha256->digest );
                #else
            MMCAU_SHA256_HashN( ( byte * ) data,
                                len / WC_SHA256_BLOCK_SIZE,
                                sha256->digest );
                #endif
        }
...
static int wc_Sha256SCE_XTRANSFORM( wc_Sha256 * sha256, const byte * data )
{
    ...
    if( WOLFSSL_SCE_SHA256_HANDLE.p_api
            ->hashUpdate( WOLFSSL_SCE_SHA256_HANDLE.p_ctrl,
                          ( word32 * ) data,
                          WC_SHA256_DIGEST_WORD_SIZE,
...
static const FLASH_QUALIFIER ALIGN32 word32 K[ 64 ] = {
    ...
    0x90BEFFFAL, 0xA4506CEBL, 0xBEF9A3F7L, 0xC67178F2L
                ...
                /* Both versions of Ch and Maj are logically the same, but with
                   the second set the compilers can recognize them better for
                   optimization */
...
static int Transform_Sha256( wc_Sha256 * sha256, const byte * data )
{
    ...
    return 0;
                    ...
                    #define SCHED1( j ) \
                        ( W[ j ] = *(   \
                              ( word32 * ) &data[ j * sizeof( word32 ) ] ) )
                    #define SCHED( j )                                   \
                        ( W[ j & 15 ] += Gamma1( W[ ( j - 2 ) & 15 ] ) + \
                                         W[ ( j - 7 ) & 15 ] +           \
                                         Gamma0( W[ ( j - 15 ) & 15 ] ) )
                    ...
                    defined( HAVE_INTEL_AVX1 ) || defined( HAVE_INTEL_AVX2 )
    {
        while( len >= WC_SHA256_BLOCK_SIZE )
        {
            word32 * local32 = sha256->buffer;
                            /* optimization to avoid memcpy if data pointer is
                             * properly aligned */
                            /* Intel transform function requires use of
                             * sha256->buffer */
                            /* Little Endian requires byte swap, so can't use
                             * data directly */
                    #if defined( WC_HASH_DATA_ALIGNMENT ) && \
                        !defined( LITTLE_ENDIAN_ORDER ) &&   \
                        !defined( HAVE_INTEL_AVX1 ) &&       \
...
int wc_Sha256Final( wc_Sha256 * sha256, byte * hash )
{
    ...
    {
                    #if defined( HAVE_INTEL_QA )
        return IntelQaSymSha256( &sha256->asyncDev,
                                 hash,
                                 NULL,
                                 WC_SHA256_DIGEST_SIZE );
                    #endif
...
int wc_InitSha224_ex( wc_Sha224 * sha224, void * heap, int devId )
{
    ...
    sha224->W = NULL;
                    ...
                    #if defined( WOLFSSL_ASYNC_CRYPT ) && \
                        defined( WC_ASYNC_ENABLE_SHA224 )
    ret = wolfAsync_DevCtxInit( &sha224->asyncDev,
                                WOLFSSL_ASYNC_MARKER_SHA224,
                                sha224->heap,
                                devId );
                    #else
    ( void ) devId;
...
int wc_Sha224Final( wc_Sha224 * sha224, byte * hash )
{
    ...
    {
                        #if defined( HAVE_INTEL_QA )
        return IntelQaSymSha224( &sha224->asyncDev,
                                 hash,
                                 NULL,
                                 WC_SHA224_DIGEST_SIZE );
                        #endif
...
void wc_Sha256Free( wc_Sha256 * sha256 )
{
    ...
    wc_DevCryptoFree( &sha256->ctx );
            ...
            #if( defined( WOLFSSL_AFALG_HASH ) &&             \
                 defined( WOLFSSL_AFALG_HASH_KEEP ) ) ||      \
                ( defined( WOLFSSL_DEVCRYPTO_HASH ) &&        \
                  defined( WOLFSSL_DEVCRYPTO_HASH_KEEP ) ) || \
                ( defined( WOLFSSL_RENESAS_TSIP_CRYPT ) &&    \
                  !defined( NO_WOLFSSL_RENESAS_TSIP_CRYPT_HASH ) )
    if( sha256->msg != NULL )
    {
