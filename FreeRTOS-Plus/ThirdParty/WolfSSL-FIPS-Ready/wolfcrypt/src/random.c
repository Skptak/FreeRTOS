...
int wc_RNG_HealthTest( int reseed,
                       ...
                       word32 outputSz )
{
    return RNG_HealthTest_fips( reseed,
                                seedA,
                                seedASz,
                                seedB,
                                seedBSz,
...
static int wc_GenerateRand_IntelRD( OS_Seed * os, byte * output, word32 sz );
        ...
        /* Start NIST DRBG code */
             ...
             * entropy you are looking for in a seed. */
                ...
                #if defined( HAVE_FIPS ) && defined( HAVE_FIPS_VERSION ) && \
                    ( HAVE_FIPS_VERSION >= 2 )
                    /* SHA-256 requires a minimum of 256-bits of entropy. The
                     * goal of 1024 will provide 4 times that. */
                    #define RNG_SECURITY_STRENGTH ( 1024 )
                #else
                    /* If not using FIPS or using old FIPS, set the number down
                     * a bit. More is better, but more is also slower. */
                    #define RNG_SECURITY_STRENGTH ( 256 )
                #endif
            #endif

            #ifndef ENTROPY_SCALE_FACTOR
                /* The entropy scale factor should be the whole number inverse
                 * of the minimum bits of entropy per bit of NDRNG output. */
                #if defined( HAVE_INTEL_RDSEED ) || defined( HAVE_INTEL_RDRAND )
                    /* The value of 2 applies to Intel's RDSEED which provides
                     * about 0.5 bits minimum of entropy per bit. */
                ...
                /* The seed block size, is the size of the output of the
                 * underlying NDRNG. This value is used for testing the output
                 * of the NDRNG. */
                #if defined( HAVE_INTEL_RDSEED ) || defined( HAVE_INTEL_RDRAND )
            ...
            /* The maximum seed size will be the seed size plus a seed block for
             * the test, and an additional half of the seed size. This
             * additional half is in case the user does not supply a nonce. A
             * nonce will be obtained from the NDRNG. */
                     ...
                     int devId )
{
    ...
    ret = wolfAsync_DevCtxInit( &rng->asyncDev,
                                WOLFSSL_ASYNC_MARKER_RNG,
                                rng->heap,
                                rng->devId );
    if( ret != 0 )
        return ret;
    ...
    if( wc_RNG_HealthTestLocal( 0 ) == 0 )
    {
        ...
        rng->drbg = ( struct DRBG * ) XMALLOC( sizeof( DRBG ),
                                               rng->heap,
                                               DYNAMIC_TYPE_RNG );
        if( rng->drbg == NULL )
        {
        ...
        typedef char drbg_data_test
            [ sizeof( rng->drbg_data ) >= sizeof( struct DRBG ) ? 1 : -1 ];
...
int wc_RNG_GenerateBlock( WC_RNG * rng, byte * output, word32 sz )
{
    ...
    if( ret == DRBG_NEED_RESEED )
    {
        if( wc_RNG_HealthTestLocal( 1 ) == 0 )
        {
            ...
            if( ret == DRBG_SUCCESS )
                ret = Hash_DRBG_Reseed( rng->drbg,
                                        newSeed + SEED_BLOCK_SZ,
                                        SEED_SZ );
            if( ret == DRBG_SUCCESS )
                ret = Hash_DRBG_Generate( rng->drbg, output, sz );
...
const FLASH_QUALIFIER byte reseedSeedA_data[] = {
    0xe6, 0x2b, 0x8a, 0x8e, 0xe8, 0xf1, 0x41, 0xb6, 0x98, 0x05, 0x66,
...
const FLASH_QUALIFIER byte seedB_data[] = {
    0xa6, 0x5a, 0xd0, 0xf3, 0x45, 0xdb, 0x4e, 0x0e, 0xff, 0xe8, 0x75,
    0xc3, 0xa2, 0xe7, 0x1f, 0x42, 0xc7, 0x12, 0x9d, 0x62, 0x0f, 0xf5,
    0xc1, 0x19, 0xa9, 0xef, 0x55, 0xf0, 0x51, 0x85, 0xe0, 0xfb, /* nonce next */
    0x85, 0x81, 0xf9, 0x31, 0x75, 0x17, 0x27, 0x6e, 0x06, 0xe9, 0x60,
    0x7d, 0xdb, 0xcb, 0xcc, 0x2e
};
...
static int wc_RNG_HealthTestLocal( int reseed )
{
    ...
    byte check[ RNG_HEALTH_TEST_CHECK_SIZE ];
            ...
            #ifdef WOLFSSL_SMALL_STACK
    check = ( byte * ) XMALLOC( RNG_HEALTH_TEST_CHECK_SIZE,
                                NULL,
    ...
    if( reseed )
    {
        ...
        byte * seedA = ( byte * ) XMALLOC( sizeof( seedA_data ),
                                           NULL,
                                           DYNAMIC_TYPE_TMP_BUFFER );
        byte * reseedSeedA = ( byte * ) XMALLOC( sizeof( reseedSeedA_data ),
                                                 NULL,
                                                 DYNAMIC_TYPE_TMP_BUFFER );
        byte * outputA = ( byte * ) XMALLOC( sizeof( outputA_data ),
                                             NULL,
                                             DYNAMIC_TYPE_TMP_BUFFER );
        ...
        else
        {
            ...
            XMEMCPY_P( reseedSeedA,
                       reseedSeedA_data,
                       sizeof( reseedSeedA_data ) );
    ...
    {
            #ifdef WOLFSSL_USE_FLASHMEM
        byte * seedB = ( byte * ) XMALLOC( sizeof( seedB_data ),
                                           NULL,
                                           DYNAMIC_TYPE_TMP_BUFFER );
        byte * outputB = ( byte * ) XMALLOC( sizeof( outputB_data ),
                                             NULL,
                                             DYNAMIC_TYPE_TMP_BUFFER );
        ...
        const byte * outputB = outputB_data;
            ...
            /* The previous test cases use a large seed instead of a seed and
             * nonce. seedB is actually from a test case with a seed and nonce,
             * and just concatenates them. The pivot point between seed and
             * nonce is byte 32, feed them into the health test separately. */
            if( ret == 0 )
            {
                ret = wc_RNG_HealthTest_ex( 0,
                                            seedB + 32,
                                            sizeof( seedB_data ) - 32,
                                            seedB,
                                            32,
                                            NULL,
                                            0,
...
static int wc_GenerateSeed_IntelRD( OS_Seed * os, byte * output, word32 sz )
{
    ...
    for( ; ( sz / sizeof( word64 ) ) > 0;
         sz -= sizeof( word64 ), output += sizeof( word64 ) )
    {
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    ...
    while( i < sz )
    {
        ...
        if( ( i + sizeof( CUSTOM_RAND_TYPE ) ) > sz ||
            ( ( wolfssl_word ) &output[ i ] % sizeof( CUSTOM_RAND_TYPE ) ) !=
                0 )
        {
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    ...
    if( !CryptAcquireContext( &os->handle,
                              0,
                              0,
                              PROV_RSA_FULL,
                              CRYPT_VERIFYCONTEXT ) )
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    ...
    return 0;
                ...
                defined( WOLFSSL_STM32_RNG_NOLIB )
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    ...
    return 0;
            ...
            #warning \
                "potential for not enough entropy, currently being used for testing"
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    int i;
    ...
    return 0;
            ...
            #warning \
                "potential for not enough entropy, currently being used for testing Deos"
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    int i;
    int seed = XTIME( 0 );
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    if( os == NULL || output == NULL || wnr_ctx == NULL || wnr_timeout < 0 )
    {
        return BAD_FUNC_ARG;
    }
...
int wc_GenerateSeed( OS_Seed * os, byte * output, word32 sz )
{
    ...
    if( blocks > 0 )
    {
        ret = WOLFSSL_SCE_TRNG_HANDLE.p_api
                  ->read( WOLFSSL_SCE_TRNG_HANDLE.p_ctrl,
                          ( uint32_t * ) output,
                          blocks );
        if( ret != SSP_SUCCESS )
        {
    ...
    if( len > 0 )
    {
        ...
        XMEMCPY( output + ( blocks * sizeof( uint32_t ) ),
                 ( byte * ) &tmp,
                 len );
    }

    ret = WOLFSSL_SCE_TRNG_HANDLE.p_api->close(
        WOLFSSL_SCE_TRNG_HANDLE.p_ctrl );
    if( ret != SSP_SUCCESS )
    {
        /* error opening TRNG driver */
        return -1;
