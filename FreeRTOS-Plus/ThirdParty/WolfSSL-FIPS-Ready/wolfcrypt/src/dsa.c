...
{
    mp_int tmp, tmp2;
    int err, msize, qsize, loop_check_prime = 0, check_prime = MP_NO;
    unsigned char * buf;
...
int wc_DsaSign( const byte * digest, byte * out, DsaKey * key, WC_RNG * rng )
{
    ...
    if( ret == 0 &&
        mp_exptmod_ex( &key->g, &k, key->q.used, &key->p, &r ) != MP_OKAY )
    {
        ret = MP_EXPTMOD_E;
    }
    ...
    if( ret == 0 &&
        mp_read_unsigned_bin( &H, digest, WC_SHA_DIGEST_SIZE ) != MP_OKAY )
        ret = MP_READ_E;
                  ...
                  int * answer )
{
    ...
    if( mp_read_unsigned_bin( &r, sig, DSA_HALF_SIZE ) != MP_OKAY ||
        mp_read_unsigned_bin( &s, sig + DSA_HALF_SIZE, DSA_HALF_SIZE ) !=
            MP_OKAY )
        ret = MP_READ_E;
    ...
    if( ret == 0 &&
        mp_read_unsigned_bin( &u1, digest, WC_SHA_DIGEST_SIZE ) != MP_OKAY )
        ret = MP_READ_E;
