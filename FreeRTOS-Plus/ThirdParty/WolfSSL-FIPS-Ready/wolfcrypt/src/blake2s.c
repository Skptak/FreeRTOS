...
static const word32 blake2s_IV[ 8 ] = { 0x6a09e667, 0xbb67ae85, 0x3c6ef372,
                      ...
                      const byte keylen )
{
...

    {
        ...
        block = ( byte * ) XMALLOC( BLAKE2S_BLOCKBYTES,
                                    NULL,
                             ...
                             const byte block[ BLAKE2S_BLOCKBYTES ] )
{
    ...
    m = ( word32 * ) XMALLOC( sizeof( word32 ) * 16,
                              NULL,
                              DYNAMIC_TYPE_TMP_BUFFER );
    ...
    v = ( word32 * ) XMALLOC( sizeof( word32 ) * 16,
                              NULL,
                              DYNAMIC_TYPE_TMP_BUFFER );
...
int blake2s_final( blake2s_state * S, byte * out, byte outlen )
{
    ...
    if( S->buflen > BLAKE2S_BLOCKBYTES )
    {
        ...
        S->buflen -= BLAKE2S_BLOCKBYTES;
        XMEMCPY( S->buf,
                 S->buf + BLAKE2S_BLOCKBYTES,
                 ( wolfssl_word ) S->buflen );
    }

    blake2s_increment_counter( S, S->buflen );
    blake2s_set_lastblock( S );
    XMEMSET( S->buf + S->buflen,
             0,
             ( wolfssl_word ) ( 2 * BLAKE2S_BLOCKBYTES - S->buflen ) );
    /* Padding */
    if( blake2s_compress( S, S->buf ) < 0 )
        return -1;
...
int main( int argc, char ** argv )
{
    ...
    for( word32 i = 0; i < KAT_LENGTH; ++i )
    {
        ...
        if( blake2s( hash, buf, key, BLAKE2S_OUTBYTES, i, BLAKE2S_KEYBYTES ) <
            0 )
        {
            puts( "error" );
            return -1;
