...
{
    WOLFSSL_EVP_CIPHER_CTX * ctx = ( WOLFSSL_EVP_CIPHER_CTX * )
        XMALLOC( sizeof *ctx, NULL, DYNAMIC_TYPE_TMP_BUFFER );
    if( ctx )
    {
                           ...
                           int inl )
{
    ...
    switch( ctx->cipherType )
    {
        ...
        case AES_256_CFB1_TYPE:
            if( ctx->enc )
                ret = wc_AesCfb1Encrypt( &ctx->cipher.aes,
                                         out,
                                         in,
                                         inl * WOLFSSL_BIT_SIZE );
            else
                ret = wc_AesCfb1Decrypt( &ctx->cipher.aes,
                                         out,
                                         in,
                                         inl * WOLFSSL_BIT_SIZE );
            break;
        ...
        case AES_256_XTS_TYPE:
            if( ctx->enc )
                ret = wc_AesXtsEncrypt( &ctx->cipher.xts,
                                        out,
                                        in,
                                        inl,
                                        ctx->iv,
                ...
                ret = wc_AesXtsDecrypt( &ctx->cipher.xts,
                                        out,
                                        in,
                                        inl,
                                        ctx->iv,
    ...
    *outl = inl;
    if( ctx->enc )
    {
        if( out )
        {
            ...
            ret = wc_AesGcmEncrypt( &ctx->cipher.aes,
                                    out,
                                    in,
                                    inl,
                                    ctx->iv,
    ...
    else
    {
        ...
        {
            /* authenticated, non-confidential data*/
            ret = wc_AesGcmDecrypt( &ctx->cipher.aes,
                                    NULL,
                                    NULL,
                                    0,
                                    ctx->iv,
                                          ...
                                          int inl )
{
    ...
    *outl = 0;
    ...
    switch( ctx->cipherType )
    {
        ...
        case AES_192_GCM_TYPE:
        case AES_256_GCM_TYPE:
            /* if out == NULL, in/inl contains the additional authenticated data
             * for GCM */
    ...
    if( ctx->bufUsed > 0 )
    { /* concatenate them if there is anything */
    ...
    switch( ctx->cipherType )
    {
        ...
        case AES_256_GCM_TYPE:
                ...
                ctx->gcmDecryptBufferLen > 0 )
            {
                ...
                ret = wc_AesGcmDecrypt( &ctx->cipher.aes,
                                        out,
                                        ctx->gcmDecryptBuffer,
                                        ctx->gcmDecryptBufferLen,
                                        ctx->iv,
                                        ctx->ivSz,
                                        ctx->authTag,
                                        ctx->authTagSz,
        ...
        default:
            ...
            else if( ctx->enc )
            {
                ...
                else if( ( ctx->bufUsed >= 0 ) && ( ctx->block_size != 1 ) )
                {
                    padBlock( ctx );
                    PRINT_BUF( ctx->buf, ctx->block_size );
                    if( evpCipherBlock( ctx, out, ctx->buf, ctx->block_size ) ==
                        0 )
                    {
...
static unsigned int cipherType( const WOLFSSL_EVP_CIPHER * cipher )
{
    if( cipher == NULL )
        ...
        return DES_CBC_TYPE;
    else if( EVP_DES_EDE3_CBC &&
             XSTRNCMP( cipher, EVP_DES_EDE3_CBC, EVP_DES_EDE3_SIZE ) == 0 )
        return DES_EDE3_CBC_TYPE;
            #if !defined( NO_DES3 )
    else if( EVP_DES_ECB && XSTRNCMP( cipher, EVP_DES_ECB, EVP_DES_SIZE ) == 0 )
        return DES_ECB_TYPE;
    else if( EVP_DES_EDE3_ECB &&
             XSTRNCMP( cipher, EVP_DES_EDE3_ECB, EVP_DES_EDE3_SIZE ) == 0 )
        return DES_EDE3_ECB_TYPE;
            ...
            #if defined( HAVE_AES_CBC )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_CBC &&
             XSTRNCMP( cipher, EVP_AES_128_CBC, EVP_AES_SIZE ) == 0 )
        return AES_128_CBC_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_CBC &&
             XSTRNCMP( cipher, EVP_AES_192_CBC, EVP_AES_SIZE ) == 0 )
        return AES_192_CBC_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_CBC &&
             XSTRNCMP( cipher, EVP_AES_256_CBC, EVP_AES_SIZE ) == 0 )
        return AES_256_CBC_TYPE;
                #endif
            #endif /* HAVE_AES_CBC */
            #if defined( HAVE_AESGCM )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_GCM &&
             XSTRNCMP( cipher, EVP_AES_128_GCM, EVP_AES_SIZE ) == 0 )
        return AES_128_GCM_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_GCM &&
             XSTRNCMP( cipher, EVP_AES_192_GCM, EVP_AES_SIZE ) == 0 )
        return AES_192_GCM_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_GCM &&
             XSTRNCMP( cipher, EVP_AES_256_GCM, EVP_AES_SIZE ) == 0 )
        return AES_256_GCM_TYPE;
                #endif
            #endif /* HAVE_AESGCM */
            #if defined( WOLFSSL_AES_COUNTER )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_CTR &&
             XSTRNCMP( cipher, EVP_AES_128_CTR, EVP_AES_SIZE ) == 0 )
        return AES_128_CTR_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_CTR &&
             XSTRNCMP( cipher, EVP_AES_192_CTR, EVP_AES_SIZE ) == 0 )
        return AES_192_CTR_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_CTR &&
             XSTRNCMP( cipher, EVP_AES_256_CTR, EVP_AES_SIZE ) == 0 )
        return AES_256_CTR_TYPE;
                #endif
            #endif /* HAVE_AES_CBC */
            #if defined( HAVE_AES_ECB )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_ECB &&
             XSTRNCMP( cipher, EVP_AES_128_ECB, EVP_AES_SIZE ) == 0 )
        return AES_128_ECB_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_ECB &&
             XSTRNCMP( cipher, EVP_AES_192_ECB, EVP_AES_SIZE ) == 0 )
        return AES_192_ECB_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_ECB &&
             XSTRNCMP( cipher, EVP_AES_256_ECB, EVP_AES_SIZE ) == 0 )
        return AES_256_ECB_TYPE;
                #endif
            #endif /*HAVE_AES_CBC */
            #if defined( WOLFSSL_AES_XTS )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_XTS &&
             XSTRNCMP( cipher, EVP_AES_128_XTS, EVP_AES_SIZE ) == 0 )
        return AES_128_XTS_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_XTS &&
             XSTRNCMP( cipher, EVP_AES_256_XTS, EVP_AES_SIZE ) == 0 )
        return AES_256_XTS_TYPE;
                #endif
            #endif /* WOLFSSL_AES_XTS */
            #if defined( WOLFSSL_AES_CFB )
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_CFB1 &&
             XSTRNCMP( cipher, EVP_AES_128_CFB1, EVP_AESCFB_SIZE ) == 0 )
        return AES_128_CFB1_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_CFB1 &&
             XSTRNCMP( cipher, EVP_AES_192_CFB1, EVP_AESCFB_SIZE ) == 0 )
        return AES_192_CFB1_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_CFB1 &&
             XSTRNCMP( cipher, EVP_AES_256_CFB1, EVP_AESCFB_SIZE ) == 0 )
        return AES_256_CFB1_TYPE;
                #endif
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_CFB8 &&
             XSTRNCMP( cipher, EVP_AES_128_CFB8, EVP_AESCFB_SIZE ) == 0 )
        return AES_128_CFB8_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_CFB8 &&
             XSTRNCMP( cipher, EVP_AES_192_CFB8, EVP_AESCFB_SIZE ) == 0 )
        return AES_192_CFB8_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_CFB8 &&
             XSTRNCMP( cipher, EVP_AES_256_CFB8, EVP_AESCFB_SIZE ) == 0 )
        return AES_256_CFB8_TYPE;
                #endif
                #ifdef WOLFSSL_AES_128
    else if( EVP_AES_128_CFB128 &&
             XSTRNCMP( cipher, EVP_AES_128_CFB128, EVP_AESCFB_SIZE ) == 0 )
        return AES_128_CFB128_TYPE;
                #endif
                #ifdef WOLFSSL_AES_192
    else if( EVP_AES_192_CFB128 &&
             XSTRNCMP( cipher, EVP_AES_192_CFB128, EVP_AESCFB_SIZE ) == 0 )
        return AES_192_CFB128_TYPE;
                #endif
                #ifdef WOLFSSL_AES_256
    else if( EVP_AES_256_CFB128 &&
             XSTRNCMP( cipher, EVP_AES_256_CFB128, EVP_AESCFB_SIZE ) == 0 )
                                                 ...
                                                 WOLFSSL_ENGINE * e )
{
    ...
    ctx = ( WOLFSSL_EVP_PKEY_CTX * ) XMALLOC( sizeof( WOLFSSL_EVP_PKEY_CTX ),
                                              NULL,
                             ...
                             size_t * keylen )
{
    ...
    if( !ctx || ctx->op != EVP_PKEY_OP_DERIVE || !ctx->pkey || !ctx->peerKey ||
        !keylen || ctx->pkey->type != ctx->peerKey->type )
    {
        return WOLFSSL_FAILURE;
    }
    switch( ctx->pkey->type )
    {
        ...
        case EVP_PKEY_EC:
            ...
            if( !ctx->peerKey->ecc->exSet ||
                !ctx->peerKey->ecc->pub_key->internal )
            {
                              ...
                              size_t inlen )
{
    ...
    switch( ctx->pkey->type )
    {
        #if !defined( NO_RSA ) && !defined( HAVE_USER_RSA )
        case EVP_PKEY_RSA:
            len = wolfSSL_RSA_private_decrypt( ( int ) inlen,
                                               ( unsigned char * ) in,
                                               out,
                                               ctx->pkey->rsa,
                                               ctx->padding );
                              ...
                              size_t inlen )
{
    ...
    switch( ctx->pkey->type )
    {
        #if !defined( NO_RSA ) && !defined( HAVE_USER_RSA )
        case EVP_PKEY_RSA:
            len = wolfSSL_RSA_public_encrypt( ( int ) inlen,
                                              ( unsigned char * ) in,
                                              out,
                                              ctx->pkey->rsa,
                                              ctx->padding );
 ...
 * The data to be signed should be hashed since the function does not hash the
 * data.
 *
 * RETURNS:
                                       ...
                                       size_t tbslen )
{
    ...
    switch( ctx->pkey->type )
    {
        #if !defined( NO_RSA ) && !defined( HAVE_USER_RSA )
        case EVP_PKEY_RSA:
            len = wolfSSL_RSA_private_encrypt( ( int ) tbslen,
                                               ( unsigned char * ) tbs,
                                               sig,
                                               ctx->pkey->rsa,
                                               ctx->padding );
                             ...
                             WOLFSSL_EVP_PKEY ** ppkey )
{
    ...
    switch( pkey->type )
    {
        ...
        case EVP_PKEY_RSA:
            pkey->rsa = wolfSSL_RSA_generate_key( ctx->nbits,
                                                  WC_RSA_EXPONENT,
                                                  NULL,
                                                  NULL );
            if( pkey->rsa )
...
int wolfSSL_EVP_PKEY_size( WOLFSSL_EVP_PKEY * pkey )
{
    ...
    switch( pkey->type )
    {
        #ifndef NO_RSA
        case EVP_PKEY_RSA:
            return ( int ) wolfSSL_RSA_size(
                ( const WOLFSSL_RSA * ) ( pkey->rsa ) );
        #endif /* !NO_RSA */
                                      ...
                                      const WOLFSSL_EVP_PKEY * b )
{
    ...
    switch( a->type )
    {
        #ifndef NO_RSA
        case EVP_PKEY_RSA:
            a_sz = ( int ) wolfSSL_RSA_size(
                ( const WOLFSSL_RSA * ) ( a->rsa ) );
            b_sz = ( int ) wolfSSL_RSA_size(
                ( const WOLFSSL_RSA * ) ( b->rsa ) );
            break;
        #endif /* !NO_RSA */
        #ifdef HAVE_ECC
        case EVP_PKEY_EC:
            if( a->ecc == NULL || a->ecc->internal == NULL || b->ecc == NULL ||
                b->ecc->internal == NULL )
            {
                return ret;
            }
...
static enum wc_HashType wolfSSL_EVP_md2macType( const WOLFSSL_EVP_MD * md )
{
    ...
    if( md != NULL )
    {
        ...
        {
            if( XSTRNCMP( ( const char * ) md,
                          ent->name,
                          XSTRLEN( ent->name ) + 1 ) == 0 )
            {
                return ent->macType;
            }
        }
                                                 ...
                                                 int keylen )
{
    ...
    if( pkey != NULL )
    {
        pkey->pkey.ptr = ( char * ) XMALLOC( keylen,
                                             NULL,
                                             DYNAMIC_TYPE_PUBLIC_KEY );
                                       ...
                                       WOLFSSL_EVP_PKEY * pkey )
{
    if( pkey->type == EVP_PKEY_HMAC )
    {
        ...
        if( wc_HmacSetKey( &ctx->hash.hmac, hashType, key, ( word32 ) keySz ) !=
            0 )
            return WOLFSSL_FAILURE;
                                 ...
                                 size_t * siglen )
{
    ...
    else if( ctx->pctx->pkey->type == EVP_PKEY_EC )
    {
        ...
        {
            /* SEQ + INT + INT */
            *siglen = ecc_sets[ ctx->pctx->pkey->ecc->group->curve_idx ].size *
                          2 +
                      8;
            return WOLFSSL_SUCCESS;
        }
    ...
    else
    {
        ...
        switch( ctx->pctx->pkey->type )
        {
            ...
            case EVP_PKEY_EC:
            {
                ...
                ecdsaSig = wolfSSL_ECDSA_do_sign( digest,
                                                  hashLen,
                                                  ctx->pctx->pkey->ecc );
                if( ecdsaSig == NULL )
                    break;
    ...
    else
    {
        ...
        switch( ctx->pctx->pkey->type )
        {
            ...
            case EVP_PKEY_EC:
            {
                ...
                if( ecdsaSig == NULL )
                    return WOLFSSL_FAILURE;
                ret = wolfSSL_ECDSA_do_verify( digest,
                                               hashLen,
                                               ecdsaSig,
                                               ctx->pctx->pkey->ecc );
                wolfSSL_ECDSA_SIG_free( ecdsaSig );
...
WOLFSSL_EVP_MD_CTX * wolfSSL_EVP_MD_CTX_new( void )
{
    ...
    ctx = ( WOLFSSL_EVP_MD_CTX * ) XMALLOC( sizeof *ctx,
                                            NULL,
                                            DYNAMIC_TYPE_OPENSSL );
    if( ctx )
    {
    ...
    switch( type )
    {
        ...
        case EVP_CTRL_AEAD_SET_IV_FIXED:
            if( arg == -1 )
            {
                ...
                ret = wolfSSL_EVP_CIPHER_CTX_set_iv( ctx,
                                                     ( byte * ) ptr,
                                                     ctx->ivSz );
            }
            else
        ...
        case EVP_CTRL_GCM_IV_GEN:
            ...
            if( ( ret = wc_AesGcmSetExtIV( &ctx->cipher.aes,
                                           ctx->iv,
                                           ctx->ivSz ) ) != 0 )
            {
                            ...
                            int enc )
{
        ...
        ( type && XSTRNCMP( type, EVP_AES_128_CBC, EVP_AES_SIZE ) == 0 ) )
    {
        ...
        if( key )
        {
            ret = AesSetKey_ex( &ctx->cipher.aes,
                                key,
                                ctx->keyLen,
                                iv,
        ...
        ( type && XSTRNCMP( type, EVP_AES_256_GCM, EVP_AES_SIZE ) == 0 ) )
    {
        ...
        if( enc == 0 || enc == 1 )
            ctx->enc = enc ? 1 : 0;
                    ...
                    #endif /* WOLFSSL_AES_256 */
                #endif     /* HAVE_AESGCM */
            #endif /*!HAVE_FIPS && !HAVE_SELFTEST ||(HAVE_FIPS_VERSION && \
                      HAVE_FIPS_VERSION > 2)*/
            #ifdef WOLFSSL_AES_COUNTER
                #ifdef WOLFSSL_AES_128
        ...
        ( type && XSTRNCMP( type, EVP_AES_128_XTS, EVP_AES_SIZE ) == 0 ) )
    {
        ...
        if( key )
        {
            ret = wc_AesXtsSetKey( &ctx->cipher.xts,
                                   key,
                                   ctx->keyLen,
                                   ctx->enc ? AES_ENCRYPTION : AES_DECRYPTION,
                                   NULL,
        ...
        ( type && XSTRNCMP( type, EVP_DES_CBC, EVP_DES_SIZE ) == 0 ) )
    {
        ...
        if( key )
        {
            ret = wc_Des_SetKey( &ctx->cipher.des,
                                 key,
                                 iv,
               ...
               XSTRNCMP( type, EVP_DES_EDE3_CBC, EVP_DES_EDE3_SIZE ) == 0 ) )
    {
        ...
        if( key )
        {
            ret = wc_Des3_SetKey( &ctx->cipher.des3,
                                  key,
                                  iv,
                                  ctx->enc ? DES_ENCRYPTION : DES_DECRYPTION );
            if( ret != 0 )
        ...
        ( type && XSTRNCMP( type, EVP_IDEA_CBC, EVP_IDEA_SIZE ) == 0 ) )
    {
        ...
        if( key )
        {
            ret = wc_IdeaSetKey( &ctx->cipher.idea,
                                 key,
                                 ( word16 ) ctx->keyLen,
                                 iv,
                        ...
                        word32 len )
{
    ...
    if( ctx == NULL || src == NULL ||
        ( dst == NULL && ctx->cipherType != AES_128_GCM_TYPE &&
          ctx->cipherType != AES_192_GCM_TYPE &&
          ctx->cipherType != AES_256_GCM_TYPE ) )
    {
                            ...
                            const WOLFSSL_EVP_MD * md )
{
    ...
    typedef char
        async_test[ WC_ASYNC_DEV_SIZE >= sizeof( WC_ASYNC_DEV ) ? 1 : -1 ];
                              ...
                              size_t sz )
{
    ...
    switch( macType )
    {
        case WC_HASH_TYPE_MD4:
        #ifndef NO_MD4
            wolfSSL_MD4_Update( ( MD4_CTX * ) &ctx->hash,
                                data,
                                ( unsigned long ) sz );
            ret = WOLFSSL_SUCCESS;
        #endif
        ...
        case WC_HASH_TYPE_MD5:
        #ifndef NO_MD5
            ret = wolfSSL_MD5_Update( ( MD5_CTX * ) &ctx->hash,
                                      data,
                                      ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA:
        #ifndef NO_SHA
            ret = wolfSSL_SHA_Update( ( SHA_CTX * ) &ctx->hash,
                                      data,
                                      ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA224:
        #ifdef WOLFSSL_SHA224
            ret = wolfSSL_SHA224_Update( ( SHA224_CTX * ) &ctx->hash,
                                         data,
                                         ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA256:
        #ifndef NO_SHA256
            ret = wolfSSL_SHA256_Update( ( SHA256_CTX * ) &ctx->hash,
                                         data,
                                         ( unsigned long ) sz );
        #endif /* !NO_SHA256 */
            break;
        case WC_HASH_TYPE_SHA384:
        #ifdef WOLFSSL_SHA384
            ret = wolfSSL_SHA384_Update( ( SHA384_CTX * ) &ctx->hash,
                                         data,
                                         ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA512:
        #ifdef WOLFSSL_SHA512
            ret = wolfSSL_SHA512_Update( ( SHA512_CTX * ) &ctx->hash,
                                         data,
                                         ( unsigned long ) sz );
        #endif /* WOLFSSL_SHA512 */
            break;
        case WC_HASH_TYPE_SHA3_224:
        #if defined( WOLFSSL_SHA3 ) && !defined( WOLFSSL_NOSHA3_224 )
            ret = wolfSSL_SHA3_224_Update( ( SHA3_224_CTX * ) &ctx->hash,
                                           data,
                                           ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA3_256:
        #if defined( WOLFSSL_SHA3 ) && !defined( WOLFSSL_NOSHA3_256 )
            ret = wolfSSL_SHA3_256_Update( ( SHA3_256_CTX * ) &ctx->hash,
                                           data,
                                           ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA3_384:
        #if defined( WOLFSSL_SHA3 )
            ret = wolfSSL_SHA3_384_Update( ( SHA3_384_CTX * ) &ctx->hash,
                                           data,
                                           ( unsigned long ) sz );
        #endif
            break;
        case WC_HASH_TYPE_SHA3_512:
        #if defined( WOLFSSL_SHA3 ) && !defined( WOLFSSL_NOSHA3_512 )
            wolfSSL_SHA3_512_Update( ( SHA3_512_CTX * ) &ctx->hash,
                                     data,
                                     ( unsigned long ) sz );
        #endif
            break;
...
int wolfSSL_EVP_PKEY_set1_RSA( WOLFSSL_EVP_PKEY * pkey, WOLFSSL_RSA * key )
{
    ...
    if( key->ownRng == 0 )
    {
        if( wc_RsaSetRNG( ( RsaKey * ) ( pkey->rsa->internal ),
                          &( pkey->rng ) ) != 0 )
        {
...
int wolfSSL_EVP_PKEY_assign( WOLFSSL_EVP_PKEY * pkey, int type, void * key )
{
    ...
    switch( type )
    {
        ...
        case EVP_PKEY_DSA:
            ...
            #ifdef HAVE_ECC
        case EVP_PKEY_EC:
            ret = wolfSSL_EVP_PKEY_assign_EC_KEY( pkey,
                                                  ( WOLFSSL_EC_KEY * ) key );
            break;
            #endif
            #ifdef NO_DH
...
static void ECC_populate_EVP_PKEY( EVP_PKEY * pkey, ecc_key * ecc )
{
    ...
    {
        int derSz = ret;
        char * derBuf = ( char * ) XMALLOC( derSz,
                                            NULL,
...
int wolfSSL_EVP_CIPHER_iv_length( const WOLFSSL_EVP_CIPHER * cipher )
{
    ...
    WOLFSSL_MSG( "wolfSSL_EVP_CIPHER_iv_length" );
                ...
                #ifdef WOLFSSL_AES_128
    if( EVP_AES_128_CBC &&
        XSTRNCMP( name, EVP_AES_128_CBC, XSTRLEN( EVP_AES_128_CBC ) ) == 0 )
        return AES_BLOCK_SIZE;
                #endif
                #ifdef WOLFSSL_AES_192
    if( EVP_AES_192_CBC &&
    ...
    if( EVP_AES_256_CBC &&
        ...
        return AES_BLOCK_SIZE;
                ...
                ( defined( HAVE_FIPS_VERSION ) && ( HAVE_FIPS_VERSION > 2 ) )
                    ...
                    #ifdef WOLFSSL_AES_128
    if( EVP_AES_128_GCM &&
        XSTRNCMP( name, EVP_AES_128_GCM, XSTRLEN( EVP_AES_128_GCM ) ) == 0 )
        return GCM_NONCE_MID_SZ;
                    #endif
                    #ifdef WOLFSSL_AES_192
    if( EVP_AES_192_GCM &&
        XSTRNCMP( name, EVP_AES_192_GCM, XSTRLEN( EVP_AES_192_GCM ) ) == 0 )
        return GCM_NONCE_MID_SZ;
                    #endif
                    #ifdef WOLFSSL_AES_256
    if( EVP_AES_256_GCM &&
        XSTRNCMP( name, EVP_AES_256_GCM, XSTRLEN( EVP_AES_256_GCM ) ) == 0 )
        return GCM_NONCE_MID_SZ;
                    #endif
                #endif /* HAVE_AESGCM */
            #endif /* (HAVE_FIPS && !HAVE_SELFTEST) || HAVE_FIPS_VERSION > 2 \
                ...
                #ifdef WOLFSSL_AES_128
    if( EVP_AES_128_CTR &&
        XSTRNCMP( name, EVP_AES_128_CTR, XSTRLEN( EVP_AES_128_CTR ) ) == 0 )
        return AES_BLOCK_SIZE;
                #endif
                #ifdef WOLFSSL_AES_192
    if( EVP_AES_192_CTR &&
    ...
    if( EVP_AES_256_CTR &&
        ...
        return AES_BLOCK_SIZE;
                ...
                #ifdef WOLFSSL_AES_128
    if( EVP_AES_128_XTS &&
        XSTRNCMP( name, EVP_AES_128_XTS, XSTRLEN( EVP_AES_128_XTS ) ) == 0 )
        return AES_BLOCK_SIZE;
                #endif /* WOLFSSL_AES_128 */

                #ifdef WOLFSSL_AES_256
    if( EVP_AES_256_XTS &&
        XSTRNCMP( name, EVP_AES_256_XTS, XSTRLEN( EVP_AES_256_XTS ) ) == 0 )
        return AES_BLOCK_SIZE;
                #endif /* WOLFSSL_AES_256 */
            #endif     /* WOLFSSL_AES_XTS */
        ...
        #ifndef NO_DES3
    if( ( EVP_DES_CBC &&
          XSTRNCMP( name, EVP_DES_CBC, XSTRLEN( EVP_DES_CBC ) ) == 0 ) ||
        ( EVP_DES_EDE3_CBC &&
          XSTRNCMP( name, EVP_DES_EDE3_CBC, XSTRLEN( EVP_DES_EDE3_CBC ) ) ==
              0 ) )
    {
        return DES_BLOCK_SIZE;
    }
        ...
        #ifdef HAVE_IDEA
    if( EVP_IDEA_CBC &&
        XSTRNCMP( name, EVP_IDEA_CBC, XSTRLEN( EVP_IDEA_CBC ) ) == 0 )
        return IDEA_BLOCK_SIZE;
        #endif
