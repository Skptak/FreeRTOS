...
#define STATUS_ERROR_MASK \
    ( LSR_OE_MASK | LSR_PE_MASK | LSR_FE_MASK | LSR_BI_MASK | LSR_FIER_MASK )
                      ...
                      uint8_t line_config )
{
    ...
    if( ( base_addr != NULL_ADDR ) && ( this_uart != NULL_INSTANCE ) )
    {
        ...
        fifo_config = ( ( uint8_t ) ( DEFAULT_RX_TRIG_LEVEL
                                      << FCR_TRIG_LEVEL_SHIFT ) |
                           ...
                           uint32_t tx_size )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( pbuff != NULL_BUFF ) &&
        ( tx_size > 0U ) )
    {
        ...
        do
        {
            ...
            if( status & LSR_THRE_MASK )
            {
                ...
                for( size_sent = 0U; size_sent < fill_size; ++size_sent )
                {
                    ...
                    HAL_set_8bit_reg( this_uart->base_address,
                                      THR,
                          ...
                          size_t buff_size )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( rx_buff != ( uint8_t * ) 0 ) &&
        ( buff_size > 0U ) )
    {
        status = HAL_get_8bit_reg( this_uart->base_address, LSR );
        this_uart->status |= status;
        ...
        {
            rx_buff[ rx_size ] = HAL_get_8bit_reg( this_uart->base_address,
                                                   RBR );
            rx_size++;
            status = HAL_get_8bit_reg( this_uart->base_address, LSR );
            this_uart->status |= status;
    ...
    if( this_uart != NULL_INSTANCE )
    {
        ...
        switch( iirf )
        {
            ...
            case IIRF_MODEM_STATUS:
            {
                ...
                {
                    HAL_ASSERT( INVALID_IRQ_HANDLER !=
                                this_uart->modemsts_handler );
                    if( INVALID_IRQ_HANDLER != this_uart->modemsts_handler )
                    {
                                ...
                                uart_16550_rx_trig_level_t trigger_level )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( handler != INVALID_IRQ_HANDLER ) &&
                              ...
                              uart_16550_loopback_t loopback )
{
        ...
        ( loopback < UART_16550_INVALID_LOOPBACK ) )
    {
        ...
        {
            HAL_set_8bit_reg_field( this_uart->base_address, MCR_LOOP, ENABLE );
        }
    }
}
...
static void default_tx_handler( uart_16550_instance_t * this_uart )
{
    ...
    if( this_uart != NULL_INSTANCE )
    {
            ...
            ( 0U != this_uart->tx_buff_size ) )
        {
            ...
            /*
             * This function should only be called as a result of a THRE
             * interrupt. Verify that this is true before proceeding to transmit
             * data.
             */
            if( status & LSR_THRE_MASK )
            {
                ...
                uint32_t fill_size = TX_FIFO_SIZE;
                uint32_t tx_remain = this_uart->tx_buff_size -
                                     this_uart->tx_idx;
                ...
                for( size_sent = 0U; size_sent < fill_size; ++size_sent )
                {
                    ...
                    HAL_set_8bit_reg( this_uart->base_address,
                                      THR,
                                      ( uint_fast8_t ) this_uart
                                          ->tx_buffer[ this_uart->tx_idx ] );
                    ++this_uart->tx_idx;
                }
            }
                                ...
                                size_t tx_size )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( tx_buffer != NULL_BUFF ) &&
        ( tx_size > 0U ) )
    {
        /* Read the Line Status Register and update the sticky record. */
        status = HAL_get_8bit_reg( this_uart->base_address, LSR );
...
uint8_t UART_16550_get_tx_status( uart_16550_instance_t * this_uart )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) )
    {
        ...
        this_uart->status |= status;
        /*
         * Extract the transmit status bits from the UART's Line Status
         * Register. Bit 5 - Transmitter Holding Register/FIFO Empty (THRE)
         * status. (If = 1, TX FIFO is empty) Bit 6 - Transmitter Empty (TEMT)
         * status. (If = 1, both TX FIFO and shift register are empty)
         */
