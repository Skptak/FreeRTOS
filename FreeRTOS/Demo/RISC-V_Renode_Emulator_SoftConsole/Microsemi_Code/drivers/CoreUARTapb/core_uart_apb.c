...
#define STATUS_ERROR_MASK                                          \
    ( ( uint8_t ) ( STATUS_PARITYERR_MASK | STATUS_OVERFLOW_MASK | \
                    STATUS_FRAMERR_MASK ) )
                ...
                uint8_t line_config )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( line_config <= MAX_LINE_CONFIG ) &&
        ( baud_value <= MAX_BAUD_VALUE ) )
    {
        /*
         * Store lower 8-bits of baud value in CTRL1.
         */
        HAL_set_8bit_reg( base_addr,
                          CTRL1,
                          ( uint_fast8_t ) ( baud_value & BAUDVALUE_LSB ) );
...
#ifndef NDEBUG
        {
            ...
            baud_val |= ( uint16_t ) ( ( uint16_t ) ( temp )
                                       << BAUDVALUE_SHIFT );
                ...
                size_t tx_size )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( tx_buffer != NULL_BUFFER ) &&
        ( tx_size > ( size_t ) 0 ) )
    {
                    ...
                    size_t buff_size )
{
    ...
    if( ( this_uart != NULL_INSTANCE ) && ( rx_buffer != NULL_BUFFER ) &&
        ( buff_size > 0u ) )
    {
        rx_idx = 0u;
