                ...
                gpio_apb_width_t bus_width )
{
    ...
    switch( this_gpio->apb_bus_width )
    {
        ...
        case GPIO_APB_16_BITS_BUS:
            HAL_set_16bit_reg( this_gpio->base_addr,
                               IRQ0,
        ...
        case GPIO_APB_8_BITS_BUS:
            HAL_set_8bit_reg( this_gpio->base_addr,
                              IRQ0,
                              ( uint8_t ) CLEAR_ALL_IRQ8 );
            HAL_set_8bit_reg( this_gpio->base_addr,
                              IRQ1,
                              ( uint8_t ) CLEAR_ALL_IRQ8 );
            HAL_set_8bit_reg( this_gpio->base_addr,
                  ...
                  uint32_t config )
{
    ...
    if( port_id < NB_OF_GPIO )
    {
         ...
         * back the expected value may indicate that the GPIO port was
         * configured as part of the hardware flow and cannot be modified
         * through software. It may also indicate that the base address passed
         * as parameter to GPIO_init() was incorrect.
         */
...
/*-------------------------------------------------------------------------*/ /**
                                                                               ...
                                                                               * See "core_gpio.h" for details of how to use this function.
                                                                               */
void GPIO_set_outputs( gpio_instance_t * this_gpio, uint32_t value )
{
    switch( this_gpio->apb_bus_width )
    {
        case GPIO_APB_32_BITS_BUS:
    ...
    /*
     * Verify that the output register was correctly written. Failure to read
     * back the expected value may indicate that some of the GPIOs may not exist
     * due to the number of GPIOs selected in the CoreGPIO hardware flow
     * configuration. It may also indicate that the base address or APB bus
     * width passed as parameter to the GPIO_init() function do not match the
     * hardware design.
     */
...
uint32_t GPIO_get_inputs( gpio_instance_t * this_gpio )
{
    ...
    switch( this_gpio->apb_bus_width )
    {
        ...
        case GPIO_APB_16_BITS_BUS:
            ...
            gpio_in |= ( HAL_get_16bit_reg( this_gpio->base_addr, GPIO_IN1 )
                         << 16 );
            break;

        case GPIO_APB_8_BITS_BUS:
            gpio_in |= HAL_get_8bit_reg( this_gpio->base_addr, GPIO_IN0 );
            gpio_in |= ( HAL_get_8bit_reg( this_gpio->base_addr, GPIO_IN1 )
                         << 8 );
            gpio_in |= ( HAL_get_8bit_reg( this_gpio->base_addr, GPIO_IN2 )
                         << 16 );
            gpio_in |= ( HAL_get_8bit_reg( this_gpio->base_addr, GPIO_IN3 )
                         << 24 );
            break;
...
uint32_t GPIO_get_outputs( gpio_instance_t * this_gpio )
{
    ...
    switch( this_gpio->apb_bus_width )
    {
        ...
        case GPIO_APB_16_BITS_BUS:
            ...
            gpio_out |= ( HAL_get_16bit_reg( this_gpio->base_addr, GPIO_OUT1 )
                          << 16 );
            break;
        ...
        case GPIO_APB_8_BITS_BUS:
            ...
            gpio_out |= ( HAL_get_16bit_reg( this_gpio->base_addr, GPIO_OUT1 )
                          << 8 );
            gpio_out |= ( HAL_get_16bit_reg( this_gpio->base_addr, GPIO_OUT2 )
                          << 16 );
            gpio_out |= ( HAL_get_16bit_reg( this_gpio->base_addr, GPIO_OUT3 )
                          << 24 );
            break;
                      ...
                      uint8_t value )
{
    ...
    switch( this_gpio->apb_bus_width )
    {
        case GPIO_APB_32_BITS_BUS:
        {
            ...
            /*
             * Verify that the output register was correctly written. Failure to
             * read back the expected value may indicate that some of the GPIOs
             * may not exist due to the number of GPIOs selected in the CoreGPIO
             * hardware flow configuration. It may also indicate that the base
             * address or APB bus width passed as parameter to the GPIO_init()
             * function do not match the hardware design.
             */
            HAL_ASSERT( HAL_get_32bit_reg( this_gpio->base_addr, GPIO_OUT ) ==
                        outputs_state );
        }
        break;
        ...
        case GPIO_APB_16_BITS_BUS:
        {
            ...
            uint32_t gpio_out_reg_addr = this_gpio->base_addr +
                                         GPIO_OUT_REG_OFFSET +
                                         ( ( port_id >> 4 ) * 4 );
             ...
             * function do not match the hardware design.
             */
            HAL_ASSERT( HW_get_16bit_reg( gpio_out_reg_addr ) ==
                        outputs_state );
        }
        break;
...
/*-------------------------------------------------------------------------*/ /**
                                                                               ...
                                                                               * See "core_gpio.h" for details of how to use this function.
                                                                               */
void GPIO_enable_irq( gpio_instance_t * this_gpio, gpio_id_t port_id )
{
    uint32_t cfg_value;
    uint32_t cfg_reg_addr = this_gpio->base_addr;
...
/*-------------------------------------------------------------------------*/ /**
                                                                               ...
                                                                               * See "core_gpio.h" for details of how to use this function.
                                                                               */
void GPIO_disable_irq( gpio_instance_t * this_gpio, gpio_id_t port_id )
{
    uint32_t cfg_value;
    uint32_t cfg_reg_addr = this_gpio->base_addr;
...
/*-------------------------------------------------------------------------*/ /**
                                                                               ...
                                                                               * See "core_gpio.h" for details of how to use this function.
                                                                               */
void GPIO_clear_irq( gpio_instance_t * this_gpio, gpio_id_t port_id )
{
