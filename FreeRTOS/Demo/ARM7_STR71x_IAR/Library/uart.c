...
{
    UARTx->BR = ( u16 ) ( RCCU_FrequencyValue( RCCU_FCLK ) /
                          ( 16 * BaudRate ) );
}
 ...
 * Description    : This function sends several 9 bits data bytes to the
 *selected UART. Input 1        : UARTx (x can be 0,1, 2 or 3) the desired UART
 * Input 2        : A pointer to the data to send
...
u16 UART_ByteReceive( UART_TypeDef * UARTx, u8 * Data, u8 TimeOut )
{
    ...
    while( !( ( wStatus = UARTx->SR ) &
              ( UART_TimeOutIdle | UART_RxHalfFull | UART_RxBufFull ) ) )
 ...
 * Description    : This function gets a 9 bits data byte from the selected
 *UART. Input 1        : UARTx (x can be 0,1, 2 or 3) the desired UART Input 2
 *: A pointer to the buffer where the data will be stored Input 3        : The
 *time-out period value Output         : The received data Return         : The
 *UARTx.SR register contents
 *******************************************************************************/
...
u16 UART_StringReceive( UART_TypeDef * UARTx, u8 * Data )
{
    ...
    {
        while( !( ( wStatus = UARTx->SR ) &
                  ( UART_RxHalfFull | UART_RxBufFull ) ) )
            ; // while the UART_RxFIFO is empty and no Timeoutidle
