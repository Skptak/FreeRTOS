...
/** @cond DO_NOT_INCLUDE_WITH_DOXYGEN */
        ...
        /** Maximum allowed core frequency when using 0 wait-states on flash
         * access. */
        #define CMU_MAX_FREQ_0WS 26000000
        /** Maximum allowed core frequency when using 1 wait-states on flash
         * access */
...
static uint32_t lfClkGet( CMU_Clock_TypeDef lfClkBranch )
{
    ...
    switch( lfClkBranch )
    {
        case cmuClock_LFA:
            ...
            sel = ( CMU->LFCLKSEL & _CMU_LFCLKSEL_LFA_MASK ) >>
        ...
        case cmuClock_LFB:
    #if defined( _CMU_LFCLKSEL_MASK )
            sel = ( CMU->LFCLKSEL & _CMU_LFCLKSEL_LFB_MASK ) >>
    ...
    #if defined( _CMU_LFCCLKEN0_MASK )
        case cmuClock_LFC:
            sel = ( CMU->LFCLKSEL & _CMU_LFCLKSEL_LFC_MASK ) >>
                  _CMU_LFCLKSEL_LFC_SHIFT;
            break;
    #endif
    ...
    return ret;
        ...
        !defined( _EZR32_LEOPARD_FAMILY ) && !defined( _EZR32_WONDER_FAMILY )
/***************************************************************************/ /**
                                                                               * @brief
...
{
    EFM_ASSERT( HFCycles <=
                ( _CMU_CALCNT_CALCNT_MASK >> _CMU_CALCNT_CALCNT_SHIFT ) );
    ...
    #if defined( _CMU_CALCTRL_UPSEL_MASK ) && \
        defined( _CMU_CALCTRL_DOWNSEL_MASK )
/***************************************************************************/ /**
                                                                               * @brief
...
CMU_ClkDiv_TypeDef CMU_ClockDivGet( CMU_Clock_TypeDef clock )
{
    ...
    switch( divReg )
    {
        ...
        case CMU_LFAPRESC0_REG:
            ...
            {
                case cmuClock_RTC:
                    ret = ( CMU_ClkDiv_TypeDef ) ( ( CMU->LFAPRESC0 &
                ...
                case cmuClock_LETIMER0:
                    ret =
                        ( CMU_ClkDiv_TypeDef ) ( ( CMU->LFAPRESC0 &
        ...
        case CMU_LFBPRESC0_REG:
            switch( clock )
            {
                ...
                case cmuClock_LEUART0:
                    ret = ( CMU_ClkDiv_TypeDef ) ( ( CMU->LFBPRESC0 &
    ...
    switch( divReg )
    {
        ...
        case CMU_HFCORECLKDIV_REG:
            ...
            /* Clear HFLE and set DIV2 factor for peripheral clock
               when running at frequencies lower than or equal to
               CMU_MAX_FREQ_HFLE. */
            if( ( CMU_ClockFreqGet( cmuClock_HF ) / div ) <=
                CMU_MAX_FREQ_HFLE() )
            {
                /* Clear CMU HFLE */
...
uint32_t CMU_ClockFreqGet( CMU_Clock_TypeDef clock )
{
    ...
    switch( clock & ( CMU_CLK_BRANCH_MASK << CMU_CLK_BRANCH_POS ) )
    {
        ...
        #if defined( CRYPTO_PRESENT ) || defined( LDMA_PRESENT ) || \
            defined( GPCRC_PRESENT ) || defined( PRS_PRESENT ) ||   \
    ...
    #if defined( _SILICON_LABS_32B_PLATFORM_1 )
        #if defined( AES_PRESENT ) || defined( DMA_PRESENT ) || \
            defined( EBI_PRESENT ) || defined( USB_PRESENT )
        case( CMU_HFCORE_CLK_BRANCH << CMU_CLK_BRANCH_POS ):
        {
...
void CMU_ClockPrescSet( CMU_Clock_TypeDef clock, CMU_ClkPresc_TypeDef presc )
{
    ...
    switch( prescReg )
    {
        ...
        case CMU_HFCLKLEPRESC_REG:
            ...
            /* Specifies the clock divider for HFCLKLE. When running at
             * frequencies higher than 32 MHz, this must be set to DIV4. */
        ...
        case CMU_HFRADIOPRESC_REG:
            ...
            CMU->HFRADIOPRESC = ( CMU->HFRADIOPRESC &
        ...
        case CMU_HFCOREPRESC_REG:
            ...
            CMU->HFCOREPRESC = ( CMU->HFCOREPRESC &
        ...
        case CMU_LFAPRESC0_REG:
            switch( clock )
            {
                ...
                case cmuClock_RTC:
                    ...
                    /* LF register about to be modified require sync. Busy
                     * check. */
                    syncReg( CMU_SYNCBUSY_LFAPRESC0 );
...
CMU_Select_TypeDef CMU_ClockSelectGet( CMU_Clock_TypeDef clock )
{
    ...
    switch( selReg )
    {
        ...
        case CMU_USBCCLKSEL_REG:
            switch( CMU->STATUS & ( CMU_STATUS_USBCLFXOSEL
...
void CMU_ClockSelectSet( CMU_Clock_TypeDef clock, CMU_Select_TypeDef ref )
{
    ...
    switch( selRegId )
    {
                ...
                case cmuSelect_HFXO:
                    ...
                    if( SystemHFXOClockGet() > CMU_MAX_FREQ_HFLE() )
                    {
                        CMU->CTRL = ( CMU->CTRL & ~_CMU_CTRL_HFXOBUFCUR_MASK ) |
                                    ...
                                    /* Must have HFLE enabled to access some LE
                                       peripherals >=32MHz */
                                    | CMU_CTRL_HFLE;

                        /* Set HFLE and DIV4 factor for peripheral clock if
                         * HFCORE  */
                    ...
                    {
                        /* This can happen if the user configures the
                         * EFM32_HFXO_FREQ to */
    ...
    #if defined( CMU_LFCLKSEL_LFAE_ULFRCO ) || \
        defined( CMU_LFACLKSEL_LFA_ULFRCO )
            ...
            /* Ensure selected oscillator is enabled, waiting for it to
             * stabilize */
            CMU_OscillatorEnable( osc, true, true );

            /* Configure worst case wait states for flash access before
             * selecting */
            flashWaitStateMax();
            ...
            /* Optimize flash access wait state setting for currently selected
             * core clk */
            flashWaitStateControl( freq );
            break;
        ...
        case CMU_LFBCLKSEL_REG:
            ...
            switch( ref )
            {
                ...
                case cmuSelect_HFCLKLE:
                    ...
                    BUS_RegBitWrite( &CMU->HFBUSCLKEN0,
        ...
        case CMU_LFBCLKSEL_REG:
            switch( ref )
            {
                ...
                case cmuSelect_CORELEDIV2:
                    /* Ensure HFCORE to LE clocking is enabled */
                    BUS_RegBitWrite( &( CMU->HFCORECLKEN0 ),
        ...
        case CMU_USBCCLKSEL_REG:
            ...
            {
                case cmuSelect_LFXO:
                    /* Select LFXO as clock source for USB, can only be used in
                     * sleep mode */
                    /* Ensure selected oscillator is enabled, waiting for it to
                     * stabilize */
                    CMU_OscillatorEnable( cmuOsc_LFXO, true, true );
                ...
                case cmuSelect_LFRCO:
                    /* Select LFRCO as clock source for USB, can only be used in
                     * sleep mode */
                    /* Ensure selected oscillator is enabled, waiting for it to
                     * stabilize */
                    CMU_OscillatorEnable( cmuOsc_LFRCO, true, true );
...
{
    return ( CMU_HFRCOBand_TypeDef ) ( ( CMU->HFRCOCTRL &
...
void CMU_HFRCOBandSet( CMU_HFRCOBand_TypeDef band )
{
    ...
    /* Set band/tuning */
    CMU->HFRCOCTRL = ( CMU->HFRCOCTRL & ~( _CMU_HFRCOCTRL_BAND_MASK |
    ...
    if( osc == cmuSelect_HFRCO )
    {
        ...
        /* (The function will update the global variable) */
        /* NOTE! We need at least 21 cycles before setting zero wait state to
         * flash */
...
void CMU_HFXOInit( CMU_HFXOInit_TypeDef * hfxoInit )
{
    ...
    BUS_RegMaskedWrite(
        ...
        _CMU_HFXOCTRL_LOWPOWER_MASK
            ...
            | _CMU_HFXOCTRL_AUTOSTARTEM0EM1_MASK |
            _CMU_HFXOCTRL_AUTOSTARTSELEM0EM1_MASK,
        ( hfxoInit->lowPowerMode ? CMU_HFXOCTRL_LOWPOWER : 0 )
        #if defined( _CMU_HFXOCTRL_AUTOSTARTRDYSELRAC_MASK )
            | ( hfxoInit->autoStartSelOnRacWakeup
                    ? CMU_HFXOCTRL_AUTOSTARTRDYSELRAC
                    : 0 )
        #endif
            | ( hfxoInit->autoStartEm01 ? CMU_HFXOCTRL_AUTOSTARTEM0EM1 : 0 ) |
            ( hfxoInit->autoSelEm01 ? CMU_HFXOCTRL_AUTOSTARTSELEM0EM1 : 0 ) );

    /* Set XTAL tuning parameters */

    /* Set peak detection threshold in CMU_HFXOCTRL1_PEAKDETTHR[2:0] (hidden).
     */
    BUS_RegMaskedWrite( ( volatile uint32_t * ) 0x400E4028,
                        0x7,
    ...
    /* Adjust CMU_HFXOSTEADYSTATECTRL_REGISHUPPER according to
       regIshSteadyState. Saturate at max value. Please see the reference manual
       page 433 and Section 12.5.10 CMU_HFXOSTEADYSTATECTRL for more details. */
...
void CMU_OscillatorEnable( CMU_Osc_TypeDef osc, bool enable, bool wait )
{
    ...
    if( enable )
    {
        ...
        if( wait )
        {
            ...
            {
                while( !BUS_RegBitRead( &CMU->STATUS,
                                        _CMU_STATUS_HFXOSHUNTOPTRDY_SHIFT ) )
                {
                }
                /* Assert on failed peak detection. Incorrect HFXO
                   initialization parameters caused startup to fail. Please
                   review parameters. */
                EFM_ASSERT(
                    BUS_RegBitRead( &CMU->STATUS,
    ...
    switch( osc )
    {
        case cmuOsc_LFRCO:
            EFM_ASSERT( val <= ( _CMU_LFRCOCTRL_TUNING_MASK >>
                                 _CMU_LFRCOCTRL_TUNING_SHIFT ) );
            val &= ( _CMU_LFRCOCTRL_TUNING_MASK >>
                     _CMU_LFRCOCTRL_TUNING_SHIFT );
    #if defined( _SILICON_LABS_32B_PLATFORM_2 )
            while(
                BUS_RegBitRead( &CMU->SYNCBUSY, _CMU_SYNCBUSY_LFRCOBSY_SHIFT ) )
                ;
    #endif
            CMU->LFRCOCTRL = ( CMU->LFRCOCTRL &
        ...
        case cmuOsc_HFRCO:
            EFM_ASSERT( val <= ( _CMU_HFRCOCTRL_TUNING_MASK >>
                                 _CMU_HFRCOCTRL_TUNING_SHIFT ) );
            val &= ( _CMU_HFRCOCTRL_TUNING_MASK >>
                     _CMU_HFRCOCTRL_TUNING_SHIFT );
    #if defined( _SILICON_LABS_32B_PLATFORM_2 )
            while(
                BUS_RegBitRead( &CMU->SYNCBUSY, _CMU_SYNCBUSY_HFRCOBSY_SHIFT ) )
            {
            }
    #endif
...
void CMU_USHFRCOBandSet( CMU_USHFRCOBand_TypeDef band )
{
    ...
    osc = CMU_ClockSelectGet( cmuClock_HF );
    EFM_ASSERT( ( CMU_USHFRCOBandGet() != band ) &&
                ( osc != cmuSelect_USHFRCO ) );

    /* Read tuning value from calibration table */
    switch( band )
    {
        case cmuUSHFRCOBand_24MHz:
            tuning = ( DEVINFO->USHFRCOCAL0 &
                       _DEVINFO_USHFRCOCAL0_BAND24_TUNING_MASK ) >>
        ...
        case cmuUSHFRCOBand_48MHz:
            tuning = ( DEVINFO->USHFRCOCAL0 &
                       _DEVINFO_USHFRCOCAL0_BAND48_TUNING_MASK ) >>
            ...
            /* Enable the clock divider before switching the band from 24 to
             * 48MHz */
            BUS_RegBitWrite( &CMU->USHFRCOCONF,
                             _CMU_USHFRCOCONF_USHFRCODIV2DIS_SHIFT,
                             0 );
            break;
    ...
    CMU->USHFRCOTUNE = ( CMU->USHFRCOTUNE &
                         ~_CMU_USHFRCOTUNE_FINETUNING_MASK ) |
