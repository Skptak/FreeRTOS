...
void RTCC_ChannelInit( int ch, RTCC_CCChConf_TypeDef const * confPtr )
{
    ...
    EFM_ASSERT(
        ( uint32_t ) confPtr->compMask <
        ( _RTCC_CC_CTRL_COMPMASK_MASK >> _RTCC_CC_CTRL_COMPMASK_SHIFT ) + 1 );

    /** Configure the selected capture/compare channel. */
    RTCC->CC[ ch ].CTRL = ( ( uint32_t ) confPtr->chMode
                            << _RTCC_CC_CTRL_MODE_SHIFT ) |
                          ( ( uint32_t ) confPtr->compMatchOutAction
                            << _RTCC_CC_CTRL_CMOA_SHIFT ) |
                          ( ( uint32_t ) confPtr->prsSel
                            << _RTCC_CC_CTRL_PRSSEL_SHIFT ) |
                          ( ( uint32_t ) confPtr->inputEdgeSel
                            << _RTCC_CC_CTRL_ICEDGE_SHIFT ) |
                          ( ( uint32_t ) confPtr->compBase
                            << _RTCC_CC_CTRL_COMPBASE_SHIFT ) |
                          ( ( uint32_t ) confPtr->compMask
                            << _RTCC_CC_CTRL_COMPMASK_SHIFT ) |
                          ( ( uint32_t ) confPtr->dayCompMode
                            << _RTCC_CC_CTRL_DAYCC_SHIFT );
}
...
void RTCC_Init( const RTCC_Init_TypeDef * init )
{
    RTCC->CTRL = ( ( uint32_t ) init->enable << _RTCC_CTRL_ENABLE_SHIFT ) |
                 ...
                 ( ( uint32_t ) init->precntWrapOnCCV0
                   << _RTCC_CTRL_PRECCV0TOP_SHIFT ) |
                 ( ( uint32_t ) init->cntWrapOnCCV1
                   << _RTCC_CTRL_CCV1TOP_SHIFT ) |
                 ...
                 | ( ( uint32_t ) init->enaBackupModeSet
                     << _RTCC_CTRL_BUMODETSEN_SHIFT )
    #endif
                 | ( ( uint32_t ) init->enaOSCFailDetect
                     << _RTCC_CTRL_OSCFDETEN_SHIFT ) |
                 ( ( uint32_t ) init->cntMode << _RTCC_CTRL_CNTMODE_SHIFT ) |
                 ( ( uint32_t ) init->disLeapYearCorr
                   << _RTCC_CTRL_LYEARCORRDIS_SHIFT );
}
