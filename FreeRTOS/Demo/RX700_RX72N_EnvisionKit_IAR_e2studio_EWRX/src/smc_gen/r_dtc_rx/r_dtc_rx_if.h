...
*         :                    Moved struct dtc_transfer_data_cfg_t to
r_dtc_rx_target_if.h
*         :                    Added include pass of target mcu interface header
file.
...
*         : 15.08.2019 3.20    Added support for RX72M.
          : 12.11.2019 3.21    Removed definitions for MTU5 in DTC activation
interrupt source for RX23W.
...
*******************************************************************************/
        ...
        #error \
            "This module must use BSP module of Rev.5.00 or higher. Please use the BSP module of Rev.5.00 or higher."
    #endif
...
typedef enum e_dtc_src_addr_mode
{
    ...
    DTC_SRC_ADDR_INCR = ( 2 << 2 ), /* Source address is incremented after each
                                       transfer. */
    DTC_SRC_ADDR_DECR = ( 3 << 2 )  /* Source address is decremented after each
                                       transfer. */
} dtc_src_addr_mode_t;
...
{
    DTC_CHAIN_TRANSFER_CONTINUOUSLY = ( 0 ), /* = (0 << 6): Chain transfer is
                                                performed continuously. */
    DTC_CHAIN_TRANSFER_NORMAL = ( 1 << 6 )   /* Chain transfer is performed only
                                                when the counter is changed to 0
                                                or CRAH. */
...
{
    DTC_INTERRUPT_AFTER_ALL_COMPLETE = ( 0 ), /* Interrupt is generated when
                                                 specified data transfer is
                                                 completed. */
    DTC_INTERRUPT_PER_SINGLE_TRANSFER = ( 1 << 5 ) /* Interrupt is generated
                                                      when each transfer time is
                                                      completed. */
} dtc_interrupt_t;
...
{
    DTC_REPEAT_BLOCK_DESTINATION = ( 0 ), /* = (0 << 4): Destination is repeat
                                             or block area. */
...
typedef enum e_dtc_dest_addr_mode
{
    ...
    DTC_DES_ADDR_INCR = ( 2 << 2 ),  /* Destination address is incremented after
                                        each transfer. */
    DTC_DES_ADDR_DECR = ( 3 << 2 )   /* Destination address is decremented after
                                        each transfer. */
...
typedef enum e_dtc_command
{
    ...
    DTC_CMD_ACT_SRC_ENABLE,  /* Enable an activation source specified by vector
                                number. */
    DTC_CMD_ACT_SRC_DISABLE, /* Disable an activation source specified by vector
                                number. */
...
{
    DTC_SUCCESS_DMAC_BUSY = 0, /* One or some DMAC resources are locked by
                                  another process. */
    DTC_SUCCESS,
    ...
    DTC_ERR_INVALID_COMMAND, /* Command parameters are invalid. Or, forced data
                                change failed. */
...
typedef struct st_dtc_cmd_arg
{
    dtc_activation_source_t act_src; /* The activation source will be controlled
                                      */
    uint32_t chain_transfer_nr;      /* Number of chain transfer when command is
                                        DTC_CMD_CHAIN_TRANSFER_ABORT. */
    dtc_transfer_data_t * p_transfer_data; /* Pointer to start address of
                                              Transfer data area on RAM */
    dtc_transfer_data_cfg_t * p_data_cfg;  /* Pointer to contains the settings
                                              for Transfer data */
} dtc_cmd_arg_t;
