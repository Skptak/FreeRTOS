 ...
 *         : 24.12.2015 2.04    Modified #define name from
 *"DTC_CFG_SHORT_ADDRRESS_MODE" :                    to
 *"DTC_CFG_SHORT_ADDRESS_MODE". :                    Added RX130, RX23T and
 *RX24T. : 30.09.2016 2.05    Added RX65N. :                    Supported to the
 *register added in DTCb. :                    Supported sequence transfer. :
 *Added DTC IP version definitions. : 13.03.2017 2.07    Added RX24U and
 *RX24T-512KB. : 31.07.2017 2.08    Supported RX65N-2MB and RX130-512KB. : Fixed
 *to correspond to Renesas coding rule. : 28.09.2018 2.10    Supported RX66T. :
 *01.02.2019 2.20    Supported RX72T, RX65N-64pin : 20.05.2019 3.00    Added
 *support for GNUC and ICCRX. : 18.06.2019 3.01    Removed
 *"defined(__BIG_ENDIAN__)" from DTC_BIG_ENDIAN macro definition.
    ...
    *******************************************************************************/
    ...
    #include <stdbool.h>
        ...
        #include ".\src\targets\rx23t\r_dtc_rx_target.h"
            ...
            #error \
                "Change to DTC_CFG_USE_DMAC_FIT_MODULE (DTC_DISABLE) in r_dtc_rx_config.h."
        #endif
        #if( DTC_CFG_USE_SEQUENCE_TRANSFER == DTC_ENABLE )
            #error \
                "Change to DTC_CFG_USE_SEQUENCE_TRANSFER (DTC_DISABLE) in r_dtc_rx_config.h."
        #endif
        ...
        #endif
        #if( DTC_CFG_USE_SEQUENCE_TRANSFER == DTC_ENABLE ) && \
            ( DTC_ENABLE == DTC_CFG_SHORT_ADDRESS_MODE )
            #error \
                "Change to DTC_CFG_USE_SEQUENCE_TRANSFER (DTC_DISABLE) in r_dtc_rx_config.h."
        #endif
...
{
    uint8_t BYTE;
    R_BSP_ATTRIB_STRUCT_BIT_ORDER_LEFT_2( uint8_t rs    : 7, /* reserved */
                                          uint8_t DISPE : 1 )
    BIT;
...
typedef union dtc_crb
{
    ...
    #if( DTC_ENABLE == DTC_CFG_SHORT_ADDRESS_MODE ) /* Transfer data in \
                                                       short-address mode */
