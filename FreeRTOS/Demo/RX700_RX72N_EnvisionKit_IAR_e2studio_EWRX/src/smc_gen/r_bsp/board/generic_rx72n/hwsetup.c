...
typedef struct bsp_bsc
{
    ...
    {
        uint32_t u_long;
        R_BSP_ATTRIB_STRUCT_BIT_ORDER_LEFT_13( uint32_t prerr  : 1,
                                               uint32_t        : 1,
                                               uint32_t rpstop : 1,
                                               uint32_t        : 10,
                                               uint32_t pr5sel : 3,
...
static void rom_cache_noncacheable_area0_set( void )
{
    ...
    FLASH.ROMCE.WORD = 0x0000;
            ...
            /* Makes settings to the NCRG0 register. */
                ...
                ( ( BSP_CFG_NONCACHEABLE_AREA0_ADDR & 0x0000000F ) == \
                  0x00000000 )
    FLASH.NCRG0 = BSP_CFG_NONCACHEABLE_AREA0_ADDR;
            #else
                #error \
                    "Error! Invalid setting for BSP_CFG_NONCACHEABLE_AREA0_ADDR in r_bsp_config.h"
            #endif
    ...
    tmp_ncrc |= 0x001FFFF0;
            #else
                #error \
                    "Error! Invalid setting for BSP_CFG_NONCACHEABLE_AREA0_SIZE in r_bsp_config.h"
            #endif
...
static void rom_cache_noncacheable_area1_set( void )
{
    ...
    FLASH.ROMCE.WORD = 0x0000;
            ...
            /* Makes settings to the NCRG1 register. */
                ...
                ( ( BSP_CFG_NONCACHEABLE_AREA1_ADDR & 0x0000000F ) == \
                  0x00000000 )
    FLASH.NCRG1 = BSP_CFG_NONCACHEABLE_AREA1_ADDR;
            #else
                #error \
                    "Error! Invalid setting for BSP_CFG_NONCACHEABLE_AREA1_ADDR in r_bsp_config.h"
            #endif
    ...
    tmp_ncrc |= 0x001FFFF0;
            #else
                #error \
                    "Error! Invalid setting for BSP_CFG_NONCACHEABLE_AREA1_SIZE in r_bsp_config.h"
            #endif
 ...
 * Arguments    : none
 * Return value : none
 * Note         : This function should be called when the PSW.I bit is 0
 *(interrupt disabled). This function should be called when the ADCSR.ADST bit
 *is 0.
 ***********************************************************************************************************************/
static void bsp_adc_initial_configure( void )
{
