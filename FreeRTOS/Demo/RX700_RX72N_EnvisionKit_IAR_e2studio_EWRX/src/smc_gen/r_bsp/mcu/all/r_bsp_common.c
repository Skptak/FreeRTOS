 ...
 *         : 03.09.2014 1.20     Corrected R_BSP_SoftwareDelay() timing when
 *using an RX64M : 30.09.2015 1.30     Added RX23T : 01.02.2016 1.40     Added
 *RX24T Changed the value of the following macro definition.
 *                               - OVERHEAD_CYCLES
 *                               - OVERHEAD_CYCLES_64
 *         : 29.02.2016 1.50     Added RX230
 *         : 01.10.2016 1.60     Added RX65N
 *         : 22.08.2016 1.70     Added RX24U
 *         : 15.05.2017 1.80     Changed method of selecting the number of CPU
 *cycles required to execute the delayWait() loop. : 27.07.2018 1.90     Changed
 *the value of the following macro definition, because added RX66T.
 *                               - CPU_CYCLES_PER_LOOP
 *         : 28.02.2019 2.00     Deleted the following definition.
 *                               (The following definition moved to the common
 *file (mcu_info.h).)
 *                               - CPU_CYCLES_PER_LOOP
 ...
 *                               - delay_wait
 *         : 26.07.2019 2.01     Modified comment of API function to Doxygen
 *style.
 ***********************************************************************************************************************/
...
Macro definitions
***********************************************************************************************************************/
#define BSP_PRV_OVERHEAD_CYCLES \
    ( 2 ) /* R_BSP_SoftwareDelay() overhead per call */
#define BSP_PRV_OVERHEAD_CYCLES_64 \
    ( 2 ) /* R_BSP_SoftwareDelay() overhead per call using 64-bit ints */
...
uint32_t R_BSP_GetVersion( void )
{
    ...
    return ( ( ( ( uint32_t ) R_BSP_VERSION_MAJOR ) << 16 ) |
             ( uint32_t ) R_BSP_VERSION_MINOR );
} /* End of function R_BSP_GetVersion() */

/***********************************************************************************************************************
 * Function Name: delay_wait
 * Description  : This asm loop executes a known number (5) of CPU cycles. If a
 *value of '4' is passed in as an argument, then this function would consume 20
 *CPU cycles before returning. Arguments    : loop_cnt - A single 32-bit value
 *is provided as the number of loops to execute. Return Value : None
 ***********************************************************************************************************************/
...
bool R_BSP_SoftwareDelay( uint32_t delay, bsp_delay_units_t units )
{
    ...
    if( ( BSP_DELAY_MICROSECS != units ) && ( BSP_DELAY_MILLISECS != units ) &&
        ( BSP_DELAY_SECS != units ) )
    {
        return ( false );
    }
#endif
    ...
    /*
     * In order to handle all possible combinations of delay/ICLK it is
     * necessary to use 64-bit integers (not all MCUs have floating point
     * support). However, there is no native hw support for 64 bit integers so
     * it requires many more clock cycles. This is not an issue if the requested
     * delay is long enough and the ICLK is fast, but for delays in the low
     * microseconds and/or a slow ICLK we use 32 bit integers to reduce the
     * overhead cycles of this function by approximately a third and stand the
     * best chance of achieving the requested delay.
     */
    if( ( BSP_DELAY_MICROSECS == units ) &&
        ( delay <= ( 0xFFFFFFFFUL / iclk_rate ) ) ) /* Ensure (iclk_rate *
                                                       delay) will not exceed 32
                                                       bits */
    {
        ...
        if( 0 == loop_cnt )
        {
            /* The requested delay is too large/small for the current ICLK.
             * Return false which also results in the minimum possible delay. */
            return ( false );
        }
    }
    else
    {
         ...
         * argument. */
        delay_cycles_64 = ( ( ( uint64_t ) iclk_rate * ( uint64_t ) delay ) /
                            units );
