 ...
 *prior to the main program starting. This is where the program counter starts
 ...
 *         : 08.10.2019 3.10      Changed for added support of Renesas RTOS
 *(RI600V4 or RI600PX).
 ***********************************************************************************************************************/
...
R_BSP_PRAGMA_STACKSIZE_SI( BSP_CFG_ISTACK_BYTES )
    ...
    ***********************************************************************************************************************/
    #if BSP_CFG_RTOS_USED == 4 /* Renesas RI600V4 & RI600PX */
        #define BSP_PRV_PSW_INIT \
            ( 0x00000000 ) /* Supervisor mode & Disable Interrupt */
    #else                  /* BSP_CFG_RTOS_USED!=4 */
        /* If the user chooses only 1 stack then the 'U' bit will not be set and
         * the CPU will always use the interrupt stack. */
        #if BSP_CFG_USER_STACK_ENABLE == 1
        ...
        /* Initialize FPSW for floating-point operations */
            ...
            #else
                #define BSP_PRV_FPU_ROUND \
                    ( 0x00000000 ) /* Let FPSW RMbits=00 (round to nearest) */
            #endif
            #ifdef __DOFF
                #define BSP_PRV_FPU_DENOM \
                    ( 0x00000100 ) /* Let FPSW DNbit=1 (denormal as zero) */
            #else
                #define BSP_PRV_FPU_DENOM \
                    ( 0x00000000 ) /* Let FPSW DNbit=0 (denormal as is) */
            #endif
        #endif
...
/* If user is requesting warm start callback functions then these are the
 * prototypes. */
...
/* A function is used to create a main task, rtos's objects required to be
 ...
 *stack sizes are set in the file bsp_config.h.
 ...
 *in user mode, set the macro BSP_CFG_RUN_IN_USER_MODE above to a 1.
...
R_BSP_POR_FUNCTION( R_BSP_STARTUP_FUNCTION )
{
    ...
    /* You can use auto variables in this function but such variables other than
     * register variables will be unavailable after you change the stack from
     * the I stack to the U stack (if change). */

    /* The bss sections have not been cleared and the data sections have not
     * been initialized and constructors of C++ objects have not been executed
     * until the _INITSCT() is executed. */
    #if defined( __GNUC__ )
        #if BSP_CFG_USER_STACK_ENABLE == 1
    ...
    R_BSP_MAIN_FUNCTION();
        ...
        /* Lock the channel that system timer of RTOS is using. */
             ...
             ( ( BSP_CFG_RTOS_SYSTEM_TIMER ) <= 3 ) )
    if( R_BSP_HardwareLock(
            ( mcu_lock_t ) ( BSP_LOCK_CMT0 + BSP_CFG_RTOS_SYSTEM_TIMER ) ) ==
        false )
    {
        /* WAIT_LOOP */
        while( 1 )
    ...
    /* Prepare the necessary tasks, FreeRTOS's resources... required to be
     * executed at the beginning after vTaskStarScheduler() is called. Other
     * tasks can also be created after starting scheduler at any time */
    ...
    if( R_BSP_HardwareLock(
            ( mcu_lock_t ) ( BSP_LOCK_CMT0 + _RI_CLOCK_TIMER ) ) == false )
    {
        /* WAIT_LOOP */
        while( 1 )
            ;
    ...
    _RI_init_cmt();
        #else
            /* When RI600PX, the above are in _RI_init_cmt_knl called from the
             * kernel. */
        #endif
