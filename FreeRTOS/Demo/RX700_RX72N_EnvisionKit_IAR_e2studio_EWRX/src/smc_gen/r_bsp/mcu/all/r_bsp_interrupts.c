 ...
 * Description  : This module allows for callbacks to be registered for certain
 *interrupts. And handle exception interrupts.
 ***********************************************************************************************************************/
/**********************************************************************************************************************
 ...
 *         : 26.07.2019 1.10     Modified comment of API function to Doxygen
 *style. Modified the following function for added function.
                                    ...
                                    bsp_int_cb_t callback )
{
    ...
    if( ( ( uint32_t ) callback == ( uint32_t ) NULL ) ||
        ( ( uint32_t ) callback == ( uint32_t ) FIT_NO_FUNC ) )
    {
        /* Casting is valid because it matches the type to the right side or
         * argument. */
                                      ...
                                      void * pdata )
{
    ...
    /* nothing */
#else
    /* This code is only used to remove compiler info messages about these
    ...
    switch( cmd )
    {
        case( BSP_INT_CMD_CALL_CALLBACK ):
             ...
             * argument. */
            if( ( ( uint32_t ) g_bsp_vectors[ vector ] != ( uint32_t ) NULL ) &&
                ( ( uint32_t ) g_bsp_vectors[ vector ] !=
                  ( uint32_t ) FIT_NO_FUNC ) )
            {
        ...
        case( BSP_INT_CMD_GROUP_INTERRUPT_ENABLE ):
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            if( ( ( uint32_t ) NULL != ( uint32_t ) pdata ) &&
                ( ( uint32_t ) FIT_NO_FUNC != ( uint32_t ) pdata ) )
            {
                /* Casting is valid because it matches the type to the right
                 * side or argument. */
                err = bsp_gr_int_enable_disable( vector,
                                                 true,
                                                 ( ( bsp_int_ctrl_t * ) pdata )
                                                     ->ipl );
            }
        ...
        case( BSP_INT_CMD_FIT_INTERRUPT_ENABLE ):
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            err = bsp_fit_interrupts_control( true,
                                              ( bsp_int_ctrl_t * ) pdata );
            break;
        ...
        case( BSP_INT_CMD_FIT_INTERRUPT_DISABLE ):
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            err = bsp_fit_interrupts_control( false,
                                              ( bsp_int_ctrl_t * ) pdata );
            break;
                                                 ...
                                                 bsp_int_ctrl_t * pdata )
{
        ...
        ( ( uint32_t ) FIT_NO_FUNC != ( uint32_t ) pdata ) )
    {
        ...
        if( 0 == pmode )
        {
            ...
            {
                /* Get the current Processor Interrupt Priority Level (IPL) and
                 * save IPL value. */
                 ...
                 * Note: Please set this macro more than IPR for other FIT
                 * module interrupts. */
 ...
 * Description  : Either enables or disables a group interrupt. If a group
 *interrupt is called multiple times to be enabled then it will use the highest
 *given IPL. A group interrupt will only be disabled when all interrupt sources
 *for that group are already disabled. Arguments    : vector - An interrupt
 *source inside the group that is to be enabled/disabled. enable - Whether to
 *enable or disable the interrupt. ipl - If enabling a group interrupt, what IPL
 *to use. Return Value : BSP_INT_SUCCESS - Interrupt enabled or disabled.
 *                BSP_INT_ERR_INVALID_ARG -
 *                    Invalid IPL or vector
 *                BSP_INT_ERR_GROUP_STILL_ENABLED -
 *                    Not all group interrupts were disabled so group interrupt
 *was not disabled.
 ***********************************************************************************************************************/
static bsp_int_err_t bsp_gr_int_enable_disable( bsp_int_src_t vector,
                                                bool enable,
                                                uint32_t ipl )
{
    ...
    if( ( true == enable ) &&
        ( ( BSP_MCU_IPL_MIN == ipl ) || ( ipl > BSP_MCU_IPL_MAX ) ) )
    {
        return BSP_INT_ERR_INVALID_ARG;
    }
    #endif

    if( ( vector > BSP_INT_SRC_GR_INT_IE0_TOP ) &&
        ( vector < BSP_INT_SRC_GR_INT_BE0_TOP ) )
    {
        /* Group IE0. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPIE0 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPIE0 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPIE0 ) );
        ...
        {
            /* Check to make sure all interrupt sources are already disabled for
             * this group. */
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_BE0_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_BL0_TOP ) )
    {
        /* Group BE0. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPBE0 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPBE0 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPBE0 ) );
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_BL0_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_BL1_TOP ) )
    {
        /* Group BL0. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPBL0 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPBL0 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPBL0 ) );
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_BL1_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_BL2_TOP ) )
    {
        /* Group BL1. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPBL1 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPBL1 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPBL1 ) );
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_BL2_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_AL0_TOP ) )
    {
        /* Group BL2. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPBL2 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPBL2 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPBL2 ) );
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_AL0_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_AL1_TOP ) )
    {
        /* Group AL0. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPAL0 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPAL0 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPAL0 ) );
    ...
    else if( ( vector > BSP_INT_SRC_GR_INT_AL1_TOP ) &&
             ( vector < BSP_INT_SRC_GR_INT_END ) )
    {
        /* Group AL1. */
        ...
        if( true == enable )
        {
            ...
            /* Casting is valid because it matches the type to the right side or
             * argument. */
            IPR( ICU, GROUPAL1 ) = ( uint8_t ) ( ( ipl > IPR( ICU, GROUPAL1 ) )
                                                     ? ipl
                                                     : IPR( ICU, GROUPAL1 ) );
...
R_BSP_ATTRIB_INTERRUPT void excep_supervisor_inst_isr( void )
{
    ...
    R_BSP_InterruptControl( BSP_INT_SRC_EXC_SUPERVISOR_INSTR,
...
{
    /* If user has registered a callback for this exception then call it. */
    R_BSP_InterruptControl( BSP_INT_SRC_EXC_ACCESS,
                            BSP_INT_CMD_CALL_CALLBACK,
                            FIT_NO_PTR );
...
{
    /* If user has registered a callback for this exception then call it. */
    R_BSP_InterruptControl( BSP_INT_SRC_EXC_UNDEFINED_INSTR,
                            BSP_INT_CMD_CALL_CALLBACK,
                            FIT_NO_PTR );
...
R_BSP_ATTRIB_INTERRUPT void excep_floating_point_isr( void )
{
    ...
    /* If user has registered a callback for this exception then call it. */
    R_BSP_InterruptControl( BSP_INT_SRC_EXC_FPU,
                            BSP_INT_CMD_CALL_CALLBACK,
                            FIT_NO_PTR );
...
R_BSP_ATTRIB_INTERRUPT void non_maskable_isr( void )
{
    ...
    if( ( 1 == ICU.NMISR.BIT.NMIST ) && ( 1 == ICU.NMIER.BIT.NMIEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_EXC_NMI_PIN,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.OSTST ) && ( 1 == ICU.NMIER.BIT.OSTEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_OSC_STOP_DETECT,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.WDTST ) && ( 1 == ICU.NMIER.BIT.WDTEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_WDT_ERROR,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.LVDST ) && ( 1 == ICU.NMIER.BIT.LVDEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_LVD1,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    }
        #endif
    ...
    if( ( 1 == ICU.NMISR.BIT.IWDTST ) && ( 1 == ICU.NMIER.BIT.IWDTEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_IWDT_ERROR,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    {
        /* Voltage monitoring 1 interrupt is requested. */
        R_BSP_InterruptControl( BSP_INT_SRC_LVD2,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.VBATST ) && ( 1 == ICU.NMIER.BIT.VBATEN ) )
    {
        ...
        R_BSP_InterruptControl( BSP_INT_SRC_VBATT,
                                BSP_INT_CMD_CALL_CALLBACK,
                                FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.ECCRAMST ) && ( 1 == ICU.NMIER.BIT.ECCRAMEN ) )
    {
        if( 1 == ECCRAM.ECCRAM1STS.BIT.ECC1ERR )
        {
            ...
            R_BSP_InterruptControl( BSP_INT_SRC_ECCRAM_1BIT,
                                    BSP_INT_CMD_CALL_CALLBACK,
                                    FIT_NO_PTR );
        ...
        {
            /* ECCRAM Error interrupt is requested. */
            R_BSP_InterruptControl( BSP_INT_SRC_ECCRAM_2BIT,
                                    BSP_INT_CMD_CALL_CALLBACK,
                                    FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.RAMST ) && ( 1 == ICU.NMIER.BIT.RAMEN ) )
    {
        ...
        if( 1 == RAM.RAMSTS.BIT.RAMERR )
        {
            ...
            R_BSP_InterruptControl( BSP_INT_SRC_RAM,
                                    BSP_INT_CMD_CALL_CALLBACK,
                                    FIT_NO_PTR );
        ...
        if( 1 == RAM.EXRAMSTS.BIT.EXRAMERR )
        {
            ...
            R_BSP_InterruptControl( BSP_INT_SRC_EXRAM,
                                    BSP_INT_CMD_CALL_CALLBACK,
                                    FIT_NO_PTR );
    ...
    if( ( 1 == ICU.NMISR.BIT.EXNMIST ) && ( 1 == ICU.NMIER.BIT.EXNMIEN ) )
    {
        ...
        /* Casting is valid because it matches the type to the right side or
         * argument. */
        if( ( 1 == ICU.EXNMISR.BIT.DPFPUST ) &&
            ( 1 == ICU.EXNMIER.BIT.DPFPUEN ) )
        {
            /* Double-Precision Floating-Point Exception interrupt is requested.
             */
            R_BSP_InterruptControl( BSP_INT_SRC_DPFPUEX,
                                    BSP_INT_CMD_CALL_CALLBACK,
                                    FIT_NO_PTR );
    ...
    while( 1 )
    {
        /* Infinite loop. Return from Non-maskable interrupt handlling routine
           is prohibited. Never use the non-maskable interrupt with an attempt
           to return to the program that was being executed at the time of
           interrupt generation after the exception handling routine is ended.
         */
        R_BSP_NOP();
    }
 ...
 *                Set a breakpoint in this function to determine which source is
 *creating unwanted interrupts. Arguments    : none Return Value : none
 ***********************************************************************************************************************/
R_BSP_ATTRIB_INTERRUPT void undefined_interrupt_source_isr( void )
{
    /* If user has registered a callback for this exception then call it. */
    R_BSP_InterruptControl( BSP_INT_SRC_UNDEFINED_INTERRUPT,
                            BSP_INT_CMD_CALL_CALLBACK,
                            FIT_NO_PTR );
 ...
 *This interrupt will fire if the user tries to access code or data from one of
 *the reserved areas in the memory map, including the areas covered by disabled
 *chip selects. A nop() statement is included here as a convenient place to set
 *a breakpoint during debugging and development, and further handling should be
 *added by the user for their application. Arguments    : none Return value :
 *none
 ***********************************************************************************************************************/
R_BSP_ATTRIB_INTERRUPT void bus_error_isr( void )
{
    ...
    /*
        To find the address that was accessed when the bus error occurred, read
       the register BSC.BERSR2.WORD. The upper 13 bits of this register contain
       the upper 13-bits of the offending address (in 512K byte units)
    */

    /* If user has registered a callback for this exception then call it. */
    R_BSP_InterruptControl( BSP_INT_SRC_BUS_ERROR,
                            BSP_INT_CMD_CALL_CALLBACK,
                            FIT_NO_PTR );
