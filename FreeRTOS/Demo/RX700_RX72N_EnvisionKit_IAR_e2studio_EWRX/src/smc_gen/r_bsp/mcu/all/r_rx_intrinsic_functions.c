 ...
 * Description  : Defines built-in functions that are in CCRX but not in the GCC
 *and IAR compiler.
 ***********************************************************************************************************************/
/**********************************************************************************************************************
 ...
 *         : 31.07.2019 1.02     Modified the compile condition of the below
 *functions.
 *                               - R_BSP_InitTFU
 *                               - R_BSP_CalcSine_Cosine
 *                               - R_BSP_CalcAtan_SquareRoot
 ...
 * Description  : Performs a multiply-and-accumulate operation with the initial
 *value specified by init, the number of multiply-and-accumulate operations
 *specified by count, and the start addresses of values to be multiplied
 *specified by addr1 and addr2. Arguments    : init   - Initial value. count  -
 *Count of multiply-and-accumulate operations. *addr1 - Start address of values
 *1 to be multiplied. *addr2 - Start address of values 2 to be multiplied.
 * Return Value : result - Lower 64 bits of the init + S(data1[n] * data2[n])
 *result. (n=0, 1, ..., const-1)
 ***********************************************************************************************************************/
#if defined( __GNUC__ )
 ...
 * Return Value : data - Result of 1-bit left rotation of data including the C
 *flag.
 ***********************************************************************************************************************/
#if defined( __GNUC__ )
 ...
 * Return Value : data - Result of 1-bit right rotation of data including the C
 *flag.
 ***********************************************************************************************************************/
#if defined( __GNUC__ )
 ...
 *Execution PSW.PM                 PSW.U                           PSW.PM PSW.U
 *                0 (supervisor mode)    0 (interrupt stack)     -->     1 (user
 *mode)       1 (user stack) 0 (supervisor mode)    1 (user stack)          -->
 *1 (user mode)       1 (user stack) 1 (user mode)          1 (user stack) -->
 *NO CHANGE 1 (user mode)          0 (interrupt stack))    <==     N/A Arguments
...
void R_BSP_ChangeToUserMode( void )
{
    ...
    R_BSP_ASM( XCHG 4 [ R2 ].L, R1;
               exchange the value of R2 destination address and the R1 value )
    R_BSP_ASM( ; ( exchange the R1 value of stack and
                   the return address value of caller ) )
...
{
    #if defined( __GNUC__ )
    signed long long result = ( ( signed long long ) __builtin_rx_mvfachi() )
                              << 32;
    result |= ( ( ( signed long long ) __builtin_rx_mvfacmi() ) << 16 ) &
              0xFFFF0000;
    return result;
    #elif defined( __ICCRX__ )
    int64_t result;
 ...
 * Description  : Performs a multiply-and-accumulate operation between data of
 *two bytes each and returns the result as four bytes. The
 *multiply-and-accumulate operation is executed with DSP functional instructions
 *(MULLO, MACLO, and MACHI). Data in the middle of the multiply-and-accumulate
 *operation is retained in ACC as 48-bit data. After all multiply-and-accumulate
 *operations have finished, the contents of ACC are fetched by the MVFACMI
 *instruction and used as the return value of the intrinsic function. Arguments
 *: data1 - Start address of values 1 to be multiplied. data2 - Start address of
 *values 2 to be multiplied. count - Count of multiply-and-accumulate
 *operations. Return Value : S(data1[n] * data2[n]) result.
 ***********************************************************************************************************************/
#if defined( __GNUC__ )
 ...
 * Description  : Performs a multiply-and-accumulate operation between data of
 *two bytes each and returns the result as two bytes. The
 *multiply-and-accumulate operation is executed with DSP functional instructions
 *(MULLO, MACLO, and MACHI). Data in the middle of the multiply-and-accumulate
 *operation is retained in ACC as 48-bit data. After all multiply-and-accumulate
 *operations have finished, rounding is applied to the multiply-and-accumulate
 *operation result of ACC. The macw1 function performs rounding with the "RACW
 *#1" instruction. Arguments    : data1 - Start address of values 1 to be
 *multiplied. data2 - Start address of values 2 to be multiplied. count - Count
 *of multiply-and-accumulate operations. Return Value : Value obtained by
 *rounding the multiply-and-accumulate operation result with the RACW
 *instruction.
 ***********************************************************************************************************************/
#if defined( __GNUC__ )
 ...
 *operation is retained in ACC as 48-bit data. After all multiply-and-accumulate
 *operations have finished, rounding is applied to the multiply-and-accumulate
 *operation result of ACC. the macw2 function performs rounding with the "RACW
 *#2" instruction. Arguments    : data1 - Start address of values 1 to be
 *multiplied. data2 - Start address of values 2 to be multiplied. count - Count
 *of multiply-and-accumulate operations. Return Value : Value obtained by
 *rounding the multiply-and-accumulate operation result with the RACW
 *instruction.
 ...
 * Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_STATIC_INLINE_ASM( bsp_get_bpsw ) void bsp_get_bpsw(
    uint32_t * ret ) {
    R_BSP_ASM_INTERNAL_USED( ret )

        R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R2 ) R_BSP_ASM( MVFC BPSW, R2 )
 ...
 * Note         : This function exists to avoid code analysis errors. Because,
 *when inline assembler function has a return value, the error of "No return, in
 *function returning non-void" occurs.
 ***********************************************************************************************************************/
...
void R_BSP_SetBPC( void * data ) { R_BSP_ASM_INTERNAL_USED( data )
                                       ...
                                       R_BSP_ASM_BEGIN R_BSP_ASM( MVTC R1, BPC )
                                           R_BSP_ASM_END }
 ...
 * Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_STATIC_INLINE_ASM( bsp_get_extb ) void bsp_get_extb(
    uint32_t * ret )
{
    R_BSP_ASM_INTERNAL_USED( ret )
 ...
 * Description  : This function moves the contents of src to the higher-order 32
 *bits of the accumulator. Arguments    : data - Input value. Return Value :
 *none
 ***********************************************************************************************************************/
 ...
 * Description  : This function moves the contents of src to the lower-order 32
 *bits of the accumulator. Arguments    : data - Input value. Return Value :
 *none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_INLINE_ASM( R_BSP_MoveToAccLoLong ) void R_BSP_MoveToAccLoLong(
    int32_t data ) { R_BSP_ASM_INTERNAL_USED( data )
 ...
 * Description  : This function moves the higher-order 32 bits of the
 *accumulator to dest. Arguments    : ret - Return value address. Return Value :
 *none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_STATIC_INLINE_ASM(
    bsp_move_from_acc_hi_long ) void bsp_move_from_acc_hi_long( uint32_t * ret ) {
    R_BSP_ASM_INTERNAL_USED( ret )
 ...
 * Description  : This function moves the higher-order 32 bits of the
 *accumulator to dest. Arguments    : none Return Value : The higher-order 32
 *bits of the accumulator. Note         : This function exists to avoid code
 *analysis errors. Because, when inline assembler function has a return value,
 *the error of "No return, in function returning non-void" occurs.
 ***********************************************************************************************************************/
 ...
 * Description  : This function moves the contents of bits 47 to 16 of the
 *accumulator to dest. Arguments    : ret - Return value address. Return Value :
 *none
 ***********************************************************************************************************************/
...
void bsp_move_from_acc_mi_long( uint32_t * ret ) {
    R_BSP_ASM_INTERNAL_USED( ret )
        ...
        R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R2 ) R_BSP_ASM( MVFACMI R2 )
            R_BSP_ASM( MOV.L R2, [R1] ) R_BSP_ASM( POP R2 ) R_BSP_ASM_END
 ...
 * Description  : This function moves the contents of bits 47 to 16 of the
 *accumulator to dest. Arguments    : none Return Value : The contents of bits
 *47 to 16 of the accumulator. Note         : This function exists to avoid code
 *analysis errors. Because, when inline assembler function has a return value,
 *the error of "No return, in function returning non-void" occurs.
 ***********************************************************************************************************************/
...
void R_BSP_BitSet( uint8_t * data, uint32_t bit ) {
    R_BSP_ASM_INTERNAL_USED( data ) R_BSP_ASM_INTERNAL_USED( bit )
        ...
        R_BSP_ASM_BEGIN R_BSP_ASM( BSET R2, [R1] ) R_BSP_ASM_END
 ...
 * Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_INLINE_ASM( R_BSP_BitClear ) void R_BSP_BitClear( uint8_t * data,
                                                               uint32_t bit ) {
    R_BSP_ASM_INTERNAL_USED( data ) R_BSP_ASM_INTERNAL_USED( bit )

        R_BSP_ASM_BEGIN R_BSP_ASM( BCLR R2, [R1] ) R_BSP_ASM_END
} /* End of function R_BSP_BitClear() */

/***********************************************************************************************************************
 * Function Name: R_BSP_BitReverse
 * Description  : Reverses the value of the specified one bit in the specified
 *1-byte area. Arguments    : data - Address of the target 1-byte area bit  -
 *Position of the bit to be manipulated Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_INLINE_ASM(
    R_BSP_BitReverse ) void R_BSP_BitReverse( uint8_t * data, uint32_t bit )
{
    R_BSP_ASM_INTERNAL_USED( data )
...
void R_BSP_SetDPSW( uint32_t data ) {
    R_BSP_ASM_INTERNAL_USED( data )
        ...
        R_BSP_ASM_BEGIN R_BSP_ASM( MVTDC R1, DPSW ) R_BSP_ASM_END
 ...
 * Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_STATIC_INLINE_ASM( bsp_get_dpsw ) void bsp_get_dpsw(
    uint32_t * ret ) {
    R_BSP_ASM_INTERNAL_USED( ret )

        R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R2 ) R_BSP_ASM( MVFDC DPSW, R2 )
            R_BSP_ASM( MOV.L R2, [R1] ) R_BSP_ASM( POP R2 ) R_BSP_ASM_END
 ...
 * Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_STATIC_INLINE_ASM( bsp_get_decnt ) void bsp_get_decnt(
    uint32_t * ret ) {
    R_BSP_ASM_INTERNAL_USED( ret )

        R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R2 ) R_BSP_ASM( MVFDC DECNT, R2 )
            R_BSP_ASM( MOV.L R2, [R1] ) R_BSP_ASM( POP R2 ) R_BSP_ASM_END
 ...
 * Description  : Uses the trigonometric function unit to calculate the sine and
 *cosine of an angle at the same time (single precision). Arguments    : f -
 *Value in radians from which to calculate the sine and cosine : sin - Address
 *for storing the result of the sine operation : cos - Address for storing the
 *result of the cosine operation Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_INLINE_ASM( R_BSP_CalcSine_Cosine )
void R_BSP_CalcSine_Cosine( float f, float * sin, float * cos ) {
    R_BSP_ASM_BEGIN R_BSP_ASM( PUSH.L R4 ) R_BSP_ASM( MOV.L#81410H, R4 )
        R_BSP_ASM( MOV.L R1, 4 [ R4 ] ) R_BSP_ASM( MOV.L 4 [ R4 ], [R2] )
            R_BSP_ASM( MOV.L[ R4 ], [R3] ) R_BSP_ASM( POP R4 ) R_BSP_ASM_END
} /* End of function R_BSP_CalcSine_Cosine() */

/***********************************************************************************************************************
 * Function Name: R_BSP_CalcAtan_SquareRoot
 * Description  : Uses the trigonometric function unit to calculate the arc
 *tangent of x and y and the square root of the sum of squares of these values
 *at the same time (single precision). Arguments    : y - Coordinate y (the
 *numerator of the tangent) x - Coordinate x (the denominator of the tangent)
 *                atan2 - Address for storing the result of the arc tangent
 *operation for y/x hypot - Address for storing the result of the square root of
 *the sum of squares of x and y Return Value : none
 ***********************************************************************************************************************/
R_BSP_PRAGMA_INLINE_ASM(
    R_BSP_CalcAtan_SquareRoot ) void R_BSP_CalcAtan_SquareRoot( float y,
                                                                float x,
                                                                float * atan2,
                                                                float * hypot )
{
