...
/* RX MCUs come in different packages and different pin counts.
   Each bit of PORTm.PDR corresponds to each pin of port m; I/O direction can be
   specified in 1-bit units. Each bit of PDR corresponding to port m that does
   not exist is reserved. Also, each bit of PDR corresponding to P35 pins is
   reserved, because such pins are input only. Make settings of the reserved bit
   according to the description in section 22.4, Initialization of the Port
   Direction Register (PDR). These values are then ORed into the direction
   registers to set non-existent pins as outputs or inputs, which can help save
   power.
 */
 ...
 * Description  : For MCUs that do not have the maximum number of pins for their
 *group (e.g. MCU with 176 pins when maximum is 224 pins) these 'non-existent'
