 ...
 *         : 10.12.2019 1.01     Removed unnecessary processing from the
 *clock_source_select fucntion. : 17.12.2019 1.02     Fixed warning of
 *clock_source_select function with IAR compiler. : 14.02.2020 1.03     Fixed
 *warning of clock_source_select function with CCRX and IAR compiler.
 ***********************************************************************************************************************/
...
#define BSP_PRV_NORMALIZE_X10 \
    ( 10 ) /* used to avoid floating point arithmetic */
...
/* This macro runs or stops the high-speed on-chip oscillator(HOCO).
   ...
   6. Clock output enable and clock output source is PLL circuit. PLL source is
   HOCO.
   7. Clock output enable and clock output source is PPLL circuit. PPLL source
   is HOCO.
 */
...
/* This macro runs or stops the Main-clock oscillator.
   If the following conditions are satisfied, Main-clock oscillator will
   operate.
   ...
   7. Clock output enable and clock output source is PLL circuit. PLL source is
   Main-clock oscillator.
   8. Clock output enable and clock output source is PPLL circuit. PPLL source
 ...
 */
    ...
    ( ( BSP_CFG_CLKOUT_OUTPUT == 1 ) && ( BSP_CFG_CLKOUT_SOURCE == 6 ) &&   \
      ( BSP_CFG_PLL_SRC == 0 ) )
    #define BSP_PRV_MAIN_CLK_OPERATING \
        ( 1 ) /* Main-clock oscillator is operating. */
#else
    #define BSP_PRV_MAIN_CLK_OPERATING \
        ( 0 ) /* Main-clock oscillator is stopped. */
#endif
 ...
 */
    ...
    #define BSP_PRV_SUB_CLK_OPERATING \
 ...
 */
#if( BSP_CFG_CLOCK_SOURCE == 4 ) || ( BSP_CFG_PHY_CLOCK_SOURCE == 0 ) || \
    ( ( BSP_CFG_CLKOUT_OUTPUT == 1 ) && ( BSP_CFG_CLKOUT_SOURCE == 4 ) )
 ...
 */
#if( BSP_CFG_USB_CLOCK_SOURCE == 3 ) || ( BSP_CFG_PHY_CLOCK_SOURCE == 1 ) || \
    ( ( BSP_CFG_CLKOUT_OUTPUT == 1 ) && ( BSP_CFG_CLKOUT_SOURCE == 6 ) )
 ...
 * Description  : Return the current ICLK frequency in Hz.  Called by
 *R_BSP_GetIClkFreqHz(). The system clock source can be changed at any time via
 *SYSTEM.SCKCR3.BIT.CKSEL, so in order to determine the ICLK frequency we need
 *to first find the current system clock source and then, in some cases where
 *the clock source can be configured for multiple frequencies, calculate the
...
uint32_t get_iclk_freq_hz( void )
{
    ...
    switch( cksel )
    {
        ...
        case BSP_PRV_CKSEL_PLL:
            ...
            /* Casting is valid because it matches the type to the retern value.
             */
            pll_multiplier = ( ( ( ( uint32_t ) ( SYSTEM.PLLCR.BIT.STC + 1 ) ) *
            ...
            /* Casting is valid because it matches the type to the retern value.
             */
            sys_clock_src_freq = ( ( pll_source_freq /
                                     ( ( ( uint32_t ) ( SYSTEM.PLLCR.BIT.PLIDIV +
                                                        1 ) ) *
                                       BSP_PRV_NORMALIZE_X10 ) ) *
                                   pll_multiplier );
            break;
    ...
    /* Finally, divide the system clock source frequency by the currently set
     * ICLK divider to get the ICLK frequency */
...
static void operating_frequency_set( void )
{
    /* Used for constructing value to write to SCKCR, SCKCR2, and SCKCR3
     * registers. */
    ...
    /* Dummy read and compare. cf."5. I/O Registers", "(2) Notes on writing to
       I/O registers" in User's manual. This is done to ensure that the register
       has been written before the next register access. The RX has a pipeline
       architecture so the next instruction could be executed before the
       previous write had finished.
    */
    ...
    #else
        #error \
            "Error! Invalid setting for BSP_CFG_BCLK_OUTPUT in r_bsp_config.h"
    #endif
    ...
    /* To change the ICLK frequency from less than 70 MHz to 70 MHz or higher
       such that the ratio of the frequency after the change to that before the
       change is greater than four, start by setting the frequency to one
       quarter of the intended frequency, wait for 3 us, and then change it to
       the intended frequency. (Refer User's Manual: 9.10.7 Notes on Changing
       the ICLK Frequency.) */
    /* Confirm whether to change ICLK frequency from less than 70 MHz to 70 MHz
     * or higher. */
    if( BSP_ICLK_HZ >= BSP_MCU_ICLK_FREQ_THRESHOLD )
    {
        /* The clock source after reset is LOCO. The frequency of LOCO is 240
           kHz. When the target clock frequency is 70 MHz or higher, it always
           exceeds 4 times the current clock frequency. Therefore, this process
           does not check whether the target clock frequency exceeds 4 times the
    ...
    */
    ...
    if( BSP_ICLK_HZ >= BSP_MCU_ICLK_FREQ_THRESHOLD )
    {
        ...
        /* Dummy read and compare. cf."5. I/O Registers", "(2) Notes on writing
           to I/O registers" in User's manual. This is done to ensure that the
           register has been written before the next register access. The RX has
           a pipeline architecture so the next instruction could be executed
           before the previous write had finished.
         */
 ...
 *function also implements the delays needed for the clocks to stabilize.
 * Arguments    : none
 * Return value : none
 ***********************************************************************************************************************/
static void clock_source_select( void )
{
    ...
    /* Main clock will be not oscillate in software standby or deep software
     * standby modes. */
    ...
    if( 1 == SYSTEM.HOCOCR.BIT.HCSTP )
    {
        ...
        {
            /* The delay period needed is to make sure that the HOCO has
             * stopped. */
            R_BSP_NOP();
        }
        ...
        #else
            #error \
                "Error! Invalid setting for BSP_CFG_HOCO_FREQUENCY in r_bsp_config.h"
        #endif
    ...
    /* If the main oscillator is >10MHz then the main clock oscillator forced
       oscillation control register (MOFCR) must be changed. */
    ...
    else
    {
        ...
        /* Set the oscillation stabilization wait time of the main clock
         * oscillator. */
    ...
    SYSTEM.MOSCWTCR.BYTE = 0x00;
        #else
            #error \
                "Error! Invalid setting for BSP_CFG_MAIN_CLOCK_SOURCE in r_bsp_config.h"
        #endif
    ...
    if( 0 == SYSTEM.RSTSR1.BIT.CWSF )
    {
        ...
        b0       RCKSEL   - Count Source Select - Sub-clock oscillator is
        selected. */
        ...
        b7:b1    Reserved - The write value should be 0.
        b0       SOSTP    - Sub-clock oscillator Stop - Sub-clock oscillator is
        stopped. */
        ...
        #else
            #error \
                "Error! Invalid setting for BSP_CFG_SOSC_DRV_CAP in r_bsp_config.h"
        #endif
        ...
        b4:b0    SSTS - Sub-Clock Oscillator Waiting Time - Waiting time
        is 2.044ms(the maximum value of fLOCO). */
        ...
        /* 6 count of sub-clock : (1000000/32768)*6=183.10546875us
           In the case of LOCO frequency is 264kHz :
           183.10546875/(1000000/264000)=48.33984375cycle
           (48.33984375+2)*(1000000/240000)=209.7493489583333us ("+2" is
           overhead cycle) */
        ...
        b5  AADJP - Automatic Adjustment Period Select - The RADJ.ADJ[5:0]
        setting value is adjusted from the count value of the prescaler every 10
        seconds. b4  AADJE - Automatic Adjustment Enable - Automatic adjustment
        is enabled. b3  RTCOE - RTCOUT Output Enable - RTCOUT output enabled. b2
        ADJ30 - 30-Second Adjustment - 30-second adjustment is executed. b1
        RESET - RTC Software Reset - The prescaler and the target registers for
        RTC software reset are initialized. b0  START - start - Prescaler is
        stopped. */
        RTC.RCR2.BYTE &= 0x7E;
        ...
        b7:b4  PES   - Periodic Interrupt Select - These bits specify the period
        for the periodic interrupt. b3     RTCOS - RTCOUT Output Select - RTCOUT
        outputs 1 Hz. b2     PIE   - Periodic Interrupt Enable - A periodic
        interrupt request is disabled. b1     CIE   - Carry Interrupt Enable - A
        carry interrupt request is disabled. b0     AIE   - Alarm Interrupt
        Enable - An alarm interrupt request is disabled. */
        RTC.RCR1.BYTE &= 0xF8;

        /* Wait for one the sub-clock cycles */
        /* 1 count of sub-clock : (1000000/32768)=30.517578125us
           In the case of LOCO frequency is 264kHz
           : 30.517578125/(1000000/264000)=8.056640625cycle
           (8.056640625+2)*(1000000/240000)=41.902669270833us ("+2" is overhead
           cycle) */
    ...
    else
    {
        ...
        ( ( BSP_PRV_SUB_CLK_OPERATING == 0 ) && ( BSP_CFG_RTC_ENABLE == 0 ) )
        /* SOSCCR - Sub-Clock Oscillator Control Register
        b7:b1    Reserved - The write value should be 0.
        b0       SOSTP    - Sub-clock oscillator Stop - Sub-clock oscillator is
        stopped. */
    ...
    /* Set PLL Multiplier. */
    SYSTEM.PLLCR.BIT.STC = ( ( uint8_t ) ( ( float ) BSP_CFG_PLL_MUL * 2.0 ) ) -
                           1;
    ...
    /* Set PPLL Multiplier. */
    SYSTEM.PPLLCR.BIT.PPLSTC = ( ( uint8_t ) ( ( float ) BSP_CFG_PPLL_MUL *
                                               2.0 ) ) -
                               1;
    ...
    #else
        /* LOCO is not chosen but it cannot be turned off yet since it is still
         * being used. */
    #endif

    /* RX72N has a MEMWAIT register which controls the cycle waiting for access
       to code flash memory. It is set as zero coming out of reset. We only want
       to set this if we are > 120 MHz.
     */
...
static void bsp_clkout_initial_configure( void )
{
    ...
    SYSTEM.CKOCR.BIT.CKOSTP = 0;
        ...
        #else
            #error \
                "Error! Invalid setting for BSP_CFG_CLKOUT_OUTPUT in r_bsp_config.h"
        #endif
    /* Protect on. */
    SYSTEM.PRCR.WORD = 0xA500;
