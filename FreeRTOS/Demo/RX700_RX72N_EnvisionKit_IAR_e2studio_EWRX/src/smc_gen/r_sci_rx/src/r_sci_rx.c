 ...
 *           01.10.2016 1.80    Initial Release. (The remake of the
 *r01an1815ju0170 to the base.) 19.12.2016 1.90    FIT_NO_PTR check added to
 *NULL check. Fixed a bug that may receive data more than the specified number
 *of bytes on Clock Synchronous Mode. Fixed that R_SCI_Control function returns
 *SCI_ERR_INVALID_ARG when using SCI_CMD_EN_CTS_IN on Simple SPI mode. Fix to
 *clear error flag even if callback function is not set. Deleted unnecessary bit
 *mask of SSR register from sci_error function. 07.03.2017 2.00    Fixed a bug
 *that error condition not clear when FIFO enabled. Fixed a bug that where
 *commands used only when FIFO mode is enable did not NULL check. Fixed a bug
 *that sending data is overwrote by new R_SCI_Send() when FIFO(async) enabled.
 *                              Fixed a bug that sending data is break up by new
 *R_SCI_Send() when FIFO(sync) enabled. Fixed a bug that the new FIFO threshold
 *was retained only on first receive. Fixed a bug that callback function work
 *many times at receive interrupt when FIFO(async) enabled. Fixed a bug that the
 *interrupt priority level can be changed only in async mode. 28.09.2018  2.10
 *Added support RX66T Add WAIT_LOOP comments. Fixed a bug that leaking memory in
 *R_SCI_Open() when FIFO(async) enabled. Fix GSCE Code Checker errors.
 ...
 *style. Added support for atomic control. Fixed to comply with GSCE Coding
 *Standards Rev.6.00. Fixed a bug that error when a reception interrupt occurs
 *before incrementing "u_tx_data.buf" in "sci_send_sync_data" and "sci_receive"
 *functions 30.12.2019  3.40    Added support RX66N, RX72N.
 ***********************************************************************************************************************/
                      ...
                      sci_hdl_t * const p_hdl )
{
    ...
    if( ( ( NULL == p_cfg ) || ( NULL == p_hdl ) ) ||
        ( ( FIT_NO_PTR == p_cfg ) || ( FIT_NO_PTR == p_hdl ) ) )
    {
    ...
    if( SCI_MODE_ASYNC == mode )
    {
        ...
        err = sci_init_async( g_handles[ chan ],
                              ( sci_uart_t * ) p_cfg,
                              &priority );
#endif
    }
    else
    {
        ...
        err = sci_init_sync( g_handles[ chan ],
                             ( sci_sync_sspi_t * ) p_cfg,
                             &priority );
#endif
    }
...
static sci_err_t sci_init_queues( uint8_t const chan )
{
    ...
    switch( chan )
    {
        ...
        case( SCI_CH0 ):
        {
            q_err1 = R_BYTEQ_Open( ch0_tx_buf,
                                   SCI_CFG_CH0_TX_BUFSIZ,
                                   &g_handles[ SCI_CH0 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch0_rx_buf,
                                   SCI_CFG_CH0_RX_BUFSIZ,
                                   &g_handles[ SCI_CH0 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH1_INCLUDED
        case( SCI_CH1 ):
        {
            q_err1 = R_BYTEQ_Open( ch1_tx_buf,
                                   SCI_CFG_CH1_TX_BUFSIZ,
                                   &g_handles[ SCI_CH1 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch1_rx_buf,
                                   SCI_CFG_CH1_RX_BUFSIZ,
                                   &g_handles[ SCI_CH1 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH2_INCLUDED
        case( SCI_CH2 ):
        {
            q_err1 = R_BYTEQ_Open( ch2_tx_buf,
                                   SCI_CFG_CH2_TX_BUFSIZ,
                                   &g_handles[ SCI_CH2 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch2_rx_buf,
                                   SCI_CFG_CH2_RX_BUFSIZ,
                                   &g_handles[ SCI_CH2 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH3_INCLUDED
        case( SCI_CH3 ):
        {
            q_err1 = R_BYTEQ_Open( ch3_tx_buf,
                                   SCI_CFG_CH3_TX_BUFSIZ,
                                   &g_handles[ SCI_CH3 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch3_rx_buf,
                                   SCI_CFG_CH3_RX_BUFSIZ,
                                   &g_handles[ SCI_CH3 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH4_INCLUDED
        case( SCI_CH4 ):
        {
            q_err1 = R_BYTEQ_Open( ch4_tx_buf,
                                   SCI_CFG_CH4_TX_BUFSIZ,
                                   &g_handles[ SCI_CH4 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch4_rx_buf,
                                   SCI_CFG_CH4_RX_BUFSIZ,
                                   &g_handles[ SCI_CH4 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH5_INCLUDED
        case( SCI_CH5 ):
        {
            q_err1 = R_BYTEQ_Open( ch5_tx_buf,
                                   SCI_CFG_CH5_TX_BUFSIZ,
                                   &g_handles[ SCI_CH5 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch5_rx_buf,
                                   SCI_CFG_CH5_RX_BUFSIZ,
                                   &g_handles[ SCI_CH5 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH6_INCLUDED
        case( SCI_CH6 ):
        {
            q_err1 = R_BYTEQ_Open( ch6_tx_buf,
                                   SCI_CFG_CH6_TX_BUFSIZ,
                                   &g_handles[ SCI_CH6 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch6_rx_buf,
                                   SCI_CFG_CH6_RX_BUFSIZ,
                                   &g_handles[ SCI_CH6 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH7_INCLUDED
        case( SCI_CH7 ):
        {
            q_err1 = R_BYTEQ_Open( ch7_tx_buf,
                                   SCI_CFG_CH7_TX_BUFSIZ,
                                   &g_handles[ SCI_CH7 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch7_rx_buf,
                                   SCI_CFG_CH7_RX_BUFSIZ,
                                   &g_handles[ SCI_CH7 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH8_INCLUDED
        case( SCI_CH8 ):
        {
            q_err1 = R_BYTEQ_Open( ch8_tx_buf,
                                   SCI_CFG_CH8_TX_BUFSIZ,
                                   &g_handles[ SCI_CH8 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch8_rx_buf,
                                   SCI_CFG_CH8_RX_BUFSIZ,
                                   &g_handles[ SCI_CH8 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH9_INCLUDED
        case( SCI_CH9 ):
        {
            q_err1 = R_BYTEQ_Open( ch9_tx_buf,
                                   SCI_CFG_CH9_TX_BUFSIZ,
                                   &g_handles[ SCI_CH9 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch9_rx_buf,
                                   SCI_CFG_CH9_RX_BUFSIZ,
                                   &g_handles[ SCI_CH9 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH10_INCLUDED
        case( SCI_CH10 ):
        {
            q_err1 = R_BYTEQ_Open( ch10_tx_buf,
                                   SCI_CFG_CH10_TX_BUFSIZ,
                                   &g_handles[ SCI_CH10 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch10_rx_buf,
                                   SCI_CFG_CH10_RX_BUFSIZ,
                                   &g_handles[ SCI_CH10 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH11_INCLUDED
        case( SCI_CH11 ):
        {
            q_err1 = R_BYTEQ_Open( ch11_tx_buf,
                                   SCI_CFG_CH11_TX_BUFSIZ,
                                   &g_handles[ SCI_CH11 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch11_rx_buf,
                                   SCI_CFG_CH11_RX_BUFSIZ,
                                   &g_handles[ SCI_CH11 ]->u_rx_data.que );
            break;
        }
    #endif
    #if SCI_CFG_CH12_INCLUDED
        case( SCI_CH12 ):
        {
            q_err1 = R_BYTEQ_Open( ch12_tx_buf,
                                   SCI_CFG_CH12_TX_BUFSIZ,
                                   &g_handles[ SCI_CH12 ]->u_tx_data.que );
            q_err2 = R_BYTEQ_Open( ch12_rx_buf,
                                   SCI_CFG_CH12_RX_BUFSIZ,
                                   &g_handles[ SCI_CH12 ]->u_rx_data.que );
            break;
        }
    #endif
        default:
                                 ...
                                 uint8_t * const p_priority )
{
    ...
    #if SCI_CFG_PARAM_CHECKING_ENABLE
    if( ( ( SCI_DATA_8BIT != p_cfg->data_size ) &&
          ( SCI_DATA_7BIT != p_cfg->data_size ) ) ||
        ( ( SCI_STOPBITS_1 != p_cfg->stop_bits ) &&
          ( SCI_STOPBITS_2 != p_cfg->stop_bits ) ) ||
        ( ( p_cfg->int_priority < ( BSP_MCU_IPL_MIN + 1 ) ) ||
          ( p_cfg->int_priority > BSP_MCU_IPL_MAX ) ) )
    {
        return SCI_ERR_INVALID_ARG;
    }
    ...
    {
        if( ( SCI_EVEN_PARITY != p_cfg->parity_type ) &&
            ( SCI_ODD_PARITY != p_cfg->parity_type ) )
        {
            return SCI_ERR_INVALID_ARG;
        }
    }
    ...
    else if( ( SCI_CLK_EXT8X != p_cfg->clk_src ) &&
             ( SCI_CLK_EXT16X != p_cfg->clk_src ) )
    {
        return SCI_ERR_INVALID_ARG;
    }
    else
    ...
    /* Configure SMR */
    hdl->rom->regs->SMR.BYTE = ( uint8_t ) ( ( p_cfg->data_size |
                                               p_cfg->stop_bits ) |
                                             ( p_cfg->parity_en |
                                               p_cfg->parity_type ) );
    ...
    if( SCI_CLK_INT == p_cfg->clk_src )
    {
        ...
        {
            hdl->baud_rate = p_cfg->baud_rate; // save baud rate for break
                                               // generation
        }
    }
    else
    {
        ...
        hdl->rom->regs->SEMR.BIT.ABCS = ( SCI_CLK_EXT8X == p_cfg->clk_src ) ? 1
                                                                            : 0;
    }
                                ...
                                uint8_t * const p_priority )
{
    ...
    if( ( SCI_MODE_SSPI == hdl->mode ) &&
        ( SCI_SPI_MODE_0 != p_cfg->spi_mode ) &&
        ( SCI_SPI_MODE_1 != p_cfg->spi_mode ) &&
        ( SCI_SPI_MODE_2 != p_cfg->spi_mode ) &&
        ( SCI_SPI_MODE_3 != p_cfg->spi_mode ) )
    {
        return SCI_ERR_INVALID_ARG;
    }
    else if( ( SCI_MODE_SYNC == hdl->mode ) &&
             ( SCI_SPI_MODE_OFF != p_cfg->spi_mode ) )
    {
        return SCI_ERR_INVALID_ARG;
    }
    else
    ...
    hdl->rom->regs->SCMR.BIT.SINV = ( uint8_t ) ( ( true == p_cfg->invert_data )
                                                      ? 1
                                                      : 0 );

    /* Configure bit order */
    hdl->rom->regs->SCMR.BIT.SDIR = ( uint8_t ) ( ( true == p_cfg->msb_first )
                                                      ? 1
                                                      : 0 );
                                     ...
                                     bool save_rx_data )
{
    ...
    if( true == hdl->tx_idle )
    {
        ...
        if( true == hdl->fifo_ctrl )
        {
            ...
            /* If length is lower than SCI_CFG_CHXX_RX_FIFO_THRESH, FCR.BIT.RTRG
             * register is set to length */
                             ...
                             uint16_t const length )
{
    ...
    #if SCI_CFG_PARAM_CHECKING_ENABLE
    /* Check arguments */
    if( ( ( ( NULL == hdl ) || ( FIT_NO_PTR == hdl ) ) /* Check if hdl is
                                                          available or not   */
          || ( ( NULL == p_src ) ||
               ( FIT_NO_PTR == p_src ) ) ) /* Check if p_src is available or not
                                            */
        || ( ( NULL == p_dst ) || ( FIT_NO_PTR == p_dst ) ) ) /* Check if p_dst
                                                                 is available or
                                                                 not */
    {
        return SCI_ERR_NULL_PTR;
...
static void sci_transfer( sci_hdl_t const hdl )
{
    ...
    /* Get data byte number from que and if the number of data bytes is 0, to
...
static void sci_fifo_transfer( sci_hdl_t const hdl )
{
    ...
    for( cnt = 0; cnt < fifo_num; cnt++ )
    {
        ...
        /* If the queue is empty(true == hdl->tx_idle), exit from FIFO transfer
         * loop */
        if( true == hdl->tx_idle )
        {
            break;
    ...
    /* When the settings of transmit data are completed, set the SSRFIFO.TDFE
     * flag to 0. */
                                        ...
                                        uint16_t const length )
{
    ...
    if( true == hdl->tx_idle )
    {
        ...
        if( true == hdl->fifo_ctrl )
        {
            ...
            {
                /* If length is lower than SCI_CFG_CHXX_RX_FIFO_THRESH,
                 * FCR.BIT.RTRG register is set to length */
                if( length < hdl->rx_curr_thresh )
                {
                    hdl->rom->regs->FCR.BIT.RTRG = length;
                }
...
static void sci_fifo_receive( sci_hdl_t const hdl )
{
    ...
    /* When the readings of receive data are completed, set the SSRFIFO.RDF flag
     * to 0. */
    ...
    if( SCI_MODE_ASYNC == hdl->mode )
    {
        ...
        {
            /* Casting 8 bits to unsigned char type is valid */
            hdl->rom->regs->SSRFIFO
                .BYTE = ( unsigned char ) ~SCI_SSRFIFO_DR_MASK;
        }
    }
...
static void sci_receive_data_match( sci_hdl_t const hdl )
{
    ...
    {
    #if( SCI_CFG_ASYNC_INCLUDED )
        if( 0 == hdl->rom->regs->DCCR.BIT.DCME ) /* DCME automatically set 0
                                                    when data matched */
        {
            hdl->rom->regs->DCCR.BIT.DCMF = 0; /* Clear Data Match Flag */

            if( ( 0 == hdl->rom->regs->DCCR.BIT.DFER ) &&
                ( 0 == hdl->rom->regs->DCCR.BIT.DPER ) ) /* Check framing error
                                                            and parity error */
            {
                /* Casting unsigned char type to unin8_t type is valid */
                byte = ( uint8_t ) ( hdl->rom->regs->CDR.BYTE.L ); /* Read data
                                                                      from
                                                                      comparison
                                                                      data
                                                                      register
...
static void sci_fifo_error( sci_hdl_t const hdl )
{
    ...
    if( 0 != ( reg & SCI_RCVR_ERR_MASK ) )
    {
        ...
        if( 0 != ( reg & SCI_RCVR_ERR_MASK ) )
        {
            ...
            /* Clear error condition */
            /* WAIT_LOOP */
            while( 0x00 != ( SCI_SSRFIFO & SCI_RCVR_ERR_MASK ) ) /* Check PER,
                                                                    FER, ORER
                                                                    flags */
            {
                ssrfifo_data = SCI_SSRFIFO; /* SSRFIFO dummy read */
                SCI_SSRFIFO = ( uint8_t ) ~SCI_RCVR_ERR_MASK; /* PER, FER, ORER
                                                                 clear */
                         ...
                         void * p_args )
{
    ...
    if( ( NULL == p_args ) || ( FIT_NO_PTR == p_args ) )
    {
        ...
        if( ( SCI_CMD_CHANGE_TX_FIFO_THRESH == cmd ) ||
            ( SCI_CMD_CHANGE_RX_FIFO_THRESH == cmd ) )
        {
            return SCI_ERR_NULL_PTR;
        }
    #endif
    #if defined( BSP_MCU_RX64M ) || defined( BSP_MCU_RX71M ) || \
        defined( BSP_MCU_RX65N ) || defined( BSP_MCU_RX66T ) || \
        defined( BSP_MCU_RX72T ) || defined( BSP_MCU_RX72M ) || \
        defined( BSP_MCU_RX72N ) || defined( BSP_MCU_RX66N )
        if( ( SCI_CMD_SET_TXI_PRIORITY == cmd ) ||
            ( SCI_CMD_SET_RXI_PRIORITY == cmd ) )
        {
            return SCI_ERR_NULL_PTR;
        }
    #endif
        ...
        ( SCI_CMD_SET_RXI_PRIORITY == cmd ) )
    {
        /* Casting void* type is valid */
        if( ( 1 > ( *( uint8_t * ) p_args ) ) ||
            ( BSP_MCU_IPL_MAX < ( *( uint8_t * ) p_args ) ) )
        {
            return SCI_ERR_INVALID_ARG;
        }
    }
    ...
    switch( cmd )
    {
        ...
        case( SCI_CMD_EN_CTS_IN ):
        {
            if( SCI_MODE_SSPI != hdl->mode )
            {
                /* PFS & port pins must be configured for CTS prior to calling
                 * this */
                hdl->rom->regs->SCR.BYTE &= ( ~SCI_EN_XCVR_MASK );
                SCI_SCR_DUMMY_READ;
            ...
            {
                /* Can not use CTS in smart card interface mode, simple SPI
                 * mode, and simple I2C mode */
