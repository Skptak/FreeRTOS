...
#define SCI_CFG_CH_INCLUDED_MASK                                          \
    ( ( SCI_CFG_CH0_INCLUDED << 0 ) | ( SCI_CFG_CH1_INCLUDED << 1 ) |     \
      ( SCI_CFG_CH2_INCLUDED << 2 ) | ( SCI_CFG_CH3_INCLUDED << 3 ) |     \
      ( SCI_CFG_CH4_INCLUDED << 4 ) | ( SCI_CFG_CH5_INCLUDED << 5 ) |     \
      ( SCI_CFG_CH6_INCLUDED << 6 ) | ( SCI_CFG_CH7_INCLUDED << 7 ) |     \
      ( SCI_CFG_CH8_INCLUDED << 8 ) | ( SCI_CFG_CH9_INCLUDED << 9 ) |     \
      ( SCI_CFG_CH10_INCLUDED << 10 ) | ( SCI_CFG_CH11_INCLUDED << 11 ) | \
...
#define SCI_EN_XCVR_MASK \
    ( SCI_SCR_RE_MASK | SCI_SCR_TE_MASK | SCI_SCR_REI_MASK | SCI_SCR_TEI_MASK )
...
#define SCI_RCVR_ERR_MASK \
    ( SCI_SSR_ORER_MASK | SCI_SSR_FER_MASK | SCI_SSR_PER_MASK )
#define SCI_SSR_CLR_MASK ( 0xC0U ) /* SSR register cleare mask (11000000b) */
#if SCI_CFG_FIFO_INCLUDED
    #define SCI_SSRFIFO_CLR_MASK \
        ( 0xC6U ) /* SSR register cleare mask (11000110b) */
    #define SCI_SSRFIFO_TDFE_MASK \
        ( 0x80U ) /* SSR register transmit data empty flag mask (10000000b) */
    #define SCI_SSRFIFO_RDF_MASK \
        ( 0x40U ) /* SSR register receive FIFO full flag mask (01000000b) */
    #define SCI_SSRFIFO_DR_MASK \
        ( 0x01U ) /* SSR register receive DR flag mask (00000001b) */
#endif

/* Macros to enable and disable ICU interrupts */
#define ENABLE_RXI_INT ( *hdl->rom->icu_rxi |= hdl->rom->rxi_en_mask )
#define DISABLE_RXI_INT \
...
{
    volatile struct st_sci7 R_BSP_EVENACCESS_SFR * regs; /* base ptr to ch
                                                            registers */
