 ...
 *r01an1815ju0170 to the base.) 28.09.2018 2.10    Added
 *SCI_CFG_DATA_MATCH_INCLUDED for configuration data match function. Fix GSCE
 *Code Checker errors. 01.02.2019 2.20    Added support received data match
 *function for RX65N (SCI10 and SCI11). 20.05.2019 3.00    Added support for
 *GNUC and ICCRX. 28.06.2019 3.10    Added support for RX23W 15.08.2019 3.20
 *Added support received data match function for RX72M (SCI0 to SCI11). Added
 *support FIFO mode for RX72M (SCI7 to SCI11).
 ***********************************************************************************************************************/
...
#else
    #define SCI_TDR( byte ) hdl->rom->regs->TDR = ( byte );
#endif
...
#else
    #define SCI_RDR( byte ) ( byte ) = hdl->rom->regs->RDR;
#endif
