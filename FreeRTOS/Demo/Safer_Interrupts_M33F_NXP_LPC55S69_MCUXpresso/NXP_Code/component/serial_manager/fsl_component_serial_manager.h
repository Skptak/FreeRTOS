 ...
 * Definitions
 ******************************************************************************/
/*! @brief Enable or disable serial manager non-blocking mode (1 - enable, 0 -
 * disable) */
#ifdef DEBUG_CONSOLE_TRANSFER_NON_BLOCKING
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE == 0U ) )
...
#if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    /*! @brief Enable or disable serial manager dual(block and non-block) mode
     * (1 - enable, 0 - disable) */
    #ifdef DEBUG_CONSOLE_TRANSFER_NON_BLOCKING
    #else
        #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
...
/*! @brief SERIAL_PORT_UART_HANDLE_SIZE/SERIAL_PORT_USB_CDC_HANDLE_SIZE + serial
 * manager dedicated size */
#if( ( defined( SERIAL_MANAGER_HANDLE_SIZE_TEMP ) && \
       ( SERIAL_MANAGER_HANDLE_SIZE_TEMP > 0U ) ) )
#else
...
/*! @brief Macro to determine whether use common task. */
    ...
    #define SERIAL_MANAGER_USE_COMMON_TASK ( 0U )
     ...
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    #if( defined( OSA_USED ) &&                          \
         !( defined( SERIAL_MANAGER_USE_COMMON_TASK ) && \
            ( SERIAL_MANAGER_USE_COMMON_TASK > 0U ) ) )
...
/*! @brief Definition of serial manager handle size. */
     ...
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
        ...
        #define SERIAL_MANAGER_HANDLE_SIZE                                    \
            ( SERIAL_MANAGER_HANDLE_SIZE_TEMP + 124U + OSA_TASK_HANDLE_SIZE + \
              OSA_EVENT_HANDLE_SIZE )
    #else /*defined(OSA_USED)*/
        #define SERIAL_MANAGER_HANDLE_SIZE \
            ( SERIAL_MANAGER_HANDLE_SIZE_TEMP + 124U )
    #endif /*defined(OSA_USED)*/
    #define SERIAL_MANAGER_BLOCK_HANDLE_SIZE \
        ( SERIAL_MANAGER_HANDLE_SIZE_TEMP + 16U )
#else
    #define SERIAL_MANAGER_HANDLE_SIZE ( SERIAL_MANAGER_HANDLE_SIZE_TEMP + 12U )
    #define SERIAL_MANAGER_BLOCK_HANDLE_SIZE \
        ( SERIAL_MANAGER_HANDLE_SIZE_TEMP + 12U )
#endif
 ...
 *
 * The macro should be global and could be optional. You could also define
 * serial manager handle by yourself.
 *
 * This is an example,
 * @code
...
#define SERIAL_MANAGER_HANDLE_DEFINE( name )                                    \
    uint32_t name[ ( ( SERIAL_MANAGER_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
                     sizeof( uint32_t ) ) ]
#define SERIAL_MANAGER_BLOCK_HANDLE_DEFINE( name )                       \
    uint32_t name[ (                                                     \
        ( SERIAL_MANAGER_BLOCK_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
        sizeof( uint32_t ) ) ]
/*!
 * @brief Defines the serial manager write handle
 *
 * This macro is used to define a 4 byte aligned serial manager write handle.
 * Then use "(serial_write_handle_t)name" to get the serial manager write
 * handle.
 *
 * The macro should be global and could be optional. You could also define
 * serial manager write handle by yourself.
 *
 * This is an example,
 * @code
...
#define SERIAL_MANAGER_WRITE_HANDLE_DEFINE( name )                       \
    uint32_t name[ (                                                     \
        ( SERIAL_MANAGER_WRITE_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
        sizeof( uint32_t ) ) ]
#define SERIAL_MANAGER_WRITE_BLOCK_HANDLE_DEFINE( name )                       \
    uint32_t name[ (                                                           \
        ( SERIAL_MANAGER_WRITE_BLOCK_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
        sizeof( uint32_t ) ) ]
/*!
 ...
 *
 * The macro should be global and could be optional. You could also define
 * serial manager read handle by yourself.
 *
 * This is an example,
 * @code
...
#define SERIAL_MANAGER_READ_HANDLE_DEFINE( name )                       \
    uint32_t name[ (                                                    \
        ( SERIAL_MANAGER_READ_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
        sizeof( uint32_t ) ) ]
#define SERIAL_MANAGER_READ_BLOCK_HANDLE_DEFINE( name )                       \
    uint32_t name[ (                                                          \
        ( SERIAL_MANAGER_READ_BLOCK_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
        sizeof( uint32_t ) ) ]
...
{
#if defined( SERIAL_MANAGER_NON_BLOCKING_MODE )
    uint8_t * ringBuffer;    /*!< Ring buffer address, it is used to buffer data
                                received by the hardware.    Besides, the memory
                                space    cannot be free during the lifetime of the
...
typedef enum _serial_manager_status
{
    ...
    kStatus_SerialManager_Notify = MAKE_STATUS( kStatusGroup_SERIALMANAGER,
                                                3 ), /*!< Ring buffer is not
                                                        empty */
    kStatus_SerialManager_Canceled = MAKE_STATUS( kStatusGroup_SERIALMANAGER,
                                                  4 ), /*!< the non-blocking
                                                          request is canceled */
    kStatus_SerialManager_HandleConflict = MAKE_STATUS(
        kStatusGroup_SERIALMANAGER,
        5 ), /*!< The handle is opened */
    kStatus_SerialManager_RingBufferOverflow = MAKE_STATUS(
        kStatusGroup_SERIALMANAGER,
        6 ), /*!< The ring buffer is overflowed */
...
typedef void ( *serial_manager_callback_t )(
    void * callbackParam,
...
/*!
 * @brief Initializes a serial manager module with the serial manager handle and
 * the user configuration structure.
 *
 * This function configures the Serial Manager module with user-defined
 * settings. The user can configure the configuration structure. The parameter
 * serialHandle is a pointer to point to a memory space of size
 * #SERIAL_MANAGER_HANDLE_SIZE allocated by the caller. The Serial Manager
 * module supports three types of serial port, UART (includes UART, USART,
 * LPSCI, LPUART, etc), USB CDC and swo. Please refer to #serial_port_type_t for
 * serial port setting. These three types can be set by using
 * #serial_manager_config_t.
 *
 ...
 *  @endcode
 *
 * @param serialHandle Pointer to point to a memory space of size
 * #SERIAL_MANAGER_HANDLE_SIZE allocated by the caller. The handle should be 4
 * byte aligned, because unaligned access doesn't be supported on some devices.
 * You can define the handle in the following two ways:
 * #SERIAL_MANAGER_HANDLE_DEFINE(serialHandle);
 * or
 * uint32_t serialHandle[((SERIAL_MANAGER_HANDLE_SIZE + sizeof(uint32_t) - 1U) /
 * sizeof(uint32_t))];
 * @param config Pointer to user-defined configuration structure.
 * @retval kStatus_SerialManager_Error An error occurred.
 * @retval kStatus_SerialManager_Success The Serial Manager module
 ...
 *
 * This function de-initializes the serial manager module instance. If the
 * opened writing or reading handle is not closed, the function will return
 * kStatus_SerialManager_Busy.
 *
 * @param serialHandle The serial manager module handle pointer.
 * @retval kStatus_SerialManager_Success The serial manager de-initialization
 * succeed.
 * @retval kStatus_SerialManager_Busy Opened reading or writing handle is not
 * closed.
 */
 ...
 *
 * This function Opens a writing handle for the serial manager module. If the
 * serial manager needs to be used in different tasks, the task should open a
 * dedicated write handle for itself by calling #SerialManager_OpenWriteHandle.
 * Since there can only one buffer for transmission for the writing handle at
 * the same time, multiple writing handles need to be opened when the multiple
 * transmission is needed for a task.
 *
 * @param serialHandle The serial manager module handle pointer.
 * The handle should be 4 byte aligned, because unaligned access doesn't be
 * supported on some devices.
 * @param writeHandle The serial manager module writing handle pointer.
 * The handle should be 4 byte aligned, because unaligned access doesn't be
 * supported on some devices. You can define the handle in the following two
 * ways: #SERIAL_MANAGER_WRITE_HANDLE_DEFINE(writeHandle); or uint32_t
 * writeHandle[((SERIAL_MANAGER_WRITE_HANDLE_SIZE + sizeof(uint32_t) - 1U) /
 * sizeof(uint32_t))];
 * @retval kStatus_SerialManager_Error An error occurred.
 ...
 *   static uint8_t s_nonBlockingWelcome1[] = "This is non-blocking writing log
 ...
 *   static uint8_t s_nonBlockingWelcome2[] = "This is non-blocking writing log
 * for task2!\r\n"; SerialManager_OpenWriteHandle((serial_handle_t)serialHandle,
 ...
 */
serial_manager_status_t SerialManager_CloseWriteHandle(
    serial_write_handle_t writeHandle );

/*!
 * @brief Opens a reading handle for the serial manager module.
 *
 * This function Opens a reading handle for the serial manager module. The
 * reading handle can not be opened multiple at the same time. The error code
 * kStatus_SerialManager_Busy would be returned when the previous reading handle
 * is not closed. And there can only be one buffer for receiving for the reading
 ...
 * The handle should be 4 byte aligned, because unaligned access doesn't be
 * supported on some devices. You can define the handle in the following two
 * ways: #SERIAL_MANAGER_READ_HANDLE_DEFINE(readHandle); or uint32_t
 * readHandle[((SERIAL_MANAGER_READ_HANDLE_SIZE + sizeof(uint32_t) - 1U) /
 * sizeof(uint32_t))];
 * @retval kStatus_SerialManager_Error An error occurred.
 ...
 * (serial_read_handle_t)s_serialReadHandle); static uint8_t
 * s_nonBlockingBuffer[64];
 ...
 */
serial_manager_status_t SerialManager_CloseReadHandle(
    serial_read_handle_t readHandle );

/*!
 * @brief Transmits data with the blocking mode.
 *
 * This is a blocking function, which polls the sending queue, waits for the
 * sending queue to be empty. This function sends data using an interrupt
 * method. The interrupt of the hardware could not be disabled. And There can
 * only one buffer for transmission for the writing handle at the same time.
 *
 * @note The function #SerialManager_WriteBlocking and the function
 * SerialManager_WriteNonBlocking cannot be used at the same time. And, the
 * function SerialManager_CancelWriting cannot be used to abort the transmission
 * of this function.
 *
 ...
 * @retval kStatus_SerialManager_Busy Previous transmission still not finished;
 * data not all sent yet.
 * @retval kStatus_SerialManager_Error An error occurred.
 */
 ...
 *
 * This is a blocking function, which polls the receiving buffer, waits for the
 * receiving buffer to be full. This function receives data using an interrupt
 * method. The interrupt of the hardware could not be disabled. And There can
 * only one buffer for receiving for the reading handle at the same time.
 *
 * @note The function #SerialManager_ReadBlocking and the function
 * SerialManager_ReadNonBlocking cannot be used at the same time. And, the
 * function SerialManager_CancelReading cannot be used to abort the transmission
 * of this function.
 *
 ...
 *
 * This is a non-blocking function, which returns directly without waiting for
 * all data to be sent. When all data is sent, the module notifies the upper
 * layer through a TX callback function and passes the status parameter @ref
 * kStatus_SerialManager_Success. This function sends data using an interrupt
 * method. The interrupt of the hardware could not be disabled. And There can
 * only one buffer for transmission for the writing handle at the same time.
 *
 * @note The function #SerialManager_WriteBlocking and the function
 * #SerialManager_WriteNonBlocking cannot be used at the same time. And, the TX
 * callback is mandatory before the function could be used.
 *
 * @param writeHandle The serial manager module handle pointer.
 * @param buffer Start address of the data to write.
 * @param length Length of the data to write.
 ...
 * @retval kStatus_SerialManager_Error An error occurred.
 */
serial_manager_status_t SerialManager_WriteNonBlocking(
    serial_write_handle_t writeHandle,
    uint8_t * buffer,
    uint32_t length );
 ...
 *
 * This is a non-blocking function, which returns directly without waiting for
 * all data to be received. When all data is received, the module driver
 * notifies the upper layer through a RX callback function and passes the status
 * parameter @ref kStatus_SerialManager_Success. This function receives data
 * using an interrupt method. The interrupt of the hardware could not be
 * disabled. And There can only one buffer for receiving for the reading handle
 * at the same time.
 *
 * @note The function #SerialManager_ReadBlocking and the function
 * #SerialManager_ReadNonBlocking cannot be used at the same time. And, the RX
 * callback is mandatory before the function could be used.
 *
 * @param readHandle The serial manager module handle pointer.
 * @param buffer Start address of the data to store the received data.
 ...
 *
 * The function tries to read data from internal ring buffer. If the ring buffer
 * is not empty, the data will be copied from ring buffer to up layer buffer.
 * The copied length is the minimum of the ring buffer and up layer length.
 * After the data is copied, the actual data length is passed by the parameter
 * length. And There can only one buffer for receiving for the reading handle at
 * the same time.
 *
 * @param readHandle The serial manager module handle pointer.
 * @param buffer Start address of the data to store the received data.
 ...
 *
 * The function cancels unfinished send transmission. When the transfer is
 * canceled, the module notifies the upper layer through a TX callback function
 * and passes the status parameter @ref kStatus_SerialManager_Canceled.
 *
 * @note The function #SerialManager_CancelWriting cannot be used to abort the
 * transmission of the function #SerialManager_WriteBlocking.
 *
 * @param writeHandle The serial manager module handle pointer.
 * @retval kStatus_SerialManager_Success Get successfully abort the sending.
 * @retval kStatus_SerialManager_Error An error occurred.
 */
serial_manager_status_t SerialManager_CancelWriting(
    serial_write_handle_t writeHandle );

/*!
 * @brief Cancels unfinished receive transmission.
 *
 * The function cancels unfinished receive transmission. When the transfer is
 * canceled, the module notifies the upper layer through a RX callback function
 * and passes the status parameter @ref kStatus_SerialManager_Canceled.
 *
 * @note The function #SerialManager_CancelReading cannot be used to abort the
 * transmission of the function #SerialManager_ReadBlocking.
 *
 * @param readHandle The serial manager module handle pointer.
 ...
 *
 * This function is used to install the TX callback and callback parameter for
 * the serial manager module. When any status of TX transmission changed, the
 * driver will notify the upper layer by the installed callback function. And
 ...
 */
serial_manager_status_t SerialManager_InstallTxCallback(
    serial_write_handle_t writeHandle,
 ...
 *
 * This function is used to install the RX callback and callback parameter for
 * the serial manager module. When any status of RX transmission changed, the
 * driver will notify the upper layer by the installed callback function. And
 * the status is also passed as status parameter when the callback is called.
 *
 * @param readHandle The serial manager module handle pointer.
 ...
 * @retval kStatus_SerialManager_Success Successfully install the callback.
 */
serial_manager_status_t SerialManager_InstallRxCallback(
    serial_read_handle_t readHandle,
    serial_manager_callback_t callback,
    void * callbackParam );
 ...
 */
serial_manager_status_t SerialManager_EnterLowpower(
    serial_handle_t serialHandle );
 ...
 * @retval kStatus_SerialManager_Success Successful operation.
 */
serial_manager_status_t SerialManager_ExitLowpower(
    serial_handle_t serialHandle );
