...
#include "fsl_component_serial_port_internal.h"
     ...
     * If the source code of the OSA component does not exist, the OSA_USED
     * cannot be defined. OR, If OSA component is not added into project event
     * the OSA source code exists, the OSA_USED also cannot be defined. The
     * source code path of the OSA component is <MCUXpresso_SDK>/components/osa.
     *
     */
    #if defined( OSA_USED )

        #if( defined( SERIAL_MANAGER_USE_COMMON_TASK ) && \
...
typedef enum _serial_manager_transmission_mode
{
    ...
    kSerialManager_TransmissionNonBlocking = 0x1U, /*!< None blocking
                                                      transmission*/
...
typedef struct _serial_manager_handle
{
    ...
    list_label_t runningWriteHandleHead; /*!< The queue of running write handle
                                          */
    list_label_t completedWriteHandleHead; /*!< The queue of completed write
                                              handle */
#endif
 ...
 ******************************************************************************/
     ...
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
             ...
             ( SERIAL_MANAGER_USE_COMMON_TASK > 0U ) )
                                       ...
                                       */
static OSA_TASK_DEFINE( SerialManager_Task,
    ...
    serial_manager_handle_t * handle )
{
    ...
    serial_manager_write_handle_t *
        writeHandle = ( serial_manager_write_handle_t * ) ( void * )
            LIST_GetHead( &handle->runningWriteHandleHead );

    if( writeHandle != NULL )
    {
        writeHandle = ( serial_manager_write_handle_t * ) ( ( uint32_t )
                                                                writeHandle -
                                                            4U );
        switch( handle->type )
        {
            ...
            case kSerialPort_Uart:
                status = Serial_UartWrite( ( ( serial_handle_t ) &handle
         ...
         ( SERIAL_PORT_TYPE_USBCDC > 0U ) )
            ...
            case kSerialPort_Swo:
                status = Serial_SwoWrite( ( ( serial_handle_t ) &handle
                                                ->lowLevelhandleBuffer[ 0 ] ),
                                          writeHandle->transfer.buffer,
                                          writeHandle->transfer.length );
                break;
    #endif
    #if( defined( SERIAL_PORT_TYPE_VIRTUAL ) && \
         ( SERIAL_PORT_TYPE_VIRTUAL > 0U ) )
            case kSerialPort_Virtual:
                status = Serial_PortVirtualWrite(
                    ( ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ),
                    writeHandle->transfer.buffer,
                    writeHandle->transfer.length );
                break;
            ...
            case kSerialPort_Rpmsg:
                status = Serial_RpmsgWrite( ( ( serial_handle_t ) &handle
                                                  ->lowLevelhandleBuffer[ 0 ] ),
                                            writeHandle->transfer.buffer,
                                            writeHandle->transfer.length );
                break;
    ...
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_DUAL_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_DUAL_MODE > 0U ) )
static serial_manager_status_t SerialManager_StartBlockWriting(
    serial_manager_handle_t * handle,
    ...
    uint32_t length )
{
    serial_manager_status_t status = kStatus_SerialManager_Error;
        ...
        #if( defined( SERIAL_PORT_TYPE_UART ) && \
             ( SERIAL_PORT_TYPE_UART > 0U ) )
    if( kSerialPort_Uart == handle->type ) /* Serial port UART */
    {
        status = Serial_UartWriteBlocking( ( ( serial_handle_t ) &handle
                                                 ->lowLevelhandleBuffer[ 0 ] ),
                                           buffer,
                                           length );
    }
    ...
    {
        status = Serial_UsbCdcWrite( ( ( serial_handle_t ) &handle
                                           ->lowLevelhandleBuffer[ 0 ] ),
                                     buffer,
                                     length );
    }
        ...
        if( kSerialPort_Swo == handle->type ) /* Serial port SWO */
    {
        ...
        if( kSerialPort_Virtual == handle->type ) /* Serial port UsbCdcVirtual
                                                   */
    {
        status = Serial_PortVirtualWrite( ( ( serial_handle_t ) &handle
                                                ->lowLevelhandleBuffer[ 0 ] ),
                                          buffer,
                                          length );
    }
        ...
        #endif
        #if( defined( SERIAL_PORT_TYPE_RPMSG ) && \
             ( SERIAL_PORT_TYPE_RPMSG > 0U ) )
        if( kSerialPort_Rpmsg == handle->type ) /* Serial port RPMSG */
    {
        status = Serial_RpmsgWriteBlocking( ( ( serial_handle_t ) &handle
                                                  ->lowLevelhandleBuffer[ 0 ] ),
                                            buffer,
                                            length );
    }
    ...
    uint32_t length )
{
    ...
    if( NULL != readHandle )
    {
        ...
        if( ( kSerialPort_Uart == handle->type ) &&
            ( kSerialManager_Blocking == handle->handleType ) ) /* Serial port
                                                                   UART */
        {
            status = Serial_UartRead( ( ( serial_handle_t ) &handle
                                            ->lowLevelhandleBuffer[ 0 ] ),
                                      buffer,
                                      length );
        }
        ...
        {
            status = Serial_UsbCdcRead( ( ( serial_handle_t ) &handle
                                              ->lowLevelhandleBuffer[ 0 ] ),
                                        buffer,
                                        length );
        }
    ...
    uint32_t length )
{
        ...
        if( kSerialPort_UsbCdc == handle->type ) /* Serial port UsbCdc */
    {
        ...
        if( kSerialPort_Swo == handle->type ) /* Serial port SWO */
    {
        status = Serial_SwoRead( ( ( serial_handle_t ) &handle
                                       ->lowLevelhandleBuffer[ 0 ] ),
                                 buffer,
                                 length );
    }
        ...
        if( kSerialPort_Virtual == handle->type ) /* Serial port UsbCdcVirtual
                                                   */
    {
        status = Serial_PortVirtualRead( ( ( serial_handle_t ) &handle
                                               ->lowLevelhandleBuffer[ 0 ] ),
                                         buffer,
                                         length );
    }
    ...
    {
        status = Serial_RpmsgRead( ( ( serial_handle_t ) &handle
                                         ->lowLevelhandleBuffer[ 0 ] ),
                                   buffer,
                                   length );
    }
...
static void SerialManager_IsrFunction( serial_manager_handle_t * handle )
{
    ...
    switch( handle->type )
    {
        ...
        case kSerialPort_Uart:
         ...
         ( SERIAL_PORT_TYPE_USBCDC > 0U ) )
        case kSerialPort_UsbCdc:
            Serial_UsbCdcIsrFunction(
                ( ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
            break;
    #endif
    #if( defined( SERIAL_PORT_TYPE_SWO ) && ( SERIAL_PORT_TYPE_SWO > 0U ) )
        case kSerialPort_Swo:
            Serial_SwoIsrFunction(
                ( ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
            break;
    #endif
    #if( defined( SERIAL_PORT_TYPE_VIRTUAL ) && \
...
static void SerialManager_Task( void * param )
{
    ...
    if( NULL != handle )
    {
        ...
        do
        {
            if( KOSA_StatusSuccess ==
                OSA_EventWait( ( osa_event_handle_t ) handle->event,
                               osaEventFlagsAll_c,
                               0U,
                               osaWaitForever_c,
                               &ev ) )
    ...
    #endif
        {
            serialWriteHandle = ( serial_manager_write_handle_t * ) ( void * )
                LIST_GetHead( &handle->completedWriteHandleHead );
            while( NULL != serialWriteHandle )
            {
                serialWriteHandle =
                    ( serial_manager_write_handle_t * ) ( ( uint32_t )
                                                              serialWriteHandle -
                                                          4U );
                ...
                {
                    serialWriteHandle
                        ->callback( serialWriteHandle->callbackParam,
                                    &msg,
            ...
            if( NULL != serialReadHandle )
            {
                if( NULL != serialReadHandle->transfer.buffer )
                {
                        ...
                        serialReadHandle->transfer.length )
                    {
                        ...
                        {
                            serialReadHandle
                                ->callback( serialReadHandle->callbackParam,
                                            &msg,
        ...
        #endif
        {
            ringBufferLength = handle->ringBuffer.ringHead +
                               handle->ringBuffer.ringBufferSize -
                               handle->ringBuffer.ringTail;
            ringBufferLength = ringBufferLength %
                               handle->ringBuffer.ringBufferSize;
            ...
            if( 0U != ringBufferLength )
            {
                ...
                if( ( NULL != handle->openedReadHandleHead ) &&
                    ( NULL != handle->openedReadHandleHead->callback ) )
                {
                    handle->openedReadHandleHead
                        ->callback( handle->openedReadHandleHead->callbackParam,
                                    &msg,
...
#if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
static void SerialManager_TxCallback(
    void * callbackParam,
    ...
    serial_manager_status_t status )
{
    ...
    if( NULL != writeHandle )
    {
    ...
    #if( defined( OSA_USED ) && defined( SERIAL_MANAGER_TASK_HANDLE_TX ) && \
         ( SERIAL_MANAGER_TASK_HANDLE_TX == 1 ) )
        #if( defined( SERIAL_MANAGER_USE_COMMON_TASK ) && \
             ( SERIAL_MANAGER_USE_COMMON_TASK > 0U ) )
        ...
        if( kSerialManager_TransmissionNonBlocking ==
            writeHandle->transfer.mode )
        {
            SerialManager_AddTail( &handle->completedWriteHandleHead,
                                   writeHandle );
    #if defined( OSA_USED )
                               ...
                               serial_manager_status_t status )
{
    ...
    {
        handle->ringBuffer
            .ringBuffer[ handle->ringBuffer.ringHead++ ] = message->buffer[ i ];
    ...
    primask = DisableGlobalIRQ();
    if( ( NULL != handle->openedReadHandleHead ) &&
        ( NULL != handle->openedReadHandleHead->transfer.buffer ) )
    {
        if( handle->openedReadHandleHead->transfer.length >
            handle->openedReadHandleHead->transfer.soFar )
        {
            uint32_t
                remainLength = handle->openedReadHandleHead->transfer.length -
                               handle->openedReadHandleHead->transfer.soFar;
            for( uint32_t i = 0; i < MIN( ringBufferLength, remainLength );
                 i++ )
            {
                handle->openedReadHandleHead->transfer
                    .buffer[ handle->openedReadHandleHead->transfer
                                 .soFar ] = handle->ringBuffer.ringBuffer
                                                [ handle->ringBuffer.ringTail ];
                handle->ringBuffer.ringTail++;
                handle->openedReadHandleHead->transfer.soFar++;
                if( handle->ringBuffer.ringTail >=
    ...
    ringBufferLength = handle->ringBuffer.ringBufferSize - 1U -
                       ringBufferLength;

    if( NULL != handle->openedReadHandleHead )
    {
        ( void ) SerialManager_StartReading( handle,
                                             handle->openedReadHandleHead,
                                             NULL,
                                             ringBufferLength );
    }
...
/*
 * This function is used for perdiodic check if the transfer is complete, and
 * will be called in blocking transfer at non-blocking mode. The perdiodic unit
 * is ms and default value is define by
 * SERIAL_MANAGER_WRITE_TIME_DELAY_DEFAULT_VALUE/SERIAL_MANAGER_READ_TIME_DELAY_DEFAULT_VALUE.
 * The function SerialManager_WriteTimeDelay()/SerialManager_ReadTimeDelay() is
 * a weak function, so it could be re-implemented by upper layer.
 */
...
static serial_manager_status_t SerialManager_Write(
    ...
    serial_manager_transmission_mode_t mode )
{
    ...
    if( ( handle->handleType == kSerialManager_Blocking ) ||
        ( kSerialManager_TransmissionBlocking == mode ) )
    {
        return SerialManager_StartBlockWriting( handle,
                                                serialWriteHandle,
                                                buffer,
                                                length );
    }
    ...
    assert( !( ( kSerialManager_TransmissionNonBlocking == mode ) &&
    ...
    if( 0U != isEmpty )
    {
        ...
        if( ( serial_manager_status_t ) kStatus_SerialManager_Success !=
            status )
        {
        #if( defined( USB_CDC_SERIAL_MANAGER_RUN_NO_HOST ) && \
             ( USB_CDC_SERIAL_MANAGER_RUN_NO_HOST == 1 ) )
    ...
    {
        while( serialWriteHandle->transfer.length >
               serialWriteHandle->transfer.soFar )
        {
            ...
            else
            {
                SerialManager_WriteTimeDelay(
                    SERIAL_MANAGER_WRITE_TIME_DELAY_DEFAULT_VALUE );
            }
        }
    }
...
static serial_manager_status_t SerialManager_Read(
    ...
    uint32_t * receivedLength )
{
     ...
     * (kSerialManager_TransmissionBlocking == mode)) */
    {
        return SerialManager_StartReading( handle,
                                           serialReadHandle,
                                           buffer,
                                           length );
    }
    ...
    dataLength = handle->ringBuffer.ringHead +
                 handle->ringBuffer.ringBufferSize -
                 handle->ringBuffer.ringTail;
    dataLength = dataLength % handle->ringBuffer.ringBufferSize;

    for( serialReadHandle->transfer.soFar = 0U;
         serialReadHandle->transfer.soFar < MIN( dataLength, length );
         serialReadHandle->transfer.soFar++ )
    {
        buffer[ serialReadHandle->transfer
                    .soFar ] = handle->ringBuffer
                                   .ringBuffer[ handle->ringBuffer.ringTail ];
        handle->ringBuffer.ringTail++;
        ...
        if( kSerialManager_TransmissionBlocking == mode )
        {
                   ...
                   serialReadHandle->transfer.soFar )
            {
                SerialManager_ReadTimeDelay(
                    SERIAL_MANAGER_READ_TIME_DELAY_DEFAULT_VALUE );
            }
        }
    }
    ...
    uint32_t length )
{
    ...
    return SerialManager_StartWriting( handle,
                                       serialWriteHandle,
                                       buffer,
                                       length );
}
    ...
    const serial_manager_config_t * config )
{
...
#if( defined( SERIAL_PORT_TYPE_UART ) && ( SERIAL_PORT_TYPE_UART > 0U ) )
        case kSerialPort_Uart:
            status = Serial_UartInit( ( ( serial_handle_t ) &handle
                                            ->lowLevelhandleBuffer[ 0 ] ),
                                      config->portConfig );
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
                ...
                status )
            {
                ...
                ( void ) Serial_UartInstallRxCallback(
                    ( ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ),
...
#if( defined( SERIAL_PORT_TYPE_USBCDC ) && ( SERIAL_PORT_TYPE_USBCDC > 0U ) )
        case kSerialPort_UsbCdc:
            status = Serial_UsbCdcInit( ( ( serial_handle_t ) &handle
                                              ->lowLevelhandleBuffer[ 0 ] ),
                                        config->portConfig );
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
...
#if( defined( SERIAL_PORT_TYPE_SWO ) && ( SERIAL_PORT_TYPE_SWO > 0U ) )
        case kSerialPort_Swo:
            status = Serial_SwoInit( ( ( serial_handle_t ) &handle
                                           ->lowLevelhandleBuffer[ 0 ] ),
                                     config->portConfig );
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
        ...
        case kSerialPort_Virtual:
         ...
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
            if( kStatus_SerialManager_Success == status )
            {
                ...
                if( kStatus_SerialManager_Success == status )
                {
                    status = Serial_PortVirtualInstallRxCallback(
                        ( ( serial_handle_t ) &handle
                              ->lowLevelhandleBuffer[ 0 ] ),
                        SerialManager_RxCallback,
                        handle );
...
#if( defined( SERIAL_PORT_TYPE_RPMSG ) && ( SERIAL_PORT_TYPE_RPMSG > 0U ) )
        case kSerialPort_Rpmsg:
            status = Serial_RpmsgInit( ( ( serial_handle_t ) &handle
                                             ->lowLevelhandleBuffer[ 0 ] ),
                                       ( void * ) config );
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
            if( kStatus_SerialManager_Success == status )
            {
                ...
                if( kStatus_SerialManager_Success == status )
                {
                    status = Serial_RpmsgInstallRxCallback(
                        ( ( serial_handle_t ) &handle
                              ->lowLevelhandleBuffer[ 0 ] ),
                        SerialManager_RxCallback,
                        handle );
    ...
    COMMON_TASK_init();
        ...
        #else
    if( KOSA_StatusSuccess !=
        OSA_EventCreate( ( osa_event_handle_t ) handle->event, 1U ) )
    {
        return kStatus_SerialManager_Error;
    }

    if( KOSA_StatusSuccess !=
        OSA_TaskCreate( ( osa_task_handle_t ) handle->taskId,
                        OSA_TASK( SerialManager_Task ),
                        handle ) )
    {
        return kStatus_SerialManager_Error;
    }
...
serial_manager_status_t SerialManager_Deinit( serial_handle_t serialHandle )
{
    ...
    primask = DisableGlobalIRQ();
    if( ( NULL != handle->openedReadHandleHead ) ||
        ( 0U != handle->openedWriteHandleCount ) )
    {
        serialManagerStatus = kStatus_SerialManager_Busy; /*Serial Manager
                                                             Busy*/
    }
    else
    {
...
#if( defined( SERIAL_PORT_TYPE_UART ) && ( SERIAL_PORT_TYPE_UART > 0U ) )
            case kSerialPort_Uart:
                ( void ) Serial_UartDeinit( (
                    ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
                break;
#endif
#if( defined( SERIAL_PORT_TYPE_USBCDC ) && ( SERIAL_PORT_TYPE_USBCDC > 0U ) )
            case kSerialPort_UsbCdc:
                ( void ) Serial_UsbCdcDeinit( (
                    ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
                break;
#endif
#if( defined( SERIAL_PORT_TYPE_SWO ) && ( SERIAL_PORT_TYPE_SWO > 0U ) )
            case kSerialPort_Swo:
                ( void ) Serial_SwoDeinit( (
                    ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
                break;
#endif
#if( defined( SERIAL_PORT_TYPE_VIRTUAL ) && ( SERIAL_PORT_TYPE_VIRTUAL > 0U ) )
            case kSerialPort_Virtual:
                ( void ) Serial_PortVirtualDeinit( (
                    ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
                break;
#endif
#if( defined( SERIAL_PORT_TYPE_RPMSG ) && ( SERIAL_PORT_TYPE_RPMSG > 0U ) )
            case kSerialPort_Rpmsg:
                ( void ) Serial_RpmsgDeinit( (
                    ( serial_handle_t ) &handle->lowLevelhandleBuffer[ 0 ] ) );
                break;
#endif
            default:
    ...
    serial_write_handle_t writeHandle )
{
    ...
    assert( NULL != serialHandle );
    assert( NULL != writeHandle );
    assert( SERIAL_MANAGER_WRITE_HANDLE_SIZE >=
            sizeof( serial_manager_write_handle_t ) );
    ...
    serial_write_handle_t writeHandle )
{
    ...
    serialWriteHandle = ( serial_manager_write_handle_t * ) writeHandle;
    handle = ( serial_manager_handle_t * ) ( void * )
    ...
    serial_read_handle_t readHandle )
{
    ...
    assert( NULL != serialHandle );
    assert( NULL != readHandle );
    assert( SERIAL_MANAGER_READ_HANDLE_SIZE >=
            sizeof( serial_manager_read_handle_t ) );
    ...
    serial_read_handle_t readHandle )
{
    ...
    serialReadHandle = ( serial_manager_read_handle_t * ) readHandle;
    handle = ( serial_manager_handle_t * ) ( void * )
                 serialReadHandle->serialManagerHandle;
...
#if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    return SerialManager_Write( writeHandle,
                                buffer,
                                length,
...
#if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
     ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    return SerialManager_Read( readHandle,
                               buffer,
                               length,
                               kSerialManager_TransmissionBlocking,
                               NULL );
    ...
    serial_write_handle_t writeHandle )
{
    ...
    if( ( NULL != serialWriteHandle->transfer.buffer ) &&
        ( kSerialManager_TransmissionBlocking ==
          serialWriteHandle->transfer.mode ) )
    {
        return kStatus_SerialManager_Error;
    }
    ...
    if( 0U == isNotNeed2Cancel )
    {
        if( 0U != isNotUsed )
        {
            ...
            SerialManager_AddTail( &serialWriteHandle->serialManagerHandle
                                        ->completedWriteHandleHead,
                                   serialWriteHandle );
    #if defined( OSA_USED )

        #if( defined( SERIAL_MANAGER_USE_COMMON_TASK ) && \
             ( SERIAL_MANAGER_USE_COMMON_TASK > 0U ) )
            serialWriteHandle->serialManagerHandle->commontaskMsg
                .callback = SerialManager_Task;
            serialWriteHandle->serialManagerHandle->commontaskMsg
                .callbackParam = serialWriteHandle->serialManagerHandle;
            COMMON_TASK_post_message(
                &serialWriteHandle->serialManagerHandle->commontaskMsg );
        #else
            ( void ) OSA_EventSet( ( osa_event_handle_t ) serialWriteHandle
                                       ->serialManagerHandle->event,
                                   SERIAL_EVENT_DATA_SENT );
        #endif
            ...
            switch( serialWriteHandle->serialManagerHandle->type )
            {
                ...
                case kSerialPort_Uart:
                    ( void ) Serial_UartCancelWrite( (
                        ( serial_handle_t ) &serialWriteHandle
                            ->serialManagerHandle->lowLevelhandleBuffer[ 0 ] ) );
                    break;
    #endif
    #if( defined( SERIAL_PORT_TYPE_USBCDC ) && \
         ( SERIAL_PORT_TYPE_USBCDC > 0U ) )
    ...
    serial_read_handle_t readHandle )
{
    ...
    if( ( NULL != serialReadHandle->transfer.buffer ) &&
        ( kSerialManager_TransmissionBlocking ==
          serialReadHandle->transfer.mode ) )
    {
        return kStatus_SerialManager_Error;
    }
