 ...
 ******************************************************************************/
    ...
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
static serial_manager_status_t Serial_UartEnableReceiving(
    ...
    if( 1U == serialUartHandle->rx.rxEnable )
    {
        ...
        if( kStatus_HAL_UartSuccess !=
            HAL_UartTransferReceiveNonBlocking(
                ( ( hal_uart_handle_t ) &serialUartHandle
                      ->usartHandleBuffer[ 0 ] ),
                &transfer ) )
        #else
        if( kStatus_HAL_UartSuccess !=
                                 ...
                                 void * userData )
{
    ...
    if( ( hal_uart_status_t ) kStatus_HAL_UartRxIdle == status )
    {
        if( ( NULL != serialUartHandle->rx.callback ) )
        {
            ...
            serialUartHandle->rx.callback( serialUartHandle->rx.callbackParam,
                                           &msg,
    ...
    else if( ( hal_uart_status_t ) kStatus_HAL_UartTxIdle == status )
    {
        if( 0U != serialUartHandle->tx.busy )
        {
            ...
            if( ( NULL != serialUartHandle->tx.callback ) )
            {
                ...
                serialUartHandle->tx
                    .callback( serialUartHandle->tx.callbackParam,
                               &msg,
                               kStatus_SerialManager_Success );
            }
        }
                                         ...
                                         void * serialConfig )
{
    ...
    #if( defined( SERIAL_MANAGER_NON_BLOCKING_MODE ) && \
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    serial_port_uart_config_t * uartConfig = ( serial_port_uart_config_t * )
        serialConfig;
    #endif
    ...
    serialManagerStatus = ( serial_manager_status_t ) HAL_UartInit(
        ...
        ( const hal_uart_config_t * ) serialConfig );
    ...
    serial_manager_type_t type = *(
        serial_manager_type_t * ) ( ( uint32_t ) serialHandle -
                                    SERIAL_MANAGER_BLOCK_OFFSET );
    ...
    ( void ) HAL_UartTransferInstallCallback(
        ( ( hal_uart_handle_t ) &serialUartHandle->usartHandleBuffer[ 0 ] ),
        Serial_UartCallback,
        serialUartHandle );
        #else
    ( void ) HAL_UartInstallCallback( ( ( hal_uart_handle_t ) &serialUartHandle
                                            ->usartHandleBuffer[ 0 ] ),
                                      Serial_UartCallback,
                                      serialUartHandle );
        #endif
                                          ...
                                          uint32_t length )
{
    ...
    uartstatus = HAL_UartTransferSendNonBlocking(
        ( ( hal_uart_handle_t ) &serialUartHandle->usartHandleBuffer[ 0 ] ),
        &transfer );
        #else

    uartstatus = HAL_UartSendNonBlocking(
        ( ( hal_uart_handle_t ) &serialUartHandle->usartHandleBuffer[ 0 ] ),
        buffer,
        length );
        #endif
...
serial_manager_status_t Serial_UartInstallTxCallback(
...
serial_manager_status_t Serial_UartInstallRxCallback(
    serial_handle_t serialHandle,
    serial_manager_callback_t callback,
    void * callbackParam )
{
...
serial_manager_status_t Serial_UartEnterLowpower( serial_handle_t serialHandle )
{
    ...
    uartstatus = HAL_UartEnterLowpower(
        ( ( hal_uart_handle_t ) &serialUartHandle->usartHandleBuffer[ 0 ] ) );
    assert( kStatus_HAL_UartSuccess == uartstatus );
    ( void ) uartstatus;
...
serial_manager_status_t Serial_UartExitLowpower( serial_handle_t serialHandle )
{
    ...
    uartstatus = HAL_UartExitLowpower(
        ( ( hal_uart_handle_t ) &serialUartHandle->usartHandleBuffer[ 0 ] ) );
    assert( kStatus_HAL_UartSuccess == uartstatus );
    ( void ) uartstatus;
         ...
         ( SERIAL_MANAGER_NON_BLOCKING_MODE > 0U ) )
    serial_manager_type_t type = *(
        serial_manager_type_t * ) ( void * ) ( ( uint8_t * ) serialHandle -
                                               SERIAL_MANAGER_BLOCK_OFFSET );
    if( type != kSerialManager_Blocking )
    {
