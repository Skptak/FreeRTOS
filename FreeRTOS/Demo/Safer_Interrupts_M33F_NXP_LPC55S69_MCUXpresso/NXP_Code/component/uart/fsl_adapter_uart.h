...
/*! @brief Enable or disable UART adapter non-blocking mode (1 - enable, 0 -
 * disable) */
    ...
    #define UART_ADAPTER_NON_BLOCKING_MODE ( 1U )
        ...
        #endif
    #else
        /* The default value 3 is used to support different ARM Core, such as
         * CM0P, CM4, CM7, and CM33, etc. The minimum number of priority bits
         * implemented in the NVIC is 2 on these SOCs. The value of mininum
...
/*! @brief Definition of uart dma adapter software idleline detection timeout
 * value in ms. */
...
/*! @brief Definition of uart adapter handle size. */
    ...
    #define HAL_UART_HANDLE_SIZE \
        ( 92U + HAL_UART_ADAPTER_LOWPOWER * 16U + HAL_UART_DMA_ENABLE * 4 )
    #define HAL_UART_BLOCK_HANDLE_SIZE \
        ( 8U + HAL_UART_ADAPTER_LOWPOWER * 16U + HAL_UART_DMA_ENABLE * 4 )
#else
    #define HAL_UART_HANDLE_SIZE \
        ( 8U + HAL_UART_ADAPTER_LOWPOWER * 16U + HAL_UART_DMA_ENABLE * 4 )
#endif
...
/*! @brief Definition of uart dma adapter handle size. */
    ...
    #if( defined( FSL_FEATURE_SOC_DMA_COUNT ) && \
         ( FSL_FEATURE_SOC_DMA_COUNT > 0U ) )
        #define HAL_UART_DMA_HANDLE_SIZE ( 124U )
    #elif( defined( FSL_FEATURE_SOC_EDMA_COUNT ) && \
           ( FSL_FEATURE_SOC_EDMA_COUNT > 0U ) )
 ...
 *
 * The macro should be global and could be optional. You could also define uart
 * handle by yourself.
 *
 * This is an example,
 * @code
...
#define UART_HANDLE_DEFINE( name )                                        \
    uint32_t name[ ( ( HAL_UART_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
                     sizeof( uint32_t ) ) ]

#if( defined( HAL_UART_DMA_ENABLE ) && ( HAL_UART_DMA_ENABLE > 0U ) )
    #define UART_DMA_HANDLE_DEFINE( name )                                   \
        uint32_t                                                             \
            name[ ( ( HAL_UART_DMA_HANDLE_SIZE + sizeof( uint32_t ) - 1U ) / \
                    sizeof( uint32_t ) ) ]
#endif

/*! @brief Whether enable transactional function of the UART. (0 - disable, 1 -
 * enable) */
...
typedef enum _hal_uart_status
{
    ...
    kStatus_HAL_UartTxBusy = MAKE_STATUS( kStatusGroup_HAL_UART, 1 ), /*!< TX
                                                                         busy */
    kStatus_HAL_UartRxBusy = MAKE_STATUS( kStatusGroup_HAL_UART, 2 ), /*!< RX
                                                                         busy */
    kStatus_HAL_UartTxIdle = MAKE_STATUS( kStatusGroup_HAL_UART,
                                          3 ), /*!< HAL UART transmitter is
                                                  idle. */
    kStatus_HAL_UartRxIdle = MAKE_STATUS( kStatusGroup_HAL_UART,
                                          4 ), /*!< HAL UART receiver is idle */
    kStatus_HAL_UartBaudrateNotSupport = MAKE_STATUS( kStatusGroup_HAL_UART,
                                                      5 ), /*!< Baudrate is not
                                                              support in current
                                                              clock source */
    kStatus_HAL_UartProtocolError = MAKE_STATUS( kStatusGroup_HAL_UART,
                                                 6 ), /*!< Error occurs for
                                                         Noise, Framing, Parity,
                                                         etc. For transactional
                                                         transfer, The up layer
                                                         needs to abort the
                                                         transfer and then
                                                         starts again */
    kStatus_HAL_UartError = MAKE_STATUS( kStatusGroup_HAL_UART,
                                         7 ), /*!< Error occurs on HAL UART */
} hal_uart_status_t;
...
typedef struct _hal_uart_config
{
    ...
    hal_uart_parity_mode_t parityMode; /*!< Parity mode, disabled (default),
                                          even, odd */
    hal_uart_stop_bit_count_t stopBitCount; /*!< Number of stop bits, 1 stop bit
    ...
    uint8_t instance; /*!< Instance (0 - UART0, 1 - UART1, ...), detail
                         information please refer to the SOC corresponding RM.
                           Invalid instance value will cause initialization
                         failure. */
#if( defined( UART_ADAPTER_NON_BLOCKING_MODE ) && \
     ( UART_ADAPTER_NON_BLOCKING_MODE > 0U ) )
...
typedef struct _hal_uart_dma_config_t
{
    ...
    #endif
    #if defined( FSL_FEATURE_EDMA_HAS_CHANNEL_MUX ) && \
...
/*! @brief UART transfer callback function. */
typedef void ( *hal_uart_dma_transfer_callback_t )(
...
/*!
 * @brief Initializes a UART instance with the UART handle and the user
 * configuration structure.
 *
 * This function configures the UART module with user-defined settings. The user
 * can configure the configuration structure. The parameter handle is a pointer
 * to point to a memory space of size #HAL_UART_HANDLE_SIZE allocated by the
 * caller. Example below shows how to use this API to configure the UART.
 *  @code
 ...
 *  @endcode
 *
 * @param handle Pointer to point to a memory space of size
 * #HAL_UART_HANDLE_SIZE allocated by the caller. The handle should be 4 byte
 * aligned, because unaligned access doesn't be supported on some devices. You
 * can define the handle in the following two ways: #UART_HANDLE_DEFINE(handle);
 * or
 * uint32_t handle[((HAL_UART_HANDLE_SIZE + sizeof(uint32_t) - 1U) /
 * sizeof(uint32_t))];
 * @param config Pointer to user-defined configuration structure.
 * @retval kStatus_HAL_UartBaudrateNotSupport Baudrate is not support in current
 * clock source.
 ...
 * for RX FIFO to have data, and reads data from the RX register.
 *
 * @note The function #HAL_UartReceiveBlocking and the function
 * HAL_UartTransferReceiveNonBlocking cannot be used at the same time. And, the
 * function HAL_UartTransferAbortReceive cannot be used to abort the
 ...
 * @retval kStatus_HAL_UartParityError A parity error occurred while receiving
 * data.
 ...
 *
 * @note The function #HAL_UartSendBlocking and the function
 * HAL_UartTransferSendNonBlocking cannot be used at the same time. And, the
 * function HAL_UartTransferAbortSend cannot be used to abort the transmission
 * of this function.
 *
 * @param handle UART handle pointer.
...
/*!
 * @name Transactional
 * @note The transactional API and the functional API cannot be used at the same
 * time. The macro #HAL_UART_TRANSFER_MODE is used to set which one will be
 * used. If #HAL_UART_TRANSFER_MODE is zero, the functional API with
 * non-blocking mode will be used. Otherwise, transactional API will be used.
 * @{
 */
 ...
 *
 * This function is used to install the callback and callback parameter for UART
 * module. When any status of the UART changed, the driver will notify the upper
 * layer by the installed callback function. And the status is also passed as
 * status parameter when the callback is called.
 *
 * @param handle UART handle pointer.
 ...
 */
hal_uart_status_t HAL_UartTransferInstallCallback(
 ...
 * function, which returns directly without waiting for all data to be received.
 ...
 * through a callback function and passes the status parameter @ref
 * kStatus_UART_RxIdle.
 *
 * @note The function #HAL_UartReceiveBlocking and the function
 * #HAL_UartTransferReceiveNonBlocking cannot be used at the same time.
 *
 * @param handle UART handle pointer.
 * @param transfer UART transfer structure, see #hal_uart_transfer_t.
 * @retval kStatus_HAL_UartSuccess Successfully queue the transfer into transmit
 * queue.
 * @retval kStatus_HAL_UartRxBusy Previous receive request is not finished.
 * @retval kStatus_HAL_UartError An error occurred.
 */
hal_uart_status_t HAL_UartTransferReceiveNonBlocking(
    hal_uart_handle_t handle,
 ...
 * to the TX register. When all data is written to the TX register in the ISR,
 * the UART driver calls the callback function and passes the @ref
 * kStatus_UART_TxIdle as status parameter.
 *
 * @note The function #HAL_UartSendBlocking and the function
 * #HAL_UartTransferSendNonBlocking cannot be used at the same time.
 *
 * @param handle UART handle pointer.
 * @param transfer UART transfer structure. See #hal_uart_transfer_t.
 * @retval kStatus_HAL_UartSuccess Successfully start the data transmission.
 * @retval kStatus_HAL_UartTxBusy Previous transmission still not finished; data
 * not all written to TX register yet.
 * @retval kStatus_HAL_UartError An error occurred.
 */
hal_uart_status_t HAL_UartTransferSendNonBlocking(
    hal_uart_handle_t handle,
    hal_uart_transfer_t * transfer );
 ...
 * the remainBytes to know how many bytes are not received yet.
 *
 * @note The function #HAL_UartTransferAbortReceive cannot be used to abort the
 * transmission of the function #HAL_UartReceiveBlocking.
 *
 * @param handle UART handle pointer.
 ...
 * remainBytes to find out how many bytes are not sent out.
 *
 * @note The function #HAL_UartTransferAbortSend cannot be used to abort the
 * transmission of the function #HAL_UartSendBlocking.
 *
 * @param handle UART handle pointer.
 ...
 * @note The functional API and the transactional API cannot be used at the same
 * time. The macro #HAL_UART_TRANSFER_MODE is used to set which one will be
 * used. If #HAL_UART_TRANSFER_MODE is zero, the functional API with
 * non-blocking mode will be used. Otherwise, transactional API will be used.
 * @{
 ...
 *
 * This function is used to install the callback and callback parameter for UART
 * module. When non-blocking sending or receiving finished, the adapter will
 * notify the upper layer by the installed callback function. And the status is
 * also passed as status parameter when the callback is called.
 *
 * @param handle UART handle pointer.
 * @param callback The callback function.
 * @param callbackParam The parameter of the callback function.
 ...
 *
 * @note The function #HAL_UartReceiveBlocking and the function
 * #HAL_UartReceiveNonBlocking cannot be used at the same time.
 *
 * @param handle UART handle pointer.
 * @param data Start address of the data to write.
 * @param length Size of the data to write.
 ...
 *
 * @note The function #HAL_UartSendBlocking and the function
 * #HAL_UartSendNonBlocking cannot be used at the same time.
 *
 * @param handle UART handle pointer.
 * @param data Start address of the data to write.
 * @param length Size of the data to write.
 ...
 * the remainBytes to know how many bytes are not received yet.
 *
 * @note The function #HAL_UartAbortReceive cannot be used to abort the
 * transmission of the function #HAL_UartReceiveBlocking.
 *
 * @param handle UART handle pointer.
 * @retval kStatus_Success Get successfully abort the receiving.
 ...
 * remainBytes to find out how many bytes are not sent out.
 *
 * @note The function #HAL_UartAbortSend cannot be used to abort the
 * transmission of the function #HAL_UartSendBlocking.
 *
 * @param handle UART handle pointer.
 * @retval kStatus_Success Get successfully abort the sending.
...
/*!
 * @brief Initializes a UART dma instance with the UART dma handle and the user
 * configuration structure.
 *
 * This function configures the UART dma module with user-defined settings. The
 * user can configure the configuration structure. The parameter handle is a
 * pointer to point to a memory space of size #HAL_UART_DMA_HANDLE_SIZE
 * allocated by the caller. Example below shows how to use this API to configure
 * the UART.
 *  @code
 *
 ...
 *
 *  Init uart dma
 *  HAL_UartDMAInit((hal_uart_handle_t *)g_uartHandle, (hal_uart_dma_handle_t
 * *)g_uartDmaHandle, &dmaConfig);
 *  @endcode
 *
 * @param handle UART handle pointer.
 * @param dmaHandle Pointer to point to a memory space of size
 * #HAL_UART_DMA_HANDLE_SIZE allocated by the caller. The handle should be 4
 * byte aligned, because unaligned access doesn't be supported on some devices.
 * You can define the handle in the following two ways:
 * #UART_DMA_HANDLE_DEFINE(handle);
 * or
 * uint32_t handle[((HAL_UART_DMA_HANDLE_SIZE + sizeof(uint32_t) - 1U) /
 * sizeof(uint32_t))];
 ...
 *
 * This function will abort uart dma receive/send transfer and deinitialize
 * UART.
 *
 * @param handle UART handle pointer.
 ...
 *
 * This function is used to install the callback and callback parameter for UART
 * DMA module. When any status of the UART DMA changed, the driver will notify
 * the upper layer by the installed callback function. And the status is also
 ...
 */
hal_uart_dma_status_t HAL_UartDMATransferInstallCallback(
    hal_uart_handle_t handle,
 ...
 *
 * This function receives data using an dma method. This is a non-blocking
 * function, which returns directly without waiting for all data to be received.
 ...
 * through a callback function and passes the status parameter @ref
 * kStatus_HAL_UartDmaRxIdle.
 *
 * When an idleline is detected, the UART DMA adapter notifies the upper layer
 * through a callback function, and passes the status parameter @ref
 * kStatus_HAL_UartDmaIdleline. For the UARTs without hardware idleline
 * interrupt(like usart), it will use a software idleline detection method with
 * the help of TimerManager.
 *
 * When the soc support cache, uplayer should do cache maintain operations for
 * transfer buffer before call this API.
 *
 * @param handle UART handle pointer.
 * @param data data Start address of the buffer to store the received data.
 * @param length Size of the buffer.
 * @param receiveAll Idleline interrupt will not end transfer process if set
 * true.
 ...
 *
 * This function sends data using an dma method. This is a non-blocking
 * function, which returns directly without waiting for all data to be written
 * to the TX register. When all data is written to the TX register by DMA, the
 * UART DMA driver calls the callback function and passes the @ref
 * kStatus_HAL_UartDmaTxIdle as status parameter.
 *
 * When the soc support cache, uplayer should do cache maintain operations for
 * transfer buffer before call this API.
 *
 ...
 * @retval kStatus_HAL_UartDmaSuccess Get successfully through the parameter \p
 * reCount.
 */
