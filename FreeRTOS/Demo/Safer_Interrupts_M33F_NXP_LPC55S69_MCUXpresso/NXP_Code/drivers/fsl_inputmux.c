...
void INPUTMUX_Init( INPUTMUX_Type * base )
{
    ...
    #if defined( FSL_FEATURE_INPUTMUX_HAS_NO_INPUTMUX_CLOCK_SOURCE ) && \
        FSL_FEATURE_INPUTMUX_HAS_NO_INPUTMUX_CLOCK_SOURCE
                            ...
                            inputmux_connection_t connection )
{
    ...
    output_id = ( ( uint32_t ) ( connection ) ) &
                ( ( 1UL << PMUX_SHIFT ) - 1U );
    /* programm signal */
    *( volatile uint32_t * ) ( ( ( uint32_t ) base ) + pmux_id +
                               ( index * 4U ) ) = output_id;
}
                            ...
                            bool enable )
{
    ...
    #if defined( FSL_FEATURE_INPUTMUX_HAS_CHANNEL_MUX ) && \
    ...
    if( enable &&
        ( ( ( ( uint32_t ) signal ) & ( 1UL << CHMUX_AVL_SHIFT ) ) != 0U ) )
    {
        chmux_offset = ( ( ( uint32_t ) signal ) >> CHMUX_OFF_SHIFT ) &
                       ( ( 1UL << ( CHMUX_AVL_SHIFT - CHMUX_OFF_SHIFT ) ) -
                         1UL );
        chmux_value = ( ( ( uint32_t ) signal ) >> CHMUX_VAL_SHIFT ) &
                      ( ( 1UL << ( CHMUX_OFF_SHIFT - CHMUX_VAL_SHIFT ) ) -
                        1UL );
        *( volatile uint32_t * ) ( ( ( uint32_t ) base ) +
                                   chmux_offset ) = chmux_value;
    }
