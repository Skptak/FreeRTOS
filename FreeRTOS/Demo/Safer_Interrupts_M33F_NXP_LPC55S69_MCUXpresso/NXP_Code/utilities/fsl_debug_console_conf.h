 ...
 * Warning: If you want to use non-blocking transfer,please make sure the
 * corresponding IO interrupt is enable, otherwise there is no output. And
 * non-blocking is combine with buffer, no matter bare-metal or rtos. Below
 * shows how to configure in your project if you want to use non-blocking mode.
 * For IAR, right click project and select "Options", define it in "C/C++
 * Compiler->Preprocessor->Defined symbols". For KEIL, click "Options for
 * Target…", define it in "C/C++->Preprocessor Symbols->Define". For ARMGCC,
 * open CmakeLists.txt and add the following lines, "SET(CMAKE_C_FLAGS_DEBUG
 * "${CMAKE_C_FLAGS_DEBUG} -DDEBUG_CONSOLE_TRANSFER_NON_BLOCKING")" for debug
 * target. "SET(CMAKE_C_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE}
 * -DDEBUG_CONSOLE_TRANSFER_NON_BLOCKING")" for release target. For MCUxpresso,
 * right click project and select "Properties", define it in "C/C++
 * Build->Settings->MCU C Complier->Preprocessor".
 *
 */
#ifdef DEBUG_CONSOLE_TRANSFER_NON_BLOCKING
    /*! @brief define the transmit buffer length which is used to store the
     * multi task log, buffer is enabled automatically when non-blocking
     * transfer is using, This value will affect the RAM's ultilization, should
     * be set per paltform's capability and software requirement. If it is
     * configured too small, log maybe missed , because the log will not be
     * buffered if the buffer is full, and the print will return immediately
     * with -1. And this value should be multiple of 4 to meet memory alignment.
     *
     */
    ...
    /*! @brief define the receive buffer length which is used to store the user
     * input, buffer is enabled automatically when non-blocking transfer is
     * using, This value will affect the RAM's ultilization, should be set per
     * paltform's capability and software requirement. If it is configured too
     * small, log maybe missed, because buffer will be overwrited if buffer is
     * too small. And this value should be multiple of 4 to meet memory
     * alignment.
     *
     */
     ...
     * If the macro is zero, the reliable TX function of the debug console is
     * disabled. When the macro is zero, the string of PRINTF will be thrown
     * away after the transmit buffer is full.
     */
...
/*!@ brief define the MAX log length debug console support , that is when you
 * call printf("log", x);, the log length can not bigger than this value. This
 * macro decide the local log buffer length, the buffer locate at stack, the
 * stack maybe overflow if the buffer is too big and current task stack size not
 * big enough.
 */
...
/*!@ brief define the buffer support buffer scanf log length, that is when you
 * call scanf("log", &x);, the log length can not bigger than this value. As
 * same as the DEBUG_CONSOLE_BUFFER_PRINTF_MAX_LOG_LEN.
 */
 ...
 * If other RTOS is used, you can implement the RTOS's specific synchronization
 * mechanism in fsl.log.c If synchronization is disabled, log maybe messed on
 * terminal.
 */
#ifndef DEBUG_CONSOLE_DISABLE_RTOS_SYNCHRONIZATION
    #ifdef DEBUG_CONSOLE_TRANSFER_NON_BLOCKING
        #ifdef SDK_OS_FREE_RTOS
            #define DEBUG_CONSOLE_SYNCHRONIZATION_MODE \
