...
static StaticSemaphore_t s_debugConsoleReadSemaphoreStatic;
    #endif
    #if( defined( DEBUG_CONSOLE_RX_ENABLE ) && \
         ( DEBUG_CONSOLE_RX_ENABLE > 0U ) )
...
#endif /* DEBUG_CONSOLE_SYNCHRONIZATION_MODE == \
          DEBUG_CONSOLE_SYNCHRONIZATION_FREERTOS */
 ...
 *such as :
 * #elif(DEBUG_CONSOLE_SYNCHRONIZATION_MODE ==
 *DDEBUG_CONSOLE_SYNCHRONIZATION_xxx)
 */
 ...
 ******************************************************************************/
/*!
 * @brief This is a printf call back function which is used to relocate the log
 * to buffer or print the log immediately when the local buffer is full.
 *
...
int DbgConsole_ReadCharacter( uint8_t * ch );
     ...
     defined( DEBUG_CONSOLE_TRANSFER_NON_BLOCKING ) && \
     ( defined( DEBUG_CONSOLE_TX_RELIABLE_ENABLE ) &&  \
       ( DEBUG_CONSOLE_TX_RELIABLE_ENABLE > 0U ) ) )
...
static void DbgConsole_SerialManagerTxCallback(
    void * callbackParam,
    ...
    serial_manager_status_t status )
{
    ...
    if( ioState->writeRingBuffer.ringTail >=
        ioState->writeRingBuffer.ringBufferSize )
    {
    ...
    {
        if( ioState->writeRingBuffer.ringTail !=
            ioState->writeRingBuffer.ringHead )
        {
            if( ioState->writeRingBuffer.ringHead >
                ioState->writeRingBuffer.ringTail )
            {
                sendDataLength = ioState->writeRingBuffer.ringHead -
                                 ioState->writeRingBuffer.ringTail;
            }
            else
            {
                sendDataLength = ioState->writeRingBuffer.ringBufferSize -
                                 ioState->writeRingBuffer.ringTail;
            }

            ( void ) SerialManager_WriteNonBlocking(
                ( ( serial_write_handle_t ) &ioState
                      ->serialWriteHandleBuffer[ 0 ] ),
                &ioState->writeRingBuffer
                     .ringBuffer[ ioState->writeRingBuffer.ringTail ],
                sendDataLength );
        }
    }
...
static void DbgConsole_SerialManagerRxCallback(
    void * callbackParam,
    serial_manager_callback_message_t * message,
    serial_manager_status_t status )
{
...
status_t DbgConsole_ReadOneCharacter( uint8_t * ch )
{
    ...
    serial_manager_status_t status = kStatus_SerialManager_Error;
        ...
        #if defined( DEBUG_CONSOLE_TRANSFER_NON_BLOCKING )
    status = SerialManager_ReadNonBlocking(
        ( ( serial_read_handle_t ) &s_debugConsoleState
              .serialReadHandleBuffer[ 0 ] ),
        ch,
        1 );
        #else  /*defined(DEBUG_CONSOLE_TRANSFER_NON_BLOCKING)*/
    status = SerialManager_ReadBlocking(
        ( ( serial_read_handle_t ) &s_debugConsoleState
              .serialReadHandleBuffer[ 0 ] ),
        ch,
        1 );
...
#endif /*(defined(DEBUG_CONSOLE_RX_ENABLE) && (DEBUG_CONSOLE_RX_ENABLE > \
          0U))*/
}
...
int DbgConsole_SendData( uint8_t * ch, size_t size )
{
    ...
    if( s_debugConsoleState.writeRingBuffer.ringHead !=
        s_debugConsoleState.writeRingBuffer.ringTail )
    {
        txBusy = 1;
        sendDataLength = ( s_debugConsoleState.writeRingBuffer.ringHead +
    ...
    sendDataLength = s_debugConsoleState.writeRingBuffer.ringBufferSize -
                     sendDataLength - 1U;
    ...
    {
        s_debugConsoleState.writeRingBuffer.ringBuffer
            [ s_debugConsoleState.writeRingBuffer.ringHead++ ] = ch[ i ];
        if( s_debugConsoleState.writeRingBuffer.ringHead >=
            s_debugConsoleState.writeRingBuffer.ringBufferSize )
        {
    ...
    if( txBusy == 0 )
    {
        ...
        status = ( status_t ) SerialManager_WriteNonBlocking(
            ( ( serial_write_handle_t ) &s_debugConsoleState
                  .serialWriteHandleBuffer[ 0 ] ),
            &s_debugConsoleState.writeRingBuffer
                 .ringBuffer[ s_debugConsoleState.writeRingBuffer.ringTail ],
            sendDataLength );
    }
    EnableGlobalIRQ( regPrimask );
#else
...
{
#if defined( DEBUG_CONSOLE_TRANSFER_NON_BLOCKING )
    #if( defined( DEBUG_CONSOLE_TX_RELIABLE_ENABLE ) && \
         ( DEBUG_CONSOLE_TX_RELIABLE_ENABLE > 0U ) )
    serial_manager_status_t status = kStatus_SerialManager_Error;
    uint32_t sendDataLength;
    ...
    do
    {
        ...
        if( sendDataLength > 0U )
        {
            ...
            sentLength = DbgConsole_SendData( &ch[ size - totalLength ],
                                              sendDataLength );
            if( sentLength > 0 )
            {
...
#else  /* DEBUG_CONSOLE_TRANSFER_NON_BLOCKING */
    status = SerialManager_WriteBlocking(
        ( ( serial_write_handle_t ) &s_debugConsoleState
              .serialWriteHandleBuffer[ 0 ] ),
        ch,
        size );
...
int DbgConsole_ReadLine( uint8_t * buf, size_t size )
{
    ...
    {
        /* recieve one char every time */
        if( ( status_t ) kStatus_Success !=
            DbgConsole_ReadOneCharacter( &buf[ i ] ) )
        {
                                      ...
                                      int len )
{
    ...
    for( i = 0; i < len; i++ )
    {
        if( ( ( uint32_t ) *indicator + 1UL ) >=
            ( uint32_t ) DEBUG_CONSOLE_PRINTF_MAX_LOG_LEN )
        {
            ( void ) DbgConsole_SendDataReliable( ( uint8_t * ) buf,
                                                  ( uint32_t ) ( *indicator ) );
            *indicator = 0;
        }
 ...
 * *******************************/
     ...
     defined( SDK_DEBUGCONSOLE_UART ) )
    #if( defined( SERIAL_USE_CONFIGURE_STRUCTURE ) && \
         ( SERIAL_USE_CONFIGURE_STRUCTURE > 0U ) )
        #include "board.h"
...
static const serial_port_uart_config_t uartConfig = {
    ...
    .enableRxRTS = 0U,
    .enableTxCTS = 0U,
            #if( defined( HAL_UART_ADAPTER_FIFO ) && \
                 ( HAL_UART_ADAPTER_FIFO > 0u ) )
    .txFifoWatermark = 0U,
    .rxFifoWatermark = 0U
            #endif
};
                          ...
                          uint32_t clkSrcFreq )
{
    ...
    {
        ( void ) memset( &s_debugConsoleState,
                         0,
                         sizeof( s_debugConsoleState ) );

    #if defined( DEBUG_CONSOLE_TRANSFER_NON_BLOCKING )
        s_debugConsoleState.writeRingBuffer
            .ringBufferSize = DEBUG_CONSOLE_TRANSMIT_BUFFER_LEN;
    #endif

        s_debugConsoleState
            .serialHandle = ( serial_handle_t ) &s_debugConsoleState
                                .serialHandleBuffer[ 0 ];
        status = SerialManager_Init( s_debugConsoleState.serialHandle,
                                     &serialConfig );
        ...
        DEBUG_CONSOLE_CREATE_MUTEX_SEMAPHORE(
            ...
            &s_debugConsoleReadSemaphoreStatic );
        #else
        ...
        DEBUG_CONSOLE_CREATE_BINARY_SEMAPHORE(
            s_debugConsoleReadWaitSemaphore,
            &s_debugConsoleReadWaitSemaphoreStatic );
        #else
        DEBUG_CONSOLE_CREATE_BINARY_SEMAPHORE(
            s_debugConsoleReadWaitSemaphore );
        #endif
...

        {
            ...
            ( void ) SerialManager_InstallTxCallback(
                ( ( serial_write_handle_t ) &s_debugConsoleState
                      .serialWriteHandleBuffer[ 0 ] ),
                DbgConsole_SerialManagerTxCallback,
                &s_debugConsoleState );
    #endif
        }
         ...
         ( DEBUG_CONSOLE_RX_ENABLE > 0U ) )
        {
            status = SerialManager_OpenReadHandle(
                s_debugConsoleState.serialHandle,
                ( ( serial_read_handle_t ) &s_debugConsoleState
                      .serialReadHandleBuffer[ 0 ] ) );
            assert( kStatus_SerialManager_Success == status );
            ...
            ( void ) SerialManager_InstallRxCallback(
                ( ( serial_read_handle_t ) &s_debugConsoleState
                      .serialReadHandleBuffer[ 0 ] ),
                DbgConsole_SerialManagerRxCallback,
                &s_debugConsoleState );
        #endif
        }
    #endif
...
status_t DbgConsole_EnterLowpower( void )
{
    ...
    {
        status = SerialManager_EnterLowpower(
            s_debugConsoleState.serialHandle );
    }
    return ( status_t ) status;
}
...
status_t DbgConsole_Deinit( void )
{
         ...
         ( DEBUG_CONSOLE_RX_ENABLE > 0U ) )
    {
        if( s_debugConsoleState.serialHandle != NULL )
        {
            ( void ) SerialManager_CloseReadHandle(
                ( ( serial_read_handle_t ) &s_debugConsoleState
                      .serialReadHandleBuffer[ 0 ] ) );
        }
    }
    ...
    if( NULL != s_debugConsoleState.serialHandle )
    {
        if( kStatus_SerialManager_Success ==
            SerialManager_Deinit( s_debugConsoleState.serialHandle ) )
        {
...
DEBUG_CONSOLE_FUNCTION_PREFIX status_t DbgConsole_Flush( void )
{
    ...
    while( s_debugConsoleState.writeRingBuffer.ringHead !=
           s_debugConsoleState.writeRingBuffer.ringTail )
    {
            #if( DEBUG_CONSOLE_SYNCHRONIZATION_MODE == \
                 DEBUG_CONSOLE_SYNCHRONIZATION_FREERTOS )
...
int DbgConsole_Printf( const char * fmt_s, ... )
{
    ...
    if( NULL != g_serialHandle )
    {
        ...
        logLength = StrFormatPrintf( fmt_s,
                                     ap,
                                     printBuf,
                                     DbgConsole_PrintCallback );
        /* print log */
        dbgResult = DbgConsole_SendDataReliable( ( uint8_t * ) printBuf,
                                                 ( size_t ) logLength );
...
int DbgConsole_Scanf( char * formatString, ... )
{
    ...
    /* scanf log */
    ( void ) DbgConsole_ReadLine( ( uint8_t * ) scanfBuf,
...
int DbgConsole_BlockingPrintf( const char * formatString, ... )
{
    ...
    va_start( ap, formatString );
    /* format print log first */
    logLength = StrFormatPrintf( formatString,
                                 ap,
                                 printBuf,
                                 DbgConsole_PrintCallback );

    #if defined( DEBUG_CONSOLE_TRANSFER_NON_BLOCKING )
    ( void ) SerialManager_CancelWriting(
        ( ( serial_write_handle_t ) &s_debugConsoleState
              .serialWriteHandleBuffer[ 0 ] ) );
    #endif
    /* print log */
    status = ( status_t ) SerialManager_WriteBlocking(
        ...
        ( uint8_t * ) printBuf,
        ( size_t ) logLength );
    dbgResult = ( ( ( status_t ) kStatus_Success == status ) ? ( int ) logLength
                                                             : -1 );
    va_end( ap );
...
status_t DbgConsole_TryGetchar( char * ch )
{
    ...
    if( kStatus_SerialManager_Success ==
        SerialManager_TryRead( ( ( serial_read_handle_t ) &s_debugConsoleState
                                     .serialReadHandleBuffer[ 0 ] ),
                               ( uint8_t * ) ch,
                               1,
                               &length ) )
 ...
 * *******************************/
/* These function __write and __read is used to support IAR toolchain to printf
 * and scanf*/
...
size_t __write( int handle, const unsigned char * buffer, size_t size )
{
    ...
    {
        /*
         * This means that we should flush internal buffers.  Since we don't we
         * just return. (Remember, "handle" == -1 means that all handles should
         * be flushed.)
         */
        ret = 0U;
    }
...
size_t __read( int handle, unsigned char * buffer, size_t size )
{
    ...
    /* This function only reads from "standard in", for all other file  handles
     * it returns failure. */
    if( 0 != handle )
    {
        actualSize = -1;
...
int __attribute__( ( weak ) ) __sys_write( int handle, char * buffer, int size )
{
    ...
    /* This function only writes to "standard out" and "standard err" for all
     * other file handles it returns failure. */
...
/* These function fputc and fgetc is used to support KEIL toolchain to printf
 * and scanf*/
...
struct __FILE
{
    int handle;
    /*
     * Whatever you require here. If the only file you are using is standard
     * output using printf() for debugging, no file handling is required.
     */
};
        #endif
...
/* These function __write and __read is used to support ARM_GCC, KDS, Atollic
 * toolchains to printf and scanf*/
...
int __attribute__( ( weak ) ) _write( int handle, char * buffer, int size )
{
    ...
    /* Send data. */
    ( void ) DbgConsole_SendDataReliable( ( uint8_t * ) buffer,
                                          ( size_t ) size );
