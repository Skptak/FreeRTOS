...
void SDIO_DataConfig( SDIO_DataInitTypeDef * SDIO_DataInitStruct )
{
    ...
    assert_param(
        IS_SDIO_BLOCK_SIZE( SDIO_DataInitStruct->SDIO_DataBlockSize ) );
    assert_param(
        IS_SDIO_TRANSFER_DIR( SDIO_DataInitStruct->SDIO_TransferDir ) );
    assert_param(
...
void SDIO_CEATAITCmd( FunctionalState NewState )
{
    ...
    *( __IO uint32_t * )
        CMD_NIEN_BB = ( uint32_t ) ( ( ~( ( uint32_t ) NewState ) ) &
                                     ( ( uint32_t ) 0x1 ) );
}
