...
#define CR1_CEN_BB \
    ( PERIPH_BB_BASE + ( CR1_OFFSET * 32 ) + ( CEN_BitNumber * 4 ) )

/* Alias word address of UDIS bit */
#define UDIS_BitNumber 0x01
#define CR1_UDIS_BB \
    ( PERIPH_BB_BASE + ( CR1_OFFSET * 32 ) + ( UDIS_BitNumber * 4 ) )

/* Alias word address of URS bit */
#define URS_BitNumber 0x02
#define CR1_URS_BB \
    ( PERIPH_BB_BASE + ( CR1_OFFSET * 32 ) + ( URS_BitNumber * 4 ) )

/* Alias word address of OPM bit */
#define OPM_BitNumber 0x03
#define CR1_OPM_BB \
    ( PERIPH_BB_BASE + ( CR1_OFFSET * 32 ) + ( OPM_BitNumber * 4 ) )

/* Alias word address of ARPE bit */
#define ARPE_BitNumber 0x07
#define CR1_ARPE_BB \
    ( PERIPH_BB_BASE + ( CR1_OFFSET * 32 ) + ( ARPE_BitNumber * 4 ) )
...
#define SMCR_MSM_BB \
    ( PERIPH_BB_BASE + ( SMCR_OFFSET * 32 ) + ( MSM_BitNumber * 4 ) )

/* Alias word address of ECE bit */
#define ECE_BitNumber 0x0E
#define SMCR_ECE_BB \
    ( PERIPH_BB_BASE + ( SMCR_OFFSET * 32 ) + ( ECE_BitNumber * 4 ) )
...
#define EGR_UG_BB \
    ( PERIPH_BB_BASE + ( EGR_OFFSET * 32 ) + ( UG_BitNumber * 4 ) )
...
#define CCER_CC1E_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC1E_BitNumber * 4 ) )

/* Alias word address of CC1P bit */
#define CC1P_BitNumber 0x01
#define CCER_CC1P_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC1P_BitNumber * 4 ) )

/* Alias word address of CC1NE bit */
#define CC1NE_BitNumber 0x02
#define CCER_CC1NE_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC1NE_BitNumber * 4 ) )

/* Alias word address of CC1NP bit */
#define CC1NP_BitNumber 0x03
#define CCER_CC1NP_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC1NP_BitNumber * 4 ) )

/* Alias word address of CC2E bit */
#define CC2E_BitNumber 0x04
#define CCER_CC2E_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC2E_BitNumber * 4 ) )

/* Alias word address of CC2P bit */
#define CC2P_BitNumber 0x05
#define CCER_CC2P_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC2P_BitNumber * 4 ) )

/* Alias word address of CC2NE bit */
#define CC2NE_BitNumber 0x06
#define CCER_CC2NE_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC2NE_BitNumber * 4 ) )

/* Alias word address of CC2NP bit */
#define CC2NP_BitNumber 0x07
#define CCER_CC2NP_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC2NP_BitNumber * 4 ) )

/* Alias word address of CC3E bit */
#define CC3E_BitNumber 0x08
#define CCER_CC3E_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC3E_BitNumber * 4 ) )

/* Alias word address of CC3P bit */
#define CC3P_BitNumber 0x09
#define CCER_CC3P_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC3P_BitNumber * 4 ) )

/* Alias word address of CC3NE bit */
#define CC3NE_BitNumber 0x0A
#define CCER_CC3NE_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC3NE_BitNumber * 4 ) )

/* Alias word address of CC3NP bit */
#define CC3NP_BitNumber 0x0B
#define CCER_CC3NP_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC3NP_BitNumber * 4 ) )

/* Alias word address of CC4E bit */
#define CC4E_BitNumber 0x0C
#define CCER_CC4E_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC4E_BitNumber * 4 ) )

/* Alias word address of CC4P bit */
#define CC4P_BitNumber 0x0D
#define CCER_CC4P_BB \
    ( PERIPH_BB_BASE + ( CCER_OFFSET * 32 ) + ( CC4P_BitNumber * 4 ) )
...
#define BDTR_MOE_BB \
    ( PERIPH_BB_BASE + ( BDTR_OFFSET * 32 ) + ( MOE_BitNumber * 4 ) )

/* --- TIM1 CCMR1 Register --- */
/* Alias word address of OC1FE bit */
#define CCMR1_OFFSET    ( TIM1_OFFSET + 0x18 )
#define OC1FE_BitNumber 0x02
#define CCMR1_OC1FE_BB \
    ( PERIPH_BB_BASE + ( CCMR1_OFFSET * 32 ) + ( OC1FE_BitNumber * 4 ) )

/* Alias word address of OC1PE bit */
#define OC1PE_BitNumber 0x03
#define CCMR1_OC1PE_BB \
    ( PERIPH_BB_BASE + ( CCMR1_OFFSET * 32 ) + ( OC1PE_BitNumber * 4 ) )

/* Alias word address of OC2FE bit */
#define OC2FE_BitNumber 0x0A
#define CCMR1_OC2FE_BB \
    ( PERIPH_BB_BASE + ( CCMR1_OFFSET * 32 ) + ( OC2FE_BitNumber * 4 ) )

/* Alias word address of OC2PE bit */
#define OC2PE_BitNumber 0x0B
#define CCMR1_OC2PE_BB \
    ( PERIPH_BB_BASE + ( CCMR1_OFFSET * 32 ) + ( OC2PE_BitNumber * 4 ) )

/* --- TIM1 CCMR2 Register ---- */
/* Alias word address of OC3FE bit */
#define CCMR2_OFFSET    ( TIM1_OFFSET + 0x1C )
#define OC3FE_BitNumber 0x02
#define CCMR2_OC3FE_BB \
    ( PERIPH_BB_BASE + ( CCMR2_OFFSET * 32 ) + ( OC3FE_BitNumber * 4 ) )

/* Alias word address of OC3PE bit */
#define OC3PE_BitNumber 0x03
#define CCMR2_OC3PE_BB \
    ( PERIPH_BB_BASE + ( CCMR2_OFFSET * 32 ) + ( OC3PE_BitNumber * 4 ) )

/* Alias word address of OC4FE bit */
#define OC4FE_BitNumber 0x0A
#define CCMR2_OC4FE_BB \
    ( PERIPH_BB_BASE + ( CCMR2_OFFSET * 32 ) + ( OC4FE_BitNumber * 4 ) )

/* Alias word address of OC4PE bit */
#define OC4PE_BitNumber 0x0B
#define CCMR2_OC4PE_BB \
    ( PERIPH_BB_BASE + ( CCMR2_OFFSET * 32 ) + ( OC4PE_BitNumber * 4 ) )
 ...
 * Description    : Initializes the TIM1 Time Base Unit according to the
 *specified parameters in the TIM1_TimeBaseInitStruct. Input          : -
 *TIM1_TimeBaseInitStruct: pointer to a TIM1_TimeBaseInitTypeDef structure that
 *contains the configuration information for the specified TIM1 peripheral.
 * Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Input          : - TIM1_OCInitStruct: pointer to a TIM1_OCInitTypeDef
 *structure that contains the configuration information for the TIM1 peripheral.
 * Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Description    : Configures the: Break feature, dead time, Lock level, the
 *OSSI, the OSSR State and the AOE(automatic output enable). Input          : -
 *TIM1_BDTRInitStruct: pointer to a TIM1_BDTRInitTypeDef structure that contains
 *the BDTR Register configuration information for the TIM1 peripheral. Output :
 *None Return         : None
 *******************************************************************************/
void TIM1_BDTRConfig( TIM1_BDTRInitTypeDef * TIM1_BDTRInitStruct )
{
    ...
    assert( IS_TIM1_AUTOMATIC_OUTPUT_STATE(
    ...
    tmpbdtr = ( u32 ) TIM1_BDTRInitStruct->TIM1_OSSRState |
              TIM1_BDTRInitStruct->TIM1_OSSIState |
              TIM1_BDTRInitStruct->TIM1_LOCKLevel |
              TIM1_BDTRInitStruct->TIM1_DeadTime |
 ...
 * Input          : - TIM1_ICInitStruct: pointer to a TIM1_ICInitTypeDef
 *structure that contains the configuration information for the specified TIM1
 *peripheral. Output         : None Return         : None
 *******************************************************************************/
 ...
 * Input          : - TIM1_OCInitStruct : pointer to a TIM1_OCInitTypeDef
 *structure which will be initialized. Output         : None Return         :
 *None
 *******************************************************************************/
 ...
 * Description    : Fills each TIM1_TimeBaseInitStruct member with its default
 *value. Input          : - TIM1_TimeBaseInitStruct : pointer to a
 *TIM1_TimeBaseInitTypeDef structure which will be initialized. Output         :
 *None Return         : None
 *******************************************************************************/
 ...
 * Description    : Fills each TIM1_BDTRInitStruct member with its default
 *value. Input          : - TIM1_BDTRInitStruct : pointer to a
 *TIM1_BDTRInitTypeDef structure which will be initialized. Output         :
 *None Return         : None
 *******************************************************************************/
 ...
 * Input          : - TIM1_IT: specifies the TIM1 interrupts sources to be
 ...
 *                       - TIM1_IT_CCUpdate: TIM1 Capture Compare Update
 *Interrupt source
 ...
 *                       - TIM1_DMABase_CR1, TIM1_DMABase_CR2,
 *TIM1_DMABase_SMCR, TIM1_DMABase_DIER, TIM1_DMABase_SR, TIM1_DMABase_EGR,
 *                         TIM1_DMABase_CCMR1, TIM1_DMABase_CCMR2,
 *TIM1_DMABase_CCER, TIM1_DMABase_CNT, TIM1_DMABase_PSC, TIM1_DMABase_ARR,
 *                         TIM1_DMABase_RCR, TIM1_DMABase_CCR1,
 *TIM1_DMABase_CCR2, TIM1_DMABase_CCR3, TIM1_DMABase_CCR4, TIM1_DMABase_BDTR,
 *                         TIM1_DMABase_DCR.
 *                   - TIM1_DMABurstLength: DMA Burst length.
 *                     This parameter can be one value between:
 *                     TIM1_DMABurstLength_1Byte and
 *TIM1_DMABurstLength_18Bytes. Output         : None Return         : None
 *******************************************************************************/
 ...
 * Input          : - Newstate: new state of the TIM1 peripheral Preload
 *register This parameter can be: ENABLE or DISABLE. Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Input          : - TIM1_MasterSlaveMode: specifies the TIM1 Master Slave
 *Mode. This paramter can be one of the following values:
 ...
 *                       - TIM1_EncoderMode_TI12: Counter counts on both TI1FP1
 ...
 *                  - TIM1_PSCReloadMode: specifies the TIM1 Prescaler Reload
 *mode. This parmeter can be one of the following values:
 ...
 *                       - TIM1_CounterMode_CenterAligned1: TIM1 Center Aligned
 *Mode1
 *                       - TIM1_CounterMode_CenterAligned2: TIM1 Center Aligned
 *Mode2
 *                       - TIM1_CounterMode_CenterAligned3: TIM1 Center Aligned
 ...
 * Description    : Forces the TIM1 Channel1 output waveform to active or
 *inactive level. Input          : - TIM1_ForcedAction: specifies the forced
 *Action to be set to the output waveform. This parameter can be one of the
 *following values:
 ...
 * Description    : Forces the TIM1 Channel2 output waveform to active or
 *inactive level. Input          : - TIM1_ForcedAction: specifies the forced
 *Action to be set to the output waveform. This parameter can be one of the
 *following values:
 *                       - TIM1_ForcedAction_Active: Force active level on
 ...
 * Description    : Forces the TIM1 Channel3 output waveform to active or
 *inactive level. Input          : - TIM1_ForcedAction: specifies the forced
 *Action to be set to the output waveform. This parameter can be one of the
 *following values:
 *                       - TIM1_ForcedAction_Active: Force active level on
 ...
 * Description    : Forces the TIM1 Channel4 output waveform to active or
 *inactive level. Input          : - TIM1_ForcedAction: specifies the forced
 *Action to be set to the output waveform. This parameter can be one of the
 *following values:
 *                       - TIM1_ForcedAction_Active: Force active level on
 ...
 *                  Control bit.
 * Input          : - Newstate: new state of the Capture Compare Preload Control
 *bit This parameter can be: ENABLE or DISABLE. Output         : None Return :
 *None
 *******************************************************************************/
 ...
 * Description    : Enables or disables the TIM1 peripheral Preload Register on
 *CCR1. Input          : - TIM1_OCPreload: new state of the Capture Compare
 *Preload register. This parameter can be one of the following values:
 ...
 * Description    : Enables or disables the TIM1 peripheral Preload Register on
 *CCR2. Input          : - TIM1_OCPreload: new state of the Capture Compare
 *Preload register. This parameter can be one of the following values:
 *                       - TIM1_OCPreload_Enable
 *                       - TIM1_OCPreload_Disable
 * Output         : None
 ...
 * Description    : Enables or disables the TIM1 peripheral Preload Register on
 *CCR3. Input          : - TIM1_OCPreload: new state of the Capture Compare
 *Preload register. This parameter can be one of the following values:
 *                       - TIM1_OCPreload_Enable
 *                       - TIM1_OCPreload_Disable
 * Output         : None
 ...
 * Description    : Enables or disables the TIM1 peripheral Preload Register on
 *CCR4. Input          : - TIM1_OCPreload: new state of the Capture Compare
 *Preload register. This parameter can be one of the following values:
 *                       - TIM1_OCPreload_Enable
 *                       - TIM1_OCPreload_Disable
 * Output         : None
 ...
 * Input          : - TIM1_OCFast: new state of the Output Compare Fast Enable
 ...
 *                  the Ouput Compare Mode. User has to enable this channel
 *using TIM1_CCxCmd and TIM1_CCxNCmd functions. Input          : - TIM1_Channel:
 *specifies the TIM1 Channel This parmeter can be one of the following values:
 *                       - TIM1_Channel1: TIM1 Channel1
 *                       - TIM1_Channel2: TIM1 Channel2
 *                       - TIM1_Channel3: TIM1 Channel3
 *                       - TIM1_Channel4: TIM1 Channel4
 ...
 * Input          : - Compare1: specifies the Capture Compare1 register new
 ...
 * Input          : - Compare2: specifies the Capture Compare2 register new
 *value. Output         : None Return         : None
 *******************************************************************************/
 ...
 * Input          : - Compare3: specifies the Capture Compare3 register new
 *value. Output         : None Return         : None
 *******************************************************************************/
 ...
 * Input          : - Compare4: specifies the Capture Compare4 register new
 *value. Output         : None Return         : None
 *******************************************************************************/
 ...
 *Flag Output         : None Return         : The new state of TIM1_FLAG (SET or
 *RESET).
 *******************************************************************************/
...
ITStatus TIM1_GetITStatus( u16 TIM1_IT )
{
    ...
    if( ( TIM1_itStatus != ( u16 ) RESET ) &&
        ( TIM1_itEnable != ( u16 ) RESET ) )
    {
        bitstatus = SET;
    }
    else
                        ...
                        u8 TIM1_ICFilter )
{
    ...
    *( vu32 * ) CCER_CC1E_BB = CCER_CCE_Reset;
    ...
    tmpccmr1 |= ( u16 ) TIM1_ICSelection |
                ( u16 ) ( ( u16 ) TIM1_ICFilter << 4 );
                        ...
                        u8 TIM1_ICFilter )
{
    ...
    *( vu32 * ) CCER_CC2E_BB = CCER_CCE_Reset;
    ...
    tmpccmr1 |= ( u16 ) ( ( u16 ) TIM1_ICSelection << 8 ) |
                ( u16 ) ( ( u16 ) TIM1_ICFilter << 12 );
                        ...
                        u8 TIM1_ICFilter )
{
    ...
    *( vu32 * ) CCER_CC4E_BB = CCER_CCE_Reset;
    ...
    tmpccmr2 |= ( u16 ) ( ( u16 ) TIM1_ICSelection << 8 ) |
                ( u16 ) ( ( u16 ) TIM1_ICFilter << 12 );
                        ...
                        u16 ExtTRGFilter )
{
    ...
    tmpsmcr |= TIM1_ExtTRGPrescaler | TIM1_ExtTRGPolarity |
               ( u16 ) ( ( u16 ) ExtTRGFilter << 8 );
