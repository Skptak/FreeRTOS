...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Global pointers on Tx and Rx descriptor used to track transmit and receive
 * descriptors */
...
#define ETH_MAC_AddrHighBase \
    ( ETH_MAC_BASE + 0x40 ) /* ETHERNET MAC address high offset */
#define ETH_MAC_AddrLowBase \
 ...
 * Desciption     : Initializes the ETHERNET peripheral according to the
 *specified parameters in the ETH_InitStruct . Input          : -
 *ETH_InitStruct: pointer to a ETH_InitTypeDef structure that contains the
 *configuration information for the specified ETHERNET peripheral. Output : None
 * Return         : None
 *******************************************************************************/
u32 ETH_Init( ETH_InitTypeDef * ETH_InitStruct, u16 PHYAddress )
{
    ...
    /* MAC --------------------------*/
    eth_assert_param(
        IS_ETH_AUTONEGOTIATION( ETH_InitStruct->ETH_AutoNegotiation ) );
    ...
    eth_assert_param(
        IS_ETH_INTER_FRAME_GAP( ETH_InitStruct->ETH_InterFrameGap ) );
    eth_assert_param(
        IS_ETH_CARRIER_SENSE( ETH_InitStruct->ETH_CarrierSense ) );
    eth_assert_param( IS_ETH_SPEED( ETH_InitStruct->ETH_Speed ) );
    eth_assert_param( IS_ETH_RECEIVE_OWN( ETH_InitStruct->ETH_ReceiveOwn ) );
    eth_assert_param(
        IS_ETH_LOOPBACK_MODE( ETH_InitStruct->ETH_LoopbackMode ) );
    eth_assert_param( IS_ETH_DUPLEX_MODE( ETH_InitStruct->ETH_Mode ) );
    eth_assert_param(
        IS_ETH_CHECKSUM_OFFLOAD( ETH_InitStruct->ETH_ChecksumOffload ) );
    eth_assert_param(
        IS_ETH_RETRY_TRANSMISSION( ETH_InitStruct->ETH_RetryTransmission ) );
    eth_assert_param( IS_ETH_AUTOMATIC_PADCRC_STRIP(
        ETH_InitStruct->ETH_AutomaticPadCRCStrip ) );
    eth_assert_param(
        IS_ETH_BACKOFF_LIMIT( ETH_InitStruct->ETH_BackOffLimit ) );
    eth_assert_param(
        IS_ETH_DEFERRAL_CHECK( ETH_InitStruct->ETH_DeferralCheck ) );
    ...
    eth_assert_param(
        IS_ETH_CONTROL_FRAMES( ETH_InitStruct->ETH_PassControlFrames ) );
    eth_assert_param( IS_ETH_BROADCAST_FRAMES_RECEPTION(
        ETH_InitStruct->ETH_BroadcastFramesReception ) );
    eth_assert_param( IS_ETH_DESTINATION_ADDR_FILTER(
        ETH_InitStruct->ETH_DestinationAddrFilter ) );
    eth_assert_param(
        IS_ETH_PROMISCUOUS_MODE( ETH_InitStruct->ETH_PromiscuousMode ) );
    eth_assert_param( IS_ETH_MULTICAST_FRAMES_FILTER(
        ETH_InitStruct->ETH_MulticastFramesFilter ) );
    eth_assert_param( IS_ETH_UNICAST_FRAMES_FILTER(
        ETH_InitStruct->ETH_UnicastFramesFilter ) );
    eth_assert_param( IS_ETH_PAUSE_TIME( ETH_InitStruct->ETH_PauseTime ) );
    eth_assert_param(
        IS_ETH_ZEROQUANTA_PAUSE( ETH_InitStruct->ETH_ZeroQuantaPause ) );
    eth_assert_param(
        IS_ETH_PAUSE_LOW_THRESHOLD( ETH_InitStruct->ETH_PauseLowThreshold ) );
    eth_assert_param( IS_ETH_UNICAST_PAUSE_FRAME_DETECT(
    ...
    /* DMA --------------------------*/
    eth_assert_param( IS_ETH_DROP_TCPIP_CHECKSUM_FRAME(
        ETH_InitStruct->ETH_DropTCPIPChecksumErrorFrame ) );
    eth_assert_param( IS_ETH_RECEIVE_STORE_FORWARD(
        ETH_InitStruct->ETH_ReceiveStoreForward ) );
    eth_assert_param(
        IS_ETH_FLUSH_RECEIVE_FRAME( ETH_InitStruct->ETH_FlushReceivedFrame ) );
    eth_assert_param( IS_ETH_TRANSMIT_STORE_FORWARD(
        ETH_InitStruct->ETH_TransmitStoreForward ) );
    eth_assert_param( IS_ETH_TRANSMIT_THRESHOLD_CONTROL(
    ...
    eth_assert_param( IS_ETH_RECEIVE_THRESHOLD_CONTROL(
        ETH_InitStruct->ETH_ReceiveThresholdControl ) );
    eth_assert_param(
        IS_ETH_SECOND_FRAME_OPERATE( ETH_InitStruct->ETH_SecondFrameOperate ) );
    eth_assert_param( IS_ETH_ADDRESS_ALIGNED_BEATS(
    ...
    eth_assert_param( IS_ETH_DMA_DESC_SKIP_LENGTH(
        ETH_InitStruct->ETH_DescriptorSkipLength ) );
    eth_assert_param( IS_ETH_DMA_ARBITRATION_ROUNDROBIN_RXTX(
    ...
    if( ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable )
    {
        ...
        } while( !( ETH_ReadPHYRegister( PHYAddress, PHY_BSR ) &
                    PHY_Linked_Status ) &&
                 ( timeout < ( ethFIVE_SECONDS / ethHUNDRED_MS ) ) );
        ...
        if( !( ETH_WritePHYRegister( PHYAddress,
                                     PHY_BCR,
                                     PHY_AutoNegotiation ) ) )
        {
            /* Return ERROR in case of write timeout */
        ...
        /* Configure the MAC with the Duplex Mode fixed by the autonegotiation
         * process */
        if( ( RegValue & PHY_Duplex_Status ) != ( u32 ) RESET )
        {
            /* Set Ethernet duplex mode to FullDuplex following the
             * autonegotiation */
            ETH_InitStruct->ETH_Mode = ETH_Mode_FullDuplex;
        }
        else
        {
            /* Set Ethernet duplex mode to HalfDuplex following the
             * autonegotiation */
    ...
    //  else
    {
        if( !ETH_WritePHYRegister( PHYAddress,
                                   PHY_BCR,
                                   ( ( u16 ) ( ETH_InitStruct->ETH_Mode >> 3 ) |
                                     ( u16 ) ( ETH_InitStruct->ETH_Speed >>
                                               1 ) ) ) )
        {
            /* Return ERROR in case of write timeout */
            return ETH_ERROR;
    ...
    ETH_DMA->DMABMR = ( u32 ) ( ETH_InitStruct->ETH_AddressAlignedBeats |
                                ...
                                ETH_InitStruct
                                    ->ETH_RxDMABurstLength | /* !! if 4xPBL is
                                                                selected for Tx
                                                                or Rx it is
                                                                applied for the
                                                                other */
                                ...
                                ( ETH_InitStruct->ETH_DescriptorSkipLength
                                  << 2 ) |
                                ETH_InitStruct->ETH_DMAArbitration |
                                ETH_DMABMR_USP ); /* Enable use of separate PBL
                                                     for Rx and Tx */
#endif                                            /* _ETH_DMA */
...
void ETH_StructInit( ETH_InitTypeDef * ETH_InitStruct )
{
    ...
    ETH_InitStruct
        ->ETH_BroadcastFramesReception = ETH_BroadcastFramesReception_Disable;
    ...
    /*------------------------   DMA   -----------------------------------*/
    ETH_InitStruct
        ->ETH_DropTCPIPChecksumErrorFrame = ETH_DropTCPIPChecksumErrorFrame_Disable;
    ETH_InitStruct->ETH_ReceiveStoreForward = ETH_ReceiveStoreForward_Enable;
    ETH_InitStruct->ETH_FlushReceivedFrame = ETH_FlushReceivedFrame_Disable;
    ETH_InitStruct->ETH_TransmitStoreForward = ETH_TransmitStoreForward_Enable;
    ETH_InitStruct
        ->ETH_TransmitThresholdControl = ETH_TransmitThresholdControl_64Bytes;
 ...
 * Desciption     : Transmits a packet, from application buffer, pointed by
 *ppkt. Input          : - ppkt: pointer to application packet Buffer.
...
u32 ETH_HandleTxPkt( u32 addr, u16 FrameLength )
{
    ...
    else // Ring Mode
    {
        ...
        {
            // Selects the first DMA Tx descriptor for next buffer to send: last
            // Tx descriptor was used
            DMATxDescToSet = ( ETH_DMADESCTypeDef * ) ( ETH_DMA->DMATDLAR );
        }
        else
        {
            // Selects the next DMA Tx descriptor list for next buffer to send
            DMATxDescToSet = ( ETH_DMADESCTypeDef * ) ( ( u32 ) DMATxDescToSet +
                                                        0x10 +
                                                        ( ( ETH_DMA->DMABMR &
                                                            ETH_DMABMR_DSL ) >>
                                                          2 ) );
...
u32 ETH_HandleRxPkt( u32 addr )
{
    ...
    // Set the buffer address to rcv frame for the same descriptor (reserved
    // packet)
    DMARxDescToGet->Buffer1Addr = addr;

    if( addr )
    {
        // Set Own bit of the Rx descriptor Status: gives the buffer back to
        // ETHERNET DMA
    ...
    else // Ring Mode
    {
        if( ( DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RER ) !=
            ( u32 ) RESET )
        {
            // Selects the first DMA Rx descriptor for next buffer to read: last
            // Rx descriptor was used
            DMARxDescToGet = ( ETH_DMADESCTypeDef * ) ( ETH_DMA->DMARDLAR );
        }
        else
        {
            // Selects the next DMA Rx descriptor list for next buffer to read
            DMARxDescToGet = ( ETH_DMADESCTypeDef * ) ( ( u32 ) DMARxDescToGet +
                                                        0x10 +
                                                        ( ( ETH_DMA->DMABMR &
                                                            ETH_DMABMR_DSL ) >>
                                                          2 ) );
 ...
 * Input          : - PHYAddress: PHY device address, is the index of one of
 *supported 32 PHY devices. This parameter can be one of the following values:
 *0,..,31
 *                  - PHYReg: PHY register address, is the index of one of the
 *32 PHY register. This parameter can be one of the following values:
 *                       - PHY_BCR    : Tranceiver Basic Control Register
 *                       - PHY_BSR    : Tranceiver Basic Status Register
 *                       - PHY_SR     : Tranceiver Status Register
 *                       - More PHY register could be read depending on the used
 *PHY Output         : None Return         : ETH_ERROR: in case of timeout Data
 *read from the selected PHY register: for correct read
 *******************************************************************************/
 ...
 *                       - More PHY register could be written depending on the
 *used PHY
 ...
 * Desciption     : Checks whether the ETHERNET flow control busy bit is set or
 *not. Input          : None Output         : None Return         : The new
 *state of flow control busy status bit (SET or RESET).
 *******************************************************************************/
FlagStatus ETH_GetFlowControlBusyStatus( void )
{
    FlagStatus bitstatus = RESET;

    /* The Flow Control register should not be written to until this bit is
     * cleared */
 ...
 * Desciption     : Enables or disables the MAC BackPressure operation
 *activation (Half-duplex only). Input          : - NewState: new state of the
 *MAC BackPressure operation activation. This parameter can be: ENABLE or
 *DISABLE. Output         : None Return         : None
 *******************************************************************************/
void ETH_BackPressureActivationCmd( FunctionalState NewState )
{
    ...
    if( NewState != DISABLE )
    {
        ...
        /* In Half duplex: during backpressure, when the MAC receives a new
        frame, the transmitter starts sending a JAM pattern resulting in a
        collision */
        ETH_MAC->MACFCR |= ETH_MACFCR_FCBBPA;
    }
    else
 ...
 * Desciption     : Checks whether the specified ETHERNET MAC flag is set or
 *not. Input          : - ETH_MAC_FLAG: specifies the flag to check. This
 ...
 * Desciption     : Checks whether the specified ETHERNET MAC interrupt has
 *occurred or not. Input          : - ETH_MAC_IT: specifies the interrupt source
 ...
 * Input          : - ETH_MAC_IT: specifies the ETHERNET MAC interrupt sources
 *to be enabled or disabled. This parameter can be any combination of the
 *following values:
 *                       - ETH_MAC_IT_TST : Time stamp trigger interrupt
 *                       - ETH_MAC_IT_PMT : PMT interrupt
 *                  - NewState: new state of the specified ETHERNET MAC
 *interrupts. This parameter can be: ENABLE or DISABLE. Output         : None
 * Return         : None
 *******************************************************************************/
...
void ETH_MACAddressConfig( u32 MacAddr, u8 * Addr )
{
    ...
    tmpreg = ( ( u32 ) Addr[ 3 ] << 24 ) | ( ( u32 ) Addr[ 2 ] << 16 ) |
             ( ( u32 ) Addr[ 1 ] << 8 ) | Addr[ 0 ];
 ...
 * Desciption     : Enables or disables the Address filter module uses the
 *specified ETHERNET MAC address for perfect filtering Input          : -
 *MacAddr: specifies the ETHERNET MAC address to be used for prfect filtering.
 *                    This parameter can be one of the following values:
 *                     - ETH_MAC_Address1 : MAC Address1
 *                     - ETH_MAC_Address2 : MAC Address2
 *                     - ETH_MAC_Address3 : MAC Address3
 *                  - NewState: new state of the specified ETHERNET MAC address
 *use. This parameter can be: ENABLE or DISABLE. Output         : None Return :
 *None
 *******************************************************************************/
void ETH_MACAddressPerfectFilterCmd( u32 MacAddr, FunctionalState NewState )
{
    ...
    else
    {
        ...
        ( *( vu32 * ) ( ETH_MAC_AddrHighBase +
                        MacAddr ) ) &= ( ~( u32 ) ETH_MACA1HR_AE );
    }
}
 ...
 *                     - ETH_MAC_Address2 : MAC Address2
 *                     - ETH_MAC_Address3 : MAC Address3
 *                  - Filter: specifies the used frame received field for
 *comparaison This parameter can be one of the following values:
 *                     - ETH_MAC_AddressFilter_SA : MAC Address is used to
 *compare with the SA fields of the received frame.
 *                     - ETH_MAC_AddressFilter_DA : MAC Address is used to
 *compare with the DA fields of the received frame. Output         : None Return
 *: None
 *******************************************************************************/
void ETH_MACAddressFilterConfig( u32 MacAddr, u32 Filter )
{
    ...
    {
        /* The selected ETHERNET MAC address is used to compare with the SA
           fields of the received frame. */
        ( *( vu32 * ) ( ETH_MAC_AddrHighBase + MacAddr ) ) |= ETH_MACA1HR_SA;
    }
    else
    {
        /* The selected ETHERNET MAC address is used to compare with the DA
           fields of the received frame. */
        ( *( vu32 * ) ( ETH_MAC_AddrHighBase +
                        MacAddr ) ) &= ( ~( u32 ) ETH_MACA1HR_SA );
    }
}
 ...
 *values:
 *                     - ETH_MAC_AddressMask_Byte6 : Mask MAC Address high reg
 *bits [15:8].
 *                     - ETH_MAC_AddressMask_Byte5 : Mask MAC Address high reg
 *bits [7:0].
 *                     - ETH_MAC_AddressMask_Byte4 : Mask MAC Address low reg
 *bits [31:24].
 *                     - ETH_MAC_AddressMask_Byte3 : Mask MAC Address low reg
 *bits [23:16].
 *                     - ETH_MAC_AddressMask_Byte2 : Mask MAC Address low reg
 *bits [15:8].
 *                     - ETH_MAC_AddressMask_Byte1 : Mask MAC Address low reg
 *bits [7:0]. Output         : None Return         : None
 *******************************************************************************/
                             ...
                             u32 TxBuffCount )
{
    ...
    /* Set the DMATxDescToSet pointer with the first one of the DMATxDescTab
     * list */
...
*                Note: see decriptor skip length defined in ETH_DMA_InitStruct
                       for the number of Words to skip between two unchained
descriptors.
* Output         : None
* Return         : None
*******************************************************************************/
                            ...
                            u32 TxBuffCount )
{
    ...
    for( i = 0; i < TxBuffCount; i++ )
    {
        ...
        DMATxDesc
            ->Buffer2NextDescAddr = ( u32 ) ( &TxBuff2[ i *
                                                        ETH_MAX_PACKET_SIZE ] );

        /* Set Transmit End of Ring bit for last descriptor: The DMA returns to
           the base address of the list, creating a Desciptor Ring */
        if( i == ( TxBuffCount - 1 ) )
        {
 ...
 * Desciption     : Checks whether the specified ETHERNET DMA Tx Desc flag is
 *set or not. Input          : - DMATxDesc: pointer on a DMA Tx descriptor
 *                  - ETH_DMATxDescFlag: specifies the flag to check.
 *                    This parameter can be one of the following values:
 *                       - ETH_DMATxDesc_OWN : OWN bit: descriptor is owned by
 *DMA engine
 ...
 *                       - ETH_DMATxDesc_FF  : Frame Flushed: DMA/MTL flushed
 *the frame due to SW flush
 *                       - ETH_DMATxDesc_PCE : Payload Checksum Error
 *                       - ETH_DMATxDesc_LCA : Loss of Carrier: carrier lost
 *during tramsmission
 *                       - ETH_DMATxDesc_NC  : No Carrier: no carrier signal
 *from the tranceiver
 *                       - ETH_DMATxDesc_LCO : Late Collision: transmission
 *aborted due to collision
 *                       - ETH_DMATxDesc_EC  : Excessive Collision: transmission
 ...
 *                       - ETH_DMATxDesc_UF  : Underflow Error: late data
...
u32 ETH_GetDMATxDescCollisionCount( ETH_DMADESCTypeDef * DMATxDesc )
{
    ...
    return ( ( DMATxDesc->Status & ETH_DMATxDesc_CC ) >>
             ETH_DMATxDesc_CollisionCountShift );
}
 ...
 * Desciption     : Enables or disables the specified DMA Tx Desc Transmit
 *interrupt. Input          : - DMATxDesc: Pointer on a Tx desc
 ...
 * Desciption     : Enables or disables the specified DMA Tx Desc Transmit
 *interrupt. Input          : - DMATxDesc: Pointer on a Tx desc
 *                  - FrameSegment: specifies is the actual Tx desc contain last
 *or first segment. This parameter can be one of the following values:
 *                       - ETH_DMATxDesc_LastSegment  : actual Tx desc contain
 *last segment
 *                       - ETH_DMATxDesc_FirstSegment : actual Tx desc contain
 *first segment Output         : None Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Selects the specified ETHERNET DMA Tx Desc Checksum
 *Insertion. Input          : - DMATxDesc: pointer on a DMA Tx descriptor
 *                  - Checksum: specifies is the DMA Tx desc checksum insertion.
 *                    This parameter can be one of the following values:
 *                       - ETH_DMATxDesc_ChecksumByPass : Checksum bypass
 *                       - ETH_DMATxDesc_ChecksumIPV4Header : IPv4 header
 *checksum
 *                       - ETH_DMATxDesc_ChecksumTCPUDPICMPSegment :
 *TCP/UDP/ICMP checksum. Pseudo header checksum is assumed to be present
 *                       - ETH_DMATxDesc_ChecksumTCPUDPICMPFull : TCP/UDP/ICMP
 *checksum fully in hardware including pseudo header Output         : None
 ...
 * Input          : - DMATxDesc: pointer on a DMA Tx descriptor
 *                  - NewState: new state of the specified DMA Tx Desc end of
 *ring. This parameter can be: ENABLE or DISABLE. Output         : NoneH Return
 *: None
 *******************************************************************************/
 ...
 * Input          : - DMATxDesc: pointer on a DMA Tx descriptor
 *                  - NewState: new state of the specified DMA Tx Desc second
 *address chained. This parameter can be: ENABLE or DISABLE. Output         :
 *None Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Enables or disables the DMA Tx Desc padding for frame
 *shorter than 64 bytes. Input          : - DMATxDesc: pointer on a DMA Tx
 *descriptor
 *                  - NewState: new state of the specified DMA Tx Desc padding
 *for frame shorter than 64 bytes. This parameter can be: ENABLE or DISABLE.
 * Output         : None
 * Return         : None
 *******************************************************************************/
                                        ...
                                        FunctionalState NewState )
{
    ...
    if( NewState != DISABLE )
    {
        /* Enable the selected DMA Tx Desc padding for frame shorter than 64
         * bytes */
        DMATxDesc->Status &= ( ~( u32 ) ETH_DMATxDesc_DP );
    }
    else
    {
        /* Disable the selected DMA Tx Desc padding for frame shorter than 64
         * bytes*/
 ...
 * Input          : - DMATxDesc: pointer on a DMA Tx descriptor
 *                  - NewState: new state of the specified DMA Tx Desc time
 *stamp. This parameter can be: ENABLE or DISABLE. Output         : None Return
 *: None
 *******************************************************************************/
 ...
 * Desciption     : Configures the specified DMA Tx Desc buffer1 and buffer2
 *sizes. Input          : - DMATxDesc: Pointer on a Tx desc
 *                  - BufferSize1: specifies the Tx desc buffer1 size.
 *                  - BufferSize2: specifies the Tx desc buffer2 size (put "0"
 *if not used). Output         : None Return         : None
 *******************************************************************************/
                                    ...
                                    u32 BufferSize2 )
{
    ...
    DMATxDesc->ControlBufferSize |= ( BufferSize1 |
                                      ( BufferSize2
                                        << ETH_DMATxDesc_BufferSize2Shift ) );
}
                             ...
                             u32 RxBuffCount )
{
    ...
    /* Set the DMARxDescToGet pointer with the first one of the DMARxDescTab
     * list */
                            ...
                            u32 RxBuffCount )
{
    ...
    for( i = 0; i < RxBuffCount; i++ )
    {
        ...
        /* Set Buffer2 address pointer */
        DMARxDesc
            ->Buffer2NextDescAddr = ( u32 ) ( &RxBuff2[ i *
                                                        ETH_MAX_PACKET_SIZE ] );

        /* Set Receive End of Ring bit for last descriptor: The DMA returns to
           the base address of the list, creating a Desciptor Ring */
        if( i == ( RxBuffCount - 1 ) )
        {
 ...
 * Desciption     : Checks whether the specified ETHERNET Rx Desc flag is set or
 *not. Input          : - DMARxDesc: pointer on a DMA Rx descriptor
 *                  - ETH_DMARxDescFlag: specifies the flag to check.
 *                    This parameter can be one of the following values:
 *                       - ETH_DMARxDesc_OWN:         OWN bit: descriptor is
 *owned by DMA engine
 *                       - ETH_DMARxDesc_AFM:         DA Filter Fail for the rx
 *frame
 *                       - ETH_DMARxDesc_ES:          Error summary
 *                       - ETH_DMARxDesc_DE:          Desciptor error: no more
 *descriptors for receive frame
 *                       - ETH_DMARxDesc_SAF:         SA Filter Fail for the
 *received frame
 *                       - ETH_DMARxDesc_LE:          Frame size not matching
 *with length field
 *                       - ETH_DMARxDesc_OE:          Overflow Error: Frame was
 *damaged due to buffer overflow
 *                       - ETH_DMARxDesc_VLAN:        VLAN Tag: received frame
 *is a VLAN frame
 *                       - ETH_DMARxDesc_FS:          First descriptor of the
 *frame
 *                       - ETH_DMARxDesc_LS:          Last descriptor of the
 *frame
 *                       - ETH_DMARxDesc_IPV4HCE:     IPC Checksum Error/Giant
 *Frame: Rx Ipv4 header checksum error
 *                       - ETH_DMARxDesc_RxLongFrame: (Giant Frame)Rx - frame is
 *longer than 1518/1522
 *                       - ETH_DMARxDesc_LC:          Late collision occurred
 *during reception
 *                       - ETH_DMARxDesc_FT:          Frame type - Ethernet,
 *otherwise 802.3
 *                       - ETH_DMARxDesc_RWT:         Receive Watchdog Timeout:
 *watchdog timer expired during reception
 *                       - ETH_DMARxDesc_RE:          Receive error: error
 *reported by MII interface
 *                       - ETH_DMARxDesc_DE:          Dribble bit error: frame
 *contains non int multiple of 8 bits
 *                       - ETH_DMARxDesc_CE:          CRC error
 *                       - ETH_DMARxDesc_MAMPCE:      Rx MAC Address/Payload
 *Checksum Error: Rx MAC address matched/ Rx Payload Checksum Error Output :
 *None Return         : The new state of ETH_DMARxDescFlag (SET or RESET).
 *******************************************************************************/
 ...
 * Desciption     : Enables or disables the specified DMA Rx Desc receive
 *interrupt. Input          : - DMARxDesc: Pointer on a Rx desc
 *                  - NewState: new state of the specified DMA Rx Desc
 *interrupt. This parameter can be: ENABLE or DISABLE. Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Input          : - DMARxDesc: pointer on a DMA Rx descriptor
 *                  - NewState: new state of the specified DMA Rx Desc end of
 *ring. This parameter can be: ENABLE or DISABLE. Output         : None Return
 *: None
 *******************************************************************************/
 ...
 * Input          : - DMARxDesc: pointer on a DMA Rx descriptor
 *                  - NewState: new state of the specified DMA Rx Desc second
 *address chained. This parameter can be: ENABLE or DISABLE. Output         :
 *None Return         : None
 *******************************************************************************/
                                ...
                                u32 DMARxDesc_Buffer )
{
    ...
    if( DMARxDesc_Buffer != ETH_DMARxDesc_Buffer1 )
    {
        ...
        return ( ( DMARxDesc->ControlBufferSize & ETH_DMARxDesc_RBS2 ) >>
                 ETH_DMARxDesc_Buffer2SizeShift );
    }
    else
    {
 ...
 * Desciption     : Checks whether the ETHERNET software reset bit is set or
 *not. Input          : None Output         : None Return         : The new
 *state of DMA Bus Mode register SR bit (SET or RESET).
 *******************************************************************************/
 ...
 * Desciption     : Checks whether the specified ETHERNET DMA flag is set or
 *not. Input          : - ETH_DMA_IT: specifies the flag to check. This
 *parameter can be one of the following values:
 *                       - ETH_DMA_FLAG_TST : Time-stamp trigger flag
 *                       - ETH_DMA_FLAG_PMT : PMT flag
 *                       - ETH_DMA_FLAG_MMC : MMC flag
 *                       - ETH_DMA_FLAG_DataTransferError : Error bits 0-data
 *buffer, 1-desc. access
 *                       - ETH_DMA_FLAG_ReadWriteError    : Error bits 0-write
 *trnsf, 1-read transfr
 *                       - ETH_DMA_FLAG_AccessError       : Error bits 0-Rx DMA,
 *1-Tx DMA
 ...
 * Desciption     : Checks whether the specified ETHERNET DMA interrupt has
 *occured or not. Input          : - ETH_DMA_IT: specifies the interrupt source
 *to check. This parameter can be one of the following values:
 ...
 *                       - ETH_DMA_IT_TBU : Transmit buffer unavailable
 *interrupt
 ...
 *                     - ETH_DMA_TransmitProcess_Stopped   : Stopped - Reset or
 *Stop Tx Command issued
 *                     - ETH_DMA_TransmitProcess_Fetching  : Running - fetching
 *the Tx descriptor
 *                     - ETH_DMA_TransmitProcess_Waiting   : Running - waiting
 *for status
 *                     - ETH_DMA_TransmitProcess_Reading   : unning - reading
 *the data from host memory
 *                     - ETH_DMA_TransmitProcess_Suspended : Suspended - Tx
 *Desciptor unavailabe
 *                     - ETH_DMA_TransmitProcess_Closing   : Running - closing
 *Rx descriptor
 *******************************************************************************/
 ...
 *                  This can be one of the following values:
 *                     - ETH_DMA_ReceiveProcess_Stopped   : Stopped - Reset or
 *Stop Rx Command issued
 *                     - ETH_DMA_ReceiveProcess_Fetching  : Running - fetching
 *the Rx descriptor
 *                     - ETH_DMA_ReceiveProcess_Waiting   : Running - waiting
 *for packet
 *                     - ETH_DMA_ReceiveProcess_Suspended : Suspended - Rx
 *Desciptor unavailable
 *                     - ETH_DMA_ReceiveProcess_Closing   : Running - closing
 *descriptor
 *                     - ETH_DMA_ReceiveProcess_Queuing   : Running - queuing
 *the recieve frame into host memory
 *******************************************************************************/
 ...
 * Desciption     : Checks whether the ETHERNET transmit FIFO bit is cleared or
 *not. Input          : None Output         : None Return         : The new
 *state of ETHERNET flush transmit FIFO bit (SET or RESET).
 *******************************************************************************/
 ...
 * Input          : - ETH_DMA_IT: specifies the ETHERNET DMA interrupt sources
 *to be enabled or disabled. This parameter can be any combination of the
 *following values:
 *                       - ETH_DMA_IT_NIS : Normal interrupt summary
 *                       - ETH_DMA_IT_AIS : Abnormal interrupt summary
 ...
 *                       - ETH_DMA_IT_TPS : Transmit process stopped interrupt
 *                       - ETH_DMA_IT_T   : Transmit interrupt
 *                  - NewState: new state of the specified ETHERNET DMA
 *interrupts. This parameter can be: ENABLE or DISABLE. Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Checks whether the specified ETHERNET DMA overflow flag is
 *set or not. Input          : - ETH_DMA_Overflow: specifies the DMA overflow
 *flag to check. This parameter can be one of the following values:
 *                       - ETH_DMA_Overflow_RxFIFOCounter : Overflow for FIFO
 *Overflow Counter
 *                       - ETH_DMA_Overflow_MissedFrameCounter : Overflow for
 *Missed Frame Counter Output         : None Return         : The new state of
 *ETHERNET DMA overflow Flag (SET or RESET).
 *******************************************************************************/
...
{
    return ( ( u32 ) ( ( ETH_DMA->DMAMFBOCR & ETH_DMAMFBOCR_MFA ) >>
                       ETH_DMA_RxOverflowMissedFramesCounterShift ) );
}

/*******************************************************************************
 * Function Name  : ETH_GetBufferUnavailableMissedFrameCounter
 * Desciption     : Get the ETHERNET DMA Buffer Unavailable Missed Frame Counter
 *value. Input          : None Output         : None Return         : The value
 *of Buffer unavailable Missed Frame Counter.
 *******************************************************************************/
 ...
 * Desciption     : Resumes the DMA Transmission by writing to the
 *DmaTxPollDemand register: (the data written could be anything). This forces
 ...
 * Desciption     : Resumes the DMA Transmission by writing to the
 *DmaRxPollDemand register: (the data written could be anything). This forces
 ...
 * Input          : - Buffer: Pointer on remote WakeUp Frame Filter Register
 *buffer data (8 words). Output         : None Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Checks whether the specified ETHERNET PMT flag is set or
 *not. Input          : - ETH_PMT_FLAG: specifies the flag to check. This
 *parameter can be one of the following values:
 ...
 *                    any combination of Rx interrupt (but not both)of the
 *following values:
 *                       - ETH_MMC_IT_TGF   : When Tx good frame counter reaches
 *half the maximum value
 *                       - ETH_MMC_IT_TGFMSC: When Tx good multi col counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_TGFSC : When Tx good single col counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_RGUF  : When Rx good unicast frames
 *counter reaches half the maximum value
 *                       - ETH_MMC_IT_RFAE  : When Rx alignment error counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_RFCE  : When Rx crc error counter reaches
 *half the maximum value
 *                  - NewState: new state of the specified ETHERNET MMC
 *interrupts. This parameter can be: ENABLE or DISABLE. Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Input          : - ETH_MMC_IT: specifies the ETHERNET MMC interrupt.
 *                    This parameter can be one of the following values:
 *                       - ETH_MMC_IT_TxFCGC: When Tx good frame counter reaches
 *half the maximum value
 *                       - ETH_MMC_IT_TxMCGC: When Tx good multi col counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_TxSCGC: When Tx good single col counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_RxUGFC: When Rx good unicast frames
 *counter reaches half the maximum value
 *                       - ETH_MMC_IT_RxAEC : When Rx alignment error counter
 *reaches half the maximum value
 *                       - ETH_MMC_IT_RxCEC : When Rx crc error counter reaches
 *half the maximum value Output         : None Return         : The value of
 *ETHERNET MMC IT (SET or RESET).
 *******************************************************************************/
ITStatus ETH_GetMMCITStatus( u32 ETH_MMC_IT )
{
    ...
    {
        /* ETHERNET MMC Rx interrupts selected */
        /* Check if the ETHERNET MMC Rx selected interrupt is enabled and
         * occured */
        if( ( ( ( ETH_MMC->MMCRIR & ETH_MMC_IT ) != ( u32 ) RESET ) ) &&
            ( ( ETH_MMC->MMCRIMR & ETH_MMC_IT ) != ( u32 ) RESET ) )
        {
            bitstatus = SET;
        }
        else
    ...
    {
        /* ETHERNET MMC Tx interrupts selected */
        /* Check if the ETHERNET MMC Tx selected interrupt is enabled and
         * occured */
        if( ( ( ( ETH_MMC->MMCTIR & ETH_MMC_IT ) != ( u32 ) RESET ) ) &&
            ( ( ETH_MMC->MMCRIMR & ETH_MMC_IT ) != ( u32 ) RESET ) )
        {
            bitstatus = SET;
        }
 ...
 * Desciption     : Updated the PTP block for fine correction with the Time
 *Stamp Addend register value. Input          : None Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Updated the PTP system time with the Time Stamp Update
 *register value. Input          : None Output         : None Return         :
 *None
 *******************************************************************************/
void ETH_EnablePTPTimeStampUpdate( void )
{
    /* Enable the PTP system time update with the Time Stamp Update register
     * value */
 ...
 * Desciption     : Enables or disables the PTP time stamp for transmit and
 *receive frames. Input          : - NewState: new state of the PTP time stamp
 *for transmit and receive frames This parameter can be: ENABLE or DISABLE.
 * Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 * Desciption     : Checks whether the specified ETHERNET PTP flag is set or
 *not. Input          : - ETH_PTP_FLAG: specifies the flag to check. This
 *parameter can be one of the following values:
 *                       - ETH_PTP_FLAG_TSARU : Addend Register Update
 *                       - ETH_PTP_FLAG_TSITE : Time Stamp Interrupt Trigger
 *Enable
 ...
 * Input          : - SubSecondValue: specifies the PTP Sub-Second Increment
 *Register value. Output         : None Return         : None
 *******************************************************************************/
 ...
 *                  - SubSecondValue: specifies the PTP Time update sub-second
 *value. this is a 31 bit value. bit32 correspond to the sign. Output         :
 *None Return         : None
 *******************************************************************************/
void ETH_SetPTPTimeStampUpdate( u32 Sign, u32 SecondValue, u32 SubSecondValue )
{
    ...
    eth_assert_param(
        IS_ETH_PTP_TIME_STAMP_UPDATE_SUBSECOND( SubSecondValue ) );
 ...
 * Input          : - HighValue: specifies the PTP Target Time High Register
 *value.
 *                  - LowValue: specifies the PTP Target Time Low Register
                                ...
                                u32 TxBuffCount )
{
    ...
    for( i = 0; i < TxBuffCount; i++ )
    {
        ...
        ( &DMAPTPTxDescTab[ i ] )
            ->Buffer2NextDescAddr = DMATxDesc->Buffer2NextDescAddr;
    }

    /* Store on the last DMAPTPTxDescTab desc status record the first list
     * address */
                                ...
                                u32 RxBuffCount )
{
    ...
    for( i = 0; i < RxBuffCount; i++ )
    {
        ...
        ( &DMAPTPRxDescTab[ i ] )
            ->Buffer2NextDescAddr = DMARxDesc->Buffer2NextDescAddr;
    }

    /* Store on the last DMAPTPRxDescTab desc status record the first list
     * address */
 ...
 *with Time Stamp values. Input          : - ppkt: pointer to application packet
 *Buffer.
 *                  - FrameLength: Tx Packet size.
 *                  - PTPTxTab: Pointer on the first PTP Tx table to store Time
 *stamp values. Output         : None Return         : ETH_ERROR: in case of Tx
 *desc owned by DMA ETH_SUCCESS: for correct transmission
 *******************************************************************************/
u32 ETH_HandlePTPTxPkt( u8 * ppkt, u16 FrameLength, u32 * PTPTxTab )
{
    ...
    /* Copy the frame to be sent into memory pointed by the current ETHERNET DMA
     * Tx descriptor */
    for( offset = 0; offset < FrameLength; offset++ )
    {
        ( *( vu8 * ) ( ( DMAPTPTxDescToSet->Buffer1Addr ) +
                       offset ) ) = ( *( ppkt + offset ) );
    }
    ...
    *PTPTxTab = DMATxDescToSet->Buffer2NextDescAddr;
    ...
    if( ( DMATxDescToSet->Status & ETH_DMATxDesc_TCH ) != ( u32 ) RESET )
    {
        ...
        DMATxDescToSet = ( ETH_DMADESCTypeDef * ) ( DMAPTPTxDescToSet
                                                        ->Buffer2NextDescAddr );

        if( DMAPTPTxDescToSet->Status != 0 )
        {
            DMAPTPTxDescToSet = ( ETH_DMADESCTypeDef * ) ( DMAPTPTxDescToSet
                                                               ->Status );
        }
        else
        {
    ...
    else /* Ring Mode */
    {
        ...
        {
            /* Selects the next DMA Tx descriptor list for next buffer read:
               this will be the first Tx descriptor in this case */
            DMATxDescToSet = ( ETH_DMADESCTypeDef * ) ( ETH_DMA->DMATDLAR );
        ...
        else
        {
            ...
            DMATxDescToSet = ( ETH_DMADESCTypeDef * ) ( ( u32 ) DMATxDescToSet +
                                                        ...
                                                        ( ( ETH_DMA->DMABMR &
                                                            ETH_DMABMR_DSL ) >>
                                                          2 ) );
            DMAPTPTxDescToSet = ( ETH_DMADESCTypeDef * ) ( ( u32 )
                                                               DMAPTPTxDescToSet +
                                                           0x10 +
                                                           ( ( ETH_DMA->DMABMR &
                                                               ETH_DMABMR_DSL ) >>
 ...
 * Desciption     : Receives a packet and copies it to memory pointed by ppkt
 *with Time Stamp values. Input          : - PTPRxTab: Pointer on the first PTP
 *Rx table to store Time stamp values. Output         : ppkt: pointer on
 *application receive buffer. Return         : ETH_ERROR: if there is error in
 *reception Received packet size: if packet reception is correct
 *******************************************************************************/
u32 ETH_HandlePTPRxPkt( u8 * ppkt, u32 * PTPRxTab )
{
        ...
        ( ( DMARxDescToGet->Status & ETH_DMARxDesc_FS ) != ( u32 ) RESET ) )
    {
        ...
        /* Copy the received frame into buffer from memory pointed by the
         * current ETHERNET DMA Rx descriptor */
        for( offset = 0; offset < FrameLength; offset++ )
        {
            ( *( ppkt + offset ) ) = ( *(
                vu8 * ) ( ( DMAPTPRxDescToGet->Buffer1Addr ) + offset ) );
        }
    }
    else
    {
    ...
    *PTPRxTab = DMARxDescToGet->Buffer2NextDescAddr;
    ...
    {
        /* Selects the next DMA Rx descriptor list for next buffer read */
        DMARxDescToGet = ( ETH_DMADESCTypeDef * ) ( DMAPTPRxDescToGet
                                                        ->Buffer2NextDescAddr );

        if( DMAPTPRxDescToGet->Status != 0 )
        {
            DMAPTPRxDescToGet = ( ETH_DMADESCTypeDef * ) ( DMAPTPRxDescToGet
                                                               ->Status );
        }
        else
        {
