...
static int32_t sha_get_data( sha_handle_t handle, uint32_t * data )
{
    ...
    if( sha_priv->mode == SHA_MODE_1 || sha_priv->mode == SHA_MODE_224 ||
        sha_priv->mode == SHA_MODE_256 )
    {
...
/**
  ...
  the SHA interface 2.registers event callback function \param[in]   idx must
  not exceed return value of csi_sha_get_instance_count() \param[in]   cb_event
  Pointer to \ref sha_event_cb_t \return      return sha handle if success
*/
                        ...
                        uint32_t len )
{
    ...
    if( left && len >= fill )
    {
        if( finish_flag )
        {
            ...
            {
                memset( &sha_buffer[ left + 4 - last_left ],
                        0x0,
                        len - 4 + last_left );
        ...
        else
        {
            if( last_left && sha_priv->endian == SHA_ENDIAN_MODE_LITTLE )
            {
                ...
                {
                    *( sha_buffer + left + 3 - last_left -
                       i ) = *( ( uint8_t * ) p + 3 - last_left - i );
                }
            ...
            {
                memcpy( ( void * ) ( sha_buffer + left + 4 - last_left ),
                        p,
                        fill );
            }
            else
    ...
    while( len >= block_size )
    {
        ...
        if( finish_flag )
        {
            ...
            {
                sha_input_data( ( uint32_t * ) &sha_buffer[ block_size ],
                                block_size >> 2 );
            }
        }
        else
...
int32_t csi_sha_finish( sha_handle_t handle, void * context, void * output )
{
    ...
    uint32_t padn = ( last < ( block_size - message_len ) )
                        ? ( block_size - last )
                        : ( block_size + block_size - last );
