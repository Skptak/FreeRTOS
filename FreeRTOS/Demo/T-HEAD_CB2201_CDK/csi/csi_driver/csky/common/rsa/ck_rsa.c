...
static uint32_t rsa_run_adjust_sp_if_need( uint32_t sp )
{
    ...
    if( ( sp < ( RSA_SP_HOLE_UP + TEE_CONEXT_SIZE ) ) &&
        ( sp > RSA_SP_HOLE_DOWN ) )
        sp_chg = 1;
    else
        sp_chg = 0;
...
{
    __asm__ __volatile__( "subi   sp, 0x10   \n"
                          "stm    r4-r7, (sp) \n"
...
#endif
            rsa_opr_start();
            while( ( !rsa_opr_done() ) && ( rsa_loop_cnt() < MAX_RSA_LP_CNT ) &&
                   ( !rsa_raise_exception() ) )
                ;
#if( CONFIG_PLATFORM_HOBBIT1_2 > 0 )
        }
#endif
        if( ( rsa_loop_cnt() >= MAX_RSA_LP_CNT ) || rsa_raise_exception() )
        {
            ret = 1;
        }
        else
                                 ...
                                 uint32_t * r )
{
    ...
    for( i = 0; i < b_words; i++ )
    {
        ...
        borrow = ( ( UINT64H_TO_UINT32( tmp ) == 0 )
                       ? ( 0 )
                                ...
                                uint32_t keywords )
{
    ...
    do
    {
        ...
        uint32_t tmpa_valid_bits = get_valid_bits( tmpa.pdata,
                                                   tmpa_words,
                                                   keywords );
                                 ...
                                 uint32_t * tmp_c )
{
    ...
    uint32_t ret = word_array_mod( tmp.pdata,
                                   tmp.words,
                                   ( uint32_t * ) modulus,
                                   words,
                                   tmp_c,
...
static const uint8_t der_sha256_t[] = { 0x30, 0x31, 0x30, 0x0d, 0x06, 0x09,
                                        0x60, 0x86, 0x48, 0x01, 0x65, 0x03,
                                        0x04, 0x02, 0x01, /* id-sha256 */
                                        0x05, 0x00, 0x04, 0x20 };

static const uint8_t der_sha1_t[] = { 0x30, 0x21, 0x30, 0x09, 0x06,
                 ...
                 uint32_t * tmp_c )
{
    ...
    if( padding == PKCS1_PADDING )
    {
        ...
        convert_byte_array( ( uint8_t * ) tmp_in_out,
                            ( uint8_t * ) tmp_in_out,
                            keybytes );
    }
    else
    {
        convert_byte_array( ( uint8_t * ) src,
                            ( uint8_t * ) tmp_in_out,
                            keybytes );
    }
              ...
              uint32_t * tmp_c )
{
    ...
    ret = rsa_padding_pkcs( src, ( uint8_t * ) tmp_in_out, type, keybytes );
...
/**
  ...
  the RSA interface 2.registers event callback function \param[in]   idx  must
  not exceed return value of csi_rsa_get_instance_count() \param[in]   cb_event
  Pointer to \ref rsa_event_cb_t \return      pointer to rsa handle
*/
                         ...
                         rsa_padding_t padding )
{
    ...
    if( src_size <= 0 || ( padding.padding_type != RSA_PADDING_MODE_PKCS1 &&
    ...
    rsa_encrypt( ( uint8_t * ) n,
                 ( uint8_t * ) e,
                 ( uint8_t * ) src,
                 ( uint32_t ) src_size,
                 ( uint8_t * ) out,
                 ( uint32_t * ) out_size,
                 ( uint32_t ) ( padding.padding_type ),
                 bit_length,
                 tmp_c );
