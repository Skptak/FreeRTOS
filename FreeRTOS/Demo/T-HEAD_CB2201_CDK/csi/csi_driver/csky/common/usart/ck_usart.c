...
/**
  ...
  \param[in]   addr  usart base to operate.
  \param[in]   stopbit two possible value: USART_STOP_BITS_1 and
  USART_STOP_BITS_2. \return      error code
*/
...
/**
  \brief       the transmit data length,and we have four choices:5, 6, 7, and 8
  bits. \param[in]   addr  usart base to operate. \param[in]   databits the data
  length that user decides. \return      error code
*/
...
static void dw_usart_intr_threshold_empty( dw_usart_priv_t * usart_priv )
{
    ...
    if( usart_priv->tx_cnt >= usart_priv->tx_total_num )
    {
        ...
        {
            usart_priv->cb_event( USART_EVENT_SEND_COMPLETE,
                                  usart_priv->cb_arg );
        }
    }
}
...
static void dw_usart_intr_recv_data( dw_usart_priv_t * usart_priv )
{
    ...
    *( ( uint8_t * ) usart_priv->rx_buf ) = data;
    ...
    if( usart_priv->rx_cnt >= usart_priv->rx_total_num )
    {
        ...
        if( usart_priv->cb_event )
        {
            usart_priv->cb_event( USART_EVENT_RECEIVE_COMPLETE,
                                  usart_priv->cb_arg );
        }
    }
}
...
void dw_usart_irqhandler( int32_t idx )
{
    ...
    switch( intr_state )
    {
        case DW_IIR_THR_EMPTY: /* interrupt source:transmitter holding register
                                  empty */
            dw_usart_intr_threshold_empty( usart_priv );
            break;

        case DW_IIR_RECV_DATA: /* interrupt source:receiver data available or
...
/**
  ...
  for the USART interface 2.registers event callback function \param[in]   usart
  pin of tx \param[in]   usart pin of rx \param[in]   cb_event  Pointer to \ref
