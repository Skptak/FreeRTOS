                              ...
                              uint32_t cnt )
{
    ...
    uint32_t pre_count = ( cnt > ( EFLASH_SECTOR_SIZE - pre_offset ) )
                             ? ( EFLASH_SECTOR_SIZE - pre_offset )
                             : cnt;
        ...
        for( i = 0; i < pre_count; i++ )
        {
            if( *( ( uint8_t * ) dst + i ) != *( ( uint8_t * ) p + i ) )
            {
                ...
                *( volatile uint32_t * ) 0x50004000 = '\n';
                ...
                eflash_program_word( handle,
                                     ( uint32_t ) block_addr,
                                     context,
                                     EFLASH_SECTOR_SIZE >> 2 );
                break;
            }
        }
...
/**
  ...
  device id \param[in]   cb_event  Pointer to \ref eflash_event_cb_t \return
  pointer to eflash handle
*/
                         ...
                         uint32_t cnt )
{
    ...
    if( eflash_priv->eflashinfo.start > addr ||
        eflash_priv->eflashinfo.end < addr ||
        eflash_priv->eflashinfo.start > ( addr + cnt - 1 ) ||
        eflash_priv->eflashinfo.end < ( addr + cnt - 1 ) )
    {
        return ERR_EFLASH( EDRV_PARAMETER );
    }
                            ...
                            uint32_t cnt )
{
    ...
    else
    {
        ...
        {
            pad_buf = buffer_b[ 0 ] | ( buffer_b[ 1 ] << 8 ) |
                      ( buffer_b[ 2 ] << 16 ) | ( buffer_b[ 3 ] << 24 );
