...
static uint32_t timeout_ms[ 16 ] = { 4,     7,     13,    26,    52,   105,
                                     210,   419,   839,   1678,  3355, 6711,
...
/**
  ...
  the WDT interface 2.registers event callback function \param[in]   idx   must
  not exceed return value of csi_wdt_get_instance_count() \param[in]   cb_event
  Pointer to \ref wdt_event_cb_t \return      pointer to wdt instance
*/
...
/**
  ...
  software resources used by the interface \param[in]   instance  wdt instance
  to operate. \return      \ref execution_status
*/
...
/**
  \brief       Set the WDT value. value = (2^t*0xffff * 10^6 /freq)/10^3(t: 0 ~
  15). \param[in]   handle wdt handle to operate. \param[in]   value     the
  timeout value(ms) \ref:timeout_ms[] \return      \ref execution_status
*/
