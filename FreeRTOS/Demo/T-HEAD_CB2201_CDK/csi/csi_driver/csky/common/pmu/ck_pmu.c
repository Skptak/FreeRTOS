...
/**
  ...
  the PMU interface 2.registers event callback function \param[in]   idx device
  id \param[in]   cb_event  Pointer to \ref pmu_event_cb_t \return      pointer
  to pmu handle
*/
...
int32_t drv_pmu_enter_sleep( pmu_handle_t handle, pmu_mode_e mode )
{
    ...
    switch( mode )
    {
        ...
        case PMU_MODE_DORMANT:
            ...
            {
                *( volatile unsigned int * ) ( 0xe000e1c0 ) = 0xffffffff; // reload
                                                                          // wakeup_IRQ
                *( volatile unsigned int * ) ( 0xe000e280 ) = 0xffffffff; // clear
                                                                          // pend
                                                                          // IRQ
