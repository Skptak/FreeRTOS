...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_iic_config[ CONFIG_IIC_NUM ] = { { CSKY_I2C0_BASE, I2C0_IRQn },
...
const static iic_pin_map_t s_iic_pin_map[] = {
    { PA4_SCL0_PWM4_SPI0RX_XX, PA5_SDA0_PWM5_SPI0CS_XX, 0, 0 },
    { PA6_SPI0CLK_PWMTRIG0_SCL0_XX, PA7_SPI0TX_PWMTRIG1_SDA0_XX, 0, 2 },
    { PA31_I2SSDA__SCL0_PWM4_XX, PB0_ADC0_SDA0_PWM5_XX, 0, 1 },
    { PA8_SPI0RX_TRIGFAULT_SCL1_XX, PA9_SPI0CS_PWM0_SDA1_XX, 1, 2 },
    { PA14_SCL1_PWM5_SPI1RX_XX, PA15_SDA1_PWMTRIG0_SPI1CS0_XX, 1, 0 },
    { PB1_ADC1_SCL1_USISCLK_XX, PB2_ADC2_SDA1_USISD0_XX, 1, 1 }
};

/**
  \param[in]   instance idx, must not exceed return value of
  target_get_iic_count() \brief       get iic instance. \return      pointer to
  iic instance
*/
int32_t target_iic_init( pin_t scl, pin_t sda, uint32_t * base, uint32_t * irq )
{
    ...
    for( idx = 0; idx < sizeof( s_iic_pin_map ) / sizeof( iic_pin_map_t );
         idx++ )
    {
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_usart_config[ CONFIG_USART_NUM ] = { { CSKY_UART0_BASE, UART0_IRQn },
...
const static usart_pin_map_t s_usart_pin_map[] = {
    { PA0_TXD0_PWM0_XX_SIROUT0, PA1_RXD0_PWM1_XX_SIRIN0, -1, -1, 0, 0 },
    { PA10_TXD1_PWM1_XX_SIROUT1, PA11_RXD1_PWM2_XX_SIRIN1, -1, -1, 1, 0 },
    { PA23_TXD2_PWM5_XX_SIROUT2,
      ...
      2,
      0 },
    { PA26_TXD3_PWMFAULT_XX_SIROUT3, PA27_RXD3_PWM0_XX_SIRIN3, -1, -1, 3, 0 }
};

/**
  \param[in]   instance idx, must not exceed return value of
  target_get_usart_count() \brief       get usart instance. \return      pointer
  to usart instance
*/
int32_t target_usart_init( pin_t tx, pin_t rx, uint32_t * base, uint32_t * irq )
{
    ...
    for( idx = 0; idx < sizeof( s_usart_pin_map ) / sizeof( usart_pin_map_t );
         idx++ )
    {
        if( s_usart_pin_map[ idx ].tx == tx && s_usart_pin_map[ idx ].rx == rx )
        {
            ...
            *irq = sg_usart_config[ s_usart_pin_map[ idx ].cfg_idx ].irq;
            /*pinmux*/
            pin_mux( s_usart_pin_map[ idx ].tx,
                     s_usart_pin_map[ idx ].function );
            pin_mux( s_usart_pin_map[ idx ].rx,
                     s_usart_pin_map[ idx ].function );
                                    ...
                                    uint32_t flag )
{
    ...
    for( idx = 0; idx < sizeof( s_usart_pin_map ) / sizeof( usart_pin_map_t );
         idx++ )
    {
        if( ( s_usart_pin_map[ idx ].cts == tx_flow ) &&
            ( s_usart_pin_map[ idx ].rts == rx_flow ) )
            break;
    }
...
{
    uint32_t base;
} const sg_trng_config[ CONFIG_TRNG_NUM ] = { { CSKY_TRNG_BASE } };
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_trng_count() \brief       get trng instance. \return      pointer
  to trng instance
*/
...
{
    uint32_t base;
} const sg_crc_config[ CONFIG_CRC_NUM ] = { { CSKY_CRC_BASE } };
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_crc_count() \brief       get crc instance. \return      pointer to
  crc instance
*/
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_eflash_count() \brief       get eflash instance. \return pointer to
  eflash instance
*/
...
const static gpio_pin_map_t s_gpio_pin_map[] = {
    { PA0_TXD0_PWM0_XX_SIROUT0, 0 },       { PA1_RXD0_PWM1_XX_SIRIN0, 0 },
    { PA2_CTS0_PWM2_SPI0CLK_XX, 0 },       { PA3_RTS0_PWM3_SPI0TX_XX, 0 },
    { PA4_SCL0_PWM4_SPI0RX_XX, 0 },        { PA5_SDA0_PWM5_SPI0CS_XX, 0 },
    { PA6_SPI0CLK_PWMTRIG0_SCL0_XX, 0 },   { PA7_SPI0TX_PWMTRIG1_SDA0_XX, 0 },
    { PA8_SPI0RX_TRIGFAULT_SCL1_XX, 0 },   { PA9_SPI0CS_PWM0_SDA1_XX, 0 },
    { PA10_TXD1_PWM1_XX_SIROUT1, 0 },      { PA11_RXD1_PWM2_XX_SIRIN1, 0 },
    { PA12_CTS1_PWM3_SPI1CLK_XX, 0 },      { PA13_RTS1_PWM4_SPI1TX_XX, 0 },
    { PA14_SCL1_PWM5_SPI1RX_XX, 0 },       { PA15_SDA1_PWMTRIG0_SPI1CS0_XX, 0 },
    { PA16_SPI1CLK_PWMTRIG1_XX_XX, 0 },    { PA17_SPI1TX_PWMFAULT_XX_XX, 0 },
    { PA18_SPI1RX_PWM0_XX_XX, 0 },         { PA19_SPI1CS0_PWM1_XX_XX, 0 },
    { PA20_SPI1CS1_PWM2_XX_XX, 0 },        { PA21_SPI1CS2_PWM3_XX_XX, 0 },
    { PA22_RXD2_PWM4_XX_SIRIN2, 0 },       { PA23_TXD2_PWM5_XX_SIROUT2, 0 },
    { PA24_CTS2_PWMTRIG0_SPI1CS1_XX, 0 },  { PA25_XX_PWMTRIG1_SPI1CS2_XX, 0 },
    { PA26_TXD3_PWMFAULT_XX_SIROUT3, 0 },  { PA27_RXD3_PWM0_XX_SIRIN3, 0 },
    { PA28_I2SMCLK_PWM1_XX_XX, 0 },        { PA29_I2SSCLK_PWM2_XX_XX, 0 },
    { PA30_I2SWSCLK_PWM3_XX_XX, 0 },       { PA31_I2SSDA__SCL0_PWM4_XX, 0 },
    { PB0_ADC0_SDA0_PWM5_XX, 1 },          { PB1_ADC1_SCL1_USISCLK_XX, 1 },
    { PB2_ADC2_SDA1_USISD0_XX, 1 },        { PB3_ADC3_SPI1CLK_USISD1_XX, 1 },
    { PB4_ADC4_SPI1TX_USINSS_XX, 1 },      { PB5_ADC5_SPI1RX_USISCLK_XX, 1 },
    { PB6_ADC6_SPI1CS0_USISD0_XX, 1 },     { PB7_ADC7_SPI1CS1_USISD1_XX, 1 },
    { PB8_PWMTRIG0_SPI1CS2_USINSS_XX, 1 }, { PB9_PWMTRIG1_CTS3_XX_XX, 1 },
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_gpio_count() \brief       get gpio instance. \return      pointer
  to gpio instance
*/
int32_t target_gpio_pin_init( pin_t gpio_pin, uint32_t * port_idx )
{
    ...
    for( idx = 0; idx < sizeof( s_gpio_pin_map ) / sizeof( gpio_pin_map_t );
         idx++ )
    {
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_aes_config[ CONFIG_AES_NUM ] = { { CSKY_AES_BASE, AES_IRQn } };
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_aes_count() \brief       get aes instance. \return      pointer to
  aes instance
*/
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_rsa_config[ CONFIG_RSA_NUM ] = { { CSKY_RSA_BASE, RSA_IRQn } };
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_rsa_count() \brief       get rsa instance. \return      pointer to
  rsa instance
*/
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_rtc_config[ CONFIG_RTC_NUM ] = { { CSKY_RTC_BASE, RTC_IRQn } };
...
const sg_spi_config[ CONFIG_SPI_NUM ] = { { CSKY_SPI0_BASE, SPI0_IRQn },
...
const static spi_pin_map_t s_spi_pin_map[] = { { PA7_SPI0TX_PWMTRIG1_SDA0_XX,
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_spi_count() \brief       get spi instance. \return      pointer to
  spi instance
*/
                         ...
                         uint32_t * irq )
{
    ...
    for( idx = 0; idx < sizeof( s_spi_pin_map ) / sizeof( spi_pin_map_t );
         idx++ )
    {
        if( s_spi_pin_map[ idx ].mosi == mosi &&
            s_spi_pin_map[ idx ].miso == miso &&
            s_spi_pin_map[ idx ].sclk == sclk &&
            s_spi_pin_map[ idx ].ssel == ssel )
        {
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_wdt_config[ CONFIG_WDT_NUM ] = { { CSKY_WDT_BASE, WDT_IRQn } };
...
struct
{
    uint32_t base;
    uint32_t irq;
} const sg_sha_config[ CONFIG_SHA_NUM ] = { { CSKY_SHA_BASE, SHA_IRQn } };
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_sha_count() \brief       get sha instance. \return      pointer to
  sha instance
*/
...
/**
  \param[in]   instance idx, must not exceed return value of
  target_get_pwm_count() \brief       get pwm instance. \return      pointer to
  pwm instance
*/
                         ...
                         uint32_t * irq )
{
    ...
    for( idx = 0; idx < sizeof( s_pwm_pin_map ) / sizeof( pwm_pin_map_t );
         idx++ )
    {
        if( s_pwm_pin_map[ idx ].pwm_pin == pwm_pin )
        {
            ...
            *ch_num = s_pwm_pin_map[ idx ].ch_num;
            /*pinmux*/
            pin_mux( s_pwm_pin_map[ idx ].pwm_pin,
                     s_pwm_pin_map[ idx ].function );
