...
#define MAX_FRAMELEN \
    1518 /*  (note: maximum ethernet frame length would be 1518) */

#define CSI_ETH_MAC_API_VERSION \
    CSI_DRIVER_VERSION_MAJOR_MINOR( 2, 1 ) /* API version */
...
#define CSI_ETH_MAC_CONTROL_TX \
    ( 0x02 ) ///< Transmitter; arg: 0=disabled (default), 1=enabled
#define CSI_ETH_MAC_CONTROL_RX \
    ( 0x03 ) ///< Receiver; arg: 0=disabled (default), 1=enabled
#define CSI_ETH_MAC_FLUSH \
    ( 0x04 ) ///< Flush buffer; arg = CSI_ETH_MAC_FLUSH_...
#define CSI_ETH_MAC_SLEEP \
    ( 0x05 ) ///< Sleep mode; arg: 1=enter and wait for Magic packet, 0=exit
#define CSI_ETH_MAC_VLAN_FILTER \
    ( 0x06 ) ///< VLAN Filter for received frames; arg15..0: VLAN Tag; arg16:
             ///< optional CSI_ETH_MAC_VLAN_FILTER_ID_ONLY; 0=disabled
             ///< (default)

/*----- Ethernet MAC Configuration -----*/
#define CSI_ETH_MAC_SPEED_Pos 0
#define CSI_ETH_MAC_SPEED_Msk ( 3UL << CSI_ETH_MAC_SPEED_Pos )
#define CSI_ETH_MAC_SPEED_10M \
    ( CSI_ETH_SPEED_10M << CSI_ETH_MAC_SPEED_Pos ) ///< 10 Mbps link speed
#define CSI_ETH_MAC_SPEED_100M \
    ( CSI_ETH_SPEED_100M << CSI_ETH_MAC_SPEED_Pos ) ///< 100 Mbps link speed
#define CSI_ETH_MAC_SPEED_1G \
    ( CSI_ETH_SPEED_1G << CSI_ETH_MAC_SPEED_Pos ) ///< 1 Gpbs link speed
#define CSI_ETH_MAC_DUPLEX_Pos 2
#define CSI_ETH_MAC_DUPLEX_Msk ( 1UL << CSI_ETH_MAC_DUPLEX_Pos )
#define CSI_ETH_MAC_DUPLEX_HALF \
    ( CSI_ETH_DUPLEX_HALF << CSI_ETH_MAC_DUPLEX_Pos ) ///< Half duplex link
#define CSI_ETH_MAC_DUPLEX_FULL \
    ( CSI_ETH_DUPLEX_FULL << CSI_ETH_MAC_DUPLEX_Pos ) ///< Full duplex link
#define CSI_ETH_MAC_LOOPBACK ( 1UL << 4 )             ///< Loop-back test mode
#define CSI_ETH_MAC_CHECKSUM_OFFLOAD_RX \
    ( 1UL << 5 ) ///< Receiver Checksum offload
#define CSI_ETH_MAC_CHECKSUM_OFFLOAD_TX \
    ( 1UL << 6 ) ///< Transmitter Checksum offload
#define CSI_ETH_MAC_ADDRESS_BROADCAST \
    ( 1UL << 7 ) ///< Accept frames with Broadcast address
#define CSI_ETH_MAC_ADDRESS_MULTICAST \
    ( 1UL << 8 ) ///< Accept frames with any Multicast address
#define CSI_ETH_MAC_ADDRESS_ALL \
    ( 1UL << 9 ) ///< Accept frames with any address (Promiscuous Mode)
...
#define CSI_ETH_MAC_VLAN_FILTER_ID_ONLY \
    ( 1UL << 16 ) ///< Compare only the VLAN Identifier (12-bit)

/****** Ethernet MAC Frame Transmit Flags *****/
#define CSI_ETH_MAC_TX_FRAME_FRAGMENT ( 1UL << 0 ) ///< Indicate frame fragment
#define CSI_ETH_MAC_TX_FRAME_EVENT \
    ( 1UL << 1 ) ///< Generate event when frame is transmitted
#define CSI_ETH_MAC_TX_FRAME_TIMESTAMP \
    ( 1UL << 2 ) ///< Capture frame time stamp
...
#define CSI_ETH_MAC_TIMER_ADJUST_CLOCK \
    ( 0x06 ) ///< Adjust clock frequency; time->ns: correction factor * 2^31
...
typedef void ( *eth_event_cb_t )( eth_mac_handle_t handle,
                                  uint32_t event ); ///< Pointer to \ref
                                                    ///< eth_event_cb_t : Signal
                                                    ///< Ethernet Event.
...
{
    uint32_t checksum_offload_rx_ip4 : 1; ///< 1 = IPv4 header checksum verified
                                          ///< on receive
    uint32_t checksum_offload_rx_ip6 : 1; ///< 1 = IPv6 checksum verification
                                          ///< supported on receive
    uint32_t checksum_offload_rx_udp : 1; ///< 1 = UDP payload checksum verified
                                          ///< on receive
    uint32_t checksum_offload_rx_tcp : 1; ///< 1 = TCP payload checksum verified
                                          ///< on receive
    uint32_t checksum_offload_rx_icmp : 1; ///< 1 = ICMP payload checksum
                                           ///< verified on receive
    uint32_t checksum_offload_tx_ip4  : 1; ///< 1 = IPv4 header checksum
                                           ///< generated on transmit
    uint32_t checksum_offload_tx_ip6  : 1; ///< 1 = IPv6 checksum generation
                                           ///< supported on transmit
    uint32_t checksum_offload_tx_udp  : 1; ///< 1 = UDP payload checksum
                                           ///< generated on transmit
    uint32_t checksum_offload_tx_tcp  : 1; ///< 1 = TCP payload checksum
                                           ///< generated on transmit
    uint32_t checksum_offload_tx_icmp : 1; ///< 1 = ICMP payload checksum
                                           ///< generated on transmit
...
/**
  \brief       This function is used to initialize Ethernet device and related
  resource, an event callback is registered. It is called when the middleware
  component like TCPIP starts operation. \param[in]   cb  callback to handle
  ethernet event \return      return ethernet handle if success
 */
eth_mac_handle_t csi_eth_mac_initialize( eth_event_cb_t cb );

/**
  \brief       This function is used to de-initialize Ethernet device. It is
  called when the middleware component stops operation and releases the software
  resources used by the interface. \param[in]   handle  ethernet handle \return
  error code
 */
...
/**
  ...
  \return      number of data bytes read or execution status
                 ...
                 - value < 0: error occurred, value is execution status as
  defined with execution_status
*/
