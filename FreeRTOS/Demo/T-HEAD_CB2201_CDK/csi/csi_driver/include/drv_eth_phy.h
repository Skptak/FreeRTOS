...
#define CSI_ETH_PHY_API_VERSION \
    CSI_ETH_VERSION_MAJOR_MINOR( 2, 1 ) /* API version */
...
#define CSI_ETH_PHY_SPEED_10M \
    ( CSI_ETH_SPEED_10M << CSI_ETH_PHY_SPEED_Pos ) ///< 10 Mbps link speed
#define CSI_ETH_PHY_SPEED_100M \
    ( CSI_ETH_SPEED_100M << CSI_ETH_PHY_SPEED_Pos ) ///< 100 Mbps link speed
#define CSI_ETH_PHY_SPEED_1G \
    ( CSI_ETH_SPEED_1G << CSI_ETH_PHY_SPEED_Pos ) ///< 1 Gpbs link speed
#define CSI_ETH_PHY_DUPLEX_Pos 2
#define CSI_ETH_PHY_DUPLEX_Msk ( 1UL << CSI_ETH_PHY_DUPLEX_Pos )
#define CSI_ETH_PHY_DUPLEX_HALF \
    ( CSI_ETH_DUPLEX_HALF << CSI_ETH_PHY_DUPLEX_Pos ) ///< Half duplex link
#define CSI_ETH_PHY_DUPLEX_FULL \
    ( CSI_ETH_DUPLEX_FULL << CSI_ETH_PHY_DUPLEX_Pos ) ///< Full duplex link
#define CSI_ETH_PHY_AUTO_NEGOTIATE ( 1UL << 3 )       ///< Auto Negotiation mode
#define CSI_ETH_PHY_LOOPBACK       ( 1UL << 4 )       ///< Loop-back test mode
#define CSI_ETH_PHY_ISOLATE \
    ( 1UL << 5 ) ///< Isolate PHY from MII/RMII interface

typedef int32_t ( *csi_eth_phy_read_t )( uint8_t phy_addr,
                                         uint8_t reg_addr,
                                         uint16_t * data ); ///< Read Ethernet
                                                            ///< PHY Register.
typedef int32_t ( *csi_eth_phy_write_t )( uint8_t phy_addr,
                                          uint8_t reg_addr,
                                          uint16_t data ); ///< Write Ethernet
                                                           ///< PHY Register.
