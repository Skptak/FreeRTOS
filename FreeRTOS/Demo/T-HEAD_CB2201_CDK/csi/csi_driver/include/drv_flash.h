...
typedef struct
{
    flash_sector * sector_info; ///< Sector layout information (NULL=Uniform
                                ///< sectors)
...
typedef void ( *flash_event_cb_t )( flash_event_e event ); ///< Pointer to \ref
                                                           ///< flash_event_cb_t
                                                           ///< : FLASH Event
                                                           ///< call back.
...
/**
  ...
  \param[in]   idx   must not exceed return value of
  drv_flash_get_instance_count() \return      pointer to flash instance
*/
...
/**
  \brief       Initialize FLASH Interface. 1. Initializes the resources needed
  for the FLASH interface 2.registers event callback function \param[in]
  instance  flash instance to operate. \param[in]   cb_event  Pointer to \ref
  flash_event_cb_t \return      error code
*/
int32_t drv_flash_initialize( const csi_flash_t * instance,
                              flash_event_cb_t cb_event );

/**
  \brief       De-initialize FLASH Interface. stops operation and releases the
  software resources used by the interface \param[in]   instance  flash instance
