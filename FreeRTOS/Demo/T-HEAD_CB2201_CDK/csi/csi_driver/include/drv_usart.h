...
typedef struct
{
    ...
    uint32_t tx_underflow : 1; ///< Transmit data underflow detected (cleared on
                               ///< start of next send operation)(Synchronous
                               ///< Slave)
    ...
    uint32_t rx_framing_error : 1; ///< Framing error detected on receive
                                   ///< (cleared on start of next receive
                                   ///< operation)
...
typedef enum
{
    USART_EVENT_SEND_COMPLETE = 0, ///< Send completed; however USART may still
                                   ///< transmit data
    ...
    USART_EVENT_TX_UNDERFLOW = 4, ///< Transmit data not available (Synchronous
                                  ///< Slave)
    ...
    USART_EVENT_RECEIVED = 14,        ///< Received data, but no
                                      ///< send()/receive()/transfer() called
} usart_event_e;

typedef void ( *usart_event_cb_t )( usart_event_e event,
                                    void * cb_arg ); ///< Pointer to \ref
                                                     ///< usart_event_cb_t :
                                                     ///< USART Event call back.
...
typedef struct
{
    ...
    uint32_t event_cts        : 1; ///< Signal CTS change event: \ref
                                   ///< ARM_USART_EVENT_CTS
    uint32_t event_dsr        : 1; ///< Signal DSR change event: \ref
                                   ///< ARM_USART_EVENT_DSR
    uint32_t event_dcd        : 1; ///< Signal DCD change event: \ref
                                   ///< ARM_USART_EVENT_DCD
...
/**
  ...
  for the USART interface 2.registers event callback function \param[in]   tx
  usart pin of tx \param[in]   rx usart pin of rx \param[in]   cb_event  Pointer
  to \ref usart_event_cb_t \param[in]   cb_arg    argument for cb_event \return
...
/**
  ...
  operation completes or error happens. \ref csi_usart_get_status can indicates
...
/**
  \brief       Start receiving data from USART receiver.transmits the default
  value as specified by csi_usart_set_default_tx_value. \n This function is
  non-blocking,\ref usart_event_e is signaled when operation completes or error
  happens. \ref csi_usart_get_status can indicates operation status. \param[in]
  handle  usart handle to operate. \param[out]  data  Pointer to buffer for data
  to receive from USART receiver.data_type is : uint8_t for 5..8 data bits,
  uint16_t for 9 data bits \param[in]   num   Number of data items to receive
  \return      error code
*/
...
/**
  ...
  error happens. \ref csi_usart_get_status can indicates operation status.
  \param[in]   handle  usart handle to operate.
  \param[in]   data_out  Pointer to buffer with data to send to USART
  transmitter.data_type is : uint8_t for 5..8 data bits, uint16_t for 9 data
  bits \param[out]  data_in   Pointer to buffer for data to receive from USART
  receiver.data_type is : uint8_t for 5..8 data bits, uint16_t for 9 data bits
...
/**
  ...
  \param[in]   handle  usart handle to operate.
  \return      1 - a character can be read, 0 if nothing to read ,negative for
  error code
*/
...
/**
  ...
  \param[in]   handle  usart handle to operate.
  \return      1 - a character can be written, 0 -  cannot be written ,negative
  for error code
*/
