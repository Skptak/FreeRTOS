...
typedef enum
{
    ...
    WIFI_EVENT_AP_PROBEREQRECVED,  /**< Receive probe request packet in soft-AP
                                      interface */
    WIFI_EVENT_AP_STA_GOT_IP6,     /**< station or ap interface v6IP addr is
                                      preferred */
...
typedef enum
{
    ...
    WIFI_SECOND_CHAN_ABOVE,    /**< the channel width is HT40 and the second
                                  channel is above the primary channel */
    WIFI_SECOND_CHAN_BELOW,    /**< the channel width is HT40 and the second
...
typedef struct
{
    ...
    uint32_t low_rate_enable : 1;  /**< bit: 0 flag to identify if low rate is
                                      enabled or not */
...
typedef struct
{
    ...
    uint8_t ssid_len; /**< Length of SSID. If softap_config.ssid_len==0, check
                         the SSID until there is a termination character;
                         otherwise, set the SSID length according to
                         softap_config.ssid_len. */
    uint8_t channel;  /**< Channel of soft-AP */
    wifi_auth_mode_t authmode; /**< Auth mode of soft-AP. Do not support
...
#define CSI_WIFI_MAX_CONN_NUM \
    ( 10 ) /**< max number of stations which can connect to soft-AP */
...
typedef enum
{
    WIFI_STORAGE_FLASH, /**< all configuration will strore in both memory and
                           flash */
...
typedef struct
{
    ...
    unsigned          : 16; /**< reserve */
    unsigned mcs : 7;  /**< if is 11n packet, shows the modulation(range from 0
                          to 76) */
    ...
    unsigned fec_coding   : 1; /**< if is 11n packet, shows if is LDPC packet or
                                  not */
    unsigned sgi          : 1; /**< SGI */
...
typedef struct
{
    ...
    uint8_t payload[ 0 ]; /**< ieee80211 packet buff, The length of payload is
                             described by sig_len */
...
typedef enum
{
    WIFI_PKT_CTRL, /**< control type, receive packet buf is wifi_pkt_rx_ctrl_t
                    */
    WIFI_PKT_MGMT, /**< management type, receive packet buf is
                      wifi_promiscuous_pkt_t */
...
// #define CSI_ERR_WIFI_BASE       0x3000 /*!< Starting number of WiFi error
// codes */
...
#define CSI_ERR_WIFI_NOT_SUPPORT \
    CSI_ERR_NOT_SUPPORTED /*!< Indicates that API is not supported yet */

#define CSI_ERR_WIFI_NOT_INIT                                             \
    ( CSI_DRV_ERRNO_WIFI_BASE + 1 ) /*!< WiFi driver was not installed by \
                                       csi_wifi_init */
#define CSI_ERR_WIFI_NOT_STARTED                                        \
    ( CSI_DRV_ERRNO_WIFI_BASE + 2 ) /*!< WiFi driver was not started by \
                                       csi_wifi_start */
#define CSI_ERR_WIFI_IF \
    ( CSI_DRV_ERRNO_WIFI_BASE + 3 ) /*!< WiFi interface error */
#define CSI_ERR_WIFI_MODE \
    ( CSI_DRV_ERRNO_WIFI_BASE + 4 ) /*!< WiFi mode error */
#define CSI_ERR_WIFI_STATE \
    ( CSI_DRV_ERRNO_WIFI_BASE + 5 ) /*!< WiFi internal state error */
#define CSI_ERR_WIFI_CONN                                               \
    ( CSI_DRV_ERRNO_WIFI_BASE + 6 ) /*!< WiFi internal control block of \
                                       station or soft-AP error */
#define CSI_ERR_WIFI_NVS \
    ( CSI_DRV_ERRNO_WIFI_BASE + 7 ) /*!< WiFi internal NVS module error */
#define CSI_ERR_WIFI_MAC \
    ( CSI_DRV_ERRNO_WIFI_BASE + 8 ) /*!< MAC address is invalid */
#define CSI_ERR_WIFI_SSID \
    ( CSI_DRV_ERRNO_WIFI_BASE + 9 ) /*!< SSID is invalid */
#define CSI_ERR_WIFI_PASSWORD \
    ( CSI_DRV_ERRNO_WIFI_BASE + 10 ) /*!< Password is invalid */
#define CSI_ERR_WIFI_TIMEOUT \
    ( CSI_DRV_ERRNO_WIFI_BASE + 11 ) /*!< Timeout error */
#define CSI_ERR_WIFI_WAKE_FAIL                                              \
    ( CSI_DRV_ERRNO_WIFI_BASE + 12 ) /*!< WiFi is in sleep state(RF closed) \
                                        and wakeup fail */

typedef enum
{
    FRAME_FILTER_MODE_FORWARD = 1, /*!< Packet filter engine forwards matching
                                      packets, discards non-matching packets */
    FRAME_FILTER_MODE_DISCARD = 0, /*!< Packet filter engine discards matching
                                      packets, forwards non-matching packets */
} frame_filter_mode_t;

typedef enum
{
    FRAME_FILTER_RULE_POSITIVE_MATCHING = 0, /*!< Specifies that a filter should
                                                match a given pattern     */
    FRAME_FILTER_RULE_NEGATIVE_MATCHING = 1, /*!< Specifies that a filter should
...
typedef struct
{
    ...
    frame_filter_rule_t rule; /*!< Filter matches are either POSITIVE or
                                 NEGATIVE matching */
    uint16_t offset; /*!< Offset in bytes to start filtering (referenced to the
                        start of the ethernet packet) */
    uint16_t mask_size; /*!< Size of the mask in bytes */
    uint8_t * mask;     /*!< Pattern mask bytes to be ANDed with the pattern eg.
                           "\xff00" (must be in network byte order) */
    uint8_t * pattern;  /*!< Pattern bytes used to filter eg. "\x0800"  (must be
                           in network byte order) */
    bool enabled_status; /*!< When returned from mhd_get_packet_filters,
...
typedef void ( *wifi_event_cb_t )( uint32_t event ); ///< Signal WiFi Event.
                                                     ///< Events refer to
                                                     ///< wifi_event_id_t

/**
 * @brief  Init WiFi
 *         Alloc resource for WiFi driver, such as WiFi control structure, RX/TX
 ...
 *
 * @attention 1. This API should be called if you want to remove WiFi driver
 * from the system
 *
 ...
 *
 *            Set the WiFi operating mode as station, soft-AP or
 * station+soft-AP, The default mode is soft-AP mode.
 *
 ...
 *         If mode is WIFI_MODE_STA, it create station control block and start
 * station If mode is WIFI_MODE_AP, it create soft-AP control block and start
 * soft-AP If mode is WIFI_MODE_APSTA, it create soft-AP and station control
 * block and start soft-AP and station
 *
 * @return
 *    - CSI_OK: succeed
 *    - CSI_ERR_WIFI_NOT_INIT: WiFi is not initialized by csi_wifi_init
 *    - CSI_ERR_WIFI_ARG: invalid argument
 *    - CSI_ERR_WIFI_NO_MEM: out of memory
 *    - CSI_ERR_WIFI_CONN: WiFi internal error, station or soft-AP control block
 * wrong
...
/**
 * @brief  Stop WiFi
 *         If mode is WIFI_MODE_STA, it stop station and free station control
 * block If mode is WIFI_MODE_AP, it stop soft-AP and free soft-AP control block
 *         If mode is WIFI_MODE_APSTA, it stop station/soft-AP and free
 ...
 *
 * @param     aid  when aid is 0, deauthenticate all stations, otherwise
 * deauthenticate station whose associated id is aid
 *
 * @return
 *    - CSI_OK: succeed
 *    - CSI_ERR_WIFI_NOT_INIT: WiFi is not initialized by csi_wifi_init
 ...
 *
 * @attention If this API is called, the found APs are stored in WiFi driver
 * dynamic allocated memory and that will be freed in csi_wifi_get_ap_list, so
 * generally, call csi_wifi_get_ap_list to cause the memory to be freed once the
 * scan is done
 * @attention The values of maximum active scan time and passive scan time per
 * channel are limited to 1500 milliseconds. Values above 1500ms may cause
 * station to disconnect from AP and are not recommended.
 *
 * @param     config  configuration of scanning
 * @param     block if block is true, this API will block the caller until the
 * scan is done, otherwise it will return immediately
 *
 * @return
 *    - CSI_OK: succeed
 *    - CSI_ERR_WIFI_NOT_INIT: WiFi is not initialized by csi_wifi_init
 ...
 *
 * @param[inout]  number As input param, it stores max AP number ap_records can
 ...
 *
 * @param     ifx  interface
 * @param[out] protocol_bitmap  store current WiFi protocol bitmap of interface
 * ifx
 *
 * @return
 *    - CSI_OK: succeed
 ...
 *
 * @attention 1. API return false if try to configure an interface that is not
 * enabled
 ...
 * @attention 2. This API should be called after csi_wifi_start() or
 * csi_wifi_set_promiscuous()
 *
 * @param     primary  for HT20, primary is the channel number, for HT40,
 * primary is the primary channel
 * @param     second   for HT20, second is ignored, for HT40, second is the
 * second channel
 *
 * @return
 *    - CSI_OK: succeed
 ...
 * @attention 2. soft-AP and station have different MAC addresses, do not set
 * them to be the same.
 * @attention 3. The bit 0 of the first byte of MAC address can not be 1. For
 * example, the MAC address can set to be "1a:XX:XX:XX:XX:XX", but can not be
 * "15:XX:XX:XX:XX:XX".
 *
 * @param     ifx  interface
 ...
 *
 * Each time a packet is received, the registered callback function will be
 * called.
 *
 * @param cb  callback
 *
 ...
 *
 * @attention 1. This API can be called only when specified interface is
 * enabled, otherwise, API fail
 * @attention 2. For station configuration, bssid_set needs to be 0; and it
 * needs to be 1 only when users need to check the MAC address of the AP.
 * @attention 3. Limited to only one channel, so when in the soft-AP+station
 ...
 *    - CSI_ERR_WIFI_ARG: invalid argument
 *    - CSI_ERR_WIFI_MODE: WiFi mode is wrong
 *    - CSI_ERR_WIFI_CONN: WiFi internal error, the station/soft-AP control
 * block is invalid
 */
 ...
 *    - CSI_OK: succeed
 *    - CSI_ERR_WIFI_NOT_INIT: WiFi is not initialized by csi_wifi_init
 *    - CSI_ERR_WIFI_MODE: WiFi internal error, the station/soft-AP control
 * block is invalid
 *    - others: refer to error code in csi_err.h
 */
 ...
 *
 *            Each time the WiFi need to forward the packets to high layer, the
 * callback function will be called. ebuf is rx buffer allocated by wifi driver
 */
 ...
 * TX/RX is disabled.| \b 1                          | ON, the Wi-Fi radio is
 * turned on, and Wi-Fi TX/RX is enabled.|
 *
 *
 * @return  >=0 the operation completed successfully, <0 the operation failed.
 *
 * @note In repeater mode, both Wi-Fi interface and Wi-Fi radio are turned
 * on/off at the same time.
 */
int32_t wifi_config_get_radio( uint8_t * on_off );

/**
 * @brief This function sets the radio status of the Wi-Fi driver. This
...
/**
 * @brief This function gets the RSSI of the connected AP. It's only used for
 * the STA mode and while the station is connected to the AP.
 *
