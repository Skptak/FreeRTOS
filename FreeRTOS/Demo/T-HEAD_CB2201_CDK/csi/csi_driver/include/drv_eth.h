...
#define CSI_ETH_VERSION_MAJOR_MINOR( major, minor ) \
    ( ( ( major ) << 8 ) | ( minor ) )
...
#define CSI_ETH_ERROR_UNSUPPORTED \
    CSI_DRV_ERRNO_ETH_BASE + 4 ///< Operation not supported
#define CSI_ETH_ERROR_PARAMETER CSI_DRV_ERRNO_ETH_BASE + 5 ///< Parameter error
#define CSI_ETH_ERROR_SPECIFIC \
    CSI_DRV_ERRNO_ETH_BASE + 6 ///< Start of driver specific errors
...
typedef enum eth_power_state
{
    ...
    CSI_ETH_POWER_LOW, ///< Low Power mode: retain state, detect and signal
                       ///< wake-up events
...
#define CSI_ETH_INTERFACE_RMII \
    ( 1 ) ///< Reduced Media Independent Interface (RMII)
#define CSI_ETH_INTERFACE_SMII \
    ( 2 ) ///< Serial Media Independent Interface (SMII)
...
typedef volatile struct eth_link_info
{
    ...
    uint32_t Autonegotiation : 1; ///< Set the interface to Auto Negotiation
                                  ///< mode of transmission parameters
    uint32_t Loopback  : 1; ///< Set the interface into a Loop-back test mode
    uint32_t Isolation : 1; ///< Set to indicate electrical isolation of PHY
                            ///< interface from MII/RMII interface
