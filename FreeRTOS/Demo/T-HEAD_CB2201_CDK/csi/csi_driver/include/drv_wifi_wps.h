...
#define CSI_ERR_WIFI_REGISTRAR \
    ( CSI_DRV_ERRNO_WIFI_BASE + 51 ) /*!< WPS registrar is not supported */
#define CSI_ERR_WIFI_WPS_TYPE \
    ( CSI_DRV_ERRNO_WIFI_BASE + 52 ) /*!< WPS type error */
#define CSI_ERR_WIFI_WPS_SM                                                    \
    ( CSI_DRV_ERRNO_WIFI_BASE + 53 ) /*!< WPS state machine is not initialized \
                                      */
 ...
 *
 * @param     wps_type : WPS type, so far only WPS_TYPE_PBC and WPS_TYPE_PIN is
 * supported
 *
 * @return
 *          - CSI_OK : succeed
 *          - CSI_ERR_WIFI_WPS_TYPE : wps type is invalid
 *          - CSI_ERR_WIFI_WPS_MODE : wifi is not in station mode or sniffer
 * mode is on
