...
typedef enum
{
    ...
    DMA_STATE_READY, ///< DMA process success and ready for use, but not start
                     ///< yet
...
typedef void ( *dma_event_cb_t )( dma_event_e event,
                                  int32_t ch ); ///< Pointer to \ref
                                                ///< dma_event_cb_t : CRC Event
                                                ///< call back.
...
/**
  \brief       Initialize DMA Interface. 1. Initializes the resources needed for
  the DMA interface 2.registers event callback function \param[in]   idx must
  not exceed return value of csi_dma_get_instance_count() \return      pointer
  to dma instances
*/
dmac_handle_t csi_dma_initialize( int32_t idx );

/**
  \brief       De-initialize DMA Interface. stops operation and releases the
  software resources used by the interface \param[in]   handle damc handle to
...
/**
  \brief
  \param[in]    handle damc handle to operate.
  \param[in]    ch          channel num. if -1 then allocate a free channal in
  this dma \param[in]    psrcaddr    dma transfer source address \param[in]
  pstdaddr    dma transfer source address \param[in]    length      dma transfer
  length \param[in]    config      dma transfer configure \param[in]    cb_event
  Pointer to \ref dma_event_cb_t \return       error code
 */
