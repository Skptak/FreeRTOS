...
/**
  ...
  \details Enables a secure device-specific interrupt in the NVIC interrupt
  controller. \param [in]      IRQn  External interrupt number. Value cannot be
  negative.
 */
...
/**
  ...
  \details Reads the wake up register in the NVIC and returns the pending bit
  for the specified interrupt. \param [in]      IRQn  Interrupt number. \return
...
{
    NVIC->IPTR = 0x80000000 | ( ( ( VectThreshold + 32 ) & 0xFF ) << 8 ) |
                 ( ( PrioThreshold & 0x3 ) << 6 );
}
...
{
    return ( (
        uint32_t ) ( ( ( NVIC->IPR[ _IP_IDX( IRQn ) ] >> _BIT_SHIFT( IRQn ) ) &
                       ( uint32_t ) 0xFFUL ) >>
                     ( 8U - s_nvic_prio_bits ) ) );
}
...
uint32_t drv_coret_config( uint32_t ticks, int32_t IRQn )
{
    ...
    // drv_nvic_set_prio(IRQn, (1UL << s_nvic_prio_bits) - 1UL);  fix me  /* set
