 ...
 ===============================================================================
    [..]
        ...
        (#) Optionally you can enable/configure:
            ...
            (++) LCD Pulse ON Duration using the LCD_PulseOnDurationConfig()
 function.
            (++) LCD Dead Time using the LCD_DeadTimeConfig() function
            (++) The LCD Blink mode and frequency using the LCD_BlinkConfig()
 function.
...
#define CR_LCDEN_BB \
    ( PERIPH_BB_BASE + ( CR_OFFSET * 32 ) + ( LCDEN_BitNumber * 4 ) )

/* Alias word address of MUX_SEG bit */
#define MUX_SEG_BitNumber 0x07
#define CR_MUX_SEG_BB \
    ( PERIPH_BB_BASE + ( CR_OFFSET * 32 ) + ( MUX_SEG_BitNumber * 4 ) )
...
#define FCR_HD_BB \
    ( PERIPH_BB_BASE + ( FCR_OFFSET * 32 ) + ( HD_BitNumber * 4 ) )
...
#define SR_UDR_BB \
    ( PERIPH_BB_BASE + ( SR_OFFSET * 32 ) + ( UDR_BitNumber * 4 ) )
...
void LCD_Init( LCD_InitTypeDef * LCD_InitStruct )
{
    ...
    LCD->FCR |= ( uint32_t ) ( LCD_InitStruct->LCD_Prescaler |
                               LCD_InitStruct->LCD_Divider );

    LCD_WaitForSynchro();

    LCD->CR &= ( uint32_t ) CR_MASK;
    LCD->CR |= ( uint32_t ) ( LCD_InitStruct->LCD_Duty |
                              LCD_InitStruct->LCD_Bias |
...
void LCD_StructInit( LCD_InitTypeDef * LCD_InitStruct )
{
     ...
     * -----------------*/
    LCD_InitStruct->LCD_Prescaler = LCD_Prescaler_1; /*!< Initialize the
                                                        LCD_Prescaler member */

    LCD_InitStruct->LCD_Divider = LCD_Divider_16; /*!< Initialize the
                                                     LCD_Divider member */

    LCD_InitStruct->LCD_Duty = LCD_Duty_Static; /*!< Initialize the LCD_Duty
                                                   member */
    ...
    LCD_InitStruct
        ->LCD_VoltageSource = LCD_VoltageSource_Internal; /*!<
                                                             Initialize
                                                             the
                                                             LCD_VoltageSource
                                                             member */
...
/**
 * @brief  Waits until the LCD FCR register is synchronized in the LCDCLK
 * domain. This function must be called after any write operation to LCD_FCR
 * register.
 * @param  None
 * @retval None
 */
...
@verbatim
 ...
 ===============================================================================
    [..] Using its double buffer memory the LCD controller ensures the coherency
         of the displayed information without having to use interrupts to
control LCD_RAM modification.

    [..] The application software can access the first buffer level (LCD_RAM)
through the APB interface. Once it has modified the LCD_RAM, it sets the UDR
flag in the LCD_SR register using the LCD_UpdateDisplayRequest() function.
    ...
    [..] This operation is done synchronously with the frame (at the beginning
of the next frame), until the update is completed, the LCD_RAM is write
         protected and the UDR flag stays high.

    [..] Once the update is completed another flag (UDD - Update Display Done)
is set and generates an interrupt if the UDDIE bit in the LCD_FCR register is
set.
 ...
 * @param  LCD_IT: specifies the LCD interrupts sources to be enabled or
...
ITStatus LCD_GetITStatus( uint32_t LCD_IT )
{
    ...
    if( ( ( LCD->FCR & LCD_IT ) != ( uint16_t ) RESET ) &&
