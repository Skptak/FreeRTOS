...
#define CR_MSION_BB \
    ( PERIPH_BB_BASE + ( CR_OFFSET * 32 ) + ( MSION_BitNumber * 4 ) )
...
#define CSR_LSECSSON_BB \
    ( PERIPH_BB_BASE + ( CSR_OFFSET * 32 ) + ( LSECSSON_BitNumber * 4 ) )

/* Alias word address of RTCEN bit */
#define RTCEN_BitNumber 0x16
#define CSR_RTCEN_BB \
    ( PERIPH_BB_BASE + ( CSR_OFFSET * 32 ) + ( RTCEN_BitNumber * 4 ) )

/* Alias word address of RTCRST bit */
#define RTCRST_BitNumber 0x17
#define CSR_RTCRST_BB \
    ( PERIPH_BB_BASE + ( CSR_OFFSET * 32 ) + ( RTCRST_BitNumber * 4 ) )
...
source.
         ...
         (#) CSS (Clock security system), once enable and if a HSE clock failure
             ...
             The interrupt is linked to the Cortex-M3 NMI (Non-Maskable
Interrupt) exception vector.
         (#) MCO (microcontroller clock output), used to output SYSCLK, HSI,
MSI, HSE, PLL, LSI or LSE clock (through a configurable prescaler) on PA8 pin.
 ...
 *            by this function, it keep the value configured by user
 * application)
 ...
 * constant HSE_STARTUP_TIMEOUT in stm32l1xx.h file. You can tailor it depending
 ...
 *     @arg RCC_MSIRange_5: MSI clock is around 2.097 MHz (default after Reset
 ...
 * PLLVCO clock This parameter can be:
 ...
 * @note   The application software must set correctly the output division to
 * avoid exceeding 32 MHz as SYSCLK.
 *
 * @retval None
 */
                    ...
                    uint8_t RCC_PLLDiv )
{
    ...
    *( __IO uint8_t * ) CFGR_BYTE3_ADDRESS =
        ( uint8_t ) ( RCC_PLLSource |
                      ( ( uint8_t ) ( RCC_PLLMul |
                                      ( uint8_t ) ( RCC_PLLDiv ) ) ) );
}
 ...
 * @note     The PLL is disabled by hardware when entering STOP and STANDBY
 * modes.
...
(SYSCLK): MSI, HSI, HSE and PLL. The AHB clock (HCLK) is derived from System
clock through configurable prescaler and used to clock the CPU, memory and
peripherals mapped on AHB bus (DMA and GPIO).APB1 (PCLK1) and APB2 (PCLK2)
...
(SYSCLK) except:
             ...
             (+@) The ADC clock which is always the HSI clock. A divider by 1, 2
                  or 4 allows to adapt the clock frequency to the device
operating conditions.
         ...
         (#) The maximum frequency of the SYSCLK, HCLK, PCLK1 and PCLK2 is 32
...
adapted accordingly:
    ...
    [..] It is recommended to use the following software sequences to tune the
         ...
         (+) Program the Flash 64-bit access, using
"FLASH_ReadAccess64Cmd(ENABLE)" function
...
reading the clock source status, using "RCC_GetSYSCLKSource()" function
         ...
         (+) Check that the new number of WS is taken into account by reading
             FLASH_ACR
         (+) Enable the Flash 32-bit access, using
"FLASH_ReadAccess64Cmd(DISABLE)" function
...
{
    uint32_t tmp = 0, pllmul = 0, plldiv = 0, pllsource = 0, presc = 0,
             msirange = 0;
    ...
    switch( tmp )
    {
        case 0x00: /* MSI used as system clock */
            ...
            RCC_Clocks->SYSCLK_Frequency = ( 32768 *
                                             ( 1 << ( msirange + 1 ) ) );
            break;
        ...
        case 0x0C: /* PLL used as system clock */
            ...
            if( pllsource == 0x00 )
            {
                ...
                RCC_Clocks->SYSCLK_Frequency = ( ( ( HSI_VALUE ) *pllmul ) /
                                                 plldiv );
            }
            else
            {
                /* HSE selected as PLL clock source */
                RCC_Clocks->SYSCLK_Frequency = ( ( ( HSE_VALUE ) *pllmul ) /
                                                 plldiv );
            }
            break;
...
are off except internal SRAM, Flash and JTAG. Before to start using a peripheral
...
functions.
         ...
         (#) To further reduce power consumption in SLEEP mode the peripheral
             clocks can be disabled prior to executing the WFI or WFE
instructions. You can do this using RCC_AHBPeriphClockLPModeCmd(),
 ...
 * write access using PWR_RTCAccessCmd(ENABLE) function before to configure the
 * RTC clock source (to be done once after reset).
 * @note     Once the RTC clock is configured it can't be changed unless the RTC
 *           is reset using RCC_RTCResetCmd function, or by a Power On Reset
 * (POR)
...
void RCC_RTCCLKConfig( uint32_t RCC_RTCCLKSource )
{
    ...
    {
        /* If HSE is selected as RTC clock source, configure HSE division factor
         * for RTC clock */
        tmpreg = RCC->CR;
 ...
 *     @arg RCC_AHBPeriph_FLITF: (has effect only when the Flash memory is in
 * power down mode)
 ...
 *           power consumption.
 *         - After wakeup from SLEEP mode, the peripheral clock is enabled
 * again.
