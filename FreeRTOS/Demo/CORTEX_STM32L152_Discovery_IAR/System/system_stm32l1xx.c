 ...
 *          This file contains the system clock configuration for STM32L1xx
 *Ultra Low Medium-density devices, and is generated by the clock configuration
 ...
 *main program. This call is made inside the "startup_stm32l1xx_md.s" file.
 *
 ...
 *
 * 2. After each device reset the MSI (2.1 MHz Range) is used as system clock
 *source. Then SystemInit() function is called, in "startup_stm32l1xx_md.s"
 ...
 *SystemInit() function will do nothing and MSI still used as system clock
 *source. User can add some code to deal with this issue inside the
 *SetSysClock() function.
 *
 * 4. The default value of HSE crystal is set to 8MHz, refer to "HSE_VALUE"
 *define in "stm32l1xx.h" file. When HSE is used as system clock source,
