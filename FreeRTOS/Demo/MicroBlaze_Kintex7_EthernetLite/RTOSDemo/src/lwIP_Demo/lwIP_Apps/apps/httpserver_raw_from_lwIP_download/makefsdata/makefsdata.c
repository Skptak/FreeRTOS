...
/* Compatibility defines Win32 vs. DOS */
    ...
    #define FIND_T_IS_DIR( fInfo ) \
        ( ( fInfo.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) != 0 )
    ...
    #define FIND_T_IS_DIR( fInfo ) \
        ( ( fInfo.ff_attrib & FA_DIREC ) == FA_DIREC )
...
static char file_buffer_c[ COPY_BUFSIZE * 5 +
                           ( ( COPY_BUFSIZE / HEX_BYTES_PER_LINE ) * 3 ) ];
...
int main( int argc, char * argv[] )
{
    ...
    {
        /* if no subdir named 'fs' (or the one which was given) exists, spout
         * usage verbiage */
        printf( " Failed to open directory \"%s\"." NEWLINE NEWLINE, path );
        printf( " Usage: htmlgen [targetdir] [-s] [-i] [-f:<filename>]" NEWLINE
                    NEWLINE );
        printf( "   targetdir: relative or absolute path to files to "
                "convert" NEWLINE );
        printf( "   switch -s: toggle processing of subdirectories (default is "
                "on)" NEWLINE );
        printf( "   switch -e: exclude HTTP header from file (header is "
                "created at runtime, default is off)" NEWLINE );
        printf(
            "   switch -11: include HTTP 1.1 header (1.0 is default)" NEWLINE );
        printf( "   switch -c: precalculate checksums for all pages (default "
                "is off)" NEWLINE );
        printf(
    ...
    fprintf( data_file,
             "#define file_NULL (struct fsdata_file *) NULL" NEWLINE NEWLINE
                 NEWLINE );
    ...
    fprintf( struct_file,
             "#define FS_NUMFILES %d" NEWLINE NEWLINE,
             filesProcessed );
    ...
    printf( NEWLINE "Processed %d files - done." NEWLINE NEWLINE,
            filesProcessed );
    ...
    if( processSubs )
    {
        ...
        if( FINDFIRST_SUCCEEDED( fret ) )
        {
            do
            {
                ...
                printf( NEWLINE "processing subdirectory %s/..." NEWLINE,
                        curSubdir );
                filesProcessed += process_sub( data_file, struct_file );
                CHDIR( ".." );
                     ...
                     u16_t hdr_chksum )
{
    ...
    fprintf( struct_file,
             "const struct fsdata_chksum chksums_%s[] = {" NEWLINE,
             varname );
...
int process_file( FILE * data_file, FILE * struct_file, const char * filename )
{
...
#endif /* ALIGN_PAYLOAD */
    fprintf( data_file,
             "static const unsigned char data_%s[] = {" NEWLINE,
             varname );
    ...
    if( precalcChksum )
    {
        chksum_count = write_checksums( struct_file,
                                        filename,
                                        varname,
                                        http_hdr_len,
                                        http_hdr_chksum );
    ...
    fprintf( struct_file,
             "const struct fsdata_file file_%s[] = { {" NEWLINE,
             varname );
    ...
    i = 0;
    fprintf( data_file,
             NEWLINE "/* raw file data (%d bytes) */" NEWLINE,
             file_size );
                            ...
                            u16_t * http_hdr_chksum )
{
    ...
    else
    {
        ...
        {
            printf( "failed to get file type for extension \"%s\", using "
                    "default.\n",
                    file_ext );
    ...
    if( precalcChksum )
    {
        ...
        LWIP_ASSERT( "strlen(hdr_buf) == hdr_len",
                     strlen( hdr_buf ) == hdr_len );
