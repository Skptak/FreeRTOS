 ...
 * Device(s)    : RX111
 * Description  : Defines post-reset routines that are used to configure the MCU
 *prior to the main program starting. This is were the program counter starts on
 *power-up or reset.
 ***********************************************************************************************************************/
/***********************************************************************************************************************
...
/* This macro is here so that the stack will be declared here. This is used to
 * prevent multiplication of stack size. */
...
***********************************************************************************************************************/
/* Declare the contents of the function 'Change_PSW_PM_to_UserMode' as assembler
 * to the compiler */
 ...
 *                1. The User Stack Pointer (USP) and Interrupt Stack Pointer
 *(ISP) are both set immediately after entry to this function. The USP and ISP
 *stack sizes are set in the file stacksct.h. Default sizes are USP=4K and
 *ISP=1K.
 ...
 *                4. The MCU operating frequency is set by configuring the Clock
 *Generation Circuit (CGC) in operating_frequency_set.
 *                5. Calls are made to functions to setup the C runtime
 *environment which involves initializing all initialed data, zeroing all
 *uninitialized variables, and configuring STDIO if used (calls to _INITSCT and
 *_INIT_IOLIB).
 ...
 *Program Status Word (PSW), and the stack is switched from the ISP to the USP.
 *The initial Interrupt Priority Level is set to zero, enabling any interrupts
 *with a priority greater than zero to be serviced.
 *                8. The processor is optionally switched to user mode.  To run
 *in user mode, set the macro RUN_IN_USER_MODE above to a 1.
 *                9. The bus error interrupt is enabled to catch any accesses to
 *invalid or reserved areas of memory.
 *
 *                Once this initialization is complete, the user's main()
 *function is called.  It should not return. Arguments    : none Return value :
 *none
 ***********************************************************************************************************************/
#pragma section ResetPRG // output PowerON_Reset to PResetPRG section
void PowerON_Reset_PC( void )
{
    /* Stack pointers are setup prior to calling this function - see comments
     * above */
    ...
    /* Enable the bus error interrupt to catch accesses to illegal/reserved
     * areas of memory */
    /* The ISR for this interrupt can be found in vecttbl.c in the function
     * "bus_error_isr" */
    ...
    while( 1 )
    {
        /* Infinite loop. Put a breakpoint here if you want to catch an exit of
         * main(). */
    }
}
 ...
 *supervisor to user. Arguments    : none Return value : none
 ***********************************************************************************************************************/
#if RUN_IN_USER_MODE == 1
    #if __RENESAS_VERSION__ < 0x01010000
static void Change_PSW_PM_to_UserMode( void )
{
    MVFC PSW, R1 OR #00100000h, R1 PUSH.L R1 MVFC PC, R1 ADD #10,
        R1 PUSH.L R1 RTE NOP NOP
}
    #endif
#endif
