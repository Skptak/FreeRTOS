 ...
 *         : 08.03.2012 1.20    Added GetVersion() function (though it's really
 *a macro). : 04.06.2012 1.30    Code can now be interrupt or poll driven. :
...
/* These macros define which IRQ pins are used for the switches. Note that these
   defintions cannot have parentheses around them. */
 ...
 *                    The times per second that the user will call
 *R_SWITCHES_Update(). NOTE: this is only when using polling mode. If you are
 *using interrupt mode, then this argument will be ignored. debouce_counts - The
 *number of times to check the port value before accepting the change. The
 *slower the rate at which R_SWITCHES_Update() will likely lower this number.
 * Return Value : none
 ***********************************************************************************************************************/
void R_SWITCHES_Init( uint32_t detection_hz, uint32_t debounce_counts )
{
    ...
    /* The SW#_XXX defintions are common macros amongst different boards. To see
       the definitions for these macros see the board defintion file. For
       example, this file for the RSKRX63N is rskrx63n.h. */

#if defined( MCU_RX62N ) || defined( MCU_RX62T ) || defined( MCU_RX621 ) || \
    defined( MCU_RX610 )
    ...
    defined( MCU_RX210 ) || defined( MCU_RX111 )
    ...
    */
    ...
    IOPORT.PF8IRQ.BIT.ITS15 = 0; /* IRQ15-A pin is used. */
        ...
        /* The switches on the RDKRX62N are connected to the following
        pins/IRQ's Switch  Port    IRQ
        ------  ----    ----
        SW1     P4.0    IRQ8
        SW2     P4.1    IRQ9
        SW3     P4.2    IRQ10
        */
        ...
        /* Nothing else needed to do here since RDK has 100-pin package and
         * there are no alternate pins to choose. */
...
/* Only define interrupts in interrupt detection mode. */
         ...
         * Description  : Sample ISR for switch 1 input (must do hardware setup
         *first!) Arguments    : none Return value : none
         ***********************************************************************************************************************/
...
static void sw1_isr( void )
{
    ...
    SW1_CALLBACK_FUNCTION();
         ...
         * Description  : Sample ISR for switch 2 input (must do hardware setup
         *first!) Arguments    : none Return value : none
         ***********************************************************************************************************************/
...
static void sw2_isr( void )
{
    ...
    SW2_CALLBACK_FUNCTION();
         ...
         * Description  : Sample ISR for switch 3 input (must do hardware setup
         *first!) Arguments    : none Return value : none
         ***********************************************************************************************************************/
 ...
 * Description  : Polls switches and calls callback functions as needed. If you
 *are using IRQ mode then this function is not needed and can be removed if
 *desired. It is left in so that code will not fail when switching between
 *polling or IRQ mode. Arguments    : none Return value : none
 ***********************************************************************************************************************/
void R_SWITCHES_Update( void )
{
    ...
    if( SW1 == SW_ACTIVE )
    {
        if( g_switches[ 0 ].active != true )
        {
            ...
            {
                /* Set this to true so we only call the callback function once
                 * per press. */
