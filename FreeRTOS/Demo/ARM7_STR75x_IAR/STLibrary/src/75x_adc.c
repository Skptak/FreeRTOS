...
void ADC_Init( ADC_InitTypeDef * ADC_InitStruct )
{
    ...
    /* Set conversion prescaler value (sampling and conversion prescalers are
       equal while SPEN bit is reset */
     ...
     */
    if( ADC_InitStruct->ADC_SamplingPrescaler !=
 ...
 * Description    : Starts the ADC Calibration. Calibration average
 *enabled/disabled. Input          : - ADC_CalibAverage: Enables or disables ADC
 *calibration average. This parameter can be one of the following values:
 *                         - ADC_CalibAverage_Enable:  enable calibration
 *average
 *                         - ADC_CalibAverage_Disable: disable calibration
 *average Output         : None Return         : None
 *******************************************************************************/
 ...
 * Description    : Configures the analog input channel to be used for the
 *selected Analog Watchdog and defines its corresponding High and Low threshold
 *values. Input          : - ADC_AnalogWatchdog: specifies the analog watchdog
 *which will be affected to the desired converted channel. This parameter can be
 ...
 *                    analog watchdog. This parameter can be ADC_CHANNELx where
 *x can be (0..15)
 *                  - LowThreshold: Low Threshold for the selected Analog
 *watchdog
 *                  - HighThreshold: High Threshold for the selected Analog
 *watchdog Output         : None Return         : None
 *******************************************************************************/
                               ...
                               u16 HighThreshold )
{
    ...
    {
        /* Set the selected channel and their corresponding High and Low
         * thresholds */
        case ADC_AnalogWatchdog0:
            ADC->TRA0 = ( ADC->TRA0 & ADC_AnalogWatchdogChannel_Mask ) |
                        ( ( u16 ) ADC_CHANNEL << 10 );
        ...
        case ADC_AnalogWatchdog1:
            ADC->TRA1 = ( ADC->TRA1 & ADC_AnalogWatchdogChannel_Mask ) |
                        ( ( u16 ) ADC_CHANNEL << 10 );
        ...
        case ADC_AnalogWatchdog2:
            ADC->TRA2 = ( ADC->TRA2 & ADC_AnalogWatchdogChannel_Mask ) |
                        ( ( u16 ) ADC_CHANNEL << 10 );
        ...
        case ADC_AnalogWatchdog3:
            ADC->TRA3 = ( ADC->TRA3 & ADC_AnalogWatchdogChannel_Mask ) |
                        ( ( u16 ) ADC_CHANNEL << 10 );
 ...
 * Description    : Returns the comparison result of the selected analog
 *watchdog. Input          : - ADC_AnalogWatchdog: specifies the analog watchdog
 *channel which its comparison result will be returned. This parameter can be
 *one of the following values:
 *                     - ADC_AnalogWatchdog0: select analog watchdog 0
 *                     - ADC_AnalogWatchdog1: select analog watchdog 1
 *                     - ADC_AnalogWatchdog2: select analog watchdog 2
...
u16 ADC_GetAnalogWatchdogResult( u16 ADC_AnalogWatchdog )
{
    ...
    switch( ADC_AnalogWatchdog )
    {
        ...
        default:
            return ( 0xFF ); /* if a wrong value of ADC_AnalogWatchdog is
                                selected */
    }
}
 ...
 *                    This parameter can be one of the following values:
 *                         - ADC_Injec_ExtTrigger_Disable : external trigger
 *disabled
 ...
 *                    converted.
 *                    This parameter can be ADC_CHANNELx  where x can be
 *(0..15).
 *                  - ChannelNumber: specifies the Number of the injected
 *channels to be converted. This parameter can be a  value from 1 to 16. Output
 *: None Return         : None
 *******************************************************************************/
                                   ...
                                   u8 ChannelNumber )
{
    ...
    switch( ADC_Injec_ExtTrigger )
    {
        case ADC_Injec_ExtTrigger_Disable:
            /* Disable the external trigger and start the injected conversion by
             * software */
            ADC->CLR3 &= ADC_Injec_ExtTrigger_Disable;
            break;
        case ADC_Injec_ExtTrigger_RisingEdge:
            /* Start injected conversion on rising edge of the external trigger
             * (PWM) */
            ADC->CLR3 |= ADC_Injec_ExtTrigger_RisingEdge;
            break;
        case ADC_Injec_ExtTrigger_FallingEdge:
            /* Start injected conversion on falling edge of the external trigger
             * (PWM) */
 ...
 * Description    : Starts by software the conversion of the injected input
 *channels. Input          : None Output         : None Return         : None
 *******************************************************************************/
 ...
 * Description    : Reads the conversion result from the appropriate data
 *register. Input          : - ADC_CHANNEL :specifies the ADC channel which its
 *conversion value have to be returned. This parameter can be ADC_CHANNELx where
 *x can be (0..15) to select channelx Output         : None Return         : The
 *returned value holds the conversion result of the selected channel.
 *******************************************************************************/
u16 ADC_GetConversionValue( u8 ADC_CHANNEL )
{
    /* Return the conversion result of the selected channel */
    return *( ( u16 * ) ( ADC_BASE +
                          ( ( ADC_CHANNEL << 2 ) + ADC_DataRegisterOffset ) ) );
}

/*******************************************************************************
 * Function Name  : ADC_ITConfig
 * Description    : Enables or disables the specified ADC interrupts.
 * Input          : - ADC_IT: specifies the ADC interrupts to be enabled or
 ...
 *                         - ADC_IT_JECH: Injected end of chain conversion
 *interrupt
 *                         - ADC_IT_JEOC: Injected end of channel conversion
 *interrupt
 *                         - ADC_IT_AnalogWatchdog0_LowThreshold:
 *                                        Analog Watchdog 0 LowThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog0_HighThreshold:
 *                                        Analog Watchdog 0 HighThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog1_LowThreshold:
 *                                        Analog Watchdog 1 LowThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog1_HighThreshold:
 *                                        Analog Watchdog 1 HighThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog2_LowThreshold:
 *                                        Analog Watchdog 2 LowThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog2_HighThreshold:
 *                                        Analog Watchdog 2 HighThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog3_LowThreshold:
 *                                        Analog Watchdog 3 LowThreshold
 *interrupt
 *                         - ADC_IT_AnalogWatchdog3_HighThreshold:
 *                                         Analog Watchdog 5 HighThreshold
 *interrupt
 ...
 *                           enable bit
 *                         - ADC_DMA_ExtTrigger_HighLevel: enabled by detection
 *of high level of TIM2 OC2 signal
 *                         - ADC_DMA_ExtTrigger_LowLevel: enabled by detection
 *of low level of TIM2 OC2 signal Output         : None Return         : None
 *******************************************************************************/
 ...
 * Description    : Gets the first DMA-enabled channel configured at the time
 *that DMA was last globally enabled. Input          : None Output         :
 *None Return         : The first DMA enabled channel
 *******************************************************************************/
 ...
 *                         - ADC_FLAG_ECH:  End of chain conversion flag
 *                         - ADC_FLAG_EOC:  End of channel conversion flag
 *                         - ADC_FLAG_JECH: Injected end of chain conversion
 *flag
 *                         - ADC_FLAG_JEOC: Injected end of channel conversion
 *flag
 *                         - ADC_FLAG_AnalogWatchdog0_LowThreshold:
 *                                         Analog Watchdog 0 LowThreshold flag
 *                         - ADC_FLAG_AnalogWatchdog0_HighThreshold:
 ...
 * Description    : Checks whether the specified ADC interrupt has occured or
 *not. Input          : - ADC_IT: specifies the ADC interrupt source to check.
 *This parameter can be one of the following values:
 *                         - ADC_IT_ECH :End of chain conversion interrupt
 *                         - ADC_IT_EOC :End of channel conversion interrupt
 *                         - ADC_IT_JECH :End of injected chain conversion
 *interrupt
 *                         - ADC_IT_JEOC :End of injected channel conversion
 *interrupt
 *                         - ADC_IT_AnalogWatchdog0_LowThreshold:
 *                                         Analog Watchdog 0 LowThreshold
 *interrupt
 ...
 *interrupt
 *                         - ADC_IT_AnalogWatchdog3_HighThreshold:
 *                                         Analog Watchdog 3 HighThreshold
 *interrupt Output         : None Return         : The new state of the ADC_IT
 *(SET or RESET).
 *******************************************************************************/
 ...
 *                         - ADC_IT_AnalogWatchdog3_HighThreshold:
 *                                         Analog Watchdog 5 HighThreshold
 *interrupt Output         : None Return         : None
 *******************************************************************************/
