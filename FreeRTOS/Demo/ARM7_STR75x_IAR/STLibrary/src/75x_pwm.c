 ...
 * Description    : Deinitializes PWM peripheral registers to their default
 *reset values. Input          : None Output         : None Return         :
 *None
 *******************************************************************************/
 ...
 * Input          : PWM_InitStruct : pointer to a PWM_InitTypeDef structure
 ...
 *                         - PWM_Channel_ALL: PWM Channel 1, Channel 2 and 3 are
 *used
 ...
 *                         - PWM_CounterMode_CenterAligned1: PWM Center Aligned1
 *Mode
 *                         - PWM_CounterMode_CenterAligned2: PWM Center Aligned2
 *Mode
 *                         - PWM_CounterMode_CenterAligned3: PWM Center Aligned3
 ...
 *                         - PWM_Channel_ALL: PWM Channel 1, Channel 2 and 3 are
 *used
 *                  - PWM_ForcedAction: specifies the forced Action to be set to
 *the output waveform. This parameter can be one of the following values:
 *                         - PWM_ForcedAction_Active: Force active level on
 *OCxREF
 ...
 *                         - PWM_TRGOMode_Update: The Update event is used as
 *TRGO
...
static void OCM_ModuleConfig( PWM_InitTypeDef * PWM_InitStruct )
{
    ...
    if( PWM_InitStruct->PWM_Channel == PWM_Channel_1 )
    {
        ...
        if( PWM_InitStruct->PWM_Complementary == PWM_Complementary_Enable )
        {
            ...
            PWM->OMR1 &= PWM_OC1C_Mask;
            PWM->OMR1 |= PWM_OCControl | PWM_OC1_Enable | PWM_OC1N_Enable |
                         PWM_PLD1_Set;
    ...
    if( PWM_InitStruct->PWM_Channel == PWM_Channel_2 )
    {
        ...
        if( PWM_InitStruct->PWM_Complementary == PWM_Complementary_Enable )
        {
            ...
            PWM->OMR1 &= PWM_OC2C_Mask;
            PWM->OMR1 |= ( PWM_OCControl << 8 ) | PWM_OC2_Enable |
                         PWM_OC2N_Enable | PWM_PLD2_Set;
        ...
        /* Single PWM Output configuratuion */
        {
            ...
            switch( PWM_InitStruct->PWM_OCNState )
            {
                case PWM_OCNState_Enable:
                {
                    ...
                    PWM->OMR1 &= PWM_OC2C_Mask;
                    PWM->OMR1 |= ( PWM_OCControl << 8 ) | PWM_OC2N_Enable |
                                 PWM_PLD2_Set;
                    PWM->OCR2 = PWM_InitStruct->PWM_Pulse2;
    ...
    if( PWM_InitStruct->PWM_Channel == PWM_Channel_3 )
    {
        ...
        if( PWM_InitStruct->PWM_Complementary == PWM_Complementary_Enable )
        {
            ...
            PWM->OMR2 &= PWM_OC3C_Mask;
            PWM->OMR2 |= PWM_OCControl | PWM_OC3_Enable | PWM_OC3N_Enable |
                         PWM_PLD3_Set;
    ...
    {
        DTR_REG |= PWM_InitStruct->PWM_LOCKLevel |
                   PWM_InitStruct->PWM_Emergency |
