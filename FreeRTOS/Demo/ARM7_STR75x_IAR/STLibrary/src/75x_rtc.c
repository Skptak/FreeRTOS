 ...
 * Description    : Waits until last write operation on RTC registers has
 *finished. This function must be called before any write to RTC registers.
 * Input          : None
 * Output         : None
 * Return         : None
 *******************************************************************************/
 ...
 *                          is cleared only after an APB reset or an APB Clock
 *stop.
 *                        - RTC_FLAG_Overflow: Overflow interrupt flag
 *                        - RTC_FLAG_Alarm: Alarm interrupt flag
 *                        - RTC_FLAG_Second: Second interrupt flag
 ...
 * Description    : Checks whether the specified RTC interrupt has occured or
 *not. Input          : RTC_IT: specifies the RTC interrupts sources to check.
 *                   This parameter can be a combination of one or more of
 *                   the following values:
 *                       - RTC_IT_Overflow: Overflow interrupt
 *                       - RTC_IT_Alarm: Alarm interrupt
...
{
    if( ( ( RTC->CRH & RTC_IT ) != RESET ) &&
        ( ( RTC->CRL & RTC_IT ) != RESET ) )
    {
        return SET;
    }
    else
