...
#include "em_usart.h"
    ...
    /** Validation of USART register block pointer reference for assert
     * statements. */
                             ...
                             USART_OVS_TypeDef ovs )
{
    ...
    switch( ovs )
    {
         ...
         * divisor up by half the divisor before the division in order to reduce
         * the integer division error, which consequently results in a higher
         * baudrate than desired. */
    #if defined( _USART_CLKDIV_DIV_MASK ) && \
        ( _USART_CLKDIV_DIV_MASK >= 0x7FFFF8UL )
...
void USART_InitIrDA( const USART_InitIrDA_TypeDef * init )
{
    ...
    /* Configure IrDA */
    usart->IRCTRL |= ( uint32_t ) init->irPw | ( uint32_t ) init->irPrsSel |
                     ( ( uint32_t ) init->irFilt
                       << _USART_IRCTRL_IRFILT_SHIFT ) |
                     ( ( uint32_t ) init->irPrsEn
