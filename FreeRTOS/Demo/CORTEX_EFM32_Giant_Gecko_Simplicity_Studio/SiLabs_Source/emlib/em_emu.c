...
static void emuRestore( void )
{
    ...
    oscEnCmd = 0;
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_HFRCOENS ) ? CMU_OSCENCMD_HFRCOEN
                                                      : 0 );
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_AUXHFRCOENS )
                      ? CMU_OSCENCMD_AUXHFRCOEN
                      : 0 );
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_LFRCOENS ) ? CMU_OSCENCMD_LFRCOEN
                                                      : 0 );
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_HFXOENS ) ? CMU_OSCENCMD_HFXOEN
                                                     : 0 );
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_LFXOENS ) ? CMU_OSCENCMD_LFXOEN
                                                     : 0 );
    #if defined( _CMU_STATUS_USHFRCOENS_MASK )
    oscEnCmd |= ( ( cmuStatus & CMU_STATUS_USHFRCOENS ) ? CMU_OSCENCMD_USHFRCOEN
                                                        : 0 );
    #endif
    ...
    switch( cmuHfclkStatus & _CMU_HFCLKSTATUS_SELECTED_MASK )
    {
        case CMU_HFCLKSTATUS_SELECTED_LFRCO:
             ...
             * enabled, otherwise the HFXO would be started and selected
             * automatically. Note: this error hook helps catching erroneous
             * oscillator configurations, when the AUTOSTARTSELEM0EM1 is set in
             * CMU_HFXOCTRL. */
...
void dcdcFetCntSet( bool lpModeSet )
{
    ...
    if( lpModeSet )
    {
        ...
        tmp = EMU->DCDCMISCCTRL & ~( _EMU_DCDCMISCCTRL_PFETCNT_MASK |
...
void dcdcHsFixLnBlock( void )
{
    ...
    {
        /* Wait for LNRUNNING */
        if( ( EMU->DCDCCTRL & ~_EMU_DCDCCTRL_DCDCMODE_MASK ) ==
...
void EMU_EnterEM2( bool restore )
{
    ...
    /* Fix for errata EMU_E107 - restore state of non-WIC interrupt enable
     * flags. */
    #if defined( ERRATA_FIX_EMU_E107_EN )
    if( errataFixEmuE107En )
    {
...
void EMU_EM4Init( EMU_EM4Init_TypeDef * em4Init )
{
    ...
    em4conf &= ~( _EMU_EM4CONF_LOCKCONF_MASK | _EMU_EM4CONF_OSC_MASK |
                  _EMU_EM4CONF_BURTCWU_MASK | _EMU_EM4CONF_VREGEN_MASK );
    ...
    em4ctrl &= ~( _EMU_EM4CTRL_RETAINLFXO_MASK | _EMU_EM4CTRL_RETAINLFRCO_MASK |
                  _EMU_EM4CTRL_RETAINULFRCO_MASK | _EMU_EM4CTRL_EM4STATE_MASK |
...
void EMU_BUPDInit( EMU_BUPDInit_TypeDef * bupdInit )
{
    ...
    reg = EMU->PWRCONF &
          ~( _EMU_PWRCONF_PWRRES_MASK | _EMU_PWRCONF_VOUTSTRONG_MASK |
             _EMU_PWRCONF_VOUTMED_MASK | _EMU_PWRCONF_VOUTWEAK_MASK );
    ...
    reg = EMU->BUCTRL & ~( _EMU_BUCTRL_PROBE_MASK | _EMU_BUCTRL_BODCAL_MASK |
                           _EMU_BUCTRL_STATEN_MASK | _EMU_BUCTRL_EN_MASK );

    /* Note use of ->enable to both enable BUPD, use BU_VIN pin input and
       release reset */
    reg |= bupdInit->probe | ( bupdInit->bodCal << _EMU_BUCTRL_BODCAL_SHIFT ) |
...
void EMU_BUThresholdSet( EMU_BODMode_TypeDef mode, uint32_t value )
{
    EFM_ASSERT( value < 8 );
    EFM_ASSERT( value <=
                ( _EMU_BUACT_BUEXTHRES_MASK >> _EMU_BUACT_BUEXTHRES_SHIFT ) );
...
void EMU_BUThresRangeSet( EMU_BODMode_TypeDef mode, uint32_t value )
{
    EFM_ASSERT( value < 4 );
    EFM_ASSERT( value <=
                ( _EMU_BUACT_BUEXRANGE_MASK >> _EMU_BUACT_BUEXRANGE_SHIFT ) );
...
static bool ConstCalibrationLoad( void )
{
    ...
    volatile uint32_t * const
        diCal_EMU_DCDCLNFREQCTRL = ( volatile uint32_t * ) ( 0x0FE08038 );
    volatile uint32_t * const
        diCal_EMU_DCDCLNVCTRL = ( volatile uint32_t * ) ( 0x0FE08040 );
    volatile uint32_t * const
        diCal_EMU_DCDCLPCTRL = ( volatile uint32_t * ) ( 0x0FE08048 );
    volatile uint32_t * const
        diCal_EMU_DCDCLPVCTRL = ( volatile uint32_t * ) ( 0x0FE08050 );
    volatile uint32_t * const
        diCal_EMU_DCDCTRIM0 = ( volatile uint32_t * ) ( 0x0FE08058 );
    volatile uint32_t * const
        diCal_EMU_DCDCTRIM1 = ( volatile uint32_t * ) ( 0x0FE08060 );
...
void ValidatedConfigSet( void )
{
    ...
    EMU->DCDCLNFREQCTRL = ( EMU->DCDCLNFREQCTRL &
...
static void maxCurrentUpdate( void )
{
    ...
    EMU->DCDCMISCCTRL = ( EMU->DCDCMISCCTRL &
...
static bool LpCmpHystCalibrationLoad( bool lpAttenuation, uint32_t lpCmpBias )
{
    ...
    EMU->DCDCLPCTRL = ( EMU->DCDCLPCTRL & ~_EMU_DCDCLPCTRL_LPCMPHYSSEL_MASK ) |
                      lpcmpHystSel;
...
void EMU_DCDCModeSet( EMU_DcdcMode_TypeDef dcdcMode )
{
    ...
    BUS_RegBitWrite( &EMU->DCDCCLIMCTRL,
                     ...
                     dcdcMode == emuDcdcMode_Bypass ? 0 : 1 );
...
bool EMU_DCDCInit( EMU_DCDCInit_TypeDef * dcdcInit )
{
    ...
    /* Set EM0 DCDC operating mode. Output voltage set in
       EMU_DCDCOutputVoltageSet() above takes effect if mode is changed from
       bypass here. */
                               ...
                               bool setLnVoltage )
{
    ...
    if( ( EMU->PWRCFG & _EMU_PWRCFG_PWRCFG_MASK ) ==
    ...
    /* Populate both LP and LN registers, set control reg pointer and VREF
     * shift. */
    for( lnMode = 0; lnMode <= 1; lnMode++ )
    {
        ...
        if( attSet )
        {
            mVlow = 1800;
            mVhigh = 3000;
            attMask = ( lnMode ? EMU_DCDCLNVCTRL_LNATT
        ...
        /* Get 2-point calib data from DEVINFO, calculate trimming and set
         * voltege */
        if( lnMode )
        {
        ...
        else
        {
            ...
            if( !( LpCmpHystCalibrationLoad(
                    attSet,
                    lpcmpBias >> _EMU_DCDCMISCCTRL_LPCMPBIAS_SHIFT ) ) )
            {
                EFM_ASSERT( false );
                /* Return when assertions are disabled */
                return false;
        ...
        vrefVal = ( ( mV - mVlow ) * ( vrefHigh - vrefLow ) ) /
                  ( mVhigh - mVlow );
        vrefVal += vrefLow;
...
void EMU_DCDCOptimizeSlice( uint32_t mAEm0LoadCurrent )
{
    ...
    else if( ( !( EMU->DCDCMISCCTRL & _EMU_DCDCMISCCTRL_LNFORCECCM_MASK ) ) &&
             ( rcoBand <= EMU_DcdcLnRcoBand_4MHz ) )
    {
    ...
    {
        /* This configuration is not recommended. EMU_DCDCInit() applies a
    ...
    /* Update current limit configuration as it depends on the slice
     * configuration. */
    maxCurrentUpdate();
}
...
vmonMilliVoltToFineThreshold( int mV, uint32_t coarseThreshold )
{
...
void EMU_VmonInit( EMU_VmonInit_TypeDef * vmonInit )
{
    ...
    EFM_ASSERT( ( vmonInit->threshold >= 1200 ) &&
                ( vmonInit->threshold <= 3980 ) );

    thresholdCoarse = vmonMilliVoltToCoarseThreshold( vmonInit->threshold );
    thresholdFine = vmonMilliVoltToFineThreshold( vmonInit->threshold,
                                                  thresholdCoarse );

    switch( vmonInit->channel )
    {
        case emuVmonChannel_AVDD:
            EMU->VMONAVDDCTRL = ( thresholdCoarse
                                ...
                                ( vmonInit->riseWakeup ? EMU_VMONAVDDCTRL_RISEWU
                                                       : 0 ) |
                                ( vmonInit->fallWakeup ? EMU_VMONAVDDCTRL_FALLWU
                                                       : 0 ) |
                                ( vmonInit->enable ? EMU_VMONAVDDCTRL_EN : 0 );
            break;
        case emuVmonChannel_ALTAVDD:
            EMU->VMONALTAVDDCTRL = ( thresholdCoarse
                                   ...
                                   ( vmonInit->fallWakeup
                                         ...
                                         : 0 ) |
                                   ( vmonInit->enable ? EMU_VMONALTAVDDCTRL_EN
                                                      : 0 );
            break;
        case emuVmonChannel_DVDD:
            EMU->VMONDVDDCTRL = ( thresholdCoarse
                                  << _EMU_VMONDVDDCTRL_THRESCOARSE_SHIFT ) |
                                ( thresholdFine
                                  << _EMU_VMONDVDDCTRL_THRESFINE_SHIFT ) |
                                ( vmonInit->riseWakeup ? EMU_VMONDVDDCTRL_RISEWU
                                                       : 0 ) |
                                ( vmonInit->fallWakeup ? EMU_VMONDVDDCTRL_FALLWU
                                                       : 0 ) |
                                ( vmonInit->enable ? EMU_VMONDVDDCTRL_EN : 0 );
            break;
        case emuVmonChannel_IOVDD0:
            EMU->VMONIO0CTRL = ( thresholdCoarse
                                 << _EMU_VMONIO0CTRL_THRESCOARSE_SHIFT ) |
                               ( thresholdFine
                                 << _EMU_VMONIO0CTRL_THRESFINE_SHIFT ) |
                               ( vmonInit->retDisable ? EMU_VMONIO0CTRL_RETDIS
                                                      : 0 ) |
                               ( vmonInit->riseWakeup ? EMU_VMONIO0CTRL_RISEWU
                                                      : 0 ) |
                               ( vmonInit->fallWakeup ? EMU_VMONIO0CTRL_FALLWU
                                                      : 0 ) |
...
{
    uint32_t riseThresholdCoarse, riseThresholdFine, fallThresholdCoarse,
        fallThresholdFine;
    /* VMON supports voltages between 1200 mV and 3980 mV (inclusive) in 20 mV
     * increments */
    EFM_ASSERT( ( vmonInit->riseThreshold >= 1200 ) &&
                ( vmonInit->riseThreshold < 4000 ) );
    EFM_ASSERT( ( vmonInit->fallThreshold >= 1200 ) &&
                ( vmonInit->fallThreshold < 4000 ) );
    /* Fall threshold has to be lower than rise threshold */
    EFM_ASSERT( vmonInit->fallThreshold <= vmonInit->riseThreshold );

    riseThresholdCoarse = vmonMilliVoltToCoarseThreshold(
        vmonInit->riseThreshold );
    riseThresholdFine = vmonMilliVoltToFineThreshold( vmonInit->riseThreshold,
                                                      riseThresholdCoarse );
    fallThresholdCoarse = vmonMilliVoltToCoarseThreshold(
        vmonInit->fallThreshold );
    fallThresholdFine = vmonMilliVoltToFineThreshold( vmonInit->fallThreshold,
                                                      fallThresholdCoarse );

    switch( vmonInit->channel )
    {
        case emuVmonChannel_AVDD:
            EMU->VMONAVDDCTRL = ( riseThresholdCoarse
                                  << _EMU_VMONAVDDCTRL_RISETHRESCOARSE_SHIFT ) |
                                ( riseThresholdFine
                                  << _EMU_VMONAVDDCTRL_RISETHRESFINE_SHIFT ) |
                                ( fallThresholdCoarse
