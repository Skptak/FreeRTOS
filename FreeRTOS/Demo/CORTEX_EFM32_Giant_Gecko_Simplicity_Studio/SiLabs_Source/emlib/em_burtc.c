...
__STATIC_INLINE void regSync( uint32_t mask )
{
    ...
    if( ( BURTC->FREEZE & BURTC_FREEZE_REGFREEZE ) ||
        ( ( BURTC->CTRL & _BURTC_CTRL_CLKSEL_MASK ) !=
          _BURTC_CTRL_CLKSEL_NONE ) )
    {
        return;
    }
...
void BURTC_Init( const BURTC_Init_TypeDef * burtcInit )
{
    ...
    /* New configuration */
    ctrl = ( BURTC_CTRL_RSTEN | ( burtcInit->mode ) |
             ...
             ( presc << _BURTC_CTRL_PRESC_SHIFT ) | ( burtcInit->clkSel ) |
...
uint32_t BURTC_ClockFreqGet( void )
{
    ...
    clkDiv = ( BURTC->CTRL & _BURTC_CTRL_PRESC_MASK ) >>
             _BURTC_CTRL_PRESC_SHIFT;
    ...
    switch( clkSel )
    {
        ...
        case BURTC_CTRL_CLKSEL_ULFRCO:
            ...
            {
                frequency = SystemULFRCOClockGet(); /* 1KHz when divisor is
                                                       different from 1. */
            }
            break;
