...
void EBI_Init( const EBI_Init_TypeDef * ebiInit )
{
    ...
    {
        ctrl &= ~( _EBI_CTRL_MODE_MASK | _EBI_CTRL_ARDYEN_MASK |
                   _EBI_CTRL_ARDYTODIS_MASK | _EBI_CTRL_BL_MASK |
                   _EBI_CTRL_NOIDLE_MASK | _EBI_CTRL_BANK0EN_MASK );
    ...
    {
        ctrl &= ~( _EBI_CTRL_BL1_MASK | _EBI_CTRL_MODE1_MASK |
                   _EBI_CTRL_ARDY1EN_MASK | _EBI_CTRL_ARDYTO1DIS_MASK |
                   _EBI_CTRL_NOIDLE1_MASK | _EBI_CTRL_BANK1EN_MASK );
    ...
    {
        ctrl &= ~( _EBI_CTRL_BL2_MASK | _EBI_CTRL_MODE2_MASK |
                   _EBI_CTRL_ARDY2EN_MASK | _EBI_CTRL_ARDYTO2DIS_MASK |
                   _EBI_CTRL_NOIDLE2_MASK | _EBI_CTRL_BANK2EN_MASK );
    ...
    {
        ctrl &= ~( _EBI_CTRL_BL3_MASK | _EBI_CTRL_MODE3_MASK |
                   _EBI_CTRL_ARDY3EN_MASK | _EBI_CTRL_ARDYTO3DIS_MASK |
                   _EBI_CTRL_NOIDLE3_MASK | _EBI_CTRL_BANK3EN_MASK );
    ...
    #else
    ctrl &= ~( _EBI_CTRL_MODE_MASK | _EBI_CTRL_ARDYEN_MASK |
               _EBI_CTRL_ARDYTODIS_MASK | _EBI_CTRL_BANK0EN_MASK |
               _EBI_CTRL_BANK1EN_MASK | _EBI_CTRL_BANK2EN_MASK |
    ...
    EBI_BankAddressTimingConfig( ebiInit->banks, ebiInit->addrHalfALE );
    #else
    ...
    EBI_WriteTimingSet( ebiInit->writeSetupCycles,
                        ebiInit->writeStrobeCycles,
                        ebiInit->writeHoldCycles );
    EBI_AddressTimingSet( ebiInit->addrSetupCycles, ebiInit->addrHoldCycles );
    #endif
    ...
    if( ctrl & ( _EBI_CTRL_BL_MASK | _EBI_CTRL_BL1_MASK | _EBI_CTRL_BL2_MASK |
                 _EBI_CTRL_BL3_MASK ) )
    {
        BUS_RegBitWrite( &( EBI->ROUTE ),
...
void EBI_PolaritySet( EBI_Line_TypeDef line, EBI_Polarity_TypeDef polarity )
{
    switch( line )
    {
        ...
        case ebiLineTFTVSync:
            BUS_RegBitWrite( &( EBI->TFTPOLARITY ),
...
void EBI_ReadTimingSet( int setupCycles, int strobeCycles, int holdCycles )
{
    ...
    EBI->RDTIMING = ( EBI->RDTIMING & ~( _EBI_RDTIMING_RDSETUP_MASK |
...
void EBI_WriteTimingSet( int setupCycles, int strobeCycles, int holdCycles )
{
    ...
    EBI->WRTIMING = ( EBI->WRTIMING & ~( _EBI_WRTIMING_WRSETUP_MASK |
...
void EBI_AddressTimingSet( int setupCycles, int holdCycles )
{
    ...
    EBI->ADDRTIMING = ( EBI->ADDRTIMING & ~( _EBI_ADDRTIMING_ADDRSETUP_MASK |
...
void EBI_TFTInit( const EBI_TFTInit_TypeDef * ebiTFTInit )
{
    ...
    EBI_TFTSizeSet( ebiTFTInit->hsize, ebiTFTInit->vsize );
                               ...
                               bool halfRE )
{
    ...
    EFM_ASSERT(
        ( banks & ~( EBI_BANK0 | EBI_BANK1 | EBI_BANK2 | EBI_BANK3 ) ) == 0 );

    /* Configure read operation parameters */
    if( banks & EBI_BANK0 )
    {
        BUS_RegBitWrite( &EBI->RDTIMING,
                            ...
                            int holdCycles )
{
    ...
    if( banks & EBI_BANK1 )
    {
        EBI->RDTIMING1 = ( EBI->RDTIMING1 & ~( _EBI_RDTIMING1_RDSETUP_MASK |
    ...
    if( banks & EBI_BANK2 )
    {
        EBI->RDTIMING2 = ( EBI->RDTIMING2 & ~( _EBI_RDTIMING2_RDSETUP_MASK |
    ...
    if( banks & EBI_BANK3 )
    {
        EBI->RDTIMING3 = ( EBI->RDTIMING3 & ~( _EBI_RDTIMING3_RDSETUP_MASK |
                             ...
                             int holdCycles )
{
    ...
    if( banks & EBI_BANK1 )
    {
        EBI->WRTIMING1 = ( EBI->WRTIMING1 & ~( _EBI_WRTIMING1_WRSETUP_MASK |
    ...
    if( banks & EBI_BANK2 )
    {
        EBI->WRTIMING2 = ( EBI->WRTIMING2 & ~( _EBI_WRTIMING2_WRSETUP_MASK |
    ...
    if( banks & EBI_BANK3 )
    {
        EBI->WRTIMING3 = ( EBI->WRTIMING3 & ~( _EBI_WRTIMING3_WRSETUP_MASK |
...
void EBI_BankAddressTimingConfig( uint32_t banks, bool halfALE )
{
    ...
    if( banks & EBI_BANK0 )
    {
        BUS_RegBitWrite( &EBI->ADDRTIMING,
                         _EBI_ADDRTIMING_HALFALE_SHIFT,
                         halfALE );
    }
    if( banks & EBI_BANK1 )
    {
        BUS_RegBitWrite( &EBI->ADDRTIMING1,
                         _EBI_ADDRTIMING_HALFALE_SHIFT,
                         halfALE );
    }
    if( banks & EBI_BANK2 )
    {
        BUS_RegBitWrite( &EBI->ADDRTIMING2,
                         _EBI_ADDRTIMING_HALFALE_SHIFT,
                         halfALE );
    }
    if( banks & EBI_BANK3 )
    {
        BUS_RegBitWrite( &EBI->ADDRTIMING3,
                         _EBI_ADDRTIMING_HALFALE_SHIFT,
                         halfALE );
    }
}
