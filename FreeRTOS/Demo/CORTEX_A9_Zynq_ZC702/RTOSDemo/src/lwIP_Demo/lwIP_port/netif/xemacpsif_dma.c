...
#define XEMACPS_BD_TO_INDEX( ringptr, bdptr )               \
    ( ( ( u32 ) bdptr - ( u32 ) ( ringptr )->BaseBdAddr ) / \
      ( ringptr )->Separation )
...
void emacps_send_handler( void * arg )
{
    ...
    obtaining the mutex fails. */
    {
        ...
        xTimerPendFunctionCallFromISR( vPendableSendCompleteFunction,
                                       TxRingPtr,
                                       0,
...
XStatus emacps_sgsend( xemacpsif_s * xemacpsif, struct pbuf * p )
{
    ...
    for( q = p, txbd = txbdset; q != NULL; q = q->next )
    {
        ...
        /* Send the data from the pbuf to the interface, one pbuf at a
           ...
           variable. */
        Xil_DCacheFlushRange( ( unsigned int ) q->payload,
                              ( unsigned ) q->len );

        XEmacPs_BdSetAddressTx( txbd, ( u32 ) q->payload );
        if( q->len > ( XEMACPS_MAX_FRAME_SIZE - 18 ) )
            XEmacPs_BdSetLength( txbd,
                                 ( XEMACPS_MAX_FRAME_SIZE - 18 ) & 0x3FFF );
        else
    ...
    XEmacPs_WriteReg( ( xemacpsif->emacps ).Config.BaseAddress,
                      ...
                      ( XEmacPs_ReadReg( ( xemacpsif->emacps )
                                             .Config.BaseAddress,
                                         XEMACPS_NWCTRL_OFFSET ) |
...
void setup_rx_bds( XEmacPs_BdRing * rxring )
{
    ...
    while( FreeBds > 0 )
    {
        ...
        *Temp = 0;
        ...
        {
            LWIP_DEBUGF( NETIF_DEBUG,
                         ( "Error committing RxBD to hardware: " ) );
            if( Status == XST_DMA_SG_LIST_ERROR )
                LWIP_DEBUGF( NETIF_DEBUG,
                             ( "XST_DMA_SG_LIST_ERROR: this function was "
                               "called out of sequence with "
                               "XEmacPs_BdRingAlloc()\r\n" ) );
            else
            {
                LWIP_DEBUGF( NETIF_DEBUG,
                             ( "set of BDs was rejected because the first BD "
                               "did not have its start-of-packet bit set, or "
                               "the last BD did not have its end-of-packet bit "
                               "set, or any one of the BD set has 0 as length "
                               "value\r\n" ) );
            }
            return;
        }
...
void emacps_recv_handler( void * arg )
{
    ...
    while( 1 )
    {
        bd_processed = XEmacPs_BdRingFromHwRx( rxring,
...
void clean_dma_txdescs( struct xemac_s * xemac )
{
     ...
     */
    XEmacPs_BdRingCreate( TxRingPtr,
                          ( u32 ) xemacpsif->tx_bdspace,
                          ( u32 ) xemacpsif->tx_bdspace,
                          BD_ALIGNMENT,
                          XLWIP_CONFIG_N_TX_DESC );
...
XStatus init_dma( struct xemac_s * xemac )
{
     ...
     */
    char * endAdd_aligned = ( char * ) ( ( ( int ) endAdd + 0x100000 ) &
                                         ( ~0xFFFFF ) );
    xemacpsif_s * xemacpsif = ( xemacpsif_s * ) ( xemac->state );
    ...
    {
        xil_printf( "%s@%d: Error: Unable to allocate memory for TX/RX buffer "
                    "descriptors",
                    __FILE__,
                    __LINE__ );
        return XST_FAILURE;
    ...
    XScuGic_EnableIntr( INTC_DIST_BASE_ADDR,
                        ( u32 ) xtopologyp->scugic_emac_intr );
 ...
 *
 * It is called at regular intervals through the API
 * xemacpsif_resetrx_on_no_rxdata called by the user. The EmacPs has a HW bug
 * (SI# 692601) on the Rx path for heavy Rx traffic. Under heavy Rx traffic
 * because of the HW bug there are times when the Rx path becomes unresponsive.
 * The workaround for it is to check for the Rx path for traffic (by reading the
 * stats registers regularly). If the stats register does not increment for
 * sometime (proving no Rx traffic), the function resets the Rx data path.
 *
 */
...
void resetrx_on_no_rxdata( xemacpsif_s * xemacpsif )
{
    ...
    tempcntr = XEmacPs_ReadReg( xemacpsif->emacps.Config.BaseAddress,
