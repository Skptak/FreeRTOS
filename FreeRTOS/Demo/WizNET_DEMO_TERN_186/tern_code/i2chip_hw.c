...
void i2chip_init( void )
{
    ...
    */
    ...
    #ifdef TERN_5E
    pokeb( MMCR, _GPCSDW_, peekb( MMCR, _GPCSDW_ ) & 0xf7 ); // set /CS3-/CSM
                                                             // Data Width=8
    pokeb( MMCR, _CSPFS_, peekb( MMCR, _CSPFS_ ) | 0x08 ); // set the GP CS3 PIN
                                                           // Function
    poke( MMCR, _PAR15_, 0x2000 );                         // set CS3 I/O region
    poke( MMCR, _PAR15_ + 2, 0x2dff ); // set CS3 I/O region, 512 bytes

    pokeb( MMCR, _GPCSDW_, peekb( MMCR, _GPCSDW_ ) & 0x7f ); // CS7=J4.3 Data
                                                             // Width=8, /CSI
    //	pokeb(MMCR,	_GPCSDW_, peekb(MMCR, _GPCSDW_)|0x80); // CS7=J4.3 Data
    // Width=16
    pokeb( MMCR, _CSPFS_, peekb( MMCR, _CSPFS_ ) | 0x80 ); // set the GP CS7 PIN
                                                           // Function
    ...
    poke( MMCR,
          _PIOPFS31_16_,
          peek( MMCR, _PIOPFS31_16_ ) | 0x0800 ); // P27=/CS0
    ...
    outport( 0xfff0, inport( 0xfff0 ) | 0x4000 ); // SYSCON, MCS0
                                                  // 0x80000-0xbffff
...
/*
//	Function: spu_2_isr
//	P20=Slave2 PIC IR7, Master IR5, interrupt vector=0x57, GPIRQ7=PIO16 GP
timer1
*/
