...
*/
#define _CGC_DSSEL0_SLAVE \
    ( 0x00000000UL ) /* Supplied from outside the LSI (slave operation) */
#define _CGC_DSSEL0_MASTER \
    ( 0x00000001UL ) /* Supplied from CGC (master operation) */
...
#define _CGC_DSCLK0_POL_NORMAL \
    ( 0x00000000UL ) /* Polarity not inverted (master and slave operation) */
#define _CGC_DSCLK0_POL_INVERT \
    ( 0x00000010UL ) /* Polarity inverted (master and slave operation) */
#define _CGC_DSCLK0_SLAVE_MCLK0_2 \
    ( 0x00000000UL ) /* Clock input to MCLK0,MCLK1,MCLK2 pins are used */
#define _CGC_DSCLK0_SLAVE_MCLK0 \
    ( 0x00000020UL ) /* Clock input to MCLK0 pin is used */
#define _CGC_DSSEL1_SLAVE \
    ( 0x00000000UL ) /* Supplied from outside the LSI (slave operation) */
#define _CGC_DSSEL1_MASTER \
    ( 0x00010000UL ) /* Supplied from CGC (master operation) */
...
#define _CGC_DSCLK1_POL_NORMAL \
    ( 0x00000000UL ) /* Polarity not inverted (master and slave operation) */
#define _CGC_DSCLK1_POL_INVERT \
    ( 0x00100000UL ) /* Polarity inverted (master and slave operation) */
...
#define _CGC_OSC_STOP_DET_INT_DISABLE \
    ( 0x00000000UL ) /* Stop detection interrupt is disabled */
#define _CGC_OSC_STOP_DET_INT_ENABLE \
    ( 0x00000001UL ) /* Stop detection interrupt is enabled */
/* Oscillation Stop Detection Function Enable (OSTDE) */
#define _CGC_OSC_STOP_DET_DISABLE \
    ( 0x00000000UL ) /* Oscillation stop detection function is disabled */
#define _CGC_OSC_STOP_DET_ENABLE \
    ( 0x00000080UL ) /* Oscillation stop detection function is enabled */

/*
    ECM Non-maskable Interrupt Configuration Register 0 (ECMNMICFG0)
*/
#define _ECM_NMI_OSC_STOP_DISABLE \
    ( 0x00000000UL ) /* Stop detection NMI interrupt is disabled */
#define _ECM_NMI_OSC_STOP_ENABLE \
    ( 0x00080000UL ) /* Stop detection NMI interrupt is enabled */

/*
    ECM Maskable Interrupt Configuration Register 0 (ECMMICFG0)
*/
#define _ECM_MI_OSC_STOP_DISABLE \
    ( 0x00000000UL ) /* Stop detection Maskable interrupt is disabled */
#define _ECM_MI_OSC_STOP_ENABLE \
    ( 0x00080000UL ) /* Stop detection Maskable interrupt is enabled */
...
#define _SWV_SEL_TDO \
    ( 0x00000001UL ) /* SWV output is output from the TDO pin */
#define _SWV_SEL_TRACEDATA0 \
    ( 0x00000002UL ) /* SWV output is output from the TRACEDATA0 pin */
#define _SWV_SEL_TRACECTL \
    ( 0x00000003UL ) /* SWV output is output from the TRACECTL pin */

/***********************************************************************************************************************
Macro definitions
***********************************************************************************************************************/
#define _CGC_PLL_WAIT_CYCLE \
    ( 0x1D4CU ) /* Wait 100us when switch clock source in PLL0 and PLL1 */
#define _CGC_LOCO_WAIT_CYCLE \
    ( 0x0BB8U ) /* Wait 40us for LOCO oscillation stabilization */
