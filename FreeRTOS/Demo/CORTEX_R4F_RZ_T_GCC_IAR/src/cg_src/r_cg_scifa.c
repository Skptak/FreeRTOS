...
void R_SCIFA2_Create( void )
{
    ...
    SCIFA2.SMR.WORD = _SCIF_CLOCK_SERICLK_4 | _SCIF_STOP_1 |
                      _SCIF_PARITY_DISABLE | _SCIF_DATA_LENGTH_8 |
                      _SCIF_ASYNCHRONOUS_MODE;
    SCIFA2.SEMR.BYTE = _SCIF_16_BASE_CLOCK | _SCIF_NOISE_FILTER_ENABLE |
                       _SCIF_DATA_TRANSFER_LSB_FIRST | _SCIF_BAUDRATE_SINGLE;
    ...
    SCIFA2.FTCR.WORD = _SCIF_TX_FIFO_TRIGGER_NUM_0 |
                       _SCIF_TX_TRIGGER_TFTC_VALID |
                       _SCIF_RX_FIFO_TRIGGER_NUM_1 |
 ...
 * Arguments    : rx_buf -
 *                    receive buffer pointer (Not used when receive data handled
 *by DMAC) rx_num - buffer size (Not used when receive data handled by DMAC)
 * Return Value : status -
 ...
 * Arguments    : tx_buf -
 *                    transfer buffer pointer (Not used when transmit data
 *handled by DMAC) tx_num - buffer size (Not used when transmit data handled by
 *DMAC) Return Value : status - MD_OK or MD_ARGERROR
 ***********************************************************************************************************************/
