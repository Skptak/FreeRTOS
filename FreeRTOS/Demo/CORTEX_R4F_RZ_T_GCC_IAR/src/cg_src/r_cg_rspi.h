...
#define _RSPI_MODE_CLOCK_SYNCHRONOUS \
    ( 0x01U ) /* Clock synchronous operation (three-wire method) */
/* Communications Operating Mode Select (TXMD) */
#define _RSPI_FULL_DUPLEX_SYNCHRONOUS \
    ( 0x00U ) /* Full-duplex synchronous serial communications */
#define _RSPI_TRANSMIT_ONLY \
    ( 0x02U ) /* Serial communications with transmit only operations */
/* Mode Fault Error Detection Enable (MODFEN) */
#define _RSPI_MODE_FAULT_DETECT_DISABLED \
    ( 0x00U ) /* Disables the detection of mode fault error */
#define _RSPI_MODE_FAULT_DETECT_ENABLED \
    ( 0x04U ) /* Enables the detection of mode fault error */
...
#define _RSPI_ERROR_INTERRUPT_DISABLED \
    ( 0x00U ) /* Disables the generation of RSPI error interrupt */
#define _RSPI_ERROR_INTERRUPT_ENABLED \
    ( 0x10U ) /* Enables the generation of RSPI error interrupt */
/* RSPI Transmit Interrupt Enable (SPTIE) */
#define _RSPI_TRANSMIT_INTERRUPT_DISABLED \
    ( 0x00U ) /* Disables the generation of RSPI transmit interrupt */
#define _RSPI_TRANSMIT_INTERRUPT_ENABLED \
    ( 0x20U ) /* Enables the generation of RSPI transmit interrupt */
...
#define _RSPI_RECEIVE_INTERRUPT_DISABLED \
    ( 0x00U ) /* Disables the generation of RSPI receive interrupt */
#define _RSPI_RECEIVE_INTERRUPT_ENABLED \
    ( 0x80U ) /* Enables the generation of RSPI receive interrupt */
...
#define _RSPI_LOOPBACK_ENABLED \
    ( 0x01U ) /* Loopback mode (reversed transmit data = receive data) */
/* RSPI Loopback 2 (SPLP2) */
#define _RSPI_LOOPBACK2_DISABLED ( 0x00U ) /* Normal mode */
#define _RSPI_LOOPBACK2_ENABLED \
    ( 0x02U ) /* Loopback mode (transmit data = receive data) */
...
#define _RSPI_MOSI_LEVEL_LOW \
    ( 0x00U ) /* Level output on MOSIA during idling corresponds to low */
#define _RSPI_MOSI_LEVEL_HIGH \
    ( 0x10U ) /* Level output on MOSIA during idling corresponds to high */
/* MOSI Idle Value Fixing Enable (MOIFE) */
#define _RSPI_MOSI_FIXING_PREV_TRANSFER \
    ( 0x00U ) /* MOSI output value equals final data from previous transfer */
#define _RSPI_MOSI_FIXING_MOIFV_BIT \
    ( 0x20U ) /* MOSI output value equals the value set in the MOIFV bit */
...
#define _RSPI_SEQUENCE_LENGTH_6 \
    ( 0x05U ) /* 0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 0... */
#define _RSPI_SEQUENCE_LENGTH_7 \
    ( 0x06U ) /* 0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 0... */
#define _RSPI_SEQUENCE_LENGTH_8 \
    ( 0x07U ) /* 0 -> 1 -> 2 -> 3 -> 4 -> 5 -> 6 -> 7 -> 0... */
...
#define _RSPI_READ_SPDR_RX_BUFFER \
    ( 0x00U ) /* read SPDR values from receive buffer */
#define _RSPI_READ_SPDR_TX_BUFFER                                              \
    ( 0x10U ) /* read SPDR values from transmit buffer (transmit buffer empty) \
               */
...
*/
/* Parity Enable (SPPE) */
#define _RSPI_PARITY_DISABLE \
    ( 0x00U ) /* Does not add parity bit to transmit data */
#define _RSPI_PARITY_ENABLE                           \
    ( 0x01U ) /* Adds the parity bit to transmit data \
               */
/* Parity Mode (SPOE) */
#define _RSPI_PARITY_EVEN \
    ( 0x00U ) /* Selects even parity for use in transmission and reception */
#define _RSPI_PARITY_ODD \
    ( 0x02U ) /* Selects odd parity for use in transmission and reception */
/* RSPI Idle Interrupt Enable (SPIIE) */
#define _RSPI_IDLE_INTERRUPT_DISABLED \
    ( 0x00U ) /* Disables the generation of RSPI idle interrupt */
#define _RSPI_IDLE_INTERRUPT_ENABLED \
    ( 0x04U ) /* Enables the generation of RSPI idle interrupt */
/* Parity Self-Testing (PTE) */
#define _RSPI_SELF_TEST_DISABLED \
    ( 0x00U ) /* Disables the self-diagnosis function of the parity circuit */
#define _RSPI_SELF_TEST_ENABLED \
    ( 0x08U ) /* Enables the self-diagnosis function of the parity circuit */
/* RSPCK Auto-Stop Function Enable (SCKASE) */
#define _RSPI_AUTO_STOP_DISABLED \
    ( 0x00U ) /* Disables the RSPCK auto-stop function */
#define _RSPI_AUTO_STOP_ENABLED \
    ( 0x10U ) /* Enables the RSPCK auto-stop function */

/*
    RSPI Command Registers 0 to 7 (SPCMD0 to SPCMD7)
*/
/* RSPCK Phase Setting (CPHA) */
#define _RSPI_RSPCK_SAMPLING_ODD \
    ( 0x0000U ) /* Data sampling on odd edge, data variation on even edge */
#define _RSPI_RSPCK_SAMPLING_EVEN \
    ( 0x0001U ) /* Data variation on odd edge, data sampling on even edge */
...
#define _RSPI_BASE_BITRATE_1 \
    ( 0x0000U ) /* These bits select the base bit rate */
#define _RSPI_BASE_BITRATE_2 \
    ( 0x0004U ) /* These bits select the base bit rate divided by 2 */
#define _RSPI_BASE_BITRATE_4 \
    ( 0x0008U ) /* These bits select the base bit rate divided by 4 */
#define _RSPI_BASE_BITRATE_8 \
    ( 0x000CU ) /* These bits select the base bit rate divided by 8 */
...
#define _RSPI_SSL_KEEP_DISABLE \
    ( 0x0000U ) /* Negates all SSL signals upon completion of transfer */
#define _RSPI_SSL_KEEP_ENABLE \
    ( 0x0080U ) /* Keep SSL level from end of transfer till next access */
...
#define _RSPI_NEXT_ACCESS_DELAY_DISABLE \
    ( 0x0000U ) /* Next-access delay of 1 RSPCK + 2 SERICLK */
#define _RSPI_NEXT_ACCESS_DELAY_ENABLE \
    ( 0x2000U ) /* Next-access delay equal to setting of SPND register */
/* SSL Negation Delay Setting Enable (SLNDEN) */
#define _RSPI_NEGATION_DELAY_DISABLE \
    ( 0x0000U ) /* SSL negation delay of 1 RSPCK */
#define _RSPI_NEGATION_DELAY_ENABLE \
    ( 0x4000U ) /* SSL negation delay equal to setting of SSLND register */
/* RSPCK Delay Setting Enable (SCKDEN) */
#define _RSPI_RSPCK_DELAY_DISABLE ( 0x0000U ) /* RSPCK delay of 1 RSPCK */
#define _RSPI_RSPCK_DELAY_ENABLE \
    ( 0x8000U ) /* RSPCK delay equal to setting of the SPCKD register */
