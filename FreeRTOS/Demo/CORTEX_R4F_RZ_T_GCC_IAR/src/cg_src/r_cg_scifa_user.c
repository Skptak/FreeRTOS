...
void r_scifa2_txif2_interrupt( void )
{
    ...
    while( ( g_scifa2_tx_count > 0U ) &&
           ( count < _SCIF_FIFO_MAX_SIZE - dummy_fdr ) )
    {
...
void r_scifa2_rxif2_interrupt( void )
{
    ...
    /* If remaining data is less than the receive trigger number, receive
       interrupt will not occur. In this case, set trigger number to 1 to force
       receive interrupt for each one byte of data in FRDR */
    if( ( g_scifa2_rx_length - g_scifa2_rx_count < _SCIF_RX_TRIG_NUM_2 ) &&
 ...
 * Description  : This function is a callback function when SCIFA2 finishes
 ...
 * Description  : This function is a callback function when SCIFA2 reception
 *encounters error. Arguments    : error_type - reception error type Return
 *Value : None
 ***********************************************************************************************************************/
