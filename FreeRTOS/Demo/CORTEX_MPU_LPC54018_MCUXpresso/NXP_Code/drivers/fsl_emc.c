...
#define EMC_SDRAM_BANKCS_BA_MASK \
    ( EMC_SDRAM_BANKCS_BA0_MASK | EMC_SDRAM_BANKCS_BA1_MASK )
 ...
 * @param plus The plus added to the register settings to reach the calculated
 * cycles.
...
static const clock_ip_name_t
    s_EMCClock[ FSL_FEATURE_SOC_EMC_COUNT ] = EMC_CLOCKS;
...
static uint32_t EMC_ModeOffset( uint32_t addrMap )
{
    ...
    else
    {
        ...
        offset += ( uint8_t ) ( ( addrMap & EMC_DYNCTL_COLUMNPLUS_MASK ) >>
                                EMC_DYNCTL_COLUMNPLUS_OFFSET );
    }
    ...
    /* Add bank select bit if the sdram address map mode is RBC(row-bank-column)
     * mode. */
 ...
 * and enable the EMC module. This function must be called in the first step to
 ...
 * This function initializes the dynamic memory controller in external memory
 * controller. This function must be called after EMC_Init and before accessing
 * the external dynamic memory.
 *
 * param base EMC peripheral base address.
 * param timing The timing and latency for dynamica memory controller setting.
 * It shall be used for all dynamica memory chips, threfore the worst timing
 * value for all used chips must be given. param configure The EMC dynamic
 * memory controller chip independent configuration pointer. This configuration
 * pointer is actually pointer to a configration array. the array number depends
 * on the "totalChips". param totalChips The total dynamic memory chip numbers
 * been used or the length of the "emc_dynamic_chip_config_t" type memory.
 */
                         ...
                         uint32_t totalChips )
{
    ...
    for( count = 0; ( count < totalChips ); count++ )
    {
        ...
        {
            base->DYNAMIC[ dynamicConfig->chipIndex ]
                .DYNAMICCONFIG = EMC_DYNAMIC_DYNAMICCONFIG_MD(
                                     dynamicConfig->dynamicDevice ) |
                                 EMC_ADDRMAP( dynamicConfig->devAddrMap );
            /* Abstract CAS latency from the sdram mode reigster setting values.
             */
            casLatency = ( ( uint32_t ) dynamicConfig->sdramModeReg &
                           EMC_SDRAM_MODE_CL_MASK ) >>
                         EMC_SDRAM_MODE_CL_SHIFT;
            base->DYNAMIC[ dynamicConfig->chipIndex ]
                .DYNAMICRASCAS = EMC_DYNAMIC_DYNAMICRASCAS_RAS(
                                     dynamicConfig->rAS_Nclk ) |
    ...
    base->DYNAMICRP = EMC_CalculateTimerCycles( base, timing->tRp_Ns, 1 ) &
                      EMC_DYNAMICRP_TRP_MASK;
    base->DYNAMICRAS = EMC_CalculateTimerCycles( base, timing->tRas_Ns, 1 ) &
                       EMC_DYNAMICRAS_TRAS_MASK;
    base->DYNAMICSREX = EMC_CalculateTimerCycles( base, timing->tSrex_Ns, 1 ) &
                        EMC_DYNAMICSREX_TSREX_MASK;
    base->DYNAMICAPR = EMC_CalculateTimerCycles( base, timing->tApr_Ns, 1 ) &
                       EMC_DYNAMICAPR_TAPR_MASK;
    base->DYNAMICDAL = EMC_CalculateTimerCycles( base, timing->tDal_Ns, 0 ) &
                       EMC_DYNAMICDAL_TDAL_MASK;
    base->DYNAMICWR = EMC_CalculateTimerCycles( base, timing->tWr_Ns, 1 ) &
                      EMC_DYNAMICWR_TWR_MASK;
    base->DYNAMICRC = EMC_CalculateTimerCycles( base, timing->tRc_Ns, 1 ) &
                      EMC_DYNAMICRC_TRC_MASK;
    base->DYNAMICRFC = EMC_CalculateTimerCycles( base, timing->tRfc_Ns, 1 ) &
                       EMC_DYNAMICRFC_TRFC_MASK;
    base->DYNAMICXSR = EMC_CalculateTimerCycles( base, timing->tXsr_Ns, 1 ) &
                       EMC_DYNAMICXSR_TXSR_MASK;
    base->DYNAMICRRD = EMC_CalculateTimerCycles( base, timing->tRrd_Ns, 1 ) &
                       EMC_DYNAMICRRD_TRRD_MASK;
    base->DYNAMICMRD = EMC_DYNAMICMRD_TMRD(
        ( timing->tMrd_Nclk > 0U ) ? timing->tMrd_Nclk - 1UL : 0UL );
    ...
    base->DYNAMICREFRESH = EMC_CalculateTimerCycles(
                               base,
                               timing->refreshPeriod_Nanosec,
                               0 ) /
                           EMC_REFRESH_CLOCK_PARAM;
    ...
    for( count = 0; ( count < totalChips ); count++ )
    {
        ...
        else
        {
            ...
            addr = ( s_EMCDYCSBases[ dynamicConfig->chipIndex ] |
                     ( ( uint32_t ) ( dynamicConfig->sdramModeReg &
                                      ~EMC_SDRAM_BANKCS_BA_MASK )
                       << offset ) );
    ...
    if( kEMC_Sdram != config->dynamicDevice )
    {
        ...
        for( count = 0; ( count < totalChips ); count++ )
        {
            ...
            else
            {
                ...
                offset = EMC_ModeOffset( dynamicConfig->devAddrMap );
                addr = ( s_EMCDYCSBases[ dynamicConfig->chipIndex ] |
                         ( ( ( uint32_t ) ( dynamicConfig->sdramExtModeReg &
                                            ~EMC_SDRAM_BANKCS_BA_MASK ) |
    ...
    for( count = 0; ( count < totalChips ); count++ )
    {
        ...
        {
            base->DYNAMIC[ dynamicConfig->chipIndex ]
                .DYNAMICCONFIG |= EMC_DYNAMIC_DYNAMICCONFIG_B_MASK;
            dynamicConfig++;
        }
    }
}
 ...
 * This function initializes the static memory controller in external memory
 * controller. This function must be called after EMC_Init and before accessing
 * the external static memory.
 *
 * param base EMC peripheral base address.
 * param extWait_Ns The extended wait timeout or the read/write transfer time.
 *        This is common for all static memory chips and set with NULL if not
 * required. param configure The EMC static memory controller chip independent
 * configuration pointer. This configuration pointer is actually pointer to a
 * configration array. the array number depends on the "totalChips". param
 * totalChips The total static memory chip numbers been used or the length of
 * the "emc_static_chip_config_t" type memory.
 */
                        ...
                        uint32_t totalChips )
{
    ...
    if( NULL != extWait_Ns )
    {
        for( count = 0; ( count < totalChips ) && ( staticConfig != NULL );
             count++ )
        {
            assert( 0U != ( staticConfig->specailConfig &
                            ( uint32_t ) kEMC_AsynchronosPageEnable ) );
        }

        base->STATICEXTENDEDWAIT = EMC_CalculateTimerCycles( base,
                                                             *extWait_Ns,
                                                             1 );
        staticConfig++;
    }
    ...
    for( count = 0; ( count < totalChips ); count++ )
    {
        ...
        {
            base->STATIC[ staticConfig->chipIndex ]
                .STATICCONFIG = ( staticConfig->specailConfig |
                                  ( uint32_t ) staticConfig->memWidth );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITWEN = EMC_CalculateTimerCycles(
                base,
                staticConfig->tWaitWriteEn_Ns,
                1 );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITOEN = EMC_CalculateTimerCycles( base,
                                                           staticConfig
                                                               ->tWaitOutEn_Ns,
                                                           0 );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITRD = EMC_CalculateTimerCycles(
                base,
                staticConfig->tWaitReadNoPage_Ns,
                1 );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITPAGE = EMC_CalculateTimerCycles(
                base,
                staticConfig->tWaitReadPage_Ns,
                1 );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITWR = EMC_CalculateTimerCycles( base,
                                                          staticConfig
                                                              ->tWaitWrite_Ns,
                                                          2 );
            base->STATIC[ staticConfig->chipIndex ]
                .STATICWAITTURN = EMC_CalculateTimerCycles( base,
                                                            staticConfig
                                                                ->tWaitTurn_Ns,
                                                            1 );
