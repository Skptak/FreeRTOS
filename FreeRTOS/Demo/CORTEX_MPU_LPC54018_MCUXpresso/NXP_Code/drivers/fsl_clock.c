...
#define PLL_MDEC_VAL_M \
    ( 0x1FFFFUL << PLL_MDEC_VAL_P )     /*!<  NDEC is in bits  9 downto 0 */
...
/*!<  SYS PLL NDEC reg */
#define PLL_NDEC_VAL_SET( value ) \
    ( ( ( unsigned long ) ( value ) << PLL_NDEC_VAL_P ) & PLL_NDEC_VAL_M )
/*!<  SYS PLL PDEC reg */
#define PLL_PDEC_VAL_SET( value ) \
    ( ( ( unsigned long ) ( value ) << PLL_PDEC_VAL_P ) & PLL_PDEC_VAL_M )
/*!<  SYS PLL MDEC reg */
#define PLL_MDEC_VAL_SET( value ) \
    ( ( ( unsigned long ) ( value ) << PLL_MDEC_VAL_P ) & PLL_MDEC_VAL_M )

/*!<  SYS PLL NSEL reg */
#define USB_PLL_NSEL_VAL_SET( value )                     \
    ( ( ( unsigned long ) ( value ) &USB_PLL_NSEL_VAL_M ) \
      << USB_PLL_NSEL_VAL_P )
/*!<  SYS PLL PSEL reg */
#define USB_PLL_PSEL_VAL_SET( value )                     \
    ( ( ( unsigned long ) ( value ) &USB_PLL_PSEL_VAL_M ) \
      << USB_PLL_PSEL_VAL_P )
/*!<  SYS PLL MSEL reg */
#define USB_PLL_MSEL_VAL_SET( value )                     \
    ( ( ( unsigned long ) ( value ) &USB_PLL_MSEL_VAL_M ) \
      << USB_PLL_MSEL_VAL_P )
...
#define AUDIO_PLL_MD_FRACT_SET( value )                         \
    ( ( ( unsigned long ) ( value ) << AUDIO_PLL_FRACT_MD_P ) & \
      PLL_FRAC_MD_FRACT_M )
#define AUDIO_PLL_MD_INT_SET( value )                               \
    ( ( ( unsigned long ) ( value ) << AUDIO_PLL_FRACT_MD_INT_P ) & \
 ...
 * Due to LPC540xx 0A silicon and LPC540xx 1B silicon have different ROM
 * addresses for set fro frequency api, so add this api to get rom version.
...
static const uint8_t wdtFreqLookup[ 32 ] = { 0,  8,  12, 15, 18, 20, 24, 26,
...
static uint32_t CLOCK_GetRomVersion( uint8_t * romVersion )
{
    ...
    result[ 0 ] = 0;
    result[ 1 ] = 0;
    ( ( void ( * )( uint32_t cmd[ 5 ], uint32_t stat[ 4 ] ) )
          FSL_FEATURE_SYSCON_IAP_ENTRY_LOCATION )( command, result );
...
/**
 * brief
 *  Initialize the Core clock to given frequency (12, 48 or 96 MHz), this API is
 * implememnt in ROM code. Turns on FRO and uses default CCO, if freq is
 * 12000000, then high speed output is off, else high speed output is enabled.
 * Usage: CLOCK_SetupFROClocking(frequency), (frequency must be one of 12, 48 or
 * 96 MHz) Note: Need to make sure ROM and OTP has power(PDRUNCFG0[17,29]= 0U)
 * before calling this API since this API is implemented in ROM code and the
 * FROHF TRIM value is stored in OTP
 *
 ...
 * attach id. param   attachId  : Clock attach id to get. return  Clock source
 * value.
 */
                      ...
                      bool reset )
{
    ...
    if( reset )
    {
        ( ( volatile uint32_t * ) pClkDiv )[ ( uint8_t ) div_name ] = 1UL
                                                                      << 29U;
    }
...
uint32_t CLOCK_GetWdtOscFreq( void )
{
    ...
    else
    {
        ...
        freq_sel = wdtFreqLookup[ (
            ( SYSCON->WDTOSCCTRL & SYSCON_WDTOSCCTRL_FREQSEL_MASK ) >>
...
{
    return CLK_RTC_32K_CLK; /* Needs to be corrected to check that RTC Clock is
                               enabled */
}
...
uint32_t CLOCK_GetMCanClkFreq( uint32_t MCanSel )
{
    ...
    switch( MCanSel )
    {
        case 0U:
            freq = CLOCK_GetCoreSysClkFreq() /
                   ( ( SYSCON->CAN0CLKDIV & 0xffU ) + 1U );
            break;
        case 1U:
            freq = CLOCK_GetCoreSysClkFreq() /
                   ( ( SYSCON->CAN1CLKDIV & 0xffU ) + 1U );
            break;
...
uint32_t CLOCK_GetFrgClkFreq( void )
{
    ...
    if( ( SYSCON->FRGCTRL & SYSCON_FRGCTRL_DIV_MASK ) ==
        SYSCON_FRGCTRL_DIV_MASK )
    {
        freq = ( uint32_t ) ( ( ( uint64_t ) CLOCK_GetFRGInputClock() *
...
uint32_t CLOCK_SetFRGClock( uint32_t freq )
{
    ...
    {
        mul = ( uint32_t ) ( ( ( ( uint64_t ) input - freq ) * 256U ) /
                             ( ( uint64_t ) freq ) );
        SYSCON->FRGCTRL = ( mul << SYSCON_FRGCTRL_MULT_SHIFT ) |
                          SYSCON_FRGCTRL_DIV_MASK;
        return 1U;
    }
}
                        ...
                        uint32_t * pSelR )
{
    ...
    if( *pSelI > ( ( 0x3FUL << SYSCON_SYSPLLCTRL_SELI_SHIFT ) >>
                   SYSCON_SYSPLLCTRL_SELI_SHIFT ) )
    {
        *pSelI = ( ( 0x3FUL << SYSCON_SYSPLLCTRL_SELI_SHIFT ) >>
                   SYSCON_SYSPLLCTRL_SELI_SHIFT );
    }
...
static double Binary2Fractional( uint32_t binaryPart )
{
    ...
    {
        fractional += ( double ) ( uint32_t ) ( ( binaryPart >> i ) & 0x1U ) /
                      ( double ) ( uint32_t ) power2Cal( 15U - i );
    }
    return fractional;
}
...
/*
 * Set PLL output based on desired output rate.
 * In this function, the it calculates the PLL setting for output frequency from
 * input clock frequency. The calculation would cost a few time. So it is not
 * recommaned to use it frequently. the "pllctrl", "pllndec", "pllpdec",
 * "pllmdec" would updated in this function.
 */
                                               ...
                                               pll_setup_t * pSetup )
{
    ...
    {
        /* Minmum PLL output with maximum post divider cannot go below this
         * frequency */
    ...
    {
        uint32_t a = FindGreatestCommonDivisor( fccoHz,
                                                ( multFccoDiv * finHz ) );
    ...
    if( ( nDivOutHz * ( ( multFccoDiv * pllMultiplier * 2U ) + 1U ) ) <
        ( fccoHz * 2U ) )
    {
        pllMultiplier++;
    }
    ...
    /* PLL control */
    pSetup->pllctrl = ( pllSelR
                        << SYSCON_SYSPLLCTRL_SELR_SHIFT ) | /* Filter
                                                               coefficient
                                                             */
                      ( pllSelI
                        << SYSCON_SYSPLLCTRL_SELI_SHIFT ) | /* Filter
                                                               coefficient
                                                             */
                      ( pllSelP
                        << SYSCON_SYSPLLCTRL_SELP_SHIFT ) |       /* Filter
                                                                     coefficient
                                                                   */
                      ( 0UL << SYSCON_SYSPLLCTRL_BYPASS_SHIFT ) | /* PLL bypass
                                                                     mode
                                                                     disabled */
                      ( uplimoff
                        << SYSCON_SYSPLLCTRL_UPLIMOFF_SHIFT ) | /* SS/fractional
                                                                   mode
                                                                   disabled
                                                                 */
                      ( pllDirectInput
                        << SYSCON_SYSPLLCTRL_DIRECTI_SHIFT ) | /* Bypass
                                                                  pre-divider?
                                                                */
                      ( pllDirectOutput
                        << SYSCON_SYSPLLCTRL_DIRECTO_SHIFT ); /* Bypass
                                                                 post-divider?
                                                               */
...
#if( defined( CLOCK_USR_CFG_PLL_CONFIG_CACHE_COUNT ) && \
                                       ...
                                       pll_setup_t * pSetup )
{
    ...
    if( kStatus_PLL_Success == retErr )
    {
        ...
        gPllSetupCacheIdx = ( gPllSetupCacheIdx + 1U ) %
                            CLOCK_USR_CFG_PLL_CONFIG_CACHE_COUNT;
    }
#endif /* CLOCK_USR_CFG_PLL_CONFIG_CACHE_COUNT */
...
uint32_t CLOCK_GetSystemPLLOutFromSetup( pll_setup_t * pSetup )
{
    ...
    /* If the PLL is bypassed, PLL would not be used and the output of PLL
     * module would just be the input clock*/
    if( ( pSetup->pllctrl & ( SYSCON_SYSPLLCTRL_BYPASS_MASK ) ) == 0U )
    {
        /* PLL is not in bypass mode, get pre-divider, and M divider,
         * post-divider. */
        /*
         * 1. Pre-divider
        ...
        workRate = workRate * 2U; /* SYS PLL hardware cco is divide by 2 before
                                     to M-DIVIDER*/
    }
    else
    {
...
uint32_t CLOCK_GetUsbPLLOutFromSetup( const usb_pll_setup_t * pSetup )
{
    ...
    if( pSetup->fbsel )
    {
        ...
        workRate = ( ( uint64_t ) inPllRate ) * ( ( uint64_t ) msel + 1U ) /
                   ( ( uint64_t ) nsel + 1U );
    }
    else
    {
        /* non integer_mode: Fout = M*(Fin/N)/(2*P), Fcco = M * (Fin/N) */
        workRate = ( ( uint64_t ) inPllRate / ( ( uint64_t ) nsel + 1U ) ) *
                   ( msel + 1U ) / ( 2U * SWITCH_USB_PSEL( psel ) );
    }
...
/*! brief	Return System AUDIO PLL output clock rate from audio fractioanl
...
uint32_t CLOCK_GetUsbPLLOutClockRate( bool recompute )
{
    ...
    {
        Setup.msel = ( uint8_t ) ( ( SYSCON->USBPLLCTRL >>
                                     SYSCON_USBPLLCTRL_MSEL_SHIFT ) &
                                   SYSCON_USBPLLCTRL_MSEL_MASK );
        Setup.psel = ( uint8_t ) ( ( SYSCON->USBPLLCTRL >>
                                     SYSCON_USBPLLCTRL_PSEL_SHIFT ) &
                                   SYSCON_USBPLLCTRL_PSEL_MASK );
        Setup.nsel = ( uint8_t ) ( ( SYSCON->USBPLLCTRL >>
                                     SYSCON_USBPLLCTRL_NSEL_SHIFT ) &
                                   SYSCON_USBPLLCTRL_NSEL_MASK );
        Setup.fbsel = ( ( ( SYSCON->USBPLLCTRL >>
                            SYSCON_USBPLLCTRL_FBSEL_SHIFT ) &
                          SYSCON_USBPLLCTRL_FBSEL_MASK ) != 0UL );
        Setup.bypass = ( ( ( SYSCON->USBPLLCTRL >>
                             SYSCON_USBPLLCTRL_BYPASS_SHIFT ) &
                           SYSCON_USBPLLCTRL_BYPASS_MASK ) != 0UL );
        Setup.direct = ( ( ( SYSCON->USBPLLCTRL >>
                             SYSCON_USBPLLCTRL_DIRECT_SHIFT ) &
                           SYSCON_USBPLLCTRL_DIRECT_MASK ) != 0UL );
 ...
 *  param	pControl	: Pointer to populated PLL control structure to generate
 * setup with param	pSetup		: Pointer to PLL setup structure to be filled
...
pll_error_t CLOCK_SetupSystemPLLPrec( pll_setup_t * pSetup, uint32_t flagcfg )
{
    ...
    SYSCON->SYSPLLNDEC = pSetup->pllndec |
                         ( 1UL << SYSCON_SYSPLLNDEC_NREQ_SHIFT ); /* latch */
    SYSCON->SYSPLLPDEC = pSetup->pllpdec;
    SYSCON->SYSPLLPDEC = pSetup->pllpdec |
                         ( 1UL << SYSCON_SYSPLLPDEC_PREQ_SHIFT ); /* latch */
    SYSCON->SYSPLLMDEC = pSetup->pllmdec;
    SYSCON->SYSPLLMDEC = pSetup->pllmdec |
                         ( 1UL << SYSCON_SYSPLLMDEC_MREQ_SHIFT ); /* latch */

    /* Flags for lock or power on */
    if( ( pSetup->flags &
          ( PLL_SETUPFLAG_POWERUP | PLL_SETUPFLAG_WAITLOCK ) ) != 0U )
    {
        /* If turning the PLL back on, perform the following sequence to
         * accelerate PLL lock */
        uint32_t maxCCO = ( 1UL << 18U ) |
                          0x5dd2U; /* CCO = 1.6Ghz + MDEC enabled*/
...
/*! brief	Set AUDIO PLL output from AUDIOPLL setup structure (precise
 * frequency) param	pSetup	: Pointer to populated PLL setup structure param
 * flagcfg : Flag configuration for PLL config structure return
 * PLL_ERROR_SUCCESS on success, or PLL setup error code note	This function
 * will power off the PLL, setup the PLL with the new setup data, and then
 * optionally powerup the AUDIO PLL, wait for PLL lock, and adjust system
 * voltages to the new AUDIOPLL rate. The function will not alter any source
 * clocks (ie, main systen clock) that may use the AUDIO PLL, so these should be
...
pll_error_t CLOCK_SetupAudioPLLPrec( pll_setup_t * pSetup, uint32_t flagcfg )
{
    ...
    SYSCON->AUDPLLMDEC = pSetup->pllmdec |
                         ( 1UL << SYSCON_SYSPLLMDEC_MREQ_SHIFT ); /* latch */
    SYSCON->AUDPLLFRAC = SYSCON_AUDPLLFRAC_SEL_EXT( 1 ); /* disable fractional
                                                            function */
...
/*! brief	Set AUDIO PLL output from AUDIOPLL setup structure using the Audio
 * Fractional divider register(precise frequency) param	pSetup	: Pointer to
 * populated PLL setup structure param flagcfg : Flag configuration for PLL
 * config structure return	PLL_ERROR_SUCCESS on success, or PLL setup error
 * code note	This function will power off the PLL, setup the PLL with the new
 * setup data, and then optionally powerup the AUDIO PLL, wait for PLL lock, and
 * adjust system voltages to the new AUDIOPLL rate. The function will not alter
 * any source clocks (ie, main systen clock) that may use the AUDIO PLL, so
                                          ...
                                          uint32_t flagcfg )
{
    ...
    SYSCON->AUDPLLPDEC = pSetup->pllpdec |
                         ( 1UL << SYSCON_SYSPLLPDEC_PREQ_SHIFT ); /* latch */
    SYSCON->AUDPLLMDEC = pSetup->pllmdec;
    SYSCON->AUDPLLFRAC = SYSCON_AUDPLLFRAC_SEL_EXT( 0 ); /* enable fractional
                                                            function */
    SYSCON->AUDPLLFRAC = pSetup->audpllfrac;
    SYSCON->AUDPLLFRAC = pSetup->audpllfrac |
                         ( 1UL << SYSCON_AUDPLLFRAC_REQ_SHIFT );
...
/**
 * brief	Set Audio PLL output from Audio PLL setup structure (precise
 * frequency) param	pSetup	: Pointer to populated PLL setup structure return
 * kStatus_PLL_Success on success, or Audio PLL setup error code note	This
 * function will power off the PLL, setup the Audio PLL with the new setup data,
 * and then optionally powerup the PLL, wait for Audio PLL lock, and adjust
 * system voltages to the new PLL rate. The function will not alter any source
 * clocks (ie, main systen clock) that may use the Audio PLL, so these should be
 * setup prior to and after exiting the function.
 */
pll_error_t CLOCK_SetAudioPLLFreq( const pll_setup_t * pSetup )
{
    ...
    SYSCON->AUDPLLFRAC = pSetup->audpllfrac;
    SYSCON->AUDPLLFRAC = pSetup->audpllfrac |
                         ( 1UL << SYSCON_AUDPLLFRAC_REQ_SHIFT ); /* latch */
    SYSCON->AUDPLLNDEC = pSetup->pllndec;
    SYSCON->AUDPLLNDEC = pSetup->pllndec |
                         ( 1UL << SYSCON_AUDPLLNDEC_NREQ_SHIFT ); /* latch */
    SYSCON->AUDPLLPDEC = pSetup->pllpdec;
    SYSCON->AUDPLLPDEC = pSetup->pllpdec |
                         ( 1UL << SYSCON_AUDPLLPDEC_PREQ_SHIFT ); /* latch */
    SYSCON->AUDPLLMDEC = pSetup->pllmdec;
    SYSCON->AUDPLLMDEC = pSetup->pllmdec |
                         ( 1UL << SYSCON_AUDPLLMDEC_MREQ_SHIFT ); /* latch */
    SYSCON->AUDPLLFRAC = SYSCON_AUDPLLFRAC_SEL_EXT( 1 ); /* disable fractional
                                                            function */
 ...
 * new setup data, and then optionally powerup the USB PLL, wait for USB PLL
 * lock, and adjust system voltages to the new USB PLL rate. The function will
 * not alter any source clocks (ie, usb pll clock) that may use the USB PLL, so
 * these should be setup prior to and after exiting the function.
 */
pll_error_t CLOCK_SetUsbPLLFreq( const usb_pll_setup_t * pSetup )
{
    ...
    {
        /*integer_mode: Fout = M*(Fin/N),  Fcco = 2*P*M*(Fin/N) */
        fccoHz = ( pSetup->inputRate / ( ( uint32_t ) nsel + 1U ) ) * 2U *
                 ( msel + 1U ) * SWITCH_USB_PSEL( psel );
    ...
    usbpllctrl = USB_PLL_NSEL_VAL_SET( nsel ) | /* NSEL VALUE */
                 ...
                 ( uint32_t ) pllDirectInput
                     << SYSCON_USBPLLCTRL_BYPASS_SHIFT | /* BYPASS DISABLE */
                 ( uint32_t ) pllDirectOutput
                     << SYSCON_USBPLLCTRL_DIRECT_SHIFT | /* DIRECTO DISABLE */
                 ( uint32_t ) pllfbsel
                     << SYSCON_USBPLLCTRL_FBSEL_SHIFT; /* FBSEL SELECT */
...
void CLOCK_SetupSystemPLLMult( uint32_t multiply_by, uint32_t input_freq )
{
    ...
    while( cco_freq < 275000000U )
    {
        ...
        pdec <<= 1U; /* correspondingly double pdec to cancel effect of double
                        msel */
    ...
    SYSCON->SYSPLLCTRL = directo | ( selr << SYSCON_SYSPLLCTRL_SELR_SHIFT ) |
                         ( seli << SYSCON_SYSPLLCTRL_SELI_SHIFT ) |
                         ( selp << SYSCON_SYSPLLCTRL_SELP_SHIFT );
    SYSCON->SYSPLLPDEC = pdec |
                         ( 1U << 7U ); /* set Pdec value and assert preq */
    SYSCON->SYSPLLNDEC = ndec |
                         ( 1UL << 10U ); /* set Pdec value and assert preq */
    SYSCON->SYSPLLMDEC = ( 1UL << 17U ) |
                         mdec; /* select non sscg MDEC value, assert mreq and
                                  select mdec value */
}
...
bool CLOCK_EnableUsbfs0DeviceClock( clock_usb_src_t src, uint32_t freq )
{
    ...
    if( kCLOCK_UsbSrcFro == src )
    {
        ...
        {
            case 96000000U:
                CLOCK_SetClkDiv( kCLOCK_DivUsb0Clk,
                                 2U,
                                 false ); /*!< Div by 2 to get 48MHz, no divider
                                             reset */
                break;
            ...
            case 48000000U:
                CLOCK_SetClkDiv( kCLOCK_DivUsb0Clk,
                                 1U,
                                 false ); /*!< Div by 1 to get 48MHz, no divider
                                             reset */
                break;
        ...
        SYSCON->FROCTRL = ( SYSCON->FROCTRL &
                            ~( ( 0x01UL << 15U ) | ( 0xFUL << 26U ) ) ) |
    ...
    else
    {
        ...
        usb_pll_setup_t pll_setup = { 0x3FU, 0x01U, 0x03U,    false,
...
bool CLOCK_EnableUsbhs0DeviceClock( clock_usb_src_t src, uint32_t freq )
{
    ...
    if( kCLOCK_UsbSrcFro == src )
    {
        ...
        {
            case 96000000U:
                CLOCK_SetClkDiv( kCLOCK_DivUsb1Clk,
                                 2U,
                                 false ); /*!< Div by 2 to get 48MHz, no divider
                                             reset */
                break;
