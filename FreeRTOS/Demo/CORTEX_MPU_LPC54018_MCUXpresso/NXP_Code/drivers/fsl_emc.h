...
#define EMC_ADDRMAP_MASK \
    ( EMC_DYNAMIC_DYNAMICCONFIG_AM0_MASK | EMC_DYNAMIC_DYNAMICCONFIG_AM1_MASK )
#define EMC_ADDRMAP( x )                                                 \
    ( ( ( uint32_t ) ( ( ( uint32_t ) ( x ) ) << EMC_ADDRMAP_SHIFT ) ) & \
      EMC_ADDRMAP_MASK )
...
{
    kEMC_AsynchronosPageEnable = 0x0008U, /*!< Enable the asynchronous page
                                             mode. page length four. */
    kEMC_ActiveHighChipSelect = 0x0040U,  /*!< Chip select active high. */
    kEMC_ByteLaneStateAllLow = 0x0080U, /*!< Reads/writes the respective valuie
...
typedef enum _emc_dynamic_read
{
    ...
    kEMC_CmdDelayPulseOneclk, /*!< Command delayed strategy pluse one clock
                                 cycle using EMCCLKDELAY. */
    kEMC_CmddelayPulsetwoclk, /*!< Command delayed strategy pulse two clock
                                 cycle using EMCCLKDELAY. */
} emc_dynamic_read_t;
...
typedef struct _emc_dynamic_timing_config
{
    ...
    uint32_t refreshPeriod_Nanosec; /*!< The refresh period in unit of
                                       nanosecond. */
    uint32_t tRp_Ns;   /*!< Precharge command period in unit of nanosecond. */
    uint32_t tRas_Ns;  /*!< Active to precharge command period in unit of
                          nanosecond. */
    uint32_t tSrex_Ns; /*!< Self-refresh exit time in unit of nanosecond. */
    uint32_t tApr_Ns;  /*!< Last data out to active command time in unit of
                          nanosecond. */
    ...
    uint32_t tRc_Ns; /*!< Active to active command period in unit of nanosecond.
                      */
    uint32_t tRfc_Ns;  /*!< Auto-refresh period and auto-refresh to active
                          command period in unit of nanosecond. */
    uint32_t tXsr_Ns;  /*!< Exit self-refresh to active command time in unit of
                          nanosecond. */
    uint32_t tRrd_Ns;  /*!< Active bank A to active bank B latency in unit of
                          nanosecond. */
    uint8_t tMrd_Nclk; /*!< Load mode register to active command time in unit of
                          EMCCLK cycles.*/
} emc_dynamic_timing_config_t;

/*!
 * @brief EMC dynamic memory controller independent chip configuration
 * structure. Please take refer to the address mapping table in the RM in EMC
 * chapter when you set the "devAddrMap". Choose the right Bit 14 Bit12 ~ Bit 7
 * group in the table according to the bus width/banks/row/colum length for you
 * device. Set devAddrMap with the value make up with the seven bits (bit14
 * bit12 ~ bit 7) and inset the bit 13 with 0. for example, if the bit 14 and
 * bit12 ~ bit7 is 1000001 is choosen according to the 32bit high-performance
 * bus width with 2 banks, 11 row lwngth, 8 column length. Set devAddrMap with
 * 0x81.
 */
typedef struct _emc_dynamic_chip_config
{
    uint8_t chipIndex; /*!< Chip Index, range from 0 ~ EMC_DYNAMIC_MEMDEV_NUM
                          - 1. */
    emc_dynamic_device_t dynamicDevice; /*!< All chips shall use the same device
                                           setting. mixed use are not supported.
                                         */
    uint8_t rAS_Nclk;                   /*!< Active to read/write delay tRCD. */
    uint16_t sdramModeReg;              /*!< Sdram mode register setting. */
    uint16_t sdramExtModeReg; /*!< Used for low-power sdram device. The extended
                                 mode register. */
    uint8_t devAddrMap; /*!< dynamic device address mapping, choose the address
...
typedef struct _emc_static_chip_config
{
    ...
    uint32_t specailConfig;         /*!< Static configuration,a logical OR of
                                       "emc_static_special_config_t". */
    uint32_t tWaitWriteEn_Ns; /*!< The delay form chip select to write enable in
                                 unit of nanosecond. */
    uint32_t tWaitOutEn_Ns; /*!< The delay from chip selcet to output enable in
                               unit of nanosecond. */
    uint32_t tWaitReadNoPage_Ns; /*!< In No-page mode, the delay from chip
                                    select to read access in unit of nanosecond.
                                  */
    uint32_t tWaitReadPage_Ns; /*!< In page mode, the read after the first read
                                  wait states in unit of nanosecond. */
    uint32_t tWaitWrite_Ns; /*!< The delay from chip select to write access in
                               unit of nanosecond. */
 ...
 * value for all used chips must be given.
 * @param configure The EMC dynamic memory controller chip independent
 * configuration pointer. This configuration pointer is actually pointer to a
 * configration array. the array number depends on the "totalChips".
 * @param totalChips The total dynamic memory chip numbers been used or the
 * length of the "emc_dynamic_chip_config_t" type memory.
 */
 ...
 * required.
 * @param configure The EMC static memory controller chip independent
 * configuration pointer. This configuration pointer is actually pointer to a
 * configration array. the array number depends on the "totalChips".
 * @param totalChips The total static memory chip numbers been used or the
 * length of the "emc_static_chip_config_t" type memory.
 */
...
static inline void EMC_EnableDynamicMemControl( EMC_Type * base, bool enable )
{
    if( enable )
    {
        base->DYNAMICCONTROL |= ( EMC_DYNAMICCONTROL_CE_MASK |
 ...
 *
 * @param base EMC peripheral base address.
 * @param enable True enable the address mirror, false disable the address
 * mirror.
 */
