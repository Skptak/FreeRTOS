...
#define FLEXCOMM_CLOCKS                                           \
    {                                                             \
        kCLOCK_FlexComm0, kCLOCK_FlexComm1, kCLOCK_FlexComm2,     \
            ...
            kCLOCK_FlexComm6, kCLOCK_FlexComm7, kCLOCK_FlexComm8, \
            kCLOCK_FlexComm9, kCLOCK_FlexComm10                   \
    }
/*! @brief Clock ip name array for LPUART. */
#define LPUART_CLOCKS                                                       \
    {                                                                       \
        kCLOCK_MinUart0, kCLOCK_MinUart1, kCLOCK_MinUart2, kCLOCK_MinUart3, \
            kCLOCK_MinUart4, kCLOCK_MinUart5, kCLOCK_MinUart6,              \
            kCLOCK_MinUart7, kCLOCK_MinUart8, kCLOCK_MinUart9               \
    }
...
#define BI2C_CLOCKS                                                           \
    {                                                                         \
        kCLOCK_BI2c0, kCLOCK_BI2c1, kCLOCK_BI2c2, kCLOCK_BI2c3, kCLOCK_BI2c4, \
            kCLOCK_BI2c5, kCLOCK_BI2c6, kCLOCK_BI2c7, kCLOCK_BI2c8,           \
            kCLOCK_BI2c9                                                      \
    }
...
#define LPSI_CLOCKS                                                           \
    {                                                                         \
        kCLOCK_LSpi0, kCLOCK_LSpi1, kCLOCK_LSpi2, kCLOCK_LSpi3, kCLOCK_LSpi4, \
            kCLOCK_LSpi5, kCLOCK_LSpi6, kCLOCK_LSpi7, kCLOCK_LSpi8,           \
            kCLOCK_LSpi9                                                      \
    }
...
#define FLEXI2S_CLOCKS                                                      \
    {                                                                       \
        kCLOCK_FlexI2s0, kCLOCK_FlexI2s1, kCLOCK_FlexI2s2, kCLOCK_FlexI2s3, \
            kCLOCK_FlexI2s4, kCLOCK_FlexI2s5, kCLOCK_FlexI2s6,              \
            kCLOCK_FlexI2s7, kCLOCK_FlexI2s8, kCLOCK_FlexI2s9               \
    }
...
#define CTIMER_CLOCKS                                               \
    {                                                               \
        kCLOCK_Ct32b0, kCLOCK_Ct32b1, kCLOCK_Ct32b2, kCLOCK_Ct32b3, \
            kCLOCK_Ct32b4                                           \
    }
...
#define GET_ID_ITEM_SEL( connection ) \
    ( ( uint8_t ) ( ( ( ( connection ) &0xF00U ) >> 8U ) - 1U ) )
...
typedef enum _clock_attach_id
{
    ...
    kFRO12M_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 0 ) |
                          MUX_B( CM_MAINCLKSELB, 0, 0 ),
    kEXT_CLK_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 1 ) |
                           MUX_B( CM_MAINCLKSELB, 0, 0 ),
    kWDT_OSC_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 2 ) |
                           MUX_B( CM_MAINCLKSELB, 0, 0 ),
    kFRO_HF_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 3 ) |
                          MUX_B( CM_MAINCLKSELB, 0, 0 ),
    kSYS_PLL_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 0 ) |
                           MUX_B( CM_MAINCLKSELB, 2, 0 ),
    kOSC32K_to_MAIN_CLK = MUX_A( CM_MAINCLKSELA, 0 ) |
                          MUX_B( CM_MAINCLKSELB, 3, 0 ),
...
static inline void CLOCK_EnableClock( clock_ip_name_t clk )
{
    ...
    if( index < 3UL )
    {
        SYSCON->AHBCLKCTRLSET[ index ] = ( 1UL << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                               clk ) );
    }
    else
    {
        SYSCON->ASYNCAPBCTRL = SYSCON_ASYNCAPBCTRL_ENABLE( 1 );
        ASYNC_SYSCON->ASYNCAPBCLKCTRLSET = ( 1UL
...
static inline void CLOCK_DisableClock( clock_ip_name_t clk )
{
    ...
    if( index < 3UL )
    {
        SYSCON->AHBCLKCTRLCLR[ index ] = ( 1UL << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                               clk ) );
    }
    else
    {
        ASYNC_SYSCON->ASYNCAPBCLKCTRLCLR = ( 1UL
                                             << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                                    clk ) );
...
{
    return ( async_clock_src_t ) ( uint32_t ) ( ASYNC_SYSCON->ASYNCAPBCLKSELA &
                                                0x3U );
}
...
__STATIC_INLINE uint32_t CLOCK_GetEmcClkFreq( void )
{
    ...
    freqtmp = CLOCK_GetCoreSysClkFreq() /
 ...
 *  @brief	bypass	: true to bypass PLL (PLL output = PLL input, false to
 * disable bypass
 *  @return	System PLL output clock rate
 */
...
{
    return ( bool ) ( ( SYSCON->SYSPLLSTAT & SYSCON_SYSPLLSTAT_LOCK_MASK ) !=
                      0U );
}
...
{
    return ( bool ) ( ( SYSCON->USBPLLSTAT & SYSCON_USBPLLSTAT_LOCK_MASK ) !=
                      0U );
}
...
{
    return ( bool ) ( ( SYSCON->AUDPLLSTAT & SYSCON_AUDPLLSTAT_LOCK_MASK ) !=
                      0U );
}
...
#define PLL_CONFIGFLAG_FORCENOFRACT                                           \
    ( 1U << 2U ) /*!< Force non-fractional output mode, PLL output will not   \
                    use the fractional, automatic bandwidth, or \ SS hardware \
                  */
...
typedef enum _clock_usb_src
{
    ...
    kCLOCK_UsbSrcSystemPll = ( uint32_t ) kCLOCK_PllOut,     /*!< Use System PLL
                                                                output. */
    kCLOCK_UsbSrcMainClock = ( uint32_t ) kCLOCK_CoreSysClk, /*!< Use Main
                                                                clock.    */
    kCLOCK_UsbSrcUsbPll = ( uint32_t ) kCLOCK_UsbPll, /*!< Use USB PLL clock. */

    kCLOCK_UsbSrcNone = SYSCON_USB0CLKSEL_SEL(
        7U ) /*!< Use None, this may be selected in order to reduce power when
                no output is needed.. */
} clock_usb_src_t;
...
typedef struct _usb_pll_setup
{
    ...
    uint8_t psel; /*!< USB PLL control register psel:only support inter 1U 2U 4U
                     8U */
    uint8_t nsel; /*!< USB PLL control register nsel:only suppoet inter 1U 2U 3U
                     4U */
 ...
 * Fractional divider register(precise frequency)
 * @param	pSetup	: Pointer to populated PLL setup structure
 * @param flagcfg : Flag configuration for PLL config structure
 * @return	PLL_ERROR_SUCCESS on success, or PLL setup error code
 * @note	This function will power off the PLL, setup the PLL with the
