...
static uint32_t getWdtOscFreq( void )
{
    ...
    {
        div_sel = ( uint8_t ) ( ( SYSCON->WDTOSCCTRL &
...
/* Optionally enable RAM banks that may be off by default at reset */
    ...
    SYSCON->AHBCLKCTRLSET[ 0 ] = SYSCON_AHBCLKCTRL_SRAM1_MASK |
        ...
        case 0x02: /* System PLL clock (pll_clk)*/
            ...
            if( ( SYSCON->SYSPLLCTRL & SYSCON_SYSPLLCTRL_BYPASS_MASK ) == 0UL )
            {
                ...
                workRate = ( uint64_t ) ( clkRate ) *
                           ( uint64_t ) findPllMMult( SYSCON->SYSPLLCTRL,
                                                      SYSCON->SYSPLLMDEC );
                clkRate = ( uint32_t ) ( workRate / ( ( uint64_t ) postdiv ) );
                clkRate = clkRate * 2; /* PLL CCO output is divided by 2 before
                                          to M-Divider */
            }
            break;
        case 0x03: /* RTC oscillator 32 kHz output (32k_clk) */
