...
**             kHz                     : 1
**
**         Runtime setting             : period/frequency interval (continual
*setting)
...
byte TickTimer_SetPeriodTicks16( word Ticks )
{
    ...
    PE_Timer_LngMul( ( dword ) Ticks,
                     838592365,
                     &rtval ); /* Multiply given value and high speed CPU mode
                                  coefficient */
    if( PE_Timer_LngHi4( rtval[ 0 ], rtval[ 1 ], &rtword ) ) /* Is the result
                                                                greater or equal
                                                                than 65536 ? */
        rtword = 65535;  /* If yes then use maximal possible value */
    CmpHighVal = rtword; /* Store result (compare register value for high speed
                            CPU mode) to the variable CmpHighVal */
    SetCV( CmpHighVal ); /* Store appropriate value to the compare register
                            according to the selected high speed CPU mode */
    return ERR_OK;       /* OK */
}
...
byte TickTimer_SetPeriodTicks32( dword Ticks )
{
    ...
    if( ( Ticks > 320000 ) || ( Ticks < 16000 ) ) /* Is the given value out of
                                                     range? */
        return ERR_RANGE;                         /* Range error */
    PE_Timer_LngMul( Ticks, 838592365, &rtval );  /* Multiply given value and
                                                     high speed CPU mode
                                                     coefficient */
    if( PE_Timer_LngHi4( rtval[ 0 ], rtval[ 1 ], &rtword ) ) /* Is the result
                                                                greater or equal
...
byte TickTimer_SetPeriodUS( word Time )
{
    ...
    if( ( Time > 20000 ) || ( Time < 1000 ) ) /* Is the given value out of
                                                 range? */
        return ERR_RANGE;                     /* If yes then error */
    PE_Timer_LngMul( ( dword ) Time, 52412023, &rtval ); /* Multiply given value
                                                            and high speed CPU
                                                            mode coefficient */
    if( PE_Timer_LngHi3( rtval[ 0 ], rtval[ 1 ], &rtword ) ) /* Is the result
                                                                greater or equal
                                                                than 65536 ? */
        rtword = 65535;  /* If yes then use maximal possible value */
    CmpHighVal = rtword; /* Store result (compare register value for high speed
...
byte TickTimer_SetPeriodMS( word Time )
{
    ...
    if( ( Time > 20 ) || ( Time < 1 ) ) /* Is the given value out of range? */
        return ERR_RANGE;               /* If yes then error */
    PE_Timer_LngMul( ( dword ) Time,
                     204734464,
                     &rtval ); /* Multiply given value and high speed CPU mode
                                  coefficient */
    if( PE_Timer_LngHi2( rtval[ 0 ], rtval[ 1 ], &rtword ) ) /* Is the result
                                                                greater or equal
                                                                than 65536 ? */
        rtword = 65535;  /* If yes then use maximal possible value */
    CmpHighVal = rtword; /* Store result (compare register value for high speed
...
byte TickTimer_SetFreqHz( word Freq )
{
    ...
    if( ( Freq > 1000 ) || ( Freq < 50 ) ) /* Is the given value out of range?
        ...
        return ERR_RANGE;                  /* If yes then error */
    rtval[ 1 ] = 799744000 /
                 ( dword ) Freq; /* Divide high speed CPU mode coefficient by
                                    the given value */
    rtval[ 0 ] = 0;              /* Convert result to the type dlong */
    if( PE_Timer_LngHi1( rtval[ 0 ], rtval[ 1 ], &rtword ) ) /* Is the result
                                                                greater or equal
                                                                than 65536 ? */
        rtword = 65535;  /* If yes then use maximal possible value */
    CmpHighVal = rtword; /* Store result (compare register value for high speed
...
void TickTimer_Init( void )
{
    ...
    SetCV( CmpHighVal ); /* Store appropriate value to the compare register
                            according to the selected high speed CPU mode */
    SetPV( 3 ); /* Set prescaler register according to the selected high speed
                   CPU mode */
