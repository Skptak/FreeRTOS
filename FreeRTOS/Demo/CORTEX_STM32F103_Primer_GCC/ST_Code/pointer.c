...
unsigned char BallPointerBmp[ POINTER_WIDTH ] = { 0x38, 0x7C, 0xFF, 0xFF,
                                                  0xFF, 0x7C, 0x38 };
...
tPointer_Info POINTER_Info = { SCREEN_WIDTH - POINTER_WIDTH / 2,
...
static int POINTER_Move( void )
{
    ...
    if( Pointer_Mode == POINTER_APPLICATION )
    {
        ...
        {
            Application_Pointer_Mgr( POINTER_Info.shift_PosX,
                                     POINTER_Info.shift_PosY );
        }
    ...
    if( ( Pointer_Mode != POINTER_MENU ) &&
        ( Pointer_Mode != POINTER_RESTORE_LESS ) &&
        ( ( oldPointer_xPos != POINTER_Info.xPos ) ||
          ( oldPointer_yPos != POINTER_Info.yPos ) ) )
    {
        // Use default area.
        POINTER_SetCurrentAreaStore( 0 );

        // Restore previously drawn area.
        POINTER_Restore( oldPointer_xPos,
                         oldPointer_yPos,
                         POINTER_WIDTH,
                         POINTER_WIDTH );

        // Save new area and draw pointer
        POINTER_Save( POINTER_Info.xPos,
                      POINTER_Info.yPos,
                      POINTER_WIDTH,
                      POINTER_WIDTH );
        POINTER_Draw( POINTER_Info.xPos,
                      POINTER_Info.yPos,
                      POINTER_WIDTH,
                      POINTER_WIDTH,
                      CurrentPointerBmp );
 ...
 *
 *  @warning    Memory space pointed by the provided pointer must be large
 *enough to store a color bitmap corresponding to the pointer area. In other
 *words, space must be width * height * 2 large.
 *
 **/
/******************************************************************************/
...
void POINTER_SetMode( enum POINTER_mode mode )
{
    ...
    switch( mode )
    {
        case POINTER_APPLICATION:
            ...
            for( i = 0; i < ( CurrentPointerWidth * CurrentPointerHeight );
                 i++ )
            {
                *ptr++ = color;
            }
        ...
        case POINTER_MENU:
            ...
            {
                POINTER_SetCurrentAreaStore( 0 );
                POINTER_Restore( POINTER_Info.xPos,
                                 POINTER_Info.yPos,
                                 POINTER_WIDTH,
                                 POINTER_WIDTH );
            }
 ...
 *
 *  @param[in]  x        Horizontal coordinate of the bottom left corner of the
 *new area.
 *  @param[in]  y        Vertical coordinate of the bottom left corner of the
 *new are.
 ...
 *
 *  @return  The current pointer screen position with X in the LSB and Y in the
 *MSB.
 *
 *  @warning       The (0x0) point in on the low left corner.
 **/
 ...
 *  @param[in]  bmp      Pointer to width * height bit array. If null used
...
void POINTER_Draw( u8 x, u8 y, u8 width, u8 height, u8 * bmp )
{
    ...
    while( n < ( width * height ) )
    {
        ...
        {
            // POINTER_RESTORE_LESS: use current background color for background
            // color.
            c = DRAW_GetBGndColor();
            val = ( bmp[ l + ( i / 8 ) ] & ( 1 << ( 7 - ( i % 8 ) ) ) )
                      ? POINTER_GetColor()
                      : c;
 ...
 *
 *  @note          The store area must be large enough to store all the pixels
 *(16 bits).
 *  @warning       The (0x0) point in on the low left corner.
 *  @see  POINTER_Restore
...
/**
 *
 *  Restore the background of the pointer with data saved in the current store
 *area.
 *
 *  @param[in]  x        Horizontal coordinate of the bottom left corner of the
 *area to restore.
