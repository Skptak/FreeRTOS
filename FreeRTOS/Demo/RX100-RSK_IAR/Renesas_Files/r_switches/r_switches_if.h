...
/* The process of getting the version number is done through the macro below.
   The version number is encoded where the top 2 bytes are the major version
   number and the bottom 2 bytes are the minor version number. For example,
   Version 4.25 would be returned as 0x00040019. */
#define R_SWITCHES_GetVersion()                           \
    ( ( ( ( uint32_t ) SWITCHES_VERSION_MAJOR ) << 16 ) | \
      ( uint32_t ) SWITCHES_VERSION_MINOR )
