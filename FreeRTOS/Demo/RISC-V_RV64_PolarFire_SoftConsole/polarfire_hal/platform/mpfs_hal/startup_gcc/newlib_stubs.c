...
#ifdef DEBUG_HEAP_SIZE /* add this define if you want to debug crash due to \
                          overflow of  heap */
    /* fixme- this test needs to be reworked to take account of multiple harts
     * and TLS */
    stack_ptr = read_csr( sp );
    /* stack_ptr has just been placed on the stack, so its address in currently
     * pointing to the stack end */
