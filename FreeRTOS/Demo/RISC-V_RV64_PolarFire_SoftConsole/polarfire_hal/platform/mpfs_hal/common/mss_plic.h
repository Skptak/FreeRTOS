...
static inline void PLIC_init( void )
{
    ...
    switch( hart_id )
    {
        case 0:
             ...
             * PLIC provides context based threshold register for the settings
             * of a interrupt priority threshold of each context. The threshold
             * register is a WARL field. The PLIC will mask all PLIC interrupts
             * of a priority less than or equal to threshold. For example, a
             * threshold value of zero permits all interrupts with non-zero
             * priority.*/
...
/***************************************************************************/ /**
  ...
  *     handler.
  *     If you wish to disable the external interrupt while the interrupt
  handler
