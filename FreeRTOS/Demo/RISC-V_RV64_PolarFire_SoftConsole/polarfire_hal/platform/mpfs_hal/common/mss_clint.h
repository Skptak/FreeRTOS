...
typedef struct CLINT_Type_t
{
    ...
    volatile uint64_t MTIMECMP[ 5 ]; /* mtime compare value for each hart. When
                                        mtime equals this value, interrupt is
                                        generated for particular hart */
...
static inline void raise_soft_interrupt( unsigned long hart_id )
{
    ...
    /*Note:  set_csr(mie, MIP_MSIP) needs to be set on hart you are setting sw
     * interrupt */
    CLINT->MSIP[ hart_id ] = 0x01U; /*raise soft interrupt for hart(x) where x==
                                       hart ID*/
    mb();
}
...
static inline void clear_soft_interrupt( void )
{
    ...
    reg = CLINT->MSIP[ hart_id ];   /* we read back to make sure it has been
