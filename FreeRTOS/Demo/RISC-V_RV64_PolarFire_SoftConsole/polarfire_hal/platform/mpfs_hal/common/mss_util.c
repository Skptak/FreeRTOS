...
{
    set_csr( mstatus, MSTATUS_MIE ); /* mstatus Register- Machine Interrupt
                                        Enable */
}
...
{
    if( ( local_interrupt > ( int8_t ) 0 ) &&
        ( local_interrupt <= LOCAL_INT_MAX ) )
    {
        set_csr( mie,
                 ( 0x1LLU << ( int8_t ) ( local_interrupt +
                                          16U ) ) ); /* mie Register- Machine
                                                        Interrupt Enable
                                                        Register */
    }
}
