...
const uint32_t LIBERO_SETTING_CONTEXT_EN[][ 2U ] = {
    { LIBERO_SETTING_CONTEXT_A_EN, LIBERO_SETTING_CONTEXT_B_EN },
    { LIBERO_SETTING_CONTEXT_A_EN_FIC, LIBERO_SETTING_CONTEXT_B_EN_FIC },
};
...
const uint32_t PERIPHERAL_SETUP[][ 4U ] = { { MSS_PERIPH_MMUART0,
                                              CONTEXT_EN_INDEX,
 ...
 * @param option - Two option, , FIC enables set separately.
 * CONTEXT_EN_INDEX_FIC or CONTEXT_EN_INDEX
...
{
    uint8_t result = 1U;
#if( ( LIBERO_SETTING_MEM_CONFIGS_ENABLED & PMP_ENABLED_MASK ) == \
     PMP_ENABLED_MASK )
    if( hart != ( uint8_t ) 0U )
    {
        if( LIBERO_SETTING_CONTEXT_A_HART_EN & hart )
        {
            if( LIBERO_SETTING_CONTEXT_EN[ option ][ 0U ] &
                periph_context_mask )
            {
                result = 0U;
            }
        ...
        {
            if( LIBERO_SETTING_CONTEXT_EN[ option ][ 1U ] &
                periph_context_mask )
            {
                result = 0U;
            }
...
/***************************************************************************/ /**
                                                                               ...
                                                                               */
__attribute__( ( weak ) ) uint8_t mss_config_clk_rst(
    mss_peripherals peripheral,
    uint8_t hart,
    PERIPH_RESET_STATE req_state )
{
    uint8_t result = 1U;

    ASSERT( PERIPHERAL_SETUP[ peripheral ][ PERIPHERAL_INDEX_OFFSET ] ==
            peripheral );

    result = verify_context_enable(
        PERIPHERAL_SETUP[ peripheral ][ CONTEXT_EN_INDEX_OFFSET ],
        PERIPHERAL_SETUP[ peripheral ][ CONTEXT_MASK_INDEX_OFFSET ],
        hart );

    if( result == 0U )
    {
        peripheral_on_off( PERIPHERAL_SETUP[ peripheral ]
                                           [ CONTEXT_SUBCLK_INDEX_OFFSET ],
                           req_state );
    }
    return result;
}
