...
{
    const uint32_t tick_rate[ 5 ] = { HART0_TICK_RATE_MS,
                                      HART1_TICK_RATE_MS,
                                      HART2_TICK_RATE_MS,
                                      HART3_TICK_RATE_MS,
                                      HART4_TICK_RATE_MS };
    ...
    g_systick_increment[ mhart_id ] = ( ( LIBERO_SETTING_MSS_RTC_TOGGLE_CLK /
                                          1000U ) *
                                        tick_rate[ mhart_id ] );

    if( g_systick_increment[ mhart_id ] > 0ULL )
    {
        CLINT->MTIMECMP[ mhart_id ] = CLINT->MTIME +
                                      g_systick_increment[ mhart_id ];

        set_csr( mie, MIP_MTIP ); /* mie Register - Machine Timer Interrupt
                                     Enable */
...
void handle_m_timer_interrupt( void )
{
    ...
    CLINT->MTIMECMP[ read_csr( mhartid ) ] = CLINT->MTIME +
                                             g_systick_increment[ hart_id ];
