...
uint32_t sgmii_setup( void )
{
     ...
     */
    if( ( LIBERO_SETTING_SGMII_MODE &
          ( TX_RX_CH_EN_MASK << TX_RX_CH_EN_OFFSET ) ) != 0U )
    {
    ...
    switch( sgmii_training_state )
    {
        case SGMII_SETUP_INIT:
            ...
            CFG_DDR_SGMII_PHY->SOFT_RESET_SGMII
                .SOFT_RESET_SGMII = ( 0x01 << 8U ) |
                                    1U; /* PERIPH   soft reset */
             ...
             * Bring the sgmii bank controller out of reset =-
             * ioscb_bank_ctrl_sgmii
             */
        ...
        case SGMII_IO_EN:
             ...
             * This is an output from the bank controller power detector, which
             * are turned on using MSS_IO_EN
             */
            /* aro_ioen_bnk  - PVT calibrator IOEN  */
            timer_out++;
            if( ( CFG_DDR_SGMII_PHY->PVT_STAT.PVT_STAT & ( 0x01U << 6U ) ) !=
                0U )
            {
                timer_out = 0U;
        ...
        case SGMII_RAMP_TIMER:
             ...
             * After IOEN is received from power detectors DDR and SGMii, extra
             * time required for voltage to ramp. This time will come from the
             * user- Dependent on ramp time of power supply Approximately - Bank
             * power timer (from ioen_in to ioen_out = 10uS)?
             *
             */
            timer_out++;
            if( timer_out >= 0xFU )
        ...
        case SGMII_IO_SETUP:
             ...
             * Asserts a reset the SGMII block containing the MSS reference
             * clock input. Warning that setting this bit causes the external
             * reference clock input to the MSS PLL to disappear. It is
             * advisable to use the SGMII channel soft resets in the PHY instead
             * of this bit. However if E51 software wants to set this bit, the
             * MSS clock source should be switched over to the standby source in
             * advance.
             */
             ...
             */
            /*
             * so we have to use scb register to reset as no APB register
             * available to soft reset the IP ioscb_dll_sgmii
             * */
            IOSCB_DLL_SGMII->soft_reset = ( 0x01U << 0x00U ); /*  reset sgmii
                                                                 DLL */

            /*
             * I have discovered the problem with the tx channels (soft reset
             * issue) So we require the:
             *
             * sgmiiphy_lane 01 soft-reset register (0x3650_0000) to be written
             * to with 0x1 (to set the nv_map bit[0] =1 (self clears)) same for
             * sgmiiphy_lane 23 soft-reset register (0x3651_0000).
             *
             * This will result in the rpc bits for the Lane controls to get
             * loaded. Not happening currently.
             *
             * The soft_reset_sgmii occurs in the mss_ddr.c line 436, so I
             * suppose we put the 2 new soft reset writes after that.
             *
             */
            {
             ...
             */
            {
                ...
                CFG_DDR_SGMII_PHY->DYN_CNTL.DYN_CNTL = ( 0x01U << 10U ) |
                                                       ( 0x7FU << 0U );
                /* reg_pvt_soft_reset_periph  */
        ...
        case SGMII_WAIT_FOR_CALIB_COMPLETE:
            /*
             * Verify calibration
             * Bank 5 PVT calibrator can be controlled by MSS firmware through
             * APB registers to do initial calibration and re-calibration.
             * During startup, the initial calibration can be started by default
             * when MSS releases SGMII reset. Re-calibration is enabled by
             * default with reg_pvt_calib_start/lock bits being set to 1 before
             * startup, and MSS firmware can start re-calibration after startup
             * by toggling  pvt_calib_start/lock bits per PVT calibrator spec.
             *
             */
            if( ( CFG_DDR_SGMII_PHY->PVT_STAT.PVT_STAT & ( 1U << 14U ) ) ==
                ( 1U << 14U ) )
            {
        ...
        case SGMII_TURN_ON_MACS:
            /*
             * Provide mac clocks
             * The nw_config register for mac0 (0x2011_0004): I am forcing
             * 'gigabit' and 'tbi' bits = 11. The same for Mac1. This starts up
             * the tx_mac_clocks for the 2 macs.
             *
             */
            /* the mac clocks need to be turned on when setting up the sgmii */
            ( void ) mss_config_clk_rst( MSS_PERIPH_MAC0,
                                         ( uint8_t ) MPFS_HAL_FIRST_HART,
                                         PERIPHERAL_ON );
            ( void ) mss_config_clk_rst( MSS_PERIPH_MAC0,
                                         ( uint8_t ) MPFS_HAL_FIRST_HART,
                                         PERIPHERAL_ON );
            GEM_A_LO->network_config |= ( 0x01U << 10U ) |
                                        ( 0x01U << 11U ); /* GEM0 */
            GEM_B_LO->network_config |= ( 0x01U << 10U ) |
                                        ( 0x01U << 11U ); /* GEM1 */
        ...
        case SGMII_DETERMINE_SILICON_VARIANT:
            /*
             * Determine Silicon variant from generated dll generated
             * sro_dll_90_code
             */
            sro_dll_90_code = ( ( CFG_DDR_SGMII_PHY->RECAL_CNTL.RECAL_CNTL >>
                                  16U ) &
                                0x7FU );

            if( CFG_DDR_SGMII_PHY->SPARE_STAT.SPARE_STAT &
                ( 01U << 31U ) ) /* bit31 == 1 => post rev B silicon */
            {
            ...
            else
            {
                 ...
                 */
                if( sro_dll_90_code <
                    MIN_DLL_90_CODE_VALUE_INDICATING_TT_PART_REVB ) /* SS part
                                                                     */
                {
                    silicon_variant = SS_PART_REVB;
                    set_early_late_thresholds( LATE_EYE_WIDTH_SS_PART_REVB,
                                               EARLY_EYE_WIDTH_SS_PART_REVB );
                }
                else
                {
                    silicon_variant = TT_PART_REVB;
                    set_early_late_thresholds( LATE_TT_PART_REVB,
                                               EARLY_TT_PART_REVB );
                }
            }
        ...
        case SGMII_RESET_CHANNELS:
             ...
             * bit 14
             */
            CFG_DDR_SGMII_PHY->DYN_CNTL.DYN_CNTL = ( 1U << 14U ) |
                                                   ( 1U << 13U ) |
                                                   ( 0x7FU << 0U );
            CFG_DDR_SGMII_PHY->DYN_CNTL.DYN_CNTL = ( 0U << 14U ) |
                                                   ( 0U << 13U ) |
                                                   ( 0x7FU << 0U );
        ...
        case SGMII_CHECK_REVC_RESULT:
            if( ( CFG_DDR_SGMII_PHY->SPARE_STAT.SPARE_STAT &
                  ARO_REF_PCODE_MASK ) > ARO_REF_PCODE_REVC_THRESHOLD )
            {
                /* OK, we are good */
                sgmii_training_state = SGMII_CHANNELS_UP;
            }
            else
            {
                ...
                set_early_late_thresholds( LATE_EYE_WIDTH_PART_REVC_OR_LATER,
        ...
        case SGMII_CHANNELS_UP:
            /*
             * SGMii Step 4)    Monitor the DLL codes for Voltage and Temp
             * variation MSS E51 software sets the magnitude value of variation
             * to flag. MSS E51 software can poll this flag. Re-calibration, if
             * needed, is controlled by E51 software if needed. ML step 4- This
             * is a monitoring step- to be run constantly in the back ground
             */
...
void ddr_pvt_recalibration( void )
{
    ...
    {
    #if 0 /*                                                        \
           * fixme: this appears to cause wite calibration to fail, \
           * investigating                                          \
           */
        CFG_DDR_SGMII_PHY->IOC_REG0.IOC_REG0    &= ~(0x01U<<14U);
        IOSCB_IO_CALIB_DDR->IOC_REG0            &= ~(0x01U<<14U);
                                       ...
                                       uint8_t p_early_threshold )
{
     ...
     * bits 31:29 for CH1, bits 28:26  for CH0 in spare control (N eye width
     * value)
     */
