 ...
 * master_port_num = AXI Master Port number. See Enum mss_axisw_mport_t above.
 ...
 Note: QoS values are programmable through registers only for AXI3
 configurations. We have AXI4 so the QoS value programming should not be
 attempted. IF you try to write/read QoS value you will get return value =1
 (AXI_ERR_BIT)
                                  ...
                                  uint32_t data )
{
    ...
    AXISW->DATA = data &
                  AXISW_DATA_QOSVAL_MASK; /*only valid values of bits[3:0]*/

    AXISW->CMD = ( AXISW_CMD_RW_MASK | ( master_port_num << AXISW_CMD_RWCHAN ) |
                   MSS_AXISW_QOS_VAL | AXISW_CMD_EN_MASK );

    while( AXISW->CMD & AXISW_CMD_EN_MASK )
        ; /*Wait for command to complete*/

    return ( ( AXISW->CMD & AXISW_CMD_ERR_MASK ) >>
             AXISW_CMD_ERR ); /*return error bit value*/
}
...
/* Programs the peak rate and transaction rate value for the given master port
 ...
 NOTE: Peak rate and transaction rate are programmed simultaneously in one
command. So we must make sure that both desired valid values must be provided.
                               ...
                               mss_axisw_rate_t xct_rate )
{
    while( AXISW->CMD & AXISW_CMD_EN_MASK )
        ; /*make sure previous command completed*/
    AXISW->DATA = ( ( peak_rate ) << AXISW_DATA_PEAKRT ) |
                  ( ( xct_rate ) << AXISW_DATA_XCTRT );

    AXISW->CMD = ( AXISW_CMD_RW_MASK | ( master_port_num << AXISW_CMD_RWCHAN ) |
                   ( MSS_AXISW_PEAKRT_XCTRT ) | AXISW_CMD_EN_MASK );
                              ...
                              mss_axisw_rate_t * xct_rate )
{
    ...
    AXISW->CMD &= ~( AXISW_CMD_RW_MASK ); /*Clear read/write and command EN
                                             bit*/
...
/* Programs the burstiness value for the given master port read/write address
channel

burstiness_val: burstiness value to be programmed
NOTE: Burstiness value formula as mentioned in AXISW document is Burstiness =
DataReg[23:16] + 1
                                    ...
                                    uint32_t regulator_en )
{
    ...
    Burstiness value formula as mentioned in AXISW document is Burstiness =
    DataReg[23:16] + 1*/
    ...
    {
        AXISW->DATA = ( ( burstiness_val - 1u ) << AXISW_DATA_BURSTI ) |
                      ( regulator_en & 0x01 );
    }

    AXISW->CMD = ( AXISW_CMD_RW_MASK | ( master_port_num << AXISW_CMD_RWCHAN ) |
                   ( MSS_AXISW_BURSTINESS_EN ) | AXISW_CMD_EN_MASK );
...
/* Reads the burstiness value for the given master port read/write address
channel
                                    ...
                                    uint32_t * burstiness_val )
{
    ...
    *burstiness_val = ( ( AXISW->DATA & AXISW_DATA_BURSTI_MASK ) >>
                        AXISW_DATA_BURSTI ) +
                      1u;
                                      ...
                                      uint8_t slave_ready_en )
{
    ...
    AXISW->CMD = ( AXISW_CMD_RW_MASK | ( master_port_num << AXISW_CMD_RWCHAN ) |
                   MSS_AXISW_SLV_RDY | AXISW_CMD_EN_MASK );
 ...
 *
 *
 * slave_ready_en: returns the data returned by AXI SWITCH read operation for
 * slave ready command return value: As received form AXI_ERR_BIT in CMD
 * register.
 *
 */
