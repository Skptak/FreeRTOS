 ...
 *    --- Error 751: --- \n
 *    } InterruptType_Type; \n
 *    Info 751: local typedef 'InterruptType_Type' (line 170, file ./cm3_core.h)
 * not referenced \n \n \n Note:  To re-enable a Message, insert a space before
 * 'lint' * \n
 *
 */
 ...
 */
    ...
    #define NVIC_AIRCR_VECTKEY \
        ( 0x5FA << 16 )             /*!< AIRCR Key for write access   */
    #define NVIC_AIRCR_ENDIANESS 15 /*!< Endianess                    */

    /* Core Debug */
    #define CoreDebug_DEMCR_TRCENA \
        ( 1 << 24 )          /*!< DEMCR TRCENA enable          */
...
typedef struct
{
    ...
    __IO uint8_t SHP[ 12 ]; /*!< System Handlers Priority Registers (4-7, 8-11,
                               12-15)    */
...
static __INLINE void __set_CONTROL( uint32_t control )
{
    ...
    /* IAR iccarm specific functions */
            ...
            #define __NOP         __no_operation
                ...
                #if( __VER__ < 6010002 ) // If iccarm version is older
                                         // than 6.10.2 ----------
...
static uint32_t __REV16( uint32_t value )
{
    __ASM( "rev16 r0, r0" );
            ...
            #if( __CORTEX_M >= 0x03 ) // __CORTEX_M is defined in core_cm0.h,
                                      // core_cm3.h and core_cm4.h.

                #if( __VER__ < 6020000 ) // If iccarm version is older
                                         // than 6.20.0 ----------
...
static __INLINE void __CLREX( void )
{
    __ASM( "clrex" );
            ...
            #if( __CORTEX_M == 0x04 ) // __CORTEX_M is defined in core_cm0.h,
                                      // core_cm3.h and core_cm4.h.
...
{
                    #if( __FPU_PRESENT == 1 ) // __FPU_PRESENT is defined in the
                                              // device header file, if present
                                              // in current device.
    __ASM( "vmrs r0, fpscr" );
                    #else
    return ( 0 );
...
static __INLINE void NVIC_SetPriorityGrouping( uint32_t priority_grouping )
{
    ...
    reg_value = ( ( reg_value | NVIC_AIRCR_VECTKEY |
                    ( priority_grouping << 8 ) ) ); /* Insert write key and
                                                       priorty group */
    SCB->AIRCR = reg_value;
}
...
{
    return ( (
        IRQn_Type ) ( NVIC->ISPR[ ( uint32_t ) ( IRQn ) >> 5 ] &
                      ( 1 << ( ( uint32_t ) ( IRQn ) &0x1F ) ) ) ); /* Return
                                                                       Interrupt
                                                                       bit or
                                                                       'zero' */
}
...
{
    return ( (
        IRQn_Type ) ( NVIC->IABR[ ( uint32_t ) ( IRQn ) >> 5 ] &
                      ( 1 << ( (
                            uint32_t ) ( IRQn ) &0x1F ) ) ) ); /* Return
                                                                  Interruptnumber
                                                                  or
                                                                  'zero'
                                                                */
 ...
 * ############################################ */
        ...
        #define SYSTICK_ENABLE \
            0 /* Config-Bit to start or stop the SysTick Timer */
        #define SYSTICK_TICKINT \
            1 /* Config-Bit to enable or disable the SysTick interrupt */
        #define SYSTICK_CLKSOURCE                                           \
            2 /* Clocksource has the offset 2 in SysTick Control and Status \
                 Register   */
...
static __INLINE uint32_t SysTick_Config( uint32_t ticks )
{
    ...
    SysTick->CTRL = ( 1 << SYSTICK_CLKSOURCE ) | ( 1 << SYSTICK_ENABLE ) |
                    ( 1 << SYSTICK_TICKINT ); /* Enable SysTick IRQ and SysTick
                                                 Timer */
