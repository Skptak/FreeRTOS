...
/*! @brief This definition is maximum line that debugconsole can scanf each
 * time.*/
...
typedef struct DebugConsoleState
{
    ...
    hal_uart_status_t ( *putChar )( hal_uart_handle_t handle,
                                    const uint8_t * data,
                                    size_t length ); /*!< put char function
                                                        pointer */
    hal_uart_status_t ( *getChar )( hal_uart_handle_t handle,
                                    uint8_t * data,
                                    size_t length ); /*!< get char function
                                                        pointer */
    serial_port_type_t serial_port_type; /*!< The initialized port of the debug
                                            console. */
                          ...
                          uint32_t clkSrcFreq )
{
    ...
    /* Set debug console to initialized to avoid duplicated initialized
     * operation. */
    ...
    ( void ) HAL_UartInit( ( hal_uart_handle_t ) &s_debugConsole
                               .uartHandleBuffer[ 0 ],
                           &usrtConfig );
...
status_t DbgConsole_Deinit( void )
{
    ...
    ( void ) HAL_UartDeinit(
        ( hal_uart_handle_t ) &s_debugConsole.uartHandleBuffer[ 0 ] );
...
int DbgConsole_Vprintf( const char * fmt_s, va_list formatStringArg )
{
    ...
    result = DbgConsole_PrintfFormattedData( DbgConsole_Putchar,
                                             fmt_s,
                                             formatStringArg );
...
int DbgConsole_Putchar( int ch )
{
    ...
    ( void ) s_debugConsole
        .putChar( ( hal_uart_handle_t ) &s_debugConsole.uartHandleBuffer[ 0 ],
                  ( uint8_t * ) ( &ch ),
                  1 );
...
int DbgConsole_Getchar( void )
{
    ...
    while( kStatus_HAL_UartSuccess !=
           s_debugConsole.getChar( ( hal_uart_handle_t ) &s_debugConsole
                                       .uartHandleBuffer[ 0 ],
                                   ( uint8_t * ) ( &ch ),
                                   1 ) )
    {
                                                   ...
                                                   bool use_caps )
{
    ...
    *nstrp++ = '\0';
    ...
    ( void ) neg;
    /*
     * Fix MISRA issue: CID 15985711 (#15 of 15): MISRA C-2012 Control Flow
     * Expressions (MISRA C-2012 Rule 14.3) misra_c_2012_rule_14_3_violation:
     * Execution cannot reach this statement: a = *((int *)nump);
     */
    #if PRINTF_ADVANCED_ENABLE
    if( 0 != neg )
    {
        ...
        while( a != 0 )
        {
            ...
            c = ( long long int ) a -
                ( ( long long int ) b * ( long long int ) radix );
            if( c < 0 )
            {
    ...
    #endif /* PRINTF_ADVANCED_ENABLE */
    {
        ...
        {
    #if PRINTF_ADVANCED_ENABLE
            ub = ( unsigned long long int ) ua /
                 ( unsigned long long int ) radix;
            uc = ( unsigned long long int ) ua -
                 ( ( unsigned long long int ) ub *
                   ( unsigned long long int ) radix );
    #else
 ...
 * @return Length of the converted string.
 */
static int32_t DbgConsole_ConvertFloatRadixNumToString(
    char * numstr,
    void * nump,
    int32_t radix,
                                           ...
                                           va_list ap )
{
    ...
    while( true )
    {
        ...
        if( *++p == '.' )
        {
            ...
            while( !done )
            {
                ...
                if( ( c >= '0' ) && ( c <= '9' ) )
                {
                    precision_width = ( precision_width * 10U ) +
                                      ( ( uint32_t ) c - ( uint32_t ) '0' );
    #if PRINTF_ADVANCED_ENABLE
        ...
        c = *++p;
        {
                ...
                ( c == 'p' ) || ( c == 'u' ) )
            {
                if( ( c == 'd' ) || ( c == 'i' ) )
                {
    #if PRINTF_ADVANCED_ENABLE
                    if( 0U != ( flags_used &
                                ( uint32_t ) kPRINTF_LengthLongLongInt ) )
                    {
                        ival = ( long long int ) va_arg( ap, long long int );
                    }
                    else if( 0U != ( flags_used &
                                     ( uint32_t ) kPRINTF_LengthLongInt ) )
                    {
                    ...
                    vlen = DbgConsole_ConvertRadixNumToString( vstr,
                                                               &ival,
                                                               1,
                                                               10,
                                                               use_caps );
                    ...
                    if( 0U != ( flags_used & ( uint32_t ) kPRINTF_Zero ) )
                    {
                        ...
                        DbgConsole_PrintfPaddingCharacter( '0',
                                                           vlen,
                                                           ( int32_t )
                                                               field_width,
                                                           &count,
                ...
                if( ( c == 'f' ) || ( c == 'F' ) )
                {
                    ...
                    vlen = DbgConsole_ConvertFloatRadixNumToString(
                        vstr,
                        &fval,
                        10,
                        precision_width );
                ...
                if( ( c == 'X' ) || ( c == 'x' ) )
                {
                    ...
                    if( 0U != ( flags_used &
                                ( uint32_t ) kPRINTF_LengthLongLongInt ) )
                    {
                        uval = ( unsigned long long int )
                            va_arg( ap, unsigned long long int );
                    }
                    else if( 0U != ( flags_used &
                                     ( uint32_t ) kPRINTF_LengthLongInt ) )
                    {
                        uval = ( unsigned long int )
                    ...
                    ( c == 'u' ) )
                {
                    ...
                    {
                        /*
                         * Fix MISRA issue: CID 16209727 (#15 of 15): MISRA
                         * C-2012 Pointer Type Conversions (MISRA C-2012
                         * Rule 11.6)
                         * 1. misra_c_2012_rule_11_6_violation: The expression
                         * va_arg (ap, void *) of type void * is cast to type
                         * unsigned int.
                         *
...
size_t __write( int handle, const unsigned char * buffer, size_t size )
{
    ...
    {
        /* Send data. */
        ( void ) s_debugConsole.putChar( ( hal_uart_handle_t ) &s_debugConsole
                                             .uartHandleBuffer[ 0 ],
                                         buffer,
                                         size );
        ret = size;
...
size_t __read( int handle, unsigned char * buffer, size_t size )
{
    ...
    {
        /* Receive data. */
        ( void ) s_debugConsole.getChar( ( hal_uart_handle_t ) &s_debugConsole
                                             .uartHandleBuffer[ 0 ],
                                         buffer,
                                         size );
        ret = size;
...
int __attribute__( ( weak ) ) __sys_readc( void )
{
    ...
    /* Receive data. */
    s_debugConsole
        .getChar( ( hal_uart_handle_t ) &s_debugConsole.uartHandleBuffer[ 0 ],
                  ( uint8_t * ) &tmp,
                  sizeof( tmp ) );
...
/* These function __write_r and __read_r are used to support Xtensa Clang
 * toolchain to printf and scanf */
#elif defined( __XTENSA__ ) && defined( __XT_CLANG__ )
    #if defined( SDK_DEBUGCONSOLE_UART )

int __attribute__( ( weak ) )
_write_r( void * ptr, int handle, char * buffer, int size );
int __attribute__( ( weak ) )
_write_r( void * ptr, int handle, char * buffer, int size )
{
    if( NULL == buffer )
...
int __attribute__( ( weak ) )
_read_r( void * ptr, int handle, char * buffer, int size );
int __attribute__( ( weak ) )
_read_r( void * ptr, int handle, char * buffer, int size )
{
    /* This function only reads from "standard in", for all other file handles
