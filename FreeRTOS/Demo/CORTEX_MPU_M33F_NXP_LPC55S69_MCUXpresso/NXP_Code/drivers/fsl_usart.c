...
/*!
 * @brief Used for conversion from `flexcomm_usart_irq_handler_t` to
 * `flexcomm_irq_handler_t`
 */
...
static const uint32_t
    s_usartBaseAddrs[ FSL_FEATURE_SOC_USART_COUNT ] = USART_BASE_ADDRS;
 ...
 *
 * note When using the RX ring buffer, one byte is reserved for internal use. In
 ...
 * Pass NULL to disable the ring buffer. param ringBufferSize size of the ring
 * buffer.
 */
 ...
 * shows how to use this API to configure USART. code usart_config_t
 * usartConfig; usartConfig.baudRate_Bps = 115200U; usartConfig.parityMode =
 * kUSART_ParityDisabled; usartConfig.stopBitCount = kUSART_OneStopBit;
 ...
 * clock source. retval kStatus_InvalidArgument USART base address is not valid
                     ...
                     uint32_t srcClock_Hz )
{
    ...
    /* check arguments */
    assert(
        !( ( NULL == base ) || ( NULL == config ) || ( 0U == srcClock_Hz ) ) );
    ...
    USART_SetRxTimeoutConfig( base,
                              ( usart_rx_timeout_config * ) &(
                                  config->rxTimeout ) );
#endif
    /* setup configuration and enable USART */
    base->CFG = USART_CFG_PARITYSEL( config->parityMode ) |
                USART_CFG_STOPLEN( config->stopBitCount ) |
                USART_CFG_DATALEN( config->bitCountPerChar ) |
                USART_CFG_LOOP( config->loopback ) |
                USART_CFG_SYNCEN( ( uint32_t ) config->syncMode >> 1 ) |
                USART_CFG_SYNCMST( ( uint8_t ) config->syncMode ) |
                USART_CFG_CLKPOL( config->clockPolarity ) |
                USART_CFG_MODE32K( config->enableMode32k ) |
                USART_CFG_CTSEN( config->enableHardwareFlowControl ) |
 ...
 * used to calculate suitable prescaler and counter for target_us. Example below
 * shows how to use this API to configure USART. code usart_config_t config;
 *   config.rxWatermark                     = kUSART_RxFifo2;
 *   config.rxTimeout.enable                = true;
 *   config.rxTimeout.resetCounterOnEmpty   = true;
 *   config.rxTimeout.resetCounterOnReceive = true;
 *   USART_CalcTimeoutConfig(200, &config.rxTimeout.prescaler,
 * &config.rxTimeout.counter, CLOCK_GetFreq(kCLOCK_BusClk)); endcode param
 * target_us  Time for rx timeout unit us. param rxTimeoutPrescaler The
 * prescaler to be setted after function. param rxTimeoutcounter The counter to
 * be setted after function. param srcClock_Hz The clockSrc for rx timeout.
 */
...
{
    uint16_t counter = 0U;
    uint32_t perscalar = 0U, calculate_us = 0U, us_diff = 0U,
             min_diff = 0xffffffffUL;
    /* find the suitable value */
    for( perscalar = 0U; perscalar < 256U; perscalar++ )
    {
        counter = target_us * ( srcClock_Hz / 1000000UL ) /
                  ( 16U * ( perscalar + 1U ) );
        calculate_us = 16U * ( perscalar + 1U ) * counter /
                       ( srcClock_Hz / 1000000UL );
        us_diff = ( calculate_us > target_us ) ? ( calculate_us - target_us )
                                               : ( target_us - calculate_us );
        if( us_diff == 0U )
        {
...
{
    base->FIFORXTIMEOUTCFG = 0U;
    base->FIFORXTIMEOUTCFG = USART_FIFORXTIMEOUTCFG_RXTIMEOUT_COW(
                                 ~config->resetCounterOnReceive ) |
                             USART_FIFORXTIMEOUTCFG_RXTIMEOUT_COE(
                                 ~config->resetCounterOnEmpty ) |
                             USART_FIFORXTIMEOUTCFG_RXTIMEOUT_EN(
                                 config->enable ) |
                             USART_FIFORXTIMEOUTCFG_RXTIMEOUT_VALUE(
                                 config->counter ) |
 ...
 * the USART_Init. code USART_SetBaudRate(USART1, 115200U, 20000000U); endcode
 *
 * param base USART peripheral base address.
 * param baudrate_Bps USART baudrate to be set.
 * param srcClock_Hz USART clock source frequency in HZ.
 * retval kStatus_USART_BaudrateNotSupport Baudrate is not support in current
 * clock source. retval kStatus_Success Set baudrate succeed. retval
...
status_t USART_SetBaudRate( USART_Type * base,
                            ...
                            uint32_t srcClock_Hz )
{
    uint32_t best_diff = ( uint32_t ) -1, best_osrval = 0xf,
             best_brgval = ( uint32_t ) -1;
    uint32_t osrval, brgval, diff, baudrate, allowed_error;

    /* check arguments */
    assert( !( ( NULL == base ) || ( 0U == baudrate_Bps ) ||
               ( 0U == srcClock_Hz ) ) );
    ...
    {
        /* Actual baud rate must be within 3% of desired baud rate based on the
         * calculated OSR and BRG value */
        ...
        {
            /* Break if the best baudrate's diff is in the allowed error range
               and the osrval is below 8, only use lower osrval if the baudrate
               cannot be obtained with an osrval of 8 or above. */
            if( ( osrval <= 8U ) && ( best_diff <= allowed_error ) )
            {
                break;
            }

            brgval = ( ( ( srcClock_Hz * 10U ) /
                         ( ( osrval + 1U ) * baudrate_Bps ) ) -
                       5U ) /
                     10U;
            if( brgval > 0xFFFFU )
            {
            ...
            diff = ( baudrate_Bps < baudrate ) ? ( baudrate - baudrate_Bps )
        ...
        baudrate = srcClock_Hz /
                   ( ( best_osrval + 1U ) * ( best_brgval + 1U ) );
        diff = ( baudrate_Bps < baudrate ) ? ( baudrate - baudrate_Bps )
                                           : ( baudrate_Bps - baudrate );
...
void USART_Enable9bitMode( USART_Type * base, bool enable )
{
    ...
    if( enable )
    {
        ...
        temp = base->CFG & ~( ( uint32_t ) USART_CFG_DATALEN_MASK |
 ...
 * data. retval kStatus_USART_ParityError Noise error happened while receiving
 * data. retval kStatus_USART_NoiseError Framing error happened while receiving
 * data. retval kStatus_USART_RxError Overflow or underflow rxFIFO happened.
 * retval kStatus_USART_Timeout Transmission timed out and was aborted.
 ...
 * not all written to TX register yet. retval kStatus_InvalidArgument Invalid
 * argument.
 */
                                        ...
                                        usart_transfer_t * xfer )
{
    ...
    {
        /* Disable IRQ when configuring transfer handle, in case interrupt
         * occurs during the process and messes up the handle value. */
                                           ...
                                           size_t * receivedBytes )
{
    ...
    /* How to get data:
       1. If RX ring buffer is not enabled, then save xfer->data and
       xfer->dataSize to uart handle, enable interrupt to store received data to
       xfer->data. When all data received, trigger callback.
       2. If RX ring buffer is enabled and not empty, get data from ring buffer
       first. If there are enough data in ring buffer, copy them to xfer->data
       and return. If there are not enough data in ring buffer, copy all of them
       to xfer->data, save the xfer->data remained empty space to uart handle,
       receive data to this empty space and trigger callback when finished. */
        ...
        if( handle->rxRingBuffer != NULL )
        {
            ...
            if( bytesToCopy != 0U )
            {
                ...
                for( i = 0U; i < bytesToCopy; i++ )
                {
                    ...
                    /* Wrap to 0. Not use modulo (%) because it might be large
                     * and slow. */
            ...
            /* If ring buffer does not have enough data, still need to read more
             * data. */
...
void USART_TransferAbortReceive( USART_Type * base, usart_handle_t * handle )
{
    ...
    /* Only abort the receive to handle->rxData, the RX ring buffer is still
     * working. */
...
void USART_TransferHandleIRQ( USART_Type * base, usart_handle_t * handle )
{
    ...
    /* TX under run, happens when slave is in synchronous mode and the data is
     * not written in tx register in time. */
             ...
             ( ( base->FIFOSTAT & USART_FIFOSTAT_TXNOTFULL_MASK ) != 0U ) ) )
    {
            ...
            ( ( base->FIFOSTAT & USART_FIFOSTAT_RXNOTEMPTY_MASK ) != 0U ) )
        {
            ...
            else
            {
                if( handle->rxRingBuffer != NULL )
                {
                    /* If RX ring buffer is full, trigger callback to notify
                     * over run. */
                    ...
                    /* If ring buffer is still full after callback function, the
                     * oldest data is overridden. */
                    if( USART_TransferIsRxRingBufferFull( handle ) )
                    {
                        /* Increase handle->rxRingBufferTail to make room for
                         * new data. */
                        if( ( size_t ) handle->rxRingBufferTail + 1U ==
                            handle->rxRingBufferSize )
                        {
    ...
    if( NULL == handle->rxRingBuffer )
    {
        ...
        /* restore if rx transfer ends and rxLevel is different from default
         * value */
        if( ( tmpsize == 0U ) &&
            ( USART_FIFOTRIG_RXLVL_GET( base ) != handle->rxWatermark ) )
        {
            base->FIFOTRIG = ( base->FIFOTRIG &
