...
#define PLL_MAX_IN_SSMODE                                               \
    ( 100000000U ) /*!<  Not find the value in UM, Just use the maximum \
                      frequency which device support */
...
/* SSCG control1 */
#define PLL_SSCG1_MDEC_VAL_SET( value )                    \
    ( ( ( uint64_t ) ( value ) << PLL_SSCG1_MDEC_VAL_P ) & \
      PLL_SSCG1_MDEC_VAL_M )
...
#define PLL0_SSCG_MD_FRACT_SET( value )                    \
    ( ( ( uint64_t ) ( value ) << PLL0_SSCG_MD_FRACT_P ) & \
      PLL0_SSCG_MD_FRACT_M )
#define PLL0_SSCG_MD_INT_SET( value ) \
    ( ( ( uint64_t ) ( value ) << PLL0_SSCG_MD_INT_P ) & PLL0_SSCG_MD_INT_M )
    ...
    if( kNONE_to_NONE != connection )
    {
        for( i = 0U; i < 2U; i++ )
        {
            ...
            if( item != 0U )
            {
                ...
                {
                    PMC->RTCOSC32K = ( PMC->RTCOSC32K &
                                       ~PMC_RTCOSC32K_SEL_MASK ) |
                                     PMC_RTCOSC32K_SEL( sel );
                }
                else
                {
                    pClkSel[ mux ] = sel;
                      ...
                      bool reset )
{
    ...
    if( ( div_name >= kCLOCK_DivFlexFrg0 ) &&
        ( div_name <= kCLOCK_DivFlexFrg7 ) )
    {
        /*!<  Flexcomm Interface function clock = (clock selected via FCCLKSEL)
         * / (1+ MULT /DIV), DIV = 0xFF */
        ( ( volatile uint32_t * )
              pClkDiv )[ ( uint8_t ) div_name ] = SYSCON_FLEXFRG0CTRL_DIV_MASK |
                                                  SYSCON_FLEXFRG0CTRL_MULT(
                                                      divided_by_value );
    }
    else
    {
        ...
        {
            ( ( volatile uint32_t * )
                  pClkDiv )[ ( uint32_t ) div_name ] = 1UL << 29U;
        }
        if( divided_by_value == 0U ) /*!<  halt */
        {
            ( ( volatile uint32_t * )
                  pClkDiv )[ ( uint32_t ) div_name ] = 1UL << 30U;
        }
        else
        {
            ( ( volatile uint32_t * )
                  pClkDiv )[ ( uint32_t ) div_name ] = ( divided_by_value -
                                                         1U );
        }
    }
}
...
void CLOCK_SetRtc1khzClkDiv( uint32_t divided_by_value )
{
    PMC->RTCOSC32K |= ( ( ( divided_by_value - 28U )
                          << PMC_RTCOSC32K_CLK1KHZDIV_SHIFT ) |
                        PMC_RTCOSC32K_CLK1KHZDIV_MASK );
}
...
void CLOCK_SetRtc1hzClkDiv( uint32_t divided_by_value )
{
    ...
    {
        PMC->RTCOSC32K |= ( ( ( divided_by_value - 31744U )
                              << PMC_RTCOSC32K_CLK1HZDIV_SHIFT ) |
...
status_t CLOCK_SetupFROClocking( uint32_t iFreq )
{
    ...
    SYSCON->PRESETCTRLCLR[ 2 ] = ( 1UL
                                   << SYSCON_PRESETCTRL2_ANALOG_CTRL_RST_SHIFT );
...
{
    uint32_t num_wait_states; /* Flash Controller & FMC internal number of Wait
                                 States (minus 1) */
    ...
    FLASH->DATAW[ 0 ] = ( FLASH->DATAW[ 0 ] & 0xFFFFFFF0UL ) |
                        ( num_wait_states & ( SYSCON_FMCCR_FLASHTIM_MASK >>
 ...
 * Option for external clock input (bypass mode) for clock frequencies of up to
 * 25 MHz. param   iFreq   : Desired frequency (must be equal to exact rate in
 * Hz) return  returns success or fail status.
 */
...
{
    return ( ( ANACTRL->XO32M_CTRL &
               ANACTRL_XO32M_CTRL_ENABLE_PLL_USB_OUT_MASK ) != 0UL )
               ? s_Ext_Clk_Freq
               : 0U;
}
...
{
    return ( ( ANACTRL->FRO192M_CTRL &
               ANACTRL_FRO192M_CTRL_ENA_12MHZCLK_MASK ) != 0UL )
               ? 12000000U
               : 0U;
}
...
{
    return ( ( SYSCON->CLOCK_CTRL & SYSCON_CLOCK_CTRL_FRO1MHZ_CLK_ENA_MASK ) !=
             0UL )
               ? 1000000U
               : 0U;
}
...
{
    return ( ( ANACTRL->XO32M_CTRL &
               ANACTRL_XO32M_CTRL_ENABLE_SYSTEM_CLK_OUT_MASK ) != 0UL )
               ? s_Ext_Clk_Freq
               : 0U;
}
...
{
    return ( ( ANACTRL->FRO192M_CTRL &
               ANACTRL_FRO192M_CTRL_ENA_96MHZCLK_MASK ) != 0UL )
               ? 96000000U
               : 0U;
}
...
uint32_t CLOCK_GetOsc32KFreq( void )
{
    return ( ( 0UL == ( PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_FRO32K_MASK ) ) &&
             ( 0UL == ( PMC->RTCOSC32K & PMC_RTCOSC32K_SEL_MASK ) ) )
               ? CLK_RTC_32K_CLK
           : ( ( 0UL ==
                 ( PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_XTAL32K_MASK ) ) &&
               ( 0UL != ( PMC->RTCOSC32K & PMC_RTCOSC32K_SEL_MASK ) ) )
               ? CLK_RTC_32K_CLK
               : 0U;
}
...
uint32_t CLOCK_GetFlexCommClkFreq( uint32_t id )
{
    ...
    frgMul = ( SYSCON->FLEXFRGXCTRL[ id ] & SYSCON_FLEXFRG0CTRL_MULT_MASK ) >>
             8U;
    frgDiv = SYSCON->FLEXFRGXCTRL[ id ] & SYSCON_FLEXFRG0CTRL_DIV_MASK;
    return (
        uint32_t ) ( ( ( uint64_t ) freq * ( ( uint64_t ) frgDiv + 1ULL ) ) /
                     ( frgMul + frgDiv + 1UL ) );
}
                        ...
                        uint32_t * pSelR )
{
    ...
    {
        /* Note: If the spread spectrum mode, choose N to ensure 3 MHz < Fin/N <
         * 5 MHz */
        *pSelP = 3U;
        *pSelI = 4U;
        *pSelR = 4U;
...
static float findPll0MMult( void )
{
    ...
    {
        mMult_int = ( ( SYSCON->PLL0SSCG1 & SYSCON_PLL0SSCG1_MD_MBS_MASK )
                      << 7U );
 ...
 * In this function, the it calculates the PLL0 setting for output frequency
 * from input clock frequency. The calculation would cost a few time. So it is
 * not recommaned to use it frequently. the "pllctrl", "pllndec", "pllpdec",
                                                ...
                                                bool useSS )
{
    ...
    if( useSS == false )
    {
         ...
         */
        pSetup->pllsscg
            [ 1 ] = ( uint32_t ) ( ( PLL_SSCG1_MDEC_VAL_SET( pllMultiplier ) ) |
    ...
    else
    {
        ...
        fc = ( ( ( uint64_t ) fccoHz % ( uint64_t ) nDivOutHz ) << 25U ) /
             nDivOutHz;

        /* Set multiplier */
        pSetup->pllsscg[ 0 ] = ( uint32_t ) ( PLL0_SSCG_MD_INT_SET(
                                                  pllMultiplier ) |
                                              PLL0_SSCG_MD_FRACT_SET(
                                                  ( uint32_t ) fc ) );
        pSetup->pllsscg[ 1 ] = ( uint32_t ) ( PLL0_SSCG_MD_INT_SET(
                                                  pllMultiplier ) >>
                                              32U );
    }
    ...
    pSetup
        ->pllctrl = ( pllSelR
                      << SYSCON_PLL0CTRL_SELR_SHIFT ) | /* Filter coefficient */
                    ( pllSelI
                      << SYSCON_PLL0CTRL_SELI_SHIFT ) | /* Filter coefficient */
                    ( pllSelP
                      << SYSCON_PLL0CTRL_SELP_SHIFT ) | /* Filter coefficient */
                    ( 0UL << SYSCON_PLL0CTRL_BYPASSPLL_SHIFT ) | /* PLL bypass
                                                                    mode
                                                                    disabled */
                    ( uplimoff
                      << SYSCON_PLL0CTRL_LIMUPOFF_SHIFT ) | /* SS/fractional
                                                               mode disabled */
                    ( pllDirectInput
                      << SYSCON_PLL0CTRL_BYPASSPREDIV_SHIFT ) | /* Bypass
                                                                   pre-divider?
                                                                 */
                    ( pllDirectOutput
                      << SYSCON_PLL0CTRL_BYPASSPOSTDIV_SHIFT ) | /* Bypass
                                                                    post-divider?
                                                                  */
                    ( 1UL << SYSCON_PLL0CTRL_CLKEN_SHIFT ); /* Ensure the PLL
                                                               clock output */
                                        ...
                                        bool useSS )
{
    ...
    for( i = 0U; i < CLOCK_USR_CFG_PLL_CONFIG_CACHE_COUNT; i++ )
    {
        if( ( finHz == s_FinHzCache[ i ] ) &&
            ( foutHz == s_FoutHzCache[ i ] ) && ( useSS == s_UseSSCache[ i ] ) )
        {
    ...
    s_PllSetupCacheStruct[ s_PllSetupCacheIdx ]
        .pllsscg[ 0 ] = pSetup->pllsscg[ 0 ];
    s_PllSetupCacheStruct[ s_PllSetupCacheIdx ]
        .pllsscg[ 1 ] = pSetup->pllsscg[ 1 ];
 ...
 * generate setup with param    pSetup      : Pointer to PLL setup structure to
 * be filled return   PLL_ERROR_SUCCESS on success, or PLL setup error code note
 * Actual frequency for setup may vary from the desired frequency based on the
...
pll_error_t CLOCK_SetupPLL0Data( pll_config_t * pControl, pll_setup_t * pSetup )
{
    ...
    /* PLL flag options */
    pllError = CLOCK_GetPll0Config( inRate,
...
pll_error_t CLOCK_SetupPLL0Prec( pll_setup_t * pSetup, uint32_t flagcfg )
{
    ...
    SYSCON->PLL0NDEC = pSetup->pllndec |
                       ( 1UL << SYSCON_PLL0NDEC_NREQ_SHIFT ); /* latch */
    SYSCON->PLL0PDEC = pSetup->pllpdec;
    SYSCON->PLL0PDEC = pSetup->pllpdec |
                       ( 1UL << SYSCON_PLL0PDEC_PREQ_SHIFT ); /* latch */
    ...
    if( ( pSetup->flags & PLL_SETUPFLAG_WAITLOCK ) != 0U )
    {
            ...
            0UL ) /* normal mode */
        {
            ...
            /* The lock signal is only reliable between fref[2] :100 kHz to 20
             * MHz. */
            ...
            else
            {
                SDK_DelayAtLeastUs(
                    6000U,
                    SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY ); /* software should
                                                                 use a 6 ms time
                                                                 interval to
                                                                 insure the PLL
                                                                 will be stable
        ...
        else /* spread spectrum mode */
        {
            SDK_DelayAtLeastUs(
                6000U,
                SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY ); /* software should use
                                                             a 6 ms time
                                                             interval to insure
                                                             the PLL will be
                                                             stable */
...
pll_error_t CLOCK_SetPLL1Freq( const pll_setup_t * pSetup )
{
    ...
    SYSCON->PLL1NDEC = pSetup->pllndec |
                       ( 1UL << SYSCON_PLL1NDEC_NREQ_SHIFT ); /* latch */
    SYSCON->PLL1PDEC = pSetup->pllpdec;
    SYSCON->PLL1PDEC = pSetup->pllpdec |
                       ( 1UL << SYSCON_PLL1PDEC_PREQ_SHIFT ); /* latch */
    SYSCON->PLL1MDEC = pSetup->pllmdec;
    SYSCON->PLL1MDEC = pSetup->pllmdec |
                       ( 1UL << SYSCON_PLL1MDEC_MREQ_SHIFT ); /* latch */
...
void CLOCK_SetupPLL0Mult( uint32_t multiply_by, uint32_t input_freq )
{
    ...
    {
        seli = 2UL * ( ( uint32_t ) ( multiply_by / 4UL ) ) +
               3UL; /* 2*floor(M/4) + 3 */
    }
    ...
    SYSCON->PLL0PDEC = pdec |
                       ( 1UL << SYSCON_PLL0PDEC_PREQ_SHIFT ); /* set Pdec value
                                                                 and assert preq
                                                               */
    SYSCON->PLL0NDEC = ndec |
                       ( 1UL << SYSCON_PLL0NDEC_NREQ_SHIFT ); /* set Pdec value
                                                                 and assert preq
                                                               */
    SYSCON->PLL0SSCG1 = mdec |
                        ( 1UL << SYSCON_PLL0SSCG1_MREQ_SHIFT ); /* select non
                                                                   sscg MDEC
                                                                   value, assert
                                                                   mreq and
                                                                   select mdec
...
bool CLOCK_EnableUsbfs0DeviceClock( clock_usbfs_src_t src, uint32_t freq )
{
    ...
    else
    {
        ...
        SYSCON->CLOCK_CTRL |= SYSCON_CLOCK_CTRL_CLKIN_ENA_MASK; /* Enable clk_in
                                                                   from XTAL32M
                                                                   clock  */
        ANACTRL->XO32M_CTRL |=
            ANACTRL_XO32M_CTRL_ENABLE_SYSTEM_CLK_OUT_MASK; /* Enable
                                                              clk_in
                                                              to
                                                              system
                                                            */
...
bool CLOCK_EnableUsbhs0PhyPllClock( clock_usb_phy_src_t src, uint32_t freq )
{
    ...
    if( ret )
    {
        USBPHY->PLL_SIC = ( USBPHY->PLL_SIC &
                            ~USBPHY_PLL_SIC_PLL_DIV_SEL( 0x7 ) ) |
                          phyPllDiv;
        USBPHY->PLL_SIC_SET = USBPHY_PLL_SIC_SET_PLL_REG_ENABLE_MASK;
        USBPHY->PLL_SIC_CLR = ( 1UL << 16U ); // Reserved. User must set this
                                              // bit to 0x0
...
bool CLOCK_EnableUsbhs0DeviceClock( clock_usbhs_src_t src, uint32_t freq )
{
    ...
    SYSCON->CLOCK_CTRL |= SYSCON_CLOCK_CTRL_CLKIN_ENA_MASK; /* Enable clock_in
                                                               clock for clock
                                                               module. */
