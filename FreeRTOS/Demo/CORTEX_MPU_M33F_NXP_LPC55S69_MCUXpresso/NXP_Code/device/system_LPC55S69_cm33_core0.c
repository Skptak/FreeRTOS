...
**                          MCUXpresso Compiler
**
**     Reference manual:    LPC55S6x/LPC55S2x/LPC552x User manual(UM11126)
*Rev.1.3  16 May 2019
...
__attribute__( ( weak ) ) void SystemInit( void )
{
    ...
    SCB->CPACR |= ( ( 3UL << 10 * 2 ) | ( 3UL << 11 * 2 ) ); /* set CP10, CP11
                                                                Full Access in
                                                                Secure mode */
    #if defined( __ARM_FEATURE_CMSE ) && ( __ARM_FEATURE_CMSE == 3U )
    SCB_NS->CPACR |= ( ( 3UL << 10 * 2 ) |
                       ( 3UL << 11 * 2 ) ); /* set CP10, CP11 Full Access in
                                               Non-secure mode */
    #endif /* (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U) */
#endif     /* ((__FPU_PRESENT == 1) && (__FPU_USED == 1)) */

    SCB->CPACR |= ( ( 3UL << 0 * 2 ) |
                    ( 3UL << 1 * 2 ) ); /* set CP0, CP1 Full Access in Secure
                                           mode (enable PowerQuad) */
#if defined( __ARM_FEATURE_CMSE ) && ( __ARM_FEATURE_CMSE == 3U )
    SCB_NS->CPACR |= ( ( 3UL << 0 * 2 ) |
                       ( 3UL << 1 * 2 ) ); /* set CP0, CP1 Full Access in Normal
                                              mode (enable PowerQuad) */
#endif /* (__ARM_FEATURE_CMSE) && (__ARM_FEATURE_CMSE == 3U) */

    SCB->NSACR |= ( ( 3UL << 0 ) | ( 3UL << 10 ) ); /* enable CP0, CP1, CP10,
                                                       CP11 Non-secure Access */
...
void SystemCoreClockUpdate( void )
{
    ...
    switch( SYSCON->MAINCLKSELB & SYSCON_MAINCLKSELB_SEL_MASK )
    {
        ...
        case 0x01: /* PLL0 clock (pll0_clk)*/
            ...
            if( ( ( SYSCON->PLL0CTRL & SYSCON_PLL0CTRL_BYPASSPLL_MASK ) ==
                  0UL ) &&
                ( ( SYSCON->PLL0CTRL & SYSCON_PLL0CTRL_CLKEN_MASK ) != 0UL ) &&
                ( ( PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_PLL0_MASK ) == 0UL ) &&
                ( ( PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_PLL0_SSCG_MASK ) ==
                  0UL ) )
            {
        ...
        case 0x02: /* PLL1 clock (pll1_clk)*/
            ...
            if( ( ( SYSCON->PLL1CTRL & SYSCON_PLL1CTRL_BYPASSPLL_MASK ) ==
                  0UL ) &&
                ( ( SYSCON->PLL1CTRL & SYSCON_PLL1CTRL_CLKEN_MASK ) != 0UL ) &&
                ( ( PMC->PDRUNCFG0 & PMC_PDRUNCFG0_PDEN_PLL1_MASK ) == 0UL ) )
            {
                /* PLL is not in bypass mode, get pre-divider, post-divider, and
                 * M divider */
