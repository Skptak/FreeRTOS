...
static USART_Type * const s_UsartAdapterBase[] = USART_BASE_PTRS;
    ...
    #if !( defined( HAL_UART_TRANSFER_MODE ) && \
           ( HAL_UART_TRANSFER_MODE > 0U ) )
                              ...
                              void * callbackParam )
{
    ...
    {
        uartHandle->callback( uartHandle,
                              uartStatus,
...
static void HAL_UartInterruptHandle( USART_Type * base, void * handle )
{
    ...
    if( ( 0U != ( USART_FIFOSTAT_RXNOTEMPTY_MASK & status ) ) &&
        ( 0U != ( USART_GetEnabledInterrupts( s_UsartAdapterBase[ instance ] ) &
                  USART_FIFOINTENSET_RXLVL_MASK ) ) )
    {
        if( NULL != uartHandle->rx.buffer )
        {
            uartHandle->rx
                .buffer[ uartHandle->rx.bufferSofar++ ] = USART_ReadByte(
                  ...
                  USART_FIFOINTENSET_TXLVL_MASK ) ) )
    {
        if( NULL != uartHandle->tx.buffer )
        {
            USART_WriteByte( s_UsartAdapterBase[ instance ],
                             uartHandle->tx
                                 .buffer[ uartHandle->tx.bufferSofar++ ] );
                                             ...
                                             const hal_uart_config_t * config )
{
    ...
    status = USART_Init( s_UsartAdapterBase[ config->instance ],
                         &usartConfig,
                         config->srcClock_Hz );
                                ...
                                const hal_uart_config_t * config )
{
    ...
    USART_TransferCreateHandle( s_UsartAdapterBase[ config->instance ],
                                ...
                                ( usart_transfer_callback_t ) HAL_UartCallback,
                                handle );
    #else
    /* Enable interrupt in NVIC. */
    FLEXCOMM_SetIRQHandler( s_UsartAdapterBase[ config->instance ],
                            HAL_UartInterruptHandle_Wapper,
                            handle );
    NVIC_SetPriority( ( IRQn_Type ) s_UsartIRQ[ config->instance ],
                                           ...
                                           size_t length )
{
    ...
    status = USART_ReadBlocking( s_UsartAdapterBase[ uartHandle->instance ],
                                 data,
                                 length );
                                        ...
                                        size_t length )
{
    ...
    ( void ) USART_WriteBlocking( s_UsartAdapterBase[ uartHandle->instance ],
                                  data,
                                  length );
    ...
    hal_uart_transfer_t * transfer )
{
    ...
    status = USART_TransferReceiveNonBlocking(
        s_UsartAdapterBase[ uartHandle->instance ],
        &uartHandle->hardwareHandle,
    ...
    hal_uart_transfer_t * transfer )
{
    ...
    status = USART_TransferSendNonBlocking(
        s_UsartAdapterBase[ uartHandle->instance ],
        &uartHandle->hardwareHandle,
        ( usart_transfer_t * ) transfer );
                                                   ...
                                                   uint32_t * count )
{
    ...
    status = USART_TransferGetReceiveCount(
        s_UsartAdapterBase[ uartHandle->instance ],
        &uartHandle->hardwareHandle,
        count );
                                                ...
                                                uint32_t * count )
{
    ...
    status = USART_TransferGetSendCount(
        s_UsartAdapterBase[ uartHandle->instance ],
        &uartHandle->hardwareHandle,
        count );
...
void HAL_UartIsrFunction( hal_uart_handle_t handle )
{
    ...
    DisableIRQ(s_UsartIRQ[uartHandle->instance]);
        #endif
    HAL_UartInterruptHandle( s_UsartAdapterBase[ uartHandle->instance ],
                             ( void * ) uartHandle );
        #if 0
    NVIC_SetPriority((IRQn_Type)s_UsartIRQ[uartHandle->instance], HAL_UART_ISR_PRIORITY);
    EnableIRQ(s_UsartIRQ[uartHandle->instance]);
                                ...
                                void * userData )
{
    ...
    if( NULL != uartDmaHandle->dma_callback )
    {
        ...
        uartDmaHandle->dma_callback( uartDmaHandle,
                                     &msg,
                                     uartDmaHandle->dma_callback_param );
    }
}
...
static void TimeoutTimer_Callbcak( void * param )
{
    ...
    while( NULL != uartDmaHandle )
    {
            ...
            ( false == uartDmaHandle->dma_rx.receiveAll ) )
        {
            ...
            if( ( 0U == newReceived ) &&
                ( 0U < uartDmaHandle->dma_rx.bufferSofar ) )
            {
                uartDmaHandle->dma_rx.timeout++;
                if( uartDmaHandle->dma_rx.timeout >=
                    HAL_UART_DMA_IDLELINE_TIMEOUT )
                {
                    /* HAL_UartDMAAbortReceive(uartDmaHandle); */
                    USART_TransferAbortReceiveDMA(
                        s_UsartAdapterBase[ uartDmaHandle->instance ],
                        &uartDmaHandle->dmaHandle );
                    msg.data = uartDmaHandle->dma_rx.buffer;
                                       ...
                                       hal_uart_dma_config_t * dmaConfig )
{
    ...
    DMA_EnableChannel( dmaBases[ dmaConfig->dma_instance ],
                       dmaConfig->tx_channel );
    DMA_EnableChannel( dmaBases[ dmaConfig->dma_instance ],
                       dmaConfig->rx_channel );

    DMA_CreateHandle( &uartDmaHandle->txDmaHandle,
                      dmaBases[ dmaConfig->dma_instance ],
                      dmaConfig->tx_channel );
    DMA_CreateHandle( &uartDmaHandle->rxDmaHandle,
                      dmaBases[ dmaConfig->dma_instance ],
                      dmaConfig->rx_channel );
    ...
    if( 0U == s_dmaHandleList.activeCount )
    {
        ...
        timerStatus = TM_Open(
            ( timer_handle_t ) s_dmaHandleList.timerManagerHandle );
        assert( kStatus_TimerSuccess == timerStatus );

        timerStatus = TM_InstallCallback( ( timer_handle_t ) s_dmaHandleList
                                              .timerManagerHandle,
                                          TimeoutTimer_Callbcak,
                                          NULL );
        assert( kStatus_TimerSuccess == timerStatus );

        ( void ) TM_Start( ( timer_handle_t ) s_dmaHandleList.timerManagerHandle,
...
hal_uart_dma_status_t HAL_UartDMADeinit( hal_uart_handle_t handle )
{
    ...
    if( NULL != uartDmaHandle->dmaHandle.txDmaHandle )
    {
        DMA_DisableChannel( uartDmaHandle->txDmaHandle.base,
                            uartDmaHandle->txDmaHandle.channel );
    }
    if( NULL != uartDmaHandle->dmaHandle.rxDmaHandle )
    {
        DMA_DisableChannel( uartDmaHandle->rxDmaHandle.base,
                            uartDmaHandle->rxDmaHandle.channel );
    }
    ...
    s_dmaHandleList.activeCount = ( s_dmaHandleList.activeCount > 0 )
                                      ? ( s_dmaHandleList.activeCount - 1 )
                                      : 0;
    ...
    void * callbackParam )
{
    ...
    USART_TransferCreateHandleDMA( s_UsartAdapterBase[ uartDmaHandle->instance ],
                                   &uartDmaHandle->dmaHandle,
                                   USART_DMACallbacks,
                                   uartDmaHandle,
                                   &uartDmaHandle->txDmaHandle,
                                                  ...
                                                  bool receiveAll )
{
    ...
    USART_TransferReceiveDMA( s_UsartAdapterBase[ uartDmaHandle->instance ],
                              &uartDmaHandle->dmaHandle,
                              &xfer );
                                               ...
                                               size_t length )
{
    ...
    USART_TransferSendDMA( s_UsartAdapterBase[ uartDmaHandle->instance ],
                           &uartDmaHandle->dmaHandle,
                           &xfer );
                                                  ...
                                                  uint32_t * reCount )
{
    ...
    if( kStatus_Success != USART_TransferGetReceiveCountDMA(
                               s_UsartAdapterBase[ uartDmaHandle->instance ],
                               &uartDmaHandle->dmaHandle,
                               reCount ) )
    {
...
hal_uart_dma_status_t HAL_UartDMAAbortSend( hal_uart_handle_t handle )
{
    ...
    USART_TransferAbortSendDMA( s_UsartAdapterBase[ uartDmaHandle->instance ],
                                &uartDmaHandle->dmaHandle );
