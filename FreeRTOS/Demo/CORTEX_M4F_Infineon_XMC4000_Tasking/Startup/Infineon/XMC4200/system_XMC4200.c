...
void SystemCoreClockUpdate( void )
{
    ...
    if( SCU_CLK->SYSCLKCR == 0x00010000 )
    {
        if( SCU_PLL->PLLSTAT & SCU_PLL_PLLSTAT_VCOLOCK_Msk )
        {
            ...
            PDIV = ( ( SCU_PLL->PLLCON1 & SCU_PLL_PLLCON1_PDIV_Msk ) >> 24 ) +
                   1;
            NDIV = ( ( SCU_PLL->PLLCON1 & SCU_PLL_PLLCON1_NDIV_Msk ) >> 8 ) + 1;
            K2DIV = ( ( SCU_PLL->PLLCON1 & SCU_PLL_PLLCON1_K2DIV_Msk ) >> 16 ) +
                    1;
...
static int SystemClockSetup( void )
{
    ...
    if( AllowPLLInitByStartup() )
    {
        ...
        if( SCU_PLL_CLOCK_INPUT == SCU_CLOCK_CRYSTAL )
        {
            ...
            {
                SCU_OSC->OSCHPCTRL &= ~( SCU_OSC_HP_MODE ); /*enable the
                                                               OSC_HP*/
        ...
        else if( SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_AUTOMATIC )
        {
            ...
            if( ( SCU_POWER->PWRSTAT & SCU_POWER_PWRSTAT_HIBEN_Msk ) == 0 )
                SCU_POWER->PWRSET |= SCU_POWER_PWRSET_HIB_Msk; /*enable
                                                                  Hibernate
                                                                  domain*/

            /* check for HIB Domain is not in reset state  */
            if( ( SCU_RESET->RSTSTAT & SCU_RESET_RSTSTAT_HIBRS_Msk ) == 1 )
                SCU_RESET->RSTCLR |= SCU_RESET_RSTCLR_HIBRS_Msk; /*de-assert
                                                                    hibernate
                                                                    reset*/
            ...
            else if( SCU_STANDBY_CLOCK == HIB_CLOCK_OSCULP )
            {
                ...
                if( ( SCU_HIBERNATE->OSCULCTRL &
                      SCU_HIBERNATE_OSCULCTRL_MODE_Msk ) != 0 )
                {
                    while( SCU_GENERAL->MIRRSTS &
                           SCU_GENERAL_MIRRSTS_OSCULCTRL_Msk )
                        ;

                    SCU_HIBERNATE
                        ->OSCULCTRL &= ~SCU_HIBERNATE_OSCULCTRL_MODE_Msk; /*enable
                                                                             OSCUL*/
                    /*now ceck if the clock is OK using OSCULP Oscillator
                     * Watchdog (ULPWDG)*/
                    ...
                    do
                    {
                        ...
                        while( SCU_GENERAL->MIRRSTS &
                               SCU_GENERAL_MIRRSTS_HDCLR_Msk )
                            ;
                        for( temp = 0; temp <= 0xFFFF; temp++ )
                            ;
                    } while( ( SCU_HIBERNATE->HDSTAT &
        ...
        if( !( SCU_PLL->PLLSTAT & SCU_PLL_PLLSTAT_VCOLOCK_Msk ) )
        {
            ...
            if( ( SCU_CLK->SYSCLKCR & SCU_CLK_SYSCLKCR_SYSSEL_Msk ) != 0x0 )
                SCU_CLK->SYSCLKCR &= ~SCU_CLK_SYSCLKCR_SYSSEL_Msk; /*Select
                                                                      FOFI*/

            /*calulation for stepping*/
            if( SCU_PLL_CLOCK_INPUT == SCU_CLOCK_CRYSTAL )
                VCO = ( CLOCK_CRYSTAL_FREQUENCY / ( SCU_PLL_PDIV + 1 ) ) *
                      ( SCU_PLL_NDIV + 1 );
            if( ( SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_AUTOMATIC ) ||
                ( SCU_PLL_CLOCK_INPUT == SCU_CLOCK_BACK_UP_FACTORY ) )
                VCO = ( CLOCK_BACK_UP / ( SCU_PLL_PDIV + 1 ) ) *
                      ( SCU_PLL_NDIV + 1 );
            ...
            /********************************/
            SysTick->LOAD = ( ( 5000000 + 100 ) & SysTick_LOAD_RELOAD_Msk ) -
                            1; /* set reload register */
            SysTick->VAL = 0;  /* Load the SysTick Counter Value */
            SysTick->CTRL = SysTick_CTRL_CLKSOURCE_Msk |
            ...
            *********************************************************/
            ...
            /*********************************************************/
            SysTick->LOAD = ( ( 1250 + 100 ) & SysTick_LOAD_RELOAD_Msk ) -
                            1; /* set reload register */
            SysTick->VAL = 0;  /* Load the SysTick Counter Value */
            SysTick->CTRL = SysTick_CTRL_CLKSOURCE_Msk |
                            SysTick_CTRL_ENABLE_Msk; /* Enable SysTick IRQ and
            ...
            *********************************************************/
            ...
            else
            {
                ...
                SCU_PLL->PLLCON1 = ( ( SCU_PLL_K1DIV ) | ( SCU_PLL_NDIV << 8 ) |
                                     ...
                                     ( SCU_PLL_PDIV << 24 ) );
                SCU_TRAP
                    ->TRAPCLR = SCU_TRAP_TRAPCLR_SOSCWDGT_Msk |
                                SCU_TRAP_TRAPCLR_SVCOLCKT_Msk; /* clear request
                                                                  for System OCS
                                                                  Watchdog Trap
                                                                  and System VCO
                                                                  Lock Trap  */
...
static int USBClockSetup( void )
{
    ...
    if( AllowPLLInitByStartup() )
    {
        ...
        /* Setup devider settings for main PLL */
        SCU_PLL->USBPLLCON = ( ( SCU_USBPLL_NDIV << 8 ) |
                               ( SCU_USBPLL_PDIV << 24 ) );
