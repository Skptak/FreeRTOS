...
#define CAN_RF0R_RFOM0 \
    ( ( u32 ) 0x00000020 ) /* Release FIFO 0 output mailbox */
...
#define CAN_RF1R_RFOM1 \
    ( ( u32 ) 0x00000020 ) /* Release FIFO 1 output mailbox */
...
u8 CAN_Init( CAN_InitTypeDef * CAN_InitStruct )
{
    ...
    else
    {
        ...
        CAN->BTR = ( u32 ) ( ( u32 ) CAN_InitStruct->CAN_Mode << 30 ) |
                   ...
                   ( ( u32 ) CAN_InitStruct->CAN_BS1 << 16 ) |
                   ( ( u32 ) CAN_InitStruct->CAN_BS2 << 20 ) |
...
void CAN_FilterInit( CAN_FilterInitTypeDef * CAN_FilterInitStruct )
{
    ...
    FilterNumber_BitPos = ( u16 ) ( ( u16 ) 0x0001
                                    << ( ( u16 ) CAN_FilterInitStruct
                                             ->CAN_FilterNumber ) );
    ...
    if( CAN_FilterInitStruct->CAN_FilterScale == CAN_FilterScale_16bit )
    {
        ...
        CAN->sFilterRegister[ CAN_FilterInitStruct->CAN_FilterNumber ]
            .FR1 = ( ( u32 ) ( ( u32 ) 0x0000FFFF &
        ...
        CAN->sFilterRegister[ CAN_FilterInitStruct->CAN_FilterNumber ]
            .FR2 = ( ( u32 ) ( ( u32 ) 0x0000FFFF &
                               CAN_FilterInitStruct->CAN_FilterMaskIdHigh )
                     << 16 ) |
                   ( ( u32 ) 0x0000FFFF &
 ...
 * Input          : - CAN_IT: specifies the CAN interrupt sources to be enabled
 *or disabled. This parameter can be: CAN_IT_TME, CAN_IT_FMP0, CAN_IT_FF0,
 *                                           CAN_IT_FOV0, CAN_IT_FMP1,
 *CAN_IT_FF1, CAN_IT_FOV1, CAN_IT_EWG, CAN_IT_EPV, CAN_IT_LEC, CAN_IT_ERR,
 *CAN_IT_WKU or CAN_IT_SLK.
...
u8 CAN_Transmit( CanTxMsg * TxMessage )
{
    ...
    if( TransmitMailbox != CAN_NO_MB )
    {
        ...
        if( TxMessage->IDE == CAN_ID_STD )
        {
            ...
            CAN->sTxMailBox[ TransmitMailbox ].TIR |= ( TxMessage->StdId |
                                                        TxMessage->IDE |
                                                        TxMessage->RTR );
        }
        else
        {
            ...
            CAN->sTxMailBox[ TransmitMailbox ].TIR |= ( TxMessage->ExtId |
                                                        TxMessage->IDE |
                                                        TxMessage->RTR );
        }
...
void CAN_Receive( u8 FIFONumber, CanRxMsg * RxMessage )
{
    ...
    {
        RxMessage->StdId = ( u32 ) 0x000007FF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RIR >> 21 );
    }
    else
    {
        RxMessage->ExtId = ( u32 ) 0x1FFFFFFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RIR >> 3 );
    }
    ...
    /* Get the FMI */
    RxMessage->FMI = ( u8 ) 0xFF &
                     ( CAN->sFIFOMailBox[ FIFONumber ].RDTR >> 8 );

    /* Get the data field */
    RxMessage->Data[ 0 ] = ( u8 ) 0xFF & CAN->sFIFOMailBox[ FIFONumber ].RDLR;
    RxMessage->Data[ 1 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDLR >> 8 );
    RxMessage->Data[ 2 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDLR >> 16 );
    RxMessage->Data[ 3 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDLR >> 24 );

    RxMessage->Data[ 4 ] = ( u8 ) 0xFF & CAN->sFIFOMailBox[ FIFONumber ].RDHR;
    RxMessage->Data[ 5 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDHR >> 8 );
    RxMessage->Data[ 6 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDHR >> 16 );
    RxMessage->Data[ 7 ] = ( u8 ) 0xFF &
                           ( CAN->sFIFOMailBox[ FIFONumber ].RDHR >> 24 );
 ...
 * Input          : None.
 * Output         : None.
 * Return         : CANSLEEPOK if sleep entered, CANSLEEPFAILED in an other
 *case.
 *******************************************************************************/
u8 CAN_Sleep( void )
{
 ...
 *                  This parameter can be: CAN_IT_RQCP0, CAN_IT_RQCP1,
 *CAN_IT_RQCP2, CAN_IT_FF0, CAN_IT_FOV0, CAN_IT_FF1, CAN_IT_FOV1, CAN_IT_EWG,
 *CAN_IT_EPV, CAN_IT_BOF, CAN_IT_WKU or CAN_IT_SLK. Output         : None.
