...
#define CR3_HDSEL_Reset                                  \
    ( ( u16 ) 0xFFF7 ) /* USART Half-Duplex Disable Mask \
                        */
...
void USART_Init( USART_TypeDef * USARTx, USART_InitTypeDef * USART_InitStruct )
{
    ...
    tmpreg |= ( u32 ) USART_InitStruct->USART_WordLength |
    ...
    integerdivider = ( ( 0x19 * apbclock ) /
                       ( 0x04 * ( USART_InitStruct->USART_BaudRate ) ) );
    tmpreg = ( integerdivider / 0x64 ) << 0x04;

    /* Determine the fractional part */
    fractionaldivider = integerdivider - ( 0x64 * ( tmpreg >> 0x04 ) );
    tmpreg |= ( ( ( ( fractionaldivider * 0x10 ) + 0x32 ) / 0x64 ) ) &
              ( ( u8 ) 0x0F );
 ...
 * Input          : - USARTx: where x can be 1, 2, 3 to select the USART
 *peripheral. Note: The Smart Card mode is not available for UART4 and UART5.
                      ...
                      USART_ClockInitTypeDef * USART_ClockInitStruct )
{
    ...
    tmpreg |= ( u32 ) USART_ClockInitStruct->USART_Clock |
 ...
 * Description    : Fills each USART_ClockInitStruct member with its default
 *value. Input          : - USART_ClockInitStruct: pointer to a
 *USART_ClockInitTypeDef structure which will be initialized. Output         :
 *None Return         : None
 *******************************************************************************/
                     ...
                     FunctionalState NewState )
{
    ...
    assert_param( IS_USART_PERIPH_IT( USARTx, USART_IT ) ); /* The CTS interrupt
                                                               is not available
                                                               for UART4 and
                                                               UART5 */
    assert_param( IS_FUNCTIONAL_STATE( NewState ) );
 ...
 * Description    : Returns the most recent received data by the USARTx
 *peripheral. Input          : - USARTx: Select the USART or the UART
 ...
 *                    peripheral.
 *                  Note: The guard time bits are not available for UART4 and
 *UART5.
 ...
 *                    This parameter can be one of the following values:
 *                     - USART1, USART2, USART3, UART4 or UART5.
 *                  Note: The function is used for IrDA mode with UART4 and
 *UART5.
 ...
 * Input          : - USARTx: where x can be 1, 2 or 3 to select the USART
 *                    peripheral.
 *                    Note: The Smart Card mode is not available for UART4 and
 *UART5.
...
FlagStatus USART_GetFlagStatus( USART_TypeDef * USARTx, u16 USART_FLAG )
{
    ...
    assert_param(
        IS_USART_PERIPH_FLAG( USARTx, USART_FLAG ) ); /* The CTS flag is not
                                                         available for UART4 and
                                                         UART5 */
 ...
 *                       - USART_FLAG_PE:   Parity Error flag.
 *
 *                    Note: - For IDLE, ORE, NE, FE and PE flags user has to
 ...
 * Description    : Checks whether the specified USART interrupt has occurred or
 *not. Input          : - USARTx: Select the USART or the UART peripheral. This
 ...
 *                       - USART_IT_PE:   Parity Error interrupt.
 *
 *                    Note: - For IDLE, ORE, NE, FE and PE pending bits user has
 *to read the USART DR register after calling this function.
 *                          - TXE pending bit can't be cleared by this function,
 *it's cleared only by a write to the USART DR register. Output         : None
 * Return         : None
 *******************************************************************************/
