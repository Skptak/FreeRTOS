...
/* define for rtos */
    ...
    #define LOG_GIVE_BINARY_SEMAPHORE_FROM_ISR( binary ) \
        ( xSemaphoreGiveFromISR( binary, NULL ) )
 ...
 *DEBUG_CONSOLE_SYNCHRONIZATION_xxx)
 */
 ...
 * The total buffer size should be calucate as (BUFFER_SUPPORT_LOG_LENGTH + 1) *
 * BUFFER_SUPPORT_LOG_NUM * 4
 */
typedef struct _log_buffer
{
    ...
    uint8_t txBuf[ DEBUG_CONSOLE_TRANSMIT_BUFFER_LEN ]; /*!< buffer to store
                                                           printf log */

    uint8_t rxBuf[ DEBUG_CONSOLE_RECEIVE_BUFFER_LEN ]; /*!< buffer to store
                                                          scanf log */
} log_buffer_t;
#endif /* DEBUG_CONSOLE_TRANSFER_NON_BLOCKING */
...
static volatile bool s_logReadSemaphore = false; /* transferred event from ISR
                                                    for bare-metal + interrupt
                                                  */
...
static int LOG_BufPush( uint8_t * buf, size_t size )
{
    ...
    LOG_TAKE_MUTEX_SEMAPHORE_BLOCKING( s_logPushSemaphore );
    if( size <=
        ( DEBUG_CONSOLE_TRANSMIT_BUFFER_LEN - s_log_buffer.totalIndex ) )
    {
        /* get push index */
    ...
    if( pushAvaliable )
    {
        for( i = size; i > 0; i-- )
        {
            /* copy log to buffer, the buffer only support a fixed length
            argument, if the log argument is longer than the fixed length, the
            left argument will be losed */
...
static uint8_t * LOG_BufGetNextAvaliableLog( size_t * size )
{
    ...
    /* get avaliable size */
    if( s_log_buffer.totalIndex >
        ( DEBUG_CONSOLE_TRANSMIT_BUFFER_LEN - popIndex ) )
    {
