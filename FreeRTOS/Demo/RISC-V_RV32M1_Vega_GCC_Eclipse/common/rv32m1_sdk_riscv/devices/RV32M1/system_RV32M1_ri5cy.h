...
/**
 * @brief System IRQ handler which dispatches specific IRQ to corresponding
 * registered handler.
 *
 * It is called from IRQ exception context and dispatches to registered handler
 * according to MCAUSE interrupt number.
 *
 ...
 *
 * If the return value is 0, then the context is not ISR, otherwise the context
 * is ISR.
 *
