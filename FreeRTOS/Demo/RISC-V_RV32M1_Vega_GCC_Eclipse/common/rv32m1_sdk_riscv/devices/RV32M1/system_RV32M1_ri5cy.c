...
DEFINE_IRQ_HANDLER( DMA0_2_6_10_14_IRQHandler,
                    DMA0_2_6_10_14_DriverIRQHandler );
DEFINE_IRQ_HANDLER( DMA0_3_7_11_15_IRQHandler,
                    DMA0_3_7_11_15_DriverIRQHandler );
...
void SystemInit( void )
{
    ...
    __ASM volatile(
        "csrw 0x305, %0" ::"r"( ( uint32_t ) __VECTOR_TABLE ) ); /* MTVEC */
    __ASM volatile(
        "csrw 0x005, %0" ::"r"( ( uint32_t ) __VECTOR_TABLE ) ); /* UTVEC */
...
{
    uint32_t SCGOUTClock; /* Variable to store output clock frequency of the SCG
                             module */
    uint16_t Divider;
    ...
    switch( ( SCG->CSR & SCG_CSR_SCS_MASK ) >> SCG_CSR_SCS_SHIFT )
    {
        ...
        case 0x2:
            /* Slow IRC */
            SCGOUTClock = ( ( ( SCG->SIRCCFG & SCG_SIRCCFG_RANGE_MASK ) >>
        ...
        case 0x3:
            /* Fast IRC */
            SCGOUTClock = 48000000 +
                          ( ( SCG->FIRCCFG & SCG_FIRCCFG_RANGE_MASK ) >>
        ...
        case 0x5:
            /* Low Power FLL */
            SCGOUTClock = 48000000 +
                          ( ( SCG->LPFLLCFG & SCG_LPFLLCFG_FSEL_MASK ) >>
                            SCG_LPFLLCFG_FSEL_SHIFT ) *
                              24000000;
            break;
        default:
            return;
...
void SystemSetupSystick( uint32_t tickRateHz, uint32_t intPriority )
{
    ...
    SYSTICK_LPIT->MCR = LPIT_MCR_DBG_EN_MASK | LPIT_MCR_DOZE_EN_MASK |
                        LPIT_MCR_M_CEN_MASK;

    /* Set timer period for channel 0 */
    SYSTICK_LPIT->CHANNEL[ SYSTICK_LPIT_CH ]
        .TVAL = ( CLOCK_GetIpFreq( kCLOCK_Lpit0 ) / tickRateHz ) - 1;
...
void EVENT_SetIRQPriority( IRQn_Type IRQn, uint8_t intPriority )
{
    ...
    if( ( IRQn < 32 ) && ( intPriority < 8 ) )
    {
        ...
        EVENT_UNIT->INTPTPRI[ regIdx ] = ( EVENT_UNIT->INTPTPRI[ regIdx ] &
                                           ~( 0x0F << regOffset ) ) |
                                         ( intPriority << regOffset );
    }
}
