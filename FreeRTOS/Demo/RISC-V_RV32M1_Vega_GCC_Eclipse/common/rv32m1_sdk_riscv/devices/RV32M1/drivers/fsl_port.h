...
#if defined( FSL_FEATURE_PORT_HAS_PULL_ENABLE ) && \
...
#if defined( FSL_FEATURE_PORT_HAS_OPEN_DRAIN ) && \
...
#if defined( FSL_FEATURE_PORT_HAS_PASSIVE_FILTER ) && \
...
#if defined( FSL_FEATURE_PORT_HAS_DRIVE_STRENGTH ) && \
    FSL_FEATURE_PORT_HAS_DRIVE_STRENGTH
...
{
    kPORT_UnlockRegister = 0U, /*!< Pin Control Register fields [15:0] are not
                                  locked. */
...
{
    kPORT_PinDisabledOrAnalog = 0U, /*!< Corresponding pin is disabled, but is
...
typedef enum _port_interrupt
{
    kPORT_InterruptOrDMADisabled = 0x0U, /*!< Interrupt/DMA request is disabled.
                                          */
#if defined( FSL_FEATURE_PORT_HAS_DMA_REQUEST ) && \
    FSL_FEATURE_PORT_HAS_DMA_REQUEST
...
#if defined( FSL_FEATURE_PORT_HAS_IRQC_TRIGGER ) && \
    FSL_FEATURE_PORT_HAS_IRQC_TRIGGER
    kPORT_ActiveHighTriggerOutputEnable = 0xDU, /*!< Enable active high-trigger
                                                   output. */
    kPORT_ActiveLowTriggerOutputEnable = 0xEU,  /*!< Enable active low-trigger
                                                   output. */
#endif
} port_interrupt_t;

#if defined( FSL_FEATURE_PORT_HAS_DIGITAL_FILTER ) && \
    FSL_FEATURE_PORT_HAS_DIGITAL_FILTER
/*! @brief Digital filter clock source selection */
typedef enum _port_digital_filter_clock_source
{
    kPORT_BusClock = 0U, /*!< Digital filters are clocked by the bus clock. */
    kPORT_LpoClock = 1U, /*!< Digital filters are clocked by the 1 kHz LPO
                            clock. */
...
typedef struct _port_digital_filter_config
{
    ...
    port_digital_filter_clock_source_t clockSource; /*!< Set digital filter
                                                       clockSource */
...
typedef struct _port_pin_config
{
    ...
    #if defined( FSL_FEATURE_PORT_HAS_SLEW_RATE ) && \
    ...
    #if defined( FSL_FEATURE_PORT_PCR_MUX_WIDTH ) && \
        ( FSL_FEATURE_PORT_PCR_MUX_WIDTH == 3 )
    uint16_t mux : 3; /*!< Pin mux Configure */
    uint16_t     : 4;
    #elif defined( FSL_FEATURE_PORT_PCR_MUX_WIDTH ) && \
        ( FSL_FEATURE_PORT_PCR_MUX_WIDTH == 4 )
                                               ...
                                               const port_pin_config_t * config )
{
    ...
    #if defined( FSL_FEATURE_PORT_HAS_MULTIPLE_IRQ_CONFIG ) && \
        FSL_FEATURE_PORT_HAS_MULTIPLE_IRQ_CONFIG
/*!
 * @brief Sets the port interrupt configuration in PCR register for multiple
 * pins.
 *
 * @param base   PORT peripheral base pointer.
 * @param mask   PORT pin number macro.
 ...
 *        - #kPORT_DMARisingEdge : DMA request on rising edge(if the DMA
 * requests exit).
 *        - #kPORT_DMAFallingEdge: DMA request on falling edge(if the DMA
 * requests exit).
 *        - #kPORT_DMAEitherEdge : DMA request on either edge(if the DMA
 * requests exit).
 *        - #kPORT_FlagRisingEdge : Flag sets on rising edge(if the Flag states
 * exit).
 *        - #kPORT_FlagFallingEdge : Flag sets on falling edge(if the Flag
 * states exit).
 *        - #kPORT_FlagEitherEdge : Flag sets on either edge(if the Flag states
 * exit).
 ...
 *        - #kPORT_ActiveHighTriggerOutputEnable : Enable active high-trigger
 * output (if the trigger states exit).
 *        - #kPORT_ActiveLowTriggerOutputEnable  : Enable active low-trigger
 * output (if the trigger states exit)..
 */
 ...
 * @Note : This function is NOT recommended to use together with the
 * PORT_SetPinsConfig, because the PORT_SetPinsConfig need to configure the pin
 * mux anyway (Otherwise the pin mux is reset to zero :
 * kPORT_PinDisabledOrAnalog). This function is recommended to use to reset the
 * pin mux
 *
 */
...
/*!
 * @brief Enables the digital filter in one port, each bit of the 32-bit
...
/*!
 * @brief Sets the digital filter in one port, each bit of the 32-bit register
 * represents one pin.
 *
 * @param base   PORT peripheral base pointer.
 * @param config PORT digital filter configuration structure.
 */
static inline void PORT_SetDigitalFilterConfig(
    PORT_Type * base,
...
static inline void PORT_SetPinDriveStrength( PORT_Type * base,
                                             ...
                                             uint8_t strength )
{
    base->PCR[ pin ] = ( base->PCR[ pin ] & ~PORT_PCR_DSE_MASK ) |
 ...
 *
 * @param base PORT peripheral base pointer.
 * @return Current port interrupt status flags, for example, 0x00010001 means
 * the pin 0 and 16 have the interrupt.
 */
