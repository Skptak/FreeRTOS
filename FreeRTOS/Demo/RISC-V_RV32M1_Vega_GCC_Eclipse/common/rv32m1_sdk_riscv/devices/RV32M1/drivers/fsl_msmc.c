...
#if defined( __riscv )
    #define CONFIG_NORMAL_SLEEP \
        EVENT_UNIT->SLPCTRL = ( EVENT_UNIT->SLPCTRL & ~0x03 ) | ( 1 << 0 )
                               ...
                               smc_partial_stop_option_t option )
{
    ...
    reg |= ( ( uint32_t ) option << SMC_PMCTRL_PSTOPO_SHIFT ) |
           ( kSMC_StopNormal << SMC_PMCTRL_STOPM_SHIFT );
    base->PMCTRL = reg;
...
#if( defined( FSL_FEATURE_SMC_HAS_PMCTRL_STOPA ) && \
                                  ...
                                  const smc_reset_pin_filter_config_t * config )
{
    ...
    reg = SMC_RPC_FILTCFG( config->slowClockFilterCount ) |
          SMC_RPC_FILTEN( config->enableFilter );
