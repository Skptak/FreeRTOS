...
#define SCG_SIRC_LOW_RANGE_FREQ \
    2000000U /* Slow IRC low range clock frequency. */
#define SCG_SIRC_HIGH_RANGE_FREQ \
    8000000U /* Slow IRC high range clock frequency.   */
...
#define SCG_SOSCDIV_SOSCDIV1_VAL                      \
    ( ( SCG->SOSCDIV & SCG_SOSCDIV_SOSCDIV1_MASK ) >> \
      SCG_SOSCDIV_SOSCDIV1_SHIFT )
#define SCG_SOSCDIV_SOSCDIV2_VAL                      \
    ( ( SCG->SOSCDIV & SCG_SOSCDIV_SOSCDIV2_MASK ) >> \
      SCG_SOSCDIV_SOSCDIV2_SHIFT )
#define SCG_SOSCDIV_SOSCDIV3_VAL                      \
    ( ( SCG->SOSCDIV & SCG_SOSCDIV_SOSCDIV3_MASK ) >> \
      SCG_SOSCDIV_SOSCDIV3_SHIFT )
#define SCG_SIRCDIV_SIRCDIV1_VAL                      \
    ( ( SCG->SIRCDIV & SCG_SIRCDIV_SIRCDIV1_MASK ) >> \
      SCG_SIRCDIV_SIRCDIV1_SHIFT )
#define SCG_SIRCDIV_SIRCDIV2_VAL                      \
    ( ( SCG->SIRCDIV & SCG_SIRCDIV_SIRCDIV2_MASK ) >> \
      SCG_SIRCDIV_SIRCDIV2_SHIFT )
#define SCG_SIRCDIV_SIRCDIV3_VAL                      \
    ( ( SCG->SIRCDIV & SCG_SIRCDIV_SIRCDIV3_MASK ) >> \
      SCG_SIRCDIV_SIRCDIV3_SHIFT )
#define SCG_FIRCDIV_FIRCDIV1_VAL                      \
    ( ( SCG->FIRCDIV & SCG_FIRCDIV_FIRCDIV1_MASK ) >> \
      SCG_FIRCDIV_FIRCDIV1_SHIFT )
#define SCG_FIRCDIV_FIRCDIV2_VAL                      \
    ( ( SCG->FIRCDIV & SCG_FIRCDIV_FIRCDIV2_MASK ) >> \
      SCG_FIRCDIV_FIRCDIV2_SHIFT )
#define SCG_FIRCDIV_FIRCDIV3_VAL                      \
    ( ( SCG->FIRCDIV & SCG_FIRCDIV_FIRCDIV3_MASK ) >> \
      SCG_FIRCDIV_FIRCDIV3_SHIFT )

#define SCG_LPFLLDIV_LPFLLDIV1_VAL                       \
    ( ( SCG->LPFLLDIV & SCG_LPFLLDIV_LPFLLDIV1_MASK ) >> \
      SCG_LPFLLDIV_LPFLLDIV1_SHIFT )
#define SCG_LPFLLDIV_LPFLLDIV2_VAL                       \
    ( ( SCG->LPFLLDIV & SCG_LPFLLDIV_LPFLLDIV2_MASK ) >> \
      SCG_LPFLLDIV_LPFLLDIV2_SHIFT )
#define SCG_LPFLLDIV_LPFLLDIV3_VAL                       \
    ( ( SCG->LPFLLDIV & SCG_LPFLLDIV_LPFLLDIV3_MASK ) >> \
      SCG_LPFLLDIV_LPFLLDIV3_SHIFT )

#define SCG_SIRCCFG_RANGE_VAL \
    ( ( SCG->SIRCCFG & SCG_SIRCCFG_RANGE_MASK ) >> SCG_SIRCCFG_RANGE_SHIFT )
#define SCG_FIRCCFG_RANGE_VAL \
    ( ( SCG->FIRCCFG & SCG_FIRCCFG_RANGE_MASK ) >> SCG_FIRCCFG_RANGE_SHIFT )

#define SCG_LPFLLCFG_FSEL_VAL \
    ( ( SCG->LPFLLCFG & SCG_LPFLLCFG_FSEL_MASK ) >> SCG_LPFLLCFG_FSEL_SHIFT )

/* Get the value of each field in PCC register. */
#define PCC_PCS_VAL( reg ) \
    ( ( reg & PCC_CLKCFG_PCS_MASK ) >> PCC_CLKCFG_PCS_SHIFT )
#define PCC_FRAC_VAL( reg ) \
    ( ( reg & PCC_CLKCFG_FRAC_MASK ) >> PCC_CLKCFG_FRAC_SHIFT )
#define PCC_PCD_VAL( reg ) \
    ( ( reg & PCC_CLKCFG_PCD_MASK ) >> PCC_CLKCFG_PCD_SHIFT )
...
uint32_t CLOCK_GetIpFreq( clock_ip_name_t name )
{
    ...
    {
        return freq * ( PCC_FRAC_VAL( reg ) + 1U ) /
               ( PCC_PCD_VAL( reg ) + 1U );
    }
    else
    {
...
uint32_t CLOCK_GetSysClkFreq( scg_sys_clk_t type )
{
    ...
    CLOCK_GetCurSysClkConfig( &sysClkConfig ); /* Get the main clock for SoC
                                                  platform. */
...
uint32_t CLOCK_GetSysOscFreq( void )
{
    if( SCG->SOSCCSR & SCG_SOSCCSR_SOSCVLD_MASK ) /* System OSC clock is valid.
                                                   */
    {
        /* Please call CLOCK_SetXtal0Freq base on board setting before using
         * OSC0 clock. */
        assert( g_xtal0Freq );
        return g_xtal0Freq;
    }
...
{
    static const uint32_t sircFreq[] = { SCG_SIRC_LOW_RANGE_FREQ,
                                         SCG_SIRC_HIGH_RANGE_FREQ };
...
status_t CLOCK_InitFirc( const scg_firc_config_t * config )
{
    ...
    {
        SCG->FIRCTCFG = SCG_FIRCTCFG_TRIMDIV( config->trimConfig->trimDiv ) |
        ...
        {
            SCG->FIRCSTAT = SCG_FIRCSTAT_TRIMCOAR(
                                config->trimConfig->trimCoar ) |
                            SCG_FIRCSTAT_TRIMFINE(
    ...
    /* Step 4. Enable clock. */
    SCG->FIRCCSR |= ( SCG_FIRCCSR_FIRCEN_MASK | SCG_FIRCCSR_FIRCTREN_MASK |
                      config->enableMode );
...
uint32_t CLOCK_GetRtcOscFreq( void )
{
    ...
    {
        /* Please call CLOCK_SetXtal32Freq base on board setting before using
         * RTC OSC clock. */
        assert( g_xtal32Freq );
        return g_xtal32Freq;
    }
...
status_t CLOCK_InitLpFll( const scg_lpfll_config_t * config )
{
    ...
    if( config->trimConfig )
    {
        SCG->LPFLLTCFG = SCG_LPFLLTCFG_TRIMDIV( config->trimConfig->trimDiv ) |
                         ...
                         SCG_LPFLLTCFG_LOCKW2LSB(
                             config->trimConfig->lockMode );
    ...
    if( ( config->trimConfig ) &&
        ( kSCG_LpFllTrimUpdate == config->trimConfig->trimMode ) )
    {
