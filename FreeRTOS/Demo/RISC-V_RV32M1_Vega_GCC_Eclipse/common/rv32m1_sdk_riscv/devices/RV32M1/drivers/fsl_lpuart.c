 ...
 * sure the TX register is empty or TX FIFO has empty room before calling this
 * function.
 *
 * @note This function does not check whether all the data has been sent out to
 * bus, so before disable TX, check kLPUART_TransmissionCompleteFlag to ensure
 * the TX is finished.
 *
 * @param base LPUART peripheral base address.
 ...
 * sure the RX register is full or TX FIFO has data before calling this
 * function.
 *
 * @param base LPUART peripheral base address.
                                     ...
                                     size_t length )
{
    ...
    /* The Non Blocking write data API assume user have ensured there is enough
    space in peripheral to write. */
    for( i = 0; i < length; i++ )
    {
        base->DATA = data[ i ];
    }
                                    ...
                                    size_t length )
{
    ...
    /* The Non Blocking read data API assume user have ensured there is enough
    space in peripheral to write. */
    for( i = 0; i < length; i++ )
    {
#if defined( FSL_FEATURE_LPUART_HAS_7BIT_DATA_SUPPORT ) && \
                      ...
                      uint32_t srcClock_Hz )
{
     ...
     * baudDiff iterate through the rest of the supported values of OSR */
    ...
    for( osrTemp = 4; osrTemp <= 32; osrTemp++ )
    {
        ...
        /*set sbrTemp to 1 if the sourceClockInHz can not satisfy the desired
         * baud rate*/
        if( sbrTemp == 0 )
        {
            sbrTemp = 1;
        ...
        if( tempDiff > ( config->baudRate_Bps -
    ...
    temp = base->CTRL &
           ~( LPUART_CTRL_PE_MASK | LPUART_CTRL_PT_MASK | LPUART_CTRL_M_MASK |
              LPUART_CTRL_ILT_MASK | LPUART_CTRL_IDLECFG_MASK );
    ...
    /* Set tx/rx WATER watermark
       Note:
       Take care of the RX FIFO, RX interrupt request only assert when received
       bytes equal or more than RX water mark, there is potential issue if RX
       water mark larger than 1. For example, if RX FIFO water mark is 2, upper
       layer needs 5 bytes and 5 bytes are received. the last byte will be saved
       in FIFO but not trigger RX interrupt because the water mark is 2.
     */
    ...
    temp = ( LPUART_STAT_RXEDGIF_MASK | LPUART_STAT_IDLE_MASK |
             LPUART_STAT_OR_MASK | LPUART_STAT_NF_MASK | LPUART_STAT_FE_MASK |
             LPUART_STAT_PF_MASK );
    ...
    base->MODIR |= LPUART_MODIR_TXCTSC( config->txCtsConfig ) |
                   LPUART_MODIR_TXCTSSRC( config->txCtsSource );
...
void LPUART_Deinit( LPUART_Type * base )
{
    ...
    while( 0 != ( ( base->WATER & LPUART_WATER_TXCOUNT_MASK ) >>
...
status_t LPUART_ClearStatusFlags( LPUART_Type * base, uint32_t mask )
{
    ...
    temp |= mask & ( LPUART_STAT_RXEDGIF_MASK | LPUART_STAT_IDLE_MASK |
                     LPUART_STAT_OR_MASK | LPUART_STAT_NF_MASK |
                     LPUART_STAT_FE_MASK | LPUART_STAT_PF_MASK );
#if defined( FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING ) && \
    FSL_FEATURE_LPUART_HAS_ADDRESS_MATCHING
    ...
    {
        /* Some flags can only clear or set by the hardware itself, these flags
        are: kLPUART_TxDataRegEmptyFlag, kLPUART_TransmissionCompleteFlag,
        kLPUART_RxDataRegFullFlag, kLPUART_RxActiveFlag,
        kLPUART_NoiseErrorInRxDataRegFlag, kLPUART_ParityErrorInRxDataRegFlag,
        kLPUART_TxFifoEmptyFlag, kLPUART_RxFifoEmptyFlag. */
        status = kStatus_LPUART_FlagCannotClearManually; /* flags can not clear
                                                            manually */
    }
    else
    {
                           ...
                           size_t length )
{
    ...
    /* This API can only ensure that the data is written into the data buffer
    but can't ensure all data in the data buffer are sent into the transmit
    shift buffer. */
    while( length-- )
    {
                              ...
                              size_t length )
{
    ...
    {
#if defined( FSL_FEATURE_LPUART_HAS_FIFO ) && FSL_FEATURE_LPUART_HAS_FIFO
        while( 0 == ( ( base->WATER & LPUART_WATER_RXCOUNT_MASK ) >>
                                            ...
                                            size_t * receivedBytes )
{
    ...
    /* How to get data:
       ...
       xfer->dataSize to lpuart handle, enable interrupt to store received data
       to xfer->data. When all data received, trigger callback.
       ...
       to xfer->data, save the xfer->data remained empty space to lpuart handle,
...
/* Get the size that can be stored into buffer for this interrupt. */
        ...
        while( ( count ) && ( handle->rxDataSize ) )
        {
            ...
            /* If all the data required for upper layer is ready, trigger
             * callback. */
            if( !handle->rxDataSize )
            {
                handle->rxState = kLPUART_RxIdle;
        ...
        if( handle->rxRingBuffer )
        {
            while( count-- )
            {
                /* If RX ring buffer is full, trigger callback to notify over
                 * run. */
                 ...
                 * oldest data is overrided. */
                if( LPUART_TransferIsRxRingBufferFull( base, handle ) )
                {
                    /* Increase handle->rxRingBufferTail to make room for new
                     * data. */
                    if( handle->rxRingBufferTail + 1U ==
                        handle->rxRingBufferSize )
                    {
...
/* Read data. */
    ...
    FSL_FEATURE_LPUART_HAS_7BIT_DATA_SUPPORT
                ...
                {
                    handle->rxRingBuffer
                        [ handle->rxRingBufferHead ] = ( base->DATA & 0x7F );
                }
                else
                {
                    handle
                        ->rxRingBuffer[ handle->rxRingBufferHead ] = base->DATA;
                }
#else
...
/* Get the bytes that available at this moment. */
        ...
        while( ( count ) && ( handle->txDataSize ) )
        {
            ...
            /* If all the data are written to data register, notify user with
             * the callback, then TX finished. */
...
{
    s_lpuartIsr( CM4_0_LPUART,
                 s_lpuartHandle[ LPUART_GetInstance( CM4_0_LPUART ) ] );
}
#endif
...
{
    s_lpuartIsr( CM4_1_LPUART,
                 s_lpuartHandle[ LPUART_GetInstance( CM4_1_LPUART ) ] );
}
#endif
...
{
    s_lpuartIsr( DMA_LPUART0,
                 s_lpuartHandle[ LPUART_GetInstance( DMA_LPUART0 ) ] );
}
#endif
...
{
    s_lpuartIsr( DMA_LPUART1,
                 s_lpuartHandle[ LPUART_GetInstance( DMA_LPUART1 ) ] );
}
#endif
...
{
    s_lpuartIsr( DMA_LPUART2,
                 s_lpuartHandle[ LPUART_GetInstance( DMA_LPUART2 ) ] );
}
#endif
...
{
    s_lpuartIsr( DMA_LPUART3,
                 s_lpuartHandle[ LPUART_GetInstance( DMA_LPUART3 ) ] );
}
#endif
...
{
    s_lpuartIsr( DMA_LPUART4,
                 s_lpuartHandle[ LPUART_GetInstance( DMA_LPUART4 ) ] );
}
#endif
