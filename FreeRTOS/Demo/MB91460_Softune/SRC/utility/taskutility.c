...
static void vUART5Task( void * pvParameters )
{
    ...
    SSR05_RIE = 1;
    Puts5( "\n -------------MB91467D FreeRTOS DEMO Task List and Trace "
           "Utility----------- \n" );

    for( ;; )
    {
        Puts5( "\n\rPress any of the following keys for the corresponding "
               "functionality: " );

        Puts5( "\n\r1: To call vTaskList() and display current task status " );

        /* The legacy trace is no longer supported.  Use FreeRTOS+Trace instead.
        Puts5( "\n\r2: To call vTaskStartTrace() and to display trace results
        once the trace ends" ); */
        ...
        switch( ch )
        {
            case '1':
                ...
                /* The legacy trace is no longer supported.  Use FreeRTOS+Trace
                instead. case '2': vTaskStartTrace( (signed char *) buff,
                sizeof( buff ) ); Puts5( "\n\rThe trace started!!" );
