...
/*                  and extended IDs                                          */
/*----------------------------------------------------------------------------*/
#define RANGE_ID_MSK( range_start, range_end ) \
    ( ~( ( range_end ) - ( range_start ) ) )
#define RANGE_ID_ARB( range_start, range_end ) \
...
#define STD_RANGE_ID_MSK( range_start, range_end )                         \
    ( ( u16 ) ( ( RANGE_ID_MSK( ( range_start ), ( range_end ) ) & 0x7FF ) \
                << 2 ) )
#define STD_RANGE_ID_ARB( range_start, range_end ) \
    ( ( u16 ) ( RANGE_ID_ARB( ( range_start ), ( range_end ) ) << 2 ) )

#define STD_FIXED_ID_MSK( id ) \
    ( ( u16 ) ( ( FIXED_ID_MSK( id ) & 0x7FF ) << 2 ) )
#define STD_FIXED_ID_ARB( id ) ( ( u16 ) ( FIXED_ID_ARB( id ) << 2 ) )

#define EXT_RANGE_ID_MSK_L( range_start, range_end ) \
    ( ( u16 ) ( RANGE_ID_MSK( ( range_start ), ( range_end ) ) >> 11 ) )
#define EXT_RANGE_ID_MSK_H( range_start, range_end )                         \
    ( ( u16 ) ( STD_RANGE_ID_MSK( ( range_start ), ( range_end ) ) |         \
                ( ( RANGE_ID_MSK( ( range_start ), ( range_end ) ) >> 27 ) & \
                  0x03 ) ) )
#define EXT_RANGE_ID_ARB_L( range_start, range_end ) \
    ( ( u16 ) ( RANGE_ID_ARB( ( range_start ), ( range_end ) ) >> 11 ) )
#define EXT_RANGE_ID_ARB_H( range_start, range_end )                         \
    ( ( u16 ) ( STD_RANGE_ID_ARB( ( range_start ), ( range_end ) ) |         \
                ( ( RANGE_ID_ARB( ( range_start ), ( range_end ) ) >> 27 ) & \
                  0x03 ) ) )

#define EXT_FIXED_ID_MSK_L( id ) ( ( u16 ) ( FIXED_ID_MSK( id ) >> 11 ) )
#define EXT_FIXED_ID_MSK_H( id )         \
    ( ( u16 ) ( STD_FIXED_ID_MSK( id ) | \
                ( ( FIXED_ID_MSK( id ) >> 27 ) & 0x03 ) ) )
#define EXT_FIXED_ID_ARB_L( id ) ( ( u16 ) ( FIXED_ID_ARB( id ) >> 11 ) )
#define EXT_FIXED_ID_ARB_H( id )         \
    ( ( u16 ) ( STD_FIXED_ID_ARB( id ) | \
                ( ( FIXED_ID_ARB( id ) >> 27 ) & 0x03 ) ) )

/* macro to format the timing register value from the timing parameters*/
#define CAN_TIMING( tseg1, tseg2, sjw, brp )                                   \
    ( ( ( ( tseg2 - 1 ) & 0x07 ) << 12 ) | ( ( ( tseg1 - 1 ) & 0x0F ) << 8 ) | \
      ( ( ( sjw - 1 ) & 0x03 ) << 6 ) | ( ( brp - 1 ) & 0x3F ) )
...
ErrorStatus CAN_SetUnusedMsgObj( u32 msgobj )
{
    ...
    CAN->sMsgObj[ msg_if ].CMR = CAN_CMR_WRRD | CAN_CMR_MASK | CAN_CMR_ARB |
                                 CAN_CMR_CONTROL | CAN_CMR_DATAA |
                                 CAN_CMR_DATAB;
 ...
 *                    This parameter can be one of the following values:
 *                          - TRUE: for a single receive object or a FIFO
 *receive object that is the last one of the FIFO.
                             ...
                             bool singleOrFifoLast )
{
    ...
    if( idType == CAN_STD_ID )
    {
        ...
        CAN->sMsgObj[ msg_if ].A1R = 0;
        CAN->sMsgObj[ msg_if ].A2R = CAN_A2R_MSGVAL |
                                     STD_RANGE_ID_ARB( idLow, idHigh );
    }
    else
    {
        CAN->sMsgObj[ msg_if ].M1R = EXT_RANGE_ID_MSK_L( idLow, idHigh );
        CAN->sMsgObj[ msg_if ].M2R = CAN_M2R_MXTD |
                                     EXT_RANGE_ID_MSK_H( idLow, idHigh );
...
ErrorStatus CAN_SendMessage( u32 msgobj, canmsg * pCanMsg )
{
    ...
    CAN->sMsgObj[ 0 ].CMR = CAN_CMR_WRRD | CAN_CMR_ARB | CAN_CMR_CONTROL |
                            CAN_CMR_DATAA | CAN_CMR_DATAB;
    ...
    CAN->sMsgObj[ 0 ].MCR = ( CAN->sMsgObj[ 0 ].MCR & 0xFEF0 ) |
                            CAN_MCR_NEWDAT | CAN_MCR_TXRQST | pCanMsg->Dlc;

    CAN->sMsgObj[ 0 ].DA1R = ( ( u16 ) pCanMsg->Data[ 1 ] << 8 ) |
                             pCanMsg->Data[ 0 ];
    CAN->sMsgObj[ 0 ].DA2R = ( ( u16 ) pCanMsg->Data[ 3 ] << 8 ) |
                             pCanMsg->Data[ 2 ];
    CAN->sMsgObj[ 0 ].DB1R = ( ( u16 ) pCanMsg->Data[ 5 ] << 8 ) |
                             pCanMsg->Data[ 4 ];
    CAN->sMsgObj[ 0 ].DB2R = ( ( u16 ) pCanMsg->Data[ 7 ] << 8 ) |
                             pCanMsg->Data[ 6 ];
...
ErrorStatus CAN_ReceiveMessage( u32 msgobj, bool release, canmsg * pCanMsg )
{
    ...
    CAN->sMsgObj[ 1 ].CMR = CAN_CMR_MASK | CAN_CMR_ARB | CAN_CMR_CONTROL |
                            CAN_CMR_CLRINTPND |
 ...
 * Input          : pCanMsg: pointer to the message structure where message is
 *copied. Output         : None Return         : An ErrorStatus enumuration
 *value:
 *                         - SUCCESS: Reception OK
 *                         - ERROR: No message pending
 *******************************************************************************/
ErrorStatus CAN_BasicReceiveMessage( canmsg * pCanMsg )
{
    ...
    CAN->sMsgObj[ 1 ].CMR = CAN_CMR_ARB | CAN_CMR_CONTROL | CAN_CMR_DATAA |
                            CAN_CMR_DATAB;
 ...
 *CAN_EnterInitMode(). Input          : None Output         : None Return : None
 *******************************************************************************/
 ...
 * Description    : Switchs the CAN into test mode. This function must be used
 *in conjunction with CAN_LeaveTestMode(). Input          : TestMask: specifies
 *the configuration in test modes. Output         : None Return         : None
 *******************************************************************************/
 ...
 *                  This function must be used in conjunction with
 *CAN_EnterTestMode(). Input          : None Output         : None Return : None
 *******************************************************************************/
...
{
    return ( msgobj < 16 ? CAN->ND1R & ( 1 << msgobj )
                         : CAN->ND2R & ( 1 << ( msgobj - 16 ) ) );
}
...
{
    return ( msgobj < 16 ? CAN->TXR1R & ( 1 << msgobj )
                         : CAN->TXR2R & ( 1 << ( msgobj - 16 ) ) );
}
...
{
    return ( msgobj < 16 ? CAN->IP1R & ( 1 << msgobj )
                         : CAN->IP2R & ( 1 << ( msgobj - 16 ) ) );
}
...
{
    return ( msgobj < 16 ? CAN->MV1R & ( 1 << msgobj )
                         : CAN->MV2R & ( 1 << ( msgobj - 16 ) ) );
}
