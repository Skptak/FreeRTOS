...
void NVIC_Init( NVIC_InitTypeDef * NVIC_InitStruct )
{
    ...
    if( NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE )
    {
        ...
        tmppriority = ( ( u32 ) tmppriority )
                      << ( ( NVIC_InitStruct->NVIC_IRQChannel & ( u8 ) 0x03 ) *
                           0x08 );
         ...
         * --------------------------------------*/
        NVIC->Enable[ ( NVIC_InitStruct->NVIC_IRQChannel >>
                        0x05 ) ] = ( u32 ) 0x01
    ...
    else
    {
         ...
         * -------------------------------------*/
        NVIC->Disable[ ( NVIC_InitStruct->NVIC_IRQChannel >>
                         0x05 ) ] = ( u32 ) 0x01
                                    << ( NVIC_InitStruct->NVIC_IRQChannel &
                                         ( u8 ) 0x1F );
    }
 ...
 * Description    : Enables the PRIMASK priority: Raises the execution priority
 ...
 * Description    : Enables the FAULTMASK priority: Raises the execution
 *priority to -1. Input          : None Output         : None Return         :
 *None
 *******************************************************************************/
...
{
    return (
        ( u16 ) ( ( SCB->IRQControlState & ( u32 ) 0x003FF000 ) >> 0x0C ) );
}
 ...
 *                  or not.
 * Input          : - NVIC_IRQChannel: specifies the interrupt pending bit to
 *check. Output         : None Return         : The new state of IRQ Channel
 *pending bit(SET or RESET).
 *******************************************************************************/
...
void NVIC_ClearIRQChannelPendingBit( u8 NVIC_IRQChannel )
{
    ...
    NVIC->Clear[ ( NVIC_IRQChannel >> 0x05 ) ] = ( u32 ) 0x01
                                                 << ( NVIC_IRQChannel &
                                                      ( u32 ) 0x1F );
}
 ...
 *                  or not.
 * Input          : - NVIC_IRQChannel: specifies the interrupt active bit to
 *check. Output         : None Return         : The new state of IRQ Channel
 *active bit(SET or RESET).
 *******************************************************************************/
 ...
 * Description    : Returns the ID number, the version number and the
 *implementation details of the Cortex-M3 core. Input          : None Output :
 *None Return         : CPU ID.
 *******************************************************************************/
...
void NVIC_SetVectorTable( u32 NVIC_VectTab, u32 Offset )
{
    ...
    SCB->ExceptionTableOffset = ( ( ( u32 ) Offset << 0x07 ) &
                                  ( u32 ) 0x1FFFFF80 );
 ...
 * Description    : Selects the condition for the system to enter low power
 *mode. Input          : - LowPowerMode: Specifies the new mode for the system
 *to enter low power mode. This parameter can be one of the following values:
 *                       - NVIC_LP_SEVONPEND
 *                       - NVIC_LP_SLEEPDEEP
 *                       - NVIC_LP_SLEEPONEXIT
 ...
 *                  - SystemHandlerSubPriority: new sub priority of the
 *specified system handlers. Output         : None Return         : None
 *******************************************************************************/
 ...
 *fault sources. This parameter can be one of the following values:
