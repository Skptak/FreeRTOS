...
void SystemInit( void )
{
    ...
    /* Workaround for Errata 20 "RAM content cannot be trusted upon waking up
       from System ON Idle or System OFF mode" found at the Errata document for
    ...
    for( uint32_t index = 0;
         index < 256ul && !is_empty_word( &NRF_FICR_S->TRIMCNF[ index ].ADDR );
         index++ )
    {
    #if defined( __ICCARM__ )
        #pragma diag_suppress = Pa082
    #endif
        *( volatile uint32_t * ) NRF_FICR_S->TRIMCNF[ index ]
             .ADDR = NRF_FICR_S->TRIMCNF[ index ].DATA;
    #if defined( __ICCARM__ )
    ...
    if( uicr_HFXOSRC_erased() || uicr_HFXOCNT_erased() )
    {
        ...
        if( uicr_HFXOCNT_erased() )
        {
            ...
            uicr_new_value = ( uicr_erased_value & ~UICR_HFXOCNT_HFXOCNT_Msk ) |
                             0x20;
    ...
    /* Enable Trace functionality. If ENABLE_TRACE is not defined, TRACE pins
       will be used as GPIOs (see Product Specification to see which ones). */
    ...
    *( ( volatile uint32_t * ) ( 0xE0058FB0ul ) ) = 0x00000000ul;
         ...
         * peripherals to enable tracing. ETB_BASE = 0xE0051000 TPIU_BASE =
         * 0xE0054000
         */
    #endif
...
bool uicr_HFXOSRC_erased()
{
    ...
    __DSB();
    if( ( HFXOSRC_readout & UICR_HFXOSRC_HFXOSRC_Msk ) !=
        UICR_HFXOSRC_HFXOSRC_TCXO )
    {
        return true;
    }
...
/* Workaround to allow NS code to access FICR. Override NRF_FICR_NS to move
 * FICR_NS buffer. */
...
void SystemStoreFICRNS()
{
    if( ( uint32_t ) NRF_FICR_NS < RAM_BASE ||
        ( uint32_t ) NRF_FICR_NS + FICR_SIZE > RAM_END )
    {
    ...
    volatile uint32_t *
        from = ( volatile uint32_t * ) ( ( uint32_t ) NRF_FICR_S +
                                         ( FICR_SIZE - sizeof( uint32_t ) ) );
    volatile uint32_t * to = ( volatile uint32_t * ) ( ( uint32_t ) NRF_FICR_NS +
                                                       ( FICR_SIZE -
                                                         sizeof( uint32_t ) ) );
    ...
    /* Make RAM region NS. */
    uint32_t ram_region = ( ( uint32_t ) NRF_FICR_NS - ( uint32_t ) RAM_BASE ) /
                          SPU_RAMREGION_SIZE;
    __DSB();
    NRF_SPU_S->RAMREGION[ ram_region ].PERM &= ~(
        1 << SPU_RAMREGION_PERM_SECATTR_Pos );
}
