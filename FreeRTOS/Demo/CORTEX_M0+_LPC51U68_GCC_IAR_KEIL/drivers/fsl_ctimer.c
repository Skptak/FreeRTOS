...
#if !( defined( FSL_FEATURE_CTIMER_HAS_NO_RESET ) && \
       ( FSL_FEATURE_CTIMER_HAS_NO_RESET ) )
    #if !( defined( FSL_SDK_DISABLE_DRIVER_RESET_CONTROL ) && \
           FSL_SDK_DISABLE_DRIVER_RESET_CONTROL )
        #if defined( FSL_FEATURE_CTIMER_WRITE_ZERO_ASSERT_RESET ) && \
            FSL_FEATURE_CTIMER_WRITE_ZERO_ASSERT_RESET
/*! @brief Pointers to Timer resets for each instance, writing a zero asserts
 * the reset */
static const reset_ip_name_t s_ctimerResets[] = CTIMER_RSTS_N;
        #else
/*! @brief Pointers to Timer resets for each instance, writing a one asserts the
 * reset */
...
static ctimer_callback_t * s_ctimerCallback[ FSL_FEATURE_SOC_CTIMER_COUNT ] = {
    0
};

/*! @brief Callback type installed by drivers for each instance. */
static ctimer_callback_type_t
    ctimerCallbackType[ FSL_FEATURE_SOC_CTIMER_COUNT ] = {
...
{
    uint32_t instance;
    uint32_t ctimerArrayCount = ( sizeof( s_ctimerBases ) /
                                  sizeof( s_ctimerBases[ 0 ] ) );
...
/* Setup the cimer mode and count select */
    ...
    base->CTCR = CTIMER_CTCR_CTMODE( config->mode ) |
                 CTIMER_CTCR_CINSEL( config->input );
#endif
                          ...
                          bool enableInt )
{
    ...
    reg = base->MCR;
    reg &= ~(
        ( CTIMER_MCR_MR0R_MASK | CTIMER_MCR_MR0S_MASK | CTIMER_MCR_MR0I_MASK )
        << ( matchChannel * 3 ) );

    /* If call back function is valid then enable match interrupt for the
     * channel */
    if( enableInt )
    {
        reg |= ( CTIMER_MCR_MR0I_MASK
                 << ( CTIMER_MCR_MR0I_SHIFT + ( matchChannel * 3 ) ) );
    }
    ...
    /* If call back function is valid then enable interrupt and update the call
     * back function */
    if( enableInt )
    {
...
/* Some CTimers only have 16bits , so the value is limited*/
    ...
    assert( !( ( FSL_FEATURE_CTIMER_BIT_SIZEn( base ) < 32 ) &&
               ( pulsePeriod > 0xFFFFU ) ) );
#endif
                                ...
                                uint8_t dutyCyclePercent )
{
    ...
    /* For 0% dutycyle, make pulse period greater than period so the event will
     * never occur */
    if( dutyCyclePercent == 0 )
    {
        pulsePeriod = period + 1;
...
/* Some CTimers only have 16bits , so the value is limited*/
    ...
    reg |= ( uint32_t ) ( ( uint32_t ) ( config->enableCounterStop )
                          << ( CTIMER_MCR_MR0S_SHIFT + ( matchChannel * 3 ) ) );
    reg |= ( uint32_t ) ( ( uint32_t ) ( config->enableInterrupt )
                          << ( CTIMER_MCR_MR0I_SHIFT + ( matchChannel * 3 ) ) );
    base->MCR = reg;
    ...
    reg |= ( uint32_t ) config->outControl
           << ( CTIMER_EMR_EMC0_SHIFT + ( matchChannel * 2 ) );
    ...
    /* Clear status flags */
    CTIMER_ClearStatusFlags( base, CTIMER_IR_MR0INT_MASK << matchChannel );
    /* If interrupt is enabled then enable interrupt and update the call back
     * function */
                          ...
                          bool enableInt )
{
    ...
    reg &= ~( ( CTIMER_CCR_CAP0RE_MASK | CTIMER_CCR_CAP0FE_MASK |
    ...
    /* If call back function is valid then enable capture interrupt for the
     * channel and update the call back function */
    if( enableInt )
    {
...
void CTIMER_GenericIRQHandler( uint32_t index )
{
    ...
    FSL_FEATURE_CTIMER_HAS_NO_INPUT_CAPTURE
        ...
        {
            mask = 0x01 << i;
            /* For each status flag bit that was set call the callback function
             * if it is valid */
