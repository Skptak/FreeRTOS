...
typedef enum _clock_ip_name
{
    ...
    kCLOCK_Gint = CLK_GATE_DEFINE( AHB_CLK_CTRL0, 19 ), /* GPIO_GLOBALINT0 and
                                                           GPIO_GLOBALINT1 share
                                                           the same slot  */
...
static inline void CLOCK_EnableClock( clock_ip_name_t clk )
{
    ...
    if( index < 2 )
    {
        SYSCON->AHBCLKCTRLSET[ index ] = ( 1U << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                               clk ) );
    }
    else
    {
        ASYNC_SYSCON->ASYNCAPBCLKCTRLSET = ( 1U << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                                 clk ) );
    }
}
...
static inline void CLOCK_DisableClock( clock_ip_name_t clk )
{
    ...
    if( index < 2 )
    {
        SYSCON->AHBCLKCTRLCLR[ index ] = ( 1U << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                               clk ) );
    }
    else
    {
        ASYNC_SYSCON->ASYNCAPBCLKCTRLCLR = ( 1U << CLK_GATE_ABSTRACT_BITS_SHIFT(
                                                 clk ) );
    }
}
/**
...
static inline void CLOCK_SetFLASHAccessCycles( clock_flashtim_t clks )
{
    ...
    SYSCON->FLASHCFG = tmp |
                       ( ( uint32_t ) clks << SYSCON_FLASHCFG_FLASHTIM_SHIFT );
}
...
#define PLL_CONFIGFLAG_USEINRATE                                             \
    ( 1 << 0 ) /*!< Flag to use InputRate in PLL configuration structure for \
                  setup */
#define PLL_CONFIGFLAG_FORCENOFRACT                                           \
    ( 1 << 2 ) /*!< Force non-fractional output mode, PLL output will not use \
                  the fractional, automatic bandwidth, or SS \ \              \
                   \ \ \ \                                                    \
                     \ \ \ \ \ \                                              \
                        \ \ \ \ \ \ \ \                                       \
                          \ \ \ \ \ \ \ \ \ \                                 \
...
#define PLL_SETUPFLAG_POWERUP \
    ( 1 << 0 ) /*!< Setup will power on the PLL after setup */
#define PLL_SETUPFLAG_WAITLOCK                                            \
    ( 1 << 1 ) /*!< Setup will wait for PLL lock, implies the PLL will be \
                  pwoered on */
#define PLL_SETUPFLAG_ADGVOLT \
    ( 1 << 2 ) /*!< Optimize system voltage for the new PLL rate */
#define PLL_SETUPFLAG_USEFEEDBACKDIV2 \
    ( 1 << 3 ) /*!< Use feedback divider by 2 in divider path */
...
typedef enum _clock_usb_src
{
    ...
    kCLOCK_UsbSrcNone = SYSCON_USBCLKSEL_SEL(
        7 ) /*!< Use None, this may be selected in order to reduce power when no
               output is needed. */
} clock_usb_src_t;
