...
#define PLL_SSCG0_MDEC_VAL_M \
    ( 0x1FFFFUL << PLL_SSCG0_MDEC_VAL_P ) /* NDEC is in bits  9 downto 0 */
...
/* SSCG control0 */
#define PLL_SSCG0_MDEC_VAL_SET( value )                         \
    ( ( ( unsigned long ) ( value ) << PLL_SSCG0_MDEC_VAL_P ) & \
      PLL_SSCG0_MDEC_VAL_M )
...
#define PLL_SSCG1_MD_FRACT_SET( value )                         \
    ( ( ( unsigned long ) ( value ) << PLL_SSCG1_MD_FRACT_P ) & \
      PLL_SSCG1_MD_FRACT_M )
#define PLL_SSCG1_MD_INT_SET( value )                         \
    ( ( ( unsigned long ) ( value ) << PLL_SSCG1_MD_INT_P ) & \
      PLL_SSCG1_MD_INT_M )
 ...
 * output is off, else high speed output is enabled. param	iFreq	: Desired
 * frequency (must be one of CLK_FRO_12MHZ or CLK_FRO_48MHZ or CLK_FRO_96MHZ)
 * return	returns success or fail status.
 */
status_t CLOCK_SetupFROClocking( uint32_t iFreq )
{
    uint32_t usb_adj;
    if( ( iFreq != 12000000U ) && ( iFreq != 48000000U ) &&
        ( iFreq != 96000000U ) )
    {
        return kStatus_Fail;
    }
    ...
    /* back up the value of whether USB adj is selected, in which case we will
     * have a value of 1 else 0 */
    usb_adj = ( ( SYSCON->FROCTRL ) & SYSCON_FROCTRL_USBCLKADJ_MASK ) >>
              SYSCON_FROCTRL_USBCLKADJ_SHIFT;
    if( iFreq > 12000000U )
    {
        if( iFreq == 96000000U )
        {
            SYSCON->FROCTRL = ( ( SYSCON_FROCTRL_TRIM_MASK |
                                  SYSCON_FROCTRL_FREQTRIM_MASK ) &
                                *INDEX_SECTOR_TRIM96 ) |
                              SYSCON_FROCTRL_SEL( 1 ) |
                              SYSCON_FROCTRL_WRTRIM( 1 ) |
                              SYSCON_FROCTRL_USBCLKADJ( usb_adj ) |
...
uint32_t CLOCK_GetCoreSysClkFreq( void )
{
    return ( ( SYSCON->MAINCLKSELB == 0U ) && ( SYSCON->MAINCLKSELA == 0U ) )
           ...
           : ( SYSCON->MAINCLKSELB == 3U ) ? CLOCK_GetOsc32KFreq()
                                           : 0U;
}
...
{
    return ( SYSCON->FXCOMCLKSEL[ id ] == 0U )   ? CLOCK_GetFro12MFreq()
           : ( SYSCON->FXCOMCLKSEL[ id ] == 1U ) ? CLOCK_GetFroHfFreq()
           : ( SYSCON->FXCOMCLKSEL[ id ] == 2U ) ? CLOCK_GetPllOutFreq()
           : ( SYSCON->FXCOMCLKSEL[ id ] == 3U ) ? CLOCK_GetI2SMClkFreq()
           : ( SYSCON->FXCOMCLKSEL[ id ] == 4U ) ? CLOCK_GetFreq( kCLOCK_Frg )
                                                 : 0U;
}
...
uint32_t CLOCK_GetFRGInputClock( void )
{
    return ( SYSCON->FRGCLKSEL == 0U )   ? CLOCK_GetCoreSysClkFreq()
           ...
           : ( SYSCON->FRGCLKSEL == 3U ) ? CLOCK_GetFroHfFreq()
                                         : 0U;
}
    ...
    switch( clockName )
    {
        ...
        case kCLOCK_UsbClk:
            freq = ( SYSCON->USBCLKSEL == 0U )  ? CLOCK_GetFroHfFreq()
                   : ( SYSCON->USBCLKSEL == 1 ) ? CLOCK_GetPllOutFreq()
                                                : 0U;
        ...
        case kCLOCK_WdtOsc:
            ...
            break;
        case kCLOCK_Frg:
            freq = ( ( SYSCON->FRGCTRL & SYSCON_FRGCTRL_DIV_MASK ) ==
                     SYSCON_FRGCTRL_DIV_MASK )
                       ? ( ( uint64_t ) CLOCK_GetFRGInputClock() *
    ...
    switch( MDEC )
    {
        ...
        default:
            ...
            for( i = MVALMAX; ( ( i >= 3U ) && ( m == 0xFFFFFFFFU ) ); i-- )
            {
                x = ( ( ( x ^ ( x >> 1U ) ) & 1 ) << 14U ) |
                    ( ( x >> 1U ) & 0x3FFFU );
                if( ( x & ( PLL_SSCG0_MDEC_VAL_M >> PLL_SSCG0_MDEC_VAL_P ) ) ==
                    MDEC )
                {
                                               ...
                                               bool useSS )
{
    ...
    if( useSS )
    {
        ...
        pllPreDivider = finHz /
                        ( ( PLL_MIN_IN_SSMODE + PLL_MAX_IN_SSMODE ) / 2 );
    ...
    if( useSS == false )
    {
         ...
         */
        pSetup
            ->syspllssctrl[ 0 ] = ( PLL_SSCG0_MDEC_VAL_SET(
                                        pllEncodeM( pllMultiplier ) ) |
                                    ( 1U
    ...
    else
    {
        ...
        fc = ( ( uint64_t ) ( fccoHz % ( multFccoDiv * nDivOutHz ) ) << 11U ) /
             ( multFccoDiv * nDivOutHz );
        ...
        ->syspllctrl = ( pllSelR
                       ...
                       ( pllSelP
                         << SYSCON_SYSPLLCTRL_SELP_SHIFT ) |     /* Filter
                                                                    coefficient
                                                                  */
                       ( 0 << SYSCON_SYSPLLCTRL_BYPASS_SHIFT ) | /* PLL bypass
                                                                    mode
                                                                    disabled
                                                                  */
                       ( pllBypassFBDIV2
                         << SYSCON_SYSPLLCTRL_BYPASSCCODIV2_SHIFT ) | /* Extra
                                                                         M / 2
                                                                         divider?
                                                                       */
                       ( uplimoff
                         << SYSCON_SYSPLLCTRL_UPLIMOFF_SHIFT ) | /* SS/fractional
                                                                  ...
                                                                  */
                       ( bandsel
                         << SYSCON_SYSPLLCTRL_BANDSEL_SHIFT ) | /* Manual
                                                                   bandwidth
                                                                   selection
                                                                   enabled
                                                                 */
                                       ...
                                       bool useSS )
{
    ...
    for( i = 0U; i < CLOCK_USR_CFG_PLL_CONFIG_CACHE_COUNT; i++ )
    {
            ...
            ( useSS == gUseSSCache[ i ] ) )
        {
            ...
            pSetup->syspllssctrl[ 0 ] = gPllSetupCacheStruct[ i ]
                                            .syspllssctrl[ 0 ];
            pSetup->syspllssctrl[ 1 ] = gPllSetupCacheStruct[ i ]
                                            .syspllssctrl[ 1 ];
    ...
    retErr = CLOCK_GetPllConfigInternal( finHz,
                                         foutHz,
                                         pSetup,
                                         useFeedbackDiv2,
                                         useSS );
    ...
    gPllSetupCacheStruct[ gPllSetupCacheIdx ]
        .syspllssctrl[ 0 ] = pSetup->syspllssctrl[ 0 ];
    gPllSetupCacheStruct[ gPllSetupCacheIdx ]
        .syspllssctrl[ 1 ] = pSetup->syspllssctrl[ 1 ];
...
uint32_t CLOCK_GetSystemPLLOutFromSetup( pll_setup_t * pSetup )
{
    ...
    if( ( pSetup->syspllctrl & ( SYSCON_SYSPLLCTRL_BYPASS_MASK ) ) == 0U )
    {
        ...
        {
            /* MDEC used for rate */
            mMult = findPllMMult( pSetup->syspllctrl,
        ...
        else
        {
            ...
            mMult = ( pSetup->syspllssctrl[ 1 ] & PLL_SSCG1_MD_INT_M ) >>
                    PLL_SSCG1_MD_INT_P;
            workRate = ( uint64_t ) inPllRate * ( uint64_t ) mMult;

            /* Adjust by fractional */
            fract = ( uint64_t ) ( pSetup->syspllssctrl[ 1 ] &
                                   PLL_SSCG1_MD_FRACT_M ) >>
...
{
    uint32_t inRate;
    bool useSS = ( bool ) ( ( pControl->flags & PLL_CONFIGFLAG_FORCENOFRACT ) ==
                            0U );
    bool useFbDiv2;
    ...
    /* PLL flag options */
    pllError = CLOCK_GetPllConfig( inRate,
                                   pControl->desiredRate,
                                   pSetup,
                                   useFbDiv2,
                                   useSS );
    if( ( useSS ) && ( pllError == kStatus_PLL_Success ) )
    {
        ...
        pSetup->syspllssctrl[ 1 ] |= ( uint32_t ) pControl->ss_mf |
                                     ( uint32_t ) pControl->ss_mr |
...
pll_error_t CLOCK_SetupSystemPLLPrec( pll_setup_t * pSetup, uint32_t flagcfg )
{
    ...
    SYSCON->SYSPLLNDEC = pSetup->syspllndec |
                         ( 1U << SYSCON_SYSPLLNDEC_NREQ_SHIFT ); /* latch */
    SYSCON->SYSPLLPDEC = pSetup->syspllpdec;
    SYSCON->SYSPLLPDEC = pSetup->syspllpdec |
                         ( 1U << SYSCON_SYSPLLPDEC_PREQ_SHIFT ); /* latch */
    SYSCON->SYSPLLSSCTRL0 = pSetup->syspllssctrl[ 0 ];
    SYSCON->SYSPLLSSCTRL0 = pSetup->syspllssctrl[ 0 ] |
                            ( 1U << SYSCON_SYSPLLSSCTRL0_MREQ_SHIFT ); /* latch
                                                                        */
    SYSCON->SYSPLLSSCTRL1 = pSetup->syspllssctrl[ 1 ];
    SYSCON->SYSPLLSSCTRL1 = pSetup->syspllssctrl[ 1 ] |
                            ( 1U << SYSCON_SYSPLLSSCTRL1_MDREQ_SHIFT ); /* latch
                                                                         */
...
void CLOCK_SetupSystemPLLMult( uint32_t multiply_by, uint32_t input_freq )
{
    ...
    SYSCON->SYSPLLCTRL = SYSCON_SYSPLLCTRL_BANDSEL( 1 ) | directo |
                         SYSCON_SYSPLLCTRL_BYPASSCCODIV2( 1 ) |
                         ( selr << SYSCON_SYSPLLCTRL_SELR_SHIFT ) |
    ...
    SYSCON->SYSPLLNDEC = ndec |
                         ( 1U << 10U ); /* set Pdec value and assert preq */
...
bool CLOCK_EnableUsbfs0Clock( clock_usb_src_t src, uint32_t freq )
{
    ...
    if( kCLOCK_UsbSrcFro == src )
    {
        ...
        {
            case 96000000U:
                CLOCK_SetClkDiv( kCLOCK_DivUsbClk,
                                 2,
 ...
 * Please note that, this API will calculate the microsecond period with the
 * maximum devices supported CPU frequency, so this API will only delay for at
 * least the given microseconds, if precise delay count was needed, please
 * implement a new timer count to achieve this function.
 *
...
__attribute__( ( weak ) ) void SDK_DelayAtLeastUs( uint32_t delay_us )
{
    ...
    uint32_t count = ( uint32_t )
        USEC_TO_COUNT( delay_us, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY );

    /*
     * Calculate the real delay count depend on the excute instructions cycles,
     * users can change the divider value to adapt to the real IDE optimise
     * level.
     */
    count = ( count / 4U );
