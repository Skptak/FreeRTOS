 ...
 *          ===================================================================
 *
 *          After reset the device is running from Internal High Speed
 *oscillator (HSI 16MHz) with Flash 0 wait state, Flash prefetch buffer, D-Cache
 *          and I-Cache are disabled, and all peripherals are off except
 ...
 *           - Configure the clock source(s) for peripherals which clocks are
 *not derived from the System clock (I2S, RTC, ADC, USB OTG FS/SDIO/RNG)
 *
 *  @endverbatim
 *
 ******************************************************************************
...
168 MHz)
      - The second output is used to generate the clock for the USB OTG FS (48
MHz), the random analog generator (<=48 MHz) and the SDIO (<= 48 MHz).
 ...
 * constant HSE_STARTUP_TIMEOUT in stm32f4xx.h file. You can tailor it depending
 ...
 * modes.
 * @param  NewState: new state of the main PLL. This parameter can be: ENABLE or
 * DISABLE.
 * @retval None
 */
 ...
 * clock This parameter must be a number between 192 and 432.
...
(DMA, GPIO...). APB1 (PCLK1) and APB2 (PCLK2) clocks are derived from AHB clock
through configurable prescalers and used to clock the peripherals mapped on
...
@note All the peripheral clocks are derived from the System clock (SYSCLK)
       ...
       - I2S: the I2S clock can be derived either from a specific PLL (PLLI2S)
or from an external clock mapped on the I2S_CKIN pin. You have to use
...
RCC_RTCCLKCmd() functions to configure this clock.
       - USB OTG FS, SDIO and RTC: USB OTG FS require a frequency equal to 48
MHz to work correctly, while the SDIO require a frequency equal or lower than
          to 48. This clock is derived of the main PLL through PLLQ divider.
       - IWDG clock which is always the LSI clock.

  2. The maximum frequency of the SYSCLK and HCLK is 168 MHz, PCLK2 82 MHz and
PCLK1 42 MHz. Depending on the device voltage range, the maximum frequency
...
<= 120|
 +-------------------------------------------------------------------------------------+
   @note When VOS bit (in PWR_CR register) is reset to '0’, the maximum value of
HCLK is 144 MHz. You can use PWR_MainRegulatorModeConfig() function to set or
reset this bit.
...
@verbatim
 ...
 ===============================================================================
  ...
  1. The RTC clock which is derived from the LSI, LSE or HSE clock divided by 2
to 31.
...
functions.
  ...
  3. To reset the peripherals configuration (to the default state after device
reset) you can use RCC_AHBPeriphResetCmd(), RCC_APB2PeriphResetCmd() and
