...
void lcd_initialize( void )
{
    ...
    /* Display initialises in 8 bit mode - so send one write (seen as 8 bit) to
     * set to 4 bit mode. */
 ...
 *to specify the starting position. For example, to start at the 2nd position on
 *line 1... lcd_display(LCD_LINE1 + 1, "Hello") Arguments     : position - Line
 *number of display string - Pointer to null terminated string Return Value  :
 *none
 ***********************************************************************************************************************/
void lcd_display( uint8_t position, uint8_t const * string )
{
    ...
    /* Set line position if needed. We don't want to if we don't need to because
       LCD control operations take longer than LCD data operations. */
 ...
 * Arguments     : nsecs -
 *                     Number of nanoseconds to delay. RX111 has max clock of
 *32MHz which gives a cycle time of 31.3ns. This means that nothing under 313ns
 *should be input. 313ns would be 10 cycles which is still being optimistic for
 *getting in and out of this function. Return Value  : none
 ***********************************************************************************************************************/
static void lcd_delay( volatile int32_t nsecs )
{
    while( 0 < nsecs )
    {
        /* Subtract off 10 cycles per iteration. This number was obtained when
           using the Renesas toolchain at optimization level 2. The number to
           nanoseconds to subtract off below is calculated off of the ICLK
           speed. */
 ...
 * Description   : This function controls LCD writes to line 1 or 2 of the LCD.
 *You need to use the defines LCD_LINE1 and LCD_LINE2 in order to specify the
