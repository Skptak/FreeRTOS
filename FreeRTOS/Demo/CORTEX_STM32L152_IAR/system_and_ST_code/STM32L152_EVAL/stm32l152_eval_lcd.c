 ...
 *(LCD_ILI9320), AM-240320LDTNQW00H (LCD_SPFD5408B) Liquid Crystal Display
 *Module of STM32L152-EVAL board.
 ******************************************************************************
 * @copy
 *
...
void LCD_Setup( void )
{
    ...
    if( LCDType == LCD_SPFD5408 )
    {
        ...
        LCD_WriteReg( LCD_REG_3, 0x1030 );   /* Set GRAM write direction and
                                                BGR=1. */
        LCD_WriteReg( LCD_REG_4, 0x0000 );   /* Resize register */
        LCD_WriteReg( LCD_REG_8, 0x0202 ); /* Set the back porch and front porch
                                            */
        LCD_WriteReg( LCD_REG_9, 0x0000 ); /* Set non-display area refresh cycle
                                              ISC[3:0] */
...
void LCD_DisplayStringLine( uint8_t Line, uint8_t * ptr )
{
    ...
    /* Send the string character by character on lCD */
    while( ( *ptr != 0 ) &
           ( ( ( refcolumn + 1 ) & 0xFFFF ) >= LCD_Currentfonts->Width ) )
    {
        /* Display one character on LCD */
...
void LCD_FillPolyLine( pPoint Points, uint16_t PointCount )
{
    ...
    uint16_t nodes = 0, nodeX[ MAX_POLY_CORNERS ], pixelX = 0, pixelY = 0,
             i = 0, j = 0, swap = 0;
...
void LCD_SPIConfig( void )
{
    ...
    /* Enable LCD_SPI_SCK_GPIO_CLK, LCD_SPI_MISO_GPIO_CLK and
     * LCD_SPI_MOSI_GPIO_CLK clock */
    ...
    GPIO_PinAFConfig( LCD_SPI_SCK_GPIO_PORT,
                      LCD_SPI_SCK_SOURCE,
                      LCD_SPI_SCK_AF );

    /* Connect PE.14 to SPI MISO */
    GPIO_PinAFConfig( LCD_SPI_MISO_GPIO_PORT,
                      LCD_SPI_MISO_SOURCE,
                      LCD_SPI_MISO_AF );

    /* Connect PE.15 to SPI MOSI */
    GPIO_PinAFConfig( LCD_SPI_MOSI_GPIO_PORT,
                      LCD_SPI_MOSI_SOURCE,
                      LCD_SPI_MOSI_AF );
