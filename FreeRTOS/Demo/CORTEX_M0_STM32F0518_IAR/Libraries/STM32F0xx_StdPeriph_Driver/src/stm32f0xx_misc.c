...
interrupts (IRQ). The Cortex-M0 exceptions are managed by CMSIS functions.
...
void NVIC_Init( NVIC_InitTypeDef * NVIC_InitStruct )
{
    ...
    assert_param(
        IS_NVIC_PRIORITY( NVIC_InitStruct->NVIC_IRQChannelPriority ) );
    ...
    if( NVIC_InitStruct->NVIC_IRQChannelCmd != DISABLE )
    {
        ...
        tmppriority |= ( uint32_t ) ( ( ( ( uint32_t ) NVIC_InitStruct
