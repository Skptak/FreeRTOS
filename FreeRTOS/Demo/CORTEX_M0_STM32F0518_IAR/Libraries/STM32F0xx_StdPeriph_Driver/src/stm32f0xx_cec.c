/**
  ...
  *  @verbatim
  ...
  ==============================================================================
      [..] This device provides some features:
           ...
           (#) Configurable Signal Free Time before of transmission start. The
               number of nominal data bit periods waited before transmission can
  be ruled by Hardware or Software.
  ...
  destination can be received without interfering with CEC bus when Listen mode
  option is enabled.
           (#) Configurable Rx-Tolerance(Standard and Extended tolerance
  margin).
           (#) Error detection with configurable error bit generation.
           (#) Arbitration lost error in the case of two CEC devices starting at
  the same time.
  ...
  CEC_OwnAddressConfig().
           (#) Optionally, you can configure the Listen mode using the function
  CEC_ListenModeCmd().
           (#) Enable the NVIC and the corresponding interrupt using the
  function CEC_ITConfig() if you need to use interrupt mode. CEC_ITConfig() must
  be called before enabling the CEC peripheral.
  ...
  CEC_ReceiveDataByte().
           (#) Enable the CEC_EndOfMessage() in order to indicate the last byte
  of the message.
      [..]
           (@) If the listen mode is enabled, Stop reception generation and Bit
  error generation must be in reset state.
...
#####
 ===============================================================================
      ...
      [..] This section provides also a function to configure the CEC peripheral
in Listen Mode. Messages addressed to different destination can be received when
Listen mode is enabled without interfering with CEC bus.
@endverbatim
  * @{
  */
...
void CEC_Init( CEC_InitTypeDef * CEC_InitStruct )
{
    ...
    assert_param(
        IS_CEC_SIGNAL_FREE_TIME( CEC_InitStruct->CEC_SignalFreeTime ) );
    ...
    assert_param(
        IS_CEC_BIT_RISING_ERROR( CEC_InitStruct->CEC_BitRisingError ) );
...
manage the communication: Polling mode or Interrupt mode.
...
transmission.
            (+) CEC_FLAG_TXBR   : to indicate the next transmission data has to
be written to TXDR.
...
mode.
            (+) CEC_FLAG_LBPE   : to indicate a long bit period error generated
during receive mode.
            (+) CEC_FLAG_SBPE   : to indicate a short bit period error generated
during receive mode.
...
in the RXDR register.
            ...
            (+) CEC_FLAG_RXBR   : to indicate a new byte has been received from
...
sources:
           ...
           (+) CEC_IT_TXACKE : to indicate a missing acknowledge in transmission
mode.
...
transmission mode. The transmission is enabled while the software has not yet
           ...
           (+) CEC_IT_TXBR   : to indicate the next transmission data has to be
written to TXDR register.
...
in the RXDR register.
           ...
           (+) CEC_IT_RXBR   : to indicate a new byte has been received from the
CEC line and stored into the RXDR buffer.
    [..]
           (@)In this Mode it is advised to use the following functions:
 ...
 *     @arg CEC_IT_TXEND: End of Transmission (successful transmission of the
 * last byte).
 ...
 *     @arg CEC_FLAG_TXEND: End of transmission (successful transmission of the
 * last byte).
