  ...
  *
    ...
    ===========================================================================
      ...
      (#) Configure the GPIO pin(s) using GPIO_Init()
         ...
         (++) In output mode, the speed is configurable: Low, Medium, Fast or
  High.
 ...
 *         values.
 * @param  GPIOx: where x can be (A, B, C, D or F) to select the GPIO
 * peripheral.
 * @retval None
 */
 ...
 * @note   The configured pins can be: GPIO_Pin_0 -> GPIO_Pin_15 for GPIOA,
 * GPIOB and GPIOC, GPIO_Pin_0 -> GPIO_Pin_2 for GPIOD, GPIO_Pin_0 -> GPIO_Pin_3
 * for GPIOF.
 * @retval None
 */
    ...
    for( pinpos = 0x00; pinpos < 0x10; pinpos++ )
    {
        ...
        if( currentpin == pos )
        {
                ...
                ( GPIO_InitStruct->GPIO_Mode == GPIO_Mode_AF ) )
            {
                ...
                GPIOx->OTYPER &= ~( ( GPIO_OTYPER_OT_0 )
                                    << ( ( uint16_t ) pinpos ) );
                GPIOx->OTYPER |= ( uint16_t ) ( ( ( uint16_t ) GPIO_InitStruct
                                                      ->GPIO_OType )
                                                << ( ( uint16_t ) pinpos ) );
            }
 ...
 * TIM14, BOOT,USART1, CEC, IR_OUT, SPI2
 *     @arg GPIO_AF_1:USART2, CEC, Tim3, USART1, USART2,EVENTOUT, I2C1, I2C2,
 * TIM15
                       ...
                       uint8_t GPIO_AF )
{
    ...
    GPIOx->AFR[ GPIO_PinSource >> 0x03 ] &= ~(
        ( uint32_t ) 0xF << ( ( uint32_t ) ( ( uint32_t ) GPIO_PinSource &
                                             ( uint32_t ) 0x07 ) *
                              4 ) );
