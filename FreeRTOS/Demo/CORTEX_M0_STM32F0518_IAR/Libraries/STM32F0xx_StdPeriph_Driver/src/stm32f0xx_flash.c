...
@verbatim
 ...
 ===============================================================================
    ...
    [..] FLASH_Interface configuration_Functions, includes the following
functions:
...
void FLASH_Lock( void )
{
    /* Set the LOCK Bit to lock the FLASH control register and program memory
     * access */
...
uint8_t OB_STDBY);
   ...
   (#) Call the FLASH_OB_Unlock() function to enable the Option Bytes registers
access
...
uint8_t OB_STDBY)
   ...
   (#) Once all needed option bytes to be programmed are correctly written, call
the FLASH_OB_Launch(void) function to launch the Option Bytes programming
process.

   (#) Call the FLASH_OB_Lock() to disable the Option Bytes registers access
 ...
 *             @arg OB_VDDA_ANALOG_OFF: Analog monitoring on VDDA Power source
 * OFF
 * @retval None
 */
FLASH_Status FLASH_OB_VDDAConfig( uint8_t OB_VDDA_ANALOG )
 ...
 * entering in STOP
 *             @arg OB_STDBY_NoRST / OB_STDBY_RST: No reset / Reset generated
 * when entering in STANDBY
 *             @arg OB_BOOT1_RESET / OB_BOOT1_SET: BOOT1 Reset / Set
 *             @arg OB_VDDA_ANALOG_ON / OB_VDDA_ANALOG_OFF: Analog monitoring on
 * VDDA Power source ON / OFF
 * @retval FLASH Status: The returned value can be:
 *         FLASH_ERROR_PROGRAM, FLASH_ERROR_WRP, FLASH_COMPLETE or
 * FLASH_TIMEOUT.
 */
 ...
 *         This parameter can be one of the following values:
 *             @arg FLASH_FLAG_BSY: FLASH write/erase operations in progress
 * flag
...
FLASH_Status FLASH_GetStatus( void )
{
    ...
    else
    {
        ...
        {
            if( ( FLASH->SR & ( uint32_t ) ( FLASH_SR_PGERR ) ) !=
                ( uint32_t ) 0x00 )
            {
