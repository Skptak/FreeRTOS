/**
 ...
 ===============================================================================
    [..]
         ...
         (+) The COMP output is internally is available using
 COMP_GetOutputLevel() and can be set on GPIO pins: PA0, PA6, PA11 for COMP1 and
 PA2, PA7, PA12 for COMP2
         ...
         (+) The two comparators COMP1 and COMP2 have interrupt capability
             with wake-up from Sleep and Stop modes (through the EXTI
 controller). COMP1 and COMP2 outputs are internally connected to EXTI Line 21
 ...
 ===============================================================================
    [..]
           This driver provides functions to configure and program the
 Comparators of all STM32F0xx devices.

    [..]   To use the comparator, perform the following steps:

         (#) Enable the SYSCFG APB clock to get write access to comparator
             register using RCC_APB2PeriphClockCmd(RCC_APB2Periph_SYSCFG,
 ENABLE);
 ...
 * values.
 * @note   Deinitialization can't be performed if the COMP configuration is
...
{
    COMP->CSR = ( ( uint32_t ) 0x00000000 ); /*!< Set COMP_CSR register to reset
                                                value */
}
 ...
 *         To use PA4 as COMP1 non inverting input call COMP_SwitchCmd() after
 * COMP_Init()
 * @param  COMP_Selection: the selected comparator.
 *   This parameter can be one of the following values:
 *     @arg COMP_Selection_COMP1: COMP1 selected
 *     @arg COMP_Selection_COMP2: COMP2 selected
 * @param  COMP_InitStruct: pointer to an COMP_InitTypeDef structure that
...
void COMP_Init( uint32_t COMP_Selection, COMP_InitTypeDef * COMP_InitStruct )
{
    ...
    assert_param(
        IS_COMP_INVERTING_INPUT( COMP_InitStruct->COMP_InvertingInput ) );
    ...
    /*!< Clear the COMP1SW1, COMPx_IN_SEL, COMPx_OUT_TIM_SEL, COMPx_POL,
     * COMPx_HYST and COMPx_PWR_MODE bits */
    tmpreg &= ( uint32_t ) ~( COMP_CSR_CLEAR_MASK << COMP_Selection );

    /*!< Configure COMP: inverting input, output redirection, hysteresis value
     * and power mode */
    /*!< Set COMPxINSEL bits according to COMP_InitStruct->COMP_InvertingInput
     * value */
    ...
    tmpreg |= ( uint32_t ) ( ( COMP_InitStruct->COMP_InvertingInput |
                               ...
                               COMP_InitStruct->COMP_Hysteresis |
 ...
 * @note   If the selected comparator is locked, enable/disable can't be
 * performed. To unlock the configuration, perform a system reset.
 * @param  COMP_Selection: the selected comparator.
 *   This parameter can be one of the following values:
 *     @arg COMP_Selection_COMP1: COMP1 selected
 ...
 *                       the inverting input and the comparison result is
 * available on comparator output.
 ...
 *         impedance input, such as COMP1 non-inverting input (highly resistive
 * switch)
 ...
 * @note     -Comparator output is low when the non-inverting input is at a
 * lower voltage than the inverting input
 * @note     -Comparator output is high when the non-inverting input is at a
 ...
 *           @arg ENABLE: COMP1 and COMP2 non inverting inputs are connected
 * together.
