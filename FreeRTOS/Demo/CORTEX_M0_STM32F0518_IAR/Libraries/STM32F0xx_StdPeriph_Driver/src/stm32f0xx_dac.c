/**
 ...
 DAC_Trigger_None and DAC_OUT1 is available once writing to DHRx register using
         DAC_SetChannel1Data().
...
void DAC_SoftwareTriggerCmd( uint32_t DAC_Channel, FunctionalState NewState )
{
    ...
    if( NewState != DISABLE )
    {
        ...
        DAC->SWTRIGR |= ( uint32_t ) DAC_SWTRIGR_SWTRIG1
                        << ( DAC_Channel >> 4 );
    }
    else
    {
 ...
 *   This parameter can be: ENABLE or DISABLE.
 *  The DAC channel1 is mapped on DMA1 channel3 which must be already
 * configured.
 * @retval None
 */
 ...
 *       the acknowledgement for the first external trigger is received (first
 * request).
