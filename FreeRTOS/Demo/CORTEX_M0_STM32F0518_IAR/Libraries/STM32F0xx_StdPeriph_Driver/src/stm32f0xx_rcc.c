...
internal/external clocks, PLL, CSS and MCO.
         (#) HSI (high-speed internal), 8 MHz factory-trimmed RC used directly
             or through the PLL as System clock source.
             The HSI clock can be used also to clock the USART, I2C and CEC
peripherals.
         (#) HSI14 (high-speed internal for ADC), 14 MHz factory-trimmed RC used
to clock the ADC peripheral.
...
source. LSE can be used also to clock the USART and CEC peripherals.
...
enabled. The interrupt is linked to the Cortex-M0 NMI (Non-Maskable Interrupt)
             exception vector.
         (#) MCO (microcontroller clock output), used to output SYSCLK, HSI,
HSI14, HSE, PLL clock on PA8 pin.
 ...
 * modes.
 * @note   This function resets the CSSON bit, so if the Clock security
 ...
 * constant HSE_STARTUP_TIMEOUT in stm32f0xx.h file. You can tailor it depending
 *         on the HSE crystal used in your application.
 *         - The HSE is stopped by hardware when entering STOP and STANDBY
 * modes.
 * @param  None
 ...
 *         Refer to the Application Note AN3300 for more details on how to
 *         calibrate the HSI14.
 * @param  HSI14CalibrationValue: specifies the HSI14 calibration trimming
...
/**
 * @brief  Enables or disables the Internal High Speed oscillator for ADC
 * (HSI14).
 * @note     After enabling the HSI14, the application software should wait on
 *           HSIRDY flag to be set indicating that HSI clock is stable and can
 *           be used to clock the ADC.
 * @note     The HSI14 is stopped by hardware when entering STOP and STANDBY
 * modes.
 * @param  NewState: new state of the HSI14.
 *         This parameter can be: ENABLE or DISABLE.
 * @note   When the HSI14 is stopped, HSI14RDY flag goes low after 6 HSI14
...
/**
 * @brief  Enables or disables the Internal High Speed oscillator request from
 * ADC.
 ...
 *   This parameter can be one of the following values:
 *     @arg RCC_PLLSource_HSI_Div2: HSI oscillator clock selected as PLL clock
 * source
 *     @arg RCC_PLLSource_PREDIV1: PREDIV1 clock selected as PLL clock entry
 * @note   The minimum input clock frequency for PLL is 2 MHz (when using HSE as
 *         PLL source).
 *
 * @param  RCC_PLLMul: specifies the PLL multiplication factor, which drive the
 * PLLVCO clock This parameter can be RCC_PLLMul_x where x:[2,16]
 *
 * @retval None
 */
 ...
 *         - The PLL is disabled by hardware when entering STOP and STANDBY
 * modes.
...
mapped on AHB bus (DMA and GPIO). and APB (PCLK) clocks are derived from AHB
clock through configurable prescalers and used to clock the peripherals mapped
on these busses. You can use "RCC_GetClocksFreq()" function to retrieve the
frequencies of these clocks.

         -@- All the peripheral clocks are derived from the System clock
(SYSCLK) except:
             (+@) The ADC clock which is derived from HSI14 or APB (APB divided
by a programmable prescaler: 2 or 4).
             (+@) The CEC clock which is derived from LSE or HSI divided by 244.
             (+@) The I2C clock which is derived from HSI or system clock
(SYSCLK).
             (+@) The USART clock which is derived from HSI, system clock
(SYSCLK), APB or LSE.
             (+@) The RTC/LCD clock which is derived from the LSE, LSI or 2 MHz
HSE_RTC (HSE divided by a programmable prescaler). The System clock (SYSCLK)
frequency must be higher or equal to the RTC/LCD clock frequency.
             (+@) IWDG clock which is always the LSI clock.

         (#) The maximum frequency of the SYSCLK, HCLK and PCLK is 48 MHz.
             Depending on the maximum frequency, the FLASH wait states (WS)
...
number of wait states needed to access the Flash memory with the CPU frequency
(HCLK).
         (+) Increasing the CPU frequency
         (++) Program the Flash Prefetch buffer, using
"FLASH_PrefetchBufferCmd(ENABLE)" function
         (++) Check that Flash Prefetch buffer activation is taken into account
by reading FLASH_ACR using the FLASH_GetPrefetchBufferStatus() function
...
FLASH_ACR
         ...
         (++) If needed, modify the CPU clock prescaler by using
"RCC_HCLKConfig()" function
         (++) Check that the new CPU clock source is taken into account by
reading the clock source status, using "RCC_GetSYSCLKSource()" function
...
reading the clock source status, using "RCC_GetSYSCLKSource()" function
         ...
         (++) Check that the new number of WS is taken into account by reading
FLASH_ACR
         (++) Disable the Flash Prefetch buffer using
"FLASH_PrefetchBufferCmd(DISABLE)" function
         (++) Check that Flash Prefetch buffer deactivation is taken into
account by reading FLASH_ACR using the FLASH_GetPrefetchBufferStatus() function.
 ...
 *
 *         (*) HSI_VALUE is a constant defined in stm32f0xx.h file (default
 * value 8 MHz) but the real value may vary depending on the variations in
 * voltage and temperature, refer to RCC_AdjustHSICalibrationValue().
 *
 *         (**) HSE_VALUE is a constant defined in stm32f0xx.h file (default
 * value 8 MHz), user has to ensure that HSE_VALUE is same as the real frequency
 * of the crystal used. Otherwise, this function may return wrong result.
 *
    ...
    switch( tmp )
    {
        ...
        case 0x08: /* PLL used as system clock */
            ...
            else
            {
                ...
                RCC_Clocks->SYSCLK_Frequency = ( HSE_VALUE / prediv1factor ) *
                                               pllmull;
            }
            break;
...
clocks.
         (#) The RTC clock which is derived from the LSE, LSI or  HSE_Div32 (HSE
             divided by 32).
         (#) After restart from Reset or wakeup from STANDBY, all peripherals
are off except internal SRAM, Flash and SWD. Before to start using a peripheral
you have to enable its interface clock. You can do this using
RCC_AHBPeriphClockCmd(), RCC_APB2PeriphClockCmd() and RCC_APB1PeriphClockCmd()
functions.
         (#) To reset the peripherals configuration (to the default state after
device reset) you can use RCC_AHBPeriphResetCmd(), RCC_APB2PeriphResetCmd() and
 ...
 *
 * @note     The maximum input clock frequency for RTC is 2MHz (when using HSE
 * as RTC clock source).
 *
 * @retval None
 */
 ...
 *             @arg RCC_AHBPeriph_FLITF: (has effect only when the Flash memory
 ...
 *         the application will be stacked in the NMI ISR unless the CSS
...
void RCC_ITConfig( uint8_t RCC_IT, FunctionalState NewState )
{
    ...
    if( NewState != DISABLE )
    {
        /* Perform Byte access to RCC_CIR[13:8] bits to enable the selected
         * interrupts */
        *( __IO uint8_t * ) CIR_BYTE1_ADDRESS |= RCC_IT;
    }
    else
    {
        /* Perform Byte access to RCC_CIR[13:8] bits to disable the selected
         * interrupts */
