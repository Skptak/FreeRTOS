...
typedef struct
{
    ...
    uint32_t USART_WordLength; /*!< Specifies the number of data bits
                                  transmitted or received in a frame. This
    ...
    uint32_t USART_Parity; /*!< Specifies the parity mode.
                              ...
                              transmitted data (9th bit when the word length is
                              set to 9 data bits; 8th bit when the word length
                              is set to 8 data bits). */

    uint32_t USART_Mode; /*!< Specifies wether the Receive or Transmit mode is
                            enabled or disabled. This parameter can be a value
                            of @ref USART_Mode */

    uint32_t USART_HardwareFlowControl; /*!< Specifies wether the hardware flow
...
typedef struct
{
    uint32_t USART_Clock; /*!< Specifies whether the USART clock is enabled or
    ...
    uint32_t USART_CPHA; /*!< Specifies the clock transition on which the bit
                            capture is made. This parameter can be a value of
                            @ref USART_Clock_Phase */

    uint32_t USART_LastBit; /*!< Specifies whether the clock pulse corresponding
 ...
 */
    ...
    #define USART_WakeUpSource_RXNE \
        ( ( uint32_t ) USART_CR3_WUS_0 | USART_CR3_WUS_1 )
 ...
 */
    ...
    #define IS_USART_FLAG( FLAG )                                              \
        ( ( ( FLAG ) == USART_FLAG_PE ) || ( ( FLAG ) == USART_FLAG_TXE ) ||   \
          ...
          ( ( FLAG ) == USART_FLAG_nCTSS ) ||                                  \
          ( ( FLAG ) == USART_FLAG_RTO ) || ( ( FLAG ) == USART_FLAG_EOB ) ||  \
          ( ( FLAG ) == USART_FLAG_ABRE ) ||                                   \
          ( ( FLAG ) == USART_FLAG_ABRF ) ||                                   \
          ( ( FLAG ) == USART_FLAG_BUSY ) || ( ( FLAG ) == USART_FLAG_CM ) ||  \
          ( ( FLAG ) == USART_FLAG_SBK ) || ( ( FLAG ) == USART_FLAG_RWU ) ||  \
          ( ( FLAG ) == USART_FLAG_WU ) || ( ( FLAG ) == USART_FLAG_TEACK ) || \
          ( ( FLAG ) == USART_FLAG_REACK ) )
