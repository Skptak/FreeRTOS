...
typedef struct
{
    ...
    uint32_t ADC_ExternalTrigConvEdge; /*!< Selects the external trigger Edge
                                          and enables the trigger of a regular
    ...
    uint32_t
        ADC_ExternalTrigConv; /*!< Defines the external trigger used to
                                 start the analog to digital conversion of
                                 regular channels. This parameter can be a
                                 value of @ref
                                 ADC_external_trigger_sources_for_channels_conversion
                               */

    uint32_t ADC_DataAlign; /*!< Specifies whether the ADC data alignment is
                               left or right. This parameter can be a value of
                               @ref ADC_data_align */

    uint32_t ADC_ScanDirection; /*!< Specifies in which direction the channels
                                   will be scanned in the sequence. This
 ...
 */
    ...
    #define IS_ADC_CLEAR_FLAG( FLAG )           \
        ( ( ( FLAG ) != ( uint32_t ) RESET ) && \
          ( ( ( FLAG ) &0xEFFFFF60 ) == ( uint32_t ) RESET ) )
    ...
    #define IS_ADC_GET_FLAG( FLAG )                                           \
        ( ( ( FLAG ) == ADC_FLAG_ADRDY ) || ( ( FLAG ) == ADC_FLAG_EOSMP ) || \
          ...
          ( ( FLAG ) == ADC_FLAG_ADSTP ) || ( ( FLAG ) == ADC_FLAG_ADCAL ) )
/**
 * @}
 */
