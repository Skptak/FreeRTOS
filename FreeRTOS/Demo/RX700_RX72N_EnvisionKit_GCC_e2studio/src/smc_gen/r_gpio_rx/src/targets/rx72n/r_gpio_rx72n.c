...
#include "platform.h"
    ...
    ***********************************************************************************************************************/
    /* These arrays hold which pins have extra functionality. For example, not
     * all pins have the option of enabling open-drain N-channel output instead
     * of the default CMOS output. Each entry in the array corresponds to a
     * port. Each bit in each entry corresponds to a pin on that port. If bit 3
     * of array entry [4] was set to 1 then that would mean that PORT 4 PIN 3
     * supported the feature that array represented.
     *
     * These arrays are only used when GPIO_CFG_PARAM_CHECKING_ENABLE is set to
     * 1 (checking enabled). If you know that your code does not need to check
     * the pins then you can set this macro to 0 and save a little execution
     * time and ROM space.
     *
     * Note: These arrays are defined for the largest package part. For smaller
     * packages where some pins do not exist, pin checking is filtered by the
     * enumerated port_pin list for that package as defined in r_gpio_rx72n.h.
     */
