 ...
 * Description  : This module maps Interrupt A & B interrupts. More information
 *on how this is done is given below.
 ***********************************************************************************************************************/
/**********************************************************************************************************************
...
/* If you wish to use one of the interrupt below then set the appropriate
 * configuration macro to the vector number you wish to use for that interrupt.
 * For example, if you want to use the RTC carry interrupt (CUP) at vector 176
 ...
 * -There are more vector slots for B sources than actual B sources. By default
 * none of the CAN2 interrupts are mapped. -If using the 'TPU1, TGI1A' interrupt
 * it must be vector 144 or 145. It is set to 144 by default. -If a peripheral
 * interrupt is going to be used to wake up the MCU from All-Module Clock Stop
 * Mode then it must be in a vector between 146 to 157. Peripheral interrupts
 * that can do this are TMR interrupts and the 'USB0, USBI0' interrupt. By
 * default the TMR interrupts are chosen since there are 12 of them and there
 * are 12 slots.
 */
 ...
 * -There are more A sources than A vector slots. By default none of the GPTW
 * interrupts and the ESC interrupts are mapped. -If using the 'MTU1, TGI1A'
 * interrupt it must be vector 208 or 209. It is set to 208 by default.
 */
