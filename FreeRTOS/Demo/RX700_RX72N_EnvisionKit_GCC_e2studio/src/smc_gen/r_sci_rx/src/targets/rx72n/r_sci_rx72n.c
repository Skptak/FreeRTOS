...
void sci_init_register( sci_hdl_t const hdl )
{
    ...
    /* WAIT_LOOP */
    while( ( 0 != hdl->rom->regs->SCR.BIT.TE ) ||
           ( 0 != hdl->rom->regs->SCR.BIT.RE ) )
    {
                           ...
                           uint32_t const baud )
{
    ...
    {
        /* Casting int16_t to uint32_t is valid. Because clock divisor is
         * positive integer */
    ...
    /* Casting int16_t to uint32_t is valid. Because clock divisor is a positive
     * integer */
     ...
     */
    hdl->rom->regs->BRR = ( uint8_t ) ( ( tmp & 0x01 ) ? ( tmp / 2 )
                                                       : ( ( tmp / 2 ) - 1 ) );
    ...
    /* Casting uint32_t type to uint8_t type in this case is valid. Range value
     * of m is not exceed uint8_t */
    hdl->rom->regs->MDDR = ( uint8_t ) int_M; // write M
    hdl->rom->regs->SEMR.BIT.BRME = 1;        // enable MDDR

    /* Casting uint32_t to float is valid*/
    error = ( ( ( float ) ( pclk ) / ( ( ( divisor * tmp ) * baud ) *
                                       ( ( float ) ( 256 ) / int_M ) ) ) -
              1 ) *
            100;
...
#if( ( SCI_CFG_CH0_INCLUDED == 1 ) || ( SCI_CFG_CH1_INCLUDED == 1 ) || \
     ( SCI_CFG_CH2_INCLUDED == 1 ) || ( SCI_CFG_CH3_INCLUDED == 1 ) || \
     ( SCI_CFG_CH4_INCLUDED == 1 ) || ( SCI_CFG_CH5_INCLUDED == 1 ) || \
...
#if( ( SCI_CFG_CH7_INCLUDED == 1 ) || ( SCI_CFG_CH8_INCLUDED == 1 ) ||  \
     ( SCI_CFG_CH9_INCLUDED == 1 ) || ( SCI_CFG_CH10_INCLUDED == 1 ) || \
     ( SCI_CFG_CH11_INCLUDED == 1 ) )
...
/* REGISTER GROUP INTERRUPTS WITH BSP */
    ...
    R_BSP_InterruptControl( hdl->rom->eri_vector,
    ...
    hdl->rom->regs->SCR.BYTE |= SCI_EN_XCVR_MASK; // enable TE, RE, TXI, and
                                                  // RXI/ERI
                          ...
                          void * p_args )
{
    ...
    switch( cmd )
    {
        ...
        case( SCI_CMD_START_BIT_EDGE ):
        {
            hdl->rom->regs->SCR.BYTE &= ( ~SCI_EN_XCVR_MASK );
            SCI_SCR_DUMMY_READ;
            hdl->rom->regs->SEMR.BIT.RXDESEL = 1; /* detect start bit on falling
                                                     edge */
            SCI_IR_TXI_CLEAR;
            hdl->rom->regs->SCR.BYTE |= SCI_EN_XCVR_MASK;
            break;
    ...
    #if SCI_CFG_TEI_INCLUDED
        case( SCI_CMD_EN_TEI ): /* SCI_CMD_EN_TEI is obsolete command, but it
                                   exists only for compatibility with older
                                   version. */
        {
            break;
        }
        ...
        {
            hdl->rom->regs->DCCR.BIT.DFER = 0; /* Clear Match Data Framing Error
                                                  Flag */
            hdl->rom->regs->DCCR.BIT.DPER = 0; /* Clear Match Data Parity Error
                                                  Flag */
            hdl->rom->regs->DCCR.BIT.DCME = 1; /* Enable Data match function */
            hdl->rom->regs->CDR.BYTE.L = *(
                ( unsigned char * ) p_args ); /* Comparison data */
            break;
        }
    #endif
                         ...
                         void * p_args )
{
    ...
    switch( cmd )
    {
        ...
        case( SCI_CMD_ABORT_XFER ):
        {
            ...
            hdl->rom->regs->SCR.BYTE &= ( ~(
                SCI_SCR_REI_MASK | SCI_SCR_RE_MASK | SCI_SCR_TE_MASK ) );
            ...
            *hdl->rom->ir_rxi = 0; /* clear rxi interrupt flag */
            ( *hdl->rom->icu_grp ) &= ( ~hdl->rom->eri_ch_mask ); /* clear eri
                                                                     interrupt
                                                                     flag */
            ...
            /* Enable receive interrupt in peripheral after rcvr or will get
             * "extra" interrupt */
        ...
        case( SCI_CMD_CHANGE_SPI_MODE ):
        {
            ...
            if( ( SCI_SPI_MODE_0 != spi_mode ) &&
                ( SCI_SPI_MODE_1 != spi_mode ) &&
                ( SCI_SPI_MODE_2 != spi_mode ) &&
                ( SCI_SPI_MODE_3 != spi_mode ) )
            {
                return SCI_ERR_INVALID_ARG;
            }
    #endif
