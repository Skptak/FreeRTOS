 ...
 *be enabled for channel 2. 25.11.2014 1.40    Added RX113 support
 *           11.03.2015 1.40    Consolidated with r_sci_rx64m and added support
 *for RX71M 11.05.2015 1.60    Added RX231 support 30.09.2015 1.70    Added
 *RX23T support 01.10.2016 1.80    Added support for RX65N (comments and TX/RX
 *FIFO THRESHOLD options) 19.12.2016 1.90    Added RX24U support SCI_CMD_EN_TEI
 *was Changed to ineffective, because it is meaningless command. 07.03.2017 2.00
 *Fixed a bug that send/receive is incorrect when changed setting when FIFO
 *enabled. Fixed a bug that callback function work many times at receive
 *interrupt when FIFO(async) enabled. Fixed a bug that the interrupt priority
 *level can be changed only in async mode. 31.10.2017 2.01    Added the demo for
 *RX65N, RX65N-2M. 28.09.2018 2.10    Added support RX66T Added
 *SCI_CMD_COMPARE_RECEIVED_DATA command Added SCI_EVT_RX_CHAR_MATCH for
 *receiving data match event Fixed section layout follow GSCE 5.0
...
#define SCI_CLK_EXT16X                                    \
    ( 0x02U ) /* use external clock 16x baud rate (ASYNC) \
               */
...
typedef enum e_sci_cb_evt // callback function events
{
    ...
    SCI_EVT_RX_CHAR_MATCH, // received a matched character; already placed in
                           // queue
...
typedef enum e_sci_cmd
{
    ...
    /* SSPI/Sync commands */
    SCI_CMD_CHECK_XFER_DONE, /* see if send, rcv, or both are done; SCI_SUCCESS
                                if yes */
    SCI_CMD_ABORT_XFER,
