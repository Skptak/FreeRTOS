...
typedef enum
{
    ...
    BSP_INT_SRC_EXC_UNDEFINED_INSTR, /* Occurs when MCU encounters an unknown
                                        instruction */
    ...
    BSP_INT_SRC_EMPTY,
    BSP_INT_SRC_TOTAL_ITEMS /* DO NOT MODIFY! This is used for sizing the
...
typedef enum
{
    BSP_INT_CMD_CALL_CALLBACK = 0, /* Calls registered callback function if one
                                      exists */
    BSP_INT_CMD_INTERRUPT_ENABLE,  /* Enables a given interrupt (Available for
                                      NMI pin, FPU, and Bus Error) */
    BSP_INT_CMD_INTERRUPT_DISABLE, /* Disables a given interrupt (Available for
                                      FPU, and Bus Error) */
    BSP_INT_CMD_GROUP_INTERRUPT_ENABLE,  /* Enables a group interrupt when a
                                            group interrupt source is given. The
                                            pdata argument should give the IPL to
                                            be used using the bsp_int_ctrl_t
                                            type. If a group interrupt is enabled
                                            multiple times with different IPL
                                            levels it will use the highest given
                                            IPL. */
    BSP_INT_CMD_GROUP_INTERRUPT_DISABLE, /* Disables a group interrupt when a
                                            group interrupt source is given.
...
typedef union
{
    uint32_t ipl; /* Used at the following times.
                     - When enabling an interrupt to set that interrupt's
                     priority level by BSP_INT_CMD_GROUP_INTERRUPT_ENABLE
                     command.
                     - When disabling an interrupt to save that interrupt's
                     priority level by BSP_INT_CMD_FIT_INTERRUPT_DISABLE
                     command.
                     - When enabling an interrupt to set that interrupt's
                     priority level by BSP_INT_CMD_FIT_INTERRUPT_ENABLE command.
                   */
} bsp_int_ctrl_t;
...
/* This structure is the common one that is passed as the 'void *' argument to
 * callback functions when an exception occurs.
 */
typedef struct
{
