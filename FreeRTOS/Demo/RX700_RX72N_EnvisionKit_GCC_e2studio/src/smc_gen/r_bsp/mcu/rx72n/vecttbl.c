 ...
 * File Name    : vecttbl.c
 * Device(s)    : RX72N
 * Description  : Definition of the exception vector table, reset vector, and
 *user boot options.
 ***********************************************************************************************************************/
/***********************************************************************************************************************
...
R_BSP_UB_POR_FUNCTION( R_BSP_UB_POWER_ON_RESET_FUNCTION );
    ...
    /***********************************************************************************************************************
     * The following array fills in the option function select registers, fixed
     *vector table, and the ID code protection bytes.
     ***********************************************************************************************************************/
    #ifdef __BIG
        ...
        /* The address range of bank 1 from FFC00000h to FFDFFFFFh and bank 0
         * from FFE00000h to FFFFFFFFh. */
        #define BSP_PRV_START_BANK_VALUE ( 0xffffffff )
    #else
        /* The address range of bank 1 from FFE00000h to FFFFFFFFh and bank 0
         * from FFC00000h to FFDFFFFFh. */
...
const st_ofsm_sec_ofs1_t __ofsm_sec_ofs1
    __attribute__( ( section( ".ofs1" ) ) ) = {
...
const uint32_t __BANKSELreg
    __attribute__( ( section( ".ofs3" ) ) ) = BSP_PRV_START_BANK_VALUE;
const uint32_t __SPCCreg __attribute__( ( section( ".ofs4" ) ) ) = 0xffffffff;
const uint32_t __TMEFreg
    __attribute__( ( section( ".ofs5" ) ) ) = BSP_CFG_TRUSTED_MODE_FUNCTION;
const st_ofsm_sec_ofs6_t __ofsm_sec_ofs6
    __attribute__( ( section( ".ofs6" ) ) ) = {
...
const uint32_t __FAWreg
    __attribute__( ( section( ".ofs7" ) ) ) = BSP_CFG_FAW_REG_VALUE;
const uint32_t __ROMCODEreg
    __attribute__( ( section( ".ofs8" ) ) ) = BSP_CFG_ROMCODE_REG_VALUE;

    #elif defined( __ICCRX__ )

        #pragma public_equ = "__MDE", \
                ( BSP_PRV_MDE_VALUE & BSP_PRV_BANK_MODE_VALUE )
    ...
    /* System configurator generates the ritble.src as interrupt & exception
     * vector tables. */
