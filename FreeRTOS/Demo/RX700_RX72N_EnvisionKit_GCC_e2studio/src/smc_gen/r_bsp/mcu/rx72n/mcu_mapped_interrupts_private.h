...
/* These macros are calculated values for x % 8. These macros are used when
 * preprocessor arithmetic cannot be used. For example, 'IEN0'-'IEN7' needs to
 * be defined for a macro based on 'vector_number % 8' but preprocessor
 * arithmetic is only evaluated at compile-time when it is used in a
 * preprocessor conditional. For this reason the calculated value of
 * 'vector_number % 8' must be calculated before compile-time.
 */
...
/* These macros are used to determine if the SLIBXRn or SLIBRn select register
...
/* Interrupt A interrupts are also defined even though there is only 1 select
 * register. */
...
/* Starting IER register for mapped interrupts. This is used for calculating the
 * IER register based on the given vector number.
 */
...
#define BSP_PRIV_CALC_IER_REG( x ) \
    ( ( ( x - BSP_PRV_B_INT_VECTOR_START ) / 8 ) + BSP_PRV_MAP_INT_IER_START )

/* These macros generate the macros needed to use the function-like macros from
 * iodefine.h */
