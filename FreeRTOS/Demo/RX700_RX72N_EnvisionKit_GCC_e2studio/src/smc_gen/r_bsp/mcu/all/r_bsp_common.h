 ...
 *         : 25.06.2013 1.10     Now contains standard includes (stdint.h,
 *stdbool.h, etc) as well as include for r_typedefs.h when needed. :
 *02.07.2013 1.11     Added #include for machine.h. : 10.02.2014 1.12 Changed
 *minor version to '40'. : 24.03.2014 1.12     Changed minor version to '60'.
...
***********************************************************************************************************************/
/* C99 (or later) is necessary because r_rx_compiler.h uses Pragma operator and
 * variadic macros. This means that r_typedefs.h is not used in any case. */
...
/* Multiple inclusion prevention macro */
    ...
    /* This macro is used to suppress compiler messages about not only a
     * parameter but also a auto variable not being used in a function. The nice
     * thing about using this implementation is that it does not take any extra
     * RAM or ROM. This macro is available for the followings: CC-RX's
     * 'M0520826:Parameter "XXXX" was never referenced' CC-RX's
     * 'W0520550:Variable "XXXX" was set but never used' GNURX's 'unused
     * parameter 'XXXX' [-Wunused-parameter]' GNURX's 'variable 'XXXX' set but
     * not used [-Wunused-but-set-variable]' When the variable is declared as
     * volatile, the '&' can be applied like
     * 'R_INTERNAL_NOT_USED(&volatile_variable);'.
     */
