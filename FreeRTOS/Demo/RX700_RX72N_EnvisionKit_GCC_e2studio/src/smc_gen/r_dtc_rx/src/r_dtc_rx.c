...
*         :                    Modified #define name from
"DTC_CFG_SHORT_ADDRRESS_MODE"
...
*         :                    DTC_CMD_SEQUENCE_TRANSFER_DISABLE and
DTC_CMD_SEQUENCE_TRANSFER_ABORT
*         :                    to R_DTC_Control().
*         : 31.01.2017 2.06    Added the default setting of "writeback_disable",
"sequence_end",
*         :                    "refer_index_table_enable" and "disp_add_enable"
in R_DTC_Create()
...
*         :                    Added DTC_CMD_CHANGING_DATA_FORCIBLY_SET command
to R_DTC_Control().
...
Macro definitions
*******************************************************************************/
#define DTC_PRV_ACT_BIT_MASK \
    ( 0x8000 ) /* DTC Active flag (DTCSTS.ACT) bit mask */
#define DTC_PRV_VECT_NR_MASK \
    ( 0x00FF ) /* DTC-Activating Vector Number bits mask */
#define DTC_PRV_MAX_16BITS_COUNT_VAL \
    ( 65536 ) /* The maximum value of 16bit count value */
#define DTC_PRV_MAX_8BITS_COUNT_VAL \
    ( 256 ) /* The maximum value of 8bit count value */
#define DTC_PRV_MIN_COUNT_VAL \
    ( 1 ) /* The minimum of count value  and block size */
#define DTC_PRV_ESPSEL_BIT_MASK \
    ( 0x8000 ) /* DTC Sequence transfer vector number setting bit mask */
...
static dtc_err_t r_dtc_check_create_param(
...
dtc_err_t R_DTC_Open( void )
{
    ...
    gp_dtc_table_work[ 0 ] = ( uint32_t * ) malloc(
        DTC_VECTOR_TABLE_SIZE_BYTES );
    ...
    p_dtc_table_work2 = ( uint8_t * ) ( ( uint32_t ) p_dtc_table_work2 &
                                        0xfffffc00 );

#if( DTC_ENABLE == DTC_CFG_DISABLE_ALL_ACT_SOURCE ) /* Clear all DTCER \
                                                       registers. */
...
#if( DTC_ENABLE == DTC_CFG_TRANSFER_DATA_READ_SKIP_EN ) /* Enable Data Read \
                                                           Skip. */
    DTC.DTCCR.BIT.RRS = 1;
...
dtc_err_t R_DTC_Close( void )
{
    ...
    /* Cast type of "gp_dtc_table_work" to match type of parameter in "free"
     * function */
                         ...
                         dtc_cmd_arg_t * p_args )
{
    ...
    else if( ( ( ( DTC_CMD_ACT_SRC_ENABLE == command ) ||
                 ( DTC_CMD_ACT_SRC_DISABLE == command ) ) ||
               ( DTC_CMD_CHAIN_TRANSFER_ABORT == command ) ) ||
             ( DTC_CMD_SEQUENCE_TRANSFER_ENABLE == command ) )
    {
    ...
    else if( DTC_CMD_CHANGING_DATA_FORCIBLY_SET == command )
    {
        ...
        if( r_dtc_check_create_param( p_args->p_transfer_data,
                                      p_args->p_data_cfg ) != DTC_SUCCESS )
        {
            return DTC_ERR_INVALID_COMMAND;
        }
    }
    ...
    switch( command )
    {
        ...
        case DTC_CMD_ACT_SRC_ENABLE: /* Select one interrupt as a DTC activation
                                        source. */
        {
        ...
        case DTC_CMD_ACT_SRC_DISABLE: /* Remove one interrupt as a DTC
                                         activation source. */
        {
        ...
        {
            /* Check DTC Status*/
            if( 0 == ( DTC.DTCSTS.WORD &
                       DTC_PRV_ACT_BIT_MASK ) ) /* DTC transfer operation is not
                                                   in progress. */
            {
            ...
            else /* DTC transfer operation is in progress. */
            {
                ...
                p_stat->vect_nr = ( uint8_t ) ( DTC.DTCSTS.WORD &
                                                DTC_PRV_VECT_NR_MASK ); /* get
                                                                           lower
                                                                           8
                                                                           bits:
                                                                           0-7*/
        ...
        case DTC_CMD_SEQUENCE_TRANSFER_ENABLE:
            ...
            /* Set the sequence transfer vector number and sequence transfer is
             * enabled. */
            DTC.DTCSQE.WORD = ( DTC_PRV_ESPSEL_BIT_MASK |
                                ( uint16_t ) p_args->act_src );
            break;
        ...
        case DTC_CMD_CHANGING_DATA_FORCIBLY_SET:
        {
            ...
            while( count > 0 )
            {
                if( r_dtc_set_transfer_data( p_args->p_transfer_data,
                                             p_args->p_data_cfg ) !=
                    DTC_SUCCESS )
                {
                    /* Fail to apply configurations for Transfer data. */
            ...
            /* The row in Vector table corresponding to act_source */
            p_ptr = ( uint32_t * ) ( ( uint32_t ) DTC.DTCVBR +
                                     ( 4 * p_args->act_src ) );
 ...
 * Description  : Applies configurations to a Transfer data area, it is an
 *internal function called by R_DTC_Create(); and all arguments are validated in
 *R_DTC_Create() Arguments    : transfer_data - Start address of Transfer data
 *                data_cfg -
                                          ...
                                          dtc_transfer_data_cfg_t * p_cfg )
{
    ...
    volatile dtc_internal_registers_t *
        p_td_ptr = ( volatile dtc_internal_registers_t * ) p_transfer_data;
    ...
    t_mra.BYTE = ( ( ( ( uint8_t ) p_cfg->writeback_disable |
                       ( uint8_t ) p_cfg->src_addr_mode ) |
                     ( uint8_t ) p_cfg->data_size ) |
                   ( uint8_t ) p_cfg->transfer_mode );

    /* Casting to match type of "t_mrb.BYTE" */
    t_mrb.BYTE = ( ( ( ( ( ( ( uint8_t ) p_cfg->sequence_end |
                             ( uint8_t ) p_cfg->refer_index_table_enable ) |
                           ( uint8_t ) p_cfg->dest_addr_mode ) |
                         ( uint8_t ) p_cfg->repeat_block_side ) |
                       ( uint8_t ) p_cfg->response_interrupt ) |
                     ( uint8_t ) p_cfg->chain_transfer_enable ) |
                   ( uint8_t ) p_cfg->chain_transfer_mode );
#else
    /* Casting to match type of "t_mra.BYTE" */
    t_mra.BYTE = ( uint8_t ) ( p_cfg->src_addr_mode | p_cfg->data_size |
                               p_cfg->transfer_mode );

    /* Casting to match type of "t_mrb.BYTE" */
    t_mrb.BYTE = ( uint8_t ) ( p_cfg->dest_addr_mode |
                               p_cfg->repeat_block_side |
                               p_cfg->response_interrupt |
                               p_cfg->chain_transfer_enable |
    ...
    switch( t_mra.BIT.MD ) /* DTC transfer mode */
    {
        ...
        {
            if( DTC_PRV_MAX_16BITS_COUNT_VAL ==
                p_cfg->transfer_count ) /* Transfer count = 65536 */
            {
                t_cra.WORD = 0x0000;
            }
            else /* 1 - 65535 */
            {
                /* Cast type of "p_cfg->transfer_count" to uint16_t to match
                 * type of "t_cra.WORD" */
        ...
        {
            /* Set counter. */
            if( p_cfg->transfer_count <
                DTC_PRV_MAX_8BITS_COUNT_VAL ) /* count 1-255 */
            {
                /* Cast type of "p_cfg->transfer_count" to match type of
                 * "t_cra.BYTE.CRA_H" */
        ...
        case 0x2: /* DTC_TRANSFER_MODE_BLOCK - Block transfer mode */
        {
            ...
            if( p_cfg->block_size < DTC_PRV_MAX_8BITS_COUNT_VAL ) /* Block size
                                                                     1-255 */
            {
                /* Cast type of "p_cfg->block_size" to match type of
                 * "t_cra.BYTE.CRA_H" */
            ...
            else if( DTC_PRV_MAX_8BITS_COUNT_VAL ==
                     p_cfg->block_size ) /* Block size = 256 */
            {
                t_cra.BYTE.CRA_H = 0;
                t_cra.BYTE.CRA_L = 0;
            }
    ...
    p_td_ptr->FIRST_LWORD.LWORD |= ( p_cfg->source_addr &
                                     0x00FFFFFF ); /* 3 byte SAR */

    /* settings for second long word: MRB & DAR */
    p_td_ptr->SECOND_LWORD.LWORD = 0;       /* clear */
    p_td_ptr->SECOND_LWORD.REG.MRB = t_mrb; /* 1 byte MRB */
    p_td_ptr->SECOND_LWORD.LWORD |= ( p_cfg->dest_addr & 0x00FFFFFF ); /* 3 byte
                                                                          DAR */
...
static bool r_dtc_abort_chain_transfer( uint32_t chain_transfer_nr )
{
    ...
    p_td_ptr = ( ( ( volatile dtc_internal_registers_t * ) *(
                     ( uint32_t * ) DTC.DTCVBR + status_reg ) ) +
                 chain_transfer_nr ) -
               1;
...
#if( ( 0 != BSP_CFG_USER_LOCKING_ENABLED ) ||       \
     ( bsp_lock_t != BSP_CFG_USER_LOCKING_TYPE ) || \
    ...
    dtc_transfer_data_cfg_t * p_data_cfg )
{
    ...
    #if( DTC_ENABLE == DTC_CFG_SHORT_ADDRESS_MODE ) /* Short-address mode */
    /* Address must be in: 0x00000000h to 0x007FFFFF and 0xFF800000 to
     * 0xFFFFFFFF */
    if( ( p_data_cfg->source_addr > 0x007FFFFF ) &&
        ( p_data_cfg->source_addr < 0xFF800000 ) )
    {
        return DTC_ERR_INVALID_ARG;
    }

    if( ( p_data_cfg->dest_addr > 0x007FFFFF ) &&
        ( p_data_cfg->dest_addr < 0xFF800000 ) )
    {
        return DTC_ERR_INVALID_ARG;
    }
