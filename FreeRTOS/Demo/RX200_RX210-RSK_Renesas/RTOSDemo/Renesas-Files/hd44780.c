...
#define OUT_DATA( d ) \
    LCD_DATA = ( d & 0x0F ) // LCD_DATA = ((LCD_DATA & 0xF0) | (d & 0x0F))	/*
                            // Output a byte d on the data bus (higher 4 bits of
                            // d in 4-bit mode) */
#define IF_DLY60() \
    {              \
        nop();     \
...
static void lcd_write( uint8_t reg, /* b0:command(0)/data(1),
                                       b2..1:E1(2)/E2(1)/both(0)(don't care on
                                       single controller), b3:write high nibble
...
void lcd_setcg( uint8_t chr,      /* Character code to be registered (0..7) */
                ...
                const uint8_t * p /* Pointer to the character pattern (8 * n
                                     bytes) */
)
{
...
void lcd_put_fuel( int8_t val, /* Fuel level (-1:plugged, 0:empty cell, ...,
                                  5:full cell) */
...
void lcd_put_bar( uint16_t val,  /* Bar length (0 to _MAX_BAR represents bar
                                    length from left end) */
                  uint8_t width, /* Display area (number of chars from cursor
                                    position) */
                  uint8_t chr /* User character code (2 chars used from this) */
)
{
    static const uint8_t ptn[] = {
        0xE0, 0xE0, 0xE0, 0xC0, 0xC0, 0xC0, 0x80, 0,    0xF0, 0xE0, 0xE0, 0xE0,
    ...
    s = val / 3 % 6;              /* Bit shift */
    for( n = 0; n < 7; n++ )
    { /* Draw edge pattern into the pattern buffer */
        m = ( *pp++ | 0xFF00 ) >> s;
        gfx[ n ] = m;
        gfx[ n + 8 ] = m >> 6;
    }
    ...
    for( n = 1; n <= width; n++ )
    { /* Draw each location in the bargraph */
        if( n == gi )
        {                /* When edge pattern is exist at the location */
...
void lcd_put_point( uint16_t val,  /* Dot position (0 to _MAX_POINT represents
                                      left end to write end) */
                    uint8_t width, /* Display area (number of chars from cursor
                                      position) */
                    uint8_t chr /* User character code (2 chars used from this)
                                 */
...
{
    static const uint8_t ptn[] = {
        0x06, 0x0C, 0x0C, 0x0C, 0x18, 0x18, 0x18, 0,    0x06, 0x06, 0x0C, 0x0C,
