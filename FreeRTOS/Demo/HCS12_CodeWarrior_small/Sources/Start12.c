...
Note: C++ destructors of global objects are NOT yet supported in the HIWARE
Object File Format. To use this feature, please build your application with the
...
#pragma DATA_SEG __NEAR_SEG STARTUP_DATA /* _startupData can be accessed using \
                                            16 bit accesses. This is needed    \
                                            because it contains the stack top, \
                                            and without stack, far data cannot \
                                            be accessed */
...
extern "C"
    #endif
    void
    _SET_PAGE( void ); /* the inline assembler needs a prototype */
...
/* for Monitor based software remap the RAM & EEPROM to adhere
   to EB386. Edit RAM and EEPROM sections in PRM file to match these. */
    ...
    #define __FEED_COP_IN_HLI()         \
        ...
        __asm movb #0x55, _COP_RST_ADR; \
        __asm movb #0xAA, _COP_RST_ADR; \
        __asm {
#else
...
nextInit:
    ...
    #if !defined( FAR_DATA ) && \
        ( defined( __BANKED__ ) || defined( __LARGE__ ) )
static void __far Fini( void )
    #else
static void Fini( void )
    #endif
...
/* The function _Startup must be called in order to initialize global variables
 * and to call main */
/* You can adapt this function or call it from your startup code to implement a
 * different startup */
/* functionality. */

/* You should also setup the needed IO registers as WINDEF (HC12A4 only) or the
 * COP registers to run */
/* on hardware */
...
/* the reset vector must be set so that the application has a defined entry
 * point */
...
void _Startup( void )
{
    ...
    for( ;; )
    { /* forever: initialize the program; call the root-procedure */
        if( !( _startupData.flags & STARTUP_FLAGS_NOT_INIT_SP ) )
        {
            /* initialize the stack pointer */
            INIT_SP_FROM_STARTUP_DESC();
            /*lint !e522 asm code */ /* HLI macro definition in hidef.h */
        }
...
#ifdef HC812A4 /* HC12 A4 derivative needs WINDEF to configure which pages are \
                  available */
        ...
        /* Here user defined code could be inserted, all global variables are
         * initilized */
        ...
        /* call destructors. Only done when this file is compiled as C++ and for
