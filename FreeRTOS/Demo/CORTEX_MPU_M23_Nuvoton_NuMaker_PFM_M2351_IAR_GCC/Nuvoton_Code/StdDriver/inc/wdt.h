...
extern "C" {
    ...
    */
    ...
    #define WDT_TIMEOUT_2POW4                                                \
        ( 0UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^4 * WDT clocks \hideinitializer */
    #define WDT_TIMEOUT_2POW6                                                \
        ( 1UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^6 * WDT clocks \hideinitializer */
    #define WDT_TIMEOUT_2POW8                                                \
        ( 2UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^8 * WDT clocks \hideinitializer */
    #define WDT_TIMEOUT_2POW10                                               \
        ( 3UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^10 * WDT clocks \hideinitializer \
                                        */
    #define WDT_TIMEOUT_2POW12                                               \
        ( 4UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^12 * WDT clocks \hideinitializer \
                                        */
    #define WDT_TIMEOUT_2POW14                                               \
        ( 5UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^14 * WDT clocks \hideinitializer \
                                        */
    #define WDT_TIMEOUT_2POW16                                               \
        ( 6UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^16 * WDT clocks \hideinitializer \
                                        */
    #define WDT_TIMEOUT_2POW18                                               \
        ( 7UL << WDT_CTL_TOUTSEL_Pos ) /*!< Setting WDT time-out interval to \
                                          2^18 * WDT clocks \hideinitializer \
                                        */
    ...
    #define WDT_RESET_DELAY_1026CLK                                           \
        ( 0UL << WDT_ALTCTL_RSTDSEL_Pos ) /*!< Setting WDT reset delay period \
                                             to 1026 * WDT clocks             \
                                             \hideinitializer */
    #define WDT_RESET_DELAY_130CLK                                            \
        ( 1UL << WDT_ALTCTL_RSTDSEL_Pos ) /*!< Setting WDT reset delay period \
                                             to 130 * WDT clocks              \
                                             \hideinitializer */
    #define WDT_RESET_DELAY_18CLK                                             \
        ( 2UL << WDT_ALTCTL_RSTDSEL_Pos ) /*!< Setting WDT reset delay period \
                                             to 18 * WDT clocks               \
                                             \hideinitializer */
    #define WDT_RESET_DELAY_3CLK                                              \
        ( 3UL << WDT_ALTCTL_RSTDSEL_Pos ) /*!< Setting WDT reset delay period \
                                             to 3 * WDT clocks                \
                                             \hideinitializer */
    ...
    #define WDT_RESET_COUNTER_KEYWORD                                        \
        ( 0x00005AA5UL ) /*!< Fill this value to WDT_RSTCNT register to free \
                            reset WDT counter \hideinitializer */
    ...
    */
    ...
    #define WDT_CLEAR_RESET_FLAG()                                          \
        ( WDT->CTL = ( WDT->CTL & ~( WDT_CTL_IF_Msk | WDT_CTL_WKF_Msk ) ) | \
                     WDT_CTL_RSTF_Msk )
    ...
    #define WDT_CLEAR_TIMEOUT_INT_FLAG()                                      \
        ( WDT->CTL = ( WDT->CTL & ~( WDT_CTL_RSTF_Msk | WDT_CTL_WKF_Msk ) ) | \
                     WDT_CTL_IF_Msk )
     ...
     *
     * @details    This macro indicates system has been reset by WDT time-out
     * reset or not. \hideinitializer
     */
    ...
    #define WDT_GET_TIMEOUT_INT_FLAG() \
        ( ( WDT->CTL & WDT_CTL_IF_Msk ) ? 1UL : 0UL )
     ...
     *
     * @details    This macro indicates WDT time-out interrupt event has waked
     * up system or not. \hideinitializer
     */
    #define WDT_GET_TIMEOUT_WAKEUP_FLAG() \
        ( ( WDT->CTL & WDT_CTL_WKF_Msk ) ? 1UL : 0UL )
     ...
     * @return     None
     *
     * @details    This macro is used to reset the internal 18-bit WDT up
     * counter value.
     * @note       If WDT is activated and time-out reset system function is
     * enabled also, user should \n reset the 18-bit WDT up counter value to
     * avoid generate WDT time-out reset signal to \n reset system before the
     * WDT time-out reset delay period expires. \hideinitializer
     */
