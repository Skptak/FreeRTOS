...
typedef enum NSATTR
{
    ...
    */
    ...
    /**
      ...
      *
      * @param[in]  nsattr     The secure/non-secure attribution of specified
      module. The possible value could be refer to \ref NSATTR.
      *
      * @return     None
      *
      * @details    This macro is used to set a peripheral to be non-secure
      peripheral.
      *
      */
    #define SCU_SET_PNSSET( nsattr )                                           \
        {                                                                      \
            SCU->PNSSET[ ( nsattr ) / 32 ] |= ( 1 << ( ( nsattr ) &0x1ful ) ); \
        }
     ...
     *
     * @details     This macro gets the peripheral secure/non-secure
     attribution.
     */
    #define SCU_GET_PNSSET( nsattr ) \
        ( ( SCU->PNSSET[ ( nsattr ) / 32 ] >> ( ( nsattr ) &0x1ful ) ) & 1ul )
     ...
     * @return      None
     *
     * @details     This macro is used to enable secure violation interrupt of
     * SCU. The secure violation interrupt could be used to detect attack of
     * secure elements.
     */
     ...
     * @return      None
     *
     * @details     This macro is used to disable secure violation interrupt of
     * SCU.
     *
     */
     ...
     * @return     None
     *
     * @details    Clear SCU related interrupt flags specified by flag
     * parameter.
     *
     */
    ...
    /**
      * @brief      Control the behavior of non-secure monitor when CPU is in
      idle state.
      *
      * @param[in]  opt Option for behavior control of non-secure monitor when
      CPU in idle.
      ...
      *
      */
    #define SCU_NSM_IDLE_ON( opt )                           \
        ( ( opt ) ? ( SCU->NSMCTL |= SCU_NSMCTL_IDLEON_Msk ) \
                  : ( SCU->NSMCTL &= ~SCU_NSMCTL_IDLEON_Msk ) )
    ...
    /**
      ...
      *
      */
    #define SCU_NSM_DBG_ON( opt )                           \
        ( ( opt ) ? ( SCU->NSMCTL |= SCU_NSMCTL_DBGON_Msk ) \
                  : ( SCU->NSMCTL &= ~SCU_NSMCTL_DBGON_Msk ) )
...
/**
  ...
  *
  * @param[in]  u32Ticks       A specified period for timeout in non-secure
  state
  ...
  * @return     None
  *
  * @details    This function is used to configure non-secure monitor. If the
  CPU state stay in non-secure state for
  *             a specified period. The non-secure monitor will timeout and
  assert an interrupt. Otherwise, the
  *             non-secure monitor will auto clear whenever returning to secure
  state. This could be used to avoid
  *             CPU state in non-secure state too long time for security
  purpose. User must enable SCU_IRQn if interrupt
  *             is necessary.
  *
  */
__STATIC_INLINE void SCU_NSMConfig( uint32_t u32Ticks, uint32_t u32Prescale )
{
    ...
    SCU->NSMVAL = 0ul;
    SCU->NSMCTL = SCU_NSMCTL_AUTORLD_Msk | SCU_NSMCTL_NSMIEN_Msk |
                  ( u32Prescale & 0xfful );
}
...
/**
  ...
  * @return     None
  *
  * @details    This function is used to configure non-secure monitor as a
  timer. In other words, the timer counter
...
__STATIC_INLINE void SCU_TimerConfig( uint32_t u32Ticks, uint32_t u32Prescale )
{
    ...
    SCU->NSMVAL = 0ul;
    SCU->NSMCTL = SCU_NSMCTL_AUTORLD_Msk | SCU_NSMCTL_NSMIEN_Msk |
                  SCU_NSMCTL_TMRMOD_Msk | ( u32Prescale & 0xfful );
}
