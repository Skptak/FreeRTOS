...
extern "C" {
    ...
    */
    ...
    #define I2C_CTL_STA_SI                                                    \
        ( 0x28U ) /*!< I2C_CTL setting for I2C control bits. It would set STA \
                     and SI bits          */
    #define I2C_CTL_STA_SI_AA                                                  \
        ( 0x2CU ) /*!< I2C_CTL setting for I2C control bits. It would set STA, \
                     SI and AA bits      */
    #define I2C_CTL_STO_SI                                                    \
        ( 0x18U ) /*!< I2C_CTL setting for I2C control bits. It would set STO \
                     and SI bits          */
    #define I2C_CTL_STO_SI_AA                                                  \
        ( 0x1CU ) /*!< I2C_CTL setting for I2C control bits. It would set STO, \
                     SI and AA bits      */
    #define I2C_CTL_SI                                                       \
        ( 0x08U ) /*!< I2C_CTL setting for I2C control bits. It would set SI \
                     bit                   */
    #define I2C_CTL_SI_AA                                                    \
        ( 0x0CU ) /*!< I2C_CTL setting for I2C control bits. It would set SI \
                     and AA bits           */
    #define I2C_CTL_STA                                                       \
        ( 0x20U ) /*!< I2C_CTL setting for I2C control bits. It would set STA \
                     bit                  */
    #define I2C_CTL_STO                                                       \
        ( 0x10U ) /*!< I2C_CTL setting for I2C control bits. It would set STO \
                     bit                  */
    #define I2C_CTL_AA                                                       \
        ( 0x04U ) /*!< I2C_CTL setting for I2C control bits. It would set AA \
                     bit                   */
    ...
    #define I2C_PECTX_ENABLE \
        ( 1U ) /*!< Enable  SMBus Packet Error Check Transmit function */
    #define I2C_PECTX_DISABLE \
        ( 0U ) /*!< Disable SMBus Packet Error Check Transmit function */
    ...
    */
     ...
     *    @return       None
     *
     *    @details      Set I2C_CTL register to control I2C bus conditions of
     * START, STOP, SI, ACK.
     */
    #define I2C_SET_CONTROL_REG( i2c, u8Ctrl ) \
        ( ( i2c )->CTL0 = ( ( i2c )->CTL0 & ~0x3Cu ) | ( u8Ctrl ) )
    ...
    #define I2C_START( i2c )                                    \
        ( ( i2c )->CTL0 = ( ( i2c )->CTL0 | I2C_CTL0_SI_Msk ) | \
                          I2C_CTL0_STA_Msk )
     ...
     *    @return       None
     *
     *    @details      When a new status is presented of I2C bus, the SI flag
     * will be set in I2C_CTL register.
     */
     ...
     *    @return       None
     *
     *    @details      When write a data to I2C_DAT register, the I2C
     * controller will shift it to I2C bus.
     */
     ...
     *
     *    @details      When I2C bus occurs time-out event, the time-out flag
     * will be set.
     */
    #define I2C_GET_TIMEOUT_FLAG( i2c )                                      \
        ( ( ( i2c )->TOCTL & I2C_TOCTL_TOIF_Msk ) == I2C_TOCTL_TOIF_Msk ? 1u \
                                                                        : 0u )
    ...
    #define I2C_GET_WAKEUP_FLAG( i2c )                                       \
        ( ( ( i2c )->WKSTS & I2C_WKSTS_WKIF_Msk ) == I2C_WKSTS_WKIF_Msk ? 1u \
                                                                        : 0u )
     ...
     *    @param[in]    i2c     Specify I2C port
     *
     *    @retval       0       The ACK bit cycle of address match frame is not
     * done
     *    @retval       1       The ACK bit cycle of address match frame is done
     * in power-down
     *
     *    @details      I2C bus occurs wake-up event and address frame ACK is
     * done, this flag will be set.
     *
     *    \hideinitializer
     */
    #define I2C_GET_WAKEUP_DONE( i2c )                   \
        ( ( ( i2c )->WKSTS & I2C_WKSTS_WKAKDONE_Msk ) == \
     ...
     *    \hideinitializer
     */
    #define I2C_CLEAR_WAKEUP_DONE( i2c ) \
        ( ( i2c )->WKSTS = I2C_WKSTS_WKAKDONE_Msk )
     ...
     *    @param[in]    i2c     Specify I2C port
     *
     *    @retval       0       Write command be record on the address match
     * wakeup frame
     *    @retval       1       Read command be record on the address match
     * wakeup frame.
     *
     *    @details      I2C bus occurs wake-up event and address frame is
     * received, this bit will record read/write status.
     *
     *    \hideinitializer
     */
    #define I2C_GET_WAKEUP_WR_STATUS( i2c )                                   \
        ( ( ( i2c )->WKSTS & I2C_WKSTS_WRSTSWK_Msk ) == I2C_WKSTS_WRSTSWK_Msk \
              ? 1u                                                            \
              : 0u )
     ...
     *
     * @details    The CRC check value after a transmission or a reception by
     * count by using CRC8
     *
     */
     ...
     * @return     None
     *
     * @details    The transmission or receive byte number in one transaction
     * when PECEN is set. The maximum is 255 bytes.
     *
     */
     ...
     * @return     None
     *
     * @details    Device Mode(BMHEN=0): If ALERTEN(I2C_BUSCTL[4]) is set, the
     * Alert pin will pull lo, and reply ACK when get ARP from host Host
     * Mode(BMHEN=1): If ALERTEN(I2C_BUSCTL[4]) is set, the Alert pin is
     * supported to receive alert state(Lo trigger)
     *
     */
    #define I2C_SMBUS_ENABLE_ALERT( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_ALERTEN_Msk )
     ...
     * @return     None
     *
     * @details    Device Mode(BMHEN=0): If ALERTEN(I2C_BUSCTL[4]) is clear, the
     * Alert pin will pull hi, and reply NACK when get ARP from host Host
     * Mode(BMHEN=1): If ALERTEN(I2C_BUSCTL[4]) is clear, the Alert pin is not
     * supported to receive alert state(Lo trigger)
     *
     */
     ...
     * @return     None
     *
     * @details    This function to set SUSCON(I2C_BUSCTL[6]) pin is output
     * mode.
     *
     *
     */
    #define I2C_SMBUS_SET_SUSCON_OUT( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_SCTLOEN_Msk )
     ...
     * @return     None
     *
     * @details    This function to set SUSCON(I2C_BUSCTL[6]) pin is output hi
     * state.
     *
     */
    #define I2C_SMBUS_SET_SUSCON_HIGH( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_SCTLOSTS_Msk )
     ...
     * @return     None
     *
     * @details    This function to set SUSCON(I2C_BUSCTL[6]) pin is output lo
     * state.
     *
     */
     ...
     * @return     None
     *
     * @details    The 9th bit can response the ACK or NACK according the
     * received data by user. When the byte is received, SCLK line stretching to
     * low between the 8th and 9th SCLK pulse.
     *
     */
    #define I2C_SMBUS_ACK_MANUAL( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_ACKMEN_Msk )
     ...
     * @return     None
     *
     * @details    This function is used to enable SMBUS acknowledge manual
     * interrupt on the 9th clock cycle when SMBUS=1 and ACKMEN=1
     *
     */
    #define I2C_SMBUS_9THBIT_INT_ENABLE( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_ACKM9SI_Msk )
     ...
     * @return     None
     *
     * @details    This function is used to disable SMBUS acknowledge manual
     * interrupt on the 9th clock cycle when SMBUS=1 and ACKMEN=1
     *
     */
     ...
     * @return     None
     *
     * @details    This function is used to enable the condition of REAEAT START
     * can clear the PEC calculation.
     *
     */
    #define I2C_SMBUS_RST_PEC_AT_START_ENABLE( i2c ) \
        ( ( i2c )->BUSCTL |= I2C_BUSCTL_PECCLR_Msk )
     ...
     * @return     None
     *
     * @details    This function is used to disable the condition of Repeat
     * START can clear the PEC calculation.
     *
     */
     ...
     * @return     None.
     * @details    Set RXPDMAEN bit of I2C_CTL1 register to enable RX PDMA
     * transfer function.
     */
     ...
     * @param[in]  i2c The pointer of the specified I2C module.
     * @return     None.
     * @details    Set TXPDMAEN bit of I2C_CTL1 register to enable TX PDMA
     * transfer function.
     */
     ...
     * @param[in]  i2c The pointer of the specified I2C module.
     * @return     None.
     * @details    Clear RXPDMAEN bit of I2C_CTL1 register to disable RX PDMA
     * transfer function.
     */
    #define I2C_DISABLE_RX_PDMA( i2c ) \
        ( ( i2c )->CTL1 &= ~I2C_CTL1_RXPDMAEN_Msk )

    /**
     * @brief      Disable TX PDMA transfer.
     * @param[in]  i2c The pointer of the specified I2C module.
     * @return     None.
     * @details    Clear TXPDMAEN bit of I2C_CTL1 register to disable TX PDMA
     * transfer function.
     */
    #define I2C_DISABLE_TX_PDMA( i2c ) \
        ( ( i2c )->CTL1 &= ~I2C_CTL1_TXPDMAEN_Msk )

    /**
     * @brief      Enable PDMA stretch function.
     * @param[in]  i2c The pointer of the specified I2C module.
     * @return     None.
     * @details    Enable this function is to stretch bus by hardware after PDMA
     * transfer is done if SI is not cleared.
     */
    #define I2C_ENABLE_PDMA_STRETCH( i2c ) \
        ( ( i2c )->CTL1 |= I2C_CTL1_PDMASTR_Msk )
