...
*/
    ...
    #define PRNG_KEY_SIZE_64 \
        0UL /*!< Select to generate 64-bit random key    \hideinitializer */
    #define PRNG_KEY_SIZE_128 \
        1UL /*!< Select to generate 128-bit random key   \hideinitializer */
    #define PRNG_KEY_SIZE_192 \
        2UL /*!< Select to generate 192-bit random key   \hideinitializer */
    #define PRNG_KEY_SIZE_256 \
        3UL /*!< Select to generate 256-bit random key   \hideinitializer */

    #define PRNG_SEED_CONT \
        0UL /*!< PRNG using current seed                 \hideinitializer */
    #define PRNG_SEED_RELOAD \
        1UL /*!< PRNG reload new seed                    \hideinitializer */

    #define AES_KEY_SIZE_128 \
        0UL /*!< AES select 128-bit key length           \hideinitializer */
    #define AES_KEY_SIZE_192 \
        1UL /*!< AES select 192-bit key length           \hideinitializer */
    #define AES_KEY_SIZE_256 \
        2UL /*!< AES select 256-bit key length           \hideinitializer */

    #define AES_MODE_ECB \
        0UL /*!< AES select ECB mode                     \hideinitializer */
    #define AES_MODE_CBC \
        1UL /*!< AES select CBC mode                     \hideinitializer */
    #define AES_MODE_CFB \
        2UL /*!< AES select CFB mode                     \hideinitializer */
    #define AES_MODE_OFB \
        3UL /*!< AES select OFB mode                     \hideinitializer */
    #define AES_MODE_CTR \
        4UL /*!< AES select CTR mode                     \hideinitializer */
    #define AES_MODE_CBC_CS1                                                 \
        0x10UL /*!< AES select CBC CS1 mode                 \hideinitializer \
                */
    #define AES_MODE_CBC_CS2                                                 \
        0x11UL /*!< AES select CBC CS2 mode                 \hideinitializer \
                */
    #define AES_MODE_CBC_CS3                                                 \
        0x12UL /*!< AES select CBC CS3 mode                 \hideinitializer \
                */

    #define AES_NO_SWAP \
        0UL /*!< AES do not swap input and output data   \hideinitializer */
    #define AES_OUT_SWAP \
        1UL /*!< AES swap output data                    \hideinitializer */
    #define AES_IN_SWAP \
        2UL /*!< AES swap input data                     \hideinitializer */
    #define AES_IN_OUT_SWAP \
        3UL /*!< AES swap both input and output data     \hideinitializer */

    #define DES_MODE_ECB                                                      \
        0x000UL /*!< DES select ECB mode                     \hideinitializer \
                 */
    #define DES_MODE_CBC                                                      \
        0x100UL /*!< DES select CBC mode                     \hideinitializer \
                 */
    #define DES_MODE_CFB                                                      \
        0x200UL /*!< DES select CFB mode                     \hideinitializer \
                 */
    #define DES_MODE_OFB                                                      \
        0x300UL /*!< DES select OFB mode                     \hideinitializer \
                 */
    #define DES_MODE_CTR                                                      \
        0x400UL /*!< DES select CTR mode                     \hideinitializer \
                 */
    #define TDES_MODE_ECB                                                     \
        0x004UL /*!< TDES select ECB mode                    \hideinitializer \
                 */
    #define TDES_MODE_CBC                                                     \
        0x104UL /*!< TDES select CBC mode                    \hideinitializer \
                 */
    #define TDES_MODE_CFB                                                     \
        0x204UL /*!< TDES select CFB mode                    \hideinitializer \
                 */
    #define TDES_MODE_OFB                                                     \
        0x304UL /*!< TDES select OFB mode                    \hideinitializer \
                 */
    #define TDES_MODE_CTR                                                     \
        0x404UL /*!< TDES select CTR mode                    \hideinitializer \
                 */

    #define TDES_NO_SWAP                                                      \
        0UL /*!< TDES do not swap data                       \hideinitializer \
             */
    #define TDES_WHL_SWAP                                                     \
        1UL /*!< TDES swap high-low word                     \hideinitializer \
             */
    #define TDES_OUT_SWAP                                                     \
        2UL /*!< TDES swap output data                       \hideinitializer \
             */
    #define TDES_OUT_WHL_SWAP                                                 \
        3UL /*!< TDES swap output data and high-low word     \hideinitializer \
             */
    #define TDES_IN_SWAP                                                      \
        4UL /*!< TDES swap input data                        \hideinitializer \
             */
    #define TDES_IN_WHL_SWAP                                                  \
        5UL /*!< TDES swap input data and high-low word      \hideinitializer \
             */
    #define TDES_IN_OUT_SWAP                                                  \
        6UL /*!< TDES swap both input and output data        \hideinitializer \
             */
    #define TDES_IN_OUT_WHL_SWAP                                              \
        7UL /*!< TDES swap input, output and high-low word   \hideinitializer \
             */

    #define SHA_MODE_SHA1 \
        0UL /*!< SHA select SHA-1 160-bit                \hideinitializer */
    #define SHA_MODE_SHA224 \
        5UL /*!< SHA select SHA-224 224-bit              \hideinitializer */
    #define SHA_MODE_SHA256 \
        4UL /*!< SHA select SHA-256 256-bit              \hideinitializer */
    #define SHA_MODE_SHA384 \
        7UL /*!< SHA select SHA-384 384-bit              \hideinitializer */
    #define SHA_MODE_SHA512 \
        6UL /*!< SHA select SHA-512 512-bit              \hideinitializer */

    #define SHA_NO_SWAP \
        0UL /*!< SHA do not swap input and output data   \hideinitializer */
    #define SHA_OUT_SWAP \
        1UL /*!< SHA swap output data                    \hideinitializer */
    #define SHA_IN_SWAP \
        2UL /*!< SHA swap input data                     \hideinitializer */
    #define SHA_IN_OUT_SWAP \
        3UL /*!< SHA swap both input and output data     \hideinitializer */

    #define CRYPTO_DMA_FIRST \
        0x4UL /*!< Do first encrypt/decrypt in DMA cascade \hideinitializer */
    #define CRYPTO_DMA_ONE_SHOT                                               \
        0x5UL /*!< Do one shot encrypt/decrypt with DMA      \hideinitializer \
               */
    #define CRYPTO_DMA_CONTINUE                                 \
        0x6UL /*!< Do continuous encrypt/decrypt in DMA cascade \
                 \hideinitializer */
    #define CRYPTO_DMA_LAST \
        0x7UL /*!< Do last encrypt/decrypt in DMA cascade \hideinitializer */
...
typedef struct e_curve_t
{
    ...
    */
    ...
    #define PRNG_ENABLE_INT( crpt ) \
        ( ( crpt )->INTEN |= CRPT_INTEN_PRNGIEN_Msk )
    ...
    #define AES_ENABLE_INT( crpt )                     \
        ( ( crpt )->INTEN |= ( CRPT_INTEN_AESIEN_Msk | \
    ...
    #define AES_DISABLE_KEY_PROTECT( crpt )                              \
        ( ( crpt )->AES_CTL = ( ( crpt )->AES_CTL &                      \
                                ~CRPT_AES_CTL_KEYPRT_Msk ) |             \
                              ( 0x16UL << CRPT_AES_CTL_KEYUNPRT_Pos ) ); \
        ( ( crpt )->AES_CTL &= ~CRPT_AES_CTL_KEYPRT_Msk )
    ...
    #define TDES_ENABLE_INT( crpt )                     \
        ( ( crpt )->INTEN |= ( CRPT_INTEN_TDESIEN_Msk | \
    ...
    #define TDES_GET_INT_FLAG( crpt ) \
        ( ( crpt )->INTSTS &          \
          ( CRPT_INTSTS_TDESIF_Msk | CRPT_INTSTS_TDESEIF_Msk ) )
    ...
    #define TDES_CLR_INT_FLAG( crpt )                   \
        ( ( crpt )->INTSTS = ( CRPT_INTSTS_TDESIF_Msk | \
    ...
    #define TDES_DISABLE_KEY_PROTECT( crpt )                               \
        ( ( crpt )->TDES_CTL = ( ( crpt )->TDES_CTL &                      \
                                 ~CRPT_TDES_CTL_KEYPRT_Msk ) |             \
                               ( 0x16UL << CRPT_TDES_CTL_KEYUNPRT_Pos ) ); \
        ( ( crpt )->TDES_CTL &= ~CRPT_TDES_CTL_KEYPRT_Msk )
    ...
    #define SHA_GET_INT_FLAG( crpt ) \
        ( ( crpt )->INTSTS &         \
          ( CRPT_INTSTS_HMACIF_Msk | CRPT_INTSTS_HMACEIF_Msk ) )
    ...
    #define ECC_GET_INT_FLAG( crpt ) \
        ( ( crpt )->INTSTS &         \
          ( CRPT_INTSTS_ECCIF_Msk | CRPT_INTSTS_ECCEIF_Msk ) )
