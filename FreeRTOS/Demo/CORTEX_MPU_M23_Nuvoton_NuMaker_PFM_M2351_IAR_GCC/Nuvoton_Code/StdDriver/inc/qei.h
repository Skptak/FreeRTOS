...
extern "C" {
    ...
    */
    ...
    #define QEI_CTL_X4_FREE_COUNTING_MODE                                     \
        ( 0x0 << QEI_CTL_MODE_Pos ) /*!< QEI operate in X4 free-counting mode \
                                       \hideinitializer */
    #define QEI_CTL_X2_FREE_COUNTING_MODE                                     \
        ( 0x1 << QEI_CTL_MODE_Pos ) /*!< QEI operate in X2 free-counting mode \
                                       \hideinitializer */
    #define QEI_CTL_X4_COMPARE_COUNTING_MODE                                \
        ( 0x2 << QEI_CTL_MODE_Pos ) /*!< QEI operate in X4 compare-counting \
                                       mode \hideinitializer */
    #define QEI_CTL_X2_COMPARE_COUNTING_MODE                                \
        ( 0x3 << QEI_CTL_MODE_Pos ) /*!< QEI operate in X2 compare-counting \
                                       mode \hideinitializer */
    ...
    #define QEI_CTL_NFCLKSEL_DIV1                                          \
        ( 0x0 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK         \
                                           \hideinitializer */
    #define QEI_CTL_NFCLKSEL_DIV2                                          \
        ( 0x1 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK/2       \
                                           \hideinitializer */
    #define QEI_CTL_NFCLKSEL_DIV4                                          \
        ( 0x2 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK/4       \
                                           \hideinitializer */
    #define QEI_CTL_NFCLKSEL_DIV16                                         \
        ( 0x3 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK/16      \
                                           \hideinitializer */
    #define QEI_CTL_NFCLKSEL_DIV32                                         \
        ( 0x4 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK/32      \
                                           \hideinitializer */
    #define QEI_CTL_NFCLKSEL_DIV64                                         \
        ( 0x5 << QEI_CTL_NFCLKSEL_Pos ) /*!< The sampling frequency of the \
                                           noise filter is QEI_CLK/64      \
                                           \hideinitializer */
    ...
    */
     ...
     * @return     None
     * @details    This macro disable specified QEI signal inverted input
     * polarity. \hideinitializer
     */
     ...
     *                         - \ref QEI_CTL_DIRIEN_Msk   : Direction change
     * interrupt
     *                         - \ref QEI_CTL_OVUNIEN_Msk  : Counter overflow or
     * underflow interrupt
     *                         - \ref QEI_CTL_CMPIEN_Msk   : Compare-match
     * interrupt
     *                         - \ref QEI_CTL_IDXIEN_Msk   : Index detected
     * interrupt
     * @return     None
     ...
     * @return     None
     * @details    This macro enable QEI noise filter function and select noise
     * filter clock. \hideinitializer
     */
    #define QEI_ENABLE_NOISE_FILTER( qei, u32ClkSel )                       \
        ( ( qei )->CTL = ( ( qei )->CTL & ( ~( QEI_CTL_NFDIS_Msk |          \
    ...
    #define QEI_GET_DIR( qei ) \
        ( ( ( qei )->STATUS & ( QEI_STATUS_DIRF_Msk ) ) ? 1 : 0 )

    /**
     * @brief      Get QEI counter hold value
     * @param[in]  qei     The pointer of the specified QEI module.
     * @return     QEI pulse counter hold register value.
     * @details    This macro get QEI pulse counter hold value, which is updated
     * with counter value in hold counter value control. \hideinitializer
     */
     ...
     * @details    This macro get QEI pulse counter index latch value, which is
     * updated with counter value when the index is detected. \hideinitializer
     */
     ...
     * @return     QEI pulse counter index latch value
     * @details    This macro set QEI pulse counter index latch value, which is
     * updated with counter value when the index is detected. \hideinitializer
     */
     ...
     * @param[in]  qei         The pointer of the specified QEI module.
     * @param[in]  u32IntSel   Interrupt type selection.
     *                           - \ref QEI_STATUS_DIRF_Msk      : Counting
     * direction flag
     *                         - \ref QEI_STATUS_DIRCHGF_Msk   : Direction
     * change flag
     *                         - \ref QEI_STATUS_OVUNF_Msk     : Counter
     * overflow or underflow flag
     *                         - \ref QEI_STATUS_CMPF_Msk      : Compare-match
     * flag
     *                         - \ref QEI_STATUS_IDXF_Msk      : Index detected
     * flag
     ...
     *                         - \ref QEI_CTL_HOLDCNT_Msk      : Hold QEI_CNT
     * control
     *                         - \ref QEI_CTL_HOLDTMR0_Msk     : Hold QEI_CNT by
     * Timer0
     *                         - \ref QEI_CTL_HOLDTMR1_Msk     : Hold QEI_CNT by
     * Timer1
     *                         - \ref QEI_CTL_HOLDTMR2_Msk     : Hold QEI_CNT by
     * Timer2
     *                         - \ref QEI_CTL_HOLDTMR3_Msk     : Hold QEI_CNT by
     * Timer3
     * @return     None
    ...
    #define QEI_SET_CNT_MODE( qei, u32Mode )                        \
        ( ( qei )->CTL = ( ( qei )->CTL & ( ~QEI_CTL_MODE_Msk ) ) | \
                         ( u32Mode ) )
