...
*/
    ...
    #define QSPI_MODE_0 \
        ( QSPI_CTL_TXNEG_Msk ) /*!< CLKPOL=0; RXNEG=0; TXNEG=1 */
    #define QSPI_MODE_1 \
        ( QSPI_CTL_RXNEG_Msk ) /*!< CLKPOL=0; RXNEG=1; TXNEG=0 */
    #define QSPI_MODE_2                                                      \
        ( QSPI_CTL_CLKPOL_Msk | QSPI_CTL_RXNEG_Msk ) /*!< CLKPOL=1; RXNEG=1; \
                                                        TXNEG=0 */
    #define QSPI_MODE_3                                                      \
        ( QSPI_CTL_CLKPOL_Msk | QSPI_CTL_TXNEG_Msk ) /*!< CLKPOL=1; RXNEG=0; \
                                                        TXNEG=1 */
    ...
    #define QSPI_SS_ACTIVE_HIGH \
        ( QSPI_SSCTL_SSACTPOL_Msk )      /*!< SS active high */
    #define QSPI_SS_ACTIVE_LOW ( 0x0UL ) /*!< SS active low */

    /* QSPI Interrupt Mask */
    #define QSPI_UNIT_INT_MASK                        \
        ( 0x001UL ) /*!< Unit transfer interrupt mask \
                     */
    ...
    #define QSPI_SLVTO_INT_MASK \
        ( 0x020UL ) /*!< Slave Mode Time-out interrupt mask */
    ...
    */
     ...
     * @return     None.
     * @details    Write 1 to UNITIF bit of QSPI_STATUS register to clear the
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear TWOBIT bit of QSPI_CTL register to disable 2-bit
     * Transfer mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear SLV3WIRE bit of QSPI_SSCTL register to disable Slave
     * 3-wire mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear DUALIOEN bit of QSPI_CTL register to disable Dual I/O
     * mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear QUADIOEN bit of QSPI_CTL register to disable Quad I/O
     * mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set TWOBIT bit of QSPI_CTL register to enable 2-bit Transfer
     * mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set SLV3WIRE bit of QSPI_SSCTL register to enable Slave
     * 3-wire mode.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear DATDIR bit and set DUALIOEN bit of QSPI_CTL register to
     * enable Dual input mode.
     */
    #define QSPI_ENABLE_DUAL_INPUT_MODE( qspi )                          \
        ( ( qspi )->CTL = ( ( qspi )->CTL & ( ~QSPI_CTL_DATDIR_Msk ) ) | \
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set DATDIR bit and DUALIOEN bit of QSPI_CTL register to
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear DATDIR bit and set QUADIOEN bit of QSPI_CTL register to
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set DATDIR bit and QUADIOEN bit of QSPI_CTL register to
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set RXPDMAEN bit of QSPI_PDMACTL register to enable RX PDMA
     * transfer function.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set TXPDMAEN bit of QSPI_PDMACTL register to enable TX PDMA
     * transfer function.
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear RXPDMAEN bit of QSPI_PDMACTL register to disable RX
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear TXPDMAEN bit of QSPI_PDMACTL register to disable TX
     * PDMA transfer function.
     */
     ...
     * @details    Read RXCNT (QSPI_STATUS[27:24]) to get the count of available
     * data in RX FIFO.
     */
    #define QSPI_GET_RX_FIFO_COUNT( qspi )                \
        ( ( ( qspi )->STATUS & QSPI_STATUS_RXCNT_Msk ) >> \
     ...
     * @details    Read RXEMPTY bit of QSPI_STATUS register to get the RX FIFO
     * empty flag.
     */
    #define QSPI_GET_RX_FIFO_EMPTY_FLAG( qspi )             \
        ( ( ( qspi )->STATUS & QSPI_STATUS_RXEMPTY_Msk ) >> \
     ...
     * @details    Read TXEMPTY bit of QSPI_STATUS register to get the TX FIFO
     * empty flag.
     */
    #define QSPI_GET_TX_FIFO_EMPTY_FLAG( qspi )             \
        ( ( ( qspi )->STATUS & QSPI_STATUS_TXEMPTY_Msk ) >> \
     ...
     * @details    Read TXFULL bit of QSPI_STATUS register to get the TX FIFO
     * full flag.
     */
    #define QSPI_GET_TX_FIFO_FULL_FLAG( qspi )             \
        ( ( ( qspi )->STATUS & QSPI_STATUS_TXFULL_Msk ) >> \
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Disable automatic slave selection function and set QSPIx_SS
     * pin to high state.
     */
     ...
     * the setting of SUSPITV (QSPI_CTL[7:4]).
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear REORDER bit field of QSPI_CTL register to disable Byte
     * Reorder function.
     */
     ...
     * length of one QSPI bus clock cycle).
     */
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Set LSB bit of QSPI_CTL register to set the QSPI transfer
     ...
     * @param[in]  qspi The pointer of the specified QSPI module.
     * @return     None.
     * @details    Clear LSB bit of QSPI_CTL register to set the QSPI transfer
