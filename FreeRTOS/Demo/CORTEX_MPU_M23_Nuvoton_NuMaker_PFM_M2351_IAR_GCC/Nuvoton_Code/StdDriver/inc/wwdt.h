...
extern "C" {
    ...
    */
    ...
    #define WWDT_PRESCALER_1                                                \
        ( 0 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 1 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_2                                                \
        ( 1 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 2 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_4                                                \
        ( 2 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 4 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_8                                                \
        ( 3 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 8 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_16                                                \
        ( 4 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 16 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_32                                                \
        ( 5 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 32 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_64                                                \
        ( 6 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 64 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_128                                                \
        ( 7 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 128 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_192                                                \
        ( 8 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 192 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_256                                                \
        ( 9 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 256 * \
                                        (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_384                                                 \
        ( 10 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 384 * \
                                         (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_512                                                 \
        ( 11 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 512 * \
                                         (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_768                                                 \
        ( 12 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 768 * \
                                         (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_1024                                               \
        ( 13 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 1024 \
                                       * (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_1536                                               \
        ( 14 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 1536 \
                                       * (64*WWDT_CLK) \hideinitializer */
    #define WWDT_PRESCALER_2048                                               \
        ( 15 << WWDT_CTL_PSCSEL_Pos ) /*!< Select max time-out period to 2048 \
                                       * (64*WWDT_CLK) \hideinitializer */
    ...
    #define WWDT_RELOAD_WORD                                                  \
        ( 0x00005AA5 ) /*!< Fill this value to WWDT_RLDCNT register to reload \
                          WWDT counter \hideinitializer */
    ...
    */
     ...
     * @return     None
     *
     * @details    This macro is used to clear WWDT compared match interrupt
     ...
     *
     * @details    This macro is used to indicate system has been reset by WWDT
     * time-out reset or not. \hideinitializer
     */
    #define WWDT_GET_RESET_FLAG() \
        ( ( WWDT->STATUS & WWDT_STATUS_WWDTRF_Msk ) ? 1 : 0 )
     ...
     *
     * @details    This macro is used to indicate WWDT counter value matches
     * CMPDAT value or not. \hideinitializer
     */
    #define WWDT_GET_INT_FLAG() \
        ( ( WWDT->STATUS & WWDT_STATUS_WWDTIF_Msk ) ? 1 : 0 )
     ...
     * @note       User can only write WWDT_RLDCNT register to reload WWDT
     * counter value when current WWDT counter value \n between 0 and CMPDAT
     * value. If user writes WWDT_RLDCNT when current WWDT counter value is
     * larger than CMPDAT, \n WWDT reset signal will generate immediately to
     * reset system. \hideinitializer
     */
