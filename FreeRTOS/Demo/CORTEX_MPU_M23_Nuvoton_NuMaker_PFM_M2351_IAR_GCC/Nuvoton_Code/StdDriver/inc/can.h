...
typedef struct
{
    ...
    */
     ...
     *
     * @details Status Interrupt is generated by bits BOff (CAN_STATUS[7]),
     * EWarn (CAN_STATUS[6]), EPass (CAN_STATUS[5]), RxOk (CAN_STATUS[4]), TxOk
     * (CAN_STATUS[3]), and LEC (CAN_STATUS[2:0]).
     */
     ...
     *
     * @details If several interrupts are pending, the CAN Interrupt Register
     * will point to the pending interrupt with the highest priority,
     * disregarding their chronological order.
     */
     ...
     * @return None
     *
     * @details User can wake-up system when there is a falling edge in the
     * CAN_Rx pin.
     */
     ...
     *
     * @param[in] can The base address of can module.
     * @param[in] u32MsgNum Specified Message Object number, valid value are
     * from 0 to 31.
     *
     * @return Specified Message Object new data into bit value.
     *
     * @details The NewDat bit (CAN_IFn_MCON[15]) of a specific Message Object
     * can be set/reset by the software through the IFn Message Interface
     * Registers or by the Message Handler after reception of a Data Frame or
     * after a successful transmission.
     */
    #define CAN_GET_NEW_DATA_IN_BIT( can, u32MsgNum )   \
        ( ( u32MsgNum ) < 16                            \
              ? ( can )->NDAT1 & ( 1 << ( u32MsgNum ) ) \
              : ( can )->NDAT2 & ( 1 << ( ( u32MsgNum ) -16 ) ) )
