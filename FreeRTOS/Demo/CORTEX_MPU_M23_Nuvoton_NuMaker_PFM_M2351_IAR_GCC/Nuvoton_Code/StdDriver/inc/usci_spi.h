...
*/
    ...
    #define USPI_MODE_0                                                        \
        ( 0x0UL << USPI_PROTCTL_SCLKMODE_Pos ) /*!< SCLK idle low; data        \
                                                  transmit with falling edge   \
                                                  and receive with rising edge \
                                                */
    #define USPI_MODE_1                                                     \
        ( 0x1UL << USPI_PROTCTL_SCLKMODE_Pos ) /*!< SCLK idle low; data     \
                                                  transmit with rising edge \
                                                  and receive with falling  \
                                                  edge */
    #define USPI_MODE_2                                                     \
        ( 0x2UL << USPI_PROTCTL_SCLKMODE_Pos ) /*!< SCLK idle high; data    \
                                                  transmit with rising edge \
                                                  and receive with falling  \
                                                  edge */
    #define USPI_MODE_3                                                        \
        ( 0x3UL << USPI_PROTCTL_SCLKMODE_Pos ) /*!< SCLK idle high; data       \
                                                  transmit with falling edge   \
                                                  and receive with rising edge \
                                                */
    ...
    #define USPI_SS_ACTIVE_LOW \
        ( USPI_LINECTL_CTLOINV_Msk ) /*!< SS active low */

    /* USCI_SPI Interrupt Mask */
    #define USPI_SSINACT_INT_MASK \
        ( 0x001UL ) /*!< Slave Slave Inactive interrupt mask */
    #define USPI_SSACT_INT_MASK \
        ( 0x002UL ) /*!< Slave Slave Active interrupt mask */
    #define USPI_SLVTO_INT_MASK \
        ( 0x004UL ) /*!< Slave Mode Time-out interrupt mask */
    #define USPI_SLVBE_INT_MASK \
        ( 0x008UL ) /*!< Slave Mode Bit Count Error interrupt mask */
    #define USPI_TXUDR_INT_MASK \
        ( 0x010UL ) /*!< Slave Transmit Under Run interrupt mask */
    #define USPI_RXOV_INT_MASK \
        ( 0x020UL ) /*!< Receive Buffer Overrun interrupt mask */
    #define USPI_TXST_INT_MASK                         \
        ( 0x040UL ) /*!< Transmit Start interrupt mask \
                     */
    #define USPI_TXEND_INT_MASK                      \
        ( 0x080UL ) /*!< Transmit End interrupt mask \
                     */
    #define USPI_RXST_INT_MASK                                                \
        ( 0x100UL )                         /*!< Receive Start interrupt mask \
                                             */
    ...
    #define USPI_SSLINE_STS_MASK                     \
        ( 0x20UL ) /*!< USCI_SPI_SS line status mask \
                    */
    ...
    */
    ...
    #define USPI_GET_RX_EMPTY_FLAG( uspi )                  \
        ( ( ( uspi )->BUFSTS & USPI_BUFSTS_RXEMPTY_Msk ) == \
    ...
    #define USPI_GET_TX_EMPTY_FLAG( uspi )                  \
        ( ( ( uspi )->BUFSTS & USPI_BUFSTS_TXEMPTY_Msk ) == \
    ...
    #define USPI_GET_TX_FULL_FLAG( uspi )                  \
        ( ( ( uspi )->BUFSTS & USPI_BUFSTS_TXFULL_Msk ) == \
     ...
     * @param[in]  uspi The pointer of the specified USCI_SPI module.
     * @return     None.
     * @details    Disable automatic slave selection function and set
     * USCI_SPI_SS pin to high state. Only available in Master mode.
     * \hideinitializer
     */
     ...
     * @return     None.
     * @details    Disable automatic slave selection function and set
     * USCI_SPI_SS pin to low state. Only available in Master mode.
     * \hideinitializer
     */
    ...
    #define USPI_IS_BUSY( uspi )                           \
        ( ( ( uspi )->PROTSTS & USPI_PROTSTS_BUSY_Msk ) == \
    ...
    #define USPI_GET_WAKEUP_FLAG( uspi )                                     \
        ( ( ( uspi )->WKSTS & USPI_WKSTS_WKF_Msk ) == USPI_WKSTS_WKF_Msk ? 1 \
                                                                         : 0 )
     ...
     * @param[in]  uspi The pointer of the specified USCI_SPI module.
     * @return     None.
     * @details    Set RXPDMAEN bit of USPI_PDMACTL register to enable RX PDMA
     * transfer function.
     */
     ...
     * @param[in]  uspi The pointer of the specified USCI_SPI module.
     * @return     None.
     * @details    Set TXPDMAEN bit of USPI_PDMACTL register to enable TX PDMA
     * transfer function.
     */
     ...
     * @param[in]  uspi The pointer of the specified USCI_SPI module.
     * @return     None.
     * @details    Clear RXPDMAEN bit of USPI_PDMACTL register to disable RX
     ...
     * @param[in]  uspi The pointer of the specified USCI_SPI module.
     * @return     None.
     * @details    Clear TXPDMAEN bit of USPI_PDMACTL register to disable TX
     * PDMA transfer function.
     */
