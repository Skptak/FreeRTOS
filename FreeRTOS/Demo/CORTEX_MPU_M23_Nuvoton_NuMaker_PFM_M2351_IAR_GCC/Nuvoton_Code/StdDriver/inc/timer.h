...
extern "C" {
    ...
    */
    /*---------------------------------------------------------------------------------------------------------*/
    /*  TIMER Operation Mode, External Counter and Capture Mode Constant
     * Definitions                           */
    /*---------------------------------------------------------------------------------------------------------*/
    #define TIMER_ONESHOT_MODE                                              \
        ( 0UL << TIMER_CTL_OPMODE_Pos ) /*!< Timer working in one-shot mode \
                                           \hideinitializer */
    #define TIMER_PERIODIC_MODE                                             \
        ( 1UL << TIMER_CTL_OPMODE_Pos ) /*!< Timer working in periodic mode \
                                           \hideinitializer */
    #define TIMER_TOGGLE_MODE                                               \
        ( 2UL << TIMER_CTL_OPMODE_Pos ) /*!< Timer working in toggle-output \
                                           mode \hideinitializer */
    #define TIMER_CONTINUOUS_MODE                                        \
        ( 3UL << TIMER_CTL_OPMODE_Pos ) /*!< Timer working in continuous \
                                           counting mode \hideinitializer */
    #define TIMER_TOUT_PIN_FROM_TMX                                        \
        ( 0UL << TIMER_CTL_TGLPINSEL_Pos ) /*!< Timer toggle-output pin is \
                                              from TMx pin \hideinitializer */
    #define TIMER_TOUT_PIN_FROM_TMX_EXT                                    \
        ( 1UL << TIMER_CTL_TGLPINSEL_Pos ) /*!< Timer toggle-output pin is \
                                              from TMx_EXT pin             \
                                              \hideinitializer */

    #define TIMER_COUNTER_EVENT_FALLING                                       \
        ( 0UL << TIMER_EXTCTL_CNTPHASE_Pos ) /*!< Counter increase on falling \
                                                edge detection                \
                                                \hideinitializer */
    #define TIMER_COUNTER_EVENT_RISING                                       \
        ( 1UL << TIMER_EXTCTL_CNTPHASE_Pos ) /*!< Counter increase on rising \
                                                edge detection               \
                                                \hideinitializer */
    #define TIMER_CAPTURE_FREE_COUNTING_MODE                                 \
        ( 0UL << TIMER_EXTCTL_CAPFUNCS_Pos ) /*!< Timer capture event to get \
                                                timer counter value          \
                                                \hideinitializer */
    #define TIMER_CAPTURE_COUNTER_RESET_MODE                                   \
        ( 1UL << TIMER_EXTCTL_CAPFUNCS_Pos ) /*!< Timer capture event to reset \
                                                timer counter \hideinitializer \
                                              */

    #define TIMER_CAPTURE_EVENT_FALLING                                    \
        ( 0UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< Falling edge detection to \
                                               trigger capture event       \
                                               \hideinitializer */
    #define TIMER_CAPTURE_EVENT_RISING                                    \
        ( 1UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< Rising edge detection to \
                                               trigger capture event      \
                                               \hideinitializer */
    #define TIMER_CAPTURE_EVENT_FALLING_RISING                                \
        ( 2UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< Both falling and rising edge \
                                               detection to trigger capture   \
                                               event, and first event at      \
                                               falling edge \hideinitializer  \
                                             */
    #define TIMER_CAPTURE_EVENT_RISING_FALLING                                \
        ( 3UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< Both rising and falling edge \
                                               detection to trigger capture   \
                                               event, and first event at      \
                                               rising edge \hideinitializer */
    #define TIMER_CAPTURE_EVENT_GET_LOW_PERIOD                                 \
        ( 6UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< First capture event is at     \
                                               falling edge, follows are at at \
                                               rising edge \hideinitializer */
    #define TIMER_CAPTURE_EVENT_GET_HIGH_PERIOD                               \
        ( 7UL << TIMER_EXTCTL_CAPEDGE_Pos ) /*!< First capture event is at    \
                                               rising edge, follows are at at \
                                               falling edge \hideinitializer  \
                                             */

    #define TIMER_TRGSRC_TIMEOUT_EVENT                                    \
        ( 0UL << TIMER_TRGCTL_TRGSSEL_Pos ) /*!< Select internal trigger  \
                                               source from timer time-out \
                                               event \hideinitializer */
    #define TIMER_TRGSRC_CAPTURE_EVENT                                         \
        ( 1UL << TIMER_TRGCTL_TRGSSEL_Pos ) /*!< Select internal trigger       \
                                               source from timer capture event \
                                               \hideinitializer */
    #define TIMER_TRG_TO_EPWM                                              \
        ( TIMER_TRGCTL_TRGEPWM_Msk ) /*!< Each timer event as EPWM counter \
                                        clock source \hideinitializer */
    #define TIMER_TRG_TO_EADC                                           \
        ( TIMER_TRGCTL_TRGEADC_Msk ) /*!< Each timer event to start ADC \
                                        conversion \hideinitializer */
    #define TIMER_TRG_TO_DAC                                           \
        ( TIMER_TRGCTL_TRGDAC_Msk ) /*!< Each timer event to start DAC \
                                       conversion \hideinitializer */
    #define TIMER_TRG_TO_PDMA                                              \
        ( TIMER_TRGCTL_TRGPDMA_Msk ) /*!< Each timer event to trigger PDMA \
                                        transfer \hideinitializer */
    ...
    */
     ...
     * @param[in]  u32Value    Timer compare value. Valid values are between 2
     * to 0xFFFFFF.
     *
     * @return     None
     *
     * @details    This macro is used to set timer compared value to adjust
     * timer time-out interval.
     * @note       1. Never write 0x0 or 0x1 in this field, or the core will run
     * into unknown state. \n
     *             2. If update timer compared value in continuous counting
     * mode, timer counter value will keep counting continuously. \n But if
     * timer is operating at other modes, the timer up counter will restart
     * counting and start from 0. \hideinitializer
     */
     ...
     * @param[in]  timer       The pointer of the specified Timer module. It
     * could be TIMER0, TIMER1, TIMER2, TIMER3.
     * @param[in]  u32Value    Timer prescale value. Valid values are between 0
     * to 0xFF.
     *
     * @return     None
     *
     * @details    This macro is used to set timer prescale value and timer
     * source clock will be divided by (prescale + 1) \n before it is fed into
     * timer. \hideinitializer
     */
    #define TIMER_SET_PRESCALE_VALUE( timer, u32Value )              \
        ( ( timer )->CTL = ( ( timer )->CTL & ~TIMER_CTL_PSC_Msk ) | \
                           ( u32Value ) )
     ...
     *
     * @details    This macro is used to check if specify Timer counter is
     * inactive or active. \hideinitializer
     */
    #define TIMER_IS_ACTIVE( timer ) \
        ( ( ( timer )->CTL & TIMER_CTL_ACTSTS_Msk ) ? 1 : 0 )
     ...
     * @return     None
     *
     * @details    This macro is used to select timer toggle-output pin is
     * output on TMx or TMx_EXT pin. \hideinitializer
     */
    #define TIMER_SELECT_TOUT_PIN( timer, u32ToutSel )                     \
        ( ( timer )->CTL = ( ( timer )->CTL & ~TIMER_CTL_TGLPINSEL_Msk ) | \
                           ( u32ToutSel ) )
 ...
 * @return     None
 *
 * @details    This function is used to enable the timer interrupt wake-up
 * function and interrupt source could be time-out interrupt, \n counter event
 * interrupt or capture trigger interrupt.
 * @note       To wake the system from Power-down mode, timer clock source must
 * be ether LXT or LIRC.
 */
__STATIC_INLINE void TIMER_EnableWakeup( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to disable the timer interrupt wake-up
 * function.
 */
__STATIC_INLINE void TIMER_DisableWakeup( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to enable the detect de-bounce function of
 * capture pin.
 */
__STATIC_INLINE void TIMER_EnableCaptureDebounce( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to disable the detect de-bounce function of
 * capture pin.
 */
__STATIC_INLINE void TIMER_DisableCaptureDebounce( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to enable the timer time-out interrupt
 * function.
 */
__STATIC_INLINE void TIMER_EnableInt( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to disable the timer time-out interrupt
 * function.
 */
__STATIC_INLINE void TIMER_DisableInt( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to enable the timer capture trigger
 * interrupt function.
 */
__STATIC_INLINE void TIMER_EnableCaptureInt( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to disable the timer capture trigger
 * interrupt function.
 */
__STATIC_INLINE void TIMER_DisableCaptureInt( TIMER_T * timer )
{
 ...
 *
 * @param[in]  timer   The pointer of the specified Timer module. It could be
 ...
 *
 * @details    This function indicates timer capture trigger interrupt occurred
 * or not.
 */
__STATIC_INLINE uint32_t TIMER_GetCaptureIntFlag( TIMER_T * timer )
{
 ...
 *
 * @details    This function indicates timer interrupt event has waked up system
 * or not.
 */
__STATIC_INLINE uint32_t TIMER_GetWakeupFlag( TIMER_T * timer )
{
 ...
 * @return     None
 *
 * @details    This function is used to reset current counter value and internal
