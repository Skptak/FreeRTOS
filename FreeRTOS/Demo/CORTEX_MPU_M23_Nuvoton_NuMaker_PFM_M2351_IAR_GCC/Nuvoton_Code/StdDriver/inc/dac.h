...
extern "C" {
    ...
    */
    ...
    #define DAC_CTL_LALIGN_RIGHT_ALIGN \
        ( 0UL << DAC_CTL_LALIGN_Pos ) /*!< Right alignment. */
    #define DAC_CTL_LALIGN_LEFT_ALIGN \
        ( 1UL << DAC_CTL_LALIGN_Pos ) /*!< Left alignment */

    #define DAC_WRITE_DAT_TRIGGER ( 0UL ) /*!< Write DAC_DAT trigger */
    #define DAC_SOFTWARE_TRIGGER \
        ( 0UL | DAC_CTL_TRGEN_Msk ) /*!< Software trigger */
    #define DAC_LOW_LEVEL_TRIGGER                                          \
        ( ( 0UL << DAC_CTL_ETRGSEL_Pos ) | ( 1UL << DAC_CTL_TRGSEL_Pos ) | \
          DAC_CTL_TRGEN_Msk ) /*!< STDAC pin low level trigger */
    #define DAC_HIGH_LEVEL_TRIGGER                                         \
        ( ( 1UL << DAC_CTL_ETRGSEL_Pos ) | ( 1UL << DAC_CTL_TRGSEL_Pos ) | \
          DAC_CTL_TRGEN_Msk ) /*!< STDAC pin high level trigger */
    #define DAC_FALLING_EDGE_TRIGGER                                       \
        ( ( 2UL << DAC_CTL_ETRGSEL_Pos ) | ( 1UL << DAC_CTL_TRGSEL_Pos ) | \
          DAC_CTL_TRGEN_Msk ) /*!< STDAC pin falling edge trigger */
    #define DAC_RISING_EDGE_TRIGGER                                        \
        ( ( 3UL << DAC_CTL_ETRGSEL_Pos ) | ( 1UL << DAC_CTL_TRGSEL_Pos ) | \
          DAC_CTL_TRGEN_Msk ) /*!< STDAC pin rising edge trigger */
    #define DAC_TIMER0_TRIGGER                                             \
        ( ( 2UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< Timer 0 \
                                                                 trigger */
    #define DAC_TIMER1_TRIGGER                                             \
        ( ( 3UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< Timer 1 \
                                                                 trigger */
    #define DAC_TIMER2_TRIGGER                                             \
        ( ( 4UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< Timer 2 \
                                                                 trigger */
    #define DAC_TIMER3_TRIGGER                                             \
        ( ( 5UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< Timer 3 \
                                                                 trigger */
    #define DAC_EPWM0_TRIGGER                                            \
        ( ( 6UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< EPWM0 \
                                                                 trigger */
    #define DAC_EPWM1_TRIGGER                                            \
        ( ( 7UL << DAC_CTL_TRGSEL_Pos ) | DAC_CTL_TRGEN_Msk ) /*!< EPWM1 \
                                                                 trigger */

    #define DAC_TRIGGER_MODE_DISABLE \
        ( 0UL << DAC_CTL_TRGEN_Pos ) /*!< Trigger mode disable */
    #define DAC_TRIGGER_MODE_ENABLE \
        ( 1UL << DAC_CTL_TRGEN_Pos ) /*!< Trigger mode enable */
    ...
    */
     ...
     * @return None
     * @details User writes SWTRG bit (DAC_SWTRG[0]) to generate one shot pulse
     * and it is cleared to 0 by hardware automatically.
     */
     ...
     * @param[in] dac The pointer of the specified DAC module.
     * @return None
     * @details User has to load data into DAC_DAT[15:4] bits. DAC_DAT[31:16]
     * and DAC_DAT[3:0] are ignored in DAC conversion.
     */
     ...
     * @param[in] dac The pointer of the specified DAC module.
     * @return None
     * @details User has to load data into DAC_DAT[11:0] bits, DAC_DAT[31:12]
     * are ignored in DAC conversion.
     */
    #define DAC_ENABLE_RIGHT_ALIGN( dac ) \
        ( ( dac )->CTL &= ~DAC_CTL_LALIGN_Msk )

    /**
     * @brief Enable output voltage buffer.
     * @param[in] dac The pointer of the specified DAC module.
     * @return None
     * @details The DAC integrates a voltage output buffer that can be used to
     * reduce output impedance and drive external loads directly without having
     * to add an external operational amplifier.
     */
    #define DAC_ENABLE_BYPASS_BUFFER( dac ) \
        ( ( dac )->CTL |= DAC_CTL_BYPASS_Msk )
     ...
     * occurs while DMAEN (DAC_CTL[2]) is set.
     */
     ...
     * @param[in] dac The pointer of the specified DAC module.
     * @param[in] u32Ch Not used in M2351 Series DAC.
     * @param[in] u32Data Decides the data for conversion, valid range are
     * between 0~0xFFF.
     * @return None
     * @details 12 bit left alignment: user has to load data into DAC_DAT[15:4]
     * bits. 12 bit right alignment: user has to load data into DAC_DAT[11:0]
     * bits.
     */
     ...
     * @retval 0 Idle state.
     * @retval 1 Busy state.
     * @details This macro is used to read BUSY bit (DAC_STATUS[8]) to get busy
     * state.
     */
    #define DAC_IS_BUSY( dac, u32Ch ) \
        ( ( ( dac )->STATUS & DAC_STATUS_BUSY_Msk ) >> DAC_STATUS_BUSY_Pos )
     ...
     * @details This macro is used to read FINISH bit (DAC_STATUS[0]) to get DAC
     * conversion complete finish flag.
     */
     ...
     * @details This macro is used to read DMAUDR bit (DAC_STATUS[1]) to get DMA
     * under-run state.
     */
    #define DAC_GET_DMAUDR_FLAG( dac ) \
        ( ( ( dac )->STATUS & DAC_STATUS_DMAUDR_Msk ) >> DAC_STATUS_DMAUDR_Pos )
     ...
     * @param[in] u32Ch Not used in M2351 Series DAC.
     * @return None
     * @details User writes FINISH bit (DAC_STATUS[0]) to clear DAC conversion
     * complete finish flag.
     */
     ...
     * @param[in] dac The pointer of the specified DAC module.
     * @return None
     * @details User writes DMAUDR bit (DAC_STATUS[1]) to clear DMA under-run
     * flag.
     */
    #define DAC_CLR_DMAUDR_FLAG( dac ) \
        ( ( dac )->STATUS = DAC_STATUS_DMAUDR_Msk )
