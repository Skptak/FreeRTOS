...
enum UI2C_SLAVE_EVENT
{
    ...
    #define UI2C_CTL_PTRG                                                  \
        ( 0x20U ) /*!< USCI_CTL setting for I2C control bits. It would set \
                     PTRG bit */
    #define UI2C_CTL_STA                                                       \
        ( 0x08U ) /*!< USCI_CTL setting for I2C control bits. It would set STA \
                     bit */
    #define UI2C_CTL_STO                                                       \
        ( 0x04U ) /*!< USCI_CTL setting for I2C control bits. It would set STO \
                     bit */
    #define UI2C_CTL_AA                                                       \
        ( 0x02U ) /*!< USCI_CTL setting for I2C control bits. It would set AA \
                     bit  */
    ...
    #define UI2C_DATA_TOGGLE_WK                                               \
        ( 0x0U << UI2C_WKCTL_WKADDREN_Pos ) /*!< Wakeup according data toggle \
                                             */
    #define UI2C_ADDR_MATCH_WK                                            \
        ( 0x1U << UI2C_WKCTL_WKADDREN_Pos ) /*!< Wakeup according address \
                                               match */
    ...
    #define UI2C_STAR_INT_MASK \
        ( 0x002U ) /*!< Start condition received interrupt mask */
    #define UI2C_STOR_INT_MASK \
        ( 0x004U ) /*!< Stop condition received interrupt mask */
    #define UI2C_NACK_INT_MASK                         \
        ( 0x008U ) /*!< Non-acknowledge interrupt mask \
                    */
    #define UI2C_ARBLO_INT_MASK \
        ( 0x010U )                       /*!< Arbitration lost interrupt mask */
    ...
    */
    ...
    /**
     *    @brief        This macro sets the USCI_I2C protocol control register
     * at one time
     *
     *    @param[in]    ui2c      The pointer of the specified USCI_I2C module.
     *    @param[in]    u8Ctrl    Set the register value of USCI_I2C control
     * register.
     *
     *    @return       None
     *
     *    @details      Set UI2C_PROTCTL register to control USCI_I2C bus
     * conditions of START, STOP, PTRG, ACK.
     */
    #define UI2C_SET_CONTROL_REG( ui2c, u8Ctrl ) \
        ( ( ui2c )->PROTCTL = ( ( ui2c )->PROTCTL & ~0x2EU ) | ( u8Ctrl ) )

    /**
     *    @brief        This macro only set START bit to protocol control
     ...
     *    @return       None
     *
     *    @details      Set the USCI_I2C bus START condition in UI2C_PROTCTL
     * register.
     */
    #define UI2C_START( ui2c )                                                 \
        ( ( ui2c )->PROTCTL = ( ( ui2c )->PROTCTL & ~UI2C_PROTCTL_PTRG_Msk ) | \
                              UI2C_PROTCTL_STA_Msk )

    /**
     *    @brief        This macro only set STOP bit to the control register of
     * USCI_I2C module
     *
     *    @param[in]    ui2c      The pointer of the specified USCI_I2C module.
     *
     *    @return       None
     *
     *    @details      Set the USCI_I2C bus STOP condition in UI2C_PROTCTL
     * register.
     */
    ...
    /**
     *    @brief        This macro returns the data stored in data register of
     * USCI_I2C module
     *
     *    @param[in]    ui2c      The pointer of the specified USCI_I2C module.
     *
     *    @return       Data
     *
     *    @details      Read a byte data value of UI2C_RXDAT register from
     * USCI_I2C bus
     */
    #define UI2C_GET_DATA( ui2c )         ( ( ui2c )->RXDAT )

    /**
     *    @brief        This macro writes the data to data register of USCI_I2C
     * module
     *
     *    @param[in]    ui2c     The pointer of the specified USCI_I2C module.
     *    @param[in]    u8Data   The data which will be written to data register
     * of USCI_I2C module.
     *
     *    @return       None
     *
     *    @details      Write a byte data value of UI2C_TXDAT register, then
     * sends address or data to USCI I2C bus
     */
     ...
     *
     *    @details      USCI_I2C bus occurs time-out event, the time-out flag
     * will be set. If not occurs time-out event, this bit is cleared.
     */
     ...
     *
     *    @details      USCI_I2C controller wake-up flag will be set when
     * USCI_I2C bus occurs wake-up from deep-sleep.
     */
    #define UI2C_GET_WAKEUP_FLAG( ui2c )                                     \
        ( ( ( ui2c )->WKSTS & UI2C_WKSTS_WKF_Msk ) == UI2C_WKSTS_WKF_Msk ? 1 \
                                                                         : 0 )
     ...
     *    @return       None
     *
     *    @details      If USCI_I2C wake-up flag is set, use this macro to clear
     * it.
     */
    #define UI2C_CLR_WAKEUP_FLAG( ui2c ) \
        ( ( ui2c )->WKSTS = UI2C_WKSTS_WKF_Msk )
     ...
     *    @return       None
     *
     *    @details      The UI2C_I2C is 7-bit address mode, when disable
     * USCI_I2C 10-bit address match function.
     */
    #define UI2C_DISABLE_10BIT_ADDR_MODE( ui2c ) \
        ( ( ui2c )->PROTCTL &= ~( UI2C_PROTCTL_ADDR10EN_Msk ) )
    ...
    /**
     *    @brief        This macro gets USCI_I2C protocol interrupt flag or bus
     * status
     *
     *    @param[in]    ui2c     The pointer of the specified USCI_I2C module.
     *
     ...
     *
     *    @details      Read a word data of USCI_I2C PROTSTS register to get
     * USCI_I2C bus Interrupt flags or status.
     */
     ...
     *    @return None
     *
     *    @details      To clear interrupt flag when USCI_I2C occurs interrupt
     * and set interrupt flag.
     */
    #define UI2C_CLR_PROT_INT_FLAG( ui2c, u32IntTypeFlag ) \
        ( ( ui2c )->PROTSTS = ( u32IntTypeFlag ) )
     ...
     *    @return None
     *
     *    @details      Set specified USCI_I2C protocol interrupt bits to enable
     * interrupt function.
     */
    #define UI2C_ENABLE_PROT_INT( ui2c, u32IntSel ) \
        ( ( ui2c )->PROTIEN |= ( u32IntSel ) )
     ...
     *    @return None
     *
     *    @details      Clear specified USCI_I2C protocol interrupt bits to
     * disable interrupt funtion.
     */
