...
extern "C" {
    ...
    */
    ...
    #define UUART_WORD_LEN_6                                                  \
        ( 6UL << UUART_LINECTL_DWIDTH_Pos ) /*!< UUART_LINECTL setting to set \
                                               UART word length to 6 bits */
    #define UUART_WORD_LEN_7                                                  \
        ( 7UL << UUART_LINECTL_DWIDTH_Pos ) /*!< UUART_LINECTL setting to set \
                                               UART word length to 7 bits */
    #define UUART_WORD_LEN_8                                                  \
        ( 8UL << UUART_LINECTL_DWIDTH_Pos ) /*!< UUART_LINECTL setting to set \
                                               UART word length to 8 bits */
    #define UUART_WORD_LEN_9                                                  \
        ( 9UL << UUART_LINECTL_DWIDTH_Pos ) /*!< UUART_LINECTL setting to set \
                                               UART word length to 9 bits */

    /*---------------------------------------------------------------------------------------------------------*/
    /* UUART_PROTCTL constants definitions */
    /*---------------------------------------------------------------------------------------------------------*/
    #define UUART_PARITY_NONE                                                 \
        ( 0x0UL << UUART_PROTCTL_PARITYEN_Pos ) /*!< UUART_PROTCTL setting to \
                                                   set UART as no parity */
    #define UUART_PARITY_ODD                                                  \
        ( 0x1UL << UUART_PROTCTL_PARITYEN_Pos ) /*!< UUART_PROTCTL setting to \
                                                   set UART as odd parity */
    #define UUART_PARITY_EVEN                                                 \
        ( 0x3UL << UUART_PROTCTL_PARITYEN_Pos ) /*!< UUART_PROTCTL setting to \
                                                   set UART as even parity */

    #define UUART_STOP_BIT_1 \
        ( 0x0UL ) /*!< UUART_PROTCTL setting for one stop bit */
    #define UUART_STOP_BIT_2 \
        ( 0x1UL ) /*!< UUART_PROTCTL setting for two stop bit */

    /*---------------------------------------------------------------------------------------------------------*/
    /* USCI UART interrupt mask definitions */
    /*---------------------------------------------------------------------------------------------------------*/
    #define UUART_ABR_INT_MASK                         \
        ( 0x002UL ) /*!< Auto-baud rate interrupt mask \
                     */
    #define UUART_RLS_INT_MASK \
        ( 0x004UL ) /*!< Receive line status interrupt mask */
    #define UUART_BUF_RXOV_INT_MASK \
        ( 0x008UL ) /*!< Buffer RX overrun interrupt mask */
    ...
    */
    ...
    #define UUART_GET_TX_EMPTY( uuart ) \
        ( ( uuart )->BUFSTS & UUART_BUFSTS_TXEMPTY_Msk )
    ...
    #define UUART_GET_RX_EMPTY( uuart ) \
        ( ( uuart )->BUFSTS & UUART_BUFSTS_RXEMPTY_Msk )
     ...
     * value. \n It indicates if specified usci_uart port transmission is over
     * nor not.
     */
     ...
     *    @return       None
     *
     *    @details      This macro wait specified usci_uart port transmission is
     * over.
     */
    #define UUART_WAIT_TX_EMPTY( uuart )                                  \
        while( !( ( ( ( uuart )->BUFSTS ) & UUART_BUFSTS_TXEMPTY_Msk ) >> \
                  UUART_BUFSTS_TXEMPTY_Pos ) )
    ...
    #define UUART_IS_TX_FULL( uuart )                        \
        ( ( ( uuart )->BUFSTS & UUART_BUFSTS_TXFULL_Msk ) >> \
    ...
    #define UUART_IS_RX_FULL( uuart )                        \
        ( ( ( uuart )->BUFSTS & UUART_BUFSTS_RXFULL_Msk ) >> \
     ...
     *                             - \ref UUART_PROTIEN_RLSIEN_Msk   : Rx Line
     * status interrupt
     *                             - \ref UUART_PROTIEN_ABRIEN_Msk   : Auto-baud
     * rate interrupt
     *
     *    @return       None
     *
     *    @details      This macro enable specified USCI_UART protocol
     * interrupt.
     */
    #define UUART_ENABLE_PROT_INT( uuart, u32IntSel ) \
        ( ( uuart )->PROTIEN |= ( u32IntSel ) )
     ...
     *    @return       None
     *
     *    @details      This macro disable specified USCI_UART protocol
     * interrupt.
     */
     ...
     *    @param[in]    uuart      The pointer of the specified USCI_UART module
     *    @param[in]    u32IntSel  Interrupt type select
     *                             - \ref UUART_BUFCTL_RXOVIEN_Msk     : Receive
     ...
     *    @param[in]    uuart      The pointer of the specified USCI_UART module
     *    @param[in]    u32IntSel  Interrupt type select
     *                             - \ref UUART_INTEN_RXENDIEN_Msk  : Receive
     * end interrupt
     *                             - \ref UUART_INTEN_RXSTIEN_Msk   : Receive
     * start interrupt
     *                             - \ref UUART_INTEN_TXENDIEN_Msk  : Transmit
     * end interrupt
     *                             - \ref UUART_INTEN_TXSTIEN_Msk   : Transmit
     * start interrupt
     *
     *    @return       None
     *
     *    @details      This macro enable specified USCI_UART transfer
     * interrupt.
     */
    #define UUART_ENABLE_TRANS_INT( uuart, u32IntSel ) \
        ( ( uuart )->INTEN |= ( u32IntSel ) )
     ...
     *    @return       None
     *
     *    @details      This macro disable specified USCI_UART transfer
     * interrupt.
     */
     ...
     *                                  - \ref UUART_PROTSTS_ABERRSTS_Msk    :
     * Auto-baud Rate Error Interrupt Indicator
     *                                  - \ref UUART_PROTSTS_ABRDETIF_Msk    :
     * Auto-baud Rate Detected Interrupt Flag
     *                                  - \ref UUART_PROTSTS_BREAK_Msk       :
     * Break Flag
     *                                  - \ref UUART_PROTSTS_FRMERR_Msk      :
     * Framing Error Flag
     *                                  - \ref UUART_PROTSTS_PARITYERR_Msk   :
     * Parity Error Flag
     *                                  - \ref UUART_PROTSTS_RXENDIF_Msk     :
     * Receive End Interrupt Flag
     *                                  - \ref UUART_PROTSTS_RXSTIF_Msk      :
     * Receive Start Interrupt Flag
     *                                  - \ref UUART_PROTSTS_TXENDIF_Msk     :
     * Transmit End Interrupt Flag
     *                                  - \ref UUART_PROTSTS_TXSTIF_Msk      :
     ...
     * module
     *    @param[in]    u32IntTypeFlag  Interrupt Type Flag, should be
     *                                  - \ref UUART_BUFSTS_RXOVIF_Msk : Receive
     * Buffer Over-run Error  Interrupt Indicator
     *
     *    @return       None
     *
    ...
    #define UUART_GET_WAKEUP_FLAG( uuart ) \
        ( ( uuart )->WKSTS & UUART_WKSTS_WKF_Msk ? 1 : 0 )
    ...
    #define UUART_CLR_WAKEUP_FLAG( uuart ) \
        ( ( uuart )->WKSTS = UUART_WKSTS_WKF_Msk )
