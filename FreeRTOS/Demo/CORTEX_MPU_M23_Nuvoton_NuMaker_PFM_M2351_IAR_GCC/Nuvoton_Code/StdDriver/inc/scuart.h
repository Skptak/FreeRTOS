...
extern "C" {
    ...
    */
    #define SCUART_CHAR_LEN_5                                                 \
        ( 0x3UL << SC_UARTCTL_WLS_Pos ) /*!< Set SCUART word length to 5 bits \
                                           \hideinitializer */
    #define SCUART_CHAR_LEN_6                                                 \
        ( 0x2UL << SC_UARTCTL_WLS_Pos ) /*!< Set SCUART word length to 6 bits \
                                           \hideinitializer */
    #define SCUART_CHAR_LEN_7                                                 \
        ( 0x1UL << SC_UARTCTL_WLS_Pos ) /*!< Set SCUART word length to 7 bits \
                                           \hideinitializer */
    #define SCUART_CHAR_LEN_8 \
        ( 0UL ) /*!< Set SCUART word length to 8 bits \hideinitializer */

    #define SCUART_PARITY_NONE                                           \
        ( SC_UARTCTL_PBOFF_Msk ) /*!< Set SCUART transfer with no parity \
                                    \hideinitializer */
    #define SCUART_PARITY_ODD                                           \
        ( SC_UARTCTL_OPE_Msk ) /*!< Set SCUART transfer with odd parity \
                                  \hideinitializer */
    #define SCUART_PARITY_EVEN \
        ( 0UL ) /*!< Set SCUART transfer with even parity \hideinitializer */

    #define SCUART_STOP_BIT_1                                         \
        ( SC_CTL_NSB_Msk ) /*!< Set SCUART transfer with one stop bit \
                              \hideinitializer */
    #define SCUART_STOP_BIT_2                                                \
        ( 0UL ) /*!< Set SCUART transfer with two stop bits \hideinitializer \
                 */
    ...
    */
     ...
     * @return     None
     *
     * @details    By writing data to DAT register, the SC will send out an
     * 8-bit data. \hideinitializer
     */
     ...
     *
     * @details    When the last byte of Tx buffer has been transferred to
     * Transmitter Shift Register, hardware sets TXEMPTY (SC_STATUS[9]) high. It
     * will be cleared when writing data into DAT (SC_DAT[7:0]).
     * \hideinitializer
     */
     ...
     *
     * @details    TXFULL (SC_STATUS[10]) is set when Tx buffer counts equals to
     * 4, otherwise is cleared by hardware. \hideinitializer
     */
     ...
     * @return     None
     *
     * @details    TXACT (SC_STATUS[31]) is cleared automatically when Tx
     * transfer is finished or the last byte transmission has completed.
     *
     ...
     *             This bit is set when Tx buffer counts equals to 4, otherwise
     * is cleared by hardware. \hideinitializer
     */
    #define SCUART_IS_TX_FULL( sc ) \
        ( ( ( sc )->STATUS & SC_STATUS_TXFULL_Msk ) ? 1 : 0 )
     ...
     *
     * @details    TXACT (SC_STATUS[31]) indicates Tx Transmit is complete or
     * not. \hideinitializer
     */
    #define SCUART_IS_TX_EMPTY( sc ) \
        ( ( ( sc )->STATUS & SC_STATUS_TXACT_Msk ) ? 0 : 1 )
     ...
     *
     * @details    TXEMPTY (SC_STATUS[9]) is set by hardware when the last byte
     * of Tx buffer has been transferred to Transmitter Shift Register.
     * \hideinitializer
     */
    #define SCUART_IS_TX_FIFO_EMPTY( sc ) \
        ( ( ( sc )->STATUS & SC_STATUS_TXEMPTY_Msk ) ? 1 : 0 )
     ...
     *
     * @details    TXACT (SC_STATUS[31]) is set by hardware when Tx transfer is
     * in active and the STOP bit of the last byte has been transmitted.
     * \hideinitializer
     */
    #define SCUART_IS_TX_ACTIVE( sc ) \
        ( ( ( sc )->STATUS & SC_STATUS_TXACT_Msk ) ? 1 : 0 )
     ...
     *
     * @details    By reading DAT register, the SC will return an 8-bit received
     ...
     *
     * @details    When the last byte of Rx buffer has been read by CPU,
     * hardware sets RXEMPTY (SC_STATUS[1]) high. It will be cleared when SC
     * receives any new data. \hideinitializer
     */
     ...
     *
     * @details    RXFULL (SC_STATUS[2]) is set when Rx buffer counts equals to
     * 4, otherwise it is cleared by hardware. \hideinitializer
     */
    #define SCUART_GET_RX_FULL( sc )  ( ( sc )->STATUS & SC_STATUS_RXFULL_Msk )

    /**
     * @brief      Check if receive data number in FIFO reach FIFO trigger level
     * or not
     *
     * @param[in]  sc      The pointer of smartcard module.
     *
     * @return     Receive FIFO data status
     * @retval     0       The number of bytes in receive FIFO is less than
     * trigger level
     * @retval     1       The number of bytes in receive FIFO equals or larger
     * than trigger level
     *
     * @details    RDAIF (SC_INTSTS[0]) is used for received data reaching
     * trigger level RXTRGLV (SC_CTL[7:6]) interrupt status flag.
     *
     * @note       If receive trigger level is \b not 1 byte, this macro return
     * 0 does not necessary indicates there is no data in FIFO. \hideinitializer
     */
    #define SCUART_IS_RX_READY( sc ) \
        ( ( ( sc )->INTSTS & SC_INTSTS_RDAIF_Msk ) ? 1 : 0 )
     ...
     *
     * @details    RXFULLF( SC_STATUS[2]) is set when Rx buffer counts equals to
     * 4, otherwise it is cleared by hardware. \hideinitializer
     */
    #define SCUART_IS_RX_FULL( sc ) \
        ( ( ( sc )->STATUS & SC_STATUS_RXFULL_Msk ) ? 1 : 0 )
     ...
     *
     * @param[in]  sc      The pointer of smartcard module.
     * @param[in]  u32Mask Interrupt masks to enable, a combination of following
     * bits,
     ...
     * @return     None
     *
     * @details    The macro is used to enable receiver buffer time-out
     * interrupt, transfer error interrupt, transmit buffer empty interrupt or
     * receive data reach trigger level interrupt. \hideinitializer
     */
     ...
     *
     * @param[in]  sc      The pointer of smartcard module.
     * @param[in]  u32Mask Interrupt masks to disable, a combination of
     * following bits,
     *                         - \ref SC_INTEN_RXTOIEN_Msk
     *                         - \ref SC_INTEN_TERRIEN_Msk
     *                         - \ref SC_INTEN_TBEIEN_Msk
     ...
     * @return     None
     *
     * @details    The macro is used to disable receiver buffer time-out
     * interrupt, transfer error interrupt, transmit buffer empty interrupt or
     * receive data reach trigger level interrupt. \hideinitializer
     */
     ...
     *
     * @param[in]  sc      The pointer of smartcard module.
     * @param[in] u32Type  Interrupt flag/status to check, could be one of
     * following value
     ...
     *
     * @details    The macro is used to get receiver buffer time-out interrupt
     * status, transfer error interrupt status, transmit buffer empty interrupt
     * status or receive data reach interrupt status. \hideinitializer
     */
    #define SCUART_GET_INT_FLAG( sc, u32Type ) \
        ( ( ( sc )->INTSTS & ( u32Type ) ) ? 1 : 0 )

    /**
     * @brief      Clear specified interrupt flag/status
     *
     * @param[in]  sc      The pointer of smartcard module.
     * @param[in] u32Type  Interrupt flag/status to clear, only \ref
     * SC_INTSTS_TERRIF_Msk valid for this macro.
     *
     * @return     None
     *
     ...
     *
     * @details    The macro is used to get receiver parity error status, frame
     * error status or break error status. \hideinitializer
     */
     ...
     *
     * @param[in]  sc      The pointer of smartcard module.
     * @param[in]  u32Mask Receive error flag/status to clear, combination
     * following values
     ...
     * @return     None
     *
     * @details    The macro is used to clear receiver parity error flag, frame
     * error flag or break error flag. \hideinitializer
     */
