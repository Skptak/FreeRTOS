...
extern "C" {
    ...
    */
    ...
    /*--------------------------------------------------------------------------------------------------*/
    #define BL_ERR_TT_CHECK \
        0xF0F00000UL /*!< Not a Non-secure parameter         */
    #define BL_ERR_PARAMETER \
        0xF0F00001UL /*!< Invalid parameter                  */
    #define BL_PARAM_ALIGN \
        0xF0F00002UL /*!< Parameter alignment error          */
    #define BL_NOT_FLASH_ADDR \
        0xF0F00003UL /*!< Invalid flash address              */
    #define BL_NOT_SRAM_ADDR \
        0xF0F00004UL /*!< Invalid sram address               */
    #define BL_XOM_NOT_CONFIG \
        0xF0F00005UL /*!< XOM is not configure yet           */
    #define BL_XOM_HAS_CONFIG \
        0xF0F00006UL /*!< XOM has beeen configured           */
    #define BL_XOM_HAS_ACTIVE \
        0xF0F00007UL /*!< XOM is actived                     */
    #define BL_XOM_BASE_ERROR \
        0xF0F00008UL /*!< Invalid XOM base address           */
    #define BL_KPROM_NOT_ENABLE \
        0xF0F00009UL /*!< KPROM is not enabled yet           */
    #define BL_KPROM_KEY_FORBID \
        0xF0F0000AUL /*!< KPROM comparison is forbidden      */
    #define BL_KPROM_KEY_UNMATCH \
        0xF0F0000BUL /*!< KPROM comparison is unmatched      */
    #define BL_KPROM_KEY_LOCKED \
        0xF0F0000CUL /*!< KPROM write-protect is enabled     */
    #define BL_KPROM_SET_FAIL \
        0xF0F0000EUL               /*!< Set KPROM key fail                 */
    #define BL_ISP_CMD_FAIL ( -1 ) /*!< FMC command fail                   */
    #define BL_FLASH_ALLONE \
        0xA11FFFFFUL /*!< Check-all-one result is all one    */
    #define BL_FLASH_NOT_ALLONE \
        0xA1100000UL /*!< Check-all-one result is not all one*/
    ...
    #define BL_RNG_LIRC32K                                                 \
        ( 0UL )                 /*!<Use LIRC32 for random number generator \
                                 */
...
/*---------------------------------------------------------------------------------------------------*/
/*  Define a global constant XCRPT as the access address of g_xcrpta for using
 * MKROM Crypto library. */
/*---------------------------------------------------------------------------------------------------*/
extern XCRPT_T g_xcrpt;
    #define XCRPT ( &g_xcrpt )

/**
 * @details    Command packet structure for transmit/receive data in SecureISP
 * function
 */
typedef struct
{
...
typedef struct
{
    ...
    ECC_PUBKEY_T ClientPubKey; /* Client's ECC public key, 64-bytes (256-bits +
                                  256-bits) */
    ECC_PUBKEY_T ServerPubKey; /* Server's ECC public key, 64-bytes (256-bits +
                                  256-bits) */
    ...
    uint8_t rcvbuf[ MAX_PKT_SIZE ]; /* Internal use in SecureISP */
    __attribute__( ( aligned( 4 ) ) )
    uint8_t rspbuf[ MAX_PKT_SIZE ]; /* Internal use in SecureISP */
 ...
 * @details    This API will unlock register write-protect, enable relative
 * settings for access FMC ISP commands and return maximum APROM by chip
 * package.
 */
 ...
 * @param      None
 * @return     None
 * @details    This API will disable relative settings for disable FMC ISP
 * function and lock register write-protect until last ISP operation is
 * finished.
 */
 ...
 * @details    This API will protect all flash region read/write operate by ICE
 * interface.
 */
...
/**
 * @brief      Read Multi-Word Data from Non-secure Flash Address (for
 * Non-secure region)
 * @param[in]  u32NSAddr       Starting Non-secure flash address
 * @param[out] pu32NSRamBuf    Non-secure sram address to store reading data
 * @param[in]  u32Size         Total read byte counts, it should be word aligned
 * and maximum size is one page size.
 * @retval     0xF0F00000      u32NSAddr or pu32NSRamBuf region isn't in
 * Non-secure area
 * @retval     0xF0F00001      Wrong u32NSAddr, pu32NSRamBuf or u32Size
 * parameter
 * @retval     0xF0F00003      u32NSAddr isn't valid flash address
 ...
 * @details    To read multi-words data start from specified Non-secure flash
 * address. And maximum read size is one page size, 2048 bytes.
 */
...
/**
 * @brief      Program Data into Non-secure Flash Address (for Non-secure
 * region)
 * @param[in]  u32NSAddr   Non-secure flash address
...
/**
 * @brief      Program Multi-Word Data into Non-secure Flash Address (for
 * Non-secure region)
 * @param[in]  u32NSAddr       Starting Non-secure flash address
 * @param[in]  pu32NSRamBuf    Non-secure sram buffer address to store program
 * data
 * @param[in]  u32Size         Total program byte counts, it should be word
 * aligned and maximum size is one page size.
 * @retval     0xF0F00000      u32NSAddr or pu32NSRamBuf region isn't in
 * Non-secure area
 * @retval     0xF0F00001      Wrong u32NSAddr, pu32NSRamBuf or u32Size
 * parameter
 ...
 * @details    To program multi-words data start from specified Non-secure flash
 * address. And maximum program size is one page size, 2048 bytes.
 */
 ...
 * @param[in]  u32NSAddr   Non-secure flash region to be erased and must be a
 * page size aligned address.
 ...
 * @details    This API is used to perform page erase command on specified
 * Non-secure flash address. And this address must be a page size aligned
 * address.
 */
 ...
 * @param[in]  u32NSAddr       Non-secure flash region to be calculated.
 * u32NSAddr must be a page size aligned address.
 * @param[in]  u32ByteCount    Byte counts of Non-secure flash area to be
 * calculated. It must be multiple of 2048 bytes.
 * @retval     0xF0F00000      u32NSAddr region isn't in Non-secure area
 ...
 * @details    This API will calculate the CRC32 checksum result of specified
 * non-secure flash area. The starting address and calculated size must be all
 * 2048 bytes page size aligned.
 */
uint32_t BL_FlashChecksum( uint32_t u32NSAddr, uint32_t u32ByteCount );

/**
 * @brief      Check Non-secure Flash Area Data are all ONE or not (for
 * Non-secure region)
 * @param[in]  u32NSAddr       Non-secure flash region to be calculated.
 * u32NSAddr must be a page size aligned address.
 * @param[in]  u32ByteCount    Byte counts of Non-secure flash area to be
 ...
 * @retval     0xA11FFFFF      The contents of verified Non-secure flash area
 * are 0xFFFFFFFF
 * @retval     0xA1100000      Some contents of verified Non-secure flash area
 * are not 0xFFFFFFFF
 * @details    This API is used to check specified Non-secure flash area are all
 * 0xFFFFFFFF or not.
 */
...
/**
 * @brief      Read UCID
 * @param[in]  u32Index    Index of the UCID to read and u32Index must be 0, 1,
 * 2, or 3.
...
/**
 * @brief      Read UID
 * @param[in]  u32Index    UID index. 0 = UID[31:0], 1 = UID[63:32], 2 =
 * UID[95:64]
 ...
 * @retval     0xF0F00000      pu32Base, pu32PageCnt or XOM base address isn't
 * in Non-secure area
 ...
 * @param[in]  u32IsDebugMode  1: Enable XOM debug mode; others will disable XOM
 * debug mode.
 ...
 * @retval     0xF0F00000  u32XOMBase or erase XOM region isn't in Non-secure
 * area
 ...
 * @details    This API will erase specified XOM region data and relative XOM
 * setting.
 */
 ...
 * @details    With this API, user can unlock KPROM write-protection and then
 * execute FMC program command well.
 */
 ...
 * @details    This API is used to check if the private key is placed in valid
 * range of curve.
 */
 ...
 * @details    This API is used to generate a public key pair by a specified ECC
 * private key and ECC curve.
 */
 ...
 * @details    This API is used to perform the ECDSA digital signature
 * verification.
 */
 ...
 * @retval     0           Success
 * @details    This API will return ECC R, S digital signature of chip ID,
 * include PDID, UID0~2 and UCID0~3.
 */
 ...
 *                 - \ref CRYPTO_DMA_LAST      Last TDES encrypt/decrypt of a
 * series of XTDES_Start.
 * @return     None
 ...
 * @param[in]  xcrpt           The pointer of the global XCRPT data
 * @param[in]  u32Channel      TDES channel. Must be 0~3.
 * @param[in]  au32Keys        The TDES keys. au32Keys[0][0] is Key0 high word
 ...
 *                     - \ref CRYPTO_DMA_CONTINUE  Continuous AES
 * encrypt/decrypt.
 *                     - \ref CRYPTO_DMA_LAST      Last AES encrypt/decrypt of a
 * series of XAES_Start.
 * @return     None
 ...
 *             After initial this API success, user can call XTRNG_Random API to
 ...
 *
 * @details    This API is used to initial random number generator.
 *             After initial this API success, user can perform BL_Random API to
 * generate the random number.
 */
 ...
 * @details    Executing this API will initialize USB or UART1 SecureISP
 * function. User can use SecureISP Tool to communicate with target chip.
 * @note       Configure relate USB and UART1 settings are necessary before
 * executing this API.
 */
 ...
 * @details    This API is used to process USBD command and should be called in
 * USBD_IRQHandler().
 */
 ...
 * @details    This API is used to process UART1 command and should be called in
 * UART1_IRQHandler().
 */
 ...
 * @param[in]  pInfo       The ISP information data buffer address
 * @param[out] pu32Data    Data buffer to store vendor data. Maximum buffer size
 * is 44 bytes.
 * @param[in]  pu32Buf     Internal used data buffer address
 * @retval     0           Success
 * @retval     -1          Invalid command packet
 * @retval     -2          Not in vendor function
 * @details    This API is used to get the vendor data and should be called in
 * the vendor function.
 */
 ...
 * @retval     -2          Not in vendor function
 * @details    This API is used to return vendor data to server and should be
 * called in the vendor function.
 */
 ...
 * @retval     0           Success
 * @retval     Other       Command fail
 * @details    This API is used for parse all commands except CONNECT and
 * related ECDH commands.
 */
 ...
 * @retval     -1      Execute API in Non-secure code
 * @retval     0       Success
 * @details    Enable WAKEUP, FLDET, USB and BUS interrupts. Disable
 * software-disconnect function after 100ms delay with SysTick timer.
 */
