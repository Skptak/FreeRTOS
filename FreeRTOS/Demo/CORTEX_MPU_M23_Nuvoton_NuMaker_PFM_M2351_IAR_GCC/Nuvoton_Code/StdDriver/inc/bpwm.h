...
extern "C" {
    ...
    */
    ...
    #define BPWM_CH_0_MASK \
        ( 0x1UL ) /*!< BPWM channel 0 mask \hideinitializer */
    #define BPWM_CH_1_MASK \
        ( 0x2UL ) /*!< BPWM channel 1 mask \hideinitializer */
    #define BPWM_CH_2_MASK \
        ( 0x4UL ) /*!< BPWM channel 2 mask \hideinitializer */
    #define BPWM_CH_3_MASK \
        ( 0x8UL ) /*!< BPWM channel 3 mask \hideinitializer */
    #define BPWM_CH_4_MASK \
        ( 0x10UL ) /*!< BPWM channel 4 mask \hideinitializer */
    #define BPWM_CH_5_MASK \
        ( 0x20UL ) /*!< BPWM channel 5 mask \hideinitializer */
    ...
    #define BPWM_EDGE_ALIGNED \
        ( 1UL ) /*!< BPWM working in edge aligned type(down count) */
    #define BPWM_CENTER_ALIGNED \
        ( 2UL ) /*!< BPWM working in center aligned type */
    ...
    /*---------------------------------------------------------------------------------------------------------*/
    #define BPWM_SSCTL_SSRC_PWM0                                           \
        ( 0UL << BPWM_SSCTL_SSRC_Pos ) /*!< Synchronous start source comes \
                                          from PWM0  */
    #define BPWM_SSCTL_SSRC_PWM1                                           \
        ( 1UL << BPWM_SSCTL_SSRC_Pos ) /*!< Synchronous start source comes \
                                          from PWM1  */
    #define BPWM_SSCTL_SSRC_BPWM0                                          \
        ( 2UL << BPWM_SSCTL_SSRC_Pos ) /*!< Synchronous start source comes \
                                          from BPWM0 */
    #define BPWM_SSCTL_SSRC_BPWM1                                          \
        ( 3UL << BPWM_SSCTL_SSRC_Pos ) /*!< Synchronous start source comes \
                                          from BPWM1 */
    ...
    #define BPWM_TRIGGER_ADC_EVEN_ZERO_POINT                                \
        ( 0UL ) /*!< BPWM trigger ADC while counter of even channel matches \
                   zero point \hideinitializer */
    #define BPWM_TRIGGER_ADC_EVEN_PERIOD_POINT                              \
        ( 1UL ) /*!< BPWM trigger ADC while counter of even channel matches \
                   period point \hideinitializer */
    #define BPWM_TRIGGER_ADC_EVEN_ZERO_OR_PERIOD_POINT                      \
        ( 2UL ) /*!< BPWM trigger ADC while counter of even channel matches \
                   zero or period point \hideinitializer */
    #define BPWM_TRIGGER_ADC_EVEN_CMP_UP_COUNT_POINT                           \
        ( 3UL ) /*!< BPWM trigger ADC while counter of even channel matches up \
                   count to comparator point \hideinitializer */
    #define BPWM_TRIGGER_ADC_EVEN_CMP_DOWN_COUNT_POINT                      \
        ( 4UL ) /*!< BPWM trigger ADC while counter of even channel matches \
                   down count to comparator point \hideinitializer */
    #define BPWM_TRIGGER_ADC_ODD_CMP_UP_COUNT_POINT                           \
        ( 8UL ) /*!< BPWM trigger ADC while counter of odd channel matches up \
                   count to comparator point \hideinitializer */
    #define BPWM_TRIGGER_ADC_ODD_CMP_DOWN_COUNT_POINT                      \
        ( 9UL ) /*!< BPWM trigger ADC while counter of odd channel matches \
                   down count to comparator point \hideinitializer */
    ...
    #define BPWM_CAPTURE_INT_RISING_LATCH                                    \
        ( 1UL ) /*!< BPWM capture interrupt if channel has rising transition \
                 */
    #define BPWM_CAPTURE_INT_FALLING_LATCH                             \
        ( 0x100UL ) /*!< BPWM capture interrupt if channel has falling \
                       transition */
    ...
    #define BPWM_DUTY_INT_DOWN_COUNT_MATCH_CMP                                 \
        ( 1 << BPWM_INTEN_CMPDIEN0_Pos ) /*!< BPWM duty interrupt triggered if \
                                            down count match comparator        \
                                            \hideinitializer */
    #define BPWM_DUTY_INT_UP_COUNT_MATCH_CMP                                   \
        ( 1 << BPWM_INTEN_CMPUIEN0_Pos ) /*!< BPWM duty interrupt triggered if \
                                            up down match comparator           \
                                            \hideinitializer */
    ...
    #define BPWM_LOAD_MODE_IMMEDIATE                                    \
        ( 1 << BPWM_CTL0_IMMLDEN0_Pos ) /*!< BPWM immediately load mode \
                                           \hideinitializer */
    #define BPWM_LOAD_MODE_CENTER                                \
        ( 1 << BPWM_CTL0_CTRLD0_Pos ) /*!< BPWM center load mode \
                                         \hideinitializer */
    ...
    #define BPWM_CLKSRC_BPWM_CLK                                         \
        ( 0UL ) /*!< BPWM Clock source selects to BPWM0_CLK or BPWM1_CLK \
                   \hideinitializer */
    #define BPWM_CLKSRC_TIMER0                                    \
        ( 1UL ) /*!< BPWM Clock source selects to TIMER0 overflow \
                   \hideinitializer */
    #define BPWM_CLKSRC_TIMER1                                    \
        ( 2UL ) /*!< BPWM Clock source selects to TIMER1 overflow \
                   \hideinitializer */
    #define BPWM_CLKSRC_TIMER2                                    \
        ( 3UL ) /*!< BPWM Clock source selects to TIMER2 overflow \
                   \hideinitializer */
    #define BPWM_CLKSRC_TIMER3                                    \
        ( 4UL ) /*!< BPWM Clock source selects to TIMER3 overflow \
                   \hideinitializer */
    ...
    */
     ...
     * @param[in] u32ChannelMask Combination of enabled channels. This parameter
     * is not used.
     ...
     * function of specified channel(s).
     * @note All channels share channel 0's setting.
     * \hideinitializer
     */
    #define BPWM_ENABLE_TIMER_SYNC( bpwm, u32ChannelMask, u32SyncSrc )   \
        ( ( bpwm )->SSCTL = ( ( bpwm )->SSCTL & ~BPWM_SSCTL_SSRC_Msk ) | \
                            ( u32SyncSrc ) | BPWM_SSCTL_SSEN0_Msk )
    ...
    #define BPWM_DISABLE_TIMER_SYNC( bpwm, u32ChannelMask ) \
        ( ( bpwm )->SSCTL &= ~BPWM_SSCTL_SSEN0_Msk )

    /**
     * @brief This macro enable BPWM counter synchronous start counting
     * function.
     * @param[in] bpwm The pointer of the specified BPWM module
     * @return None
     * @details This macro is used to make selected BPWM0 and BPWM1 channel(s)
     * start counting at the same time. To configure synchronous start counting
     * channel(s) by BPWM_ENABLE_TIMER_SYNC() and BPWM_DISABLE_TIMER_SYNC().
     * \hideinitializer
     */
    #define BPWM_TRIGGER_SYNC_START( bpwm ) \
        ( ( bpwm )->SSTRG = BPWM_SSTRG_CNTSEN_Msk )
    ...
    #define BPWM_ENABLE_OUTPUT_INVERTER( bpwm, u32ChannelMask ) \
        ( ( bpwm )->POLCTL = ( u32ChannelMask ) )

    /**
     * @brief This macro get captured rising data
     * @param[in] bpwm The pointer of the specified BPWM module
     * @param[in] u32ChannelNum BPWM channel number. Valid values are between
     * 0~5
     * @return None
     * \hideinitializer
     */
    #define BPWM_GET_CAPTURE_RISING_DATA( bpwm, u32ChannelNum ) \
        ( ( bpwm )->CAPDAT[ ( u32ChannelNum ) ].RCAPDAT )
    ...
    #define BPWM_GET_CAPTURE_FALLING_DATA( bpwm, u32ChannelNum ) \
        ( ( bpwm )->CAPDAT[ ( u32ChannelNum ) ].FCAPDAT )
     ...
     * @param[in] u32ChannelNum BPWM channel number. Valid values are between
     * 0~5
     * @param[in] u32CMR Duty of specified channel. Valid values are between
     * 0~0xFFFF
     * @return None
    ...
    #define BPWM_SET_CMR( bpwm, u32ChannelNum, u32CMR ) \
        ( ( bpwm )->CMPDAT[ ( u32ChannelNum ) ] = ( u32CMR ) )
    ...
    #define BPWM_CLR_COUNTER( bpwm, u32ChannelMask ) \
        ( ( bpwm )->CNTCLR = ( BPWM_CNTCLR_CNTCLR0_Msk ) )
