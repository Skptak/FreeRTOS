 ...
 * @param[in] u32DataFormat Data format. This is also used to select I2S or
 * PCM(TDM) function. Valid values are:
 *                                     - \ref I2S_FORMAT_I2S
 ...
 *          The sample rate may not be used from the parameter, it depends on
 * system's clock settings, but real sample rate used by system will be returned
 * for reference.
                   ...
                   uint32_t u32DataFormat )
{
    ...
    i2s->CTL0 = u32MasterSlave | u32WordWidth | u32MonoData | u32DataFormat |
                I2S_FIFO_TX_LEVEL_WORD_8 | I2S_FIFO_RX_LEVEL_WORD_8;

    /* Get I2S source clock frequency */
    u32SrcClk = I2S_GetSourceClockFreq( i2s );

    /* Calculate bit clock rate */
    u32BitRate = u32SampleRate * ( ( ( u32WordWidth >> 4UL ) & 0x3UL ) + 1UL ) *
                 16UL;
    u16Divider = ( uint16_t ) ( ( ( ( ( ( u32SrcClk * 10UL ) / u32BitRate ) >>
                                      1UL ) +
                                    5UL ) /
                                  10UL ) -
                                1UL ); /* Round to the nearest integer */
    i2s->CLKDIV = ( i2s->CLKDIV & ~I2S_CLKDIV_BCLKDIV_Msk ) |
                  ( ( uint32_t ) u16Divider << 8UL );

    /* Calculate real sample rate */
    u32BitRate = u32SrcClk / ( ( ( uint32_t ) u16Divider + 1UL ) * 2UL );
    u32SampleRate = u32BitRate /
                    ( ( ( ( u32WordWidth >> 4UL ) & 0x3UL ) + 1UL ) * 16UL );
 ...
 *            Each bit corresponds to a interrupt bit.
 * @return None
 * @details This function disables the interrupt according to the mask
 * parameter.
 */
...
/**
 * @brief  Configure PCM(TDM) function parameters, such as channel width,
 * channel number and sync pulse width
 * @param[in]  i2s The pointer of the specified I2S module.
...
{
    i2s->CTL0 = ( i2s->CTL0 & ~( I2S_CTL0_TDMCHNUM_Msk | I2S_CTL0_CHWIDTH_Msk |
