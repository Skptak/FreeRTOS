 ...
 * @details This function is used to Configure BPWM capture and get the nearest
 * unit time.
 */
                                    ...
                                    uint32_t u32CaptureEdge )
{
    ...
    if( ( ( ( uint32_t ) bpwm ) == BPWM0_BASE ) ||
        ( ( ( uint32_t ) bpwm ) == BPWM0_BASE + NS_OFFSET ) )
    {
...
/**
 * @brief This function Configure BPWM generator and get the nearest frequency
 ...
 * @note Since all channels shares a prescaler. Call this API to configure BPWM
 * frequency may affect existing frequency of other channel.
                                   ...
                                   uint32_t u32DutyCycle )
{
    ...
    ( bpwm )->WGCTL0 = ( ( bpwm )->WGCTL0 &
                         ~( ( BPWM_WGCTL0_PRDPCTL0_Msk | BPWM_WGCTL0_ZPCTL0_Msk )
                            << ( u32ChannelNum << 1 ) ) ) |
                       ( BPWM_OUTPUT_HIGH
                         << ( u32ChannelNum << 1UL
    ...
    ( bpwm )
        ->WGCTL1 = ( ( bpwm )->WGCTL1 &
                     ~( ( BPWM_WGCTL1_CMPDCTL0_Msk | BPWM_WGCTL1_CMPUCTL0_Msk )
                        << ( u32ChannelNum << 1 ) ) ) |
                   ( BPWM_OUTPUT_LOW
                     << ( u32ChannelNum << 1UL << BPWM_WGCTL1_CMPUCTL0_Pos ) );
 ...
 *                - BPWM0 : BPWM Group 0
 *                - BPWM1 : BPWM Group 1
 * @param[in] u32ChannelMask Combination of enabled channels. This parameter is
 * not used.
 * @return None
 ...
 * not used.
 * @return None
 * @details This function is used to stop BPWM generation immediately by clear
 * channel enable bit.
 * @note All channels share one counter.
 */
                            ...
                            uint32_t u32Condition )
{
    ...
    {
        ( bpwm )->EADCTS0 &= ~( ( BPWM_EADCTS0_TRGSEL0_Msk )
                                << ( u32ChannelNum << 3 ) );
        ( bpwm )->EADCTS0 |= ( ( BPWM_EADCTS0_TRGEN0_Msk | u32Condition )
                               << ( u32ChannelNum << 3 ) );
    }
    else
    {
        ( bpwm )->EADCTS1 &= ~( ( BPWM_EADCTS1_TRGSEL4_Msk )
                                << ( ( u32ChannelNum - 4UL ) << 3 ) );
        ( bpwm )->EADCTS1 |= ( ( BPWM_EADCTS1_TRGEN4_Msk | u32Condition )
                               << ( ( u32ChannelNum - 4UL ) << 3 ) );
    }
}
...
void BPWM_DisableADCTrigger( BPWM_T * bpwm, uint32_t u32ChannelNum )
{
    if( u32ChannelNum < 4UL )
    {
        ( bpwm )->EADCTS0 &= ~( BPWM_EADCTS0_TRGEN0_Msk
                                << ( u32ChannelNum << 3 ) );
    }
    else
    {
        ( bpwm )->EADCTS1 &= ~( BPWM_EADCTS1_TRGEN4_Msk
                                << ( ( u32ChannelNum - 4UL ) << 3 ) );
    }
}
 ...
 * set
 * @details This function is used to get BPWM trigger ADC to start of conversion
 ...
 * @return None
 * @details This function is used to enables BPWM output generation of selected
 * channel(s)
 */
 ...
 * @return None
 * @details This function is used to disables BPWM output generation of selected
 * channel(s)
 */
...
uint32_t BPWM_GetCaptureIntFlag( BPWM_T * bpwm, uint32_t u32ChannelNum )
{
    ...
    u32CapIf = ( ( ( ( bpwm )->CAPIF & ( BYTE1_Msk << u32ChannelNum ) ) ? 1UL
                                                                        : 0UL )
                 << 1 );
    u32CapIf |= ( ( ( bpwm )->CAPIF & ( BYTE0_Msk << u32ChannelNum ) ) ? 1UL
                                                                       : 0UL );
    return u32CapIf;
}
/**
...
{
    ( bpwm )->INTEN &= ~(
        ( BPWM_DUTY_INT_DOWN_COUNT_MATCH_CMP | BPWM_DUTY_INT_UP_COUNT_MATCH_CMP )
        << u32ChannelNum );
}
...
{
    return ( ( ( ( bpwm )->INTSTS &
                 ( ( BYTE2_Msk | BYTE3_Msk ) << u32ChannelNum ) ) )
                 ? 1UL
                 : 0UL );
}
