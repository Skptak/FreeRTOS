...
/* global variables */
/* For response R3 (such as ACMD41, CRC-7 is invalid; but SD controller will
 * still */
/* calculate CRC-7 and get an error result, software should ignore this error
 * and clear SDISR [CRC_IF] flag */
...
uint32_t SDH_SDCommand( SDH_T * sdh, uint32_t u32Cmd, uint32_t u32Arg )
{
    ...
    sdh->CMDARG = u32Arg;
    sdh->CTL = ( sdh->CTL & ( ~SDH_CTL_CMDCODE_Msk ) ) | ( u32Cmd << 8 ) |
               ( SDH_CTL_COEN_Msk );
                          ...
                          uint32_t u32TickCount )
{
    ...
    sdh->CMDARG = u32Arg;
    sdh->CTL = ( sdh->CTL & ( ~SDH_CTL_CMDCODE_Msk ) ) | ( u32Cmd << 8 ) |
               ( SDH_CTL_COEN_Msk | SDH_CTL_RIEN_Msk );
                           ...
                           uint32_t pu32R2ptr[] )
{
    ...
    sdh->CMDARG = u32Arg;
    sdh->CTL = ( sdh->CTL & ( ~SDH_CTL_CMDCODE_Msk ) ) | ( u32Cmd << 8 ) |
               ( SDH_CTL_COEN_Msk | SDH_CTL_R2EN_Msk );
    ...
    if( sdh->INTSTS & SDH_INTSTS_CRC7_Msk )
    {
        ...
        for( i = 0UL; i < 4UL; i++ )
        {
            pu32R2ptr[ i ] = ( ( au32TmpBuf[ i ] & 0x00ffffffUL ) << 8 ) |
                             ( ( au32TmpBuf[ i + 1UL ] & 0xff000000UL ) >> 24 );
        }
        return Successful;
    }
    else
...
uint32_t SDH_SDCmdAndRspDataIn( SDH_T * sdh, uint32_t u32Cmd, uint32_t u32Arg )
{
    ...
    sdh->CMDARG = u32Arg;
    sdh->CTL = ( sdh->CTL & ( ~SDH_CTL_CMDCODE_Msk ) ) |
               ( ( uint32_t ) u32Cmd << 8 ) |
...
uint32_t SDH_Init( SDH_T * sdh )
{
    ...
    else
    {
        ...
        if( i == 2UL ) /* MMC memory */
        {
            ...
            if( SDH_SDCmdAndRsp( sdh, 1UL, 0x40ff8000UL, u32CmdTimeOut ) !=
                2UL ) /* eMMC memory */
            {
                u32Resp = sdh->RESP0;
                while( !( u32Resp & 0x00800000UL ) ) /* check if card is ready
    ...
    if( pSD->CardType != SDH_TYPE_UNKNOWN )
    {
        ...
        if( ( pSD->CardType == SDH_TYPE_MMC ) ||
            ( pSD->CardType == SDH_TYPE_EMMC ) )
        {
            if( ( u32Status = SDH_SDCmdAndRsp( sdh, 3UL, 0x10000UL, 0UL ) ) !=
                Successful ) /* set RCA */
            {
                return u32Status;
            }
        ...
        {
            if( ( u32Status = SDH_SDCmdAndRsp( sdh, 3UL, 0x00UL, 0UL ) ) !=
                Successful ) /* get RCA */
            {
                return u32Status;
            }
...
uint32_t SDH_SwitchToHighSpeed( SDH_T * sdh, SDH_INFO_T * pSD )
{
    ...
    sdh->DMASA = ( uint32_t ) _SDH_ucSDHCBuffer; /* set DMA transfer starting
                                                    address */
    sdh->BLEN = 63UL;                            /* 512 bit */

    if( ( u32Status = SDH_SDCmdAndRspDataIn( sdh, 6UL, 0x00ffff01UL ) ) !=
        Successful )
    {
        return Fail;
    }
    ...
    {
        sdh->DMASA = ( uint32_t ) _SDH_ucSDHCBuffer; /* set DMA transfer
                                                        starting address */
        sdh->BLEN = 63UL;                            /* 512 bit */

        if( ( u32Status = SDH_SDCmdAndRspDataIn( sdh, 6UL, 0x80ffff01UL ) ) !=
            Successful )
        {
            return Fail;
        }
...
uint32_t SDH_SelectCardType( SDH_T * sdh )
{
    ...
    if( ( u32Status = SDH_SDCmdAndRsp( sdh, 7UL, pSD->RCA, 0UL ) ) !=
        Successful )
    {
        return u32Status;
    }
    ...
    if( pSD->CardType == SDH_TYPE_SD_HIGH )
    {
        ...
        if( ( u32Status = SDH_SDCmdAndRsp( sdh, 55UL, pSD->RCA, 0UL ) ) !=
            Successful )
        {
            return u32Status;
        }
        ...
        if( ( u32Status = SDH_SDCmdAndRspDataIn( sdh, 51UL, 0x00UL ) ) !=
            Successful )
        {
            return u32Status;
        }
        ...
        if( ( u32Status = SDH_SDCmdAndRsp( sdh, 6UL, 0x02UL, 0UL ) ) !=
            Successful ) /* set bus width */
        {
            return u32Status;
        }
    ...
    else if( ( pSD->CardType == SDH_TYPE_MMC ) ||
             ( pSD->CardType == SDH_TYPE_EMMC ) )
    {
        if( pSD->CardType == SDH_TYPE_MMC )
        {
        ...
        /* set CMD6 argument Access field to 3, Index to 183, Value to 1 (4-bit
         * mode) */
        u32Param = ( 3UL << 24 ) | ( 183UL << 16 ) | ( 1UL << 8 );
        if( ( u32Status = SDH_SDCmdAndRsp( sdh, 6UL, u32Param, 0UL ) ) !=
            Successful )
        {
            return u32Status;
        }
        ...
        sdh->CTL |= SDH_CTL_DBW_Msk; /* set bus width to 4-bit mode for SD host
                                        controller */
    }

    if( ( u32Status = SDH_SDCmdAndRsp( sdh, 16UL, SDH_BLOCK_SIZE, 0UL ) ) !=
        Successful ) /* set block length */
    {
        return u32Status;
    }
...
void SDH_Get_SD_info( SDH_T * sdh )
{
        ...
        ( pSD->CardType == SDH_TYPE_EMMC ) )
    {
        ...
        if( ( au32Buffer[ 0 ] & 0xc0000000UL ) == 0xc0000000UL )
        {
            ...
            /* CSD version depend on EXT_CSD register in eMMC v4.4 for card size
             * > 2GB */
        ...
        else
        {
            ...
            u32CSize = ( ( au32Buffer[ 1 ] & 0x000003ffUL ) << 2 ) |
                       ( ( au32Buffer[ 2 ] & 0xc0000000UL ) >> 30 );
            u32Mult = ( au32Buffer[ 2 ] & 0x00038000UL ) >> 15;
            u32Size = ( u32CSize + 1UL ) * ( 1UL << ( u32Mult + 2UL ) ) *
                      ( 1UL << u32RLen );
    ...
    else
    {
        ...
        {
            u32CSize = ( ( au32Buffer[ 1 ] & 0x0000003fUL ) << 16 ) |
                       ( ( au32Buffer[ 2 ] & 0xffff0000UL ) >> 16 );
...
/**
 *  @brief  This function use to reset SD function and select card detection
 * source and pin.
 *
...
void SDH_Open( SDH_T * sdh, uint32_t u32CardDetSrc )
{
    ...
    sdh->GCTL = SDH_GCTL_GCTLRST_Msk |
                SDH_GCTL_SDEN_Msk; /* Start reset FMI controller. */
 ...
 *           SD initial state needs 400KHz clock output, driver will use HIRC
 * for SD initial clock source. And then switch back to the user's setting.
 */
                   ...
                   uint32_t u32SecCount )
{
    ...
    sdh->BLEN = u32BlkSize -
                1UL; /* the actual byte count is equal to (SDBLEN+1) */
                    ...
                    uint32_t u32SecCount )
{
    ...
    /* According to SD Spec v2.0, the write CMD block size MUST be 512, and the
     * start address MUST be 512*n. */
    sdh->BLEN = SDH_BLOCK_SIZE - 1UL; /* set the block size */
    ...
    {
        sdh->CMDARG = u32StartSec *
                      SDH_BLOCK_SIZE; /* set start address for SD CMD */
    }

    sdh->DMASA = ( uint32_t ) pu8BufAddr;
    u32Loop = u32SecCount / 255UL; /* the maximum block count is 0xFF=255 for
                                      register SDCR[BLK_CNT] */
    while( u32Loop > 0UL )
    {
        g_u8SDDataReadyFlag = ( uint8_t ) FALSE;
