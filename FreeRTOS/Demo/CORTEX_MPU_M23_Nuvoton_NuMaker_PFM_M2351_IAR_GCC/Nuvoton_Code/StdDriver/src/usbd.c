...
volatile uint8_t g_USBD_u8RemoteWakeupEn = 0UL; /*!< Remote wake up function
                                                   enable flag */
...
VENDOR_REQ g_USBD_pfnVendorRequest = NULL; /*!< USB Vendor Request Functional
                                              Pointer */
CLASS_REQ g_USBD_pfnClassRequest = NULL;   /*!< USB Class Request Functional
                                              Pointer */
SET_INTERFACE_REQ g_USBD_pfnSetInterface = NULL;  /*!< USB Set Interface
                                                     Functional Pointer */
SET_CONFIG_CB g_USBD_pfnSetConfigCallback = NULL; /*!< USB Set configuration
                                                     callback function pointer
                                                   */
uint32_t g_USBD_u32EpStallLock = 0UL; /*!< Bit map flag to lock specified EP
                                         when SET_FEATURE */
...
void USBD_Start( void )
{
    ...
    USBD_CLR_INT_FLAG( USBD_INT_BUS | USBD_INT_USB | USBD_INT_FLDET |
                       USBD_INT_WAKEUP );

    /* Enable USB-related interrupts. */
    USBD_ENABLE_INT( USBD_INT_BUS | USBD_INT_USB | USBD_INT_FLDET |
                     USBD_INT_WAKEUP );
}
...
void USBD_GetDescriptor( void )
{
    ...
    switch( g_USBD_au8SetupPacket[ 3 ] )
    {
        ...
        case DESC_DEVICE:
        {
            ...
            USBD_PrepareCtrlIn( ( uint8_t * ) g_USBD_sInfo->gu8DevDesc,
                                u32Len );
            break;
        }
        ...
        case DESC_CONFIG:
        {
            ...
            u32TotalLen = g_USBD_sInfo->gu8ConfigDesc[ 2 ] +
                          ( u32TotalLen << 8UL );
            ...
            USBD_PrepareCtrlIn( ( uint8_t * ) g_USBD_sInfo->gu8ConfigDesc,
                                u32Len );
            break;
        }
        /* Get BOS Descriptor */
        case DESC_BOS:
        {
            ...
            USBD_PrepareCtrlIn( ( uint8_t * ) g_USBD_sInfo->gu8BosDesc,
                                u32Len );
            break;
        }
        /* Get HID Descriptor */
        case DESC_HID:
        {
            /* CV3.0 HID Class Descriptor Test,
               Need to indicate index of the HID Descriptor within
               gu8ConfigDescriptor, specifically HID Composite device. */
            uint32_t u32ConfigDescOffset; /* u32ConfigDescOffset is
                                             configuration descriptor offset
                                             (HID descriptor start index) */
            u32Len = USBD_Minimum( u32Len, LEN_HID );
            u32ConfigDescOffset = g_USBD_sInfo->gu32ConfigHidDescIdx
                                      [ g_USBD_au8SetupPacket[ 4 ] ];
            USBD_PrepareCtrlIn( ( uint8_t * ) &g_USBD_sInfo
                                    ->gu8ConfigDesc[ u32ConfigDescOffset ],
                                u32Len );
            break;
        }
        ...
        {
            if( u32Len >
                g_USBD_sInfo->gu32HidReportSize[ g_USBD_au8SetupPacket[ 4 ] ] )
            {
                u32Len = g_USBD_sInfo
                             ->gu32HidReportSize[ g_USBD_au8SetupPacket[ 4 ] ];
                if( ( u32Len % g_USBD_u32CtrlMaxPktSize ) == 0UL )
                {
                    g_USBD_u8CtrlInZeroFlag = ( uint8_t ) 1UL;
                }
            ...
            USBD_PrepareCtrlIn(
                ( uint8_t * ) g_USBD_sInfo
                    ->gu8HidReportDesc[ g_USBD_au8SetupPacket[ 4 ] ],
                u32Len );
            break;
        }
        ...
        case DESC_STRING:
        {
            ...
            if( g_USBD_au8SetupPacket[ 2 ] < 4UL )
            {
                if( u32Len >
                    g_USBD_sInfo
                        ->gu8StringDesc[ g_USBD_au8SetupPacket[ 2 ] ][ 0 ] )
                {
                    u32Len = g_USBD_sInfo
                                 ->gu8StringDesc[ g_USBD_au8SetupPacket[ 2 ] ]
                                                [ 0 ];
                    if( ( u32Len % g_USBD_u32CtrlMaxPktSize ) == 0UL )
                    {
                        g_USBD_u8CtrlInZeroFlag = ( uint8_t ) 1UL;
...
void USBD_StandardRequest( void )
{
    ...
    if( ( g_USBD_au8SetupPacket[ 0 ] & 0x80UL ) ==
        0x80UL ) /* request data transfer direction */
    {
        /* Device to host */
            ...
            case GET_STATUS:
            {
                ...
                if( g_USBD_au8SetupPacket[ 0 ] == 0x80UL )
                {
                    ...
                    if( ( g_USBD_sInfo->gu8ConfigDesc[ 7 ] & 0x20UL ) ==
                        0x20UL )
                    {
                        u8Tmp |= ( uint8_t ) ( g_USBD_u8RemoteWakeupEn
                                               << 1UL ); /* Remote wake up */
                    }
                ...
                {
                    uint8_t ep = ( uint8_t ) ( g_USBD_au8SetupPacket[ 4 ] &
                                               0xFUL );
                    u32Addr = USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR( EP0 );
                    M8( u32Addr ) = ( uint8_t ) ( USBD_GetStall( ep ) ? 1UL
                                                                      : 0UL );
                }
        ...
        switch( g_USBD_au8SetupPacket[ 1 ] )
        {
            case CLEAR_FEATURE:
            {
                if( g_USBD_au8SetupPacket[ 2 ] == FEATURE_ENDPOINT_HALT )
                {
                    ...
                    /* EP number stall is not allow to be clear in MSC class
                       "Error Recovery Test". a flag: g_USBD_u32EpStallLock is
                       added to support it */
                    epNum = ( uint8_t ) ( g_USBD_au8SetupPacket[ 4 ] & 0xFUL );
                    for( i = 0UL; i < USBD_MAX_EP; i++ )
                    {
                        if( ( ( pUSBD->EP[ i ].CFG & 0xFUL ) == epNum ) &&
                            ( ( g_USBD_u32EpStallLock & ( 1UL << i ) ) ==
                              0UL ) )
                        {
                ...
                else if( g_USBD_au8SetupPacket[ 2 ] ==
            ...
            case SET_FEATURE:
            {
                if( ( g_USBD_au8SetupPacket[ 0 ] & 0xFUL ) == 0UL ) /* 0: device
                                                                     */
                {
                    if( ( g_USBD_au8SetupPacket[ 2 ] == 3UL ) &&
                        ( g_USBD_au8SetupPacket[ 3 ] == 0UL ) ) /* 3: HNP enable
                                                                 */
                    {
                        pOTG->CTL |= ( OTG_CTL_HNPREQEN_Msk |
                                       OTG_CTL_BUSREQ_Msk );
                    }
                }
                if( g_USBD_au8SetupPacket[ 2 ] == FEATURE_ENDPOINT_HALT )
                {
                    USBD_SetStall(
                        ( uint8_t ) ( g_USBD_au8SetupPacket[ 4 ] & 0xFUL ) );
                }
                else if( g_USBD_au8SetupPacket[ 2 ] ==
                         FEATURE_DEVICE_REMOTE_WAKEUP )
                {
...
void USBD_CtrlIn( void )
{
    ...
    if( g_USBD_u32CtrlInSize )
    {
        ...
        if( g_USBD_u32CtrlInSize > g_USBD_u32CtrlMaxPktSize )
        {
            /* Data size > MXPLD */
            u32Addr = USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR( EP0 );
            USBD_MemCopy( ( uint8_t * ) u32Addr,
                          ( uint8_t * ) g_USBD_pu8CtrlInPointer,
                          g_USBD_u32CtrlMaxPktSize );
            USBD_SET_PAYLOAD_LEN( EP0, g_USBD_u32CtrlMaxPktSize );
    ...
    else /* No more data for IN token */
    {
        ...
        if( ( g_USBD_au8SetupPacket[ 0 ] == REQ_STANDARD ) &&
            ( g_USBD_au8SetupPacket[ 1 ] == SET_ADDRESS ) )
        {
...
void USBD_CtrlOut( void )
{
    ...
    if( g_USBD_u32CtrlOutSize < g_USBD_u32CtrlOutSizeLimit )
    {
        ...
        USBD_MemCopy( ( uint8_t * ) g_USBD_pu8CtrlOutPointer,
                      ( uint8_t * ) u32Addr,
                      u32Size );
 ...
 * @return   None
 *
 * @details  This function resets all variables for protocol and resets USB
 * device address to 0.
 *
 */
 ...
 * @return      None
 *
 * @details     This function is used to set USBD vendor request callback
 * function
 */
...
/**
 * @brief       The callback function which called when get SET CONFIGURATION
 * request
 *
 * @param[in]   pfnSetConfigCallback    Callback function pointer for SET
 * CONFIGURATION request
 *
 * @return      None
 *
 * @details     This function is used to set the callback function which will be
 * called at SET CONFIGURATION request.
 */
...
/**
 * @brief       EP stall lock function to avoid stall clear by USB SET FEATURE
 * request.
 *
 * @param[in]   u32EpBitmap    Use bitmap to select which endpoints will be
 * locked
 *
 * @return      None
 *
 * @details     This function is used to lock relative endpoint to avoid stall
 * clear by SET FEATURE request. If ep stall locked, user needs to reset USB
 * device or re-configure device to clear it.
 */
