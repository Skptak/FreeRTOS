 ...
 * @param[in]  u32MasterSlave Decides the SPI module is operating in master mode
 * or in slave mode. (SPI_SLAVE, SPI_MASTER)
 * @param[in]  u32SPIMode Decides the transfer timing. (SPI_MODE_0, SPI_MODE_1,
 ...
 * @details By default, the SPI transfer sequence is MSB first, the slave
 * selection signal is active low and the automatic slave selection function is
 * disabled. In Slave mode, the u32BusClock shall be NULL and the SPI clock
 * divider setting will be 0. The actual clock rate may be different from the
 * target SPI clock rate. For example, if the SPI source clock rate is 12 MHz
 * and the target SPI bus clock rate is 7 MHz, the actual SPI clock rate will be
 * 6MHz.
 * @note   If u32BusClock = 0, DIVIDER setting will be set to the maximum value.
 * @note   If u32BusClock >= system clock frequency for Secure, SPI peripheral
 * clock source will be set to APB clock and DIVIDER will be set to 0.
 * @note   If u32BusClock >= system clock frequency for Non-Secure, this
 * function does not do anything to avoid the situation that the frequency of
 *         SPI bus clock cannot be faster than the system clock rate. User
 * should set up carefully.
 * @note   If u32BusClock >= SPI peripheral clock source, DIVIDER will be set to
 * 0.
 * @note   In slave mode for Secure, the SPI peripheral clock rate will equal to
 * APB clock rate.
 * @note   In slave mode for Non-Secure, the SPI peripheral clock rate will
 * equal to the clock rate set in secure mode.
 */
...
{
    uint32_t u32ClkSrc = 0UL, u32Div, u32HCLKFreq, u32PCLK0Freq, u32PCLK1Freq,
             u32RetValue = 0UL;
    ...
    {
        /* Default setting: slave selection signal is active low; disable
         * automatic slave selection function. */
        spi->SSCTL = SPI_SS_ACTIVE_LOW;

        /* Default setting: MSB first, disable unit transfer interrupt, SP_CYCLE
         * = 0. */
        spi->CTL = u32MasterSlave | ( u32DataWidth << SPI_CTL_DWIDTH_Pos ) |
                   ( u32SPIMode ) | SPI_CTL_SPIEN_Msk;
        ...
        if( ( spi == SPI0 ) || ( spi == SPI0_NS ) )
        {
            if( ( CLK_GetModuleClockSource( SPI0_MODULE )
                  << CLK_CLKSEL2_SPI0SEL_Pos ) == CLK_CLKSEL2_SPI0SEL_HXT )
            {
                u32ClkSrc = __HXT; /* Clock source is HXT */
            }
            else if( ( CLK_GetModuleClockSource( SPI0_MODULE )
                       << CLK_CLKSEL2_SPI0SEL_Pos ) == CLK_CLKSEL2_SPI0SEL_PLL )
            {
        ...
        {
            if( ( CLK_GetModuleClockSource( SPI1_MODULE )
                  << CLK_CLKSEL2_SPI1SEL_Pos ) == CLK_CLKSEL2_SPI1SEL_HXT )
            {
                u32ClkSrc = __HXT; /* Clock source is HXT */
            }
            else if( ( CLK_GetModuleClockSource( SPI1_MODULE )
                       << CLK_CLKSEL2_SPI1SEL_Pos ) == CLK_CLKSEL2_SPI1SEL_PLL )
            {
                u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
            }
            else if( ( CLK_GetModuleClockSource( SPI1_MODULE )
        ...
        {
            if( ( CLK_GetModuleClockSource( SPI2_MODULE )
                  << CLK_CLKSEL2_SPI2SEL_Pos ) == CLK_CLKSEL2_SPI2SEL_HXT )
            {
                u32ClkSrc = __HXT; /* Clock source is HXT */
            }
            else if( ( CLK_GetModuleClockSource( SPI2_MODULE )
                       << CLK_CLKSEL2_SPI2SEL_Pos ) == CLK_CLKSEL2_SPI2SEL_PLL )
            {
                u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
            }
            else if( ( CLK_GetModuleClockSource( SPI2_MODULE )
        ...
        {
            if( ( CLK_GetModuleClockSource( SPI3_MODULE )
                  << CLK_CLKSEL2_SPI3SEL_Pos ) == CLK_CLKSEL2_SPI3SEL_HXT )
            {
                u32ClkSrc = __HXT; /* Clock source is HXT */
            }
            else if( ( CLK_GetModuleClockSource( SPI3_MODULE )
                       << CLK_CLKSEL2_SPI3SEL_Pos ) == CLK_CLKSEL2_SPI3SEL_PLL )
            {
                u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
            }
            else if( ( CLK_GetModuleClockSource( SPI3_MODULE )
        ...
        {
            /* Set DIVIDER to the maximum value 0x1FF. f_spi = f_spi_clk_src /
             * (DIVIDER + 1) */
        ...
        {
            u32Div = ( ( ( u32ClkSrc * 10UL ) / u32BusClock + 5UL ) / 10UL ) -
                     1UL; /* Round to the nearest integer */
            if( u32Div > 0x1FFUL )
            {
                u32Div = 0x1FFUL;
                spi->CLKDIV |= SPI_CLKDIV_DIVIDER_Msk;
    ...
    else /* For slave mode, force the SPI peripheral clock rate to equal APB
            clock rate. */
    {
        ...
        if( !( __PC() & ( 1UL << 28UL ) ) )
        {
            ...
            {
                CLK->CLKSEL2 = ( CLK->CLKSEL2 & ( ~CLK_CLKSEL2_SPI2SEL_Msk ) ) |
                               CLK_CLKSEL2_SPI2SEL_PCLK1;
                /* Return slave peripheral clock rate */
                u32RetValue = u32PCLK1Freq;
            }
            ...
            {
                CLK->CLKSEL2 = ( CLK->CLKSEL2 & ( ~CLK_CLKSEL2_SPI3SEL_Msk ) ) |
                               CLK_CLKSEL2_SPI3SEL_PCLK0;
                /* Return slave peripheral clock rate */
                u32RetValue = u32PCLK0Freq;
            }
 ...
 * @param[in]  spi The pointer of the specified SPI module.
 * @return None
 * @details This function will clear SPI RX FIFO buffer. The RXEMPTY
 * (SPI_STATUS[8]) will be set to 1.
 */
 ...
 * @param[in]  spi The pointer of the specified SPI module.
 * @return None
 * @details This function will clear SPI TX FIFO buffer. The TXEMPTY
 * (SPI_STATUS[16]) will be set to 1.
 ...
 * @param[in]  spi The pointer of the specified SPI module.
 * @return None
 * @details This function will disable the automatic slave selection function
 * and set slave selection signal to inactive state.
 */
 ...
 * @param[in]  u32ActiveLevel Specifies the active level of slave selection
 * signal. (SPI_SS_ACTIVE_HIGH, SPI_SS_ACTIVE_LOW)
 * @return None
 * @details This function will enable the automatic slave selection function.
 * Only available in Master mode. The slave selection pin and the active level
...
{
    spi->SSCTL = ( spi->SSCTL &
                   ( ~( SPI_SSCTL_AUTOSS_Msk | SPI_SSCTL_SSACTPOL_Msk |
                        SPI_SSCTL_SS_Msk ) ) ) |
                 ( u32SSPinMask | u32ActiveLevel | SPI_SSCTL_AUTOSS_Msk );
}
 ...
 * @details This function is only available in Master mode. The actual clock
 * rate may be different from the target SPI bus clock rate. For example, if the
 * SPI source clock rate is 12 MHz and the target SPI bus clock rate is 7 MHz,
 * the actual SPI bus clock rate will be 6 MHz.
 * @note   If u32BusClock = 0, DIVIDER setting will be set to the maximum value.
 * @note   If u32BusClock >= system clock frequency for Secure, SPI peripheral
 * clock source will be set to APB clock and DIVIDER will be set to 0.
 * @note   If u32BusClock >= system clock frequency for Non-Secure, this
    ...
    if( ( spi == SPI0 ) || ( spi == SPI0_NS ) )
    {
                   ...
                   << CLK_CLKSEL2_SPI0SEL_Pos ) == CLK_CLKSEL2_SPI0SEL_PLL )
        {
            u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
                   ...
                   << CLK_CLKSEL2_SPI0SEL_Pos ) == CLK_CLKSEL2_SPI0SEL_PCLK1 )
        {
            u32ClkSrc = CLK_GetPCLK1Freq(); /* Clock source is PCLK1 */
        }
        else
    ...
    else if( ( spi == SPI1 ) || ( spi == SPI1_NS ) )
    {
                   ...
                   << CLK_CLKSEL2_SPI1SEL_Pos ) == CLK_CLKSEL2_SPI1SEL_PLL )
        {
            u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
                   ...
                   << CLK_CLKSEL2_SPI1SEL_Pos ) == CLK_CLKSEL2_SPI1SEL_PCLK0 )
        {
            u32ClkSrc = CLK_GetPCLK0Freq(); /* Clock source is PCLK0 */
        }
        else
    ...
    else if( ( spi == SPI2 ) || ( spi == SPI2_NS ) )
    {
                   ...
                   << CLK_CLKSEL2_SPI2SEL_Pos ) == CLK_CLKSEL2_SPI2SEL_PLL )
        {
            u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
                   ...
                   << CLK_CLKSEL2_SPI2SEL_Pos ) == CLK_CLKSEL2_SPI2SEL_PCLK1 )
        {
            u32ClkSrc = CLK_GetPCLK1Freq(); /* Clock source is PCLK1 */
        }
        else
    ...
    else
    {
                   ...
                   << CLK_CLKSEL2_SPI3SEL_Pos ) == CLK_CLKSEL2_SPI3SEL_PLL )
        {
            u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
                   ...
                   << CLK_CLKSEL2_SPI3SEL_Pos ) == CLK_CLKSEL2_SPI3SEL_PCLK0 )
        {
            u32ClkSrc = CLK_GetPCLK0Freq(); /* Clock source is PCLK0 */
        }
        else
...
{
    spi->FIFOCTL = ( spi->FIFOCTL &
                     ~( SPI_FIFOCTL_TXTH_Msk | SPI_FIFOCTL_RXTH_Msk ) ) |
                   ( u32TxThreshold << SPI_FIFOCTL_TXTH_Pos ) |
                   ( u32RxThreshold << SPI_FIFOCTL_RXTH_Pos );
}

/**
 * @brief  Get the actual frequency of SPI bus clock. Only available in Master
 * mode.
 * @param[in]  spi The pointer of the specified SPI module.
 * @return Actual SPI bus clock frequency in Hz.
 * @details This function will calculate the actual SPI bus clock rate according
 * to the SPIxSEL and DIVIDER settings. Only available in Master mode.
 */
...
uint32_t SPI_GetIntFlag( SPI_T * spi, uint32_t u32Mask )
{
    ...
    if( ( u32Mask & SPI_UNIT_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_UNITIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_SSACT_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_SSACTIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_SSINACT_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_SSINAIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_SLVUR_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_SLVURIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_SLVBE_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_SLVBEIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_TXUF_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_TXUFIF_Msk ) )
    {
        u32IntFlag |= SPI_TXUF_INT_MASK;
    }

    /* Check TX threshold interrupt flag */
    if( ( u32Mask & SPI_FIFO_TXTH_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_TXTHIF_Msk ) )
    {
        u32IntFlag |= SPI_FIFO_TXTH_INT_MASK;
    }

    /* Check RX threshold interrupt flag */
    if( ( u32Mask & SPI_FIFO_RXTH_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_RXTHIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_FIFO_RXOV_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_RXOVIF_Msk ) )
    {
    ...
    if( ( u32Mask & SPI_FIFO_RXTO_INT_MASK ) &&
        ( u32IntStatus & SPI_STATUS_RXTOIF_Msk ) )
    {
 ...
 *                     Each bit corresponds to a interrupt source.
 *                     This parameter decides which interrupt flags will be
 * cleared. It could be the combination of:
 *                       - \ref SPI_UNIT_INT_MASK
 *                       - \ref SPI_SSACT_INT_MASK
 *                       - \ref SPI_SSINACT_INT_MASK
 *                       - \ref SPI_SLVUR_INT_MASK
...
void SPI_ClearIntFlag( SPI_T * spi, uint32_t u32Mask )
{
    if( u32Mask & SPI_UNIT_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_UNITIF_Msk; /* Clear unit transfer interrupt
                                                flag */
    }

    if( u32Mask & SPI_SSACT_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_SSACTIF_Msk; /* Clear slave selection signal
                                                 active interrupt flag */
    }

    if( u32Mask & SPI_SSINACT_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_SSINAIF_Msk; /* Clear slave selection signal
                                                 inactive interrupt flag */
    }

    if( u32Mask & SPI_SLVUR_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_SLVURIF_Msk; /* Clear slave TX under run
                                                 interrupt flag */
    }

    if( u32Mask & SPI_SLVBE_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_SLVBEIF_Msk; /* Clear slave bit count error
                                                 interrupt flag */
    }

    if( u32Mask & SPI_TXUF_INT_MASK )
    {
        spi->STATUS = SPI_STATUS_TXUFIF_Msk; /* Clear slave TX underflow
                                                interrupt flag */
    }
 ...
 *                     This parameter decides which flags will be read. It is
 * combination of:
 *                       - \ref SPI_BUSY_MASK
...
uint32_t SPI_GetStatus( SPI_T * spi, uint32_t u32Mask )
{
    ...
    /* Check RX empty flag */
    if( ( u32Mask & SPI_RX_EMPTY_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_RXEMPTY_Msk ) )
    {
        u32Flag |= SPI_RX_EMPTY_MASK;
    }

    /* Check RX full flag */
    if( ( u32Mask & SPI_RX_FULL_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_RXFULL_Msk ) )
    {
        u32Flag |= SPI_RX_FULL_MASK;
    }

    /* Check TX empty flag */
    if( ( u32Mask & SPI_TX_EMPTY_MASK ) &&
    ...
    /* Check TX full flag */
    if( ( u32Mask & SPI_TX_FULL_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_TXFULL_Msk ) )
    {
        u32Flag |= SPI_TX_FULL_MASK;
    }

    /* Check TX/RX reset flag */
    if( ( u32Mask & SPI_TXRX_RESET_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_TXRXRST_Msk ) )
    {
        u32Flag |= SPI_TXRX_RESET_MASK;
    }

    /* Check SPIEN flag */
    if( ( u32Mask & SPI_SPIEN_STS_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_SPIENSTS_Msk ) )
    {
        u32Flag |= SPI_SPIEN_STS_MASK;
    }

    /* Check SPIx_SS line status */
    if( ( u32Mask & SPI_SSLINE_STS_MASK ) &&
        ( u32TmpStatus & SPI_STATUS_SSLINE_Msk ) )
    {
 ...
 * @details Return the source clock frequency according to the setting of
 * SPI0SEL (CLK_CLKSEL2[5:4]) or SPI1SEL (CLK_CLKSEL2[7:6]) or SPI2SEL
 * (CLK_CLKSEL2[11:10]) or SPI3SEL (CLK_CLKSEL2[13:12]).
 */
static uint32_t SPII2S_GetSourceClockFreq( SPI_T * i2s )
{
    uint32_t u32Freq;
...
/**
 * @brief  This function configures some parameters of I2S interface for general
 * purpose use.
 * @param[in] i2s The pointer of the specified I2S module.
 ...
 * @return Real sample rate of master mode or peripheral clock rate of slave
 * mode.
 * @details This function will reset SPI/I2S controller and configure I2S
 * controller according to the input parameters. Set TX FIFO threshold to 2 and
 * RX FIFO threshold to 1. Both the TX and RX functions will be enabled. The
 * actual sample rate may be different from the target sample rate. The real
 * sample rate will be returned for reference.
 * @note   In slave mode for Secure, the SPI peripheral clock rate will equal to
 * APB clock rate.
 * @note   In slave mode for Non-Secure, the SPI peripheral clock rate will
 * equal to the clock rate set in secure mode.
                      ...
                      uint32_t u32DataFormat )
{
    ...
    if( u32MasterSlave == SPI_MASTER )
    {
        ...
        u32BitRate = u32SampleRate *
                     ( ( u32WordWidth >> SPI_I2SCTL_WDWIDTH_Pos ) + 1UL ) *
                     16UL;
        u32Divider = ( ( ( ( ( u32SrcClk * 10UL ) / u32BitRate ) >> 1UL ) +
                         5UL ) /
                       10UL ) -
                     1UL; /* Round to the nearest integer */
        /* Set BCLKDIV setting */
        i2s->I2SCLK = ( i2s->I2SCLK & ~SPI_I2SCLK_BCLKDIV_Msk ) |
                      ( u32Divider << SPI_I2SCLK_BCLKDIV_Pos );

        /* Calculate bit clock rate */
        u32BitRate = u32SrcClk / ( ( u32Divider + 1UL ) * 2UL );
        /* Calculate real sample rate */
        u32SampleRate = u32BitRate /
                        ( ( ( u32WordWidth >> SPI_I2SCTL_WDWIDTH_Pos ) + 1UL ) *
                          16UL );

        /* Enable TX function, RX function and I2S mode. */
        i2s->I2SCTL |= ( SPI_I2SCTL_RXEN_Msk | SPI_I2SCTL_TXEN_Msk |
 ...
 * @param[in] i2s The pointer of the specified I2S module.
 * @param[in] u32Mask The combination of all related interrupt enable bits.
 *            Each bit corresponds to a interrupt source. Valid values are
 * listed below.
 ...
 * @return None
 * @details This function enables the interrupt according to the u32Mask
 * parameter.
 */
 ...
 *            - \ref SPII2S_LEFT_ZC_INT_MASK
 * @return None
 * @details This function disables the interrupt according to the u32Mask
 * parameter.
 */
 ...
 * @details Set the master clock rate according to u32BusClock parameter and
 * enable master clock output. The actual master clock rate may be different
 * from the target master clock rate. The real master clock rate will be
...
uint32_t SPII2S_EnableMCLK( SPI_T * i2s, uint32_t u32BusClock )
{
    ...
    i2s->I2SCLK = ( i2s->I2SCLK & ~SPI_I2SCLK_MCLKDIV_Msk ) |
                  ( u32Divider << SPI_I2SCLK_MCLKDIV_Pos );
    ...
    if( u32Divider == 0UL )
    {
        u32RetValue = u32SrcClk; /* If MCLKDIV=0, master clock rate is equal to
                                    the source clock rate. */
    }
    else
    {
        u32RetValue = ( ( u32SrcClk >> 1UL ) /
                        u32Divider ); /* If MCLKDIV>0, master clock rate =
                                         source clock rate / (MCLKDIV * 2) */
    }
 ...
 * @param[in] i2s The pointer of the specified I2S module.
 * @return None
 * @details Clear MCLKEN bit of SPI_I2SCTL register to disable master clock
 * output.
 */
