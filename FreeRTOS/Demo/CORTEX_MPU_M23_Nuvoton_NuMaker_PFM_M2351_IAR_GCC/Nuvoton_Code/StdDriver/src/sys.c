 ...
 * @return     None
 * @details    This function configure Brown-out detector reset or interrupt
 * mode, enable Brown-out function and set Brown-out voltage level. The register
 * write-protection function should be disabled before using this function.
 */
void SYS_EnableBOD( int32_t i32Mode, uint32_t u32BODLevel )
{
    ...
    SYS->BODCTL = ( SYS->BODCTL & ~SYS_BODCTL_BODRSTEN_Msk ) |
                  ( uint32_t ) i32Mode;
 ...
 *             The main voltage regulator type setting to DCDC cannot finished
 * if the inductor is not detected. The register write-protection function
...
uint32_t SYS_SetPowerRegulator( uint32_t u32PowerRegulator )
{
    ...
    if( ( u32PowerRegulator == SYS_PLCTL_MVRS_DCDC ) &&
        ( u32PowerRegStatus == SYS_PLSTS_CURMVR_LDO ) )
    {
...
void SYS_SetSSRAMPowerMode( uint32_t u32SRAMSel, uint32_t u32PowerMode )
{
    ...
    SYS->SRAMPCTL = ( SYS->SRAMPCTL & ( ~u32SRAMSel ) ) |
                    ( u32PowerMode << u32SRAMSelPos );
}
...
void SYS_SetPSRAMPowerMode( uint32_t u32SRAMSel, uint32_t u32PowerMode )
{
    ...
    SYS->SRAMPPCT = ( SYS->SRAMPPCT & ( ~u32SRAMSel ) ) |
                    ( u32PowerMode << u32SRAMSelPos );
}
