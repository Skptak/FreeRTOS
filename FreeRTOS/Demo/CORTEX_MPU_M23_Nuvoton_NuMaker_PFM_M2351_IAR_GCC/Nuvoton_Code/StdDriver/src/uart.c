 ...
 *
 *    @param[in]    uart                The pointer of the specified UART
 * module.
...
{
    if( u32InterruptFlag & UART_INTSTS_RLSINT_Msk ) /* Clear Receive Line Status
                                                       Interrupt */
    {
    ...
    if( u32InterruptFlag & UART_INTSTS_MODEMINT_Msk ) /* Clear MODEM Status
                                                         Interrupt */
    {
        uart->MODEMSTS |= UART_MODEMSTS_CTSDETF_Msk;
    }

    if( u32InterruptFlag & UART_INTSTS_BUFERRINT_Msk ) /* Clear Buffer Error
                                                          Interrupt */
    {
 ...
 *                                      - \ref UART_INTEN_ABRIEN_Msk     :
 * Auto-baud Rate Interrupt
 *                                      - \ref UART_INTEN_LINIEN_Msk     : Lin
 * Bus interrupt
 *                                      - \ref UART_INTEN_WKIEN_Msk      :
 * Wake-up interrupt
 *                                      - \ref UART_INTEN_BUFERRIEN_Msk  :
 * Buffer Error interrupt
 *                                      - \ref UART_INTEN_RXTOIEN_Msk    : Rx
 * Time-out Interrupt
 *                                      - \ref UART_INTEN_MODEMIEN_Msk   : MODEM
 * Status Interrupt
 *                                      - \ref UART_INTEN_RLSIEN_Msk     :
 * Receive Line Status Interrupt
 *                                      - \ref UART_INTEN_THREIEN_Msk    :
 * Transmit Holding Register Empty Interrupt
 *                                      - \ref UART_INTEN_RDAIEN_Msk     :
 * Receive Data Available Interrupt
 *
 *    @return       None
 *
 *    @details      The function is used to disable UART specified interrupt and
 ...
 *    @return       None
 *
 *    @details      The function is used to enable UART specified interrupt and
...
void UART_Open( UART_T * uart, uint32_t u32baudrate )
{
    ...
    if( u32baudrate != 0ul )
    {
        u32BaudDiv = UART_BAUD_MODE2_DIVIDER(
            ( au32ClkTbl[ u32UartClkSrcSel ] ) / ( u32UartClkDivNum + 1ul ),
            u32baudrate );

        if( u32BaudDiv > 0xFFFFul )
        {
            uart->BAUD = ( UART_BAUD_MODE0 |
                           UART_BAUD_MODE0_DIVIDER(
                               ( au32ClkTbl[ u32UartClkSrcSel ] ) /
 ...
 *
 *    @details      The function is used to read Rx data from RX FIFO and the
...
uint32_t UART_Read( UART_T * uart, uint8_t pu8RxBuf[], uint32_t u32ReadBytes )
{
    ...
    for( u32Count = 0ul; u32Count < u32ReadBytes; u32Count++ )
    {
        ...
        while( uart->FIFOSTS & UART_FIFOSTS_RXEMPTY_Msk ) /* Check RX empty =>
                                                             failed */
        {
            u32delayno++;
        ...
        {
            pu8RxBuf[ u32Count ] = ( uint8_t ) uart->DAT; /* Get Data from UART
                                                             RX  */
        }
    }
 ...
 *
 *    @param[in]    uart            The pointer of the specified UART module.
 *    @param[in]    u32baudrate     The register value of baudrate of UART
 * module. If u32baudrate = 0, UART baudrate will not change.
 *    @param[in]    u32data_width   The data length of UART module.
 *                                  - \ref UART_WORD_LEN_5
 *                                  - \ref UART_WORD_LEN_6
 *                                  - \ref UART_WORD_LEN_7
 *                                  - \ref UART_WORD_LEN_8
 *    @param[in]    u32parity       The parity setting
 * (none/odd/even/mark/space) of UART module.
 *                                  - \ref UART_PARITY_NONE
 *                                  - \ref UART_PARITY_ODD
 *                                  - \ref UART_PARITY_EVEN
 *                                  - \ref UART_PARITY_MARK
 *                                  - \ref UART_PARITY_SPACE
 *    @param[in]    u32stop_bits    The stop bit length (1/1.5/2 bit) of UART
 * module.
 *                                  - \ref UART_STOP_BIT_1
 ...
 *    @return       None
 *
 *    @details      The function is used to configure IrDA relative settings. It
 * consists of TX or RX mode and baudrate.
 */
                          ...
                          uint32_t u32Direction )
{
    ...
    if( u32Buadrate != 0ul )
    {
        u32BaudDiv = UART_BAUD_MODE0_DIVIDER(
            ( au32ClkTbl[ u32UartClkSrcSel ] ) / ( u32UartClkDivNum + 1ul ),
            u32Buadrate );
...
void UART_SelectRS485Mode( UART_T * uart, uint32_t u32Mode, uint32_t u32Addr )
{
    ...
    uart->ALTCTL &= ~( UART_ALTCTL_RS485NMM_Msk | UART_ALTCTL_RS485AUD_Msk |
                       UART_ALTCTL_RS485AAD_Msk | UART_ALTCTL_ADDRMV_Msk );
                         ...
                         uint32_t u32BreakLength )
{
     ...
     */
    uart->ALTCTL &= ~( UART_ALTCTL_LINTXEN_Msk | UART_ALTCTL_LINRXEN_Msk |
 ...
 *
 *    @param[in]    uart            The pointer of the specified UART module.
 *    @param[in]    pu8TxBuf        The buffer to send the data to UART
 * transmission FIFO.
...
uint32_t UART_Write( UART_T * uart, uint8_t pu8TxBuf[], uint32_t u32WriteBytes )
{
    ...
    for( u32Count = 0ul; u32Count != u32WriteBytes; u32Count++ )
    {
        ...
        while( ( uart->FIFOSTS & UART_FIFOSTS_TXEMPTYF_Msk ) ==
               0ul ) /* Wait Tx empty and Time-out manner */
        {
            u32delayno++;
            if( u32delayno >= 0x40000000ul )
            {
