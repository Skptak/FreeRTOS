 ...
 * @param[in]  u32MasterSlave Decides the QSPI module is operating in master
 * mode or in slave mode. (QSPI_SLAVE, QSPI_MASTER)
 * @param[in]  u32QSPIMode Decides the transfer timing. (QSPI_MODE_0,
 * QSPI_MODE_1, QSPI_MODE_2, QSPI_MODE_3)
 ...
 * @details By default, the QSPI transfer sequence is MSB first, the slave
 * selection signal is active low and the automatic slave selection function is
 * disabled. In Slave mode, the u32BusClock shall be NULL and the QSPI clock
 * divider setting will be 0. The actual clock rate may be different from the
 * target QSPI clock rate. For example, if the QSPI source clock rate is 12 MHz
 * and the target QSPI bus clock rate is 7 MHz, the actual QSPI clock rate will
 * be 6MHz.
 * @note   If u32BusClock = 0, DIVIDER setting will be set to the maximum value.
 * @note   If u32BusClock >= system clock frequency for Secure, QSPI peripheral
 ...
 *         QSPI bus clock cannot be faster than the system clock rate. User
 * should set up carefully.
 * @note   If u32BusClock >= QSPI peripheral clock source, DIVIDER will be set
 * to 0.
 * @note   In slave mode for Secure, the QSPI peripheral clock rate will equal
 * to APB clock rate.
 * @note   In slave mode for Non-Secure, the QSPI peripheral clock rate will
...
{
    uint32_t u32ClkSrc = 0UL, u32Div, u32HCLKFreq, u32PCLK0Freq,
             u32RetValue = 0UL;
    ...
    if( u32MasterSlave == QSPI_MASTER )
    {
         ...
         * = 0. */
        qspi->CTL = u32MasterSlave | ( u32DataWidth << QSPI_CTL_DWIDTH_Pos ) |
                    ( u32QSPIMode ) | QSPI_CTL_SPIEN_Msk;
        ...
        if( ( qspi == QSPI0 ) || ( qspi == QSPI0_NS ) )
        {
            if( ( CLK_GetModuleClockSource( QSPI0_MODULE )
                  << CLK_CLKSEL2_QSPI0SEL_Pos ) == CLK_CLKSEL2_QSPI0SEL_HXT )
            {
                u32ClkSrc = __HXT; /* Clock source is HXT */
            }
            else if( ( CLK_GetModuleClockSource( QSPI0_MODULE )
    ...
    else /* For slave mode, force the QSPI peripheral clock rate to equal APB
            clock rate. */
    {
 ...
 * @param[in]  qspi The pointer of the specified QSPI module.
 * @return None
 * @details Clear SPIEN bit of QSPI_CTL register to disable QSPI transfer
 * control.
 */
 ...
 * @param[in]  qspi The pointer of the specified QSPI module.
 * @return None
 * @details This function will clear QSPI RX FIFO buffer. The RXEMPTY
 * (QSPI_STATUS[8]) will be set to 1.
 */
 ...
 * @param[in]  qspi The pointer of the specified QSPI module.
 * @return None
 * @details This function will clear QSPI TX FIFO buffer. The TXEMPTY
 * (QSPI_STATUS[16]) will be set to 1.
 ...
 * signal. (QSPI_SS_ACTIVE_HIGH, QSPI_SS_ACTIVE_LOW)
 * @return None
...
{
    qspi->SSCTL = ( qspi->SSCTL &
                    ( ~( QSPI_SSCTL_AUTOSS_Msk | QSPI_SSCTL_SSACTPOL_Msk |
                         QSPI_SSCTL_SS_Msk ) ) ) |
 ...
 * rate may be different from the target QSPI bus clock rate. For example, if
 * the QSPI source clock rate is 12 MHz and the target QSPI bus clock rate is 7
 * MHz, the actual QSPI bus clock rate will be 6 MHz.
 * @note   If u32BusClock = 0, DIVIDER setting will be set to the maximum value.
 * @note   If u32BusClock >= system clock frequency for Secure, QSPI peripheral
 * clock source will be set to APB clock and DIVIDER will be set to 0.
 * @note   If u32BusClock >= system clock frequency for Non-Secure, this
    ...
    if( ( qspi == QSPI0 ) || ( qspi == QSPI0_NS ) )
    {
              ...
              << CLK_CLKSEL2_QSPI0SEL_Pos ) == CLK_CLKSEL2_QSPI0SEL_HXT )
        {
            u32ClkSrc = __HXT; /* Clock source is HXT */
                   ...
                   << CLK_CLKSEL2_QSPI0SEL_Pos ) == CLK_CLKSEL2_QSPI0SEL_PLL )
        {
            u32ClkSrc = CLK_GetPLLClockFreq(); /* Clock source is PLL */
        }
        else if( ( CLK_GetModuleClockSource( QSPI0_MODULE )
                   << CLK_CLKSEL2_QSPI0SEL_Pos ) == CLK_CLKSEL2_QSPI0SEL_PCLK0 )
        {
            u32ClkSrc = CLK_GetPCLK0Freq(); /* Clock source is PCLK0 */
        }
        else
...
{
    qspi->FIFOCTL = ( qspi->FIFOCTL &
                      ~( QSPI_FIFOCTL_TXTH_Msk | QSPI_FIFOCTL_RXTH_Msk ) ) |
                    ( u32TxThreshold << QSPI_FIFOCTL_TXTH_Pos ) |
                    ( u32RxThreshold << QSPI_FIFOCTL_RXTH_Pos );
}

/**
 * @brief  Get the actual frequency of QSPI bus clock. Only available in Master
 * mode.
 * @param[in]  qspi The pointer of the specified QSPI module.
 * @return Actual QSPI bus clock frequency in Hz.
 * @details This function will calculate the actual QSPI bus clock rate
 * according to the QQSPISEL/QSPIxSEL and DIVIDER settings. Only available in
 * Master mode.
 */
uint32_t QSPI_GetBusClock( QSPI_T * qspi )
{
    ...
    /* Get DIVIDER setting */
    u32Div = ( qspi->CLKDIV & QSPI_CLKDIV_DIVIDER_Msk ) >>
...
uint32_t QSPI_GetIntFlag( QSPI_T * qspi, uint32_t u32Mask )
{
    ...
    if( ( u32Mask & QSPI_UNIT_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_UNITIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_SSACT_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_SSACTIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_SSINACT_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_SSINAIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_SLVUR_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_SLVURIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_SLVBE_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_SLVBEIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_SLVTO_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_SLVTOIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_TXUF_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_TXUFIF_Msk ) )
    {
        u32IntFlag |= QSPI_TXUF_INT_MASK;
    }

    /* Check TX threshold interrupt flag */
    if( ( u32Mask & QSPI_FIFO_TXTH_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_TXTHIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_FIFO_RXTH_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_RXTHIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_FIFO_RXOV_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_RXOVIF_Msk ) )
    {
    ...
    if( ( u32Mask & QSPI_FIFO_RXTO_INT_MASK ) &&
        ( u32IntStatus & QSPI_STATUS_RXTOIF_Msk ) )
    {
...
void QSPI_ClearIntFlag( QSPI_T * qspi, uint32_t u32Mask )
{
    if( u32Mask & QSPI_UNIT_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_UNITIF_Msk; /* Clear unit transfer interrupt
                                                  flag */
    }

    if( u32Mask & QSPI_SSACT_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_SSACTIF_Msk; /* Clear slave selection signal
                                                   active interrupt flag */
    }

    if( u32Mask & QSPI_SSINACT_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_SSINAIF_Msk; /* Clear slave selection signal
                                                   inactive interrupt flag */
    }

    if( u32Mask & QSPI_SLVUR_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_SLVURIF_Msk; /* Clear slave TX under run
                                                   interrupt flag */
    }

    if( u32Mask & QSPI_SLVBE_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_SLVBEIF_Msk; /* Clear slave bit count error
                                                   interrupt flag */
    }

    if( u32Mask & QSPI_SLVTO_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_SLVTOIF_Msk; /* Clear slave mode time-out
                                                   interrupt flag */
    }

    if( u32Mask & QSPI_TXUF_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_TXUFIF_Msk; /* Clear slave TX underflow
                                                  interrupt flag */
    }

    if( u32Mask & QSPI_FIFO_RXOV_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_RXOVIF_Msk; /* Clear RX overrun interrupt
                                                  flag */
    }

    if( u32Mask & QSPI_FIFO_RXTO_INT_MASK )
    {
        qspi->STATUS = QSPI_STATUS_RXTOIF_Msk; /* Clear RX time-out interrupt
                                                  flag */
    }
}
...
uint32_t QSPI_GetStatus( QSPI_T * qspi, uint32_t u32Mask )
{
    ...
    /* Check busy status */
    if( ( u32Mask & QSPI_BUSY_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_BUSY_Msk ) )
    {
        u32Flag |= QSPI_BUSY_MASK;
    }

    /* Check RX empty flag */
    if( ( u32Mask & QSPI_RX_EMPTY_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_RXEMPTY_Msk ) )
    {
        u32Flag |= QSPI_RX_EMPTY_MASK;
    }

    /* Check RX full flag */
    if( ( u32Mask & QSPI_RX_FULL_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_RXFULL_Msk ) )
    {
        u32Flag |= QSPI_RX_FULL_MASK;
    }

    /* Check TX empty flag */
    if( ( u32Mask & QSPI_TX_EMPTY_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_TXEMPTY_Msk ) )
    {
        u32Flag |= QSPI_TX_EMPTY_MASK;
    }

    /* Check TX full flag */
    if( ( u32Mask & QSPI_TX_FULL_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_TXFULL_Msk ) )
    {
        u32Flag |= QSPI_TX_FULL_MASK;
    }

    /* Check TX/RX reset flag */
    if( ( u32Mask & QSPI_TXRX_RESET_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_TXRXRST_Msk ) )
    {
        u32Flag |= QSPI_TXRX_RESET_MASK;
    }

    /* Check SPIEN flag */
    if( ( u32Mask & QSPI_SPIEN_STS_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_SPIENSTS_Msk ) )
    {
        u32Flag |= QSPI_SPIEN_STS_MASK;
    }

    /* Check QSPIx_SS line status */
    if( ( u32Mask & QSPI_SSLINE_STS_MASK ) &&
        ( u32TmpStatus & QSPI_STATUS_SSLINE_Msk ) )
    {
