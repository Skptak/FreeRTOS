 ...
 *    @return       None
 *
 *    @details      The function is used to clear USCI_UART related interrupt
 * flags specified by u32Mask parameter.
 */
...
void UUART_ClearIntFlag( UUART_T * uuart, uint32_t u32Mask )
{
    ...
    {
        uuart->PROTSTS = ( UUART_PROTSTS_BREAK_Msk | UUART_PROTSTS_FRMERR_Msk |
                           UUART_PROTSTS_PARITYERR_Msk );
    }

    if( u32Mask & UUART_BUF_RXOV_INT_MASK ) /* Clear Receive Buffer Over-run
                                               Error Interrupt */
    {
 ...
 *
 *    @details      The function is used to get USCI_UART related interrupt
 * flags specified by u32Mask parameter.
 */
...
uint32_t UUART_GetIntFlag( UUART_T * uuart, uint32_t u32Mask )
{
    ...
    u32Tmp2 = ( uuart->PROTSTS &
                ( UUART_PROTSTS_BREAK_Msk | UUART_PROTSTS_FRMERR_Msk |
                  UUART_PROTSTS_PARITYERR_Msk ) );
    if( u32Tmp1 && u32Tmp2 )
    {
 ...
 *
 *    @param[in]    uuart   The pointer of the specified USCI_UART module.
 *    @param[in]    u32Mask The combination of all related interrupt enable
 * bits. Each bit corresponds to a interrupt enable bit. This parameter decides
 * which interrupts will be disabled. It is combination of:
 *                          - \ref UUART_ABR_INT_MASK
 *                          - \ref UUART_RLS_INT_MASK
 *                          - \ref UUART_BUF_RXOV_INT_MASK
 *                          - \ref UUART_TXST_INT_MASK
 ...
 *    @return       None
 *
 *    @details      The function is used to disabled USCI_UART related
 * interrupts specified by u32Mask parameter.
 */
 ...
 *    @param[in]    u32Mask     The combination of all related interrupt enable
 * bits. Each bit corresponds to a interrupt enable bit. This parameter decides
 * which interrupts will be enabled. It is combination of:
 *                              - \ref UUART_ABR_INT_MASK
 *                              - \ref UUART_RLS_INT_MASK
 *                              - \ref UUART_BUF_RXOV_INT_MASK
 *                              - \ref UUART_TXST_INT_MASK
 ...
 *    @return       None
 *
 *    @details      The function is used to enable USCI_UART related interrupts
 * specified by u32Mask parameter..
 */
 ...
 *
 *    @param[in]    uuart           The pointer of the specified USCI_UART
 * module.
 *    @param[in]    u32baudrate     The baud rate of USCI_UART module.
 *
 *    @return       Real baud rate of USCI_UART module.
 *
 *    @details      This function use to enable USCI_UART function and set
 * baud-rate.
 */
uint32_t UUART_Open( UUART_T * uuart, uint32_t u32baudrate )
{
    ...
    else
    {
        ...
        for( u32DSCnt = 6ul; u32DSCnt <= 0x10ul; u32DSCnt++ ) /* DSCNT could be
                                                                 0x5~0xF */
        {
    ...
    uuart->DATIN0 = ( 2ul << UUART_DATIN0_EDGEDET_Pos ); /* Set falling edge
                                                            detection */
 ...
 *    @param[in]    uuart           The pointer of the specified USCI_UART
 * module.
 *    @param[in]    pu8RxBuf        The buffer to receive the data of receive
 * buffer.
 *    @param[in]    u32ReadBytes    The read bytes number of data.
 *
 *    @return       Receive byte count
 *
 *    @details      The function is used to read Rx data from RX buffer and the
 * data will be stored in pu8RxBuf.
 */
                     ...
                     uint32_t u32ReadBytes )
{
    ...
    for( u32Count = 0ul; u32Count < u32ReadBytes; u32Count++ )
    {
        ...
        while( uuart->BUFSTS & UUART_BUFSTS_RXEMPTY_Msk ) /* Check RX empty =>
                                                             failed */
        {
            u32delayno++;
        ...
        pu8RxBuf[ u32Count ] = ( uint8_t ) uuart->RXDAT; /* Get Data from USCI
                                                            RX  */
    }
 ...
 *    @param[in]    uuart           The pointer of the specified USCI_UART
 * module.
 *    @param[in]    u32baudrate     The register value of baud rate of USCI_UART
 * module. If u32baudrate = 0, USCI_UART baud rate will not change.
 ...
 *    @param[in]    u32parity       The parity setting (none/odd/even) of
 * USCI_UART module.
 *                                  - \ref UUART_PARITY_NONE
 *                                  - \ref UUART_PARITY_ODD
 *                                  - \ref UUART_PARITY_EVEN
 *    @param[in]    u32stop_bits    The stop bit length (1/2 bit) of USCI_UART
 * module.
                               ...
                               uint32_t u32stop_bits )
{
    ...
    if( u32baudrate != 0ul )
    {
        ...
        else
        {
            ...
            for( u32DSCnt = 6ul; u32DSCnt <= 0x10ul;
                 u32DSCnt++ ) /* DSCNT could be 0x5~0xF */
            {
                u32ClkDiv = u32Div / u32DSCnt;
    ...
    {
        u32PDSCnt = ( ( uuart->BRGEN & UUART_BRGEN_PDSCNT_Msk ) >>
                      UUART_BRGEN_PDSCNT_Pos ) +
                    1ul;
        u32MinDSCnt = ( ( uuart->BRGEN & UUART_BRGEN_DSCNT_Msk ) >>
                        UUART_BRGEN_DSCNT_Pos ) +
                      1ul;
        u32MinClkDiv = ( ( uuart->BRGEN & UUART_BRGEN_CLKDIV_Msk ) >>
                         UUART_BRGEN_CLKDIV_Pos ) +
                       1ul;
    }

    /* Set USCI_UART line configuration */
    uuart->LINECTL = ( uuart->LINECTL & ~UUART_LINECTL_DWIDTH_Msk ) |
                     u32data_width;
    uuart->PROTCTL = ( uuart->PROTCTL & ~( UUART_PROTCTL_STICKEN_Msk |
                                           UUART_PROTCTL_EVENPARITY_Msk |
                                           UUART_PROTCTL_PARITYEN_Msk ) ) |
                     u32parity;
    uuart->PROTCTL = ( uuart->PROTCTL & ~UUART_PROTCTL_STOPB_Msk ) |
                     u32stop_bits;
 ...
 *    @param[in]    uuart           The pointer of the specified USCI_UART
 * module.
 *    @param[in]    pu8TxBuf        The buffer to send the data to USCI
 * transmission buffer.
 *    @param[out]   u32WriteBytes   The byte number of data.
 *
 *    @return       Transfer byte count
 *
 *    @details      The function is to write data into TX buffer to transmit
 * data by USCI_UART.
 */
                      ...
                      uint32_t u32WriteBytes )
{
    ...
    {
        u32delayno = 0ul;
        while( ( uuart->BUFSTS & UUART_BUFSTS_TXEMPTY_Msk ) == 0ul ) /* Wait Tx
                                                                        empty */
        {
            u32delayno++;
            if( u32delayno >= 0x40000000ul )
        ...
        uuart->TXDAT = ( uint8_t ) pu8TxBuf[ u32Count ]; /* Send USCI_UART Data
                                                            to buffer */
    }
 ...
 *                                   - \ref UUART_PROTCTL_DATWKEN_Msk    : Data
 * wake-up Mode
 *                                   - \ref UUART_PROTCTL_CTSWKEN_Msk    : nCTS
 * wake-up Mode
 *
 *    @return       None
 *
 *    @details      The function is used to enable Wake-up function of
 * USCI_UART.
 */
 ...
 *    @return       None
 *
 *    @details      The function is used to disable Wake-up function of
 * USCI_UART.
 */
...
void UUART_DisableFlowCtrl( UUART_T * uuart )
{
    ...
    uuart->PROTCTL &= ~( UUART_PROTCTL_RTSAUTOEN_Msk |
