 ...
 *                             - \ref CRC_16
 *                             - \ref CRC_32
 * @param[in]  u32Attribute    CRC operation data attribute. Valid values are
 * combined with:
 ...
 * @return     None
 *
 * @details    This function will enable the CRC controller by specify CRC
 * operation mode, attribute, initial seed and write data length. \n After that,
 * user can start to perform CRC calculate by calling CRC_WRITE_DATA macro or
 * CRC_DAT register directly.
 */
               ...
               uint32_t u32DataLen )
{
    ...
    /* Setting CHKSINIT bit will reload the initial seed value(CRC_SEED
     * register) to CRC controller */
 ...
 *
 * @details    This macro gets the CRC checksum result by current CRC polynomial
 * mode.
 */
