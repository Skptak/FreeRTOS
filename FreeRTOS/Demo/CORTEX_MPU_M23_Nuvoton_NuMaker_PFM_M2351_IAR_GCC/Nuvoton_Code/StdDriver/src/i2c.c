 ...
 *
 * @details    The function enable the specify I2C Controller and set proper
 * Clock Divider in I2C CLOCK DIVIDED REGISTER (I2CLK) according to the target
 * I2C Bus clock. I2C Bus clock = PCLK / (4*(divider+1).
 *
 */
uint32_t I2C_Open( I2C_T * i2c, uint32_t u32BusClock )
{
    ...
    u32Div = ( uint32_t ) ( ( ( u32Pclk * 10u ) / ( u32BusClock * 4u ) + 5u ) /
                                10u -
                            1u ); /* Compute proper divider for I2C clock */
    i2c->CLKDIV = u32Div;
 ...
 * @param[in]  u8GCMode     Enable/Disable GC mode (I2C_GCMODE_ENABLE /
 * I2C_GCMODE_DISABLE)
 *
 * @return     None
 *
 * @details    This function is used to set 7-bit slave addresses in I2C SLAVE
 * ADDRESS REGISTER (I2CADDR0~3) and enable GC Mode.
 *
 */
 ...
 *
 * @param[in]  i2c              Specify I2C port
 * @param[in]  u8SlaveNo        Set the number of I2C address mask register
 * (0~3)
 ...
 * @return     None
 *
 * @details    This function enable Time-out Counter function and configure DIV4
 * to support Long Time-out.
 *
 */
 ...
 * @return     None
 *
 * @details    The transmission or receive byte number in one transaction when
 * PECEN is set. The maximum is 255 bytes.
 *
 */
 ...
 *
 * @param[in]  i2c              Specify I2C port
 * @param[in]  u8HostDevice     Init SMBus port
 * mode(I2C_SMBH_ENABLE(1)/I2C_SMBD_ENABLE(0))
 *
 * @return     None
 *
 * @details    Using SMBus communication must specify the port is a Host or a
 * Device.
 *
 */
 ...
 * @return     None
 *
 * @details    Disable all SMBus function include Bus disable, CRC check,
 * Acknowledge by manual, Host/Device Mode.
 *
 */
 ...
 *
 * @param[in]  i2c              Specify I2C port
 * @param[in]  u8PECTxEn        CRC transmit enable(PECTX_ENABLE) or
 * disable(PECTX_DISABLE)
 *
 * @return     None
 *
 * @details    When enable CRC check function, the Host or Device needs to
 * transmit CRC byte.
 *
 */
 ...
 *
 * @details    The CRC check value after a transmission or a reception by count
 * by using CRC8
 *
 */
 ...
 * @return     None
 *
 * @details    This function is used to set SMBus Time-out length when bus is in
 * Idle state.
 *
 */
 ...
 *
 * @details    This function is used to set SMBus Time-out length when bus is in
 * active state. Time-out length is calculate the SCL line "one clock" pull low
 * timing.
 *
 */
...
void I2C_SMBusTimeout( I2C_T * i2c, uint32_t ms, uint32_t u32Pclk )
{
    ...
    else
    {
        /* DIV4 enabled */
        i2c->TOCTL |= I2C_TOCTL_TOCEN_Msk;
        i2c->BUSTOUT = ( ( ( ms * u32Pclk_kHz ) / ( 16U * 1024U * 4U ) ) -
                         1U ) &
                       0xFFU; /* The max value is 255 */
    }
}
 ...
 *
 * @details    This function is used to set SMBus Time-out length when bus is in
 * Active state. Time-out length is calculate the SCL line "clocks" low
 * cumulative timing.
 *
 */
...
void I2C_SMBusClockLoTimeout( I2C_T * i2c, uint32_t ms, uint32_t u32Pclk )
{
    ...
    else
    {
        /* DIV4 enabled */
        i2c->TOCTL |= I2C_TOCTL_TOCEN_Msk;
        i2c->CLKTOUT = ( ( ( ms * u32Pclk_kHz ) / ( 16U * 1024U * 4U ) ) -
                         1U ) &
                       0xFFU; /* The max value is 255 */
    }
}
...
uint8_t I2C_WriteByte( I2C_T * i2c, uint8_t u8SlaveAddr, uint8_t u8Data )
{
    ...
    while( u8Xfering && ( u8Err == 0u ) )
    {
        ...
        switch( I2C_GET_STATUS( i2c ) )
        {
            case 0x08u:
                I2C_SET_DATA( i2c,
                              ( uint8_t ) ( u8SlaveAddr << 1u |
                                            0x00u ) ); /* Write SLA+W to
                                                          Register I2CDAT */
        ...
        I2C_SET_CONTROL_REG( i2c, u8Ctrl ); /* Write controlbit to I2C_CTL
                                               register */
    }
 ...
 *
 * @details    The function is used for I2C Master write multi bytes data to
 * Slave.
 *
 */
    ...
    while( u8Xfering && ( u8Err == 0u ) )
    {
        ...
        switch( I2C_GET_STATUS( i2c ) )
        {
            ...
            case 0x28u:
                if( u32txLen < u32wLen )
                {
                    I2C_SET_DATA( i2c, au8Data[ u32txLen++ ] ); /* Write Data to
                                                                   I2CDAT */
                }
                else
                {
 ...
 * @retval     1               Write data fail, or bus occurs error events
 *
 * @details    The function is used for I2C Master specify a address that data
 * write to in Slave.
 *
 */
    ...
    while( u8Xfering && ( u8Err == 0u ) )
    {
        ...
        switch( I2C_GET_STATUS( i2c ) )
        {
            case 0x08u:
                I2C_SET_DATA( i2c,
                              ( uint8_t ) ( u8SlaveAddr << 1u |
                                            0x00u ) ); /* Send Slave address
                                                          with write bit */
                u8Ctrl = I2C_CTL_SI;                   /* Clear SI */
                break;
            case 0x18u:                          /* Slave Address ACK */
                I2C_SET_DATA( i2c, u8DataAddr ); /* Write Lo byte address of
                                                    register */
                break;
            case 0x20u:                  /* Slave Address NACK */
            case 0x30u:                  /* Master transmit data NACK */
 ...
 * @return     A length of how many bytes have been transmitted.
 *
 * @details    The function is used for I2C Master specify a byte address that
 * multi data bytes write to in Slave.
 *
 */
 ...
 * @retval     1               Write data fail, or bus occurs error events
 *
 * @details    The function is used for I2C Master specify two bytes address
 * that data write to in Slave.
 *
 */
 ...
 *
 * @details    The function is used for I2C Master to read a byte data from
 * Slave.
 *
 */
 ...
 *
 * @details    The function is used for I2C Master to read multi data bytes from
 * Slave.
 *
 *
 */
    ...
    while( u8Xfering && ( u8Err == 0u ) )
    {
        ...
        switch( I2C_GET_STATUS( i2c ) )
        {
            ...
            case 0x48u:                  /* Slave Address NACK */
                ...
                break;
            case 0x50u:
                au8Rdata[ u32rxLen++ ] = ( unsigned char ) I2C_GET_DATA(
                    i2c ); /* Receive Data */
 ...
 * @return     Read a byte data from Slave
 *
 * @details    The function is used for I2C Master specify a byte address that a
 ...
 *
 * @details    The function is used for I2C Master specify a byte address that
 * multi data bytes read from Slave.
 *
 *
 */
    ...
    while( u8Xfering && ( u8Err == 0u ) )
    {
        ...
        switch( I2C_GET_STATUS( i2c ) )
        {
            ...
            case 0x48u:                  /* Slave Address NACK */
                ...
                break;
            case 0x50u:
                au8Rdata[ u32rxLen++ ] = ( uint8_t ) I2C_GET_DATA(
                    i2c ); /* Receive Data */
                if( u32rxLen < ( u32rLen - 1u ) )
                {
                    u8Ctrl = I2C_CTL_SI_AA; /* Clear SI and set ACK */
