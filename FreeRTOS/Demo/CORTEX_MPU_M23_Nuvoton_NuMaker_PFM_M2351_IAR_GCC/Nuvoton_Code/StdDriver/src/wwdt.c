 ...
 *
 * @param[in]  u32PreScale     Pre-scale setting of WWDT counter. Valid values
 * are:
 ...
 * @param[in]  u32CmpValue     Setting the window compared value. Valid values
 * are between 0x0 to 0x3F.
 * @param[in]  u32EnableInt    Enable WWDT time-out interrupt function. Valid
 * values are TRUE and FALSE.
 *
 * @return     None
 *
 * @details    This function makes WWDT module start counting with different
 * counter period by pre-scale setting and compared window value.
...
{
    WWDT->CTL = u32PreScale | ( u32CmpValue << WWDT_CTL_CMPDAT_Pos ) |
                ( ( u32EnableInt == ( uint32_t ) TRUE ) ? WWDT_CTL_INTEN_Msk
                                                        : 0UL ) |
                WWDT_CTL_WWDTEN_Msk;
}
