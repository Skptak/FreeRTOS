 ...
 * @details Search the first free message interface, starting from 0. If a
 * interface is available, set a flag to lock the interface.
 */
static uint32_t LockIF( CAN_T * tCAN )
{
    uint32_t u32CanNo;
    ...
    u32IntMask = tCAN->CON &
                 ( CAN_CON_IE_Msk | CAN_CON_SIE_Msk | CAN_CON_EIE_Msk );
    tCAN->CON = tCAN->CON &
                ~( CAN_CON_IE_Msk | CAN_CON_SIE_Msk | CAN_CON_EIE_Msk );
...
/**
 * @brief Check if any interface is available in a time limitation then lock it
 * for usage.
 * @param[in] tCAN The pointer to CAN module base address.
 * @retval 0 IF0 is free
 * @retval 1 IF1 is free
 * @retval 2 No IF is free
 * @details Search the first free message interface, starting from 0. If no
 * interface is it will try again until time out. If a interface is available,
 * set a flag to lock the interface.
 */
static uint32_t LockIF_TL( CAN_T * tCAN )
{
    uint32_t u32Count;
 ...
 * @return None
 * @details This function is used to set CAN to enter initialization mode and
 * enable access bit timing register. After bit timing configuration ready, user
 * must call CAN_LeaveInitMode() to leave initialization mode and lock bit
 * timing register to let new configuration take effect.
 */
 ...
 * @param[in] tCAN The pointer to CAN module base address.
 * @return None
 * @details This function is used to wait message into message buffer in basic
 * mode. Please notice the function is polling NEWDAT bit of MCON register by
 * while loop and it is used in basic mode.
 */
 ...
 * @details Return current CAN bit rate according to the user bit-timing
 * parameter settings
 */
uint32_t CAN_GetCANBitRate( CAN_T * tCAN )
{
    ...
    u8Tseg1 = ( uint8_t ) ( ( tCAN->BTIME & CAN_BTIME_TSEG1_Msk ) >>
                            CAN_BTIME_TSEG1_Pos );
    u8Tseg2 = ( uint8_t ) ( ( tCAN->BTIME & CAN_BTIME_TSEG2_Msk ) >>
                            CAN_BTIME_TSEG2_Pos );
    u32Bpr = ( tCAN->BTIME & CAN_BTIME_BRP_Msk );
    u32Bpr |= ( tCAN->BRPE << 6 );

    return ( SystemCoreClock / ( u32Bpr + 1U ) /
             ( ( uint32_t ) u8Tseg1 + ( uint32_t ) u8Tseg2 + 3U ) );
}
 ...
 *                       \ref CAN_TEST_SILENT_Msk Enable silent mode of test
 * mode \ref CAN_TEST_LBACK_Msk Enable Loop Back Mode of test mode \ref
 * CAN_TEST_TX0_Msk / \ref CAN_TEST_TX1_Msk Control CAN_TX pin bit field
 * @return None
 * @details Switch the CAN into test mode. There are four test mode
 * (BASIC/SILENT/LOOPBACK/ LOOPBACK combined SILENT/CONTROL_TX_PIN)could be
 * selected. After setting test mode,user must call CAN_LeaveInitMode() to let
 ...
 * @param[in] tCAN The pointer to CAN module base address.
 * @return   None
 * @details  This function is used to Leave the test mode (switch into normal
 * mode).
 */
void CAN_LeaveTestMode( CAN_T * tCAN )
{
    tCAN->CON |= CAN_CON_TEST_Msk;
    tCAN->TEST &= ~( CAN_TEST_LBACK_Msk | CAN_TEST_SILENT_Msk |
                     CAN_TEST_BASIC_Msk );
    tCAN->CON &= ( ~CAN_CON_TEST_Msk );
}
 ...
 * @details This function is used to get the waiting status of a received
 * message.
 */
 ...
 * @param[in] tCAN The pointer to CAN module base address.
 * @param[in] pCanMsg Pointer to the message structure containing data to
 * transmit.
 * @return TRUE:  Transmission OK
 *         FALSE: Check busy flag of interface 0 is timeout
 * @details The function is used to send CAN message in BASIC mode of test mode.
 * Before call the API, the user should be call
 * CAN_EnterTestMode(CAN_TEST_BASIC) and let CAN controller enter basic mode of
 * test mode. Please notice IF1 Registers used as Tx Buffer in basic mode.
 */
int32_t CAN_BasicSendMsg( CAN_T * tCAN, STR_CANMSG_T * pCanMsg )
{
    ...
    else
    {
        ...
        tCAN->IF[ 0 ].ARB2 = ( ( ( pCanMsg->Id ) & 0x1FFF0000UL ) >> 16 ) |
                             CAN_IF_ARB2_XTD_Msk;
    }
    ...
    tCAN->IF[ 0 ].MCON = ( tCAN->IF[ 0 ].MCON & ( ~CAN_IF_MCON_DLC_Msk ) ) |
                         pCanMsg->DLC;
    tCAN->IF[ 0 ].DAT_A1 = ( ( uint32_t ) pCanMsg->Data[ 1 ] << 8 ) |
                           pCanMsg->Data[ 0 ];
    tCAN->IF[ 0 ].DAT_A2 = ( ( uint32_t ) pCanMsg->Data[ 3 ] << 8 ) |
                           pCanMsg->Data[ 2 ];
    tCAN->IF[ 0 ].DAT_B1 = ( ( uint32_t ) pCanMsg->Data[ 5 ] << 8 ) |
                           pCanMsg->Data[ 4 ];
    tCAN->IF[ 0 ].DAT_B2 = ( ( uint32_t ) pCanMsg->Data[ 7 ] << 8 ) |
                           pCanMsg->Data[ 6 ];
 ...
 *
 * @param[in] tCAN The pointer to CAN module base address.
 * @param[in] pCanMsg Pointer to the message structure where received data is
 * copied.
 *
...
int32_t CAN_BasicReceiveMsg( CAN_T * tCAN, STR_CANMSG_T * pCanMsg )
{
    if( ( tCAN->IF[ 1 ].MCON & CAN_IF_MCON_NEWDAT_Msk ) ==
        0UL ) /* In basic mode, receive data always save in IF2 */
    {
        return ( int32_t ) FALSE;
    }
    ...
    tCAN->IF[ 1 ].CMASK = CAN_IF_CMASK_ARB_Msk | CAN_IF_CMASK_CONTROL_Msk |
    ...
    /*
    pCanMsg->FrameType = (uint32_t)(!(uint32_t)((tCAN->IF[1].ARB2 &
    (uint32_t)CAN_IF_ARB2_DIR_Msk) >> (uint32_t)CAN_IF_ARB2_DIR_Pos));
    */

    pCanMsg->FrameType = ( tCAN->IF[ 1 ].ARB2 & CAN_IF_ARB2_DIR_Msk ) ? 0UL
                                                                      : 1UL;

    pCanMsg->DLC = ( uint8_t ) ( tCAN->IF[ 1 ].MCON & CAN_IF_MCON_DLC_Msk );
    pCanMsg->Data[ 0 ] = ( uint8_t ) ( tCAN->IF[ 1 ].DAT_A1 &
                                       CAN_IF_DAT_A1_DATA0_Msk );
    pCanMsg->Data[ 1 ] = ( uint8_t ) ( ( tCAN->IF[ 1 ].DAT_A1 &
                                         CAN_IF_DAT_A1_DATA1_Msk ) >>
                                       CAN_IF_DAT_A1_DATA1_Pos );
    pCanMsg->Data[ 2 ] = ( uint8_t ) ( tCAN->IF[ 1 ].DAT_A2 &
                                       CAN_IF_DAT_A2_DATA2_Msk );
    pCanMsg->Data[ 3 ] = ( uint8_t ) ( ( tCAN->IF[ 1 ].DAT_A2 &
                                         CAN_IF_DAT_A2_DATA3_Msk ) >>
                                       CAN_IF_DAT_A2_DATA3_Pos );
    pCanMsg->Data[ 4 ] = ( uint8_t ) ( tCAN->IF[ 1 ].DAT_B1 &
                                       CAN_IF_DAT_B1_DATA4_Msk );
    pCanMsg->Data[ 5 ] = ( uint8_t ) ( ( tCAN->IF[ 1 ].DAT_B1 &
                                         CAN_IF_DAT_B1_DATA5_Msk ) >>
                                       CAN_IF_DAT_B1_DATA5_Pos );
    pCanMsg->Data[ 6 ] = ( uint8_t ) ( tCAN->IF[ 1 ].DAT_B2 &
                                       CAN_IF_DAT_B2_DATA6_Msk );
    pCanMsg->Data[ 7 ] = ( uint8_t ) ( ( tCAN->IF[ 1 ].DAT_B2 &
 ...
 * @param[in] u8MsgObj Specifies the Message object number, from 0 to 31.
 * @param[in] u8idType Specifies the identifier type of the frames that will be
 * transmitted This parameter can be one of the following values: \ref
 * CAN_STD_ID (standard ID, 11-bit) \ref CAN_EXT_ID (extended ID, 29-bit)
 ...
 * values: TRUE: for a single receive object or a FIFO receive object that is
 * the last one of the FIFO. FALSE: for a FIFO receive object that is not the
 * last one.
 * @retval TRUE SUCCESS
                               ...
                               uint8_t u8singleOrFifoLast )
{
    ...
    /* Command Setting */
    tCAN->IF[ u8MsgIfNum ].CMASK = CAN_IF_CMASK_WRRD_Msk |
    ...
    if( u8idType == CAN_STD_ID ) /* According STD/EXT ID format,Configure Mask
                                    and Arbitration register */
    {
        tCAN->IF[ u8MsgIfNum ].ARB1 = 0U;
        tCAN->IF[ u8MsgIfNum ].ARB2 = CAN_IF_ARB2_MSGVAL_Msk |
                                      ( u32id & 0x7FFUL ) << 2;
    }
    else
    {
 ...
 *                      TRUE: the message object is released when getting the
 * data. FALSE:the message object is not released.
 * @param[in] pCanMsg Pointer to the message structure where received data is
 * copied.
 * @retval TRUE Success
                        ...
                        STR_CANMSG_T * pCanMsg )
{
    ...
    tCAN->IF[ u8MsgIfNum ].CMASK = CAN_IF_CMASK_MASK_Msk |
                                   ...
                                   ( u8Release ? CAN_IF_CMASK_TXRQSTNEWDAT_Msk
                                               : 0UL ) |
                                   CAN_IF_CMASK_DATAA_Msk |
                                   CAN_IF_CMASK_DATAB_Msk;
    ...
    pCanMsg->DLC = ( uint8_t ) ( tCAN->IF[ u8MsgIfNum ].MCON &
                                 CAN_IF_MCON_DLC_Msk );
    pCanMsg->Data[ 0 ] = ( uint8_t ) ( tCAN->IF[ u8MsgIfNum ].DAT_A1 &
                                       CAN_IF_DAT_A1_DATA0_Msk );
    pCanMsg->Data[ 1 ] = ( uint8_t ) ( ( tCAN->IF[ u8MsgIfNum ].DAT_A1 &
                                         CAN_IF_DAT_A1_DATA1_Msk ) >>
                                       CAN_IF_DAT_A1_DATA1_Pos );
    pCanMsg->Data[ 2 ] = ( uint8_t ) ( tCAN->IF[ u8MsgIfNum ].DAT_A2 &
                                       CAN_IF_DAT_A2_DATA2_Msk );
    pCanMsg->Data[ 3 ] = ( uint8_t ) ( ( tCAN->IF[ u8MsgIfNum ].DAT_A2 &
                                         CAN_IF_DAT_A2_DATA3_Msk ) >>
                                       CAN_IF_DAT_A2_DATA3_Pos );
    pCanMsg->Data[ 4 ] = ( uint8_t ) ( tCAN->IF[ u8MsgIfNum ].DAT_B1 &
                                       CAN_IF_DAT_B1_DATA4_Msk );
    pCanMsg->Data[ 5 ] = ( uint8_t ) ( ( tCAN->IF[ u8MsgIfNum ].DAT_B1 &
                                         CAN_IF_DAT_B1_DATA5_Msk ) >>
                                       CAN_IF_DAT_B1_DATA5_Pos );
    pCanMsg->Data[ 6 ] = ( uint8_t ) ( tCAN->IF[ u8MsgIfNum ].DAT_B2 &
                                       CAN_IF_DAT_B2_DATA6_Msk );
    pCanMsg->Data[ 7 ] = ( uint8_t ) ( ( tCAN->IF[ u8MsgIfNum ].DAT_B2 &
                                         CAN_IF_DAT_B2_DATA7_Msk ) >>
                                       CAN_IF_DAT_B2_DATA7_Pos );
 ...
 *
 * @param[in] tCAN The pointer to CAN module base address.
 * @param[in] u32BaudRate The target CAN baud-rate. The range of u32BaudRate is
 * 1~1000KHz.
 *
 * @return u32CurrentBitRate  Real baud-rate value.
 *
 * @details The function is used to set bus timing parameter according current
...
uint32_t CAN_SetBaudRate( CAN_T * tCAN, uint32_t u32BaudRate )
{
    ...
    for( tseg = ( TSEG1_MAX + TSEG2_MAX ) * 2 + 1;
         tseg >= ( TSEG1_MIN + TSEG2_MIN ) * 2;
         tseg-- )
    {
        tsegall = 1 + tseg / 2;
         ...
         */
        brp = ( int32_t ) ( clock_freq / ( ( int64_t ) tsegall *
                                           ( int32_t ) u32BaudRate ) ) +
              ( tseg % 2 );
    ...
    /* real bit-rate */
    u32BaudRate = ( uint32_t ) ( int32_t ) ( clock_freq /
                                             ( int32_t ) ( best_brp *
                                                           ( tseg1 + tseg2 +
                                                             1 ) ) );

    tCAN->BTIME = ( ( ( uint32_t ) tseg2 - 1UL ) << CAN_BTIME_TSEG2_Pos ) |
                  ( ( ( uint32_t ) tseg1 - 1UL ) << CAN_BTIME_TSEG1_Pos ) |
                  ( ( ( uint32_t ) best_brp - 1UL ) & CAN_BTIME_BRP_Msk ) |
                  ( sjw << CAN_BTIME_SJW_Pos );
 ...
 * @return None
 *
 * @details No Status Change Interrupt and Error Status Interrupt will be
 * generated.
 */
 ...
 * @return u32CurrentBitRate  Real baud-rate value.
 *
 * @details Set bus timing parameter according current clock and target
 * baud-rate. In Basic mode, IF1 Registers used as Tx Buffer, IF2 Registers used
 * as Rx Buffer.
 */
 ...
 *
 * @details The two sets of interface registers (IF1 and IF2) control the
 * software access to the Message RAM. They buffer the data to be transferred to
 * and from the RAM, avoiding conflicts between software accesses and message
...
int32_t CAN_SetTxMsg( CAN_T * tCAN, uint32_t u32MsgNum, STR_CANMSG_T * pCanMsg )
{
    ...
    if( pCanMsg->IdType == CAN_STD_ID )
    {
        ...
        tCAN->IF[ u8MsgIfNum ].ARB2 = ( ( ( pCanMsg->Id ) & 0x7FFUL ) << 2 ) |
                                      CAN_IF_ARB2_DIR_Msk |
    ...
    else
    {
        ...
        tCAN->IF[ u8MsgIfNum ].ARB2 = ( ( ( pCanMsg->Id ) & 0x1FFF0000UL ) >>
                                        16 ) |
                                      CAN_IF_ARB2_DIR_Msk |
                                      CAN_IF_ARB2_XTD_Msk |
                                      CAN_IF_ARB2_MSGVAL_Msk;
    ...
    tCAN->IF[ u8MsgIfNum ].DAT_A1 = ( ( uint32_t ) pCanMsg->Data[ 1 ] << 8 ) |
                                    ( uint32_t ) pCanMsg->Data[ 0 ];
    tCAN->IF[ u8MsgIfNum ].DAT_A2 = ( ( uint32_t ) pCanMsg->Data[ 3 ] << 8 ) |
                                    ( uint32_t ) pCanMsg->Data[ 2 ];
    tCAN->IF[ u8MsgIfNum ].DAT_B1 = ( ( uint32_t ) pCanMsg->Data[ 5 ] << 8 ) |
                                    pCanMsg->Data[ 4 ];
    tCAN->IF[ u8MsgIfNum ].DAT_B2 = ( ( uint32_t ) pCanMsg->Data[ 7 ] << 8 ) |
                                    pCanMsg->Data[ 6 ];

    tCAN->IF[ u8MsgIfNum ].MCON = CAN_IF_MCON_NEWDAT_Msk | pCanMsg->DLC |
                                  CAN_IF_MCON_TXIE_Msk | CAN_IF_MCON_EOB_Msk;
 ...
 *
 * @details If a transmission is requested by programming bit TxRqst/NewDat
 * (IFn_CMASK[2]), the TxRqst (IFn_MCON[8]) will be ignored.
 */
 ...
 * @return None
 *
 * @details The application software has two possibilities to follow the source
 * of a message interrupt. First, it can follow the IntId in the Interrupt
 * Register and second it can poll the Interrupt Pending Register.
 */
 ...
 *
 * @param[in] tCAN The pointer to CAN module base address.
 * @param[in] u32Mask Interrupt Mask. (CAN_CON_IE_Msk / CAN_CON_SIE_Msk /
 * CAN_CON_EIE_Msk).
 *
 * @return None
 *
 * @details The interrupt remains active until the Interrupt Register is back to
 * value zero (the cause of the interrupt is reset) or until IE is reset.
 */
void CAN_DisableInt( CAN_T * tCAN, uint32_t u32Mask )
{
    tCAN->CON = tCAN->CON & ~( ( u32Mask & ( CAN_CON_IE_Msk | CAN_CON_SIE_Msk |
                                             CAN_CON_EIE_Msk ) ) );
}
 ...
 * @param[in] tCAN The pointer to CAN module base address.
 * @param[in] u32MsgNum Specifies the Message object number, from 0 to 31.
 * @param[in] u32IDType Specifies the identifier type of the frames that will be
 * transmitted. Valid values are:
 ...
 *
 * @details If the RxIE bit (CAN_IFn_MCON[10]) is set, the IntPnd bit
 * (CAN_IFn_MCON[13]) will be set when a received Data Frame is accepted and
                      ...
                      uint32_t u32ID )
{
    ...
    while( CAN_SetRxMsgObj( tCAN,
                            ( uint8_t ) u32MsgNum,
                            ( uint8_t ) u32IDType,
                            u32ID,
                            ( uint8_t ) TRUE ) == ( int32_t ) FALSE )
    {
 ...
 *                      - \ref CAN_EXT_ID The 29-bit identifier.
 * @param[in] u32ID Specifies the identifier used for acceptance filtering.
 * @param[in] u32IDMask Specifies the identifier mask used for acceptance
 * filtering.
 *
 * @retval FALSE No useful interface.
 * @retval TRUE Configure a receive message object success.
                            ...
                            uint32_t u32IDMask )
{
    ...
    while( CAN_SetRxMsgObjAndMsk( tCAN,
                                  ( uint8_t ) u32MsgNum,
                                  ( uint8_t ) u32IDType,
                                  u32ID,
                                  u32IDMask,
 ...
 *
 * @details The Interface Registers avoid conflict between the CPU accesses to
 * the Message RAM and CAN message reception and transmission by buffering the
                           ...
                           uint32_t u32ID )
{
    ...
    for( i = 1UL; i < u32MsgCount; i++ )
    {
        ...
        while( CAN_SetRxMsgObj( tCAN,
                                ...
                                ( uint8_t ) u32IDType,
                                u32ID,
                                ( uint8_t ) u32EOB_Flag ) == ( int32_t ) FALSE )
        {
            if( ++u32TimeOutCount >= RETRY_COUNTS )
            {
                return ( int32_t ) FALSE;
 ...
 *
 * @details The receive/transmit priority for the Message Objects is attached to
 * the message number. Message Object 1 has the highest priority, while Message
 ...
 * @return None
 *
 * @details An interrupt remains pending until the application software has
 * cleared it.
 */
