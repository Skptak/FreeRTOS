 ...
 *
 * @details    This function use to enable smartcard module UART mode and set
 * baudrate.
 *
 * @note       This function configures character width to 8 bits, 1 stop bit,
 * and no parity. And can use \ref SCUART_SetLineConfig function to update these
 * settings. The baudrate clock source comes from SC_CLK/SC_DIV, where SC_CLK is
 * controlled by SCxSEL in CLKSEL3 register, SC_DIV is controlled by SCxDIV in
 * CLKDIV1 register. Since the baudrate divider is 12-bit wide and must be
 * larger than 4, (clock source / baudrate) must be larger or equal to 5 and
 * smaller or equal to 4096. Otherwise this function cannot configure SCUART to
 * work with target baudrate.
 */
uint32_t SCUART_Open( SC_T * sc, uint32_t u32Baudrate )
{
    ...
    sc->CTL = SC_CTL_SCEN_Msk |
              SC_CTL_NSB_Msk; /* Enable smartcard interface and stop bit = 1 */
    sc->UARTCTL = SCUART_CHAR_LEN_8 | SCUART_PARITY_NONE |
                  SC_UARTCTL_UARTEN_Msk; /* Enable UART mode, disable parity and
                                            8 bit per character */
    sc->ETUCTL = u32Div;
 ...
 *
 * @note       This function does not block and return immediately if there's no
 * data available.
 */
uint32_t SCUART_Read( SC_T * sc, uint8_t pu8RxBuf[], uint32_t u32ReadBytes )
{
    ...
    for( u32Count = 0UL; u32Count < u32ReadBytes; u32Count++ )
    {
        ...
        pu8RxBuf[ u32Count ] = ( uint8_t ) SCUART_READ( sc ); /* get data from
                                                                 FIFO */
    }
 ...
 *
 * @param[in]  sc              The pointer of smartcard module.
 * @param[in]  u32Baudrate     Target baudrate of smartcard UART mode. If this
 * value is 0, SC UART baudrate will not change.
 ...
 * @return     Actual baudrate of smartcard UART mode
 *
 * @details    The baudrate clock source comes from SC_CLK/SC_DIV, where SC_CLK
 * is controlled by SCxSEL in CLKSEL3 register, SC_DIV is controlled by SCxDIV
 * in CLKDIV1 register. Since the baudrate divider is 12-bit wide and must be
 * larger than 4, (clock source / baudrate) must be larger or equal to 5 and
 * smaller or equal to 4096. Otherwise this function cannot configure SCUART to
 * work with target baudrate.
 */
                               ...
                               uint32_t u32StopBits )
{
    ...
    {
        /* Calculate divider for target baudrate */
        u32Div = ( ( u32Clk + ( u32Baudrate >> 1 ) - 1UL ) / u32Baudrate ) -
                 1UL;
        sc->ETUCTL = u32Div;
    }

    sc->CTL = u32StopBits | SC_CTL_SCEN_Msk; /* Set stop bit */
    sc->UARTCTL = u32Parity | u32DataWidth |
                  SC_UARTCTL_UARTEN_Msk; /* Set character width and parity */
 ...
 *
 * @param[in]  sc      The pointer of smartcard module.
 * @param[in]  u32TOC  Rx time-out counter, using baudrate as counter unit.
 * Valid range are 0~0x1FF, set this value to 0 will disable time-out counter.
 *
 * @return     None
 *
 * @details    The time-out counter resets and starts counting whenever the Rx
 * buffer received a new data word. Once the counter decrease to 1 and no new
 * data is received or CPU does not read any data from FIFO, a receiver time-out
 ...
 *
 * @param[in]  sc              The pointer of smartcard module.
 * @param[in]  pu8TxBuf        The buffer containing data to send to transmit
 * FIFO.
 * @param[in]  u32WriteBytes   Number of data to send.
 *
 * @return     None
 *
 * @details    This function is used to write data into Tx FIFO to send data
 * out.
 *
