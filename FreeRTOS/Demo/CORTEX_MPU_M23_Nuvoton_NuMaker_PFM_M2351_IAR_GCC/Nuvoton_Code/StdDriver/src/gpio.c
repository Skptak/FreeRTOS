 ...
 *
 * @param[in]   port        GPIO port. It could be PA, PB, PC, PD, PE, PF, PG or
 * PH.
 * @param[in]   u32PinMask  The single or multiple pins of specified GPIO port.
 * \n It could be BIT0 ~ BIT15 for PA, PB and PE. \n It could be BIT0 ~ BIT13
 * for PC. \n It could be BIT0 ~ BIT14 for PD. \n It could be BIT0 ~ BIT11 for
...
void GPIO_SetMode( GPIO_T * port, uint32_t u32PinMask, uint32_t u32Mode )
{
    ...
    for( u32Idx = 0ul; u32Idx < GPIO_PIN_MAX; u32Idx++ )
    {
        ...
        {
            port->MODE = ( port->MODE & ~( 0x3ul << ( u32Idx << 1 ) ) ) |
                         ( u32Mode << ( u32Idx << 1 ) );
        }
    }
}
 ...
 * @param[in]   u32IntAttribs   The interrupt attribute of specified GPIO pin.
 * It could be
 *                              - \ref GPIO_INT_RISING
...
void GPIO_EnableInt( GPIO_T * port, uint32_t u32Pin, uint32_t u32IntAttribs )
{
    ...
    port->INTTYPE = ( port->INTTYPE & ~( 1ul << u32Pin ) ) |
                    ( ( ( u32IntAttribs >> 24 ) & 0xFFUL ) << u32Pin );

    /* Enable interrupt function of specified pin */
    port->INTEN = ( port->INTEN & ~( 0x00010001ul << u32Pin ) ) |
                  ( ( u32IntAttribs & 0xFFFFFFUL ) << u32Pin );
}
...
void GPIO_SetSlewCtl( GPIO_T * port, uint32_t u32PinMask, uint32_t u32Mode )
{
    ...
    for( u32Idx = 0ul; u32Idx < GPIO_PIN_MAX; u32Idx++ )
    {
        ...
        {
            port->SLEWCTL = ( port->SLEWCTL & ~( 0x3ul << ( u32Idx << 1 ) ) ) |
                            ( u32Mode << ( u32Idx << 1 ) );
        }
    }
}
...
void GPIO_SetPullCtl( GPIO_T * port, uint32_t u32PinMask, uint32_t u32Mode )
{
    ...
    for( u32Idx = 0ul; u32Idx < GPIO_PIN_MAX; u32Idx++ )
    {
        if( u32PinMask & ( 1ul << u32Idx ) )
        {
            port->PUSEL = ( port->PUSEL & ~( 0x3ul << ( u32Idx << 1 ) ) ) |
                          ( u32Mode << ( u32Idx << 1 ) );
        }
    }
}
