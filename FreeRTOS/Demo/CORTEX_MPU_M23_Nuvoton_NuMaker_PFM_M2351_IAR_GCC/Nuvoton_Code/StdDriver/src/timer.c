 ...
 *
 * @details    This API is used to configure timer to operate in specified mode
 * and frequency. If timer cannot work in target frequency, a closest frequency
 * will be chose and returned.
 * @note       After calling this API, Timer is \b NOT running yet. But could
 * start timer running be calling \ref TIMER_Start macro or program registers
 * directly.
 */
uint32_t TIMER_Open( TIMER_T * timer, uint32_t u32Mode, uint32_t u32Freq )
{
    ...
    /* Fastest possible timer working freq is (u32Clk / 2). While cmpr = 2,
     * prescaler = 0. */
 ...
 * @return     None
 *
 * @details    This API stops timer counting and disable all timer interrupt
 * function.
 */
 ...
 * @param[in]  timer       The pointer of the specified Timer module. It could
 * be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32Usec     Delay period in micro seconds. Valid values are
 * between 100~1000000 (100 micro second ~ 1 second).
 *
 * @return     None
 *
 * @details    This API is used to create a delay loop for u32usec micro seconds
 * by using timer one-shot mode.
 * @note       This API overwrites the register setting of the timer used to
 * count the delay time.
 * @note       This API use polling mode. So there is no need to enable
 * interrupt for the timer module used to generate delay.
 */
void TIMER_Delay( TIMER_T * timer, uint32_t u32Usec )
{
    ...
    if( u32Clk <= 1000000UL ) /* min delay is 1000 us if timer clock source is
                                 <= 1 MHz */
    {
        if( u32Usec < 1000UL )
        {
    ...
    /*
        When system clock is faster than timer clock, it is possible timer
       active bit cannot set in time while we check it. And the while loop below
       return immediately, so put a tiny delay here allowing timer start
 ...
 * capture trigger edge \n to get current counter value or reset counter value
 * to 0.
 * @note       Timer frequency should be configured separately by using \ref
 * TIMER_Open API, or program registers directly.
 */
 ...
 * @return     None
 *
 * @details    This function is used to enable the timer counter function with
 * specify detection edge.
 * @note       Timer compare value should be configured separately by using \ref
 * TIMER_SET_CMP_VALUE macro or program registers directly.
 * @note       While using event counter function, \ref TIMER_TOGGLE_MODE cannot
 * set as timer operation mode.
 */
 ...
 * @note       This API cannot return correct clock rate if timer source is from
 * external clock input.
 */
uint32_t TIMER_GetModuleClock( TIMER_T * timer )
{
    uint32_t u32Src, u32Clk = __HIRC;
    const uint32_t au32Clk[] = { __HXT, __LXT,  0UL, 0UL,
                                 0UL,   __LIRC, 0UL, __HIRC };
 ...
 * @param[in]  u32EnableInt    Enable interrupt assertion after capture complete
 * or not. Valid values are TRUE and FALSE
 *
 * @return     None
 *
 * @details    This function is used to calculate input event frequency. After
 * enable this function, a pair of timers, TIMER0 and TIMER1, or TIMER2 and
 * TIMER3 will be configured for this function. The mode used to calculate input
 *             event frequency is mentioned as "Inter Timer Trigger Mode" in
 ...
 * @return     None
 *
 * @brief      This function is used to disable the Timer frequency counter
 * function.
 */
 ...
 * @param[in]  timer   The pointer of the specified Timer module. It could be
 * TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32Src  Selects the interrupt source to trigger other modules.
 * Could be:
 ...
 * @return     None
 *
 * @brief      This function is used to select the interrupt source used to
 ...
 * @param[in]  timer   The pointer of the specified Timer module. It could be
 * TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32Mask The mask of modules (EPWM, EADC, DAC and PDMA) trigger by
 * timer. Is the combination of
 ...
 * @return     None
 *
 * @details    This function is used to set EPWM, EADC, DAC and PDMA module
 * triggered by timer interrupt event.
 */
void TIMER_SetTriggerTarget( TIMER_T * timer, uint32_t u32Mask )
{
    timer->TRGCTL = ( timer->TRGCTL &
                      ~( TIMER_TRGCTL_TRGEPWM_Msk | TIMER_TRGCTL_TRGDAC_Msk |
