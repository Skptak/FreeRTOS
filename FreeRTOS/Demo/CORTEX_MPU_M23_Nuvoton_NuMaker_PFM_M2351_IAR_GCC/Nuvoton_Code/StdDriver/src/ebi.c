 ...
 * @param[in]  u32TimingClass      Default timing configuration. Valid values
 * are:
 ...
 * @return     None
 *
 * @details    This function is used to open specify EBI bank with different bus
 * width, timing setting and \n active level of CS pin to access EBI device.
 * @note       Write Buffer Enable(WBUFEN) and Extend Time Of ALE(TALE) are only
 * available in EBI bank0 control register.
 */
               ...
               uint32_t u32CSActiveLevel )
{
    ...
    uint32_t u32Index1 = ( uint32_t ) &EBI->TCTL0 +
                         ( uint32_t ) u32Bank * 0x10U;
    ...
    *pu32EBICTL |= u32BusMode;
    ...
    switch( u32TimingClass )
    {
        case EBI_TIMING_FASTEST:
            *pu32EBICTL = ( *pu32EBICTL &
                            ~( EBI_CTL_MCLKDIV_Msk | EBI_CTL_TALE_Msk ) ) |
                          ( EBI_MCLKDIV_1 << EBI_CTL_MCLKDIV_Pos ) |
                          ( u32CSActiveLevel << EBI_CTL_CSPOLINV_Pos ) |
                          EBI_CTL_EN_Msk;
            *pu32EBITCTL = 0x0U;
            break;
        ...
        case EBI_TIMING_VERYFAST:
            *pu32EBICTL = ( *pu32EBICTL &
                          ...
                          ( EBI_MCLKDIV_1 << EBI_CTL_MCLKDIV_Pos ) |
                          ( u32CSActiveLevel << EBI_CTL_CSPOLINV_Pos ) |
                          EBI_CTL_EN_Msk | ( 0x3U << EBI_CTL_TALE_Pos );
        ...
        case EBI_TIMING_SLOW:
            *pu32EBICTL = ( *pu32EBICTL &
                          ...
                          ( EBI_MCLKDIV_2 << EBI_CTL_MCLKDIV_Pos ) |
                          ( u32CSActiveLevel << EBI_CTL_CSPOLINV_Pos ) |
                          EBI_CTL_EN_Msk | ( 0x7U << EBI_CTL_TALE_Pos );
 ...
 *                                     - \ref EBI_BANK1
 *                                     - \ref EBI_BANK2
 * @param[in]  u32TimingConfig     Configure EBI timing settings, includes TACC,
 * TAHD, W2X and R2R setting.
 ...
 * @return     None
 *
 * @details    This function is used to configure specify EBI bus timing for
 * access EBI device.
 */
                       ...
                       uint32_t u32MclkDiv )
{
    ...
    *pu32EBICTL = ( *pu32EBICTL & ~EBI_CTL_MCLKDIV_Msk ) |
                  ( u32MclkDiv << EBI_CTL_MCLKDIV_Pos );
