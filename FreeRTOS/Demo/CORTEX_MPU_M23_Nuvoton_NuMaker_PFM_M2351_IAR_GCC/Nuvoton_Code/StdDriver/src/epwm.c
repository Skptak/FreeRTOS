 ...
 * @param[in] u32CaptureEdge The condition to latch the counter. This parameter
 * is not used
 * @return The nearest unit time in nano second.
 * @details This function is used to Configure EPWM capture and get the nearest
 * unit time.
 */
                                    ...
                                    uint32_t u32CaptureEdge )
{
    ...
    for( u32Prescale = 1U; u32Prescale <= 0x1000U; u32Prescale++ )
    {
        ...
        {
            if( u32Prescale == 0x1000U ) /* limit to the maximum unit time(nano
                                            second) */
            {
                u8BreakLoop = 1U;
            }
            if( !( ( 1000000UL * ( u32Prescale + 1UL ) >
                     ( u32NearestUnitTimeNsec * u32PWMClockSrc ) ) ) )
            {
                u8BreakLoop = 1U;
            }
        }
    ...
    ( epwm )->CTL1 = ( ( epwm )->CTL1 &
                       ~( EPWM_CTL1_CNTTYPE0_Msk << ( u32ChannelNum << 1 ) ) ) |
                     ( 1UL << ( u32ChannelNum << 1 ) );
...
/**
 * @brief This function Configure EPWM generator and get the nearest frequency
 * in edge aligned(up counter type) auto-reload mode
 * @param[in] epwm The pointer of the specified EPWM module
 *                - EPWM0 : EPWM Group 0
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32Frequency Target generator frequency
 * @param[in] u32DutyCycle Target generator duty cycle percentage. Valid range
 * are between 0 ~ 100. 10 means 10%, 20 means 20%...
 * @return Nearest frequency clock in nano second
 * @note Since every two channels, (0 & 1), (2 & 3), shares a prescaler. Call
 * this API to configure EPWM frequency may affect existing frequency of other
 * channel.
 * @note This function is used for initial stage.
 *       To change duty cycle later, it should get the configured period value
 * and calculate the new comparator value.
 */
                                   ...
                                   uint32_t u32DutyCycle )
{
    ...
    /* Store return value here 'cos we're gonna change u32Prescale & u32CNR to
     * the real value to fill into register */
    ...
    ( epwm )->CTL1 = ( ( epwm )->CTL1 &
                       ~( ( EPWM_CTL1_CNTTYPE0_Msk << ( u32ChannelNum << 1 ) ) |
                          ( EPWM_CTL1_CNTMODE0_Msk << u32ChannelNum ) ) );

    u32CNR = u32CNR - 1U;
    EPWM_SET_CNR( epwm, u32ChannelNum, u32CNR );
    EPWM_SET_CMR( epwm,
                  u32ChannelNum,
                  u32DutyCycle * ( u32CNR + 1UL ) / 100UL );

    ( epwm )->WGCTL0 = ( ( epwm )->WGCTL0 &
                         ~( ( EPWM_WGCTL0_PRDPCTL0_Msk | EPWM_WGCTL0_ZPCTL0_Msk )
                            << ( u32ChannelNum << 1 ) ) ) |
                       ( EPWM_OUTPUT_HIGH
                         << ( u32ChannelNum << 1UL
    ...
    ( epwm )
        ->WGCTL1 = ( ( epwm )->WGCTL1 &
                     ~( ( EPWM_WGCTL1_CMPDCTL0_Msk | EPWM_WGCTL1_CMPUCTL0_Msk )
                        << ( u32ChannelNum << 1 ) ) ) |
                   ( EPWM_OUTPUT_LOW
                     << ( u32ChannelNum << 1UL << EPWM_WGCTL1_CMPUCTL0_Pos ) );
 ...
 * corresponds to a channel. Bit 0 is channel 0, bit 1 is channel 1...
 * @return None
 * @details This function is used to stop EPWM generation immediately by clear
 * channel enable bit.
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32Condition The condition to trigger ADC. Combination of
                            ...
                            uint32_t u32Condition )
{
    ...
    {
        ( epwm )->EADCTS0 &= ~( ( EPWM_EADCTS0_TRGSEL0_Msk )
                                << ( u32ChannelNum << 3 ) );
        ( epwm )->EADCTS0 |= ( ( EPWM_EADCTS0_TRGEN0_Msk | u32Condition )
                               << ( u32ChannelNum << 3 ) );
    }
    else
    {
        ( epwm )->EADCTS1 &= ~( ( EPWM_EADCTS1_TRGSEL4_Msk )
                                << ( ( u32ChannelNum - 4UL ) << 3 ) );
        ( epwm )->EADCTS1 |= ( ( EPWM_EADCTS1_TRGEN4_Msk | u32Condition )
                               << ( ( u32ChannelNum - 4UL ) << 3 ) );
    }
}
...
void EPWM_DisableADCTrigger( EPWM_T * epwm, uint32_t u32ChannelNum )
{
    if( u32ChannelNum < 4UL )
    {
        ( epwm )->EADCTS0 &= ~( EPWM_EADCTS0_TRGEN0_Msk
                                << ( u32ChannelNum << 3 ) );
    }
    else
    {
        ( epwm )->EADCTS1 &= ~( EPWM_EADCTS1_TRGEN4_Msk
                                << ( ( u32ChannelNum - 4UL ) << 3 ) );
    }
}
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @retval 0 The specified channel trigger ADC to start of conversion flag is
 * not set
 * @retval 1 The specified channel trigger ADC to start of conversion flag is
 * set
 * @details This function is used to get EPWM trigger ADC to start of conversion
 * flag for specified channel.
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32Condition The condition to trigger DAC. Combination of
 * following conditions:
...
{
    ( epwm )->DACTRGEN &= ~( ( EPWM_TRIGGER_DAC_ZERO | EPWM_TRIGGER_DAC_PERIOD |
 ...
 * @param[in] u32Condition The condition to trigger DAC. This parameter is not
 * used
 * @return None
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. This parameter is not used
 * @retval 0 The specified channel trigger DAC to start of conversion flag is
 * not set
 * @retval 1 The specified channel trigger DAC to start of conversion flag is
 * set
 ...
 * @param[in] u32LevelMask Output high or low while fault brake occurs, each bit
 * represent the level of a channel while fault brake occurs. Bit 0 represents
 * channel 0, bit 1 represents channel 1...
 * @param[in] u32BrakeSource Fault brake source, could be one of following
 * source
 ...
 *          The write-protection function should be disabled before using this
 * function.
 */
                            ...
                            uint32_t u32BrakeSource )
{
    ...
    for( i = 0UL; i < EPWM_CHANNEL_NUM; i++ )
    {
        if( u32ChannelMask & ( 1UL << i ) )
        {
            if( ( u32BrakeSource == EPWM_FB_EDGE_SYS_CSS ) ||
                ( u32BrakeSource == EPWM_FB_EDGE_SYS_BOD ) ||
                ( u32BrakeSource == EPWM_FB_EDGE_SYS_RAM ) ||
                ( u32BrakeSource == EPWM_FB_EDGE_SYS_COR ) ||
                ( u32BrakeSource == EPWM_FB_LEVEL_SYS_CSS ) ||
                ( u32BrakeSource == EPWM_FB_LEVEL_SYS_BOD ) ||
                ( u32BrakeSource == EPWM_FB_LEVEL_SYS_RAM ) ||
                ( u32BrakeSource == EPWM_FB_LEVEL_SYS_COR ) )
            {
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit
 * corresponds to a channel. Set bit 0 to 1 enables channel 0 output, set bit 1
 * to 1 enables channel 1 output...
 * @return None
 * @details This function is used to enable EPWM output generation of selected
 * channel(s).
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelMask Combination of enabled channels. Each bit
 * corresponds to a channel Set bit 0 to 1 disables channel 0 output, set bit 1
 * to 1 disables channel 1 output...
 * @return None
 * @details This function is used to disable EPWM output generation of selected
 * channel(s).
 */
 ...
 * @param[in] u32RisingFirst The capture order is rising, falling first. Every
 * two channels share the same setting. Valid values are TRUE and FALSE.
 * @param[in] u32Mode Captured data transferred by PDMA interrupt type. It could
 * be either
 *              - \ref EPWM_CAPTURE_PDMA_RISING_LATCH
 *              - \ref EPWM_CAPTURE_PDMA_FALLING_LATCH
 *              - \ref EPWM_CAPTURE_PDMA_RISING_FALLING_LATCH
 * @return None
 * @details This function is used to enable PDMA transfer of selected channel(s)
 * for EPWM capture.
 * @note This function can only selects even or odd channel of pairs to do PDMA
 * transfer.
 */
                      ...
                      uint32_t u32Mode )
{
    ...
    ( epwm )->PDMACTL = ( ( epwm )->PDMACTL &
                          ~( ( EPWM_PDMACTL_CHSEL0_1_Msk |
                               EPWM_PDMACTL_CAPORD0_1_Msk |
                               EPWM_PDMACTL_CAPMOD0_1_Msk )
                             << ( ( u32ChannelNum >> 1 ) << 3 ) ) ) |
                        ( ( ( u32IsOddCh << EPWM_PDMACTL_CHSEL0_1_Pos ) |
                            ( u32RisingFirst << EPWM_PDMACTL_CAPORD0_1_Pos ) |
                            u32Mode | EPWM_PDMACTL_CHEN0_1_Msk )
                          << ( ( u32ChannelNum >> 1 ) << 3 ) );
}
...
{
    ( epwm )->PDMACTL &= ~( EPWM_PDMACTL_CHEN0_1_Msk
                            << ( ( u32ChannelNum >> 1 ) << 3 ) );
}
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32Duration Dead zone length in EPWM clock count, valid values are
 * between 0~0xFFF, but 0 means there is no Dead zone.
 * @return None
 ...
 * @return None
 * @details This function is used to enable capture interrupt of selected
 * channel.
 */
 ...
 *              - \ref EPWM_CAPTURE_INT_FALLING_LATCH
 * @return None
 * @details This function is used to disable capture interrupt of selected
 * channel.
 */
 ...
 *              - \ref EPWM_CAPTURE_INT_FALLING_LATCH
 * @return None
 * @details This function is used to clear capture interrupt of selected
 * channel.
 */
...
uint32_t EPWM_GetCaptureIntFlag( EPWM_T * epwm, uint32_t u32ChannelNum )
{
    ...
    u32CapIf = ( ( ( ( epwm )->CAPIF &
                     ( EPWM_CAPIF_CFLIF0_Msk << u32ChannelNum ) )
                       ? 1UL
                       : 0UL )
                 << 1 );
    u32CapIf |= ( ( ( epwm )->CAPIF &
                    ( EPWM_CAPIF_CRLIF0_Msk << u32ChannelNum ) )
                      ? 1UL
                      : 0UL );
    return u32CapIf;
}
...
{
    ( epwm )->INTEN0 &= ~(
        ( EPWM_DUTY_INT_DOWN_COUNT_MATCH_CMP | EPWM_DUTY_INT_UP_COUNT_MATCH_CMP )
        << u32ChannelNum );
}
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to clear duty interrupt flag of selected
 * channel.
 */
void EPWM_ClearDutyIntFlag( EPWM_T * epwm, uint32_t u32ChannelNum )
{
    ( epwm )->INTSTS0 = ( EPWM_INTSTS0_CMPUIF0_Msk | EPWM_INTSTS0_CMPDIF0_Msk )
                        << u32ChannelNum;
}
 ...
 * @details This function is used to get duty interrupt flag of selected
 * channel.
 */
uint32_t EPWM_GetDutyIntFlag( EPWM_T * epwm, uint32_t u32ChannelNum )
{
    return ( ( ( ( epwm )->INTSTS0 &
                 ( ( EPWM_INTSTS0_CMPDIF0_Msk | EPWM_INTSTS0_CMPUIF0_Msk )
                   << u32ChannelNum ) ) )
                 ? 1UL
                 : 0UL );
}
 ...
 *              - \ref EPWM_FB_LEVEL
 * @return None
 * @details This function is used to clear fault brake interrupt of selected
 * source. The write-protection function should be disabled before using this
 * function.
 */
 ...
 * @details This function is used to get fault brake interrupt flag of selected
 * source.
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32IntPeriodType Period interrupt type. This parameter is not
 * used.
 * @return None
 * @details This function is used to enable period interrupt of selected
 * channel.
 */
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to disable period interrupt of selected
 * channel.
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @param[in] u32IntFlagCnt Interrupt flag counter. Valid values are between
 * 0~65535.
 ...
 * @return None
 * @details This function is used to enable interrupt flag accumulator of
 * selected channel.
 */
...
{
    ( epwm )->IFA[ u32ChannelNum ] = ( ( ( epwm )->IFA[ u32ChannelNum ] &
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to Disable interrupt flag accumulator of
 * selected channel.
 */
 ...
 * @details This function is used to Get interrupt flag accumulator interrupt of
 * selected channel.
 */
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to enable accumulator interrupt trigger PDMA
 * of selected channel.
 */
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to disable accumulator interrupt trigger PDMA
 * of selected channel.
 */
 ...
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to clear free trigger duty interrupt flag of
 * selected channel.
 */
void EPWM_ClearFTDutyIntFlag( EPWM_T * epwm, uint32_t u32ChannelNum )
{
    ( epwm )->FTCI = ( ( EPWM_FTCI_FTCMU0_Msk | EPWM_FTCI_FTCMD0_Msk )
                       << ( u32ChannelNum >> 1 ) );
}
 ...
 * @details This function is used to get free trigger duty interrupt flag of
 * selected channel.
 */
 ...
 * @param[in] u32Direction Phase direction. Control EPWM counter count decrement
 * or increment  after synchronizing.
 *              - \ref EPWM_PHS_DIR_DECREMENT
 *              - \ref EPWM_PHS_DIR_INCREMENT
 * @param[in] u32StartPhase Synchronous start phase value. Valid values are
 * between 0~65535.
 * @return None
 * @details This function is used to configure synchronization phase of selected
 * channel.
 * @note Every two channels share the same setting.
 */
                           ...
                           uint32_t u32StartPhase )
{
    ...
    u32ChannelNum >>= 1;
    ( epwm )->SYNC = ( ( ( epwm )->SYNC &
                         ~( ( EPWM_SYNC_SINSRC0_Msk << ( u32ChannelNum << 1 ) ) |
                            ( EPWM_SYNC_PHSDIR0_Msk << u32ChannelNum ) ) ) |
                       ( u32Direction << EPWM_SYNC_PHSDIR0_Pos
                                      << u32ChannelNum ) |
                       ( u32SyncSrc << EPWM_SYNC_SINSRC0_Pos )
                           << ( u32ChannelNum << 1 ) );
 ...
 *                - EPWM0 : EPWM Group 0
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ClkCnt SYNC Edge Detector Filter Count. This controls the
 * counter number of edge detector. The valid value is 0~7.
...
{
    ( epwm )->SYNC = ( ( epwm )->SYNC &
                       ~( EPWM_SYNC_SFLTCNT_Msk | EPWM_SYNC_SFLTCSEL_Msk ) ) |
                     ( ( u32ClkCnt << EPWM_SYNC_SFLTCNT_Pos ) |
                       ( u32ClkDivSel << EPWM_SYNC_SFLTCSEL_Pos ) |
 ...
 *                - EPWM1 : EPWM Group 1
 * @return None
 * @details This function is used to enable EPWM SYNC input pin inverse
 * function.
 */
 ...
 *                - EPWM1 : EPWM Group 1
 * @return None
 * @details This function is used to Disable EPWM SYNC input pin inverse
 * function.
 */
 ...
 * @note Every two channels share the same setting.
 * @note If the clock source of EPWM counter is selected from TIMERn interrupt
 * events, the TRGEPWM(TIMERn_TRGCTL[1], n=0,1..3) bit must be set as 1.
 */
void EPWM_SetClockSource( EPWM_T * epwm,
                          uint32_t u32ChannelNum,
                          uint32_t u32ClkSrcSel )
{
    ( epwm )->CLKSRC = ( ( epwm )->CLKSRC &
 ...
 * @param[in] u32ClkCnt SYNC Edge Detector Filter Count. This controls the
 * counter number of edge detector
 * @param[in] u32ClkDivSel SYNC Edge Detector Filter Clock Selection.
 *              - \ref EPWM_NF_CLK_DIV_1
 *              - \ref EPWM_NF_CLK_DIV_2
 *              - \ref EPWM_NF_CLK_DIV_4
...
{
    ( epwm )->BNF = ( ( epwm )->BNF &
                      ~( ( EPWM_BNF_BRK0FCNT_Msk | EPWM_BNF_BRK0NFSEL_Msk )
                         << ( u32BrakePinNum << 3 ) ) ) |
                    ( ( ( u32ClkCnt << EPWM_BNF_BRK0FCNT_Pos ) |
                        ( u32ClkDivSel << EPWM_BNF_BRK0NFSEL_Pos ) |
...
void EPWM_DisableBrakePinInverse( EPWM_T * epwm, uint32_t u32BrakePinNum )
{
    ( epwm )->BNF &= ~(
        ...
        << ( u32BrakePinNum * ( uint32_t ) EPWM_BNF_BRK1NFEN_Pos ) );
}
 ...
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32BrakePinNum Brake pin selection. Valid values are 0 or 1.
 * @param[in] u32SelAnotherModule Select to another module. Valid values are
 * TRUE or FALSE.
 * @return None
...
{
    ( epwm )->BNF = ( ( epwm )->BNF &
                      ~( EPWM_BNF_BK0SRC_Msk << ( u32BrakePinNum << 3 ) ) ) |
                    ( u32SelAnotherModule
                      << ( ( uint32_t ) EPWM_BNF_BK0SRC_Pos +
                           ( u32BrakePinNum << 3 ) ) );
}
 ...
 * @param[in] u32BlankingCnt  Leading Edge Blanking Counter. Valid values are
 between 1~512. This counter value decides leading edge blanking window size,
 and this counter clock base is ECLK.
 * @param[in] u32BlankingEnable Enable EPWM leading edge blanking function.
 Valid values are TRUE (ENABLE) or FALSE (DISABLE).
 *              - \ref FALSE
 *              - \ref TRUE
 * @return None
 * @details This function is used to configure EPWM leading edge blanking
 function that blank the false trigger from ACMP brake source which may cause by
 EPWM output transition.
 * @note EPWM leading edge blanking function is only used for brake source from
 ACMP.
 */
...
{
    ( epwm )->LEBCTL = ( u32TrigType ) | ( u32TrigSrcSel ) |
                       ( u32BlankingEnable );
...
/**
 * @brief Get the time-base counter reached its maximum value flag of selected
 * channel
 * @param[in] epwm The pointer of the specified EPWM module
 *                - EPWM0 : EPWM Group 0
 *                - EPWM1 : EPWM Group 1
 ...
 * @details This function is used to get the time-base counter reached its
 * maximum value flag of selected channel.
 */
...
/**
 * @brief Clear the time-base counter reached its maximum value flag of selected
 * channel
 * @param[in] epwm The pointer of the specified EPWM module
 *                - EPWM0 : EPWM Group 0
 *                - EPWM1 : EPWM Group 1
 * @param[in] u32ChannelNum EPWM channel number. Valid values are between 0~5
 * @return None
 * @details This function is used to clear the time-base counter reached its
 * maximum value flag of selected channel.
 */
