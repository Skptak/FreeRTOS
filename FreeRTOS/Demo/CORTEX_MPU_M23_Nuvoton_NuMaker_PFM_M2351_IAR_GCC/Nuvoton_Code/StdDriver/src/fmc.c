 ...
 *
 * @param[in] u32addr   Starting flash address. It must be a page aligned
 * address.
 * @param[in] u32count  Byte count of flash to be calculated. It must be
 * multiple of 512 bytes.
 *
 * @retval   READ_ALLONE_YES       The contents of verified flash area are
 * 0xA11FFFFF.
 * @retval   READ_ALLONE_NOT       Some contents of verified flash area are not
 * 0xA1100000.
 * @retval   READ_ALLONE_CMD_FAIL  Unexpected error occurred.
 *
 * @details  Run ISP check all one command to check specify area is all one or
 * not.
 */
 ...
 * @return   None
 *
 * @details  This function will clear ISPEN bit of ISPCON to disable ISP
 * function
 *
 */
void FMC_Close( void )
 ...
 *
 * @details    Execute FMC_ISPCMD_PAGE_ERASE command to erase a flash page. The
 ...
 * @retval     -1  Erase failed
 *
 * @details Execute FMC_ISPCMD_BLOCK_ERASE command to erase a flash block. The
 * block size is 4 pages.
 */
 ...
 *           User need to read ISPSTA register to know if IAP mode supported or
 * not in relative boot.
 */
 ...
 * address.
 * @param[in] u32count  Byte count of flash to be calculated. It must be
 * multiple of 2048bytes.
 *
 * @return    Success or not.
 * @retval    0           Success.
 * @retval    0xFFFFFFFF  Invalid parameter.
 *
 * @details  Run ISP CRC32 checksum command to calculate checksum then get and
...
int32_t FMC_GetXOMState( uint32_t u32XomNum )
{
    ...
    if( ret >= 0 )
    {
        u32act = ( ( ( FMC->XOMSTS ) & 0xful ) & ( 1ul << u32XomNum ) ) >>
                 u32XomNum;
        ret = ( int32_t ) u32act;
    }
    return ret;
 ...
 *
 * @note     ISP functions are write-protected. user also needs to unlock it by
 * calling SYS_UnlockReg() before using all ISP functions.
 *
 */
void FMC_Open( void )
{
 ...
 * @param[out] u32data1  Place holder of word 0 read from flash address
 * u32addr+4.
 *
 * @return     0   Success
 * @return     -1  Failed
 *
 * @details    Execute FMC_ISPCMD_READ_64 command to read a double-word from
 * flash.
 */
 ...
 *
 * @param[out]  u32Config[]  The word buffer to store the User Configuration
 * data.
 ...
 * @retval      -1 Failed
 *
 * @details     This function is used to read the settings of user
 * configuration. if u32Count = 1, Only CONFIG0 will be returned to the buffer
 * specified by u32Config. if u32Count = 2, Both CONFIG0 and CONFIG1 will be
 * returned.
 */
 ...
 * @return    None
 *
 * @details   This function is used to switch APROM boot or LDROM boot. User
 * need to call FMC_SetBootSource to select boot source first, then use CPU
 * reset or System Reset Request to reset system.
 *
 */
 ...
 * @retval   -1    Forbidden. Times of key comparison mismatch reach the maximum
 * count.
 ...
 * @param[in] i32LockCONFIG   1: Security key lock CONFIG to write-protect. 0:
 * Don't lock CONFIG.
 * @param[in] i32LockSPROM    1: Security key lock SPROM to write-protect. 0:
 * Don't lock SPROM. (This param is not supported on M2351)
 *
 * @retval    0    Success.
 ...
 * @retval   -6    SPROM lock function failed. (This status is not supported on
 * M2351)
 * @retval   -7    KPMAX function failed.
 * @retval   -8    KEMAX function failed.
 *
 * @details  Set secure keys and setup key compare count. The secure key also
 * can protect user config.
 */
                      ...
                      const int32_t i32LockSPROM )
{
    ...
    if( ret == 0 )
    {
        ...
        else if( ( i32LockCONFIG &&
                   ( !( u32KeySts & FMC_KPKEYSTS_CFGFLAG_Msk ) ) ) ||
                 ( ( !i32LockCONFIG ) &&
                   ( u32KeySts & FMC_KPKEYSTS_CFGFLAG_Msk ) ) )
        {
            /* CONFIG lock failed! */
        ...
        else if( ( ( FMC->KPCNT & FMC_KPCNT_KPMAX_Msk ) >>
                   FMC_KPCNT_KPMAX_Pos ) != u32Kpmax )
        {
            /* KPMAX failed! */
            ret = -7;
        }
        else if( ( ( FMC->KPKEYCNT & FMC_KPKEYCNT_KPKEMAX_Msk ) >>
                   FMC_KPKEYCNT_KPKEMAX_Pos ) != u32Kemax )
        {
            /* KEMAX failed! */
            ret = -8;
        }
 ...
 * @param[in]  u32addr Address of the flash location to be programmed.
 *             It must be a double-word aligned address.
 * @param[in]  u32data0   The word data to be programmed to flash address
 * u32addr.
 * @param[in]  u32data1   The word data to be programmed to flash address
 * u32addr+4.
 *
 * @return     0   Success
 * @return     -1  Failed
 *
 * @ details   Execute ISP FMC_ISPCMD_PROGRAM_64 to program a double-word to
 * flash.
 */
 ...
 *
 * @param[in]  au32Config[] The word buffer to store the User Configuration
 * data.
 ...
 *           User Configuration is also be page erase. User needs to backup
 * necessary data before erase User Configuration.
 */
 ...
 *
 * @param[in]  u32Addr    Start flash address in APROM where the data chunk to
 * be programmed into. This address must be 8-bytes aligned to flash address.
                           ...
                           uint32_t u32Len )
{
    ...
    do
    {
        ...
        for( i = idx; i < ( FMC_MULTI_WORD_PROG_LEN / 4u );
             i += 4u ) /* Max data length is 256 bytes (512/4 words)*/
        {
            __set_PRIMASK( 1u ); /* Mask interrupt to avoid status check
                                    coherence error*/
            do
            {
            ...
            } while( ( FMC->MPSTS & ( 3u << FMC_MPSTS_D0_Pos ) ) &&
                     ( err == 0 ) );
            ...
            if( err == 0 )
            {
                ...
                } while( ( FMC->MPSTS & ( 3u << FMC_MPSTS_D2_Pos ) ) &&
                         ( err == 0 ) );
