 ...
 *         By default, the USCI_SPI transfer sequence is MSB first, the slave
 * selection signal is active low and the automatic slave select function is
 * disabled. In Slave mode, the u32BusClock must be NULL and the USCI_SPI clock
 *         divider setting will be 0.
 * @param[in]  uspi The pointer of the specified USCI_SPI module.
 * @param[in]  u32MasterSlave Decide the USCI_SPI module is operating in master
 * mode or in slave mode. Valid values are:
 *                    - \ref USPI_SLAVE
 *                    - \ref USPI_MASTER
                    ...
                    uint32_t u32BusClock )
{
    ...
    if( u32BusClock != 0UL )
    {
        u32ClkDiv = ( uint32_t ) ( ( ( ( ( ( u32Pclk / 2UL ) * 10UL ) /
                                         ( u32BusClock ) ) +
                                       5UL ) /
                                     10UL ) -
                                   1UL ); /* Compute proper divider for USCI_SPI
                                             clock */
    }
    ...
    uspi->PROTCTL &= ~( USPI_PROTCTL_SCLKMODE_Msk | USPI_PROTCTL_AUTOSS_Msk |
...
/**
 * @brief Enable the automatic slave select function. Only available in Master
 * mode.
 * @param[in]  uspi The pointer of the specified USCI_SPI module.
 * @param[in]  u32SSPinMask This parameter is not used.
 * @param[in]  u32ActiveLevel The active level of slave select signal. Valid
 * values are:
...
{
    uspi->LINECTL = ( uspi->LINECTL & ~USPI_LINECTL_CTLOINV_Msk ) |
                    u32ActiveLevel;
...
/**
 * @brief Get the actual frequency of USCI_SPI bus clock. Only available in
 * Master mode.
 * @param[in]  uspi The pointer of the specified USCI_SPI module.
...
uint32_t USPI_GetBusClock( USPI_T * uspi )
{
    ...
    u32ClkDiv = ( uspi->BRGEN & USPI_BRGEN_CLKDIV_Msk ) >>
 ...
 *         This parameter decides which interrupts will be enabled. Valid values
 * are:
 ...
 * @param[in]  u32Mask The combination of all related interrupt enable bits.
 *         Each bit corresponds to a interrupt bit.
 *         This parameter decides which interrupts will be disabled. Valid
 * values are:
 *           - \ref USPI_SSINACT_INT_MASK
 *           - \ref USPI_SSACT_INT_MASK
 *           - \ref USPI_SLVTO_INT_MASK
 ...
 *         This parameter decides which interrupt flags will be read. It is
 * combination of:
 *           - \ref USPI_SSINACT_INT_MASK
 *           - \ref USPI_SSACT_INT_MASK
 *           - \ref USPI_SLVTO_INT_MASK
...
uint32_t USPI_GetIntFlag( USPI_T * uspi, uint32_t u32Mask )
{
    ...
    if( ( u32Mask & USPI_SSINACT_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_SSINAIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_SSACT_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_SSACTIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_SLVTO_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_SLVTOIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_SLVBE_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_SLVBEIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_TXUDR_INT_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_TXUDRIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_RXOV_INT_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_RXOVIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_TXST_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_TXSTIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_TXEND_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_TXENDIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_RXST_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_RXSTIF_Msk ) )
    {
    ...
    if( ( u32Mask & USPI_RXEND_INT_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_RXENDIF_Msk ) )
    {
 ...
 * @param[in]  u32Mask The combination of all related interrupt sources.
 *         Each bit corresponds to a interrupt source.
 *         This parameter decides which interrupt flags will be cleared. It
 ...
 *         This parameter decides which flags will be read. It is combination
 * of:
 *           - \ref USPI_BUSY_MASK
...
uint32_t USPI_GetStatus( USPI_T * uspi, uint32_t u32Mask )
{
    ...
    /* Check busy status */
    if( ( u32Mask & USPI_BUSY_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_BUSY_Msk ) )
    {
        u32Flag |= USPI_BUSY_MASK;
    }

    /* Check RX empty flag */
    if( ( u32Mask & USPI_RX_EMPTY_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_RXEMPTY_Msk ) )
    {
        u32Flag |= USPI_RX_EMPTY_MASK;
    }

    /* Check RX full flag */
    if( ( u32Mask & USPI_RX_FULL_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_RXFULL_Msk ) )
    {
        u32Flag |= USPI_RX_FULL_MASK;
    }

    /* Check TX empty flag */
    if( ( u32Mask & USPI_TX_EMPTY_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_TXEMPTY_Msk ) )
    {
        u32Flag |= USPI_TX_EMPTY_MASK;
    }

    /* Check TX full flag */
    if( ( u32Mask & USPI_TX_FULL_MASK ) &&
        ( u32BufStatus & USPI_BUFSTS_TXFULL_Msk ) )
    {
        u32Flag |= USPI_TX_FULL_MASK;
    }

    /* Check USCI_SPI_SS line status */
    if( ( u32Mask & USPI_SSLINE_STS_MASK ) &&
        ( u32ProtStatus & USPI_PROTSTS_SSLINE_Msk ) )
    {
