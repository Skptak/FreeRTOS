...
#include <stdio.h>
    ...
    #else
        /* Insist on keeping widthprec, to avoid X propagation by benign code in
         * C-lib */
 ...
 * @param    None
 * @returns  None
 * @details  This function is called by Hardfault handler and check if it is
 * caused by __BKPT or not.
 *
 */
...
uint32_t ProcessHardFault( uint32_t lr, uint32_t msp, uint32_t psp )
{
    ...
    {
        /*
            If the instruction is 0xBEAB, it means it is caused by BKPT without
           ICE connected. We still return for output/input message to UART.
        */
    ...
    /* It is casued by hardfault (Not semihost). Just process the hard fault
     * here. */
...
void SendChar( int ch )
{
    ...
    if( g_buf_len + 1 >= sizeof( g_buf ) || ch == '\n' || ch == '\0' )
    {
        ...
        {
    #if( DEBUG_ENABLE_SEMIHOST == 2 ) // Re-direct to UART Debug Port only when
                                      // DEBUG_ENABLE_SEMIHOST=2
            int i;
...
char GetChar( void )
{
    ...
    while( SH_DoCommand( 0x101, 0, &nRet ) != 0 )
    {
        ...
        #if( DEBUG_ENABLE_SEMIHOST == 2 ) // Re-direct to UART Debug Port only
                                          // when DEBUG_ENABLE_SEMIHOST=2
 ...
 *
 * @details  Check UART RSR RX EMPTY or not to determine if any char input from
 * UART
 */
 ...
 *
 * @param[in]  ch       Character to be written. The character is passed as its
 * int promotion.
 * @param[in]  stream   Pointer to a FILE object that identifies the stream
 * where the character is to be written.
 *
 * @returns    If there are no errors, the same character that has been written
 * is returned. If an error occurs, EOF is returned and the error indicator is
 * set (see ferror).
 *
 * @details    Writes a character to the stream and advances the position
 * indicator.\n The character is written at the current position of the stream
 * as indicated \n by the internal position indicator, which is then advanced
 * one character.
 *
 * @note       The above descriptions are copied from
 ...
 *
 * @param[in]  stream   Pointer to a FILE object that identifies the stream on
 ...
 * @param[in]  stream   Pointer to a FILE object that identifies the stream.
 *
 * @returns    If the error indicator associated with the stream was set, the
 * function returns a nonzero value. Otherwise, it returns a zero value.
 *
 * @details    Checks if the error indicator associated with stream is set,
 * returning a value different from zero if it is. This indicator is generally
 * set by a previous operation on the stream that failed.
 *
 * @note       The above descriptions are copied from
