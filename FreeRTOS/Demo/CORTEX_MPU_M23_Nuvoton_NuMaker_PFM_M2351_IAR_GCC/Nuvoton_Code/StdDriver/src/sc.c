...
/* Below are variables used locally by SC driver and does not want to parse by
 * doxygen unless HIDDEN_SYMBOLS is defined */
/** @cond HIDDEN_SYMBOLS */
 ...
 *
 * @details    This function is used to check if specified smartcard slot is
 * presented.
 */
 ...
 * @return     None
 *
 * @details    This function reset both transmit and receive FIFO of specified
 * smartcard module.
 */
 ...
 * @return     None
 *
 * @details    This function disable specified smartcard module, and force all
 ...
 *
 * @param[in]  sc          The pointer of smartcard module.
 * @param[in]  u32CardDet  Card detect polarity, select the SC_CD pin state
 * which indicates card absent. Could be:
 *                         -\ref SC_PIN_STATE_HIGH
 *                         -\ref SC_PIN_STATE_LOW
 *                         -\ref SC_PIN_STATE_IGNORE, no card detect pin, always
 * assumes card present.
 * @param[in]  u32PWR      Power off polarity, select the SC_PWR pin state which
 * could set smartcard VCC to high level. Could be:
 *                         -\ref SC_PIN_STATE_HIGH
 *                         -\ref SC_PIN_STATE_LOW
 *
 * @return     None
 ...
 * @return     None
 *
 * @details    This function reset specified smartcard module to its default
 * state for activate smartcard.
 */
void SC_ResetReader( SC_T * sc )
{
    ...
    /* Set Rx trigger level to 1 character, longest card detect debounce period,
     * disable error retry (EMV ATR does not use error retry) */
    while( sc->CTL & SC_CTL_SYNC_Msk )
    {
        ;
    }
    sc->CTL &= ~( SC_CTL_RXTRGLV_Msk | SC_CTL_CDDBSEL_Msk | SC_CTL_TXRTY_Msk |
                  SC_CTL_TXRTYEN_Msk | SC_CTL_RXRTY_Msk | SC_CTL_RXRTYEN_Msk );
    while( sc->CTL & SC_CTL_SYNC_Msk )
    {
        ;
    }
    ...
    if( g_u32CardStateIgnore[ u32Intf ] ) /* Do not enable card detect interrupt
                                             if card present state ignore */
    {
        sc->INTEN = ( SC_INTEN_RDAIEN_Msk | SC_INTEN_TERRIEN_Msk |
                      SC_INTEN_TMR0IEN_Msk | SC_INTEN_TMR1IEN_Msk |
                      SC_INTEN_TMR2IEN_Msk | SC_INTEN_BGTIEN_Msk |
    ...
    else
    {
        sc->INTEN = ( SC_INTEN_RDAIEN_Msk | SC_INTEN_TERRIEN_Msk |
                      SC_INTEN_TMR0IEN_Msk | SC_INTEN_TMR1IEN_Msk |
                      SC_INTEN_TMR2IEN_Msk | SC_INTEN_BGTIEN_Msk |
                      SC_INTEN_ACERRIEN_Msk | SC_INTEN_CDIEN_Msk );
    }
 ...
 *
 * @param[in]  sc      The pointer of smartcard module.
 * @param[in]  u32BGT  Block guard time using ETU as unit, valid range are
 * between 1 ~ 32.
 *
 * @return     None
 *
 * @details    This function is used to configure block guard time (BGT) of
 * specified smartcard module.
 */
void SC_SetBlockGuardTime( SC_T * sc, uint32_t u32BGT )
{
    sc->CTL = ( sc->CTL & ~SC_CTL_BGT_Msk ) |
              ( ( u32BGT - 1UL ) << SC_CTL_BGT_Pos );
}
 ...
 *
 * @param[in]  sc      The pointer of smartcard module.
 * @param[in]  u32CGT  Character guard time using ETU as unit, valid range are
 * between 11 ~ 267.
 *
 * @return     None
 *
 ...
 * specified smartcard module.
 * @note       Before using this API, user should set the correct stop bit
 * length first.
 */
void SC_SetCharGuardTime( SC_T * sc, uint32_t u32CGT )
{
    /* CGT is "START bit" + "8-bits" + "Parity bit" + "STOP bit(s)" + "EGT
     * counts" */
 ...
 * @return     None
 *
 * @note       This function stop the timers within specified smartcard module,
 * \b not timer module.
 */
void SC_StopAllTimer( SC_T * sc )
{
    ...
    sc->ALTCTL &= ~( SC_ALTCTL_CNTEN0_Msk | SC_ALTCTL_CNTEN1_Msk |
 ...
 *                             - \ref SC_TMR_MODE_8
 *                             - \ref SC_TMR_MODE_F
 * @param[in]  u32ETUCount Timer timeout duration, ETU based. For timer 0, valid
 * range are between 1 ~ 0x1000000 ETUs. For timer 1 and timer 2, valid range
 * are between 1 ~ 0x100 ETUs.
 *
 * @return     None
 *
 * @note       This function start the timer within specified smartcard module,
 * \b not timer module.
 * @note       Depend on the timer operating mode, timer may not start counting
 * immediately and starts when condition match.
 */
 ...
 * @return     None
 *
 * @note       This function stop the timer within specified smartcard module,
 * \b not timer module.
 */
 ...
 *
 * @details    This function is used to get specified smartcard module clock
 * frequency in kHz.
 */
