 ...
 * @param[in]  u32CntClkSrc    PWM counter clock source, could be one of
 * following source
...
{
    ( timer )->PWMCLKSRC = ( ( timer )->PWMCLKSRC &
 ...
 * @param[in]  u32DutyCycle    Target generator duty cycle percentage. Valid
 * range are between 0~100. 10 means 10%, 20 means 20%...
 *
 * @return     Nearest frequency clock in nano second
 *
 * @details    This API is used to configure PWM output frequency and duty cycle
 * in up count type and auto-reload operation mode.
 * @note       This API is only available if Timer PWM counter clock source is
 * from TMRx_CLK.
 */
                                       ...
                                       uint32_t u32DutyCycle )
{
    ...
    /* Store return value here 'cos we're gonna change u32Prescaler & u32Period
     * to the real value to fill into register */
    u32TargetFreq = ( u32PWMClockFreq / u32Prescaler ) / u32Period;

    /* Set PWM to up count type */
    timer->PWMCTL = ( timer->PWMCTL & ~TIMER_PWMCTL_CNTTYPE_Msk ) |
                    ( TPWM_UP_COUNT << TIMER_PWMCTL_CNTTYPE_Pos );

    /* Set PWM to auto-reload mode */
    timer->PWMCTL = ( timer->PWMCTL & ~TIMER_PWMCTL_CNTMODE_Msk ) |
                    ( TPWM_AUTO_RELOAD_MODE << TIMER_PWMCTL_CNTMODE_Pos );
 ...
 * @param[in]  u32DTCount  Dead-Time duration in PWM clock count, valid values
 * are between 0x0~0xFFF, but 0x0 means there is no Dead-Time insertion.
 *
 * @return     None
 *
 * @details    This function is used to enable Dead-Time function and counter
 * source is the same as Timer PWM clock source.
 * @note       The register write-protection function should be disabled before
 * using this function.
 */
 ...
 *
 * @details    This function is used to enable Dead-Time function and counter
 * source is the Timer PWM clock source with prescale.
 * @note       The register write-protection function should be disabled before
 * using this function.
 */
void TPWM_EnableDeadTimeWithPrescale( TIMER_T * timer, uint32_t u32DTCount )
{
    timer->PWMDTCTL = TIMER_PWMDTCTL_DTCKSEL_Msk | TIMER_PWMDTCTL_DTEN_Msk |
                      u32DTCount;
}
 ...
 * @return     None
 *
 * @details    This function is used to enable PWM generator and start counter
 * counting.
 */
 ...
 * @return     None
 *
 * @details This function is used to disable PWM counter immediately by clear
 * CNTEN (TIMERx_PWMCTL[0]) bit.
 */
 ...
 * @param[in]  timer           The pointer of the specified Timer module. It
 * could be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32Condition    The condition to trigger ADC. It could be one of
 ...
 * @return     None
 *
 * @details    This function is used to enable specified counter compare event
 * to trigger ADC.
 */
 ...
 * @return     None
 *
 * @details    This function is used to disable counter compare event to trigger
 * ADC.
 */
 ...
 * @param[in]  timer           The pointer of the specified Timer module. It
 * could be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32CH0Level     PWMx_CH0 output level while fault brake event
 * occurs. Valid value is one of following setting
 *                                 - \ref TPWM_OUTPUT_TOGGLE
 *                                 - \ref TPWM_OUTPUT_NOTHING
 *                                 - \ref TPWM_OUTPUT_LOW
 *                                 - \ref TPWM_OUTPUT_HIGH
 * @param[in]  u32CH1Level     PWMx_CH1 output level while fault brake event
 * occurs. Valid value is one of following setting
 *                                 - \ref TPWM_OUTPUT_TOGGLE
 *                                 - \ref TPWM_OUTPUT_NOTHING
 *                                 - \ref TPWM_OUTPUT_LOW
 *                                 - \ref TPWM_OUTPUT_HIGH
 * @param[in]  u32BrakeSource  Fault brake source, combination of following
 * source
                            ...
                            uint32_t u32BrakeSource )
{
    ...
    timer->PWMBRKCTL = ( timer->PWMBRKCTL & ~( TIMER_PWMBRKCTL_BRKAEVEN_Msk |
                                               TIMER_PWMBRKCTL_BRKAODD_Msk ) ) |
                       ( u32BrakeSource & 0xFFFFUL ) |
                       ( u32CH0Level << TIMER_PWMBRKCTL_BRKAEVEN_Pos ) |
                       ( u32CH1Level << TIMER_PWMBRKCTL_BRKAODD_Pos );
}
 ...
 * @param[in]  timer           The pointer of the specified Timer module. It
 * could be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32IntSource    Interrupt source, could be one of following
 * source
 *                                 - \ref TPWM_BRAKE_EDGE
 ...
 *
 * @details    This function is used to indicate fault brake interrupt flag
 * occurred or not of selected source.
 */
 ...
 * @return     None
 *
 * @details    This function is used to clear fault brake interrupt flags of
 * selected source.
 * @note       The register write-protection function should be disabled before
 * using this function.
 */
 ...
 * @param[in]  timer       The pointer of the specified Timer module. It could
 * be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32LoadMode  Timer PWM counter loading mode, could be one of
 * following mode
...
void TPWM_SetLoadMode( TIMER_T * timer, uint32_t u32LoadMode )
{
    timer->PWMCTL = ( timer->PWMCTL &
                      ~( TIMER_PWMCTL_IMMLDEN_Msk | TIMER_PWMCTL_CTRLD_Msk ) ) |
                    u32LoadMode;
}
 ...
 * @param[in]  timer           The pointer of the specified Timer module. It
 * could be TIMER0, TIMER1, TIMER2, TIMER3.
 * @param[in]  u32BrakePinSrc  The external brake pin source, could be one of
 * following source
 *                                 - \ref TPWM_TM_BRAKE0
 *                                 - \ref TPWM_TM_BRAKE1
 *                                 - \ref TPWM_TM_BRAKE2
 *                                 - \ref TPWM_TM_BRAKE3
 * @param[in]  u32DebounceCnt  This value controls the real debounce sample
 * time. The target debounce sample time is (debounce sample clock period) *
 * (u32DebounceCnt).
 * @param[in]  u32ClkSrcSel    Brake pin detector debounce clock source, could
 * be one of following source
 ...
 * @return     None
 *
 * @details    This function is used to enable external brake pin detector noise
 * filter function.
 */
 ...
 * @return     None
 *
 * @details    This function is used to disable external brake pin detector
...
{
    timer->PWMBNF = ( ( ( timer )->PWMBNF & ~TIMER_PWMBNF_BKPINSRC_Msk ) |
                      ( u32BrakePinNum << TIMER_PWMBNF_BKPINSRC_Pos ) );
}
