...
#ifndef DEBUG_PORT
    #define DEBUG_PORT                                                      \
        UART0 /*!< Select Debug Port which is used for retarget.c to output \
                 debug message to UART */
#endif

/* Init ETM Interface Multi-function Pins */
#define ETM_INIT()                                               \
    {                                                            \
        SYS->GPC_MFPL &= ~( SYS_GPC_MFPL_PC0MFP_ETM_TRACE_Msk |  \
                            SYS_GPC_MFPL_PC1MFP_ETM_TRACE_Msk |  \
                            SYS_GPC_MFPL_PC2MFP_ETM_TRACE_Msk |  \
                            SYS_GPC_MFPL_PC3MFP_ETM_TRACE_Msk |  \
                            SYS_GPC_MFPL_PC4MFP_ETM_TRACE_Msk ); \
        SYS->GPC_MFPL |= SYS_GPC_MFPL_PC0MFP_ETM_TRACE_CLK |     \
                         SYS_GPC_MFPL_PC1MFP_ETM_TRACE_DATA0 |   \
                         SYS_GPC_MFPL_PC2MFP_ETM_TRACE_DATA1 |   \
                         SYS_GPC_MFPL_PC3MFP_ETM_TRACE_DATA2 |   \
                         SYS_GPC_MFPL_PC4MFP_ETM_TRACE_DATA3;    \
    }

/**
 *
 * @details    This is used to enable PLL to speed up booting at startup. Remove
 * it will cause system using default clock source (External crystal or
 * internal 22.1184MHz IRC). Enable this option will cause system booting in
 * 72MHz(By XTAL) or 71.8848MHz(By IRC22M) according to user configuration
 * setting in CONFIG0
 *
 */
...
#define __LIRC32 ( 32000UL )    /*!< Internal 32K RC Oscillator Frequency */
        ...
        #define __NONSECURE_ENTRY_WEAK \
            __attribute__( ( cmse_nonsecure_entry, weak ) )
...
extern uint32_t __PC( void ); /*!< Return the current program counter value */
     ...
     * @return     None
     *
     * @details    If the expression is false, an error message will be printed
     * out from debug port (UART0 or UART1).
     */
    #define ASSERT_PARAM( expr )                                 \
        {                                                        \
            if( !( expr ) )                                      \
            {                                                    \
                AssertError( ( uint8_t * ) __FILE__, __LINE__ ); \
            }                                                    \
        }
