...
{
    uint32_t masterClk;
    uint32_t u32RegisterRead; // Workaround variable for MISRA C rule
                              // conformance
    ...
    switch( ( FM3_CRG->SCM_CTL >> 5 ) & 0x07 )
    {
        ...
        case 2: /* PLL clock                      */
            // Workaround for preventing MISRA C:1998 Rule 46 (MISRA C:2004
            // Rule 12.2) violation:
            //   "Unordered accesses to a volatile location"
            u32RegisterRead = ( __CLKMO *
                                ( ( ( FM3_CRG->PLL_CTL2 ) & 0x1F ) + 1 ) );
            masterClk = ( u32RegisterRead /
                          ( ( ( FM3_CRG->PLL_CTL1 >> 4 ) & 0x0F ) + 1 ) );
            break;
...
{
    static uint32_t u32IoRegisterRead; // Workaround variable for MISRA C rule
                                       // conformance
    ...
    FM3_CRG->SWC_PSR = SWC_PSR_Val |
                       ( 1UL << 7 ); /* set SW Watchdog presacaler */
