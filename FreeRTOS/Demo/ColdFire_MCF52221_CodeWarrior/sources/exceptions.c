...
#define NO_PRINTF 1
...
#if NO_PRINTF
    #define VECTORDISPLAY( MESSAGE ) \
        asm { nop; }                 \
        ;
    #define VECTORDISPLAY2( MESSAGE, MESSAGE2 ) \
        asm { nop; }                            \
        ;
    #define VECTORDISPLAY3( MESSAGE, MESSAGE2, MESSAGE3 ) \
        asm { nop; }                                      \
        ;
#else
    #include <stdio.h>
    ...
    #define VECTORDISPLAY3( MESSAGE1, MESSAGE2, MESSAGE3 ) \
        printf( MESSAGE1, MESSAGE2, MESSAGE3 );
#endif
 ...
 * (SP += 0,1,2,3 bytes).
 *             31     28 27      26 25    18 17      16 15 0 4
 * +---------------------------------------+------------------------------------+
 *             | Format | FS[3..2] | Vector | FS[1..0] |                 SR | SP
 * -->  0
 * +---------------------------------------+------------------------------------+
 */
...
#define MCF5XXX_RD_SF_FS( PTR )                                 \
    ( ( ( *( ( unsigned short * ) ( PTR ) ) & 0x0C00 ) >> 8 ) | \
      ( *( ( unsigned short * ) ( PTR ) ) & 0x0003 ) )
...
void mcf_exception_handler( void * framepointer )
{
    ...
    volatile unsigned long stackFrameFormat = ( unsigned long )
        MCF5XXX_RD_SF_FORMAT( &stackFrameWord );
    volatile unsigned long stackFrameFS = ( unsigned long ) MCF5XXX_RD_SF_FS(
        &stackFrameWord );
    volatile unsigned long stackFrameVector = ( unsigned long )
        MCF5XXX_RD_SF_VECTOR( &stackFrameWord );
    ...
    switch( stackFrameFormat )
    {
        ...
        case 7:
            break;
        default:
            VECTORDISPLAY3( MCF5XXX_EXCEPTFMT,
                            "Illegal stack type",
                            stackFramePC );
            break;
    }
...
{
    link a6, #0 lea - 20( sp ), sp movem.l d0 - d2 / a0 - a1,
        ( sp ) lea 24( sp ),
        a0 /* A0 point to exception stack frame on the stack */
            jsr mcf_exception_handler movem.l( sp ),
        d0 - d2 / a0 - a1 lea 20( sp ), sp unlk a6 rte
}
#else
asm void asm_exception_handler( void )
{
    link a6, #0 lea - 20( sp ), sp movem.l d0 - d2 / a0 - a1,
        ( sp ) pea 24( sp ) /* push exception frame address */
        jsr mcf_exception_handler movem.l 4( sp ),
        d0 - d2 / a0 - a1 lea 24( sp ), sp unlk a6 rte
}
#endif
...
__declspec( vectortable ) vectorTableEntryType _vect[ 256 ] = {
    ...
    ( vectorTableEntryType ) __SP_AFTER_RESET, /*   0 (0x000) Initial supervisor
                                                  SP      */
...
{ /* Set VBR */
    move.l 4( SP ), D0 movec d0, VBR nop rts
}
