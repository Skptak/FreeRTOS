  ...
  Notes:
        1) Default entry point is _startup.
           ...
           . SP must be initialized to valid memory
             in case the memory it points to is not valid using MEMORY_INIT
  macro 2) __initialize_hardware is called. Here you can initialize memory and
  some peripherics at this point global variables are not initialized yet 3)
  After __initialize_hardware memory is setup; initialize SP to _SP_INIT and
  perform needed initialisations for the language (clear memory, data rom copy).
...
void _startup( void );
    ...
    /*
     * If SUPPORT_ROM_TO_RAM is set, _S_romp is used to define the copy to be
     * performed. If it is not set, there's a single block to copy, performed
     * directly without using the __S_romp structure, based on __DATA_RAM,
     * __DATA_ROM and
     * __DATA_END symbols.
     *
