/*
 * File:        uart_support.c
 * Purpose:     Implements UART basic support, Derivative Specific Interrupt
 * handler and need function needed for MSL Support (printf\cout to terminal),
 * defined in <UART.h>
 *
 * Notes:
 *
                ...
                unsigned long baudRate )
{
    ...
    MCF_PSC_PSCCSR( channel ) = ( 0 | MCF_PSC_PSCCSR_RCSEL_SYS_CLK |
    ...
    MCF_PSC_PSCMR( channel ) = ( 0
                                 ...
                                 | MCF_PSC_PSCMR_RXRTS
        #endif
                                 | MCF_PSC_PSCMR_PM_NONE | MCF_PSC_PSCMR_BC_8 );
    ...
    MCF_PSC_PSCCR( channel ) = ( 0 | MCF_PSC_PSCCR_RX_ENABLED |
...
void uart_putchar( int channel, char ch )
{
    ...
    *( ( uint8 * ) &MCF_PSC_PSCTB_8BIT( channel ) ) = ( uint8 ) ch;
            ...
            /********************************************************************/
            /*
             * 5407 derivative doesn't have macros to access URB/UTB by channel
             * number because they have different sizes for UART0 & UART1 But in
             * UART mode only 8 bits of UART1 URB/UTB is used, so define these
             * macros here if they doesn't defined before
             */
            #ifndef MCF_UART_URB
                #define MCF_UART_URB( x ) \
                    ( *( vuint8 * ) ( &__MBAR[ 0x1CC + ( ( x ) *0x40 ) ] ) )
            #endif /* MCF_UART_URB */

            #ifndef MCF_UART_UTB
                #define MCF_UART_UTB( x ) \
                    ( *( vuint8 * ) ( &__MBAR[ 0x1CC + ( ( x ) *0x40 ) ] ) )
            #endif /* MCF_UART_UTB */
                ...
                unsigned long baudRate )
{
     ...
     */
    MCF_UART_UMR( channel ) = ( 0 | MCF_UART_UMR_PM_NONE | MCF_UART_UMR_BC_8 );

    /*
     * No echo or loopback, 1 stop bit
     */
    MCF_UART_UMR( channel ) = ( 0 | MCF_UART_UMR_CM_NORMAL |
                                MCF_UART_UMR_SB_STOP_BITS_1 );

    /*
     * Set Rx and Tx baud by SYSTEM CLOCK
     */
    MCF_UART_UCSR( channel ) = ( 0 | MCF_UART_UCSR_RCS_SYS_CLK |
