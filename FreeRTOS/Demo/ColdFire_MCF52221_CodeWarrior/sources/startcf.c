...
/*
 *    Dummy routine for initializing C++.  This routine will get overloaded by
 * the C++ runtime.
 */
...
static void __copy_rom_sections_to_ram( void )
{
     ...
     */
    for( info = _S_romp;
         info->Source != 0L || info->Target != 0L || info->Size != 0;
         ++info )
        __copy_rom_section( ( char * ) info->Target,
                            ( char * ) info->Source,
                            info->Size );
}
...
asm void _startup( void )
{
    ...
    move.w #0x2700,
        sr
            ...
            /* Pre-init SP, in case memory for stack is not valid it should be
               setup using MEMORY_INIT before __initialize_hardware is called
            */
            lea __SP_AFTER_RESET,
        a7;
...
# 0
        ...
        a5
            ...
            lea _END_BSS,
        a0 lea _START_BSS, a1 suba.l a1, a0 move.l a0,
        d0
    ...
    /* copy all ROM sections to their RAM locations ... */
        ...
        lea _S_romp,
        a0 move.l a0,
        d0 beq __skip_rom_copy__ jsr __copy_rom_sections_to_ram
        ...
        jsr __call_static_initializers
            ...
            jsr __initialize_system
                ...
                /* call main(int, char **) */
                pea __argv clr.l -
            ( sp ) /* clearing a long is ok since it's caller cleanup */
            jsr main addq.l #8,
        sp
