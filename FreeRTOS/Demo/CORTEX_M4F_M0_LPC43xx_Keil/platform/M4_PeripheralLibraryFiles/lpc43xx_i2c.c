 ...
 */
    ...
    #define SFSI2C0_CONFIGURE_FASTPLUS_HIGHSPEED_MODE \
        ( 2 << 1 | 1 << 3 | 1 << 7 | 1 << 10 | 1 << 11 )
    ...
    if( i2cdat[ tmp ].dir == 0 )
    {
        switch( returnCode )
        {
            ...
            case I2C_I2STAT_M_TX_RESTART:
                 ...
                 */
                if( ( txrx_setup->tx_data != NULL ) &&
                    ( txrx_setup->tx_length != 0 ) )
                {
            ...
            case I2C_I2STAT_M_TX_DAT_ACK:
                ...
                {
                    I2Cx->DAT = *( uint8_t * ) ( txrx_setup->tx_data +
                                                 txrx_setup->tx_count );
                    txrx_setup->tx_count++;
                    I2Cx->CONCLR = I2C_I2CONCLR_SIC;
                }
                ...
                next_stage:
                        ...
                        ( txrx_setup->rx_data != NULL ) )
                    {
                        ...
                        // Don't need issue an repeat start, just goto send
                        // SLA+R
                        else
                        {
                            goto send_slar;
    ...
    else if( i2cdat[ tmp ].dir == 1 )
    {
        switch( returnCode )
        {
            ...
            case I2C_I2STAT_M_RX_RESTART:
                 ...
                 */
                if( ( txrx_setup->rx_data != NULL ) &&
                    ( txrx_setup->rx_length != 0 ) )
                {
                send_slar:
            ...
            retry:
                ...
                if( txrx_setup->retransmissions_count <
                    txrx_setup->retransmissions_max )
                {
                    // Clear tx count
...
void I2C_SlaveHandler( LPC_I2Cn_Type * I2Cx )
{
    ...
    switch( returnCode )
    {
        ...
        case I2C_I2STAT_S_RX_PRE_GENCALL_DAT_ACK:
            ...
            {
                *( uint8_t * ) ( txrx_setup->rx_data +
                                 txrx_setup->rx_count ) = ( uint8_t ) I2Cx->DAT;
                txrx_setup->rx_count++;
            }
            I2Cx->CONSET = I2C_I2CONSET_AA;
            I2Cx->CONCLR = I2C_I2CONCLR_SIC;
         ...
         * since there no proof lets us know when a stop signal has been
         * received on slave side.
         */
                               ...
                               I2C_TRANSFER_OPT_Type Opt )
{
    ...
    retry:
         ...
         * --------------------------------------------------- */
        if( ( TransferCfg->tx_length != 0 ) &&
            ( TransferCfg->tx_data != NULL ) )
        {
            /* Send slave address + WR direction bit = 0
             * ----------------------------------- */
            CodeStatus = I2C_SendByte( I2Cx,
                                       ( TransferCfg->sl_addr7bit << 1 ) );
            ...
            ( TransferCfg->rx_data != NULL ) )
        {
            ...
            while( TransferCfg->rx_count < TransferCfg->rx_length )
            {
                 ...
                 * issue an ACK signal on bus after reading to avoid of next
                 * data frame on slave side
                 */
            ...
            if( I2Cx->CONSET & I2C_I2CONSET_SI )
            {
                ...
                switch( CodeStatus = ( I2Cx->STAT & I2C_STAT_CODE_BITMASK ) )
                {
                    ...
                    case I2C_I2STAT_S_RX_PRE_GENCALL_DAT_ACK:
                         ...
                         * data length, just ignore them.
                         */
                        if( ( TransferCfg->rx_count <
                              TransferCfg->rx_length ) &&
                            ( TransferCfg->rx_data != NULL ) )
                        {
                    ...
                    /*
                     * Note that: Return code only let us know a stop condition
                     * mixed with a repeat start condition in the same code
                     * value. So we should provide a time-out. In case this is
                     * really a stop condition, this will return back after time
                     * out condition. Otherwise, next session that is slave
                     * receive data will be completed.
                     */
                    ...
                    case I2C_I2STAT_S_TX_DAT_ACK:
                         ...
                         * data length, just ignore them.
                         */
                        if( ( TransferCfg->tx_count <
                              TransferCfg->tx_length ) &&
                            ( TransferCfg->tx_data != NULL ) )
                        {
                            I2Cx->DAT = *txdat++;
                            TransferCfg->tx_count++;
                     ...
                     * that means there's no more data to send, exit now */
                    /*
                     * Note: Don't wait for stop event since in slave transmit
                     * mode, since there no proof lets us know when a stop
                     * signal has been received on slave side.
                     */
                    case I2C_I2STAT_S_TX_DAT_NACK:
                        I2Cx->CONSET = I2C_I2CONSET_AA;
                        I2Cx->CONCLR = I2C_I2CONCLR_SIC;
                          ...
                          I2C_OWNSLAVEADDR_CFG_Type * OwnSlaveAddrConfigStruct )
{
    ...
    tmp = ( ( ( uint32_t ) ( OwnSlaveAddrConfigStruct->SlaveAddr_7bit << 1 ) ) |
            ( ( OwnSlaveAddrConfigStruct->GeneralCallState == ENABLE )
                  ? 0x01
                  : 0x00 ) ) &
          I2C_I2ADR_BITMASK;
    switch( OwnSlaveAddrConfigStruct->SlaveAddrChannel )
    {
        case 0:
            I2Cx->ADR0 = tmp;
            I2Cx->MASK[ 0 ] = I2C_I2MASK_MASK(
                ( uint32_t ) ( OwnSlaveAddrConfigStruct->SlaveAddrMaskValue ) );
            break;
        case 1:
            I2Cx->ADR1 = tmp;
            I2Cx->MASK[ 1 ] = I2C_I2MASK_MASK(
                ( uint32_t ) ( OwnSlaveAddrConfigStruct->SlaveAddrMaskValue ) );
            break;
        case 2:
            I2Cx->ADR2 = tmp;
            I2Cx->MASK[ 2 ] = I2C_I2MASK_MASK(
                ( uint32_t ) ( OwnSlaveAddrConfigStruct->SlaveAddrMaskValue ) );
            break;
        case 3:
            I2Cx->ADR3 = tmp;
            I2Cx->MASK[ 3 ] = I2C_I2MASK_MASK(
                ( uint32_t ) ( OwnSlaveAddrConfigStruct->SlaveAddrMaskValue ) );
            break;
    }
}
