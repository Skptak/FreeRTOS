...
void SetClock( CLKBASE_Type target_clk, CLKSRC_Type src_clk, CLKDIV_Type div )
{
    ...
    switch( div )
    {
        ...
        case( DIV16 ):
            ...
            break;
        case( DIV256 ):
            LPC_CGU->IDIVE_CTRL = ( src_clk << 24 ) | ( 255 << 2 ) |
                                  AUTO_BLOCK; // MAX 128? IDIV bit 2:9 = 7 bits
                                              // = 127 max
    ...
    switch( target_clk )
    {
        case( BASE_OUT_CLK ):
        {
            LPC_SCU->SFSCLK_0 = 1; // function 1; CGU clk out, diable pull down,
                                   // disable pull-up
            auto_block = FALSE;
            break;
        }
        case( XTAL ):
        {
            XtalFrequency = ( uint32_t ) src_clk; // convert target clock
                                                  // directly to frequency
            break;
        }
        case( ENET_RX ):
        {
            EnetRxFrequency = ( uint32_t ) src_clk; // convert target clock
                                                    // directly to frequency
            break;
        }
        case( ENET_TX ):
        {
            EnetTxFrequency = ( uint32_t ) src_clk; // convert target clock
                                                    // directly to frequency
            break;
        }
        case( BASE_USB1_CLK ):
    ...
    if( target_clk < 200 )
    {
        target_clk_adr = ( uint32_t ) &LPC_CGU->IDIVA_CTRL +
                         ( target_clk - 2 ) * 4;
        *( uint32_t * ) target_clk_adr = ( src_clk << 24 ) |
                                         ( auto_block << 11 );
    }
}
...
void SetPL160M( CLKSRC_Type src_clk, uint32_t mult )
{
    ...
    else
    {
        ...
        //			__breakpoint(0);
        //		}
        LPC_CGU->PLL1_CTRL = ( src_clk << 24 ) | ( msel << 16 ) |
                             ( nsel << 12 ) | ( psel << 8 ) | FBSEL;
...
void SetPLLUSB( CLKSRC_Type src_clk, uint8_t enable )
{
    ...
    else
    {
        ...
        LPC_CGU->BASE_USB0_CLK = ( 0 << 0 ) | ( 1 << 11 ) |
                                 ( SRC_PL550M_0 << 24 );
...
{
    uint32_t i = 0;
    const uint32_t PlainEnable = ( 0x2 << 3 ); /* no pull up, no pull down
                                                  (plain) */

    if( src_clk == SRC_OSC32K )
    {
        LPC_CREG->CREG0 &= ~( ( 1 << 3 ) | ( 1 << 2 ) ); // Active mode of 32
                                                         // KHz osc and release
                                                         // reset
        LPC_CREG->CREG0 |= ( 1 << 1 ) |
                           ( 1 << 0 ); // Enable 32 kHz & 1 kHz on osc32k
    }
    if( src_clk == SRC_ENET_RX_CLK )
        scu_pinmux( 0xC, 0, PlainEnable, FUNC3 ); // enet_rx_clk on PC_0 func 3
    if( src_clk == SRC_ENET_TX_CLK )
        scu_pinmux( 0x1, 19, PlainEnable, FUNC0 ); // enet_tx_clk on P1_19 func
                                                   // 0
...
void DisableSourceClk( CLKSRC_Type src_clk )
{
    ...
    if( src_clk == SRC_OSC32K )
    {
        LPC_CREG->CREG0 &= ~( ( 1 << 1 ) | ( 1 << 0 ) ); // Disable 32 kHz & 1
                                                         // kHz on osc32k
        LPC_CREG->CREG0 |= ( ( 1 << 3 ) | ( 1 << 2 ) );  // osc32k in power down
                                                         // and in reset mode
    }
    if( src_clk == SRC_ENET_RX_CLK )
