...
typedef enum CLKSRC
{
    ...
    SRC_IDIV_3 = 15,
    SRC_IDIV_4 = 16,
    NOT_DEFINED = 0xFFFFFFF, // Force a signed int enum, so every possible
...
// PL550M
#define MODE1A \
    ( 0x3 << 2 ) // Normal operating mode without post-divider and without
                 // pre-divider
#define MODE1B \
    ( 0x2 << 2 ) // Normal operating mode with post-divider and without
                 // pre-divider
#define MODE1C \
    ( 0x1 << 2 ) // Normal operating mode without post-divider and with
                 // pre-divider
#define MODE1D \
    ( 0x0 << 2 ) // Normal operating mode with post-divider and with
                 // pre-divider.
...
#define FBSEL     ( 1 << 6 )
#define MSEL_FBDIV( n ) \
    ( n << 16 ) // MSEL = feedback-divider value	2*M (1 to 2^15)
#define NSEL_PREDIV( n ) \
    ( n << 12 ) // NSEL = pre-divider value			N	(1 to 2^8)
#define PSEL_POSTDIV( n ) \
    ( n << 8 ) // PSEL = post-divider value		P*2	(1 to 2^5)
