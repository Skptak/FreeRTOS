...
void EMC_Init( void )
{
    // The address/data pins for the memory interface are required for the
    // static and for dynamic memories
    EMC_Config_Pinmux();
    ...
    //      LPC_EMC->CONTROL = 0x00000001;   // Enable the external memory
    //      controller
    //	  LPC_EMC->CONFIG = 0;
    //	  // Buffers for the static memories are enabled as well. If there is
    // SDRAM as well,
    //	  // then this is done after the initialisation for the dynamic memory
    // interface.
    ...
    // Buffers for the static memories can now be enabled as well. In a system
    // with static and dynamic memory this should only been done after the SDRAM
    // initialisation --> here
    LPC_EMC->STATICCONFIG0 = 0x00080081;
}
...
{
    // Disable the external memory controller before changing pin control
    // configuration
    ...
    LPC_SCU_CLK( 0 ) = 0 +
                       EMC_IO; // EMC_CLK0 signal on pin CLK0 (needed for SDRAM)
...
void EMC_Config_Static( void )
{
    ...
    // @120MHz there should be 8 or 9 waitstates for the 70ns flash, apparently
    // it works with 7
    ...
    // @120MHz there should be 7 waitstates for the 55ns SRAM, it should work
    // with 6
...
void initEmiDelays( void )
{
    ...
    // CLK & CLKE0 delay
    *( uint32_t * ) ( LPC_SCU_BASE + 0xD00 ) = ( ( CLK0_DELAY << 16 ) |
                                                 ( CLKE0_DELAY << 0 ) );

    // EMCCTRLDELAY, address 0x4008 6D04
    *( uint32_t * ) ( LPC_SCU_BASE + 0xD04 ) = ( ( WE_DELAY << 12 ) |
                                                 ( CAS_DELAY << 4 ) |
                                                 ( RAS_DELAY << 0 ) );
    ...
    *( uint32_t * ) ( LPC_SCU_BASE +
                      0xD0C ) = ( ( DOUT_DELAY << 28 ) | ( DOUT_DELAY << 24 ) |
                                  ( DOUT_DELAY << 20 ) | ( DOUT_DELAY << 16 ) |
                                  ( DQM0_DELAY << 12 ) | ( DQM0_DELAY << 8 ) |
                                  ( DQM0_DELAY << 4 ) | ( DQM0_DELAY << 0 ) );

    // EMCFBCLKDELAY, address 0x4008 6D10
    *( uint32_t * ) ( LPC_SCU_BASE + 0xD10 ) = ( ( CCLK_DELAY << 16 ) |
                                                 ( FBCLK0_DELAY << 12 ) |
                                                 ( FBCLK0_DELAY << 8 ) |
                                                 ( FBCLK0_DELAY << 4 ) |
                                                 ( FBCLK0_DELAY << 0 ) );
    ...
    *( uint32_t * ) ( LPC_SCU_BASE +
                      0xD14 ) = ( ( ADDR_DELAY << 28 ) | ( ADDR_DELAY << 24 ) |
                                  ( ADDR_DELAY << 20 ) | ( ADDR_DELAY << 16 ) |
                                  ( ADDR_DELAY << 12 ) | ( ADDR_DELAY << 8 ) |
                                  ( ADDR_DELAY << 4 ) | ( ADDR_DELAY << 0 ) );
    *( uint32_t * ) ( LPC_SCU_BASE +
                      0xD18 ) = ( ( ADDR_DELAY << 28 ) | ( ADDR_DELAY << 24 ) |
                                  ( ADDR_DELAY << 20 ) | ( ADDR_DELAY << 16 ) |
                                  ( ADDR_DELAY << 12 ) | ( ADDR_DELAY << 8 ) |
                                  ( ADDR_DELAY << 4 ) | ( ADDR_DELAY << 0 ) );
    *( uint32_t * ) ( LPC_SCU_BASE +
                      0xD1C ) = ( ( ADDR_DELAY << 28 ) | ( ADDR_DELAY << 24 ) |
                                  ( ADDR_DELAY << 20 ) | ( ADDR_DELAY << 16 ) |
                                  ( ADDR_DELAY << 12 ) | ( ADDR_DELAY << 8 ) |
                                  ( ADDR_DELAY << 4 ) | ( ADDR_DELAY << 0 ) );
    ...
    *( uint32_t * ) ( LPC_SCU_BASE +
                      0xD24 ) = ( ( DEN_DELAY << 28 ) | ( DEN_DELAY << 24 ) |
                                  ( DEN_DELAY << 20 ) | ( DEN_DELAY << 16 ) |
                                  ( DIN_DELAY << 12 ) | ( DIN_DELAY << 8 ) |
                                  ( DIN_DELAY << 4 ) | ( DIN_DELAY << 0 ) );
}
                      ...
                      uint32_t u32ColAddrBits )
{
    ...
    LPC_EMC->DYNAMICCONFIG0 = ( ( u32Width << 7 ) | ( u32Size << 9 ) |
                                ( u32DataBus
                                  << 14 ) ); // Selects the configuration
                                             // information for dynamic memory
                                             // chip select 0.
    LPC_EMC->DYNAMICRASCAS0 = ( 2UL << 0 ) |
                              ( 2UL << 8 ); // Selects the RAS and CAS latencies
                                            // for dynamic memory chip select 0.
    LPC_EMC->DYNAMICREADCONFIG = EMC_COMMAND_DELAYED_STRATEGY; // Configures the
                                                               // dynamic memory
                                                               // read strategy.
    ...
    LPC_EMC->DYNAMICXSR = 5; // Selects the exit self-refresh to active command
                             // time
    LPC_EMC->DYNAMICRRD = 0; // Selects the active bank A to active bank B
                             // latency
    LPC_EMC->DYNAMICMRD = 0; // Selects the load mode register to active command
                             // time
