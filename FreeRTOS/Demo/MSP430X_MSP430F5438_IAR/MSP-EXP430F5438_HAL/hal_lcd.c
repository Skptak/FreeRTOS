...
unsigned char Read_Block_Address_Macro[] = {
    0x74, 0x00, 0x12, 0x77, 0x00, 0x00
};
unsigned char Draw_Block_Value_Macro[] = { 0x74, 0x00, 0x12, 0x76, 0xFF, 0xFF };
unsigned char Draw_Block_Address_Macro[] = {
    0x74, 0x00, 0x11, 0x76, 0x00, 0x00
};
...
void halLcdInit( void )
{
    ...
    LCD_SPI_DIR &= ~( LCD_MISO_PIN + LCD_MOSI_PIN ); // Pin direction controlled
                                                     // by module, Set both pins
                                                     // to input as default
...
void halLcdPixel( int x, int y, unsigned char GrayScale )
{
    ...
    if( ( x >= 0 ) && ( x < LCD_COL ) && ( y >= 0 ) && ( y < LCD_ROW ) )
    {
        ...
        Value = LCD_MEM[ ( y << 4 ) + y + ( x >> 3 ) ]; // y * 17 --> row. x>>3
                                                        // --> column
        ...
        for( j = 0; j < FONT_HEIGHT; j++ )
        {
            ...
            if( TextStyle & GRAYSCALE_TEXT )
            {
                if( TextStyle & INVERT_TEXT )
                    ...
                    BlockValue = GrayScale_fonts[ LookUpChar *
                                                      ( FONT_HEIGHT + 1 ) +
                                                  j ];
                else
                    BlockValue |= GrayScale_fonts[ LookUpChar *
...
{
    // Each line increments by 0x20
    halLcdSetAddress( ( y << 5 ) + ( x >> 3 ) ); // Narrow down to 8 possible
                                                 // pixels
