 ...
 *         : 29.01.2016 1.60     Fixed the initial setting process in the
 *R_LONGQ_Open function. Fixed a program according to the Renesas coding rules.
 *         : 01.06.2018 1.70     Added the comment to while statement.
 *         : 07.02.2019 1.80     Deleted the inline expansion of the
 *R_BYTEQ_GetVersion function.
 ******************************************************************************/
 ...
 * Return Value : BYTEQ_SUCCESS -
 *                    Successful; *p_cnt loaded with number of bytes available
 *in queue BYTEQ_ERR_NULL_PTR - hdl is NULL BYTEQ_ERR_INVALID_ARG - p_cnt is
 *NULL
 ******************************************************************************/
 ...
 *                dynamically at run time (BYTEQ_CFG_USE_HEAP_FOR_CTRL_BLKS set
 *to 1 in config.h), then that memory is free()d by this function. If the
 *control block was statically allocated at compile time
 *                (BYTEQ_CFG_USE_HEAP_FOR_CTRL_BLKS set to 0 in config.h), then
 *this function marks the control block as available for use by another buffer.
 *Nothing is done to the contents of the buffer referenced by this Handle.
 * Arguments    : hdl -
 *                    handle for queue
 * Return Value : BYTEQ_SUCCESS -
...
{
    uint32_t const version = ( BYTEQ_VERSION_MAJOR << 16 ) |
                             BYTEQ_VERSION_MINOR;
    return version;
}
