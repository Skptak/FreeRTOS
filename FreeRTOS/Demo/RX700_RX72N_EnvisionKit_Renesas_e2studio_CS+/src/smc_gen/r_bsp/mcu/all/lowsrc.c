...
/* When using the user startup program, disable the following code. */
    ...
    /* Do not include this file if stdio is disabled in r_bsp_config. */
            ...
            ***********************************************************************************************************************/
            ...
            #define BSP_PRV_O_CREAT \
                ( 0x0008 ) /* A file is created if it is not existed */
            #define BSP_PRV_O_TRUNC                                           \
                ( 0x0010 ) /* The file size is changed to 0 if it is existed. \
                            */
            #define BSP_PRV_O_APPEND                                       \
                ( 0x0020 ) /* The position is set for next reading/writing \
            ...
            #define BSP_PRV_FPATH_STDERR "C:\\stderr"
                ...
                #define BSP_PRV_MALLOC_SEM            \
                    ( 1 ) /* Semaphore No. for malloc \
                           */
                #define BSP_PRV_STRTOK_SEM            \
                    ( 2 ) /* Semaphore No. for strtok \
                           */
                #define BSP_PRV_FILE_TBL_SEM \
                    ( 3 ) /* Semaphore No. for fopen  */
                #define BSP_PRV_MBRLEN_SEM            \
                    ( 4 ) /* Semaphore No. for mbrlen \
                           */
                #define BSP_PRV_FPSWREG_SEM \
                    ( 5 ) /* Semaphore No. for FPSW register */
                #define BSP_PRV_FILES_SEM ( 6 ) /* Semaphore No. for _Files */
                #define BSP_PRV_SEMSIZE                                        \
                    ( 26 ) /* BSP_PRV_FILES_SEM + _nfiles (assumed _nfiles=20) \
                            */
...
char * env_list[] = {
    /* Array for environment variables(**environ) */
    "ENV1=temp01",
    "ENV2=temp02",
    "ENV9=end",
 ...
 *USES_SIMIO on Assembler Option. Arguments    : none Return Value : none
 ***********************************************************************************************************************/
...
void close_all( void )
{
    ...
    for( i = 0; i < _nfiles; i++ )
    {
        ...
        if( _Files[ i ]->_Mode &
            ( BSP_PRV_MOPENR | BSP_PRV_MOPENW | BSP_PRV_MOPENA ) )
        {
...
long open( const char * name, long mode, long flg )
{
    ...
    else if( 0 == strcmp( name, BSP_PRV_FPATH_STDOUT ) ) /* Standard Output
                                                            file? */
    {
...
long write( long fileno, const unsigned char * buf, long count )
{
     ...
     */
    if( ( flmod[ fileno ] & BSP_PRV_O_WRONLY ) ||
        ( flmod[ fileno ] & BSP_PRV_O_RDWR ) )
    {
        if( BSP_PRV_STDIN == fileno )
        {
            return -1; /* Standard Input */
        }
        else if( ( BSP_PRV_STDOUT == fileno ) ||
                 ( BSP_PRV_STDERR == fileno ) ) /* Standard Error/output */
        {
            /* WAIT_LOOP */
            for( i = count; i > 0; --i )
            {
...
long read( long fileno, unsigned char * buf, long count )
{
    ...
    /* Checking the file mode with the file number, each character is input and
     * stored the buffer */

    if( ( flmod[ fileno ] & BSP_PRV_MOPENR ) ||
        ( flmod[ fileno ] & BSP_PRV_O_RDWR ) )
    {
        /* WAIT_LOOP */
        for( i = count; i > 0; i-- )
