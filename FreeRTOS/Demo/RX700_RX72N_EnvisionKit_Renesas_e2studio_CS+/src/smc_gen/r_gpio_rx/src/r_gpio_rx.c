 ...
 *         :                    Modified R_GPIO_PinControl to accept
 *GPIO_CMD_DSCR_ENABLE/_DISABLE commands. : 01.10.2016 2.10    Added support for
 *RX65N :                    Modified R_GPIO_PinControl to accept
 *GPIO_CMD_DSCR2_ENABLE/_DISABLE commands. : 19.12.2016 2.20    Added support
 *for RX24U, RX24T(512KB) : 21.07.2017 2.30    Added support for RX65N-2M,
 *RX130-512KB. : 28.09.2018 2.40    Added support for RX66T. Update according to
 *GSCE Code Checker : 01.02.2019 2.50    Added support for RX72T, RX65N-64pin
 ...
 *style. : 30.12.2019 3.40    Added support RX72N, RX66N.
 ***********************************************************************************************************************/
...
void R_GPIO_PortWrite( gpio_port_t port, uint8_t value )
{
    ...
    /* PODR register addresses are incremental in memory starting with
     * PORT0.PODR. Even if a port is not available on this MCU, the address is
     * reserved. */
...
{
    /* PIDR register addresses are incremental in memory starting with
     * PORT0.PIDR. Even if a port is not available on this MCU, the address is
     * reserved. */
...
gpio_err_t R_GPIO_PinControl( gpio_port_pin_t pin, gpio_cmd_t cmd )
{
    ...
    switch( cmd )
    {
        ...
        {
    #if( GPIO_CFG_PARAM_CHECKING_ENABLE == 1 )
            if( false == gpio_pin_function_check( &g_gpio_dscr_support[ 0 ],
                                                  port_number,
                                                  pin_number ) )
            {
    ...
    #endif
            /* Get pin's address */
            addr = gpio_port_addr_get( GPIO_PRV_BASE_ADDR_DSCR,
                                       ( uint16_t ) pin );
        ...
        {
    #if( GPIO_CFG_PARAM_CHECKING_ENABLE == 1 )
            if( false == gpio_pin_function_check( &g_gpio_dscr2_support[ 0 ],
                                                  port_number,
                                                  pin_number ) )
            {
                err = GPIO_ERR_INVALID_MODE;
    ...
    #endif
            /* Get pin's address */
            addr = gpio_port_addr_get( GPIO_PRV_BASE_ADDR_DSCR2,
                                       ( uint16_t ) pin );
        ...
        {
            /* Get pin's address */
            addr = gpio_port_addr_get( GPIO_PRV_BASE_ADDR_MODE,
                                       ( uint16_t ) pin );
        ...
        {
#if( GPIO_CFG_PARAM_CHECKING_ENABLE == 1 )
            if( false == gpio_pin_function_check( &g_gpio_pull_up_support[ 0 ],
                                                  port_number,
                                                  pin_number ) )
            {
                err = GPIO_ERR_INVALID_MODE;
...
#endif
            /* Get pin's address */
            addr = gpio_port_addr_get( GPIO_PRV_BASE_ADDR_PULL_UP,
                                       ( uint16_t ) pin );
...
#if( GPIO_CFG_PARAM_CHECKING_ENABLE == 1 )
            if( false ==
                gpio_pin_function_check( &g_gpio_open_drain_n_support[ 0 ],
                                         port_number,
                                         pin_number ) )
            {
                err = GPIO_ERR_INVALID_MODE;
...
#if( GPIO_CFG_PARAM_CHECKING_ENABLE == 1 )
            if( false ==
                gpio_pin_function_check( &g_gpio_open_drain_p_support[ 0 ],
                                         port_number,
                                         pin_number ) )
            {
                err = GPIO_ERR_INVALID_MODE;
...
uint32_t R_GPIO_GetVersion( void )
{
    ...
    return ( ( ( ( uint32_t ) GPIO_RX_VERSION_MAJOR ) << 16 ) |
             ( uint32_t ) GPIO_RX_VERSION_MINOR );
} /* End of function R_GPIO_GetVersion */

/***********************************************************************************************************************
 * Function Name: gpio_port_addr_get
 * Description  : Get the address for a port register based on a base port
 *register address. Arguments    : base_addr - First port register of this type
 *(e.g. &PORT0.PODR.BYTE) index - Index off the base. (e.g. for PORT4 it would
 *be 0x0400) Return Value : Address of the register that was requested
 ***********************************************************************************************************************/
                                       ...
                                       uint16_t index )
{
    ...
    return (
        uint8_t volatile * ) ( ( ( ( uint32_t ) index >> 8 ) & 0x000000FFuL ) +
                               ( uint32_t ) base_addr );
...
void gpio_set_output_type( gpio_port_pin_t pin, gpio_pin_output_t out_type )
{
    ...
    /* 'pin' is multiplied by 2 because the ODR0 and ODR1 registers are
     * staggered. This means that PORT0.ODR0 and PORT1.ODR0 are separated by 2
     * bytes instead of 1 as with the other port registers. */
    addr = gpio_port_addr_get( GPIO_PRV_BASE_ADDR_OUT_TYPE,
                               ( uint16_t ) ( ( ( uint16_t ) pin ) * 2 ) );

    /* ODR bit fields are 2-bits a piece. This means bits 0-3 are in the 1st
     * byte (ODR0) and bits 4-7 are in the 2nd byte (ODR1).
     */
    if( pin_number > 3 )
    {
        /* Bit field is in ODR1. Increment address by 1 for ODR1 register for
         * this port. */
        addr += 1;

        /* Subtract 4 from pin number since pins 4-7 are stored in ODR1 which is
         * an 8-bit register. Multiple pin number by 2 since each pin is
