...
static void use_hfrosc( int div, int trim )
{
    ...
    PRCI_REG( PRCI_HFROSCCFG ) = ( ROSC_DIV( div ) | ROSC_TRIM( trim ) |
                                   ROSC_EN( 1 ) );
...
static void use_pll( int refsel, int bypass, int r, int f, int q )
{
    ...
    if( bypass )
    {
        ...
        PRCI_REG( PRCI_PLLDIV ) = ( PLL_FINAL_DIV_BY_1( 1 ) |
                                    PLL_FINAL_DIV( 0 ) );
    }
    else
    {
...
void _init()
{
    ...
    {                                     // if F extension is present
        write_csr( mstatus, MSTATUS_FS ); // allow FPU instructions without
                                          // trapping
